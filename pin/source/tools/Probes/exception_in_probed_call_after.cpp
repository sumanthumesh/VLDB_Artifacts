...
static VOID on_module_loading(IMG img, VOID* data)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        if (RTN_Valid(routine))
        {
            ...
            if (RTN_InsertCallProbed(routine, IPOINT_BEFORE, AFUNPTR(foo_before), IARG_PROTOTYPE, foo_proto,
                                     IARG_FUNCARG_ENTRYPOINT_VALUE, 6, IARG_FUNCARG_ENTRYPOINT_VALUE, 5,
                                     IARG_FUNCARG_ENTRYPOINT_VALUE, 4, IARG_FUNCARG_ENTRYPOINT_VALUE, 3,
                                     IARG_FUNCARG_ENTRYPOINT_VALUE, 2, IARG_FUNCARG_ENTRYPOINT_VALUE, 1,
                                     IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END))
            {
                ASSERTX(RTN_IsSafeForProbedInsertion(routine));
            }
            if (RTN_InsertCallProbed(routine, IPOINT_AFTER, AFUNPTR(foo_after), IARG_PROTOTYPE, foo_proto, IARG_RETURN_IP,
                                     IARG_END))
            {
                ASSERTX(RTN_IsSafeForProbedInsertion(routine));
            }
