...
static VOID on_module_loading(IMG img, VOID* data)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        if (RTN_Valid(routine))
        {
            ...
            if (RTN_InsertCallProbed(routine, IPOINT_BEFORE, AFUNPTR(foo2_before), IARG_PROTOTYPE, foo2_proto, IARG_END))
            {
                ASSERTX(RTN_IsSafeForProbedInsertion(routine));
            }
            if (RTN_InsertCallProbed(routine, IPOINT_AFTER, AFUNPTR(foo2_after), IARG_PROTOTYPE, foo2_proto, IARG_END))
            {
                ASSERTX(RTN_IsSafeForProbedInsertion(routine));
            }
        }
