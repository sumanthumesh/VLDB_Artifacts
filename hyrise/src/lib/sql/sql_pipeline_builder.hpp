 ...
 *
 * Minimal usage:
 *      SQLPipelineBuilder{"SELECT * FROM
 * t;"}.create_pipeline().get_result_table()
 *
 ...
 *
 * Favour this interface over calling the SQLPipeline[Statement] constructors
 * with their long parameter list. See SQLPipeline[Statement] doc for these
 * classes, in short SQLPipeline ist for queries with multiple statement,
...
class SQLPipelineBuilder final
{
  public:
    ...
    SQLPipelineBuilder &
    with_optimizer(const std::shared_ptr<Optimizer> &optimizer);
    SQLPipelineBuilder &with_transaction_context(
        const std::shared_ptr<TransactionContext> &transaction_context);
    SQLPipelineBuilder &
    with_pqp_cache(const std::shared_ptr<SQLPhysicalPlanCache> &pqp_cache);
    SQLPipelineBuilder &
    with_lqp_cache(const std::shared_ptr<SQLLogicalPlanCache> &lqp_cache);
