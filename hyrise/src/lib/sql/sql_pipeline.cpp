...
SQLPipeline::SQLPipeline(
    ...
    : pqp_cache(init_pqp_cache), lqp_cache(init_lqp_cache), _sql(sql),
      _transaction_context(transaction_context), _optimizer(optimizer)
{
    DebugAssert(!_transaction_context ||
                    _transaction_context->phase() == TransactionPhase::Active,
    ...
    DebugAssert(!_transaction_context ||
                ...
                "Auto-commit transactions are created internally and should "
                "not be passed in.");
    ...
    AssertInput(parse_result.isValid(),
                create_sql_parser_error_message(sql, parse_result));
    DebugAssert(parse_result.size() > 0, "Cannot create empty SQLPipeline.");

    _sql_pipeline_statements.reserve(parse_result.size());

    auto parsed_statements =
        std::vector<std::shared_ptr<hsql::SQLParserResult>>{};
    for (auto *statement : parse_result.releaseStatements())
    {
        parsed_statements.emplace_back(
            std::make_shared<hsql::SQLParserResult>(statement));
    }

    auto seen_altering_statement = false;

    // We want to split the (multi-) statement SQL string into the strings for
    // each statement. We can then use those statement strings to cache query
    // plans. The sql parser only offers us the length of the string, so we need
    ...
    for (auto &parsed_statement : parsed_statements)
    {
        ...
        {
        // Check if statement alters the structure of the database in a way that
        // following statements might depend upon.
        ...
        // Get the statement string from the original query string, so we can
        // pass it to the SQLPipelineStatement
        const auto statement_string_length = statement->stringLength;
        const auto statement_string = boost::trim_copy(
            sql.substr(sql_string_offset, statement_string_length));
        sql_string_offset += statement_string_length;

        auto pipeline_statement = std::make_shared<SQLPipelineStatement>(
            statement_string, std::move(parsed_statement), use_mvcc, optimizer,
            pqp_cache, lqp_cache);
        _sql_pipeline_statements.emplace_back(std::move(pipeline_statement));
    }

    // If we see at least one structure altering statement and we have more than
    // one statement, we require execution of a statement before the next one
    // can be translated (so the next statement sees the previous structural
    // changes).
...
const std::vector<std::shared_ptr<hsql::SQLParserResult>> &
SQLPipeline::get_parsed_sql_statements()
{
    ...
    for (auto &pipeline_statement : _sql_pipeline_statements)
    {
        _parsed_sql_statements.emplace_back(
            pipeline_statement->get_parsed_sql_statement());
    }

    return _parsed_sql_statements;
}

const std::vector<std::shared_ptr<AbstractLQPNode>> &
SQLPipeline::get_unoptimized_logical_plans()
{
    ...
    Assert(!_requires_execution ||
               _pipeline_status != SQLPipelineStatus::NotExecuted,
           "One or more SQL statement is dependent on the execution of a "
           "previous one. "
           "Cannot translate all statements without executing, i.e. calling "
           "get_result_table()");

    _unoptimized_logical_plans.reserve(statement_count());
    for (auto &pipeline_statement : _sql_pipeline_statements)
    {
        _unoptimized_logical_plans.emplace_back(
...
const std::vector<std::reference_wrapper<const SQLTranslationInfo>> &
SQLPipeline::get_sql_translation_infos()
{
    ...
    for (auto &pipeline_statement : _sql_pipeline_statements)
    {
        _sql_translation_infos.emplace_back(
            std::cref(pipeline_statement->get_sql_translation_info()));
    }
...
const std::vector<std::shared_ptr<AbstractLQPNode>> &
SQLPipeline::get_optimized_logical_plans()
{
    ...
    // The optimizer modifies the input LQP and requires exclusive ownership of
    // that LQP. This means that we need to clear _unoptimized_logical_plans.
    // This is not an issue as the unoptimized plans will no longer be needed.
    // Calls to get_unoptimized_logical_plans are still allowed (e.g., for
    // visualization), in which case the unoptimized plan will be recreated.
    // Note that this does not clear the unoptimized LQPs stored in the
    // SQLPipelineStatement - those are cleared as part of
    ...
    for (auto &pipeline_statement : _sql_pipeline_statements)
    {
        _optimized_logical_plans.emplace_back(
            pipeline_statement->get_optimized_logical_plan());
    }

    return _optimized_logical_plans;
}

const std::vector<std::shared_ptr<AbstractOperator>> &
SQLPipeline::get_physical_plans()
{
    ...
    Assert(!_requires_execution ||
           ...
           "One or more SQL statement is dependent on the execution of a "
           "previous one. "
           "Cannot compile all statements without executing, i.e. calling "
           "get_result_table()");
...
const std::vector<std::vector<std::shared_ptr<AbstractTask>>> &
SQLPipeline::get_tasks()
{
    if (!_tasks.empty())
    {
        return _tasks;
    ...
    Assert(!_requires_execution ||
           ...
           "One or more SQL statement is dependent on the execution of a "
           "previous one. "
           "Cannot generate tasks for all statements without executing, i.e. "
           "calling get_result_table()");
...
std::pair<SQLPipelineStatus, std::shared_ptr<const Table>>
SQLPipeline::get_result_table() &&
{
    // std::pair constructor will automatically convert the reference into a
    // copy
    return get_result_table();
}

std::pair<SQLPipelineStatus, const std::vector<std::shared_ptr<const Table>> &>
SQLPipeline::get_result_tables() &
{
    ...
    for (auto &pipeline_statement : _sql_pipeline_statements)
    {
        ...
        const auto &[statement_status, table] =
        ...
        if (statement_status == SQLPipelineStatus::Failure)
        {
            ...
            {
                // The pipeline was executed using a transaction context (i.e.,
                // no auto-commit after each statement). Previously returned
                // results are invalid.
        ...
        Assert(statement_status == SQLPipelineStatus::Success,
               "Unexpected pipeline status");

        _result_tables.emplace_back(table);

        const auto &new_transaction_context =
        ...
        {
            Assert(new_transaction_context->phase() ==
                       TransactionPhase::Committed,
                   "Auto-commit statements should always be committed at this "
                   "point");
        ...
        {
            // If a new transaction was started (BEGIN), allow the caller to
            // retrieve it so that it can be passed into the following
            // SQLPipeline
            _transaction_context = new_transaction_context;
        }
        else
        {
            // The previous, user-created transaction was sucessfully committed
            // or rolled back due to the user's request. Clear it so that the
            // next statement can either start a new transaction or run in
            // auto-commit mode
            Assert(new_transaction_context->phase() ==
                           TransactionPhase::Committed ||
                       new_transaction_context->phase() ==
                           TransactionPhase::RolledBackByUser,
                   "Invalid state for non-auto-commit transaction after "
                   "succesful statement");
            _transaction_context = nullptr;
        }
    }
...
std::pair<SQLPipelineStatus, std::vector<std::shared_ptr<const Table>>>
SQLPipeline::get_result_tables() &&
{
    // std::pair constructor will automatically convert the reference into a
    // copy
...
std::shared_ptr<SQLPipelineStatement>
SQLPipeline::failed_pipeline_statement() const
{
...
SQLPipelineMetrics &SQLPipeline::metrics()
{
    if (_metrics.statement_metrics.empty())
    {
        ...
        {
            _metrics.statement_metrics.emplace_back(
                pipeline_statement->metrics());
        }
    }
...
const std::vector<std::shared_ptr<SQLPipelineStatement>> &
SQLPipeline::_get_sql_pipeline_statements() const
{
    // Note that the execution of the pipeline sets the transaction_context
    // within the SQLPipelineStatement. If you call this method on an unexecuted
    // pipeline, you will not see the correct transaction context.
    Assert(_sql_pipeline_statements.size() == 1,
           "_get_sql_pipeline_statements helper should only be used for "
                         ...
                         const SQLPipelineMetrics &metrics)
{
    ...
    for (const auto &statement_metric : metrics.statement_metrics)
    {
        ...
        query_plan_cache_hits.emplace_back(
            statement_metric->query_plan_cache_hit);
    }

    const auto num_cache_hits = std::count(query_plan_cache_hits.begin(),
                                           query_plan_cache_hits.end(), true);
    ...
    stream << "LQP TRANSLATE: " << format_duration(total_lqp_translate_nanos)
           << ", ";
    stream << "EXECUTE: " << format_duration(total_execute_nanos)
           << " (wall time) | ";
    stream << "QUERY PLAN CACHE HITS: " << num_cache_hits << "/"
           << query_plan_cache_hits.size() << " statement(s)";
    stream << "]\n";
