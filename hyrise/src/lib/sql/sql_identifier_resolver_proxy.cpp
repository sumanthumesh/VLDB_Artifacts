...
SQLIdentifierResolverProxy::resolve_identifier_relaxed(
    const SQLIdentifier &identifier)
{
    ...
    if (expression)
    {
        ...
        return std::make_shared<CorrelatedParameterExpression>(parameter_id,
                                                               *expression);
    }
...
const ExpressionUnorderedMap<ParameterID> &
SQLIdentifierResolverProxy::accessed_expressions() const
{
