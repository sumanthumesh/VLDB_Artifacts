...
void SQLIdentifierResolver::add_column_name(
    const std::shared_ptr<AbstractExpression> &expression,
    const std::string &column_name)
{
    auto &entry = _find_or_create_expression_entry(expression);
    if (std::find(entry.column_names.begin(), entry.column_names.end(),
                  column_name) == entry.column_names.end())
    {
        // This cannot be implemented as a set because the column_names's order
        // would get lost.
        entry.column_names.emplace_back(column_name);
    }
}

void SQLIdentifierResolver::reset_column_names(
    const std::shared_ptr<AbstractExpression> &expression)
{
    auto entry_iter = std::find_if(
...
void SQLIdentifierResolver::set_table_name(
    const std::shared_ptr<AbstractExpression> &expression,
...
SQLIdentifierResolver::resolve_identifier_relaxed(
...
SQLIdentifierResolver::resolve_table_name(const std::string &table_name) const
{
...
SQLIdentifierContextEntry &
SQLIdentifierResolver::_find_or_create_expression_entry(
    const std::shared_ptr<AbstractExpression> &expression)
{
    auto entry_iter = std::find_if(
        _entries.begin(), _entries.end(),
    ...
    {
        const auto entry =
            SQLIdentifierContextEntry{expression, std::nullopt, {}};
