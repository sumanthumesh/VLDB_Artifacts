                                            ...
                                            const hsql::SQLParserResult &result)
{
    ...
    for (auto line_number = size_t{0}; line_number < sql_lines.size();
         ++line_number)
    {
        ...
        if (line_number == error_line)
        {
            ...
            auto num_tabs =
                std::count(line.begin(), line.begin() + error_column, '\t');
            ...
            error_msg << std::string(error_column - num_tabs, ' ') << color_red
