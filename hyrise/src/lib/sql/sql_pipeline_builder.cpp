...
SQLPipelineBuilder::SQLPipelineBuilder(const std::string &sql)
    : _sql(sql), _pqp_cache(Hyrise::get().default_pqp_cache),
      _lqp_cache(Hyrise::get().default_lqp_cache)
{
}
...
SQLPipelineBuilder &SQLPipelineBuilder::with_pqp_cache(
...
SQLPipelineBuilder &SQLPipelineBuilder::with_lqp_cache(
...
{
    auto optimizer =
        _optimizer ? _optimizer : Optimizer::create_default_optimizer();
    auto pipeline = SQLPipeline(_sql, _transaction_context, _use_mvcc,
                                optimizer, _pqp_cache, _lqp_cache);
    return pipeline;
}
