...
class SQLIdentifierResolver final
{
  public:
    /**
     * @{
     * Set/Update/Delete the column/table names of an expression. There can be
     * multiple column names referring to a single expression because a new
     * alias does not replace a former column name or alias.
     */
    void add_column_name(const std::shared_ptr<AbstractExpression> &expression,
                         const std::string &column_name);
    void
    reset_column_names(const std::shared_ptr<AbstractExpression> &expression);
    void set_table_name(const std::shared_ptr<AbstractExpression> &expression,
    ...
    resolve_identifier_relaxed(const SQLIdentifier &identifier) const;
    ...
    /**
     * @return   The column expressions of a table/subquery identified by @param
     * table_name.
     */
    std::vector<std::shared_ptr<AbstractExpression>>
    resolve_table_name(const std::string &table_name) const;
  ...
  private:
    SQLIdentifierContextEntry &_find_or_create_expression_entry(
