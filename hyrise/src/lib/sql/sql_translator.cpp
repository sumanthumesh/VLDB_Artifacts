...
const auto hsql_arithmetic_operators =
    std::unordered_map<hsql::OperatorType, ArithmeticOperator>{
...
const auto hsql_logical_operators =
    std::unordered_map<hsql::OperatorType, LogicalOperator>{
        {hsql::kOpAnd, LogicalOperator::And},
        {hsql::kOpOr, LogicalOperator::Or}};

const auto hsql_predicate_condition =
    std::unordered_map<hsql::OperatorType, PredicateCondition>{
...
const auto hsql_datetime_field =
    std::unordered_map<hsql::DatetimeField, DatetimeComponent>{
...
const auto order_type_to_sort_mode =
    std::unordered_map<hsql::OrderType, SortMode>{
        {hsql::kOrderAsc, SortMode::Ascending},
        {hsql::kOrderDesc, SortMode::Descending},
    };

const auto supported_hsql_data_types =
    std::unordered_map<hsql::DataType, DataType>{
        {hsql::DataType::INT, DataType::Int},
        {hsql::DataType::LONG, DataType::Long},
        {hsql::DataType::FLOAT, DataType::Float},
        {hsql::DataType::DOUBLE, DataType::Double},
        {hsql::DataType::TEXT, DataType::String},
        {hsql::DataType::BIGINT, DataType::Long}};

JoinMode translate_join_mode(const hsql::JoinType join_type)
{
    static const std::unordered_map<const hsql::JoinType, const JoinMode>
        join_type_to_mode = {
                               ...
                               const AbstractLQPNode &right_input)
{
    ...
    const auto *binary_predicate_expression =
        dynamic_cast<const BinaryPredicateExpression *>(&expression);
    ...
    const auto find_predicate_in_input = [&](const auto &input)
    {
        ...
        input.iterate_output_expressions(
            [&](const auto /*column_id*/, const auto &expression)
            {
    ...
    const auto [left_in_left, right_in_left] =
        find_predicate_in_input(left_input);
    const auto [left_in_right, right_in_right] =
        find_predicate_in_input(right_input);
    ...
    const std::vector<std::shared_ptr<AbstractExpression>> &expressions)
{
    ...
    {
        // The required expression is already available or does not need to be
        // computed (e.g., when it is a literal).
        if (!expression->requires_computation() ||
            node->find_column_id(*expression))
        {
            continue;
        }
    ...
    // If all requested expressions are available, no need to create a
    // Projection.
    ...
    projection_expressions.insert(projection_expressions.end(),
                                  output_expressions.cbegin(),
                                  output_expressions.cend());
...
{
    DebugAssert(MetaTableManager::is_meta_table_name(name),
                name + " is not a meta table name.");
    return name.substr(MetaTableManager::META_PREFIX.size());
}

std::shared_ptr<AbstractExpression>
inverse_predicate(const AbstractExpression &expression)
{
    /**
    ...
    case ExpressionType::Predicate:
    {
        ...
        {
            // If the argument is a predicate, just inverse it (e.g. NOT (a > b)
            // becomes b <= a)
            return std::make_shared<BinaryPredicateExpression>(
                inverse_predicate_condition(
                    binary_predicate_expression->predicate_condition),
                binary_predicate_expression->left_operand(),
                binary_predicate_expression->right_operand());
        }

        if (const auto *const is_null_expression =
                dynamic_cast<const IsNullExpression *>(&expression);
            is_null_expression)
        {
        ...
        if (const auto *const between_expression =
                dynamic_cast<const BetweenExpression *>(&expression);
            between_expression)
        {
            // a BETWEEN b AND c -> a < b OR a > c
            return or_(less_than_(between_expression->operand(),
                                  between_expression->lower_bound()),
                       greater_than_(between_expression->operand(),
                                     between_expression->upper_bound()));
        }

        const auto *in_expression =
            dynamic_cast<const InExpression *>(&expression);
        Assert(in_expression, "Expected InExpression");
        return std::make_shared<InExpression>(
            inverse_predicate_condition(in_expression->predicate_condition),
    ...
    {
        const auto *logical_expression =
            static_cast<const LogicalExpression *>(&expression);
        ...
        case LogicalOperator::Or:
            return and_(
                inverse_predicate(*logical_expression->left_operand()),
                inverse_predicate(*logical_expression->right_operand()));
        }
    }
    break;
    ...
    case ExpressionType::Exists:
    {
        const auto *exists_expression =
            static_cast<const ExistsExpression *>(&expression);
...
FrameBound translate_frame_bound(const hsql::FrameBound &hsql_frame_bound)
{
    ...
    return FrameBound{static_cast<uint64_t>(offset), bound_type,
                      hsql_frame_bound.unbounded};
}
...
SQLTranslator::SQLTranslator(const UseMvcc use_mvcc)
    : SQLTranslator(
          use_mvcc, nullptr, std::make_shared<ParameterIDAllocator>(),
...
SQLTranslator::translate_parser_result(const hsql::SQLParserResult &result)
{
    _cacheable = true;
    ...
    const auto &parameter_ids_of_value_placeholders =
        _parameter_id_allocator->value_placeholders();
    auto parameter_ids =
        std::vector<ParameterID>{parameter_ids_of_value_placeholders.size()};

    for (const auto &[value_placeholder_id, parameter_id] :
         parameter_ids_of_value_placeholders)
    {
...
SQLTranslator::SQLTranslator(
    ...
    const std::shared_ptr<SQLIdentifierResolverProxy>
        &external_sql_identifier_resolver_proxy,
    const std::shared_ptr<ParameterIDAllocator> &parameter_id_allocator,
    const std::unordered_map<std::string, std::shared_ptr<LQPView>>
        &with_descriptions,
    const std::shared_ptr<
        std::unordered_map<std::string, std::shared_ptr<Table>>> &meta_tables)
    : _use_mvcc(use_mvcc), _external_sql_identifier_resolver_proxy(
                               external_sql_identifier_resolver_proxy),
      _parameter_id_allocator(parameter_id_allocator),
      _with_descriptions(with_descriptions), _meta_tables(meta_tables)
{
}
...
SQLTranslator::_translate_statement(const hsql::SQLStatement &statement)
{
    switch (statement.type())
    {
    case hsql::kStmtSelect:
        return _translate_select_statement(
...
std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_select_statement(const hsql::SelectStatement &select)
{
    ...
    AssertInput(!select.selectList->empty(),
                "SELECT list needs to have entries");
    ...
    const auto select_list_elements =
        _translate_select_list(*select.selectList);

    // Translate WHERE.
    if (select.whereClause)
    {
        const auto where_expression =
            _translate_hsql_expr(*select.whereClause, _sql_identifier_resolver);
        _current_lqp =
            _translate_predicate_expression(where_expression, _current_lqp);
    }
    ...
    // Translate ORDER BY and DISTINCT. ORDER BY and LIMIT must be executed
    // after DISTINCT. Thus, we must ensure that all ORDER BY expressions are
    // part of the SELECT list if a DISTINCT result is required.
    const auto &inflated_select_list_expressions =
        unwrap_elements(_inflated_select_list_elements);
    _translate_distinct_order_by(select.order, inflated_select_list_expressions,
    ...
    if (!expressions_equal(_current_lqp->output_expressions(),
                           inflated_select_list_expressions))
    {
        _current_lqp = ProjectionNode::make(inflated_select_list_expressions,
                                            _current_lqp);
    }

    // 2. Check whether we need to create an AliasNode. This is the case
    // whenever an expression was assigned a
    //    column_name that is not its generated name.
    const auto need_alias_node = std::any_of(
        _inflated_select_list_elements.begin(),
        _inflated_select_list_elements.end(),
        [](const auto &element)
        {
            return std::any_of(element.identifiers.begin(),
                               element.identifiers.end(),
                               [&](const auto &identifier) {
                                   return identifier.column_name !=
                                          element.expression->as_column_name();
                               });
        });
    ...
    if (need_alias_node)
    {
        ...
        _current_lqp =
            AliasNode::make(unwrap_elements(_inflated_select_list_elements),
                            aliases, _current_lqp);
    }
    ...
    if (select.setOperations)
    {
        ...
        {
            // Currently, only the SQL translation of intersect and except is
            // implemented.
            AssertInput(set_operator->setType != hsql::kSetUnion,
                        "Union Operations are currently not supported");
            _translate_set_operation(*set_operator);

            // In addition to local ORDER BY and LIMIT clauses, the result of
            // the set operation(s) may have final clauses, too. Consider the
            // following example query (returns the first ten dates when
            // store_sales happened, except the days one of the five web_sales
            // with the highest price happened):
            ...
            //               LIMIT 10;
            // While ORDER BY sales_price DESC LIMIT 5 belongs to the subquery
            // and has to be executed locally, ORDER BY sold_date ASC LIMIT 10
            // refers to the intersection and must be executed on the result.
            _translate_distinct_order_by(set_operator->resultOrder,
                                         inflated_select_list_expressions,
                                         select.selectDistinct);
...
void SQLTranslator::_translate_hsql_with_description(
    hsql::WithDescription &desc)
{
    auto with_translator =
        SQLTranslator{_use_mvcc, nullptr, _parameter_id_allocator,
    ...
    for (auto column_id = ColumnID{0}; column_id < output_expression_count;
         ++column_id)
    {
        for (const auto &identifier :
             with_translator._inflated_select_list_elements[column_id]
                 .identifiers)
        {
    ...
    // A WITH description masks a preceding WITH description if their aliases
    // are identical
    AssertInput(!_with_descriptions.contains(desc.alias),
                "Invalid redeclaration of WITH alias.");
    _with_descriptions.emplace(desc.alias, lqp_view);
}

std::shared_ptr<AbstractExpression>
SQLTranslator::translate_hsql_expr(const hsql::Expr &hsql_expr,
                                   const UseMvcc use_mvcc)
{
    // Create an empty SQLIdentifier context - thus the expression cannot refer
    // to any external columns
    return SQLTranslator{use_mvcc, _meta_tables}._translate_hsql_expr(
        hsql_expr, std::make_shared<SQLIdentifierResolver>());
}

std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_insert(const hsql::InsertStatement &insert)
{
    ...
    else
    {
        AssertInput(Hyrise::get().storage_manager.has_table(table_name),
                    std::string{"Did not find a table with name "} +
                        table_name);
     ...
     *        - For `INSERT INTO <table> SELECT ...` this means evaluating the
     * select statement
     *        - For `INSERT INTO <table> VALUES ...` this means creating a one
     * row table with the VALUES
     */
    if (insert.type == hsql::kInsertSelect)
    {
        // `INSERT INTO newtable SELECT ... FROM oldtable WHERE condition`
        AssertInput(insert.select,
                    "INSERT INTO ... SELECT ...: No SELECT statement given");
    ...
    else
    {
        ...
        {
            output_expressions.emplace_back(
                _translate_hsql_expr(*value, _sql_identifier_resolver));
        }
     ...
     *    E.g., `SELECT INTO table (c, a) VALUES (1, 2)` becomes `SELECT INTO
     * table (a, b, c) VALUES (2, NULL, 1)
     */
    if (insert.columns)
    {
        // `INSERT INTO table_name (column1, column2, column3, ...) ...;`
        // Create a Projection that matches the specified columns with the
        // columns of `table_name`

        AssertInput(
            insert.columns->size() == output_expressions.size(),
            "INSERT: Target column count and number of input columns mismatch");

        auto expressions = std::vector<std::shared_ptr<AbstractExpression>>(
            target_table->column_count(), null_());
        auto source_column_id = ColumnID{0};
        for (const auto &column_name : *insert.columns)
        {
            // retrieve correct ColumnID from the target table
            const auto target_column_id =
                target_table->column_id_by_name(column_name);
            expressions[target_column_id] =
                output_expressions[source_column_id];
            ++source_column_id;
        }
    ...
    /**
     * 3. When inserting NULL literals (or not inserting into all columns), wrap
     * NULLs in `CAST(NULL AS <column_data_type>), since a temporary table with
     * the data to insert will be created and NULL is an invalid column data
     * type in Hyrise.
     */
    for (auto column_id = ColumnID{0}; column_id < target_table->column_count();
         ++column_id)
    {
        // Turn `expression` into `CAST(expression AS <column_data_type>)` if
        // expression is a NULL literal
        auto expression = output_expressions[column_id];
        if (const auto value_expression =
                std::dynamic_pointer_cast<ValueExpression>(expression);
            value_expression)
        {
            if (variant_is_null(value_expression->value))
            {
                output_expressions[column_id] =
                    cast_(null_(), target_table->column_data_type(column_id));
                insert_data_projection_required = true;
            }
        }
    }
    ...
    /**
     * 4. Perform type conversions if necessary so the types of the inserted
     * data exactly matches the table column types
     */
    for (auto column_id = ColumnID{0}; column_id < target_table->column_count();
         ++column_id)
    {
        // Always cast if the expression contains a placeholder, since we can't
        // know the actual data type of the expression until it is replaced.
        if (expression_contains_placeholder(output_expressions[column_id]) ||
            target_table->column_data_type(column_id) !=
                output_expressions[column_id]->data_type())
        {
            output_expressions[column_id] =
                cast_(output_expressions[column_id],
                      target_table->column_data_type(column_id));
        }
    }

    /**
     * 5. Project the data to insert ONLY if required, i.e. when column order
     * needed to be arranged or NULLs were wrapped in `CAST(NULL as
     * <data_type>)`
     */
    if (insert_data_projection_required)
    {
        insert_data_node =
            ProjectionNode::make(output_expressions, insert_data_node);
    }

    AssertInput(insert_data_node->output_expressions().size() ==
                    static_cast<size_t>(target_table->column_count()),
                "INSERT: Column count mismatch");

    if (is_meta_table)
    {
        return ChangeMetaTableNode::make(
            table_name, MetaTableChangeType::Insert, DummyTableNode::make(),
            insert_data_node);
    }

    /**
     * NOTE: DataType checking has to be done at runtime, as Query could still
     * contain Placeholder with unspecified type
     */
    return InsertNode::make(table_name, insert_data_node);
}

std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_delete(const hsql::DeleteStatement &delete_statement)
{
    const auto table_name = std::string{delete_statement.tableName};

    const auto sql_identifier_resolver =
        std::make_shared<SQLIdentifierResolver>();
    const bool is_meta_table = MetaTableManager::is_meta_table_name(table_name);
    ...
    if (is_meta_table)
    {
        data_to_delete_node = _translate_meta_table(delete_statement.tableName,
                                                    sql_identifier_resolver);
        AssertInput(
            Hyrise::get().meta_table_manager.can_delete_from(table_name),
            "Cannot delete from " + table_name);
    }
    else
    {
        data_to_delete_node = _translate_stored_table(
            delete_statement.tableName, sql_identifier_resolver);
        Assert(lqp_is_validated(data_to_delete_node),
               "DELETE expects rows to be deleted to have been validated");
    }

    if (delete_statement.expr)
    {
        const auto delete_where_expression = _translate_hsql_expr(
            *delete_statement.expr, sql_identifier_resolver);
        data_to_delete_node = _translate_predicate_expression(
            delete_where_expression, data_to_delete_node);
    }

    if (is_meta_table)
    {
        return ChangeMetaTableNode::make(
            table_name, MetaTableChangeType::Delete, data_to_delete_node,
            DummyTableNode::make());
    }
...
std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_update(const hsql::UpdateStatement &update)
{
    AssertInput(update.table->type == hsql::kTableName,
                "UPDATE can only reference table by name");
    ...
    // The update operator wants ReferenceSegments on its left side. Also, we
    // should make sure that we do not update invalid rows.
    Assert(is_meta_table || lqp_is_validated(selection_lqp),
           "UPDATE expects rows to be updated to have been validated");

    if (update.where)
    {
        const auto where_expression = _translate_hsql_expr(
            *update.where, translation_state.sql_identifier_resolver);
        selection_lqp =
            _translate_predicate_expression(where_expression, selection_lqp);
    }
    ...
    for (const auto *update_clause : *update.updates)
    {
        ...
        const auto column_expression =
            translation_state.sql_identifier_resolver
                ->resolve_identifier_relaxed(column_name);
        const auto column_id = selection_lqp->get_column_id(*column_expression);

        update_expressions[column_id] = _translate_hsql_expr(
            *update_clause->value, translation_state.sql_identifier_resolver);
    }

    // Perform type conversions if necessary so the types of the inserted data
    // exactly matches the table column types
    for (auto column_id = ColumnID{0}; column_id < target_table->column_count();
         ++column_id)
    {
        // Always cast if the expression contains a placeholder, since we can't
        ...
        if (expression_contains_placeholder(update_expressions[column_id]) ||
            target_table->column_data_type(column_id) !=
                update_expressions[column_id]->data_type())
        {
            update_expressions[column_id] =
                cast_(update_expressions[column_id],
                      target_table->column_data_type(column_id));
        }
    }

    // LQP that computes the updated values
    const auto updated_values_lqp =
        ProjectionNode::make(update_expressions, selection_lqp);

    if (is_meta_table)
    {
        return ChangeMetaTableNode::make(table_name,
                                         MetaTableChangeType::Update,
                                         selection_lqp, updated_values_lqp);
    }
    return UpdateNode::make(table_name, selection_lqp, updated_values_lqp);
}

SQLTranslator::TableSourceState
SQLTranslator::_translate_table_ref(const hsql::TableRef &hsql_table_ref)
{
...
SQLTranslator::TableSourceState
SQLTranslator::_translate_table_origin(const hsql::TableRef &hsql_table_ref)
{
    auto lqp = std::shared_ptr<AbstractLQPNode>{};

    // Each element in the FROM list needs to have a unique table name (i.e.
    // Subqueries are required to have an ALIAS)
    ...
    case hsql::kTableName:
    {
        // WITH descriptions or subqueries are treated as though they were
        // inline views or tables They mask existing tables or views with the
        // same name.
        const auto with_descriptions_iter =
            _with_descriptions.find(hsql_table_ref.name);
        if (with_descriptions_iter != _with_descriptions.end())
        {
            ...
            for (auto column_id = ColumnID{0};
                 column_id < output_expression_count; ++column_id)
            {
                const auto &expression = output_expressions[column_id];

                const auto column_name_iter =
                    lqp_view->column_names.find(column_id);
                if (column_name_iter != lqp_view->column_names.end())
                {
                    sql_identifier_resolver->add_column_name(
                        expression, column_name_iter->second);
                }
                sql_identifier_resolver->set_table_name(expression,
                                                        hsql_table_ref.name);
            }
        }
        else if (Hyrise::get().storage_manager.has_table(hsql_table_ref.name))
        {
            lqp = _translate_stored_table(hsql_table_ref.name,
                                          sql_identifier_resolver);
        }
        else if (MetaTableManager::is_meta_table_name(hsql_table_ref.name))
        {
            lqp = _translate_meta_table(hsql_table_ref.name,
                                        sql_identifier_resolver);
        }
        else if (Hyrise::get().storage_manager.has_view(hsql_table_ref.name))
        {
            const auto view =
                Hyrise::get().storage_manager.get_view(hsql_table_ref.name);
            lqp = view->lqp;
                 ...
                 column_id < output_expression_count; ++column_id)
            {
                ...
                const auto column_name_iter =
                    view->column_names.find(column_id);
            ...
            AssertInput(
                _use_mvcc ==
                    (lqp_is_validated(view->lqp) ? UseMvcc::Yes : UseMvcc::No),
                "Mismatch between validation of View and query it is used in");
        }
        else
        {
            FailInput(std::string("Did not find a table or view with name ") +
                      hsql_table_ref.name);
        }
        table_name = hsql_table_ref.alias ? hsql_table_ref.alias->name
                                          : hsql_table_ref.name;

        for (const auto &expression : lqp->output_expressions())
        {
            const auto identifiers =
                sql_identifier_resolver->get_expression_identifiers(expression);
    ...
    case hsql::kTableSelect:
    {
        AssertInput(hsql_table_ref.alias && hsql_table_ref.alias->name,
                    "Every nested SELECT must have its own alias");
        table_name = hsql_table_ref.alias->name;

        auto subquery_translator = SQLTranslator{
            _use_mvcc, _external_sql_identifier_resolver_proxy,
            _parameter_id_allocator, _with_descriptions, _meta_tables};
        lqp = subquery_translator._translate_select_statement(
            *hsql_table_ref.select);

        // If this statement or any of the subquery's statements is not
        // cacheable (because of meta tables), this statement should not be
        // cacheable.
        _cacheable &= subquery_translator._cacheable;

        auto identifiers = std::vector<std::vector<SQLIdentifier>>{};
        identifiers.reserve(
            subquery_translator._inflated_select_list_elements.size());
        for (const auto &element :
             subquery_translator._inflated_select_list_elements)
        {
        ...
        const auto output_expression_count = output_expressions.size();
        Assert(
            identifiers.size() == output_expression_count,
            "There have to be as many identifier lists as output expressions");
        for (auto select_list_element_idx = size_t{0};
             select_list_element_idx < output_expression_count;
             ++select_list_element_idx)
        {
            const auto &subquery_expression =
                output_expressions[select_list_element_idx];

            // Make sure each column from the Subquery has a name
            if (identifiers.empty())
            {
                sql_identifier_resolver->add_column_name(
                    subquery_expression, subquery_expression->as_column_name());
            }
            for (const auto &identifier : identifiers[select_list_element_idx])
            {
                sql_identifier_resolver->add_column_name(
                    subquery_expression, identifier.column_name);
            }

            select_list_elements.emplace_back(
                subquery_expression, identifiers[select_list_element_idx]);
        }
    ...
    if (hsql_table_ref.alias && hsql_table_ref.alias->columns)
    {
        ...
        const auto table_ref_alias_column_count =
            hsql_table_ref.alias->columns->size();
        ...
        for (auto column_id = ColumnID{0};
             column_id < table_ref_alias_column_count; ++column_id)
        {
            const auto &expression = output_expressions[column_id];

            if (renamed_expressions.find(expression) ==
                renamed_expressions.end())
            {
                // The original column names should not be accessible anymore
                // because the table schema is renamed.
            ...
            select_list_elements[column_id].identifiers.emplace_back(
                column_name);
        }
    }
...
{
    AssertInput(Hyrise::get().storage_manager.has_table(name),
         ...
         ++column_id)
    {
        ...
        const auto column_expression =
            lqp_column_(stored_table_node, column_id);
        sql_identifier_resolver->add_column_name(column_expression,
    ...
    const std::shared_ptr<SQLIdentifierResolver> &sql_identifier_resolver)
{
    ...
    // MetaTables are non-cacheable because they might contain information about
    // the general system state that can change at any time
    _cacheable = false;

    const auto meta_table_name = trim_meta_table_name(name);

    // Meta tables are integrated in the LQP as static table nodes in order to
    // avoid regeneration at every access in the pipeline afterwards.
    ...
    {
        meta_table =
            Hyrise::get().meta_table_manager.generate_table(meta_table_name);
    ...
    // Publish the columns of the table in the SQLIdentifierResolver
    for (auto column_id = ColumnID{0}; column_id < meta_table->column_count();
         ++column_id)
    {
        const auto &column_definition =
            meta_table->column_definitions()[column_id];
        const auto column_expression =
            lqp_column_(static_table_node, column_id);
        sql_identifier_resolver->add_column_name(column_expression,
                                                 column_definition.name);
        sql_identifier_resolver->set_table_name(column_expression, name);
    }
...
SQLTranslator::TableSourceState
SQLTranslator::_translate_predicated_join(const hsql::JoinDefinition &join)
{
     ...
     * The current implementation expects a single join condition in a set of
     * conjunctive clauses. The remaining clauses are expected to be relevant
     * for only one of the join partners and are therefore converted into
     * predicates inserted in between the source relations and the actual join
     * node. See TPC-H 13 for an example query.
     */
    const auto raw_join_predicate = _translate_hsql_expr(
        *join.condition, result_state.sql_identifier_resolver);
    const auto raw_join_predicate_cnf =
        flatten_logical_expressions(raw_join_predicate, LogicalOperator::And);

    auto left_local_predicates =
        std::vector<std::shared_ptr<AbstractExpression>>{};
    auto right_local_predicates =
        std::vector<std::shared_ptr<AbstractExpression>>{};
    ...
    for (const auto &predicate : raw_join_predicate_cnf)
    {
        ...
        {
            // Accept any kind of predicate here and let the LQPTranslator fail
            // on those that it doesn't support
    ...
    AssertInput(
        join_mode != JoinMode::FullOuter ||
            (left_local_predicates.empty() && right_local_predicates.empty()),
    ...
    AssertInput(join_mode != JoinMode::Left || left_local_predicates.empty(),
                "Local predicates not supported on left side of left outer "
                "join. See #1436");
    AssertInput(join_mode != JoinMode::Right || right_local_predicates.empty(),
                "Local predicates not supported on right side of right outer "
                "join. See #1436");

    /**
     * Add local predicates - ignore local predicates on the preserving side of
     * OUTER JOINs
     */
    if (join_mode != JoinMode::Left && join_mode != JoinMode::FullOuter)
    {
        ...
        {
            left_input_lqp = _translate_predicate_expression(
                left_local_predicate, left_input_lqp);
        }
    }
    if (join_mode != JoinMode::Right && join_mode != JoinMode::FullOuter)
    {
        ...
        {
            right_input_lqp = _translate_predicate_expression(
                right_local_predicate, right_input_lqp);
        }
    }
    ...
    {
        lqp = JoinNode::make(join_mode, join_predicates, left_input_lqp,
                             right_input_lqp);
    }
    else
    {
        const auto join_predicate_iter =
            std::find_if(join_predicates.begin(), join_predicates.end(),
                         [&](const auto &join_predicate)
                         {
                             return is_trivial_join_predicate(*join_predicate,
                                                              *left_input_lqp,
                                                              *right_input_lqp);
                         });

        // Inner Joins with predicates like `5 + t0.a = 6+ t1.b` can be
        // supported via Cross join + Scan. For all other join modes such
        // predicates are not supported.
        AssertInput(join_mode == JoinMode::Inner ||
                        join_predicate_iter != join_predicates.end(),
                    "Non column-to-column comparison in join predicate only "
                    "supported for inner joins");

        if (join_predicate_iter == join_predicates.end())
        {
            lqp = JoinNode::make(JoinMode::Cross, left_input_lqp,
                                 right_input_lqp);
        }
        else
        {
            lqp = JoinNode::make(join_mode, *join_predicate_iter,
                                 left_input_lqp, right_input_lqp);
        ...
        {
            PerformanceWarning(
                "Secondary Join Predicates added as normal Predicates");
...
SQLTranslator::TableSourceState
SQLTranslator::_translate_natural_join(const hsql::JoinDefinition &join)
{
    ...
    const auto left_sql_identifier_resolver =
        left_state.sql_identifier_resolver;
    const auto right_sql_identifier_resolver =
        right_state.sql_identifier_resolver;
    ...
    // b) Add columns from right input to the output when they have no match in
    // the left input
    for (const auto &right_element : right_state.elements_in_order)
    {
        ...
        if (!right_identifiers.empty())
        {
            ...
            const auto left_expression =
                left_sql_identifier_resolver->resolve_identifier_relaxed(
                    {right_identifier.column_name});

            if (left_expression)
            {
                // Two columns match, let's join on them.
                join_predicates.emplace_back(
                    equals_(left_expression, right_expression));
                continue;
            }

            // No matching column in the left input found, add the column from
            // the right input to the output
            result_state.elements_in_order.emplace_back(right_element);
            result_state.sql_identifier_resolver->add_column_name(
                right_expression, right_identifier.column_name);
            if (right_identifier.table_name)
            {
                result_state
                    .elements_by_table_name[*right_identifier.table_name]
                    .emplace_back(right_element);
                result_state.sql_identifier_resolver->set_table_name(
                    right_expression, *right_identifier.table_name);
            }
        }
    }
    ...
    else
    {
        ...
        lqp = JoinNode::make(JoinMode::Inner, join_predicates.front(),
                             left_input_lqp, right_input_lqp);
    }

    // Add remaining join predicates as normal predicates
    const auto join_predicate_count = join_predicates.size();
    for (auto join_predicate_idx = size_t{1};
         join_predicate_idx < join_predicate_count; ++join_predicate_idx)
    {
    ...
    if (!join_predicates.empty())
    {
        ...
        lqp = ProjectionNode::make(
            unwrap_elements(result_state.elements_in_order), lqp);
    }
...
SQLTranslator::TableSourceState SQLTranslator::_translate_cross_product(
    const std::vector<hsql::TableRef *> &tables)
{
    ...
    for (auto table_idx = size_t{1}; table_idx < table_count; ++table_idx)
    {
        ...
        result_table_source_state.lqp =
            JoinNode::make(JoinMode::Cross, result_table_source_state.lqp,
                           table_source_state.lqp);
...
std::vector<SQLTranslator::SelectListElement>
SQLTranslator::_translate_select_list(
    const std::vector<hsql::Expr *> &select_list)
{
    // Build the select_list_elements
    // Each expression of a select_list_element is either an Expression or
    // nullptr if the element is a Wildcard Create an SQLIdentifierResolver that
    // knows the aliases
    auto select_list_elements = std::vector<SelectListElement>{};
    auto post_select_sql_identifier_resolver =
        std::make_shared<SQLIdentifierResolver>(*_sql_identifier_resolver);
    for (const auto &hsql_select_expr : select_list)
    {
        ...
        {
            FailInput(
                "Interval can only be added to or substracted from a date");
        }
        else
        {
            const auto expression = _translate_hsql_expr(
                *hsql_select_expr, _sql_identifier_resolver, true);
            select_list_elements.emplace_back(expression);
            if (hsql_select_expr->name &&
                hsql_select_expr->type != hsql::kExprFunctionRef &&
                hsql_select_expr->type != hsql::kExprExtract)
            {
                select_list_elements.back().identifiers.emplace_back(
                    hsql_select_expr->name);
            }
            ...
            if (hsql_select_expr->alias)
            {
                ...
                post_select_sql_identifier_resolver->add_column_name(
                    expression, hsql_select_expr->alias);
                select_list_elements.back().identifiers.emplace_back(
                    identifier);
            }
        }
    ...
    const std::vector<SelectListElement> &select_list_elements)
{
    ...
    auto window_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>{};

    // Visitor that identifies still uncomputed WindowFunctionExpressions and
    // their arguments.
    const auto find_uncomputed_aggregates_and_arguments =
        [&](auto &sub_expression)
    {
        /**
         * If the WindowFunctionExpression has already been computed in a
         * previous node (consider "x" in "SELECT x FROM (SELECT MIN(a) as x
         * FROM t) AS y)", it does not count as a new Aggregate and is therefore
         * not considered an "Aggregate" in the current SELECT list. Handling
         * this as a special case seems hacky, but it is the best solution we
         * came up with.
         */
        ...
        const auto &window_expression =
            std::static_pointer_cast<WindowFunctionExpression>(sub_expression);
        ...
        if (aggregate_expression_set.emplace(window_expression).second)
        {
            ...
            for (const auto &argument : window_expression->arguments)
            {
                ...
                {
                    // Handle COUNT(*)
                    const auto *const column_expression =
                        dynamic_cast<const LQPColumnExpression *>(&*argument);
    ...
    for (const auto &element : select_list_elements)
    {
        if (element.expression)
        {
            visit_expression(element.expression,
                             find_uncomputed_aggregates_and_arguments);
        }
    }
    ...
    if (select.groupBy && select.groupBy->columns)
    {
        ...
        {
            const auto group_by_expression = _translate_hsql_expr(
                *group_by_hsql_expr, _sql_identifier_resolver);
            group_by_expressions.emplace_back(group_by_expression);
            if (pre_aggregate_expression_set.emplace(group_by_expression)
                    .second)
            {
    ...
    if (select.groupBy && select.groupBy->having)
    {
        having_expression = _translate_hsql_expr(*select.groupBy->having,
                                                 _sql_identifier_resolver);
        visit_expression(having_expression,
                         find_uncomputed_aggregates_and_arguments);
    }

    const auto is_aggregate =
        !aggregate_expressions.empty() || !group_by_expressions.empty();
    ...
    if (is_aggregate)
    {
        // If needed, add a ProjectionNode to evaluate all expressions required
        ...
        if (!pre_aggregate_expressions.empty())
        {
            ...
            const auto any_expression_not_yet_available = std::any_of(
                pre_aggregate_expressions.cbegin(),
                pre_aggregate_expressions.cend(),
            ...
            {
                _current_lqp = ProjectionNode::make(pre_aggregate_expressions,
                                                    _current_lqp);
            }
        }
        _current_lqp = AggregateNode::make(group_by_expressions,
                                           aggregate_expressions, _current_lqp);
    }
    ...
    {
        AssertInput(
            expression_evaluable_on_lqp(having_expression, *_current_lqp),
            "HAVING references columns not accessible after Aggregation");
        _current_lqp =
            _translate_predicate_expression(having_expression, _current_lqp);
    }
    ...
    if (!window_expressions.empty())
    {
        ...
        const auto computed_expression_set = ExpressionUnorderedSet{
            computed_expressions.begin(), computed_expressions.end()};
        ...
        // The argument of the window function, PARTITION BY expressions, and
        // ORDER BY expressions must either be present or can be computed by a
        // ProjectionNode. This also means a window function cannot depend on
        // the result of another window function.
        for (const auto &expression : window_expressions)
        {
            const auto &window_function =
                static_cast<const WindowFunctionExpression &>(*expression);
            const auto &argument = window_function.argument();
            AssertInput(!argument || expression_evaluable_on_lqp(argument,
                                                                 *_current_lqp),
                        "Argument of window function " +
                            window_function.as_column_name() +
                            " is not available.");
            ...
            const auto &window = static_cast<const WindowExpression &>(
                *window_function.window());
            for (const auto &required_expression : window.arguments)
            {
                AssertInput(expression_evaluable_on_lqp(required_expression,
                                                        *_current_lqp),
                            "Required column " +
                                required_expression->as_column_name() +
                                " for window definition is not available.");
        ...
        {
            computed_expressions.insert(computed_expressions.end(),
                                        required_expressions.cbegin(),
                                        required_expressions.cend());
            _current_lqp =
                ProjectionNode::make(computed_expressions, _current_lqp);
        }

        // Add WindowNodes on top of the LQP. For now, their order is just the
        // same as in the SELECT list.
    ...
    for (auto select_list_idx = size_t{0}; select_list_idx < select_list_size;
         ++select_list_idx)
    {
        ...
        {
            AssertInput(_from_clause_result,
                        "Can't SELECT with wildcards since there are no FROM "
                        "tables specified");

            if (is_aggregate)
            {
                // SELECT * is only valid if every input column is named in the
                // GROUP BY clause
                for (const auto &pre_aggregate_expression :
                     pre_aggregate_lqp->output_expressions())
                {
                    if (hsql_expr->table)
                    {
                        // Dealing with SELECT t.* here
                        auto identifiers = _sql_identifier_resolver
                                               ->get_expression_identifiers(
                                                   pre_aggregate_expression);
                        ...
                        {
                            // The pre_aggregate_expression may or may not be
                            // part of the GROUP BY clause, but since it comes
                            // from a different table, it is not included in the
                            // `SELECT t.*`.
                            continue;
                        }
                    }
                    ...
                    AssertInput(
                        std::find_if(group_by_expressions.begin(),
                                     group_by_expressions.end(),
                                     [&](const auto &group_by_expression) {
                                         return *pre_aggregate_expression ==
                                                *group_by_expression;
                                     }) != group_by_expressions.end(),
                        std::string("Expression ") +
                            pre_aggregate_expression->as_column_name() +
                            " was added to SELECT list when resolving *, but "
                            "it is not part of the GROUP BY clause");
                }
            }
            ...
            if (hsql_expr->table)
            {
                if (is_aggregate)
                {
                    ...
                    for (const auto &group_by_expression : group_by_expressions)
                    {
                        ...
                        for (const auto &identifier : identifiers)
                        {
                            ...
                            {
                                _inflated_select_list_elements.emplace_back(
                                    group_by_expression);
                            }
                        }
                    }
                ...
                {
                    // Select all columns from the FROM element with the
                    // specified name
                    const auto from_element_iter =
                        _from_clause_result->elements_by_table_name.find(
                            hsql_expr->table);
                    AssertInput(
                        from_element_iter !=
                            _from_clause_result->elements_by_table_name.end(),
                        std::string(
                            "No such element in FROM with table name '") +
                            hsql_expr->table + "'");
            ...
            else
            {
                ...
                else
                {
                    ...
                    _inflated_select_list_elements.insert(
                        _inflated_select_list_elements.end(),
        ...
        {
            _inflated_select_list_elements.emplace_back(
                select_list_elements[select_list_idx]);
        }
    }
}

void SQLTranslator::_translate_set_operation(
    const hsql::SetOperation &set_operator)
{
    const auto &left_input_lqp = _current_lqp;
    const auto left_output_expressions = left_input_lqp->output_expressions();

    // The right-hand side of the set operation has to be translated
    // independently and must not access SQL identifiers from the left-hand
    // side. To ensure this, we create a new SQLTranslator with its own
    // SQLIdentifierResolver.
    auto nested_set_translator = SQLTranslator{
        _use_mvcc, _external_sql_identifier_resolver_proxy,
        _parameter_id_allocator, _with_descriptions, _meta_tables};
    const auto right_input_lqp =
        nested_set_translator._translate_select_statement(
            *set_operator.nestedSelectStatement);
    ...
    for (auto expression_idx = ColumnID{0};
         expression_idx < left_output_expression_count; ++expression_idx)
    {
        ...
        AssertInput(left_expression->data_type() ==
                        right_expression->data_type(),
                    "Mismatching input data types for left and right side of "
                    "set operation");
    }

    auto lqp = std::shared_ptr<AbstractLQPNode>();

    // Choose the set operation mode. SQL only knows UNION and UNION ALL; the
    // Positions mode is only used for internal LQP optimizations and should not
    // be needed in the SQLTranslator.
    auto set_operation_mode =
        set_operator.isAll ? SetOperationMode::All : SetOperationMode::Unique;

    // Create corresponding node depending on the SetType
    switch (set_operator.setType)
    {
    case hsql::kSetExcept:
        lqp = ExceptNode::make(set_operation_mode, left_input_lqp,
                               right_input_lqp);
        break;
    case hsql::kSetIntersect:
        lqp = IntersectNode::make(set_operation_mode, left_input_lqp,
                                  right_input_lqp);
        break;
    case hsql::kSetUnion:
        lqp = UnionNode::make(set_operation_mode, left_input_lqp,
                              right_input_lqp);
        break;
    }
...
void SQLTranslator::_translate_distinct_order_by(
    ...
    const bool distinct)
{
    ...
    if (perform_sort)
    {
        ...
        for (auto expression_idx = size_t{0}; expression_idx < order_list_size;
             ++expression_idx)
        {
            const auto &order_description =
                hsql_order_expressions[expression_idx];
            expressions[expression_idx] = _translate_hsql_expr(
                *order_description->expr, _sql_identifier_resolver);
            sort_modes[expression_idx] =
                order_type_to_sort_mode.at(order_description->type);
        }

        _current_lqp =
            add_expressions_if_unavailable(_current_lqp, expressions);
    }

    // For SELECT DISTINCT, we add an AggregateNode that groups by all output
    // columns, but does not use any aggregate functions, e.g.: `SELECT DISTINCT
    // a, b ...` becomes `SELECT a, b ... GROUP BY a, b`.
    //
    // This might create unnecessary AggregateNodes when we already have an
    // aggregation that creates unique results: `SELECT DISTINCT a, MIN(b) FROM
    // t GROUP BY a` would have one aggregate that groups by a and calculates
    // MIN(b), and one that groups by both a and MIN(b) without calculating
    // anything. Fixing this is done by an optimizer rule
    // (DependentGroupByReductionRule) that checks if the respective columns are
    // already unique.
    if (distinct)
    {
        if (perform_sort)
        {
            // If we later sort the table by the ORDER BY expression, we must
            // ensure they are also part of the SELECT list (DISTINCT will be
            // applied before ORDER BY).
            const auto &select_expressions_set =
                ExpressionUnorderedSet{select_list.begin(), select_list.end()};
            AssertInput(std::all_of(expressions.cbegin(), expressions.cend(),
                                    [&](const auto &expression) {
                                        return select_expressions_set.contains(
                                            expression);
                                    }),
        ...
        // Add currently uncomputed expressions, e.g., a + 1 for SELECT DISTINCT
        // a + 1 FROM table_a.
        _current_lqp =
            add_expressions_if_unavailable(_current_lqp, select_list);

        _current_lqp =
            AggregateNode::make(select_list, expression_vector(), _current_lqp);
    }

    // If any expressions were added to perform the sorting, we must add a
    // ProjectionNode later and remove them again in
...
void SQLTranslator::_translate_limit(const hsql::LimitDescription &limit)
{
    ...
    const auto num_rows_expression =
        _translate_hsql_expr(*limit.limit, _sql_identifier_resolver);
...
// cannot.
std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_show(const hsql::ShowStatement &show_statement)
{
    _cacheable = false;
    ...
    {
        const auto tables_meta_table =
            Hyrise::get().meta_table_manager.generate_table("tables");
        return StaticTableNode::make(tables_meta_table);
    }
    case hsql::ShowType::kShowColumns:
    {
        const auto columns_meta_table =
            Hyrise::get().meta_table_manager.generate_table("columns");
        const auto static_table_node =
            StaticTableNode::make(columns_meta_table);
        const auto table_name_column =
            lqp_column_(static_table_node, ColumnID{0});
        const auto predicate =
            equals_(table_name_column, value_(show_statement.name));
...
std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_create(const hsql::CreateStatement &create_statement)
{
...
std::shared_ptr<AbstractLQPNode> SQLTranslator::_translate_create_view(
    const hsql::CreateStatement &create_statement)
{
    auto lqp = _translate_select_statement(
        static_cast<const hsql::SelectStatement &>(*create_statement.select));
    const auto output_expressions = lqp->output_expressions();
    ...
    {
        // The CREATE VIEW statement has renamed the columns: CREATE VIEW myview
        // (foo, bar) AS SELECT ...
        const auto view_column_count = create_statement.viewColumns->size();
        AssertInput(
            view_column_count == output_expressions.size(),
            "Number of Columns in CREATE VIEW does not match SELECT statement");

        for (auto column_id = ColumnID{0}; column_id < view_column_count;
             ++column_id)
        {
            column_names.insert_or_assign(
                column_id, (*create_statement.viewColumns)[column_id]);
        }
    }
    else
    {
        ...
        {
            for (const auto &identifier :
                 _inflated_select_list_elements[column_id].identifiers)
            {
                column_names.insert_or_assign(column_id,
                                              identifier.column_name);
            }
        }
    }

    return CreateViewNode::make(create_statement.tableName,
                                std::make_shared<LQPView>(lqp, column_names),
                                create_statement.ifNotExists);
}

std::shared_ptr<AbstractLQPNode> SQLTranslator::_translate_create_table(
    const hsql::CreateStatement &create_statement)
{
    Assert(create_statement.columns || create_statement.select,
    ...
    else
    {
        auto column_definitions =
            TableColumnDefinitions{create_statement.columns->size()};
        auto table_key_constraints = TableKeyConstraints{};

        for (auto column_id = ColumnID{0};
             column_id < create_statement.columns->size(); ++column_id)
        {
            const auto *parser_column_definition =
                create_statement.columns->at(column_id);
            auto &column_definition = column_definitions[column_id];

            // TODO(anybody) SQLParser is missing support for Hyrise's other
            // types
            ...
            case hsql::DataType::DECIMAL:
                std::cout
                    << "WARNING: Implicitly converting DECIMAL to FLOAT\n";
            ...
            case hsql::DataType::CHAR:
                std::cout << "WARNING: Ignoring the length of CHAR. Hyrise's "
                             "strings are not length-limited\n";
                column_definition.data_type = DataType::String;
                break;
            case hsql::DataType::VARCHAR:
                std::cout << "WARNING: Ignoring the length of VARCHAR. "
                             "Hyrise's strings are not length-limited\n";
                column_definition.data_type = DataType::String;
                break;
            ...
            case hsql::DataType::DATE:
                std::cout << "WARNING: Parsing DATE to string since date and "
                             "time data types are not yet supported\n";
                column_definition.data_type = DataType::String;
                break;
            case hsql::DataType::DATETIME:
                std::cout << "WARNING: Parsing DATETIME to string since date "
                             "and time data types are not yet supported\n";
                column_definition.data_type = DataType::String;
                break;
            case hsql::DataType::TIME:
                std::cout << "WARNING: Parsing TIME to string since date and "
                             "time data types are not yet supported\n";
                column_definition.data_type = DataType::String;
                break;
            ...
            // Translate column constraints. We only address UNIQUE/PRIMARY KEY
            // constraints for now.
            DebugAssert(parser_column_definition->column_constraints,
                        "Column " + column_definition.name +
                            " is missing constraint information");
            for (const auto &column_constraint :
                 *parser_column_definition->column_constraints)
            {
                ...
                table_key_constraints.emplace(std::set<ColumnID>{column_id},
                                              constraint_type);
                std::cout << "WARNING: "
                          << magic_enum::enum_name(constraint_type)
                          << " constraint for column " << column_definition.name
                          << " will not be enforced\n";
            }
        }

        // Translate table constraints. Note that a table constraint is either a
        // unique constraint, a referential con- straint, or a table check
        // constraint per SQL standard. Some constraints can be set (i) when
        // describing a single column, see above, or (ii) as a property of the
        // table, containing multiple columns. We only address UNIQUE/ PRIMARY
        // KEY constraints for now.
        ...
        for (const auto &table_constraint : *create_statement.tableConstraints)
        {
            Assert(table_constraint->type == hsql::ConstraintType::PrimaryKey ||
                   ...
                   "Only UNIQUE and PRIMARY KEY constraints are expected on a "
                   "table level");
            const auto constraint_type =
                table_constraint->type == hsql::ConstraintType::PrimaryKey
                    ? KeyConstraintType::PRIMARY_KEY
                    : KeyConstraintType::UNIQUE;
            ...
            DebugAssert(table_constraint->columnNames,
                        std::string{magic_enum::enum_name(constraint_type)} +
                            " table constraint must contain columns");
            auto column_ids = std::set<ColumnID>{};
            for (const auto &constraint_column_name :
                 *table_constraint->columnNames)
            {
                for (auto column_id = ColumnID{0}; column_id < column_count;
                     ++column_id)
                {
                    auto &column_definition = column_definitions[column_id];
                    if (column_definition.name == constraint_column_name)
                    {
                        ...
                        if (constraint_type == KeyConstraintType::PRIMARY_KEY)
                        {
                            ...
                            AssertInput(!create_statement.columns->at(column_id)
                                             ->column_constraints->contains(
                                                 hsql::ConstraintType::Null),
                                        "PRIMARY KEY column " +
            ...
            AssertInput(
                ...
                "Could not resolve columns of " +
                    std::string{magic_enum::enum_name(constraint_type)} +
                    " table constraint");
            table_key_constraints.emplace(column_ids, constraint_type);
            std::cout << "WARNING: " << magic_enum::enum_name(constraint_type)
                      << " table constraint will not be enforced\n";
        }
    ...
    return CreateTableNode::make(create_statement.tableName,
                                 create_statement.ifNotExists, input_node);
}

// NOLINTNEXTLINE - while this particular method could be made static, others
// cannot.
std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_drop(const hsql::DropStatement &drop_statement)
{
...
std::shared_ptr<AbstractLQPNode> SQLTranslator::_translate_prepare(
    const hsql::PrepareStatement &prepare_statement)
{
    ...
    AssertInput(
        parse_result.isValid(),
        create_sql_parser_error_message(prepare_statement.query, parse_result));
    AssertInput(parse_result.size() == 1u,
                "PREPAREd statement can only contain a single SQL statement");

    auto prepared_plan_translator = SQLTranslator{_use_mvcc};

    const auto translation_result =
        prepared_plan_translator.translate_parser_result(parse_result);
    ...
    const auto lqp_prepared_plan =
        std::make_shared<PreparedPlan>(lqp, parameter_ids);

    return CreatePreparedPlanNode::make(prepare_statement.name,
                                        lqp_prepared_plan);
}

std::shared_ptr<AbstractLQPNode> SQLTranslator::_translate_execute(
    const hsql::ExecuteStatement &execute_statement)
{
    const auto num_parameters =
        execute_statement.parameters ? execute_statement.parameters->size() : 0;
    auto parameters =
        std::vector<std::shared_ptr<AbstractExpression>>{num_parameters};
    for (auto parameter_idx = size_t{0}; parameter_idx < num_parameters;
         ++parameter_idx)
    {
        parameters[parameter_idx] = translate_hsql_expr(
            *(*execute_statement.parameters)[parameter_idx], _use_mvcc);
    }

    const auto prepared_plan =
        Hyrise::get().storage_manager.get_prepared_plan(execute_statement.name);

    AssertInput(
        _use_mvcc ==
            (lqp_is_validated(prepared_plan->lqp) ? UseMvcc::Yes : UseMvcc::No),
        "Mismatch between validation of Prepared statement and query it is "
        "used in");
...
// cannot.
std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_import(const hsql::ImportStatement &import_statement)
{
    // Querying tables that are freshly loaded is not easy as we need meta
    // information, such as column names and data types, to resolve queries and
    // build the query plans. For instance, we need an origin node for column
    // expressions and to provide correct output expressions for subsequent
    // nodes, some optimization rules access stored tables, and so on. Anyway,
    // we would need to decouple data loading and storing the table and have to
    // load metadata or even the whole table when translating the SQL statement.
    AssertInput(!import_statement.whereClause,
                "Predicates on imported files are not supported.");
    return ImportNode::make(import_statement.tableName,
...
// cannot.
std::shared_ptr<AbstractLQPNode>
SQLTranslator::_translate_export(const hsql::ExportStatement &export_statement)
{
    auto sql_identifier_resolver = std::make_shared<SQLIdentifierResolver>();
    auto lqp = std::shared_ptr<AbstractLQPNode>{};
    ...
    {
        AssertInput(export_statement.tableName,
                    "ExportStatement must either specify a table name or a "
                    "SelectStatement.");
        ...
        else
        {
            ...
            lqp = _translate_stored_table(export_statement.tableName,
                                          sql_identifier_resolver);
        }
    }

    return ExportNode::make(export_statement.filePath,
    ...
    std::shared_ptr<AbstractLQPNode> current_node) const
{
    ...
    case ExpressionType::Predicate:
    {
        const auto predicate_expression =
            std::static_pointer_cast<AbstractPredicateExpression>(expression);
    ...
    {
        const auto logical_expression =
            std::static_pointer_cast<LogicalExpression>(expression);

        switch (logical_expression->logical_operator)
        {
        case LogicalOperator::And:
        {
            current_node = _translate_predicate_expression(
                logical_expression->right_operand(), current_node);
            return _translate_predicate_expression(
                logical_expression->left_operand(), current_node);
        }
        case LogicalOperator::Or:
            return PredicateNode::make(expression, current_node);
        }
    ...
    bool allow_window_functions)
{
    ...
    auto left = expr.expr
                    ? _translate_hsql_expr(*expr.expr, sql_identifier_resolver,
                                           allow_window_functions)
                    : nullptr;
    const auto right =
        expr.expr2 ? _translate_hsql_expr(*expr.expr2, sql_identifier_resolver,
                                          allow_window_functions)
                   : nullptr;

    if (left)
    {
        AssertInput(left->type != ExpressionType::Interval,
                    "IntervalExpression must follow another expression.");
    }
    if (right && right->type == ExpressionType::Interval)
    {
        AssertInput(
            expr.type == hsql::kExprOperator &&
                (expr.opType == hsql::kOpPlus || expr.opType == hsql::kOpMinus),
    ...
    case hsql::kExprColumnRef:
    {
        const auto table_name =
            expr.table ? std::optional<std::string>(std::string(expr.table))
                       : std::nullopt;
        const auto identifier = SQLIdentifier{name, table_name};

        auto expression =
            sql_identifier_resolver->resolve_identifier_relaxed(identifier);
        if (!expression && _external_sql_identifier_resolver_proxy)
        {
            // Try to resolve the identifier in the outer queries
            expression = _external_sql_identifier_resolver_proxy
                             ->resolve_identifier_relaxed(identifier);
        }
        AssertInput(expression, "Couldn't resolve identifier '" +
    ...
    case hsql::kExprParameter:
    {
        ...
        auto value_placeholder_id =
            ValuePlaceholderID{static_cast<uint16_t>(expr.ival)};
        return placeholder_(
            _parameter_id_allocator->allocate_for_value_placeholder(
                value_placeholder_id));
    }

    case hsql::kExprExtract:
    {
        Assert(expr.datetimeField != hsql::kDatetimeNone,
               "No DatetimeField specified in EXTRACT. Bug in sqlparser?");
    ...
    case hsql::kExprFunctionRef:
    {
        ...
        if (expr.windowDescription)
        {
            ...
            {
                partition_by_expressions.reserve(
                    hsql_window_description.partitionList->size());
                for (const auto expression :
                     *hsql_window_description.partitionList)
                {
                    partition_by_expressions.emplace_back(_translate_hsql_expr(
                        *expression, sql_identifier_resolver));
                }
            }
            ...
            if (hsql_window_description.orderList)
            {
                ...
                {
                    order_by_expressions.emplace_back(_translate_hsql_expr(
                        *order_description->expr, sql_identifier_resolver));
                    sort_modes.emplace_back(
                        order_type_to_sort_mode.at(order_description->type));
                }
            }

            Assert(
                hsql_window_description.frameDescription,
                "Window function has no FrameDescription. Bug in sqlparser?");
            const auto &hsql_frame_description =
                *hsql_window_description.frameDescription;
            ...
            const auto start =
                translate_frame_bound(*hsql_frame_description.start);
            const auto end = translate_frame_bound(*hsql_frame_description.end);

            // "Restrictions are that `frame_start` cannot be UNBOUNDED
            // FOLLOWING, `frame_end` cannot be UNBOUNDED PRECEDING, and the
            // `frame_end` choice cannot appear earlier [...] than the
            // `frame_start` choice does — for example RANGE BETWEEN CURRENT ROW
            // AND offset PRECEDING is not allowed. But, for example, ROWS
            // BETWEEN 7
            //  PRECEDING AND 8 PRECEDING is allowed, even though it would never
            //  select any rows."
            // -
            // https://www.postgresql.org/docs/current/sql-expressions.html#SYNTAX-WINDOW-FUNCTIONS
            // To ensure this behavior, we treat CURRENT ROW as an offset of 0,
            // PRECEDING as a negative offset, and FOLLOWING as a positive
            // offset. If end offset < start offset, the frame definition is
            // invalid.
            const auto relative_frame_offset = [](const auto &frame_bound)
            {
                ...
                {
                    offset = frame_bound.unbounded
                                 ? std::numeric_limits<int64_t>::min()
                                 : -static_cast<int64_t>(frame_bound.offset);
                }
            ...
            AssertInput(
                (start.type != FrameBoundType::Following || !start.unbounded) &&
                    (end.type != FrameBoundType::Preceding || !end.unbounded) &&
                    end_offset >= start_offset,
                "Frame starting from " + start.description() +
                    " cannot end at " + end.description() + ".");

            auto frame_description = FrameDescription{frame_type, start, end};
            window_description =
                window_(std::move(partition_by_expressions),
                        std::move(order_by_expressions), std::move(sort_modes),
        ...
        // Some SQL functions have aliases, which we map to one unique
        // identifier here.
        static const auto function_aliases =
         ...
         */
        const auto window_function_iter =
            window_function_to_string.right.find(name);
        if (window_function_iter != window_function_to_string.right.end())
        {
            ...
            {
                AssertInput(allow_window_functions,
                            "Window functions are only allowed in the SELECT "
                            "list and must not be nested.");
                AssertInput(window_description,
                            "Window function " + name +
                                " requires a window definition.");
                AssertInput(!expr.exprList || expr.exprList->empty(),
                            "Window functions must not have an argument.");
            }
            else
            {
                AssertInput(
                    expr.exprList && expr.exprList->size() == 1,
            ...
            auto aggregate_expression =
                std::shared_ptr<WindowFunctionExpression>{};
            ...
            case WindowFunction::StandardDeviationSample:
            {
                aggregate_expression =
                    std::make_shared<WindowFunctionExpression>(
                        window_function,
                        _translate_hsql_expr(*expr.exprList->front(),
                                             sql_identifier_resolver),
                        window_description);
            }
            break;
            ...
            case WindowFunction::CountDistinct:
            {
                if (expr.exprList->front()->type == hsql::kExprStar)
                {
                    ...
                    const auto column_expression =
                        lqp_column_(leaf_node, INVALID_COLUMN_ID);

                    aggregate_expression =
                        std::make_shared<WindowFunctionExpression>(
                            window_function, column_expression,
                            window_description);
                }
                else
                {
            ...
            case WindowFunction::RowNumber:
            {
                aggregate_expression =
                    std::make_shared<WindowFunctionExpression>(
                        window_function, nullptr, window_description);
            }
            break;
            }

            // Check that the aggregate can be calculated on the given
            // expression.
            const auto result_type = aggregate_expression->data_type();
            AssertInput(result_type != DataType::Null,
                        std::string{"Invalid aggregate "} +
                            aggregate_expression->as_column_name() +
                            " for input data type " +
                            data_type_to_string.left.at(
                                aggregate_expression->argument()->data_type()));

            // Check for ambiguous expressions that occur both at the current
            // node and in its input tables. Example:
            //   SELECT COUNT(a) FROM (SELECT a, COUNT(a) FROM t GROUP BY a) t2
            // Our current expression system cannot handle this case and would
            // consider the two COUNT(a) to be identical, see #1902 for details.
            // This check here might have false positives, feel free to improve
            // the check or tackle the underlying issue if this ever becomes an
            // issue.
            auto table_expressions =
                std::vector<std::shared_ptr<AbstractExpression>>{};
            DebugAssert(_from_clause_result,
                        "_from_clause_result should be set by now");
            table_expressions.reserve(
                _from_clause_result->elements_in_order.size());
            for (const auto &select_list_element :
                 _from_clause_result->elements_in_order)
            {
            ...
            AssertInput(
                std::none_of(
                    table_expressions.cbegin(), table_expressions.cend(),
                    [&aggregate_expression](const auto input_expression)
                    { return *input_expression == *aggregate_expression; }),
                "Hyrise cannot handle repeated aggregate expressions, see "
                "#1902 for details.");
        ...
        {
            AssertInput(!window_description,
                        "Function " + name + " is not a window function.");
            ...
            {
                arguments.emplace_back(_translate_hsql_expr(
                    *hsql_argument, sql_identifier_resolver));
            }

            return std::make_shared<FunctionExpression>(function_iter->second,
                                                        arguments);
        }
    ...
    case hsql::kExprOperator:
    {
        ...
        const auto arithmetic_operators_iter =
            hsql_arithmetic_operators.find(expr.opType);
        if (arithmetic_operators_iter != hsql_arithmetic_operators.end())
        {
            Assert(left && right, "Unexpected SQLParserResult. Didn't receive "
                                  "two arguments for binary expression.");
            ...
            if (right->type == ExpressionType::Interval)
            {
                AssertInput(left->type == ExpressionType::Value &&
                            ...
                            "Interval can only be applied to ValueExpression "
                            "with String value");
                const auto start_date_string =
                    std::string{boost::get<pmr_string>(
                        static_cast<ValueExpression &>(*left).value)};
                const auto start_timestamp =
                    string_to_timestamp(start_date_string);
                AssertInput(start_timestamp,
                            "'" + start_date_string +
                                "' is not a valid ISO 8601 extended date");
                const auto &interval_expression =
                    static_cast<IntervalExpression &>(*right);
                // We already ensured to have either Addition or Substraction
                // right at the beginning
                const auto duration =
                    arithmetic_operator == ArithmeticOperator::Addition
                        ? interval_expression.duration
                        : -interval_expression.duration;
                const auto end_date =
                    date_interval(start_timestamp->date(), duration,
                                  interval_expression.unit);
                return value_(pmr_string{date_to_string(end_date)});
            }
            return std::make_shared<ArithmeticExpression>(arithmetic_operator,
                                                          left, right);
        }

        // Translate PredicateExpression
        const auto predicate_condition_iter =
            hsql_predicate_condition.find(expr.opType);
        if (predicate_condition_iter != hsql_predicate_condition.end())
        {
            ...
            {
                Assert(left && right,
                       "Unexpected SQLParserResult. Didn't receive two "
                       "arguments for binary_expression");
                return std::make_shared<BinaryPredicateExpression>(
                    predicate_condition, left, right);
            }

            if (predicate_condition == PredicateCondition::BetweenInclusive)
            {
                Assert(expr.exprList && expr.exprList->size() == 2,
                       "Expected two arguments for BETWEEN");
                return between_inclusive_(
                    left,
                    _translate_hsql_expr(*(*expr.exprList)[0],
                                         sql_identifier_resolver),
                    _translate_hsql_expr(*(*expr.exprList)[1],
                                         sql_identifier_resolver));
            }
        }
        ...
        case hsql::kOpIn:
        {
            ...
            {
                // `a IN (SELECT ...)`
                const auto subquery = _translate_hsql_subquery(
                    *expr.select, sql_identifier_resolver);
        ...
        case hsql::kOpExists:
            ...
            return exists_(_translate_hsql_subquery(*expr.select,
                                                    sql_identifier_resolver));

        default:
            Fail("Unexpected expression type"); // There are 19 of these, so we
    ...
    case hsql::kExprArray:
        FailInput("Can't translate a standalone array, arrays only valid in IN "
                  "expressions");
    ...
    case hsql::kExprCast:
    {
        ...
        if (target_hsql_data_type == hsql::DataType::DATE ||
            target_hsql_data_type == hsql::DataType::DATETIME)
        {
            AssertInput(
                source_data_type == DataType::String,
                "Cannot cast " + left->as_column_name() + " as " +
                    std::string{magic_enum::enum_name(target_hsql_data_type)});
            // We do not know if an expression to be casted other than a
            // ValueExpression actually contains date time values, and we cannot
            // check this later due to the lack of proper data types.
            AssertInput(
                left->type == ExpressionType::Value,
                "Only ValueExpressions can be casted as " +
                    std::string{magic_enum::enum_name(target_hsql_data_type)});
            const auto input_string = std::string{boost::get<pmr_string>(
                static_cast<ValueExpression &>(*left).value)};

            if (target_hsql_data_type == hsql::DataType::DATE)
            {
                // We do not have a Date data type, so we check if the date is
                // valid and return its ValueExpression.
            ...
            if (target_hsql_data_type == hsql::DataType::DATETIME)
            {
                ...
                AssertInput(date_time,
                            "'" + input_string +
                                "' is not a valid ISO 8601 extended timestamp");
                // Parsing valid timestamps is also possible for at first glance
                // invalid strings (see utils/date_time_utils.hpp for details).
                // To always obtain a semantically meaningful result, we
                // retrieve the created timestamp's string representation.
                return value_(pmr_string{timestamp_to_string(*date_time)});
            }
        }

        const auto data_type_iter =
            supported_hsql_data_types.find(expr.columnType.data_type);
        AssertInput(
            data_type_iter != supported_hsql_data_types.cend(),
            "CAST as " +
                std::string{magic_enum::enum_name(expr.columnType.data_type)} +
                " is not supported");
    ...
    case hsql::kExprLiteralInterval:
    {
        ...
        AssertInput(unit == DatetimeComponent::Day ||
                        unit == DatetimeComponent::Month ||
                        unit == DatetimeComponent::Year,
...
std::shared_ptr<LQPSubqueryExpression> SQLTranslator::_translate_hsql_subquery(
    ...
    const std::shared_ptr<SQLIdentifierResolver> &sql_identifier_resolver)
{
    const auto sql_identifier_proxy =
        std::make_shared<SQLIdentifierResolverProxy>(
            sql_identifier_resolver, _parameter_id_allocator,
            _external_sql_identifier_resolver_proxy);

    auto subquery_translator =
        SQLTranslator{_use_mvcc, sql_identifier_proxy, _parameter_id_allocator,
                      _with_descriptions, _meta_tables};
    const auto subquery_lqp =
        subquery_translator._translate_select_statement(select);
    const auto parameter_count =
        sql_identifier_proxy->accessed_expressions().size();

    // If this statement or any of the subquery's statements is not cacheable
    // (because of meta tables), this statement should not be cacheable.
    _cacheable &= subquery_translator._cacheable;

    auto parameter_ids = std::vector<ParameterID>{};
    parameter_ids.reserve(parameter_count);

    auto parameter_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>{};
    parameter_expressions.reserve(parameter_count);

    for (const auto &expression_and_parameter_id :
         sql_identifier_proxy->accessed_expressions())
    {
    ...
    return std::make_shared<LQPSubqueryExpression>(subquery_lqp, parameter_ids,
...
{
    /**
     * There is a "simple" and a "searched" CASE syntax, see
     * http://www.oratable.com/simple-case-searched-case/ Hyrise supports both.
     */

    Assert(expr.exprList, "Unexpected SQLParserResult. Case needs exprList");
    Assert(!expr.exprList->empty(),
           "Unexpected SQLParserResult. Case needs non-empty exprList");

    // "a + b" in "CASE a + b WHEN ... THEN ... END", or nullptr when using the
    // "searched" CASE syntax
    auto simple_case_left_operand = std::shared_ptr<AbstractExpression>{};
    if (expr.expr)
    {
        simple_case_left_operand =
            _translate_hsql_expr(*expr.expr, sql_identifier_resolver);
    }

    // Initialize CASE with the ELSE expression and then put the remaining
    // WHEN...THEN... clauses on top of that in reverse order
    auto current_case_expression = std::shared_ptr<AbstractExpression>{};
    if (expr.expr2)
    {
        current_case_expression =
            _translate_hsql_expr(*expr.expr2, sql_identifier_resolver);
    }
    else
    {
    ...
    for (auto case_reverse_idx = size_t{0};
         case_reverse_idx < expr.exprList->size(); ++case_reverse_idx)
    {
        const auto case_idx = expr.exprList->size() - case_reverse_idx - 1;
        const auto *const case_clause = (*expr.exprList)[case_idx];

        auto when =
            _translate_hsql_expr(*case_clause->expr, sql_identifier_resolver);
        ...
        const auto then =
            _translate_hsql_expr(*case_clause->expr2, sql_identifier_resolver);
...
SQLTranslator::SelectListElement::SelectListElement(
    const std::shared_ptr<AbstractExpression> &init_expression)
...
SQLTranslator::TableSourceState::TableSourceState(
    ...
    const std::unordered_map<std::string, std::vector<SelectListElement>>
        &init_elements_by_table_name,
    ...
    : lqp(init_lqp), elements_by_table_name(init_elements_by_table_name),
...
void SQLTranslator::TableSourceState::append(TableSourceState &&rhs)
{
    ...
    {
        const auto unique =
            !elements_by_table_name.contains(table_name_and_elements.first);
        AssertInput(unique, "Table Name '"s + table_name_and_elements.first +
                                "' in FROM clause is not unique");
    }

    elements_by_table_name.merge(std::move(rhs.elements_by_table_name));
    elements_in_order.insert(elements_in_order.end(),
                             rhs.elements_in_order.begin(),
                             rhs.elements_in_order.end());
