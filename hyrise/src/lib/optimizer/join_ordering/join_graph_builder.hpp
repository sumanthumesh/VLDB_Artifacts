 ...
 *        \_[4] [MockTable] y
 *
 * The JoinGraph created from it would contain two vertices (x and y), one edge
 * (between x and y, with the predicate x2 <= y1) and one output_relation
 * (Projection, left input side)
 */
class JoinGraphBuilder final
{
  public:
    /**
     * From an LQP, build a JoinGraph. The LQP is not modified during this
     * process. std::nullopt is returned if the JoinGraph would be trivial
     * (i.e., one vertex, no predicates)
     */
    std::optional<JoinGraph>
    operator()(const std::shared_ptr<AbstractLQPNode> &lqp);
  ...
  private:
    ...
    /**
     * Traverse the LQP recursively identifying predicates and vertices along
     * the way
     */
    void _traverse(const std::shared_ptr<AbstractLQPNode> &node);

    /**
     * A subgraph in the LQP consisting of PredicateNodes can be translated into
     * a single complex predicate
     *
     *                  PredicateNode(a < 3)
     *                           |
     *                  PredicateNode(b > 12)
     ...
     *                         [...]
     *
     *  Represents the JoinPlanPredicate "a < 3 AND b > 12". The AggregateNode
     * is the Predicate's "base_node" i.e. the node on which's output the
    ...
    struct PredicateParseResult
    {
        PredicateParseResult(
            const std::shared_ptr<AbstractLQPNode> &init_base_node,
    ...
    PredicateParseResult
    _parse_predicate(const std::shared_ptr<AbstractLQPNode> &node) const;

    /**
     * Returns whether a node of the given type is a JoinGraph vertex in all
     * cases. This is true for all node types that aren't Predicates, Joins or
     * Unions.
     */
