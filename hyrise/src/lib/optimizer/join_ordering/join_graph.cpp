    ...
    std::unordered_set<std::shared_ptr<AbstractLQPNode>> &visited_nodes)
{
    ...
    if (join_graph)
    {
        ...
        // A JoinGraph could be built from the subgraph rooted at `lqp`.
        // Continue the search for more JoinGraphs from the JoinGraph's vertices
        // inputs.
        for (const auto &vertex : join_graph->vertices)
        {
            build_all_in_lqp_impl(vertex->left_input(), join_graphs,
                                  visited_nodes);
            build_all_in_lqp_impl(vertex->right_input(), join_graphs,
                                  visited_nodes);
        }
    }
    else
    {
        // No JoinGraph could be built from the subgraph rooted at `lqp`.
        // Continue the search from the inputs of `lqp`.
...
JoinGraph::build_from_lqp(const std::shared_ptr<AbstractLQPNode> &lqp)
{
    return JoinGraphBuilder{}(lqp); // NOLINT - doesn't like {} followed by ()
}

std::vector<JoinGraph>
JoinGraph::build_all_in_lqp(const std::shared_ptr<AbstractLQPNode> &lqp)
{
...
JoinGraph::JoinGraph(
    const std::vector<std::shared_ptr<AbstractLQPNode>> &init_vertices,
...
JoinGraph::find_local_predicates(const size_t vertex_idx) const
{
...
std::vector<std::shared_ptr<AbstractExpression>>
JoinGraph::find_join_predicates(const JoinGraphVertexSet &vertex_set_a,
                                const JoinGraphVertexSet &vertex_set_b) const
{
    DebugAssert((vertex_set_a & vertex_set_b).none(),
                "Vertex sets are not distinct");

    std::vector<std::shared_ptr<AbstractExpression>> predicates;

    for (const auto &edge : edges)
    {
        if ((edge.vertex_set & vertex_set_a).none() ||
            (edge.vertex_set & vertex_set_b).none())
        {
            continue;
        }
