...
/**
 * A CsgCmpPair is a pair of connected subgraphs that are directly connected by
 * an edge.
 *
...
/**
 * CsgCmpPair ("CCP") enumeration algorithm described in "Analysis of two
 ...
 *
 * Input: A JoinGraph in the form of a number of vertices and edges as a set of
 * index pairs
 *
 ...
 *
 * The following CsgCmpPairs are enumerated, where the bit at 2^i represents the
 * vertex i: (0b0100, 0b1000) (0b0010, 0b0100) (0b0010, 0b1100) (0b0110, 0b1000)
 * (0b0001, 0b0010)
 * (0b0001, 0b0110)
 * (0b0001, 0b1110)
 * (0b0011, 0b0100)
 ...
 *
 * Notice how:
 *   -> these are all possible subdivisions of the JoinGraph into two connected
 * subgraphs that are connected by an edge (aka "CsgCmpPair)
 *   -> The order of the CsgCmpPairs is so that a component (both Csg and Cmp
 * are "components") is either
 *          -> a single vertex or
 *          -> a subgraph for which **all possible subdivisions have been
 * enumerated before**. This fact is essential for dynamic programming to work.
 */
class EnumerateCcp final
{
  public:
    EnumerateCcp(const size_t num_vertices,
  ...
  private:
    ...
    void _enumerate_csg_recursive(std::vector<JoinGraphVertexSet> &csgs,
    ...
    JoinGraphVertexSet
    _neighborhood(const JoinGraphVertexSet &vertex_set,
                  const JoinGraphVertexSet &exclusion_set) const;

    JoinGraphVertexSet
    _single_vertex_neighborhood(const size_t vertex_idx) const;

    // Corresponds to subset-first subset enumeration in the paper
    std::vector<JoinGraphVertexSet>
    _non_empty_subsets(const JoinGraphVertexSet &vertex_set) const;
