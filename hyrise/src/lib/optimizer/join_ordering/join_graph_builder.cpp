...
std::optional<JoinGraph>
JoinGraphBuilder::operator()(const std::shared_ptr<AbstractLQPNode> &lqp)
{
    // No need to create a join graph consisting of just one vertex and no
    // predicates
...
{
    // Makes it possible to call _traverse() on inputs without checking whether
    // they exist first.
    if (!node)
    {
        return;
    ...
    {
        /**
         * Cross joins are simply being traversed past. Outer joins are hard to
         * address during join ordering and until we do outer joins are opaque:
         * The outer join node is added as a vertex and traversal stops at this
         * point.
         */
        ...
        if (join_node->join_mode == JoinMode::Inner)
        {
            ...
            _predicates.insert(_predicates.end(), join_predicates.begin(),
                               join_predicates.end());
        }
    ...
    default:
    {
        Fail("Node type cannot be used for JoinGraph, should have been "
    ...
    const std::shared_ptr<AbstractLQPNode> &node) const
{
    ...
    case LQPNodeType::Predicate:
    {
        ...
        const auto and_predicate =
            and_(left_predicate, parse_result_right.predicate);
...
{
    return node_type != LQPNodeType::Join &&
           node_type != LQPNodeType::Predicate;
}
    ...
    const std::vector<std::shared_ptr<AbstractExpression>> &predicates)
{
    ...
    {
        const auto vertex_set =
            _get_vertex_set_accessed_by_expression(*predicate, vertices);
...
{
    /**
     * Create edges from the gathered JoinPlanPredicates. We can't directly
     * create the JoinGraph from this since we want the JoinGraph to be
     * connected and there might be edges from CrossJoins still missing. To make
     * the JoinGraph connected, we identify all Components and connect them to a
     * Chain.
     *
     * So this
     *   B
     ...
     * F
     *
     * where the edges AD and DF are being created and have no predicates. There
     * is of course the theoretical chance that different edges, say CD and EF
     * would result in a better plan. We ignore this possibility for now.
     */

    std::unordered_set<size_t> remaining_vertex_indices;
    for (auto vertex_idx = size_t{0}; vertex_idx < vertices.size();
         ++vertex_idx)
    {
    ...
    while (!remaining_vertex_indices.empty())
    {
        ...
        while (!bfs_stack.empty())
        {
            ...
            for (auto iter = edges.begin(); iter != edges.end();)
            {
                ...
                // Also skip hyperedges, as hyperedges do not connect
                // components; components connected only by a hyperedge
                //    need a cross join edge between them anyway. DPccp needs
                //    the JoinGraphs to be connected without relying on the
                //    hyperedges
                if (!edge.vertex_set.test(vertex_idx2) ||
                    edge.vertex_set.count() != 2)
                {
                    ++iter;
                    continue;
                }
                ...
                while (connected_vertex_idx != JoinGraphVertexSet::npos)
                {
                    ...
                    connected_vertex_idx =
                        edge.vertex_set.find_next(connected_vertex_idx);
                }
    ...
    for (auto component_idx = size_t{1};
         component_idx < one_vertex_per_component.size(); ++component_idx)
    {
    ...
    const std::vector<std::shared_ptr<AbstractLQPNode>> &vertices)
{
    ...
    {
        vertex_set |=
            _get_vertex_set_accessed_by_expression(*argument, vertices);
    }
