...
void NullScanRemovalRule::apply_to_plan(
    const std::shared_ptr<LogicalPlanRootNode> &root) const
{
    Assert(root->type == LQPNodeType::Root,
           "NullScanRemovalRule needs root to hold onto");

    std::vector<std::shared_ptr<AbstractLQPNode>> nodes_to_remove;

    // To determine if the rule applies to a node it must meet all of the
    ...
    auto visitor = [&](const auto &node)
    {
        ...
        const auto &column = std::dynamic_pointer_cast<LQPColumnExpression>(
        ...
        const auto table_column_definition =
            table->column_definitions()[original_column_id];
