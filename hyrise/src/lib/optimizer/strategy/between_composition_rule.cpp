...
PredicateCondition get_between_predicate_condition(bool left_inclusive,
 ...
 *   a) The ChunkPruningRule searches for chains of predicates that relate to
 * specific StoredTableNodes. Predicate pruning chains always start at a
 * StoredTableNode and continue as long as a StoredTableNode's columns remain
 *      present and become filtered.
 *   b) The BetweenCompositionRule searches for arbitrary PredicateNodes that
 * are directly linked. Therefore, predicate chains can start and end in the
 * midst of LQPs.
 */
void BetweenCompositionRule::_apply_to_plan_without_subqueries(
    const std::shared_ptr<AbstractLQPNode> &lqp_root) const
{
    std::unordered_set<std::shared_ptr<AbstractLQPNode>> visited_nodes;
    std::vector<PredicateChain> predicate_chains;

    // (1) Gather PredicateNodes and group them to predicate chains when there
    // are no other nodes in between. (1.1) Collect all leaf nodes and add them
    // to a visitation queue
    ...
    while (!node_queue.empty())
    {
        ...
        node_queue.pop();
        /**
         * Find the next predicate chain by collecting the next set of
         * PredicateNodes that are directly linked with each other. For this
         * purpose, visit the LQP upwards until
        ...
        visit_lqp_upwards(
            ...
            [&](const auto &current_node)
            {
                ...
                // Add to current predicate chain or finalize it when a
                // non-PredicateNode follows
                if (current_node->type == LQPNodeType::Predicate)
                {
                    auto current_predicate_node =
        ...
        {
            // In some cases, Between-substitutions are also possible for a
            // single PredicateNode. Think of predicates with LogicalExpression
            // and LogicalOperator::AND, for example.
...
/**
 * Looks for BinaryPredicateExpressions in @param predicate_chain, a vector of
 * directly connected PredicateNodes and replaces them with BetweenExpressions,
 * if possible. After the substitution, obsolete BinaryPredicateExpressions are
 * removed.
 */
void BetweenCompositionRule::_substitute_predicates_with_between_expressions(
    const PredicateChain &predicate_chain)
{
    ...
    const auto predicate_chain_output_input_sides =
        predicate_chain.back()->get_input_sides();
    ...
    ExpressionUnorderedMap<std::vector<std::shared_ptr<ColumnBoundary>>>
        column_boundaries;
    ...
    {
        // A logical expression can contain multiple binary predicate
        // expressions
        ...
        else
        {
            const auto logical_expression =
                std::dynamic_pointer_cast<LogicalExpression>(
                    predicate_node->predicate());
            Assert(!logical_expression ||
                       logical_expression->logical_operator !=
                           LogicalOperator::And,
        ...
        {
            const auto boundary = std::make_shared<ColumnBoundary>(
                _get_boundary(expression, id_counter));
            ++id_counter;
            if (boundary->type != ColumnBoundaryType::None)
            {
                if (boundary->boundary_is_column_expression)
                {
                    const auto inverse_boundary =
                        std::make_shared<ColumnBoundary>(
                            _create_inverse_boundary(boundary));
                    if (column_boundaries.find(
                            inverse_boundary->column_expression) ==
                        column_boundaries.end())
                    {
                        column_boundaries[inverse_boundary->column_expression] =
                            std::vector<std::shared_ptr<ColumnBoundary>>();
                    }
                    column_boundaries[inverse_boundary->column_expression]
                        .push_back(inverse_boundary);
                }
                if (column_boundaries.find(boundary->column_expression) ==
                    column_boundaries.end())
                {
                    column_boundaries[boundary->column_expression] =
                        std::vector<std::shared_ptr<ColumnBoundary>>();
                }
                column_boundaries[boundary->column_expression].push_back(
                    boundary);
            }
            else
            {
    ...
    // Store the highest lower bound and the lowest upper bound for a column in
    // order to get an optimal BetweenExpression
    ...
    // libc++ and libstdc++ have different orderings for unordered_map. This
    // results in nodes being inserted into the LQP in arbitrary order. While
    // these will be sorted by a different rule later, it can cause tests to
    // fail.
    auto column_boundaries_sorted =
        std::vector<std::vector<std::shared_ptr<ColumnBoundary>>>{};
    ...
    for (const auto &boundaries : column_boundaries_sorted)
    {
            ...
            if (!boundary->boundary_is_column_expression)
            {
                    ...
                    if (!upper_bound_value_expression ||
                        std::static_pointer_cast<ValueExpression>(
                            upper_bound_value_expression->border_expression)
                                ->value > boundary_border_expression->value)
                    {
                ...
                case ColumnBoundaryType::LowerBoundaryInclusive:
                    if (!lower_bound_value_expression ||
                        std::static_pointer_cast<ValueExpression>(
                            lower_bound_value_expression->border_expression)
                                ->value < boundary_border_expression->value)
                    {
        ...
        if (lower_bound_value_expression && upper_bound_value_expression)
        {
            ...
            const auto upper_value_expression =
                std::static_pointer_cast<ValueExpression>(
                    upper_bound_value_expression->border_expression);
            const auto between_node =
                PredicateNode::make(std::make_shared<BetweenExpression>(
                    get_between_predicate_condition(value_lower_inclusive,
                                                    value_upper_inclusive),
                    boundaries[0]->column_expression, lower_value_expression,
                    upper_value_expression));
        ...
        // Here, we could also generate BETWEEN expressions for when the lower
        // and upper bounds are LQPColumnExpressions. As the table scan does not
        // yet support that and will revert to the ExpressionEvaluator, we don't
        // do this and use two separate predicates instead.
    }

    // If no substitution was possible, all nodes referring to this column have
    // to be inserted into the LQP again later. Therefore we create a
    // semantically equal PredicateNode.
    for (const auto &boundaries : column_boundaries_sorted)
    {
        for (const auto &boundary : boundaries)
        {
            if (std::find(consumed_boundary_ids.begin(),
                          consumed_boundary_ids.end(),
                          boundary->id) == consumed_boundary_ids.end())
            {
                switch (boundary->type)
                {
                case ColumnBoundaryType::UpperBoundaryInclusive:
                    predicate_nodes.push_back(PredicateNode::make(
                        std::make_shared<BinaryPredicateExpression>(
                            PredicateCondition::LessThanEquals,
                            boundary->column_expression,
                            boundary->border_expression)));
                    break;
                case ColumnBoundaryType::LowerBoundaryInclusive:
                    predicate_nodes.push_back(PredicateNode::make(
                ...
                // Add the current boundary to consumed_boundary_ids so that we
                // do not also add the inverse
                consumed_boundary_ids.emplace_back(boundary->id);
            }
        }
    }

    // Append between nodes to PredicateNodes to get the complete chain of all
    // necessary LQP nodes
    predicate_nodes.insert(predicate_nodes.cend(), between_nodes.cbegin(),
                           between_nodes.cend());
    ...
    for (auto predicate_index = size_t{1};
         predicate_index < predicate_nodes.size(); ++predicate_index)
    {
        predicate_nodes[predicate_index]->set_left_input(
            predicate_nodes[predicate_index - 1]);
    }

    //  Connect last PredicateNode with all output nodes of the chain
    for (size_t output_index = 0;
         output_index < predicate_chain_output_nodes.size(); ++output_index)
    {
        predicate_chain_output_nodes[output_index]->set_input(
            predicate_chain_output_input_sides[output_index],
            predicate_nodes.back());
    }
}

/**
 * _get_boundary takes a BinaryPredicateExpression and the corresponding
 *PredicateNode as its input and returns a standardized ColumnBoundary. This
 *function checks where the LQPColumnExpression and the ValueExpression are
 *stored in the BinaryPredicateExpression. The expressions are transferred to a
 *standardized ColumnBoundary format and labelled with a ColumnBoundaryType that
 *depends on their positions and the predicate condition of the
 *BinaryPredicateExpression
 *
 **/
BetweenCompositionRule::ColumnBoundary BetweenCompositionRule::_get_boundary(
    const std::shared_ptr<BinaryPredicateExpression> &expression,
    const size_t expression_id)
{
    auto type = ColumnBoundaryType::None;
    const auto left_column_expression =
        std::dynamic_pointer_cast<LQPColumnExpression>(
            expression->left_operand());
    auto value_expression =
        std::dynamic_pointer_cast<ValueExpression>(expression->right_operand());
    ...
    if (left_column_expression && value_expression)
    {
        ...
        return {left_column_expression, value_expression, type, false,
                expression_id};
    }

    value_expression =
        std::dynamic_pointer_cast<ValueExpression>(expression->left_operand());
    ...
    if (value_expression && right_column_expression)
    {
        ...
        return {right_column_expression, value_expression, type, false,
                expression_id};
    }
    ...
    if (left_column_expression && right_column_expression)
    {
        ...
        return {left_column_expression, right_column_expression, type, true,
                expression_id};
    }
...
BetweenCompositionRule::ColumnBoundary
BetweenCompositionRule::_create_inverse_boundary(
    const std::shared_ptr<ColumnBoundary> &column_boundary)
{
    ...
    return {
        std::static_pointer_cast<LQPColumnExpression>(
            column_boundary->border_expression),
        std::static_pointer_cast<AbstractExpression>(
            column_boundary->column_expression),
        type,
        true,
        column_boundary->id,
    };
