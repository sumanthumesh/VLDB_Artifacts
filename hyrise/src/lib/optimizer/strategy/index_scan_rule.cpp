...
// Only if we expect num_output_rows <= num_input_rows * selectivity_threshold,
// the ScanType can be set to IndexScan. This threshold is kind of arbitrarily
// chosen, but the following paper suggests something similar: "Access Path
...
// Only if the number of input rows exceeds num_input_rows, the ScanType can be
// set to IndexScan. The threshold is taken from: "Fast Lookups for In-Memory
// Column Stores: Group-Key Indices, Lookup and Maintenance."
...
bool is_index_scan_applicable(
    const TableIndexStatistics &index_statistics,
    ...
    const std::shared_ptr<AbstractCostEstimator> &cost_estimator)
{
    ...
    const auto operator_predicates = OperatorScanPredicate::from_expression(
        *predicate_node->predicate(), *predicate_node);
    ...
    // There is no conceptual limitation to this rule with other predicate
    // conditions, but Hyrise's secondary indexes are hash-based and thus lack
    // support for other predicate conditions.
...
void IndexScanRule::_apply_to_plan_without_subqueries(
    const std::shared_ptr<AbstractLQPNode> &lqp_root) const
{
    DebugAssert(cost_estimator,
                "IndexScanRule requires cost estimator to be set.");
            ...
            if (node->type == LQPNodeType::Predicate)
            {
                ...
                if (child->type == LQPNodeType::StoredTable)
                {
                    ...
                    const auto stored_table_node =
                        std::static_pointer_cast<StoredTableNode>(child);

                    const auto &indexes_statistics =
                        stored_table_node->table_indexes_statistics();
                    for (const auto &index_statistics : indexes_statistics)
                    {
                        if (is_index_scan_applicable(index_statistics,
                                                     predicate_node,
                                                     cost_estimator))
                        {
