...
void AbstractRule::apply_to_plan(
    const std::shared_ptr<LogicalPlanRootNode> &lqp_root) const
{
    // (1) Optimize root LQP
    _apply_to_plan_without_subqueries(lqp_root);

    // (2) Optimize distinct subquery LQPs, one-by-one.
    auto subquery_expressions_by_lqp =
        collect_lqp_subquery_expressions_by_lqp(lqp_root);
    for (const auto &[lqp, subquery_expressions] : subquery_expressions_by_lqp)
    {
        if (std::all_of(subquery_expressions.cbegin(),
                        subquery_expressions.cend(),
        ...
        // (2.2) Assign optimized subplan to all corresponding
        // SubqueryExpressions
        ...
        // (2.3) Untie the root node before it goes out of scope so that the
        // outputs of the LQP remain correct.
