...
operator<<(BOOST_TEMPLATED_STREAM(ostream, E, T) & out, const NullValue &)
{
    out << "NULL";
    return out;
}
...
template <> struct is_pod<hyrise::NullValue> : mpl::true_
{
};

template <> struct is_empty<hyrise::NullValue> : mpl::true_
{
};

template <> struct is_stateless<hyrise::NullValue> : mpl::true_
{
};
