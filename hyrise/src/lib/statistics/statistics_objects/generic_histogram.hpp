...
/**
 * Generic histogram.
 * Bins do not necessarily share any common traits such as height or width or
 * distinct count. This histogram should only be used to create temporary
 * statistics objects, as its memory requirements are higher than those of other
 * histogram types.
 */
template <typename T> class GenericHistogram : public AbstractHistogram<T>
{
  public:
    ...
    with_single_bin(const T &min, const T &max,
...
std::ostream &operator<<(std::ostream &stream,
                         const GenericHistogram<T> &histogram)
{
