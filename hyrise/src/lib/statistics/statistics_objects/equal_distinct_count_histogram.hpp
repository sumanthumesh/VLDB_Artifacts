 ...
 * The first `bin_count_with_extra_value` contain each contain
 * `distinct_count_per_bin + 1` distinct values, all other bins contain
 * `distinct_count_per_bin` distinct values. There might be gaps between bins.
 */
template <typename T>
class EqualDistinctCountHistogram : public AbstractHistogram<T>
{
  public:
    ...
    EqualDistinctCountHistogram(std::vector<T> &&bin_minima,
    ...
    /**
     * Create an EqualDistinctCountHistogram for a column (spanning all
     * Segments) of a Table
     * @param max_bin_count   Desired number of bins. Less might be created, but
     * never more. Must not be zero.
     */
    static std::shared_ptr<EqualDistinctCountHistogram<T>>
    from_column(const Table &table, const ColumnID column_id,
     ...
     * creating a histogram. The number of bins is capped at the number of
     * distinct values in the segment. Otherwise, there would be empty bins
     * without any benefit.
     */
  ...
  private:
    /**
     * We use multiple vectors rather than a vector of structs for ease-of-use
     * with STL library functions.
     */
    ...
    // The first bin_count_with_extra_value bins have an additional distinct
    // value.
