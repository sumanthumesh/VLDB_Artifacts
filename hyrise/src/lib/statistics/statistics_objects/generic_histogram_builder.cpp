...
template <typename T>
GenericHistogramBuilder<T>::GenericHistogramBuilder(
    const size_t reserve_bin_count, const HistogramDomain<T> &domain)
    : _domain(domain)
{
...
template <typename T> bool GenericHistogramBuilder<T>::empty() const
{
    return _bin_minima.empty();
}

template <typename T>
void GenericHistogramBuilder<T>::add_bin(const T &min, const T &max,
                                         float height, float distinct_count)
{
    DebugAssert(_bin_minima.empty() || min > _bin_maxima.back(),
    ...
    _bin_distinct_counts.emplace_back(
        static_cast<HistogramCountType>(distinct_count));
}

template <typename T>
void GenericHistogramBuilder<T>::add_sliced_bin(
    const AbstractHistogram<T> &source, const BinID bin_id, const T &slice_min,
    const T &slice_max)
{
    DebugAssert(slice_max >= slice_min, "Invalid slice");
    DebugAssert(slice_min >= source.bin_minimum(bin_id),
                "Invalid slice minimum");
    DebugAssert(slice_max <= source.bin_maximum(bin_id),
                "Invalid slice maximum");

    const auto sliced_bin_ratio =
        source.bin_ratio_less_than_equals(bin_id, slice_max) -
        source.bin_ratio_less_than(bin_id, slice_min);
...
template <typename T>
void GenericHistogramBuilder<T>::add_copied_bins(
    const AbstractHistogram<T> &source, const BinID begin_bin_id,
