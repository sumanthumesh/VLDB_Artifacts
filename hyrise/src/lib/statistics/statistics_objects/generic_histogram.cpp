...
template <typename T>
GenericHistogram<T>::GenericHistogram(
      ...
      _bin_distinct_counts(std::move(bin_distinct_counts))
{
    ...
    _total_distinct_count =
        std::accumulate(_bin_distinct_counts.cbegin(),
                        _bin_distinct_counts.cend(), HistogramCountType{0});
}

template <typename T>
std::shared_ptr<GenericHistogram<T>> GenericHistogram<T>::with_single_bin(
    const T &min, const T &max, const HistogramCountType &height,
    const HistogramCountType &distinct_count, const HistogramDomain<T> &domain)
{
    return std::make_shared<GenericHistogram>(
        std::vector{min}, std::vector{max}, std::vector{height},
        std::vector{distinct_count}, domain);
}

template <typename T> std::string GenericHistogram<T>::name() const
{
    return "Generic";
}
...
template <typename T> BinID GenericHistogram<T>::bin_count() const
{
...
template <typename T>
HistogramCountType
GenericHistogram<T>::bin_distinct_count(const BinID index) const
{
    DebugAssert(index < _bin_distinct_counts.size(),
                "Index is not a valid bin.");
