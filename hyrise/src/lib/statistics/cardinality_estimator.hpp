...
template <typename T> class AttributeStatistics;
class AggregateNode;
class AliasNode;
class JoinNode;
class LimitNode;
...
class CardinalityEstimator : public AbstractCardinalityEstimator
{
  public:
    ...
    Cardinality estimate_cardinality(
        const std::shared_ptr<const AbstractLQPNode> &lqp) const override;
    ...
    /**
     * Estimate a simple scanning predicate. This function analyses the given
     * predicate and dispatches the actual estimation algorithm.
     */
    ...
    /**
     * Estimation of an equi scan between two histograms. Estimating equi scans
     * without correlation information is impossible, so this function is
     * restricted to computing an upper bound of the resulting histogram.
     */
    template <typename T>
    ...
    {
        /**
         * Column-to-column scan estimation is notoriously hard, selectivities
         * from 0 to 1 are possible for the same histogram pairs. Thus, we do
         * the most conservative estimation and compute the upper bound of
         * value- and distinct counts for each bin pair.
         */
        ...
        for (; left_idx < left_bin_count && right_idx < right_bin_count;)
        {
            ...
            DebugAssert(left_histogram.bin_maximum(left_idx) ==
                            right_histogram.bin_maximum(right_idx),
                        "Histogram bin boundaries do not match");

            const auto height = std::min(left_histogram.bin_height(left_idx),
                                         right_histogram.bin_height(right_idx));
            const auto distinct_count =
                std::min(left_histogram.bin_distinct_count(left_idx),
                         right_histogram.bin_distinct_count(right_idx));

            if (height > 0 && distinct_count > 0)
            {
                builder.add_bin(left_min, left_histogram.bin_maximum(left_idx),
                                height, distinct_count);
            }
     ...
     * @{
     */
    static std::shared_ptr<TableStatistics> estimate_inner_equi_join(
    ...
    static std::shared_ptr<TableStatistics>
    estimate_semi_join(const ColumnID left_column_id,
                       const ColumnID right_column_id,
                       const TableStatistics &left_input_table_statistics,
                       const TableStatistics &right_input_table_statistics);

    static std::shared_ptr<TableStatistics>
    estimate_cross_join(const TableStatistics &left_input_table_statistics,
                        const TableStatistics &right_input_table_statistics);
    ...
    {
        /**
         * left_histogram and right_histogram are turned into "unified"
         * histograms by `split_at_bin_bounds`, meaning that their bins are
         * split so that their bin boundaries match. E.g., if left_histogram has
         * a single bin [1, 10] and right histogram has a single bin [5, 20]
         * then unified_left_histogram == {[1, 4], [5, 10]}
         * unified_right_histogram == {[5, 10], [11, 20]}
         * The estimation is performed on overlapping bins only, e.g., only the
         * two bins [5, 10] will produce matches.
         */

        auto unified_left_histogram =
            left_histogram.split_at_bin_bounds(right_histogram.bin_bounds());
        auto unified_right_histogram =
            right_histogram.split_at_bin_bounds(left_histogram.bin_bounds());
        ...
        {
            const auto &left_min =
                unified_left_histogram->bin_minimum(left_idx);
            const auto &right_min =
                unified_right_histogram->bin_minimum(right_idx);
            ...
            DebugAssert(unified_left_histogram->bin_maximum(left_idx) ==
                            unified_right_histogram->bin_maximum(right_idx),
                        "Histogram bin boundaries do not match");

            // Overlapping bins found, estimate the join for these bins' range
            const auto [height, distinct_count] =
                estimate_inner_equi_join_of_bins( // NOLINT
                    unified_left_histogram->bin_height(left_idx),
                    unified_left_histogram->bin_distinct_count(left_idx),
                    unified_right_histogram->bin_height(right_idx),
                    unified_right_histogram->bin_distinct_count(right_idx));

            if (height > 0)
            {
                builder.add_bin(left_min,
                                unified_left_histogram->bin_maximum(left_idx),
                                height, distinct_count);
            }
    ...
    /**
     * Given two HistogramBins with equal bounds and the specified height and
     * distinct counts, estimate the number of matches and distinct values for
     * an equi-inner join of these two bins using a principle-of-inclusion
     * estimation.
     * @return {estimated_height, estimated_distinct_count}
     */
    static std::pair<HistogramCountType, HistogramCountType>
    estimate_inner_equi_join_of_bins(const float left_height,
