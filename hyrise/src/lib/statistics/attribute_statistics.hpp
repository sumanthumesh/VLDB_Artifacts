...
template <typename T> class MinMaxFilter;
template <typename T> class RangeFilter;
template <typename T> class CountingQuotientFilter;

/**
 * For docs, see BaseAttributeStatistics
 */
template <typename T> class AttributeStatistics : public BaseAttributeStatistics
{
  public:
    AttributeStatistics();

    void set_statistics_object(const std::shared_ptr<AbstractStatisticsObject>
                                   &statistics_object) override;
...
std::ostream &operator<<(std::ostream &stream,
                         const AttributeStatistics<T> &attribute_statistics)
{
    stream << "{" << std::endl;
    ...
    {
        stream << "NullValueRatio: "
               << attribute_statistics.null_value_ratio->ratio << std::endl;
    }

    if (attribute_statistics.distinct_value_count)
    {
        stream << "DistinctValueCount: "
               << attribute_statistics.distinct_value_count->count << std::endl;
    }
