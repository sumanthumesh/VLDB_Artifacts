...
template <typename T>
void create_pruning_statistics_for_segment(
    AttributeStatistics<T> &segment_statistics, const pmr_vector<T> &dictionary)
{
    if constexpr (std::is_arithmetic_v<T>)
    {
        segment_statistics.set_statistics_object(
            RangeFilter<T>::build_filter(dictionary));
    }
    else
    {
        if (!dictionary.empty())
        {
            segment_statistics.set_statistics_object(
                std::make_shared<MinMaxFilter<T>>(dictionary.front(),
                                                  dictionary.back()));
        }
    }

    segment_statistics.set_statistics_object(
        std::make_shared<DistinctValueCount>(dictionary.size()));
}
...
void generate_chunk_pruning_statistics(const std::shared_ptr<Chunk> &chunk)
{
    ...
    {
        // Pruning statistics should be stable no matter what encoding or sort
        // order is used. Hence, when they are present they are up to date and
            ...
            *segment,
            [&](auto type, auto &typed_segment)
            {
                ...
                if constexpr (std::is_same_v<SegmentType,
                                             DictionarySegment<ColumnDataType>>)
                {
                    // we can use the fact that dictionary segments have an
                    // accessor for the dictionary
                    const auto &dictionary = *typed_segment.dictionary();
                    create_pruning_statistics_for_segment(*segment_statistics,
                                                          dictionary);
                }
                else
                {
                    // if we have a generic segment we create the dictionary
                    // ourselves
                    auto iterable =
                    ...
                    pmr_vector<ColumnDataType> dictionary{values.cbegin(),
                                                          values.cend()};
