...
#define GET_ELEM(s, index, elem)                                               \
    BOOST_PP_TUPLE_ELEM(NUM_DATA_TYPES, index, elem)
...
#define DATA_TYPE_ENUM_VALUES                                                  \
    BOOST_PP_SEQ_TRANSFORM(GET_ELEM, 1, DATA_TYPE_INFO)
...
static constexpr auto data_type_enum_values =
    hana::make_tuple(BOOST_PP_SEQ_ENUM(BOOST_PP_SEQ_TRANSFORM(
        APPEND_ENUM_NAMESPACE, _, DATA_TYPE_ENUM_VALUES)));
...
static constexpr auto data_type_enum_string_pairs = hana::transform(
    hana::zip(data_type_enum_values, data_type_strings), to_pair);

// Prepends NullValue to tuple of types
static constexpr auto data_types_including_null =
    hana::prepend(data_types, hana::type_c<NullValue>);

// Converts tuple to mpl vector
using TypesAsMplVector = decltype(hana::to<hana::ext::boost::mpl::vector_tag>(
    data_types_including_null));

// Creates boost::variant from mpl vector
using AllTypeVariant =
    typename boost::make_variant_over<detail::TypesAsMplVector>::type;

} // namespace detail

static constexpr auto data_types = detail::data_types;
static constexpr auto data_types_including_null =
    detail::data_types_including_null;
static constexpr auto data_type_pairs = detail::data_type_enum_pairs;
static constexpr auto data_type_enum_string_pairs =
    detail::data_type_enum_string_pairs;
...
const auto data_type_to_string = hana::fold(
    data_type_enum_string_pairs, boost::bimap<DataType, std::string>{},
    [](auto map, auto pair)
    {
...
/**
 * Notes:
 *   – Use this instead of AllTypeVariant{}, AllTypeVariant{NullValue{}},
 * NullValue{}, etc. whenever a null value needs to be represented
 *   - comparing any AllTypeVariant to NULL_VALUE returns false in accordance
 * with the ternary logic
...
#define EXPLICIT_DECLARATION(r, template_class, type)                          \
    extern template class template_class<type>;

// Explicitly declares the given template class for all types in DATA_TYPES
// (used in .hpp)
...
template <> struct hash<hyrise::AllTypeVariant>
{
