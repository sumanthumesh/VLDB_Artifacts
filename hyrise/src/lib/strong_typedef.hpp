...
#define STRONG_TYPEDEF(T, D)                                                   \
    ...
    {                                                                          \
        typedef T base_type;                                                   \
        T t;                                                                   \
        constexpr explicit D(const T &t_)                                      \
    ...
    namespace std                                                              \
    {                                                                          \
    template <> struct hash<::hyrise::D>                                       \
    {                                                                          \
        size_t operator()(const ::hyrise::D &x) const { return hash<T>{}(x); } \
    };                                                                         \
    template <>                                                                \
                                                                               \
    struct numeric_limits<::hyrise::D>                                         \
    {                                                                          \
        static typename std::enable_if_t<std::is_arithmetic_v<T>, ::hyrise::D> \
        min()                                                                  \
        {                                                                      \
