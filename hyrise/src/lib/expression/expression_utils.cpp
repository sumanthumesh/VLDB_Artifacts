...
bool expressions_equal(
    const std::vector<std::shared_ptr<AbstractExpression>> &expressions_a,
    const std::vector<std::shared_ptr<AbstractExpression>> &expressions_b)
{
    return std::equal(expressions_a.begin(), expressions_a.end(),
                      expressions_b.begin(), expressions_b.end(),
    ...
    const LQPNodeMapping &node_mapping)
{
    ...
    for (auto expression_idx = size_t{0};
         expression_idx < expressions_left_count; ++expression_idx)
    {
        const auto &expression_left = *expressions_left[expression_idx];
        const auto &expression_right = *expressions_right[expression_idx];

        if (!expression_equal_to_expression_in_different_lqp(
                expression_left, expression_right, node_mapping))
        {
            return false;
        }
    }
...
bool expression_equal_to_expression_in_different_lqp(
    const AbstractExpression &expression_left,
    const AbstractExpression &expression_right,
    const LQPNodeMapping &node_mapping)
{
    /**
     * Compare expression_left to expression_right by creating a deep copy of
     * expression_left and adapting it to the LQP of expression_right, then
     * perform a normal comparison of two expressions in the same LQP.
     */
    ...
    const ExpressionUnorderedMap<std::shared_ptr<AbstractExpression>> &mapping)
{
    visit_expression(expression,
                     [&](auto &sub_expression)
                     {
                         const auto replacement_iter =
                             mapping.find(sub_expression);
...
expressions_copy_and_adapt_to_different_lqp(
    const std::vector<std::shared_ptr<AbstractExpression>> &expressions,
    const LQPNodeMapping &node_mapping)
{
    std::vector<std::shared_ptr<AbstractExpression>> copied_expressions;
    ...
    for (const auto &expression : expressions)
    {
        copied_expressions.emplace_back(
            expression_copy_and_adapt_to_different_lqp(*expression,
                                                       node_mapping));
    }
...
expression_copy_and_adapt_to_different_lqp(const AbstractExpression &expression,
                                           const LQPNodeMapping &node_mapping)
{
    ...
    const LQPNodeMapping &node_mapping)
{
    visit_expression(expression,
                     [&](auto &expression_ptr)
                     {
                         ...
                         const auto lqp_column_expression_ptr =
                             std::dynamic_pointer_cast<LQPColumnExpression>(
                                 expression_ptr);
                         Assert(lqp_column_expression_ptr,
                                "Asked to adapt expression in LQP, but "
                                "encountered non-LQP ColumnExpression");

                         expression_ptr = expression_adapt_to_different_lqp(
                             *lqp_column_expression_ptr, node_mapping);
    ...
    const LQPNodeMapping &node_mapping)
{
    ...
    Assert(node_mapping_iter != node_mapping.end(),
           "Couldn't find referenced node (" + node->description() +
               ") in NodeMapping");

    return std::make_shared<LQPColumnExpression>(
        node_mapping_iter->second, lqp_column_expression.original_column_id);
}

std::string expression_descriptions(
    const std::vector<std::shared_ptr<AbstractExpression>> &expressions,
    const AbstractExpression::DescriptionMode mode)
{
    ...
    for (auto expression_idx = size_t{1}; expression_idx < expressions.size();
         ++expression_idx)
    {
...
{
    Assert(lhs != DataType::Null || rhs != DataType::Null,
           "Can't deduce common type if both sides are NULL");
    ...
    {
        return is_floating_point_data_type(rhs) ? DataType::Double
                                                : DataType::Long;
    }

    if (rhs == DataType::Long)
    {
        return is_floating_point_data_type(lhs) ? DataType::Double
                                                : DataType::Long;
    }
...
bool expression_evaluable_on_lqp(
    ...
    const AbstractLQPNode &lqp)
{
    ...
    visit_expression(
        ...
        [&](const auto &sub_expression)
        {
            ...
            {
                // COUNT(*) needs special treatment. Because its argument is the
                // invalid column id, it is not part of any node's
                // output_expressions. Check if sub_expression is COUNT(*) - if
                // yes, ignore the INVALID_COLUMN_ID and verify that its
                // original_node is part of lqp.
                const auto &aggregate_expression =
                    static_cast<const WindowFunctionExpression &>(
                        *sub_expression);
                const auto &lqp_column_expression =
                    static_cast<const LQPColumnExpression &>(
                        *aggregate_expression.argument());
                ...
                evaluable = false;
                visit_lqp(lqp.shared_from_this(),
                          [&](const auto &sub_lqp)
                          {
        ...
        [&](const auto &sub_expression)
        {
            if (sub_expression->type == ExpressionType::Logical)
            {
                const auto logical_expression =
                    std::static_pointer_cast<LogicalExpression>(sub_expression);
    ...
    const LogicalOperator logical_operator)
{
    ...
    for (auto expression_idx = size_t{1}; expression_idx < expression_count;
         ++expression_idx)
    {
        inflated = std::make_shared<LogicalExpression>(
            logical_operator, inflated, expressions[expression_idx]);
    }

    return inflated;
}

void expression_set_parameters(
    const std::shared_ptr<AbstractExpression> &expression,
    ...
    visit_expression(
        ...
        [&](auto &sub_expression)
        {
            if (auto correlated_parameter_expression =
                    std::dynamic_pointer_cast<CorrelatedParameterExpression>(
                        sub_expression))
            {
                const auto value_iter = parameters.find(
                    correlated_parameter_expression->parameter_id);
                if (value_iter != parameters.end())
                {
                    correlated_parameter_expression->set_value(
                        value_iter->second);
                }
                return ExpressionVisitation::DoNotVisitArguments;
            }

            if (const auto pqp_subquery_expression =
...
void expressions_set_parameters(
    const std::vector<std::shared_ptr<AbstractExpression>> &expressions,
    const std::unordered_map<ParameterID, AllTypeVariant> &parameters)
{
    for (const auto &expression : expressions)
...
void expression_set_transaction_context(
    const std::shared_ptr<AbstractExpression> &expression,
    ...
    visit_expression(
        ...
        [&](auto &sub_expression)
        {
            ...
            const auto pqp_subquery_expression =
                std::dynamic_pointer_cast<PQPSubqueryExpression>(
                    sub_expression);
            Assert(pqp_subquery_expression,
                   "Expected a PQPSubqueryExpression here");
            pqp_subquery_expression->pqp->set_transaction_context_recursively(
                transaction_context);
...
void expressions_set_transaction_context(
    const std::vector<std::shared_ptr<AbstractExpression>> &expressions,
    const std::weak_ptr<TransactionContext> &transaction_context)
{
    for (const auto &expression : expressions)
...
bool expression_contains_placeholder(
    const std::shared_ptr<AbstractExpression> &expression)
{
    ...
    visit_expression(expression,
                     ...
                     {
                         placeholder_found |=
                             std::dynamic_pointer_cast<PlaceholderExpression>(
...
bool expression_contains_correlated_parameter(
    const std::shared_ptr<AbstractExpression> &expression)
{
...
std::optional<AllTypeVariant>
expression_get_value_or_parameter(const AbstractExpression &expression)
{
    if (const auto *correlated_parameter_expression =
            dynamic_cast<const CorrelatedParameterExpression *>(&expression))
    {
        DebugAssert(correlated_parameter_expression->value(),
                    "CorrelatedParameterExpression doesn't have a value set");
    ...
    if (expression.type == ExpressionType::Cast)
    {
        ...
        Assert(expression.data_type() != DataType::Null,
        ...
        const auto &value_expression =
            static_cast<const ValueExpression &>(*cast_expression.argument());
        ...
        resolve_data_type(
            ...
            [&](auto type)
            {
                ...
                try
                {
                    // lossy_variant_cast returns std::nullopt when it casts
                    // from a NULL value. We have handled this above.
                    result = *lossy_variant_cast<TargetDataType>(
                        value_expression.value);
                }
                catch (boost::bad_lexical_cast &)
                {
                    Fail("Cannot cast " +
                         cast_expression.argument()->as_column_name() + " as " +
                         std::string{
...
find_pqp_subquery_expressions(
    const std::shared_ptr<AbstractExpression> &expression)
{
    if (const auto pqp_subquery_expression =
            std::dynamic_pointer_cast<PQPSubqueryExpression>(expression))
    {
        // Quick Path
...
std::optional<ColumnID> find_expression_idx(
    const AbstractExpression &search_expression,
    const std::vector<std::shared_ptr<AbstractExpression>> &expression_vector)
{
    const auto num_expressions = expression_vector.size();
    for (auto expression_id = ColumnID{0}; expression_id < num_expressions;
         ++expression_id)
    {
...
bool contains_all_expressions(
    const ExpressionContainer &search_expressions,
    const std::vector<std::shared_ptr<AbstractExpression>> &expression_vector)
{
...
template bool
contains_all_expressions<std::vector<std::shared_ptr<AbstractExpression>>>(
