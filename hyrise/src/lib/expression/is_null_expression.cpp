...
{
    Assert(predicate_condition == PredicateCondition::IsNull ||
               predicate_condition == PredicateCondition::IsNotNull,
           "IsNullExpression only supports PredicateCondition::IsNull and "
...
{
    return std::make_shared<IsNullExpression>(predicate_condition,
                                              operand()->deep_copy(copied_ops));
}
...
bool IsNullExpression::_on_is_nullable_on_lqp(
