...
ListExpression::ListExpression(
    const std::vector<std::shared_ptr<AbstractExpression>> &elements)
...
{
    Fail("A ListExpression doesn't have a single type, each of its elements "
         "might have a different type");
}

const std::vector<std::shared_ptr<AbstractExpression>> &
ListExpression::elements() const
{
    return arguments;
}
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    return std::make_shared<ListExpression>(
        expressions_deep_copy(arguments, copied_ops));
}
