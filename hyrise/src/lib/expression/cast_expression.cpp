...
CastExpression::CastExpression(
    ...
    : AbstractExpression(ExpressionType::Cast, {argument}),
...
{
    return std::make_shared<CastExpression>(argument()->deep_copy(copied_ops),
                                            _data_type);
}
...
bool CastExpression::_shallow_equals(const AbstractExpression &expression) const
{
    ...
    const auto &other_cast_expression =
...
size_t CastExpression::_shallow_hash() const
{
    ...
    return std::hash<DataTypeUnderlyingType>{}(
        static_cast<DataTypeUnderlyingType>(_data_type));
}
