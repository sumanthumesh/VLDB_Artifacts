...
/**
 * Supported window/aggregate functions. In addition to the default SQL
 * functions (e.g., MIN(), MAX()), Hyrise internally uses the ANY() function,
 * which expects all values in the group to be equal and returns that value. In
 * SQL terms, this would be an additional, but unnecessary GROUP BY column. This
 * function is only used by the optimizer in case that all values of the group
 * are known to be equal (see DependentGroupByReductionRule).
 */
...
const auto aggregate_functions =
    std::unordered_set<WindowFunction>{WindowFunction::Min,
                                       WindowFunction::Max,
                                       WindowFunction::Sum,
                                       WindowFunction::Avg,
...
/**
 * SQL Window/aggregate functions with optionally defined window. Window
 * functions are a superset of aggregate functions: Each aggregate function is a
 * window function when defining a window. Thus, we keep it simple and use
 *  aggregate functions as window functions without a window and a certain
 * function type.
 */
