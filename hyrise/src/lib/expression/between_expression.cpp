...
BetweenExpression::BetweenExpression(
    ...
    : AbstractPredicateExpression(init_predicate_condition,
                                  {operand, lower_bound, upper_bound})
{
...
BetweenExpression::lower_bound() const
{
    return arguments[1];
}

const std::shared_ptr<AbstractExpression> &
BetweenExpression::upper_bound() const
{
    return arguments[2];
}
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    return std::make_shared<BetweenExpression>(
        predicate_condition, operand()->deep_copy(copied_ops),
...
std::string BetweenExpression::description(const DescriptionMode mode) const
{
    ...
    stream << _enclose_argument(*operand(), mode) << " " << predicate_condition
           << " " << _enclose_argument(*lower_bound(), mode) << " AND "
           << _enclose_argument(*upper_bound(), mode);
    return stream.str();
}
