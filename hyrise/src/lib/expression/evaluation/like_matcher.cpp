...
size_t LikeMatcher::get_index_of_next_wildcard(const pmr_string &pattern,
                                               const size_t offset)
{
...
LikeMatcher::PatternTokens
LikeMatcher::pattern_string_to_tokens(const pmr_string &pattern)
{
    ...
    while (current_position < pattern.size())
    {
        ...
        {
            const auto next_wildcard_position =
                get_index_of_next_wildcard(pattern, current_position);
            const auto token_length =
                next_wildcard_position == pmr_string::npos
                    ? pmr_string::npos
                    : next_wildcard_position - current_position;
...
std::optional<std::pair<pmr_string, pmr_string>>
LikeMatcher::bounds(const pmr_string &pattern)
{
    ...
    const auto next_ascii_character =
        static_cast<char>(last_character_of_lower_bound + 1);
    const auto upper_bound =
        lower_bound.substr(0, lower_bound.size() - 1) + next_ascii_character;

    return std::pair<pmr_string, pmr_string>(lower_bound, upper_bound);
}

LikeMatcher::AllPatternVariant
LikeMatcher::pattern_string_to_pattern_variant(const pmr_string &pattern)
{
    ...
    if (tokens.size() == 3 && tokens[0] == PatternToken{Wildcard::AnyChars} &&
        std::holds_alternative<pmr_string>(tokens[1]) &&
        tokens[2] == PatternToken{Wildcard::AnyChars})
    {
        // Pattern has the form '%hello%'
    ...
    /**
     * Pattern is either MultipleContainsPattern, e.g.,
     * '%hello%world%how%are%you%' or we fall back to using a regex matcher.
     *
     * A MultipleContainsPattern begins and ends with '%' and  contains only
     * strings and '%'.
     */

    // Pick ContainsMultiple or Regex
    auto pattern_is_contains_multiple =
        true; // Set to false if tokens don't match %(, string, %)* pattern
    auto strings =
        std::vector<pmr_string>{}; // arguments used for ContainsMultiple, if it
                                   // gets used
    auto expect_any_chars =
        true; // If true, expect '%', if false, expect a string

    // Check if the tokens match the layout expected for MultipleContainsPattern
    // - or break and set pattern_is_contains_multiple to false once they don't
...
{
    // Do substitution of <backslash> with <backslash><backslash> FIRST, because
    // otherwise it will also replace backslashes introduced by the other
    // substitutions
    constexpr auto REPLACE_BY =
        std::array<std::pair<const char *, const char *>, 14u>{{{"\\", "\\\\"},
                                                                {".", "\\."},
                                                                {"^", "\\^"},
                                                                {"$", "\\$"},
                                                                {"+", "\\+"},
