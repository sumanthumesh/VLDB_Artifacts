 ...
 */
template <typename T>
constexpr bool is_logical_operand =
    std::is_same_v<int32_t, T> || std::is_same_v<NullValue, T>;
...
{
    template <typename Result, typename ArgA, typename ArgB> struct supports
    {
        static constexpr bool value = is_logical_operand<Result> &&
                                      is_logical_operand<ArgA> &&
                                      is_logical_operand<ArgB>;
    };

    template <typename Result, typename ArgA, typename ArgB>
    void operator()(Result &result_value, bool &result_null,
                    const ArgA &a_value, const bool a_null, const ArgB &b_value,
                    const bool b_null)
    {
...
/**
 * Wrap a STL comparison operator (std::equal_to, ...) so that it exposes a
 * supports::value member and supports NullValues as argument
 */
...
struct STLComparisonFunctorWrapper
{
    template <typename Result, typename ArgA, typename ArgB> struct supports
    {
        static constexpr bool value = std::is_same_v<int32_t, Result> &&
                                      ...
                                      (!std::is_same_v<pmr_string, ArgB> ||
                                       (std::is_same_v<NullValue, ArgA> ||
                                        std::is_same_v<pmr_string, ArgA>));
    };

    template <typename Result, typename ArgA, typename ArgB>
    inline static constexpr bool supports_v =
        supports<Result, ArgA, ArgB>::value;
    ...
    void operator()(Result &result, const ArgA &a, const ArgB &b)
    {
        ...
        {
            result =
                static_cast<Result>(Functor<std::common_type_t<ArgA, ArgB>>{}(
                    static_cast<std::common_type_t<ArgA, ArgB>>(a),
                    static_cast<std::common_type_t<ArgA, ArgB>>(b)));
        }
    }
};
...
struct STLArithmeticFunctorWrapper
{
    template <typename Result, typename ArgA, typename ArgB> struct supports
    {
        static constexpr bool value = !std::is_same_v<pmr_string, Result> &&
                                      !std::is_same_v<pmr_string, ArgA> &&
                                      !std::is_same_v<pmr_string, ArgB>;
    };
    ...
    void operator()(Result &result, const ArgA &a, const ArgB &b)
    {
        if constexpr (std::is_same_v<NullValue, Result> ||
                      std::is_same_v<NullValue, ArgA> ||
                      std::is_same_v<NullValue, ArgB>)
        {
            result = Result{};
...
// Modulo selects between the operator `%` for integrals and std::fmod() for
// floats. Custom NULL logic returns NULL if the divisor is NULL.
struct ModuloEvaluator
{
                    ...
                    const bool b_null)
    {
        ...
        else
        {
            ...
            {
                if constexpr (std::is_integral_v<ArgA> &&
                              std::is_integral_v<ArgB>)
                {
                ...
                {
                    result_value =
                        static_cast<Result>(std::fmod(a_value, b_value));
                }
            }
        }
    }
...
struct DivisionEvaluator
{
                    ...
                    const bool b_null)
    {
        ...
        else
        {
            ...
            {
                result_value =
                    static_cast<Result>(static_cast<Result>(a_value) /
                                        static_cast<Result>(b_value));
            }
        }
    }
};
...
struct CaseEvaluator
{
    template <typename Result, typename ArgA, typename ArgB> struct supports
    {
        static constexpr bool value =
            (std::is_same_v<pmr_string, ArgA> ==
             std::is_same_v<pmr_string,
                            ArgB>)&&(std::is_same_v<pmr_string, ArgA> ==
                                     std::is_same_v<pmr_string, Result>);
    };
