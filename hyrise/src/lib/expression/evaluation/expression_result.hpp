 ...
 * Wraps a vector of `values` and a vector of `nulls` that are filled
 * differently, with the possible combinations best explained by the examples
 * below
 *
 * values
 ...
 *      Contains a bool for each element of `values` if the ExpressionResult is
 * nullable Contains a single element that the determines whether all elements
 * are either null or not
 *
 * Examples:
 *      {values: [1, 2, 3, 4]; nulls: []} --> Series [1, 2, 3, 4]
 *      {values: [1, 2, 3, 4]; nulls: [false]} --> Series [1, 2, 3, 4]
 *      {values: [1, 2, 3, 4]; nulls: [true]} --> Literal [NULL]
 *      {values: [1, 2, 3, 4]; nulls: [true, false, true, false]} --> Series
 * [NULL, 2, NULL, 4] {values: [1]; nulls: []} --> Literal [1] {values: [1];
 * nulls: [true]} --> Literal [NULL]
 *
 * Often the ExpressionEvaluator will compute nulls and values independently,
 * which is why states with redundant information, such as `{values: [1, 2, 3,
 * 4]; nulls: [true]}` or `{values: [1, 2, 3, 4]; nulls: [false]}`, are legal.
 */
template <typename T> class ExpressionResult : public BaseExpressionResult
{
  public:
    using Type = T;
    ...
    explicit ExpressionResult(pmr_vector<T> init_values,
                              pmr_vector<bool> init_nulls = {})
        : values(std::move(init_values)), nulls(std::move(init_nulls))
    {
        // Allowed size of nulls: 0 (not nullable)
        //                        1 (nullable, all values are NULL or NOT NULL,
        //                        depending on the value) n (same as values, 1:1
        //                        mapping)
        DebugAssert(nulls.empty() || nulls.size() == 1 ||
    ...
    {
        DebugAssert(size() == 1 || idx < size(),
                    "Invalid ExpressionResult access");
        return values[std::min(idx, values.size() - 1)];
    }

    AllTypeVariant value_as_variant(const size_t idx) const final
    {
        return is_null(idx) ? AllTypeVariant{NullValue{}}
                            : AllTypeVariant{value(idx)};
    }
    ...
    /**
     * Resolve ExpressionResult<T> to ExpressionResultNullableSeries<T>,
     * ExpressionResultNonNullSeries<T> or ExpressionResultLiteral<T>
     *
     * Once resolved, a View doesn't need to do bounds checking when queried for
     * value() or is_null(), thus reducing overhead
     */
    template <typename Functor> void as_view(const Functor &fn) const
    {
        if (size() == 1)
        {
            fn(ExpressionResultLiteral(values.front(),
                                       is_nullable() && nulls.front()));
        }
