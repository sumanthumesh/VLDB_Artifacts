...
WindowFunctionExpression::WindowFunctionExpression(
    const WindowFunction init_window_function,
    const std::shared_ptr<AbstractExpression> &argument,
    const std::shared_ptr<AbstractExpression> &window)
    : AbstractExpression{ExpressionType::WindowFunction,
                         {/* Expressions are set below. */}},
      window_function(init_window_function)
{
    ...
    if (window)
    {
        ...
        Assert(window->type == ExpressionType::Window,
               "Window description must be a WindowExpression");
        _has_window = true;
    }
...
std::string
WindowFunctionExpression::description(const DescriptionMode mode) const
{
    ...
    else if (is_count_star(*this))
    {
        ...
        else
        {
            ...
            DebugAssert(
                column_expression,
                "Expected aggregate argument to be LQPColumnExpression");
...
{
    if (window_function == WindowFunction::Count ||
        ...
        !aggregate_functions.contains(window_function))
    {
        ...
        case WindowFunction::Count:
            return WindowFunctionTraits<NullValue,
                                        WindowFunction::Count>::RESULT_TYPE;
        case WindowFunction::CountDistinct:
            return WindowFunctionTraits<
                NullValue, WindowFunction::CountDistinct>::RESULT_TYPE;
        case WindowFunction::CumeDist:
            return WindowFunctionTraits<NullValue,
                                        WindowFunction::CumeDist>::RESULT_TYPE;
        case WindowFunction::DenseRank:
            return WindowFunctionTraits<NullValue,
                                        WindowFunction::DenseRank>::RESULT_TYPE;
        case WindowFunction::PercentRank:
            return WindowFunctionTraits<
                NullValue, WindowFunction::PercentRank>::RESULT_TYPE;
        case WindowFunction::Rank:
            return WindowFunctionTraits<NullValue,
                                        WindowFunction::Rank>::RESULT_TYPE;
        case WindowFunction::RowNumber:
            return WindowFunctionTraits<NullValue,
                                        WindowFunction::RowNumber>::RESULT_TYPE;
        default:
    ...
    resolve_data_type(
        ...
        [&](const auto data_type_t)
        {
            ...
            case WindowFunction::Min:
                result_type =
                    WindowFunctionTraits<AggregateDataType,
                                         WindowFunction::Min>::RESULT_TYPE;
                break;
            case WindowFunction::Max:
                result_type =
                    WindowFunctionTraits<AggregateDataType,
                                         WindowFunction::Max>::RESULT_TYPE;
                break;
            case WindowFunction::Avg:
                result_type =
                    WindowFunctionTraits<AggregateDataType,
                                         WindowFunction::Avg>::RESULT_TYPE;
                break;
            case WindowFunction::Sum:
                result_type =
                    WindowFunctionTraits<AggregateDataType,
                                         WindowFunction::Sum>::RESULT_TYPE;
                break;
            case WindowFunction::StandardDeviationSample:
                result_type = WindowFunctionTraits<
                    AggregateDataType,
                    WindowFunction::StandardDeviationSample>::RESULT_TYPE;
                break;
            case WindowFunction::Any:
                result_type =
                    WindowFunctionTraits<AggregateDataType,
                                         WindowFunction::Any>::RESULT_TYPE;
                break;
            case WindowFunction::Count:
            case WindowFunction::CountDistinct:
            case WindowFunction::CumeDist:
...
bool WindowFunctionExpression::is_count_star(
    const AbstractExpression &expression)
{
    // COUNT(*) is represented by an WindowFunctionExpression with the COUNT
    // function and an INVALID_COLUMN_ID.
    ...
    const auto &lqp_column_expression =
        static_cast<LQPColumnExpression &>(*aggregate_expression.argument());
...
{
    const auto &argument = this->argument();
    const auto argument_copy =
        argument ? argument->deep_copy(copied_ops) : nullptr;
    const auto window_copy =
        _has_window ? arguments.back()->deep_copy(copied_ops) : nullptr;
    return std::make_shared<WindowFunctionExpression>(
        window_function, argument_copy, window_copy);
}

bool WindowFunctionExpression::_shallow_equals(
    const AbstractExpression &expression) const
{
    DebugAssert(dynamic_cast<const WindowFunctionExpression *>(&expression),
                "Different expression type should have been caught by "
...
bool WindowFunctionExpression::_on_is_nullable_on_lqp(
    const AbstractLQPNode & /*lqp*/) const
{
    ...
    // Aggregates (except COUNT and COUNT DISTINCT) will return NULL when
    // executed on an empty group. Thus, they are always nullable.
    return window_function != WindowFunction::Count &&
           window_function != WindowFunction::CountDistinct;
}
