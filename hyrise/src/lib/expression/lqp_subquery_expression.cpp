...
LQPSubqueryExpression::parameter_expression(const size_t parameter_idx) const
{
    ...
    const
{
    ...
    return std::make_shared<LQPSubqueryExpression>(
        lqp_copy, parameter_ids, expressions_deep_copy(arguments));
}
...
std::string LQPSubqueryExpression::description(const DescriptionMode mode) const
{
    ...
    {
        stream << ", Parameters: ";
        for (auto parameter_idx = size_t{0}; parameter_idx < arguments.size();
             ++parameter_idx)
        {
            stream << "[" << arguments[parameter_idx]->description(mode)
                   << ", id=" << parameter_ids[parameter_idx] << "]";
...
DataType LQPSubqueryExpression::data_type() const
{
    ...
    Assert(output_expressions.size() == 1,
           "Can only determine the DataType of SubqueryExpressions that return "
           "exactly one column");
    return output_expressions[0]->data_type();
}

bool LQPSubqueryExpression::_on_is_nullable_on_lqp(
    const AbstractLQPNode & /*node*/) const
{
    Assert(lqp->output_expressions().size() == 1,
           "Can only determine the nullability of SelectExpressions that "
...
bool LQPSubqueryExpression::_shallow_equals(
    const AbstractExpression &expression) const
{
    ...
    const auto &subquery_expression =
        static_cast<const LQPSubqueryExpression &>(expression);
    return *lqp == *subquery_expression.lqp &&
           parameter_ids == subquery_expression.parameter_ids;
}

size_t LQPSubqueryExpression::_shallow_hash() const
{
    // Return AbstractExpression::_shallow_hash() (i.e., 0), thus forcing a hash
    // collision for LQPSubqueryExpressions and triggering a full equality
    // check. Though we often hash entire query plans, we expect most plans to
    // contain only few LQPSubqueryExpressions. Thus, these hash collisions
    // should be fine.
