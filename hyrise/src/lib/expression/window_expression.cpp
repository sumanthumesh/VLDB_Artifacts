...
FrameBound::FrameBound(const size_t init_offset, const FrameBoundType init_type,
                       const bool init_unbounded)
...
FrameDescription::FrameDescription(const FrameType init_type,
                                   const FrameBound &init_start,
                                   const FrameBound &init_end)
...
std::ostream &operator<<(std::ostream &stream,
                         const FrameDescription &frame_description)
{
...
WindowExpression::WindowExpression(
    ...
    FrameDescription &&init_frame_description)
    : AbstractExpression{ExpressionType::Window,
                         {/* Expressions added below. */}},
      sort_modes{init_sort_modes}, frame_description{init_frame_description},
      order_by_expressions_begin_idx{partition_by_expressions.size()}
{
    ...
    Assert(order_by_expression_count == sort_modes.size(),
           "Passed sort modes do not match ORDER BY expressions.");

    arguments.resize(order_by_expressions_begin_idx +
                     order_by_expression_count);
    std::copy(partition_by_expressions.begin(), partition_by_expressions.end(),
              arguments.begin());
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    ...
    const auto order_by_expression_count =
        expression_count - order_by_expressions_begin_idx;
    auto partition_by_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>(
            order_by_expressions_begin_idx);
    auto order_by_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>(
            order_by_expression_count);

    for (auto expression_idx = size_t{0};
         expression_idx < order_by_expressions_begin_idx; ++expression_idx)
    {
        partition_by_expressions[expression_idx] =
            arguments[expression_idx]->deep_copy(copied_ops);
    }

    for (auto expression_idx = size_t{0};
         expression_idx < order_by_expression_count; ++expression_idx)
    {
        order_by_expressions[expression_idx] =
            arguments[order_by_expressions_begin_idx + expression_idx]
                ->deep_copy(copied_ops);
    }
    auto frame_description_copy = frame_description;
    auto sort_modes_copy = sort_modes;

    return std::make_shared<WindowExpression>(
...
std::string WindowExpression::description(const DescriptionMode mode) const
{
    ...
    if (order_by_expressions_begin_idx > 0)
    {
        ...
        for (auto expression_idx = size_t{1};
             expression_idx < order_by_expressions_begin_idx; ++expression_idx)
        {
    ...
    {
        stream << "ORDER BY "
               << arguments[order_by_expressions_begin_idx]->description(mode)
               << " " << sort_modes[0];

        for (auto expression_idx = order_by_expressions_begin_idx + 1;
             expression_idx < expression_count; ++expression_idx)
        {
            stream
                << ", " << arguments[expression_idx]->description(mode) << " "
...
{
    Fail("WindowExpression must only be used as descriptor for "
         "WindowFunctionExpression.");
}

bool WindowExpression::_shallow_equals(
    const AbstractExpression &expression) const
{
    ...
    return sort_modes == rhs.sort_modes &&
           frame_description == rhs.frame_description;
}
...
bool WindowExpression::_on_is_nullable_on_lqp(
