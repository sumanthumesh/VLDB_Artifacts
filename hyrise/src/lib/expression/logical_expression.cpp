...
LogicalExpression::LogicalExpression(
    const LogicalOperator init_logical_operator,
    const std::shared_ptr<AbstractExpression> &left_operand,
    const std::shared_ptr<AbstractExpression> &right_operand)
    : AbstractExpression(ExpressionType::Logical,
...
LogicalExpression::left_operand() const
{
    return arguments[0];
}

const std::shared_ptr<AbstractExpression> &
LogicalExpression::right_operand() const
{
    return arguments[1];
}
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    return std::make_shared<LogicalExpression>(
        logical_operator, left_operand()->deep_copy(copied_ops),
...
bool LogicalExpression::_shallow_equals(
    const AbstractExpression &expression) const
{
    ...
    return logical_operator ==
           static_cast<const LogicalExpression &>(expression).logical_operator;
}
