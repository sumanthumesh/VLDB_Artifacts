...
BinaryPredicateExpression::BinaryPredicateExpression(
    ...
    : AbstractPredicateExpression(init_predicate_condition,
                                  {left_operand, right_operand})
{
    if constexpr (HYRISE_DEBUG)
    {
        const auto valid_predicate_conditions = {
        ...
        const auto *const it =
            std::find(valid_predicate_conditions.begin(),
                      valid_predicate_conditions.end(), predicate_condition);
        Assert(it != valid_predicate_conditions.end(),
               "Specified PredicateCondition is not valid for a "
               "BinaryPredicateExpression");
    }
}

const std::shared_ptr<AbstractExpression> &
BinaryPredicateExpression::left_operand() const
{
    return arguments[0];
}

const std::shared_ptr<AbstractExpression> &
BinaryPredicateExpression::right_operand() const
{
    return arguments[1];
}
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    return std::make_shared<BinaryPredicateExpression>(
        predicate_condition, left_operand()->deep_copy(copied_ops),
        right_operand()->deep_copy(copied_ops));
}

std::string
BinaryPredicateExpression::description(const DescriptionMode mode) const
{
