 ...
 *
 * Does NOT contain a shared_ptr to the expression it references since that
 * would make LQP/PQP/Expression deep_copy()ing extremely cumbersome to
 * implement. Instead, it extracts all information it needs from the referenced
 * expression into ReferencedExpressionInfo
 *
 * NOTE: It is assumed that all correlated expression are nullable - it is very
 * taxing, code-wise, to determine whether it actually is.
 */
