...
LQPColumnExpression::LQPColumnExpression(
    const std::shared_ptr<const AbstractLQPNode> &init_original_node,
...
std::shared_ptr<AbstractExpression> LQPColumnExpression::_on_deep_copy(
    ...
    const
{
    return std::make_shared<LQPColumnExpression>(original_node.lock(),
                                                 original_column_id);
}

std::string LQPColumnExpression::description(const DescriptionMode mode) const
{
    // Even if the LQP is invalid, we still want to be able to print it as good
    // as possible
    ...
    case LQPNodeType::Mock:
    {
        const auto mock_node =
            std::static_pointer_cast<const MockNode>(original_node_locked);
        Assert(original_column_id < mock_node->column_definitions().size(),
    ...
    case LQPNodeType::StaticTable:
    {
        const auto static_table_node =
            std::static_pointer_cast<const StaticTableNode>(
                original_node_locked);
...
{
    const auto original_node_locked = original_node.lock();
    Assert(original_node_locked, "Trying to retrieve data_type of expired "
                                 "LQPColumnExpression, LQP is invalid");
...
bool LQPColumnExpression::_shallow_equals(
    const AbstractExpression &expression) const
{
    ...
    const auto &lqp_column_expression =
        static_cast<const LQPColumnExpression &>(expression);
...
{
    // It is important not to combine the address of the original_node with the
    // hash code as it was done before #1795. If this address is combined with
    // the return hash code, equal LQP nodes that are not identical and that
    // have LQPColumnExpressions or child nodes with LQPColumnExpressions would
...
bool LQPColumnExpression::_on_is_nullable_on_lqp(
    const AbstractLQPNode & /*lqp*/) const
{
    Fail("Should not be called. This should have been forwarded to "
         "StoredTableNode/StaticTableNode/MockNode by "
