...
CaseExpression::CaseExpression(
    const std::shared_ptr<AbstractExpression> &when,
...
std::string CaseExpression::description(const DescriptionMode mode) const
{
    ...
    stream << "CASE WHEN " << when()->description(mode) << " THEN "
           << then()->description(mode) << " ELSE "
...
{
    return expression_common_type(then()->data_type(),
                                  otherwise()->data_type());
}
...
{
    return std::make_shared<CaseExpression>(when()->deep_copy(copied_ops),
                                            then()->deep_copy(copied_ops),
