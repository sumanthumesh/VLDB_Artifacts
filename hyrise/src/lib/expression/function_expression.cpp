...
FunctionExpression::FunctionExpression(
    const FunctionType init_function_type,
    const std::vector<std::shared_ptr<AbstractExpression>> &init_arguments)
    : AbstractExpression(ExpressionType::Function, init_arguments),
      function_type(init_function_type)
{
    ...
    case FunctionType::Concatenate:
        Assert(arguments.size() >= 2,
               "Concatenate expects at least 2 parameters");
        for (const auto &argument : arguments)
        {
            Assert(argument->data_type() == DataType::String ||
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    return std::make_shared<FunctionExpression>(
        function_type, expressions_deep_copy(arguments, copied_ops));
}
...
std::string FunctionExpression::description(const DescriptionMode mode) const
{
    ...
    for (auto argument_idx = size_t{0}; argument_idx < arguments.size();
         ++argument_idx)
    {
...
bool FunctionExpression::_shallow_equals(
