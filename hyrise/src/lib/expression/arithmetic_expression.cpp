...
ArithmeticExpression::ArithmeticExpression(
    const ArithmeticOperator init_arithmetic_operator,
...
ArithmeticExpression::left_operand() const
{
    return arguments[0];
}

const std::shared_ptr<AbstractExpression> &
ArithmeticExpression::right_operand() const
{
    return arguments[1];
}
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    return std::make_shared<ArithmeticExpression>(
        arithmetic_operator, left_operand()->deep_copy(copied_ops),
        right_operand()->deep_copy(copied_ops));
}

DataType ArithmeticExpression::data_type() const
{
    return expression_common_type(left_operand()->data_type(),
...
bool ArithmeticExpression::_shallow_equals(
...
bool ArithmeticExpression::_on_is_nullable_on_lqp(
    const AbstractLQPNode &lqp) const
{
    // We return NULL for divisions/modulo by 0
    return AbstractExpression::_on_is_nullable_on_lqp(lqp) ||
