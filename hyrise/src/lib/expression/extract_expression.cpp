...
ExtractExpression::ExtractExpression(
    const DatetimeComponent init_datetime_component,
    const std::shared_ptr<AbstractExpression> &from)
    : AbstractExpression(ExpressionType::Extract, {from}),
      datetime_component(init_datetime_component)
{
}
...
{
    // Timestamps can contain fractional seconds, so the result must be a
    // floating-point number for seconds.
    return datetime_component == DatetimeComponent::Second ? DataType::Double
                                                           : DataType::Int;
}
...
bool ExtractExpression::_shallow_equals(
...
size_t ExtractExpression::_shallow_hash() const
{
    ...
    return std::hash<DatetimeUnderlyingType>{}(
        static_cast<DatetimeUnderlyingType>(datetime_component));
}
