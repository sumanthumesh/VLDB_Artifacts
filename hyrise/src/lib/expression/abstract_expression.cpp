...
size_t AbstractExpression::hash() const
{
    ...
    {
        // Include the hash value of the inputs, but do not recurse any deeper.
        // We will have to perform a deep comparison anyway.
...
bool AbstractExpression::_on_is_nullable_on_lqp(
...
std::string
AbstractExpression::_enclose_argument(const AbstractExpression &argument,
                                      const DescriptionMode mode) const
{
    // TODO(anybody) Using >= to make divisions ("(2/3)/4") and logical
    // operations ("(a AND (b OR c))") unambiguous -
    //               Sadly this makes cases where the parentheses could be
    //               avoided look ugly ("(2+3)+4")

    if (static_cast<std::underlying_type_t<ExpressionPrecedence>>(
            argument._precedence()) >=
        static_cast<std::underlying_type_t<ExpressionPrecedence>>(
            _precedence()))
    {
