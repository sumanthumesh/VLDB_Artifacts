...
/**
 * AllParameterVariant holds either an AllTypeVariant, a ColumnID or a
 * Placeholder. It should be used to generalize Hyrise operator calls.
 */
...
static constexpr auto parameter_types_as_hana_sequence =
    hana::transform(parameter_types, hana::second);

// Convert tuple to mpl vector
using ParameterTypesAsMplVector =
    decltype(hana::to<hana::ext::boost::mpl::vector_tag>(
        parameter_types_as_hana_sequence));

// Create boost::variant from mpl vector
using AllParameterVariant =
    typename boost::make_variant_over<ParameterTypesAsMplVector>::type;
...
std::ostream &operator<<(std::ostream &stream,
                         const AllParameterVariant &variant);
