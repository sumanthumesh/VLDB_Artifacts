...
constexpr auto encoding_supports_data_type(SegmentEncodingType encoding_type,
                                           ColumnDataType data_type)
{
    return hana::contains(
        hana::at_key(supported_data_types_for_encoding_type, encoding_type),
        data_type);
}

// Version for when EncodingType and DataType are only known at runtime
bool encoding_supports_data_type(EncodingType encoding_type,
                                 DataType data_type);
...
struct SegmentEncodingSpec
{
    ...
    explicit constexpr SegmentEncodingSpec(EncodingType init_encoding_type)
        : encoding_type{init_encoding_type}
    {
    }

    constexpr SegmentEncodingSpec(
        EncodingType init_encoding_type,
        std::optional<VectorCompressionType> init_vector_compression_type)
        : encoding_type{init_encoding_type}, vector_compression_type{
                                                 init_vector_compression_type}
    {
    }
...
inline bool operator==(const SegmentEncodingSpec &lhs,
                       const SegmentEncodingSpec &rhs)
{
