      ...
      _indexes(std::move(indexes))
{
    DebugAssert(!_segments.empty(),
                "Chunks without Segments are not legal, as the row count of "
                "such a Chunk cannot be determined");

    if constexpr (HYRISE_DEBUG)
    {
        const auto is_reference_chunk =
            !_segments.empty() ? std::dynamic_pointer_cast<ReferenceSegment>(
                                     _segments.front()) != nullptr
                               : false;
        ...
        for (const auto &segment : _segments)
        {
            ...
            Assert(!mvcc_data ||
                       !std::dynamic_pointer_cast<ReferenceSegment>(segment),
                   "Chunks containing ReferenceSegments should not contain "
                   "MvccData. They implicitly use the MvccData of the "
                   "referenced Table");
            Assert((std::dynamic_pointer_cast<ReferenceSegment>(segment) !=
                    nullptr) == is_reference_chunk,
                   "Chunk can either contain only ReferenceSegments or only "
                   "non-ReferenceSegments");
        }
    }
...
void Chunk::replace_segment(size_t column_id,
...
void Chunk::append(const std::vector<AllTypeVariant> &values)
{
    ...
    {
        const auto &base_value_segment =
            std::dynamic_pointer_cast<BaseValueSegment>(*segment_it);
        DebugAssert(base_value_segment,
                    "Can't append to segment that is not a ValueSegment");
...
void Chunk::finalize()
{
    Assert(is_mutable(), "Only mutable chunks can be finalized. Chunks cannot "
                         "be finalized twice.");
    _is_mutable = false;

    // Only perform the max_begin_cid check if it hasn't already been set.
    if (has_mvcc_data() &&
        _mvcc_data->max_begin_cid.load() == MvccData::MAX_COMMIT_ID)
    {
        const auto chunk_size = size();
        Assert(chunk_size > 0,
               "finalize() should not be called on an empty chunk.");
        auto max_begin_cid = CommitID{0};
        for (auto chunk_offset = ChunkOffset{0}; chunk_offset < chunk_size;
             ++chunk_offset)
        {
            max_begin_cid = std::max(max_begin_cid,
                                     _mvcc_data->get_begin_cid(chunk_offset));
        }
        set_atomic_max(_mvcc_data->max_begin_cid, max_begin_cid);

        Assert(_mvcc_data->max_begin_cid != MvccData::MAX_COMMIT_ID,
               "max_begin_cid should not be MAX_COMMIT_ID when finalizing a "
               "chunk. This probably means the chunk was "
               "finalized before all transactions committed/rolled back.");
    }
}

std::vector<std::shared_ptr<AbstractChunkIndex>>
Chunk::get_indexes(const std::vector<ColumnID> &column_ids) const
{
...
std::shared_ptr<AbstractChunkIndex> Chunk::get_index(
    const ChunkIndexType index_type,
    const std::vector<std::shared_ptr<const AbstractSegment>> &segments) const
{
    auto index_it = std::find_if(_indexes.cbegin(), _indexes.cend(),
                                 [&](const auto &index) {
                                     return index->is_index_for(segments) &&
...
Chunk::get_index(const ChunkIndexType index_type,
...
bool Chunk::references_exactly_one_table() const
{
    ...
    auto first_segment = std::dynamic_pointer_cast<const ReferenceSegment>(
    ...
    {
        const auto segment = std::dynamic_pointer_cast<const ReferenceSegment>(
...
Chunk::_get_segments_for_ids(const std::vector<ColumnID> &column_ids) const
{
    if constexpr (HYRISE_DEBUG)
    {
        ...
        {
            Assert(column_id < number_of_columns,
                   "ColumnID " + std::to_string(column_id) +
    ...
    std::transform(
        column_ids.cbegin(), column_ids.cend(), std::back_inserter(segments),
...
void Chunk::set_pruning_statistics(
    const std::optional<ChunkPruningStatistics> &pruning_statistics)
{
    ...
    Assert(!pruning_statistics || pruning_statistics->size() ==
...
void Chunk::set_individually_sorted_by(
    const std::vector<SortColumnDefinition> &sorted_by)
{
    Assert(!is_mutable(),
           "Cannot set_individually_sorted_by on mutable chunks.");
    // Currently, we assume that set_individually_sorted_by is called only once
    // at most. As such, there should be no existing sorting and the new sorting
    // should contain at least one column. Feel free to remove this assertion if
    // necessary.
    Assert(!sorted_by.empty() && _sorted_by.empty(),
           "Sorting information cannot be empty or reset.");
                ...
                *sorted_segment,
                [&](auto begin, auto end)
                {
                    Assert(std::is_sorted(
                               begin, end,
                               [sort_mode = sorted_by_column.sort_mode](
                                   const auto &left, const auto &right)
                               {
                                   // is_sorted evaluates the segment by calling
                                   // the lambda with the SegmentPositions at
                                   // it+n and it (n being non-negative), which
                                   // needs to evaluate to false.
                                   if (right.is_null())
                                   {
                                       return false; // handles right side is
                                                     // NULL and both are NULL
                                   }
                                   if (left.is_null())
                                   {
                                       return true;
                           ...
                           "Setting a sort order for a segment which is not "
                           "sorted accordingly.");
                });
        }
    }
