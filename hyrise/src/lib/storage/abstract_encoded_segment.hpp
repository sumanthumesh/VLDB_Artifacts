...
class AbstractEncodedSegment : public AbstractSegment
{
  public:
    ...
    /**
     * An encoded segment may use a compressed vector to reduce its memory
     * footprint. Returns the vector’s type if it does, else std::nullopt
     */
    virtual std::optional<CompressedVectorType>
    compressed_vector_type() const = 0;
};
