 ...
 *
 * Frame-of-Reference encoding divides the values of segment into fixed-size
 * blocks. The values of each block are encoded as an offset from the block's
 * minimum value. These offsets, which can ideally be represented by fewer bits,
 ...
 *
 * Null values are stored in a separate vector. Note, for correct offset
 * handling, the minimum of each frame is stored in the offset_values vector at
 * each position that is NULL.
 *
 * std::enable_if_t must be used here and cannot be replaced by a static_assert
 * in order to prevent instantiation of FrameOfReferenceSegment<T> with T other
 * than int32_t. Otherwise, the compiler might instantiate
 * FrameOfReferenceSegment with other types even if they are never actually
 * needed. "If the function selected by overload resolution can be determined
 * without instantiating a class template definition, it is unspecified whether
 * that instantiation actually takes place." Draft Std. N4800 12.8.1.8
 */
...
class FrameOfReferenceSegment : public AbstractEncodedSegment
{
  public:
    ...
    explicit FrameOfReferenceSegment(
        pmr_vector<T> block_minima, std::optional<pmr_vector<bool>> null_values,
    ...
    std::optional<T> get_typed_value(const ChunkOffset chunk_offset) const
    {
        ...
        const auto value =
            static_cast<T>(_decompressor->get(chunk_offset)) + minimum;
#ifdef NEW_SEG_PRINTS
        log_to_file("Frame of Reference Segment\n");
#endif
        return value;
    }
