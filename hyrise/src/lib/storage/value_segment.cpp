...
template <typename T>
ValueSegment<T>::ValueSegment(bool nullable, ChunkOffset capacity)
    : BaseValueSegment(data_type_from_type<T>())
{
    ...
    log_to_file("New Value Segment C1\n");
#endif
}
...
{
#ifdef NEW_SEG_PRINTS
    log_to_file("New Value Segment C2\n");
#endif
}
...
{
    DebugAssert(_values.size() == _null_values->size(),
                "The number of values and null_values should be equal");

    // We cannot check for the capacity being equal because of the
    // implementation details of vector<bool>
    DebugAssert(_values.capacity() <= _null_values->capacity(),
                "The capacity of values and null_values should be compatible");
#ifdef NEW_SEG_PRINTS
    log_to_file("New Value Segment C3\n");
#endif
}
...
T ValueSegment<T>::get(const ChunkOffset chunk_offset) const
{
    ...
    Assert(!is_nullable() || !(*_null_values).at(chunk_offset),
           "Can’t return value of segment type because it is null.");
    access_counter[SegmentAccessCounter::AccessType::Point] += 1;
    return _values.at(chunk_offset);
}

template <typename T> void ValueSegment<T>::append(const AllTypeVariant &val)
{
...
template <typename T> const pmr_vector<T> &ValueSegment<T>::values() const
{
    return _values;
}

template <typename T> pmr_vector<T> &ValueSegment<T>::values()
{
    return _values;
}

template <typename T> bool ValueSegment<T>::is_nullable() const
{
...
{
    DebugAssert(is_nullable(),
                "This ValueSegment does not support null values.");
...
void ValueSegment<T>::set_null_value(const ChunkOffset chunk_offset)
{
    ...
    const auto lock =
        std::lock_guard<std::mutex>{_null_value_modification_mutex};
    (*_null_values)[chunk_offset] = true;
}

template <typename T> ChunkOffset ValueSegment<T>::size() const
{
    return static_cast<ChunkOffset>(_values.size());
}

template <typename T> void ValueSegment<T>::resize(const size_t size)
{
    DebugAssert(size > _values.size() && size <= _values.capacity(),
                "ValueSegments should not be shrunk or resized beyond their "
                "original capacity");
    _values.resize(size);
    if (is_nullable())
    {
...
{
    auto new_values =
        pmr_vector<T>{_values, alloc}; // NOLINT(cppcoreguidelines-slicing)
    auto copy = std::shared_ptr<AbstractSegment>{};
    if (is_nullable())
    {
        auto new_null_values = pmr_vector<bool>{
            *_null_values,
            alloc}; // NOLINT(cppcoreguidelines-slicing) (see above)
        copy = std::make_shared<ValueSegment<T>>(std::move(new_values),
                                                 std::move(new_null_values));
    }
    else
    {
        copy = std::make_shared<ValueSegment<T>>(std::move(new_values));
...
template <typename T>
size_t
ValueSegment<T>::memory_usage(const MemoryUsageCalculationMode mode) const
{
