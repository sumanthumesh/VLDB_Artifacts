 ...
 */
template <typename T> class RunLengthSegment : public AbstractEncodedSegment
{
  public:
    explicit RunLengthSegment(
    ...
    std::optional<T> get_typed_value(const ChunkOffset chunk_offset) const
    {
        ...
        const auto end_position_it = std::lower_bound(
            _end_positions->cbegin(), _end_positions->cend(), chunk_offset);
        const auto index =
            std::distance(_end_positions->cbegin(), end_position_it);
