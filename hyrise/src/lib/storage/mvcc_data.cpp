...
{
    DebugAssert(size > 0,
                "No point in having empty MVCC data, as it cannot grow");
...
{
    DebugAssert(offset < _begin_cids.size(),
                "offset out of bounds; MvccData insufficently preallocated?");
...
{
    DebugAssert(offset < _end_cids.size(),
                "offset out of bounds; MvccData insufficently preallocated?");
...
{
    DebugAssert(offset < _tids.size(),
                "offset out of bounds; MvccData insufficently preallocated?");
    return _tids[offset];
}

void MvccData::set_tid(const ChunkOffset offset,
...
bool MvccData::compare_exchange_tid(const ChunkOffset offset,
                                    ...
                                    TransactionID new_transaction_id)
{
    ...
    return _tids[offset].compare_exchange_strong(expected_transaction_id,
                                                 new_transaction_id);
}
