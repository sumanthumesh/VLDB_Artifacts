 ...
 *
 * NOT thread-safe. In a multi-threaded context, the ChunkCompressionTask should
 * invoke the ChunkEncoder.
 *
...
class ChunkEncoder
{
  public:
    ...
    encode_segment(const std::shared_ptr<AbstractSegment> &segment,
     ...
     *
     * Encodes a chunk using the passed encoding specifications. Reduces also
     * the fragmentation of the chunk’s MVCC data.
     */
    static void encode_chunk(const std::shared_ptr<Chunk> &chunk,
     ...
     */
    static void
    encode_chunk(const std::shared_ptr<Chunk> &chunk,
                 const std::vector<DataType> &column_data_types,
     ...
     *
     * The encoding is specified per segment (SegmentEncodingSpec) for each
     * chunk.
     */
    static void encode_chunks(
        const std::shared_ptr<Table> &table,
        const std::vector<ChunkID> &chunk_ids,
        const std::map<ChunkID, ChunkEncodingSpec> &chunk_encoding_specs);

    /**
     * @brief Encodes the specified chunks of the passed table using a single
     * SegmentEncodingSpec
     */
    static void
    encode_chunks(const std::shared_ptr<Table> &table,
                  const std::vector<ChunkID> &chunk_ids,
                  const SegmentEncodingSpec &segment_encoding_spec = {});
     ...
     */
    static void
    encode_all_chunks(const std::shared_ptr<Table> &table,
                      const SegmentEncodingSpec &segment_encoding_spec = {});
};
