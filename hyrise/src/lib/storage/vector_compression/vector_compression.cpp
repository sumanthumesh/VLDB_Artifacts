 ...
 */
const auto vector_compressor_for_type =
    std::map<VectorCompressionType, std::shared_ptr<BaseVectorCompressor>>{
        {VectorCompressionType::FixedWidthInteger,
         std::make_shared<FixedWidthIntegerCompressor>()},
        {VectorCompressionType::BitPacking,
         std::make_shared<BitPackingCompressor>()}};

std::unique_ptr<BaseVectorCompressor>
create_compressor_by_type(VectorCompressionType type)
{
    auto iter = vector_compressor_for_type.find(type);
    Assert(
        iter != vector_compressor_for_type.cend(),
...
compress_vector(const pmr_vector<uint32_t> &vector,
...
std::ostream &operator<<(std::ostream &stream,
                         const VectorCompressionType vector_compression_type)
{
    return stream << vector_compression_type_to_string.left.at(
