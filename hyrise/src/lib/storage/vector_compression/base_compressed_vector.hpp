 ...
 * - the encoder, which encapsulates the encoding algorithm (base class:
 * BaseVectorCompressor)
 * - the vector, which is returned by the encoder and contains the encoded data
 * (base class: BaseCompressedVector)
 * - the iterator, for sequentially decoding the vector (base class:
 * BaseCompressedVectorIterator)
 * - the decompressor, which implements point access into the vector (base
 * class: BaseVectorDecompressor)
 *
 * The iterators and decompressors are created via virtual and non-virtual
 * methods of the vector interface.
 *
...
class BaseCompressedVector : private Noncopyable
{
  public:
    ...
    virtual std::unique_ptr<BaseVectorDecompressor>
    create_base_decompressor() const = 0;

    virtual std::unique_ptr<const BaseCompressedVector>
    copy_using_allocator(const PolymorphicAllocator<size_t> &alloc) const = 0;
};
...
using BaseCompressedVectorIterator =
    boost::iterator_facade<Derived, uint32_t,
                           boost::random_access_traversal_tag, uint32_t>;
 ...
 */
template <typename Derived> class CompressedVector : public BaseCompressedVector
{
  public:
    /**
  ...
  public:
    ...
    create_base_decompressor() const final
    {
