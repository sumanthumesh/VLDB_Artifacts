...
class FixedWidthIntegerCompressor : public BaseVectorCompressor
{
  public:
    ...
    compress(const pmr_vector<uint32_t> &vector,
  ...
  private:
    ...
    static std::unique_ptr<BaseCompressedVector>
    _compress_using_max_value(const PolymorphicAllocator<size_t> &alloc,
                              const pmr_vector<uint32_t> &vector,
                              const uint32_t max_value);

    template <typename UnsignedIntType>
    static std::unique_ptr<BaseCompressedVector>
    _compress_using_uint_type(const PolymorphicAllocator<size_t> &alloc,
                              const pmr_vector<uint32_t> &vector);
};
