 ...
 *
 * This is simplest vector compression scheme. It matches the old
 * FittedAttributeVector
 */
template <typename UnsignedIntType>
class FixedWidthIntegerVector
    : public CompressedVector<FixedWidthIntegerVector<UnsignedIntType>>
{
    static_assert(hana::contains(hana::tuple_t<uint8_t, uint16_t, uint32_t>,
                                 hana::type_c<UnsignedIntType>),
                  "UnsignedIntType must be any of the three listed unsigned "
                  "integer types.");
  ...
  public:
    ...
    {
        return std::make_unique<FixedWidthIntegerDecompressor<UnsignedIntType>>(
            _data);
    }
    ...
    on_copy_using_allocator(const PolymorphicAllocator<size_t> &alloc) const
    {
