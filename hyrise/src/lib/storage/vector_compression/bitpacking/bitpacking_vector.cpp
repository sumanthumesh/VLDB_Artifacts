...
BitPackingVector::BitPackingVector(pmr_compact_vector data)
    : _data{std::move(data)}
{
}
...
BitPackingVector::on_create_base_decompressor() const
{
...
BitPackingVector::on_copy_using_allocator(
    const PolymorphicAllocator<size_t> &alloc) const
{
    auto data_copy = pmr_compact_vector(_data.bits(), _data.size(), alloc);

    // zero initialize the compact_vector's memory, see
    // bitpacking_compressor.cpp
    using InternalType = std::remove_reference_t<decltype(*data_copy.get())>;
    std::fill_n(data_copy.get(), data_copy.bytes() / sizeof(InternalType),
                InternalType{0});
