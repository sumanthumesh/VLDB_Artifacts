 ...
 * All values of the sequences are compressed with the same bit length, which is
 * determined by the bits required to represent the maximum value of the
 * sequence. The decoding runtime is only marginally slower than
 * FixedWidthIntegerVector but the compression rate of BitPacking is
