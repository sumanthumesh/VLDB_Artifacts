...
void lqp_bind_placeholders_impl(
    const std::shared_ptr<AbstractLQPNode> &lqp,
    const std::unordered_map<ParameterID, std::shared_ptr<AbstractExpression>>
        &parameters,
    ...
    std::unordered_set<std::shared_ptr<AbstractLQPNode>> &visited_nodes)
{
    visit_expression(
        ...
        [&](auto &sub_expression)
        {
            if (const auto placeholder_expression =
                    std::dynamic_pointer_cast<PlaceholderExpression>(
                        sub_expression))
            {
                const auto parameter_iter =
                    parameters.find(placeholder_expression->parameter_id);
                Assert(parameter_iter != parameters.end(),
                       "No expression specified for ValuePlaceholder. This "
            ...
            if (const auto subquery_expression =
                    std::dynamic_pointer_cast<LQPSubqueryExpression>(
                        sub_expression))
            {
                lqp_bind_placeholders_impl(subquery_expression->lqp, parameters,
                                           visited_nodes);
            }
    ...
    std::unordered_set<std::shared_ptr<AbstractLQPNode>> &visited_nodes)
{
    visit_lqp(lqp,
              [&](const auto &node)
              {
                  ...
                  {
                      expression_bind_placeholders_impl(expression, parameters,
                                                        visited_nodes);
                  }
    ...
    const std::vector<std::shared_ptr<AbstractExpression>> &parameters) const
{
    Assert(parameters.size() == parameter_ids.size(),
           std::string("Incorrect number of parameters supplied - expected ") +
               std::to_string(parameter_ids.size()) + " got " +
               std::to_string(parameters.size()));

    auto parameters_by_id =
        std::unordered_map<ParameterID, std::shared_ptr<AbstractExpression>>{};
    for (auto parameter_idx = size_t{0}; parameter_idx < parameters.size();
         ++parameter_idx)
    {
        parameters_by_id.emplace(parameter_ids[parameter_idx],
    ...
    lqp_bind_placeholders_impl(instantiated_lqp, parameters_by_id,
                               visited_nodes);
...
std::ostream &operator<<(std::ostream &stream,
                         const PreparedPlan &prepared_plan)
{
    stream << "ParameterIDs: [";
    for (auto parameter_idx = size_t{0};
         parameter_idx < prepared_plan.parameter_ids.size(); ++parameter_idx)
    {
