...
class BaseSegmentEncoder
{
  public:
     ...
     */
    virtual std::shared_ptr<AbstractEncodedSegment>
    encode(const std::shared_ptr<const AbstractSegment> &segment,
...
template <typename Derived> class SegmentEncoder : public BaseSegmentEncoder
{
  public:
    /**
    ...
    // Resolves the data type and calls the appropriate instantiation of
    // encode().
    ...
    {
        Assert(
            uses_vector_compression(),
            "Vector compression type can only be set if supported by encoder.");
  ...
  public:
    ...
    template <typename ColumnDataType>
    std::shared_ptr<AbstractEncodedSegment>
    encode(const std::shared_ptr<const AbstractSegment> &abstract_segment,
           hana::basic_type<ColumnDataType> data_type_c)
    {
        static_assert(decltype(supports(data_type_c))::value);
        const auto iterable =
            create_any_segment_iterable<ColumnDataType>(*abstract_segment);

        // For now, we allocate without a specific memory source.
        return _self()._on_encode(iterable,
  ...
  private:
    // LZ4Encoder only supports BitPacking in order to reduce the compile time,
    // see the comment in lz4_encoder.hpp.
