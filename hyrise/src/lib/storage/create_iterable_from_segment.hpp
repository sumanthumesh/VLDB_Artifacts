...
template <typename T> class ValueSegment;

template <typename T> class DictionarySegment;

template <typename T> class RunLengthSegment;

template <typename T> class FixedStringDictionarySegment;

template <typename T, typename> class FrameOfReferenceSegment;

template <typename T> class LZ4Segment;
...
template <typename T, bool EraseSegmentType = HYRISE_DEBUG>
auto create_iterable_from_segment(
    const FixedStringDictionarySegment<T> &segment);

template <typename T, typename Enabled, bool EraseSegmentType = HYRISE_DEBUG>
auto create_iterable_from_segment(
    const FrameOfReferenceSegment<T, Enabled> &segment);

// Fix template deduction so that we can call `create_iterable_from_segment<T,
// false>` on FrameOfReferenceSegments
...
template <typename T, bool EraseSegmentType = HYRISE_DEBUG,
          EraseReferencedSegmentType = (HYRISE_DEBUG
                                            ? EraseReferencedSegmentType::Yes
