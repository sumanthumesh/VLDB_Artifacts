 ...
 */
const auto encoder_for_type =
    std::map<EncodingType, std::shared_ptr<BaseSegmentEncoder>>{
        {EncodingType::Dictionary,
         std::make_shared<DictionaryEncoder<EncodingType::Dictionary>>()},
        {EncodingType::RunLength, std::make_shared<RunLengthEncoder>()},
        {EncodingType::FixedStringDictionary,
         std::make_shared<
             DictionaryEncoder<EncodingType::FixedStringDictionary>>()},
        {EncodingType::FrameOfReference,
         std::make_shared<FrameOfReferenceEncoder>()},
...
{
    Assert(encoding_type != EncodingType::Unencoded,
           "Encoding type must not be Unencoded`.");

    auto iter = encoder_for_type.find(encoding_type);
    Assert(iter != encoder_for_type.cend(),
           "All encoding types must be in encoder_for_type.");
...
SegmentEncodingSpec
get_segment_encoding_spec(const std::shared_ptr<const AbstractSegment> &segment)
{
    ...
    if (const auto encoded_segment =
            std::dynamic_pointer_cast<const AbstractEncodedSegment>(segment))
    {
        std::optional<VectorCompressionType> vector_compression;
        if (encoded_segment->compressed_vector_type())
        {
            vector_compression = parent_vector_compression_type(
                *encoded_segment->compressed_vector_type());
        }
        return SegmentEncodingSpec{encoded_segment->encoding_type(),
                                   vector_compression};
    }
...
VectorCompressionType parent_vector_compression_type(
