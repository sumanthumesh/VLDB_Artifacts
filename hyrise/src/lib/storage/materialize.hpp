 ...
 *   materialize_values_and_nulls(*chunk->get_segment(expression->column_id()),
 * values_and_nulls); return values_and_nulls;
 * ```
 */
...
void materialize_values(const AbstractSegment &segment, Container &container)
{
    ...
    resolve_segment_type<ContainerValueType>(
        segment,
        [&](const auto &typed_segment)
        {
            create_iterable_from_segment<ContainerValueType>(typed_segment)
                .materialize_values(container);
        });
}

// Materialize the values/nulls in the segment
template <typename Container>
void materialize_values_and_nulls(const AbstractSegment &segment,
                                  Container &container)
{
    ...
    resolve_segment_type<ContainerValueType>(
        ...
        {
            create_iterable_from_segment<ContainerValueType>(typed_segment)
                .materialize_values_and_nulls(container);
        });
}
...
{
    resolve_segment_type<SegmentValueType>(
        segment,
        [&](const auto &typed_segment)
        {
            create_iterable_from_segment<SegmentValueType>(typed_segment)
                .materialize_nulls(container);
        });
}
