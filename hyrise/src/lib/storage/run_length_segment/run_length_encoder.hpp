...
class RunLengthEncoder : public SegmentEncoder<RunLengthEncoder>
{
  public:
    static constexpr auto _encoding_type =
        enum_c<EncodingType, EncodingType::RunLength>;
               ...
               const PolymorphicAllocator<T> &allocator)
    {
        ...
        auto end_positions =
            std::make_shared<pmr_vector<ChunkOffset>>(allocator);

        segment_iterable.with_iterators(
            [&](auto it, auto end)
            {
                // Early out for empty segments, code below assumes it to be
                // non-empty
                if (it == end)
                {
                    return;
                ...
                // Init is_current_null such that it does not equal the first
                // entry
                ...
                for (; it != end; ++it)
                {
                    ...
                    if ((is_previous_null == is_current_null) &&
                        (is_previous_null || (previous_value == current_value)))
                    {
        ...
        // The resize method of the vector might have overallocated memory -
        // hand that memory back to the system
