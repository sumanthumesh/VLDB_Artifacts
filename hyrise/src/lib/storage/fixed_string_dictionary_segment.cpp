      ...
      _decompressor{_attribute_vector->create_base_decompressor()}
{
    ...
    log_to_file("New FSD Segment\n");
#endif
}

template <typename T>
AllTypeVariant FixedStringDictionarySegment<T>::operator[](
...
template <typename T>
std::optional<T> FixedStringDictionarySegment<T>::get_typed_value(
    const ChunkOffset chunk_offset) const
{
    ...
    log_to_file("Fixed String Segment\n");
    return _dictionary->get_string_at(value_id);
}

template <typename T>
std::shared_ptr<const FixedStringVector>
FixedStringDictionarySegment<T>::fixed_string_dictionary() const
{
    return _dictionary;
}

template <typename T> ChunkOffset FixedStringDictionarySegment<T>::size() const
{
...
FixedStringDictionarySegment<T>::copy_using_allocator(
    const PolymorphicAllocator<size_t> &alloc) const
{
    auto new_dictionary =
        std::make_shared<FixedStringVector>(*_dictionary, alloc);
    auto new_attribute_vector = _attribute_vector->copy_using_allocator(alloc);

    auto copy = std::make_shared<FixedStringDictionarySegment<T>>(
        new_dictionary, std::move(new_attribute_vector));
...
template <typename T>
size_t FixedStringDictionarySegment<T>::memory_usage(
    const MemoryUsageCalculationMode /*mode*/) const
{
...
FixedStringDictionarySegment<T>::compressed_vector_type() const
{
...
template <typename T>
ValueID
FixedStringDictionarySegment<T>::lower_bound(const AllTypeVariant &value) const
{
    DebugAssert(!variant_is_null(value), "Null value passed.");

    const auto typed_value = boost::get<pmr_string>(value);

    auto it = std::lower_bound(_dictionary->cbegin(), _dictionary->cend(),
                               typed_value);
    ...
    return ValueID{static_cast<ValueID::base_type>(
        std::distance(_dictionary->cbegin(), it))};
}

template <typename T>
ValueID
FixedStringDictionarySegment<T>::upper_bound(const AllTypeVariant &value) const
{
    DebugAssert(!variant_is_null(value), "Null value passed.");

    const auto typed_value = boost::get<pmr_string>(value);

    auto it = std::upper_bound(_dictionary->cbegin(), _dictionary->cend(),
                               typed_value);
    if (it == _dictionary->cend())
    {
        return INVALID_VALUE_ID;
...
template <typename T>
AllTypeVariant
FixedStringDictionarySegment<T>::value_of_value_id(const ValueID value_id) const
{
...
FixedStringDictionarySegment<T>::attribute_vector() const
{
