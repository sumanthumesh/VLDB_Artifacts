...
{
    DebugAssert(static_cast<size_t>(AccessType::Count) ==
                    access_type_string_mapping.size(),
                "access_type_string_mapping should contain as many entries as "
                "there are access types.");
}
...
void SegmentAccessCounter::_set_counters(const SegmentAccessCounter &counter)
{
    ...
    for (auto counter_index = size_t{0}; counter_index < counter_count;
         ++counter_index)
    {
...
SegmentAccessCounter::CounterType &
SegmentAccessCounter::operator[](const AccessType type)
{
    return _counters[static_cast<size_t>(type)];
}

const SegmentAccessCounter::CounterType &
SegmentAccessCounter::operator[](const AccessType type) const
{
    return _counters[static_cast<size_t>(type)];
}

std::array<SegmentAccessCounter::CounterType, static_cast<size_t>(SegmentAccessCounter::AccessType::Count)> &SegmentAccessCounter::get_counter()
{
    return _counters;
}
...
std::string SegmentAccessCounter::to_string() const
{
    ...
    for (auto access_type = size_t{1};
         access_type < static_cast<size_t>(AccessType::Count); ++access_type)
    {
        result.append(",");
...
SegmentAccessCounter::AccessType
SegmentAccessCounter::access_type(const AbstractPosList &positions)
{
...
// Iterates over the first n (currently 100) elements in positions to determine
// the access pattern (see enum AccessPattern in header). The access pattern is
// computed by building a finite-state machine. The states are given by the enum
// AccessPattern. The alphabet is defined by the internal enum Input. The
// initial state is AccessPattern::Point. positions is iterated over from the
// beginning. For two adjacent elements (in positions) the difference is
// computed and mapped to an element of the enum Input. That input is used to
// transition from one state to the next. The predefined, two dimensional array,
// TRANSITIONS, acts as the transition function.
SegmentAccessCounter::AccessPattern
SegmentAccessCounter::_access_pattern(const AbstractPosList &positions)
{
  ...
  constexpr std::array<std::array<AccessPattern, 5 /*|Input|*/>, 6 /*|AccessPattern|*/> TRANSITIONS{{
    ...
    for (auto item_index = size_t{1}; item_index < max_items_to_compare;
         ++item_index)
    {
        const int64_t diff =
            static_cast<int64_t>(positions[item_index].chunk_offset) -
        ...
        access_pattern = TRANSITIONS[static_cast<size_t>(access_pattern)]
                                    [static_cast<size_t>(input)];

        if (access_pattern == AccessPattern::Random)
        {
            // There is no way that leads back from the Random state, so we can
            // stop here
            break;
        }
    }
