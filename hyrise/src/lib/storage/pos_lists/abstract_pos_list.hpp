...
{
  public:
    template <typename PosListType = AbstractPosList,
              typename DereferenceReturnType = RowID>
    class PosListIterator
        : public boost::iterator_facade<
              PosListIterator<PosListType, DereferenceReturnType>, RowID,
              boost::random_access_traversal_tag, DereferenceReturnType>
    {
      public:
        ...
        {
            DebugAssert(_pos_list == other._pos_list,
                        "PosListIterator compared to iterator on different "
                        "PosList instance");
        ...
        {
            DebugAssert(_chunk_offset < _pos_list->size(),
                        "past-the-end PosListIterator dereferenced");
    ...
    // Returns whether it is guaranteed that the PosList references a single
    // ChunkID. However, it may be false even if this is the case.
    ...
    friend bool operator==(const AbstractPosList &lhs,
                           const AbstractPosList &rhs);
};
