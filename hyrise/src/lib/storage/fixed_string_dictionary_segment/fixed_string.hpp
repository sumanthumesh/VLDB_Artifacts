...
// FixedString is a data type, in order to access the elements of a
// `fixed_string_vector` and interact with them. It has two different
// functionalities:
//     1. Represent an object in the `fixed_string_vector` (some sort of view)
//     not owning the memory itself
//     2. Standalone object owning the memory
// It stores a string in an array of chars in order to save memory space by
// avoiding small string optimization (SSO).
class FixedString
{
  public:
    ...
    // Creates a string_view object from FixedString (much cheaper than
    // string())
    std::string_view string_view() const;

    // Compare FixedStrings by comparing the underlying char arrays.
    // If one FixedString is longer than the other FixedString and the beginning
    // of the longer FixedString is equal to the other FixedString, the shorter
    // FixedString is smaller. Example: "defg" < "defghi"
    bool operator<(const FixedString &other) const;

    // Regular comparators that convert the fixed string into a regular string
    // first
    ...
    friend std::ostream &operator<<(std::ostream &stream,
                                    const FixedString &obj);

    // Support swappable concept needed for sorting values. See:
