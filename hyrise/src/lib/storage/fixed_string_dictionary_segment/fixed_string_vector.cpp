...
FixedStringVector::FixedStringVector(
    const FixedStringVector &other, const PolymorphicAllocator<char> &allocator)
    : _string_length(other._string_length), _chars(other._chars, allocator),
      _size(other._size)
{
    // For pmr_vectors, operator= does not change the allocator. As such, we
    // need to set _chars in the initializer list. Otherwise, it would be
    // created using the default allocator and ignore the passed-in allocator.
}

void FixedStringVector::push_back(const pmr_string &string)
{
    Assert(string.size() <= _string_length,
           "Inserted string is too long to insert in FixedStringVector");
...
pmr_string FixedStringVector::get_string_at(const size_t pos) const
{
    ...
    // String end checks if the string length is zero to avoid reading the data
    // directly "in front" of `chars`. If the string length is > 0, it is the
    // position of the last char.
    const auto string_end = _string_length == 0 ? 0 : _string_length - 1;

    if (*(string_start + string_end) == '\0')
    {
        // The string is zero-padded - the pmr_string constructor takes care of
...
void FixedStringVector::erase(const FixedStringIterator<false> start,
                              const FixedStringIterator<false> end)
{
