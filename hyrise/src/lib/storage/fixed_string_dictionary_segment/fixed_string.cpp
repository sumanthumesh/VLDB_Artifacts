...
// NOLINTNEXTLINE(bugprone-unhandled-self-assignment,cert-oop54-cpp): Tested by
// FixedStringTest.Assign
FixedString &FixedString::operator=(const FixedString &other)
{
    DebugAssert(
        other.maximum_length() <= _maximum_length,
        "Other FixedString is longer than current maximum string length");

    const auto copied_length =
        std::min(other.maximum_length(), _maximum_length);
    std::memmove(_mem, other._mem, copied_length);

    // Fill unused fields of char array with null terminator, in order to
    // overwrite the content of the old FixedString. This is especially
    // important if the old FixedString was longer than the other FixedString.
...
{
    DebugAssert(
        _maximum_length == other.maximum_length(),
