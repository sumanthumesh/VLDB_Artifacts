...
class FixedStringVector
{
  public:
    ...
    FixedStringVector(const FixedStringVector &other,
                      const PolymorphicAllocator<char> &allocator = {});
    ...
    FixedStringVector(pmr_vector<char> chars, const size_t string_length)
        : _string_length(string_length), _chars{std::move(chars)},
          _size(_chars.size() / string_length)
    {
    }

    // Create a FixedStringVector of FixedStrings with given values by iterating
    // over other container
    template <typename Iter>
    FixedStringVector(Iter first, Iter last, const size_t string_length,
                      const PolymorphicAllocator<char> &allocator = {})
        : _string_length(string_length), _chars(allocator)
    {
        const auto value_count = std::distance(first, last);
        // If string_length equals 0 we would not have any elements in the
        // vector. Hence, we would have to deal with null pointers. In order to
        // avoid this, we insert a null terminator to the vector by using
        // resize.
        if (_string_length == 0)
        {
            _chars.resize(1u);
    ...
    void erase(const FixedStringIterator<false> start,
               const FixedStringIterator<false> end);

    // Reduce capacity to fit its size
    void shrink_to_fit();

    // Return a copy of the allocator object associated with the vector of
    // values
