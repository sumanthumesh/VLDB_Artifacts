 ...
 */
template <typename T> class DictionarySegment : public BaseDictionarySegment
{
  public:
    explicit DictionarySegment(
    ...
    std::optional<T> get_typed_value(const ChunkOffset chunk_offset) const
    {
        ...
        log_to_file("Dictionary Segment\n");
    ...
    // Returns the first value ID that refers to a value >= the search value and
    // INVALID_VALUE_ID if all values are smaller than the search value. Here,
    // INVALID_VALUE_ID does not represent NULL (which isn't stored in the
    // dictionary anyway). Imagine a segment with values from 1 to 10. A scan
    // for `WHERE a < 12` would retrieve `lower_bound(12) == INVALID_VALUE_ID`
    // and compare all values in the attribute vector to `< INVALID_VALUE_ID`.
    // Thus, returning INVALID_VALUE_ID makes comparisons much easier. However,
    // the caller has to make sure that NULL values stored in the attribute
    // vector (stored with a value ID of unique_values_count()) are excluded.
    // See #1471 for a deeper discussion.
    ValueID lower_bound(const AllTypeVariant &value) const final;

    // Returns the first value ID that refers to a value > the search value and
    // INVALID_VALUE_ID if all values are smaller than or equal to the search
    // value (see also lower_bound).
