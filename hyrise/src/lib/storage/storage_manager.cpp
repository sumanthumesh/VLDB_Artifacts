                               ...
                               std::shared_ptr<Table> table)
{
    ...
    {
        // We currently assume that all tables stored in the StorageManager are
        // mutable and, as such, have MVCC data. This way, we do not need to
        // check query plans if they try to update immutable tables. However,
        // this is not a hard limitation and might be changed into more
        // fine-grained assertions if the need arises.
        Assert(table->get_chunk(chunk_id)->has_mvcc_data(),
               "Table must have MVCC data.");
    }
...
void StorageManager::drop_table(const std::string &name)
{
    ...
    // The concurrent_unordered_map does not support concurrency-safe erasure.
    // Thus, we simply reset the table pointer.
...
std::shared_ptr<Table> StorageManager::get_table(const std::string &name) const
{
    ...
    Assert(table, "Nullptr found when accessing table named '" + name +
...
StorageManager::tables() const
{
    std::unordered_map<std::string, std::shared_ptr<Table>> result;

    for (const auto &[table_name, table] : _tables)
    {
        // Skip dropped table, as we don't remove the map entry when dropping,
        // but only reset the table pointer.
        if (!table)
        {
            continue;
        }
...
std::shared_ptr<LQPView> StorageManager::get_view(const std::string &name) const
{
    ...
    Assert(view, "Nullptr found when accessing view named '" + name +
                     "'. This can happen if a dropped view is accessed.");
...
void StorageManager::add_prepared_plan(
    const std::string &name, const std::shared_ptr<PreparedPlan> &prepared_plan)
{
    const auto iter = _prepared_plans.find(name);
    Assert(iter == _prepared_plans.end() || !iter->second,
           "Cannot add prepared plan " + name +
               " - a prepared plan with the same name already exists");

    _prepared_plans[name] = prepared_plan;
}

std::shared_ptr<PreparedPlan>
StorageManager::get_prepared_plan(const std::string &name) const
{
    const auto iter = _prepared_plans.find(name);
    Assert(iter != _prepared_plans.end(),
           "No such prepared plan named '" + name + "'");

    auto prepared_plan = iter->second;
    Assert(prepared_plan,
           "Nullptr found when accessing prepared plan named '" + name +
...
void StorageManager::drop_prepared_plan(const std::string &name)
{
    const auto iter = _prepared_plans.find(name);
    Assert(iter != _prepared_plans.end() && iter->second,
           "Error deleting prepared plan. No such prepared plan named '" +
               name + "'");

    _prepared_plans[name] = nullptr;
}

std::unordered_map<std::string, std::shared_ptr<PreparedPlan>>
StorageManager::prepared_plans() const
{
    ...
    for (const auto &table_item : _tables)
    {
        ...
        auto job_task = std::make_shared<JobTask>(
            [table_item, &path]()
            {
                ...
                // not worth it, no performance-critical path.
                auto export_csv = std::make_shared<Export>(
                    table_wrapper, path + "/" + name + ".csv", FileType::Csv);
                export_csv->execute();
            });
                         ...
                         const StorageManager &storage_manager)
{
    ...
    for (auto const &table : storage_manager.tables())
    {
        ...
        stream << " (" << table.second->column_count() << " columns, "
               << table.second->row_count() << " rows in "
