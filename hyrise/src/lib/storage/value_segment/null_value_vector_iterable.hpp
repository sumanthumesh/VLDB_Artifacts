...
class NullValueVectorIterable
    : public PointAccessibleSegmentIterable<NullValueVectorIterable>
{
  public:
    using ValueType = bool;

    explicit NullValueVectorIterable(const pmr_vector<bool> &null_values)
    ...
    {
        auto begin = PointAccessIterator{
            _null_values, position_filter->cbegin(), position_filter->cbegin()};
        auto end = PointAccessIterator{_null_values, position_filter->begin(),
  ...
  private:
    class Iterator
        : public AbstractSegmentIterator<Iterator, IsNullSegmentPosition>
    {
      public:
        using ValueType = bool;
      ...
      public:
        explicit Iterator(const NullValueIterator &begin_null_value_it,
                          const NullValueIterator &null_value_it)
            : _begin_null_value_it{begin_null_value_it}, _null_value_it{
                                                             null_value_it}
        {
        }
      ...
      private:
        ...
        {
            return IsNullSegmentPosition{
                *_null_value_it, static_cast<ChunkOffset>(std::distance(
                                     _begin_null_value_it, _null_value_it))};
        }
