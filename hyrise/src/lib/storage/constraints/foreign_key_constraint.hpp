...
/**
 * Container class to define foreign key relationships for tables. Consider the
 following example when creating a table
 ...
 *     CREATE TABLE nation (n_nationkey int NOT NULL,
                            ...
                            FOREIGN KEY (n_regionkey) REFERENCES
 region(r_regionkey));
 *
 * Given this example, @param foreign_key_columns and @param foreign_key_table
 reference the table with the foreign key
 * (e.g., nation.n_regionkey) whereas @param primary_key_columns and @param
 primary_key_table reference the table with
 * the primary key (e.g., region.r_regionkey).
 *
 * Foreign key constraints are translated to inclusion dependencies (INDs) in
 the LQP. An inclusion dependency states
 * that all values for a combination of attributes are also present in a
 combination of attributes of another table.
 */
...
template <> struct hash<hyrise::ForeignKeyConstraint>
{
    size_t operator()(
