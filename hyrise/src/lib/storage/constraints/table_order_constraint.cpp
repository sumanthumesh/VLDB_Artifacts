...
TableOrderConstraint::TableOrderConstraint(
    const std::vector<ColumnID> &ordering_columns,
    ...
    : _ordering_columns(ordering_columns), _ordered_columns(ordered_columns)
{
    if constexpr (HYRISE_DEBUG)
    {
        ...
        {
            Assert(std::find(ordered_columns.begin(), ordered_columns.end(),
                             column) == ordered_columns.end(),
...
bool TableOrderConstraint::_on_equals(
    const AbstractTableConstraint &table_constraint) const
{
    ...
    const auto &rhs =
        static_cast<const TableOrderConstraint &>(table_constraint);
    return _ordering_columns == rhs._ordering_columns &&
           _ordered_columns == rhs._ordered_columns;
}
