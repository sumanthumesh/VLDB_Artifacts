...
TableKeyConstraint::TableKeyConstraint(const std::set<ColumnID> &columns,
...
bool TableKeyConstraint::_on_equals(
...
{
    // PRIMARY_KEY constraints are "smaller" than UNIQUE constraints. Thus, they
    // are listed first when printing them.
    if (_key_type != rhs._key_type)
    {
        return _key_type < rhs._key_type;
    }

    // As the columns are stored in a std::set, iteration is sorted and the
    // result is not ambiguous.
    return std::lexicographical_compare(_columns.cbegin(), _columns.cend(),
                                        rhs._columns.cbegin(),
                                        rhs._columns.cend());
}
...
size_t hash<hyrise::TableKeyConstraint>::operator()(
    const hyrise::TableKeyConstraint &table_key_constraint) const
{
