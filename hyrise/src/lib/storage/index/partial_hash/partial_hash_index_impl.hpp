...
/**
 * Base class that holds a PartialHashIndexImpl object with the correctly
 * resolved datatype.
 */
class BasePartialHashIndexImpl : public Noncopyable
{
  ...
  public:
    ...
    /**
     * Adds the given chunks to this index. If a chunk is already indexed, it is
     * not indexed again.
     *
    ...
    virtual IteratorRangePair
    range_not_equals(const AllTypeVariant &value) const = 0;

    virtual tsl::sparse_set<ChunkID> get_indexed_chunk_ids() const = 0;
};

/**
 * Templated implementation of the PartialHashIndex. It is possible to index any
 * immutable chunk of the indexed column. Chunks can be added via `insert()`.
 *
 * We did not perform a comprehensive experimental evaluation of different hash
 * maps for concurrent workloads (including reads and modifications) yet. Thus,
 * choosing a different hash map might result in better performance.
 */
...
class PartialHashIndexImpl : public BasePartialHashIndexImpl
{
    friend PartialHashIndexTest;
    using MapIterator =
        typename tsl::sparse_map<DataType, std::vector<RowID>>::const_iterator;
  ...
  public:
    ...
    range_equals(const AllTypeVariant &value) const final;
    BasePartialHashIndexImpl::IteratorRangePair
    range_not_equals(const AllTypeVariant &value) const final;
