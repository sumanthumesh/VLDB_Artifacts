...
{
    Assert(!chunks_to_index.empty(),
           "PartialHashIndex requires chunks_to_index not to be empty.");
    resolve_data_type(
        chunks_to_index.front().second->get_segment(column_id)->data_type(),
        [&](const auto column_data_type)
        {
            using ColumnDataType = typename decltype(column_data_type)::type;
            _impl = std::make_unique<PartialHashIndexImpl<ColumnDataType>>(
                chunks_to_index, _column_id);
        });
}
...
PartialHashIndex::_range_equals(const AllTypeVariant &value) const
{
    return _impl->range_equals(value);
}

PartialHashIndex::IteratorRangePair
PartialHashIndex::_range_not_equals(const AllTypeVariant &value) const
{
...
ColumnID PartialHashIndex::get_indexed_column_id() const { return _column_id; }
