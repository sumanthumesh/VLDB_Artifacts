    ...
    const ColumnID column_id)
{
    ...
    for (const auto &chunk : chunks_to_index)
    {
        ...
        segment_iterate<DataType>(
            *indexed_segment,
            [&](const auto &position)
            {
                // If value is NULL, add to NULL vector, otherwise add into
                // value map.
                if (position.is_null())
                {
                    _null_positions[DataType{}].emplace_back(
                        chunk.first, position.chunk_offset());
                }
                else
                {
                    _positions[position.value()].emplace_back(
                        chunk.first, position.chunk_offset());
                }
            });
    }
...
template <typename DataType>
size_t
PartialHashIndexImpl<DataType>::remove(const std::vector<ChunkID> &chunks)
{
    ...
    // Checks whether a given RowID's ChunkID is in the set of ChunkIDs to be
    // unindexed.
    ...
    while (map_iter != _positions.end())
    {
        ...
        // Remove every RowID entry of the value that references one of the
        // chunks.
        row_ids.erase(
            std::remove_if(row_ids.begin(), row_ids.end(), is_to_remove),
            row_ids.end());
    ...
    if (_null_positions.contains(DataType{}))
    {
        ...
        nulls.erase(std::remove_if(nulls.begin(), nulls.end(), is_to_remove),
                    nulls.end());
    }
...
template <typename DataType>
BasePartialHashIndexImpl::IteratorRange
PartialHashIndexImpl<DataType>::range_equals(const AllTypeVariant &value) const
{
    ...
    return std::make_pair(
        CreateFlatMapIterator<DataType>::from_map_iterator(begin),
        CreateFlatMapIterator<DataType>::from_map_iterator(++end));
}

template <typename DataType>
BasePartialHashIndexImpl::IteratorRangePair
PartialHashIndexImpl<DataType>::range_not_equals(
    const AllTypeVariant &value) const
{
    const auto &range_eq = range_equals(value);
    return std::make_pair(std::make_pair(cbegin(), range_eq.first),
                          std::make_pair(range_eq.second, cend()));
}

template <typename DataType>
BasePartialHashIndexImpl::Iterator
PartialHashIndexImpl<DataType>::cbegin() const
{
    return CreateFlatMapIterator<DataType>::from_map_iterator(
        _positions.cbegin());
}
...
template <typename DataType>
BasePartialHashIndexImpl::Iterator
PartialHashIndexImpl<DataType>::null_cbegin() const
{
    return CreateFlatMapIterator<DataType>::from_map_iterator(
        _null_positions.cbegin());
}

template <typename DataType>
BasePartialHashIndexImpl::Iterator
PartialHashIndexImpl<DataType>::null_cend() const
{
    return CreateFlatMapIterator<DataType>::from_map_iterator(
        _null_positions.cend());
}
...
size_t PartialHashIndexImpl<DataType>::estimate_memory_usage() const
{
    ...
    // It is not trivial to estimate the memory consumption of a hash set. We
    // use a tsl::sparse_set, so that the result of size() estimates the used
    // capacity sufficiently. Sparse sets/maps usually require only a few bits
    // of overhead per entry. Further details about sparse sets/maps:
    ...
    // Size of the keys.
    bytes +=
        sizeof(std::pair<DataType, std::vector<RowID>>) * _positions.size();
...
template <typename DataType>
tsl::sparse_set<ChunkID>
PartialHashIndexImpl<DataType>::get_indexed_chunk_ids() const
{
