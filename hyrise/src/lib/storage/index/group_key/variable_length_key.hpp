...
/**
 * The VariableLengthKey class can be used to create keys with a length up to
 * 255 byte. The keys are interpreted like unsigned integers, in contrast to big
 * unsigned integer implementations no mathematical operations are provided.
 * Each key manages its own memory, so the key itself points to the data in
 * memory. Since this class wraps VariableLengthKeyBase (and adds memory
 * management) please refer to its documentation for most member functions.
 */
