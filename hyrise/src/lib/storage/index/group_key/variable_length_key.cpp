...
VariableLengthKey::VariableLengthKey(CompositeKeyLength bytes_per_key)
    : _owned_data(
          std::make_unique<VariableLengthKeyWord[]>(bytes_per_key)), // NOLINT
...
VariableLengthKey::VariableLengthKey(const VariableLengthKeyBase &other)
    : _owned_data(
          std::make_unique<VariableLengthKeyWord[]>(other._size)), // NOLINT
      _impl(_owned_data.get(), other._size)
{
    std::copy(other._data, other._data + other._size, _impl._data);
}

VariableLengthKey::VariableLengthKey(const VariableLengthKey &other)
    : VariableLengthKey(other._impl)
{
}
...
bool VariableLengthKey::operator==(
...
VariableLengthKey &VariableLengthKey::shift_and_set(uint64_t value,
                                                    uint8_t bits_to_set)
{
