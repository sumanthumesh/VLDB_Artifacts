...
std::pair<uintX_t, uintX_t>
shift_left_with_borrow(uintX_t value, hyrise::CompositeKeyLength bits)
{
...
VariableLengthKeyBase::VariableLengthKeyBase(VariableLengthKeyWord *data,
...
{
    static_assert(std::is_same_v<VariableLengthKeyWord, uint8_t>,
                  "Changes for new word type required.");
    const auto *const raw_other =
        reinterpret_cast<VariableLengthKeyWord *>(&other);
    auto operation_width =
        std::min(static_cast<CompositeKeyLength>(sizeof(other)), _size);
...
VariableLengthKeyBase::operator<<=(CompositeKeyLength shift)
{
    ...
    else
    {
        if constexpr (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
        {
            ...
            {
                const auto [value, borrow] = shift_left_with_borrow(
                    _data[index - byte_shift], bit_shift);
                _data[index] = value;
                if (index + 1 < _size)
                {
        ...
        {
            // perform shifting
            for (auto index = int16_t{0};
                 index < _size - static_cast<int16_t>(byte_shift); ++index)
            {
                const auto [value, borrow] = shift_left_with_borrow(
                    _data[index + byte_shift], bit_shift);
                _data[index] = value;
            ...
            // fill now "empty" positions with zeros
            std::fill(_data + _size - byte_shift, _data + _size,
                      VariableLengthKeyWord{0});
        }
    }
    return *this;
}

VariableLengthKeyBase &VariableLengthKeyBase::shift_and_set(uint64_t value,
                                                            uint8_t bits_to_set)
{
    auto mask = uint64_t{0xFFFFFFFFFFFFFFFF};
    // shifting is undefined if right operand is greater than or equal to the
    // number of bits of left operand
...
bool operator==(const VariableLengthKeyBase &left,
                const VariableLengthKeyBase &right)
{
    return left._size == right._size &&
           std::memcmp(left._data, right._data, left._size) == 0;
}
               ...
               const VariableLengthKeyBase &right)
{
    ...
    if constexpr (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
    {
        ...
        // loop overflows after iteration with i == 0, so i becomes greater than
        // left._size
        for (CompositeKeyLength index = left._size - 1; index < left._size;
             --index)
        {
