...
VariableLengthKeyStore::VariableLengthKeyStore(ChunkOffset size,
                                               CompositeKeyLength bytes_per_key)
    : _bytes_per_key{bytes_per_key}
{
    static const CompositeKeyLength alignment = 8u;
    _key_alignment = (bytes_per_key / alignment +
                      (bytes_per_key % alignment == 0u ? 0u : 1u)) *
                     alignment;
    _data = std::vector<VariableLengthKeyWord>(
        static_cast<std::vector<VariableLengthKeyWord>::size_type>(
            size * _key_alignment));
}

VariableLengthKeyProxy VariableLengthKeyStore::operator[](ChunkOffset position)
{
    return VariableLengthKeyProxy(_data.data() + static_cast<size_t>(position) *
                                                     _key_alignment,
                                  _bytes_per_key);
}

VariableLengthKeyConstProxy
VariableLengthKeyStore::operator[](ChunkOffset position) const
{
    auto *const self = const_cast<VariableLengthKeyStore *>(
        this); // NOLINT(cppcoreguidelines-pro-type-const-cast)
    // The const_cast is grandfathered in, because it would require significant
    // changes to get rid of it. New code should not require it.
    return VariableLengthKeyConstProxy(
        self->_data.data() + static_cast<size_t>(position) * _key_alignment,
        _bytes_per_key);
}
...
VariableLengthKeyStore::iterator VariableLengthKeyStore::erase(iterator first,
                                                               iterator last)
{
    auto underlying_first = _data.begin();
    std::advance(underlying_first,
                 std::distance(begin(), first) * _key_alignment);
    auto underlying_last = _data.begin();
    std::advance(underlying_last,
                 std::distance(begin(), last) * _key_alignment);
...
{
    return iterator(_bytes_per_key, _key_alignment,
...
{
    auto *const self = const_cast<VariableLengthKeyStore *>(
        this); // NOLINT(cppcoreguidelines-pro-type-const-cast) (see above)
...
VariableLengthKeyStore::const_iterator VariableLengthKeyStore::cend() const
{
    auto *const self = const_cast<VariableLengthKeyStore *>(
        this); // NOLINT(cppcoreguidelines-pro-type-const-cast) (see above)
    return const_iterator(_bytes_per_key, _key_alignment,
                          self->_data.data() + _data.size());
}
