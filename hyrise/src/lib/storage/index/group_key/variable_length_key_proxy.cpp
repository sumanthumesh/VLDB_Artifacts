...
VariableLengthKeyConstProxy::VariableLengthKeyConstProxy(
    VariableLengthKeyWord *data, CompositeKeyLength bytes_per_key)
...
bool VariableLengthKeyConstProxy::operator==(
    const VariableLengthKeyConstProxy &other) const
{
...
bool VariableLengthKeyConstProxy::operator==(
    const VariableLengthKey &other) const
{
    return _impl == other._impl;
}
...
VariableLengthKeyProxy::VariableLengthKeyProxy(VariableLengthKeyWord *data,
                                               CompositeKeyLength bytes_per_key)
    : VariableLengthKeyConstProxy(data, bytes_per_key)
{
}

VariableLengthKeyProxy &
VariableLengthKeyProxy::operator=(const VariableLengthKeyBase &other)
{
    DebugAssert(
        (_impl._size == other._size),
        "Copying the data of a VariableLengthKey to a VariableLenghtKeyProxy "
        "requires that both have the same key size.");
