 ...
 */
template <typename DataType> class BTreeIndexImpl : public BaseBTreeIndexImpl
{
    friend BTreeIndexTest;

  public:
    explicit BTreeIndexImpl(
    ...
    Iterator
    lower_bound(const std::vector<AllTypeVariant> &values) const override;
    Iterator
    upper_bound(const std::vector<AllTypeVariant> &values) const override;
    Iterator cbegin() const override;
    Iterator cend() const override;

  protected:
    void _bulk_insert(const std::shared_ptr<const AbstractSegment> &,
                      std::vector<ChunkOffset> &_null_positions);
