...
BTreeIndexImpl<DataType>::BTreeIndexImpl(
    const std::shared_ptr<const AbstractSegment> &segments_to_index,
...
template <typename DataType>
BaseBTreeIndexImpl::Iterator BTreeIndexImpl<DataType>::lower_bound(
...
template <typename DataType>
BaseBTreeIndexImpl::Iterator BTreeIndexImpl<DataType>::upper_bound(
    const std::vector<AllTypeVariant> &values) const
{
...
template <typename DataType>
BaseBTreeIndexImpl::Iterator
BTreeIndexImpl<DataType>::lower_bound(DataType value) const
{
...
template <typename DataType>
BaseBTreeIndexImpl::Iterator
BTreeIndexImpl<DataType>::upper_bound(DataType value) const
{
...
size_t BTreeIndexImpl<DataType>::memory_consumption() const
{
    return sizeof(std::vector<ChunkOffset>) +
           sizeof(ChunkOffset) * _chunk_offsets.capacity() +
           _btree.bytes_used() + _heap_bytes_used;
}

template <typename DataType>
void BTreeIndexImpl<DataType>::_bulk_insert(
...
template <typename DataType>
void BTreeIndexImpl<DataType>::_add_to_heap_memory_usage(
    const DataType & /*value*/)
{
    // Except for pmr_string (see below), no supported data type uses heap
    // allocations
}

template <>
void BTreeIndexImpl<pmr_string>::_add_to_heap_memory_usage(
    const pmr_string &value)
{
    // Track only strings that are longer than the reserved stack space for
