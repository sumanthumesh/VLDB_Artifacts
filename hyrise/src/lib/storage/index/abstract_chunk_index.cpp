...
size_t AbstractChunkIndex::estimate_memory_consumption(
    ChunkIndexType type, ChunkOffset row_count, ChunkOffset distinct_count,
    uint32_t value_bytes)
{
    switch (type)
    {
    case ChunkIndexType::GroupKey:
        return GroupKeyIndex::estimate_memory_consumption(
            row_count, distinct_count, value_bytes);
    case ChunkIndexType::CompositeGroupKey:
        return CompositeGroupKeyIndex::estimate_memory_consumption(
            row_count, distinct_count, value_bytes);
    case ChunkIndexType::AdaptiveRadixTree:
        return AdaptiveRadixTreeIndex::estimate_memory_consumption(
            row_count, distinct_count, value_bytes);
    case ChunkIndexType::BTree:
        return BTreeIndex::estimate_memory_consumption(
            row_count, distinct_count, value_bytes);
    }
...
bool AbstractChunkIndex::is_index_for(
    const std::vector<std::shared_ptr<const AbstractSegment>> &segments) const
{
    ...
    for (auto segment_idx = size_t{0}; segment_idx < segments.size();
         ++segment_idx)
    {
...
AbstractChunkIndex::lower_bound(const std::vector<AllTypeVariant> &values) const
{
    DebugAssert(_get_indexed_segments().size() >= values.size(),
                "AbstractChunkIndex: The number of queried segments has to be "
                "less or equal to the number of indexed segments.");

    return _lower_bound(values);
}

AbstractChunkIndex::Iterator
AbstractChunkIndex::upper_bound(const std::vector<AllTypeVariant> &values) const
{
    DebugAssert(_get_indexed_segments().size() >= values.size(),
                "AbstractChunkIndex: The number of queried segments has to be "
                "less or equal to the number of indexed segments.");
