 ...
 * In order to store its partial keys, the ART uses 4 different node-types,
 * which can hold up to 4, 16, 48 and 256 partial keys respectively. Each node
 * has an array which contains pointers to its children and (if needed) an index
 * array in order to map partial keys to positions in the array of the
 * child-pointers
 *
 */
...
class ARTNode : private Noncopyable
{
  public:
    ...
    virtual std::vector<ChunkOffset>::const_iterator
    lower_bound(const AdaptiveRadixTreeIndex::BinaryComparable &key,
                size_t depth) const = 0;
    virtual std::vector<ChunkOffset>::const_iterator
    upper_bound(const AdaptiveRadixTreeIndex::BinaryComparable &key,
                size_t depth) const = 0;
...
class ARTNode4 final : public ARTNode
{
  ...
  public:
    ...
    std::vector<ChunkOffset>::const_iterator
    lower_bound(const AdaptiveRadixTreeIndex::BinaryComparable &key,
  ...
  private:
    /**
     * _delegate_to_child is used to avoid code duplication between
     * lower_bound() and upper_bound as the access to the children is the same,
     * only the method called on the matching child differs
     */
 ...
 *
 * The default value of the _index_to_child array is 255u. This is safe as the
...
class ARTNode48 final : public ARTNode
{
  public:
    explicit ARTNode48(
        const std::vector<std::pair<uint8_t, std::shared_ptr<ARTNode>>>
            &children);
...
/**
 *
 * leaves contain two std::vector<ChunkOffset>::const_iterator: _lower_bound and
 * _upper_bound.
 *
 * Consider the following example tree showing only its leaves:
 *
 *           Leaf(0x00000000) Leaf(0x00000001) ... Leaf(0xa101fe07)
 * Leaf(0xa101feaf) ... Leaf(0xfebb34f1) _lower_ |  | _upper_     |       |
 * _upper_    | |             |       |           |          | bound |  | bound
 * |---|       | bound      | |         |---|       |           |          | |
 * |--------|| _lower_   |--|      |--| |--------||           |      |----| | |
 * || bound        |      |             ||           |      |               |
 * _chunk_offsets:
 * |17|a2|a4|b4|fe|02|03|04|a1|a3|...|12|c1|f3|1a|4f|6d|...|92|9a|27|...|00|13|aa|ab|f1|
 *
 *
 * _lower_bound points to the first ChunkOffset in _chunk_offsets that belongs
 * to the value in the attribute vector that the leaf represents. eg: on
 * ChunkOffset 17 the value is 0x00000000, on ChunkOffset f3 the value is
 * 0xa101fe07
 *
 * _upper_bound points to the first ChunkOffset in _chunk_offsets that does not
 * contain the value that the leaf represents. eg: at ChunkOffset fe, the value
 * is 0x00000001, not 0x00000000 for the last leaf, _upper_bound =
 * _chunk_offsets.end()
 *
...
class Leaf final : public ARTNode
{
  ...
  public:
    explicit Leaf(std::vector<ChunkOffset>::const_iterator &lower,
                  std::vector<ChunkOffset>::const_iterator &upper);
