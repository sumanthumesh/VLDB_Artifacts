 ...
 *
 * Exposes all methods of value segments that do not rely on its specific data
 * type.
 */
class BaseValueSegment : public AbstractSegment
{
  public:
    ...
    /**
     * @brief Returns vector of NULL values (which is true for offsets where the
     * segment's value is NULL). Cannot be written to, see value_segment.hpp for
     * details.
     *
