...
/**
 * Function takes an arbitrary segment and (re-)encodes it. This reencoding can
 * both mean that an encoded segment is being recreated as an uncompressed
 * segment (created on the fly via an AnySegmentIterable) or that another
 * encoding is applied (via the segment encoding utils).
 */
std::shared_ptr<AbstractSegment>
ChunkEncoder::encode_segment(const std::shared_ptr<AbstractSegment> &segment,
                             const DataType data_type,
                             const SegmentEncodingSpec &encoding_spec)
{
    Assert(!std::dynamic_pointer_cast<const ReferenceSegment>(segment),
           "Reference segments cannot be encoded.");
    ...
    resolve_data_type(
        ...
        [&](const auto type)
        {
            ...
            // TODO(anyone): After #1489, build segment statistics in
            // encode_segment() instead of encode_chunk() and store them within
            // the segment instead of a chunk-owned list of statistics.

            // Check if early exit is possible when passed segment is already
            // encoded with requested spec. In case no vector compression is
            // specified, only the correct encoding type is checked and the
            // current vector compression type is ignored.
            const auto current_segment_encoding_spec =
                get_segment_encoding_spec(segment);
            if (current_segment_encoding_spec == encoding_spec ||
                (!encoding_spec.vector_compression_type &&
                 current_segment_encoding_spec.encoding_type ==
            ...
            // In case of unencoded re-encoding, an AnySegmentIterable iterable
            // is used to manually setup the data vectors for a ValueSegment. If
            // another encoding is requested, the segment encoding utitilies are
            // used (which create and call the according encoder).
            if (encoding_spec.encoding_type == EncodingType::Unencoded)
            {
                ...
                auto iterable =
                    create_any_segment_iterable<ColumnDataType>(*segment);
                iterable.with_iterators(
                    [&](auto it, const auto end)
                    {
                        ...
                        for (auto current_position = size_t{0}; it != end;
                             ++it, ++current_position)
                        {
                ...
                if (contains_nulls)
                {
                    result = std::make_shared<ValueSegment<ColumnDataType>>(
            ...
            else
            {
                ...
                {
                    encoder->set_vector_compression(
                        *encoding_spec.vector_compression_type);
                }
...
void ChunkEncoder::encode_chunk(const std::shared_ptr<Chunk> &chunk,
                                ...
                                const ChunkEncodingSpec &chunk_encoding_spec)
{
    ...
    Assert(
        chunk_encoding_spec.size() == static_cast<size_t>(column_count),
    ...
    for (auto column_id = ColumnID{0}; column_id < column_count; ++column_id)
    {
        ...
        const auto encoded_segment =
            encode_segment(abstract_segment, data_type, spec);
...
void ChunkEncoder::encode_chunk(
    const std::shared_ptr<Chunk> &chunk,
    const std::vector<DataType> &column_data_types,
    const SegmentEncodingSpec &segment_encoding_spec)
{
    const auto chunk_encoding_spec =
        ChunkEncodingSpec{chunk->column_count(), segment_encoding_spec};
    encode_chunk(chunk, column_data_types, chunk_encoding_spec);
}

void ChunkEncoder::encode_chunks(
    const std::shared_ptr<Table> &table, const std::vector<ChunkID> &chunk_ids,
    const std::map<ChunkID, ChunkEncodingSpec> &chunk_encoding_specs)
{
    ...
    {
        Assert(chunk_id < table->chunk_count(),
               "Chunk with given ID does not exist.");
...
void ChunkEncoder::encode_all_chunks(
    ...
    const std::vector<ChunkEncodingSpec> &chunk_encoding_specs)
{
    ...
    Assert(chunk_encoding_specs.size() == chunk_count,
           "Number of encoding specs must match table’s chunk count.");
...
void ChunkEncoder::encode_all_chunks(
    ...
    const ChunkEncodingSpec &chunk_encoding_spec)
{
    Assert(chunk_encoding_spec.size() ==
