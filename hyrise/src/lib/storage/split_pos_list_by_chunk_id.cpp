...
{
    DebugAssert(
        !input_pos_list->references_single_chunk() || input_pos_list->empty(),
        "No need to split a reference segment that references a single chunk");

    // The input_pos_list references multiple chunks and we actually need to
    // split it. Because we are supposed to return shared_ptr<const
    // RowIDPosList>, we first create regular PosLists, add the values to them,
    ...
    {
        DebugAssert(chunk_id < number_of_chunks,
                    "Inconsistent number_of_chunks passed");
        ...
        mapping.original_positions.reserve(input_pos_list->size() /
                                           number_of_chunks);
    }
