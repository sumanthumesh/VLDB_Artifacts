 ...
 *
 */
template <typename Derived> class SegmentIterable
{
  public:
    /**
    ...
    /**
     * @param functor is a generic lambda accepting a SegmentPosition as an
     * argument
     */
    template <typename Functor> void for_each(const Functor &functor) const
    {
        with_iterators(
            [&functor](auto it, auto end)
            {
                for (; it != end; ++it)
                {
                    functor(*it);
     ...
     * @param container   Container with the same value_type as the values in
     * the segment
     */
    ...
    /**
     * Materialize all values in this iterable as std::optional<ValueType>.
     * std::nullopt if value is NULL.
     * @param container   Container with value_type std::pair<bool, T>, where
     *                        bool indicates whether the value is NULL or not
     *                        T is the same as the type of the values in the
     * segment pair in favour over optional to avoid branches for initialization
     */
    template <typename Container>
    void materialize_values_and_nulls(Container &container) const
    {
        ...
        for_each(
            [&](const auto &value) {
                container[index++] =
                    std::make_pair(value.is_null(), value.value());
            });
    }

    /**
     * Materialize all null values in this Iterable.
     * @param container   The container with value_type bool storing the
     * information whether a value is NULL or not
     */
    template <typename Container>
...
{
  public:
    using SegmentIterable<Derived>::with_iterators; // needed because of “name
                                                    // hiding”

    /**
     * @tparam ErasePosListType controls whether AbstractPosLists are erased
     * (i.e., resolved dynamically instead of statically), which reduces the
     * compile time at the cost of virtual method calls during the run time -
     * see the implementation of resolve_pos_list_type.
     * @param  functor is a generic lambda accepting two iterators as arguments
     */
    template <ErasePosListType erase_pos_list_type =
                  ErasePosListType::OnlyInDebugBuild,
              typename Functor>
    void with_iterators(
    ...
    {
        if (!position_filter ||
            dynamic_cast<const EntireChunkPosList *>(&*position_filter))
        {
            _self()._on_with_iterators(functor);
        }
        else
        {
            ...
            {
                resolve_pos_list_type<erase_pos_list_type>(
                    position_filter,
                    [&functor, this](auto &resolved_position_filter) {
                        _self()._on_with_iterators(resolved_position_filter,
                                                   functor);
                    });
            }
        }
    ...
    template <ErasePosListType erase_pos_list_type =
                  ErasePosListType::OnlyInDebugBuild,
              typename Functor>
    void for_each(const std::shared_ptr<const AbstractPosList> &position_filter,
                  const Functor &functor) const
    {
        DebugAssert(!position_filter ||
                        position_filter->references_single_chunk(),
                    "Expected PosList to reference single chunk");
        with_iterators<erase_pos_list_type>(position_filter,
                                            [&functor](auto it, auto end)
                                            {
                                                for (; it != end; ++it)
                                                {
