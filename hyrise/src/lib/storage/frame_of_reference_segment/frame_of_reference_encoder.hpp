  ...
  public:
               ...
               const PolymorphicAllocator<T> &allocator)
    {
        ...
        segment_iterable.with_iterators(
            [&](auto segment_it, auto segment_end)
            {
                ...
                while (segment_it != segment_end)
                {
                    ...
                    for (; value_block_it != current_value_block.end() &&
                    ...
                    {
                        // Make sure that the largest offset fits into uint32_t
                        // (required for vector compression).
                        Assert(static_cast<std::make_unsigned_t<T>>(
                                   max_value - min_value) <=
                    ...
                    for (; value_block_it != this_value_block_end;
                         ++value_block_it, ++current_block_null_values_it)
                    {
                        auto value = *value_block_it;
                        if (*current_block_null_values_it)
                        {
                            // To ensure NULL values do not interfere with the
                            // min/max calculation (needed to calculate (i) the
                            // frame offset and (ii) the required width of the
                            // compressed vector), we set them to the minimum
                            // value. As NULL values are stored as zeros, we
                            // might run in an overflow of the uint32_t when
                            // minimum > 0.
                            value = min_value;
                        }
                        const auto offset =
                            static_cast<uint32_t>(value - min_value);
        ...
        auto compressed_offset_values = compress_vector(
            offset_values, vector_compression_type(), allocator, {max_offset});

        if (segment_contains_null_values)
        {
            return std::make_shared<FrameOfReferenceSegment<T>>(
                std::move(block_minima), std::move(null_values),
                std::move(compressed_offset_values));
        }
        return std::make_shared<FrameOfReferenceSegment<T>>(
            std::move(block_minima), std::nullopt,
            std::move(compressed_offset_values));
    }
};
