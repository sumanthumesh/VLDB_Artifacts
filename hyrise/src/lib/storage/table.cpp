...
// Checks if the two vectors have common elements, e.g., lhs = [0, 2] and rhs =
// [2, 1, 3]. We expect very short vectors, so we use a simple but quadratic
// solution. For larger vectors, we could create sets and check set containment.
bool columns_intersect(const std::vector<ColumnID> &lhs,
                       const std::vector<ColumnID> &rhs)
{
...
bool columns_intersect(const std::set<ColumnID> &lhs,
                       const std::set<ColumnID> &rhs)
{
    for (const auto column_id : lhs)
    {
...
Table::create_dummy_table(const TableColumnDefinitions &column_definitions)
{
...
Table::Table(const TableColumnDefinitions &column_definitions,
             ...
             const std::optional<ChunkOffset> target_chunk_size,
             const UseMvcc use_mvcc,
             const pmr_vector<std::shared_ptr<PartialHashIndex>> &table_indexes)
    : _column_definitions(column_definitions), _type(type), _use_mvcc(use_mvcc),
      _target_chunk_size(type == TableType::Data
                             ? target_chunk_size.value_or(Chunk::DEFAULT_SIZE)
                             : Chunk::MAX_SIZE),
      _append_mutex(std::make_unique<std::mutex>()),
      _table_indexes(table_indexes)
{
    DebugAssert(target_chunk_size <= Chunk::MAX_SIZE,
                "Chunk size exceeds maximum");
    DebugAssert(type == TableType::Data || !target_chunk_size,
                "Must not set target_chunk_size for reference tables");
    DebugAssert(!target_chunk_size || *target_chunk_size > 0,
                "Table must have a chunk size greater than 0.");
}

Table::Table(const TableColumnDefinitions &column_definitions,
             const TableType type, std::vector<std::shared_ptr<Chunk>> &&chunks,
             const UseMvcc use_mvcc,
             pmr_vector<std::shared_ptr<PartialHashIndex>> const &table_indexes)
    : Table(column_definitions, type,
            type == TableType::Data ? std::optional{Chunk::DEFAULT_SIZE}
                                    : std::nullopt,
            use_mvcc, table_indexes)
{
    ...
    if constexpr (HYRISE_DEBUG)
    {
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
        {
            ...
            Assert(chunk->size() > 0 ||
                       (type == TableType::Data &&
                        chunk_id == chunk_count - 1 && chunk->is_mutable()),
            ...
            Assert(chunk->column_count() == num_columns,
                   "Invalid Chunk column count");

            for (auto column_id = ColumnID{0}; column_id < num_columns;
                 ++column_id)
            {
                Assert(chunk->get_segment(column_id)->data_type() ==
                           column_data_type(column_id),
...
{
    return ColumnCount{
        static_cast<ColumnCount::base_type>(_column_definitions.size())};
}
...
ColumnID Table::column_id_by_name(const std::string &column_name) const
{
    ...
    Assert(iter != _column_definitions.end(),
           "Couldn't find column '" + column_name + "'.");
    return ColumnID{static_cast<ColumnID::base_type>(
        std::distance(_column_definitions.begin(), iter))};
}

void Table::append(const std::vector<AllTypeVariant> &values)
{
    auto last_chunk =
        !_chunks.empty() ? get_chunk(ChunkID{chunk_count() - 1}) : nullptr;
    if (!last_chunk || last_chunk->size() >= _target_chunk_size ||
        !last_chunk->is_mutable())
    {
...
void Table::append_mutable_chunk()
{
            ...
            [&](auto type)
            {
                ...
                segments.push_back(
                    std::make_shared<ValueSegment<ColumnDataType>>(
                        column_definition.nullable, _target_chunk_size));
            });
    }
    ...
    {
        mvcc_data = std::make_shared<MvccData>(_target_chunk_size,
...
uint64_t Table::row_count() const
{
    ...
    if (_type == TableType::References)
    {
        ...
        DebugAssert(!_cached_row_count || row_count == *_cached_row_count,
                    "Size of reference table has changed");

        // row_count() is called by AbstractOperator after the operator has
        // finished to fill the performance data. As such, no synchronization is
        // necessary.
...
bool Table::empty() const { return row_count() == 0; }
...
{
    DebugAssert(_type == TableType::Data,
                "target_chunk_size is only valid for data tables");
...
std::shared_ptr<Chunk> Table::get_chunk(ChunkID chunk_id)
{
    DebugAssert(chunk_id < _chunks.size(),
                "ChunkID " + std::to_string(chunk_id) + " out of range");
    if (_type == TableType::References)
    {
        // Not written concurrently, since reference tables are not modified
        // anymore once they are written.
...
{
    DebugAssert(!_chunks.empty(),
                "last_chunk() called on Table without chunks");
    if (_type == TableType::References)
    {
        // Not written concurrently, since reference tables are not modified
        // anymore once they are written.
...
void Table::remove_chunk(ChunkID chunk_id)
{
    DebugAssert(chunk_id < _chunks.size(),
                "ChunkID " + std::to_string(chunk_id) + " out of range");
    DebugAssert((
                    [this, chunk_id]() { // NOLINT
                        const auto chunk = get_chunk(chunk_id);
                        return (chunk->invalid_row_count() == chunk->size());
                    }()),
                "Physical delete of chunk prevented: Chunk needs to be fully "
                "invalidated before.");
    Assert(_type == TableType::Data, "Removing chunks from other tables than "
                                     "data tables is not intended yet.");
    std::atomic_store(&_chunks[chunk_id], std::shared_ptr<Chunk>(nullptr));
}

void Table::append_chunk(
    const Segments &segments, std::shared_ptr<MvccData> mvcc_data, // NOLINT
    const std::optional<PolymorphicAllocator<Chunk>> &alloc)
{
    Assert(_type != TableType::Data ||
               static_cast<bool>(mvcc_data) == (_use_mvcc == UseMvcc::Yes),
           "Supply MvccData to data Tables if MVCC is enabled.");
    AssertInput(static_cast<ColumnCount::base_type>(segments.size()) ==
                    column_count(),
    ...
    if constexpr (HYRISE_DEBUG)
    {
        for (const auto &segment : segments)
        {
            const auto is_reference_segment =
                std::dynamic_pointer_cast<ReferenceSegment>(segment) != nullptr;
            Assert(is_reference_segment == (_type == TableType::References),
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
        {
            ...
            // An empty, mutable chunk at the end is fine, but in that case,
            // append_chunk shouldn't have to be called.
            DebugAssert(
                chunk->size() > 0,
                "append_chunk called on a table that has an empty chunk");
        }
    }

    // tbb::concurrent_vector does not guarantee that elements reported by
    // size() are fully initialized yet.
    // https://oneapi-src.github.io/oneTBB/main/tbb_userguide/concurrent_vector_ug.html
    // To avoid someone reading an incomplete shared_ptr<Chunk>, we (1) use the
    // ZeroAllocator for the concurrent_vector, making sure that an
    // uninitialized entry compares equal to nullptr and (2) insert the desired
    // chunk atomically.

    auto new_chunk_iter = _chunks.push_back(nullptr);
    std::atomic_store(&*new_chunk_iter,
                      std::make_shared<Chunk>(segments, mvcc_data, alloc));
}
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
        ...
        if (row_idx < chunk->size())
        {
            ...
            for (auto column_id = ColumnID{0}; column_id < num_columns;
                 ++column_id)
            {
                row[column_id] = chunk->get_segment(column_id)->operator[](
                    static_cast<ChunkOffset>(row_idx));
            }
            ...
            segment_iterate(
                *chunk->get_segment(column_id),
                [&](const auto &segment_position)
                {
                    ...
                    {
                        rows[chunk_begin_row_idx +
                             segment_position.chunk_offset()][column_id] =
                            segment_position.value();
                    }
                });
        }
...
void Table::set_table_statistics(
...
void Table::create_chunk_index(const std::vector<ColumnID> &column_ids,
                               const std::string &name)
{
    ...
    _chunk_indexes_statistics.emplace_back(
        ChunkIndexStatistics{column_ids, name, chunk_index_type});
}

void Table::add_soft_key_constraint(
    const TableKeyConstraint &table_key_constraint)
{
    Assert(_type == TableType::Data, "TableKeyConstraints are not tracked for "
                                     "reference tables across the PQP.");
    ...
    for (const auto &column_id : table_key_constraint.columns())
    {
        ...
        {
            Assert(!column_is_nullable(column_id),
                   "Column must be non-nullable to comply with PRIMARY KEY.");
        }
    }
    ...
    for (const auto &existing_constraint : _table_key_constraints)
    {
        ...
        // Ensure there is only one key constraint per column(s). Theoretically,
        // there could be two unique constraints {a, b} and {b, c}, but for now
        // we prohibit these cases.
        Assert(!columns_intersect(existing_constraint.columns(),
                                  table_key_constraint.columns()),
               "Another TableKeyConstraint for the same column(s) has already "
               "been defined.");
    }
...
void Table::add_soft_foreign_key_constraint(
    const ForeignKeyConstraint &foreign_key_constraint)
{
    Assert(_type == TableType::Data, "ForeignKeyConstraints are not tracked "
                                     "for reference tables across the PQP.");

    Assert(&*foreign_key_constraint.foreign_key_table() == this,
           "ForeignKeyConstraint is added to the wrong table.");
    ...
    Assert(referenced_table && &*referenced_table != this,
    ...
    {
        Assert(column_id < referenced_table_column_count,
    ...
    {
        // Do not allow intersecting foreign key constraints. Though a table may
        // have unlimited inclusion dependencies for the same columns (and
        // especially the existence of [a] in [x] and [b] in [y] does not mean
        // [a, b] in [x, y] holds), it is reasonable to assume disjoint (soft)
        // foreign keys for now.
        Assert(
            !columns_intersect(existing_constraint.foreign_key_columns(),
                               foreign_key_constraint.foreign_key_columns()),
            "ForeignKeyConstraint for required columns has already been set.");
    }
    _foreign_key_constraints.insert(foreign_key_constraint);
    const auto referenced_table_append_lock =
        referenced_table->acquire_append_mutex();
    referenced_table->_referenced_foreign_key_constraints.insert(
...
void Table::add_soft_order_constraint(
    const TableOrderConstraint &table_order_constraint)
{
    Assert(_type == TableType::Data, "TableOrderConstraints are not tracked "
                                     "for reference tables across the PQP.");
    ...
    {
        // Do not allow intersecting key constraints. Though they can be valid,
        // we are pessimistic for now and notice if we run into intricate cases.
        const auto ordering_columns_invalid =
            columns_intersect(existing_constraint.ordering_columns(),
                              table_order_constraint.ordering_columns()) &&
            existing_constraint.ordered_columns() ==
                table_order_constraint.ordered_columns();
        const auto ordered_columns_invalid =
            columns_intersect(existing_constraint.ordered_columns(),
                              table_order_constraint.ordered_columns()) &&
            existing_constraint.ordering_columns() ==
                table_order_constraint.ordering_columns();
        Assert(
            !ordering_columns_invalid && !ordered_columns_invalid,
...
void Table::set_value_clustered_by(
    const std::vector<ColumnID> &value_clustered_by)
{
    // Ensure that all chunks are finalized because the table should not be
    // altered afterwards.
    const auto chunk_count = _chunks.size();
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
        ...
        Assert(!get_chunk(chunk_id)->is_mutable(),
               "Cannot set value_clustering on table with mutable chunks");
    }
    ...
    if constexpr (HYRISE_DEBUG)
    {
        if (chunk_count > 1)
        {
            ...
            {
                resolve_data_type(
                    _column_definitions[column_id].data_type,
                    [&](const auto column_data_type)
                    {
                        using ColumnDataType =
                            typename decltype(column_data_type)::type;

                        auto value_to_chunk_map =
                            std::unordered_map<ColumnDataType, ChunkID>{};
                                ...
                                *segment,
                                [&](const auto position)
                                {
                                    Assert(!position.is_null(),
                                           "Value clustering is not defined "
                                           "for columns storing NULLs.");

                                    const auto &[iter, inserted] =
                                        value_to_chunk_map.try_emplace(
                                            position.value(), chunk_id);
                                    if (!inserted)
                                    {
                                        Assert(
                                            iter->second == chunk_id,
                                            "Table cannot be set to "
                                            "value-clustered as same value "
...
std::vector<std::shared_ptr<PartialHashIndex>>
Table::get_table_indexes(const ColumnID column_id) const
{
    auto result = std::vector<std::shared_ptr<PartialHashIndex>>();
    std::copy_if(_table_indexes.cbegin(), _table_indexes.cend(),
...
void Table::create_partial_hash_index(const ColumnID column_id,
                                      const std::vector<ChunkID> &chunk_ids)
{
    if (chunk_ids.empty())
    {
        Fail("Creating a partial hash index with no chunks being indexed is "
             "not supported.");
    }

    auto table_index = std::shared_ptr<PartialHashIndex>{};
    auto chunks_to_index =
        std::vector<std::pair<ChunkID, std::shared_ptr<Chunk>>>{};
    ...
    table_index =
        std::make_shared<PartialHashIndex>(chunks_to_index, column_id);

    _table_indexes.emplace_back(table_index);

    _table_indexes_statistics.emplace_back(
        TableIndexStatistics{{column_id}, chunks_to_index});
}

template void Table::create_chunk_index<GroupKeyIndex>(
    const std::vector<ColumnID> &column_ids, const std::string &name);
template void Table::create_chunk_index<CompositeGroupKeyIndex>(
    const std::vector<ColumnID> &column_ids, const std::string &name);
template void Table::create_chunk_index<AdaptiveRadixTreeIndex>(
    const std::vector<ColumnID> &column_ids, const std::string &name);
