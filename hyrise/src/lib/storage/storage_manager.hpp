...
class StorageManager : public Noncopyable
{
  public:
    /**
     * @defgroup Manage Tables, this is only thread-safe for operations on
    ...
    /**
     * @defgroup Manage SQL VIEWs, this is only thread-safe for operations on
     ...
     * this is only thread-safe for operations on prepared plans with different
     * names
     * @{
     */
    void add_prepared_plan(const std::string &name,
                           const std::shared_ptr<PreparedPlan> &prepared_plan);
    std::shared_ptr<PreparedPlan>
    get_prepared_plan(const std::string &name) const;
  ...
  protected:
    ...
    tbb::concurrent_unordered_map<std::string, std::shared_ptr<LQPView>> _views{
        INITIAL_MAP_SIZE};
    tbb::concurrent_unordered_map<std::string, std::shared_ptr<PreparedPlan>>
        _prepared_plans{INITIAL_MAP_SIZE};
};
