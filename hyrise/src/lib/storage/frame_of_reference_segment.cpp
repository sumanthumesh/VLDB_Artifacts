...
FrameOfReferenceSegment<T, U>::FrameOfReferenceSegment(
    ...
    : AbstractEncodedSegment{data_type_from_type<T>()}, _block_minima{std::move(
                                                            block_minima)},
      _null_values{std::move(null_values)}, _offset_values{std::move(
                                                offset_values)},
      _decompressor{_offset_values->create_base_decompressor()}
{
#ifdef NEW_SEG_PRINTS
    log_to_file("New FOR Segment\n");
#endif
}
...
template <typename T, typename U>
const std::optional<pmr_vector<bool>> &
FrameOfReferenceSegment<T, U>::null_values() const
{
    return _null_values;
}
...
template <typename T, typename U>
AllTypeVariant
FrameOfReferenceSegment<T, U>::operator[](const ChunkOffset chunk_offset) const
{
...
FrameOfReferenceSegment<T, U>::copy_using_allocator(
    const PolymorphicAllocator<size_t> &alloc) const
{
    ...
    auto copy = std::make_shared<FrameOfReferenceSegment>(
        std::move(new_block_minima), std::move(null_values),
...
template <typename T, typename U>
size_t FrameOfReferenceSegment<T, U>::memory_usage(
    const MemoryUsageCalculationMode /*mode*/) const
{
    // MemoryUsageCalculationMode ignored since full calculation is efficient.
    size_t segment_size = sizeof(*this) + sizeof(T) * _block_minima.capacity() +
                          _offset_values->data_size() + sizeof(_null_values);
...
FrameOfReferenceSegment<T, U>::compressed_vector_type() const
{
...
// also in reference_segment_iterable.hpp template class
// FrameOfReferenceSegment<int64_t>;
