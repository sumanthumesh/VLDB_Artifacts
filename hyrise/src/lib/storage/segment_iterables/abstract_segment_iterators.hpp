 ...
 *
 * Boost’s iterator facade needs access to the three methods listed in the
 * example. They could be made public, but that’s not really desirable because
 * they are part of an internal interface and should not be accessed by the user
 * of iterator. This friend class gives the iterator facade access to the
 * methods.
 *
 *
 * Example Usage
 ...
 *  private:
 *   friend class boost::iterator_core_access;  // the following methods need to
 * be accessible by the base class
 *
 *   void increment() { ... }
...
class AbstractSegmentIterator
    : public boost::iterator_facade<Derived, Value,
...
struct ChunkOffsetMapping
{
    ...
    ChunkOffset
        offset_in_referenced_chunk; // chunk offset in referenced segment
};
...
class AbstractPointAccessSegmentIterator
    : public AbstractSegmentIterator<Derived, Value>
{
  public:
    explicit AbstractPointAccessSegmentIterator(
        PosListIteratorType position_filter_begin,
        PosListIteratorType position_filter_it)
        : _position_filter_begin{std::move(position_filter_begin)},
          _position_filter_it{std::move(position_filter_it)}
    {
    }
  ...
  protected:
    ...
    {
        DebugAssert(
            _position_filter_it->chunk_offset != INVALID_CHUNK_OFFSET,
            "Invalid ChunkOffset, calling code should handle null values");
        return {static_cast<ChunkOffset>(_position_filter_it -
                                         _position_filter_begin),
                _position_filter_it->chunk_offset};
    }

  private:
    friend class boost::iterator_core_access; // grants the
                                              // boost::iterator_facade access
    ...
    std::ptrdiff_t
    distance_to(const AbstractPointAccessSegmentIterator &other) const
    {
