...
/**
 * @defgroup Uniform interface to create an attribute vector iterable from a
 * dictionary segment
 *
...
inline auto
create_iterable_from_attribute_vector(const BaseDictionarySegment &segment)
{
    return erase_type_from_iterable_if_debug(
        AttributeVectorIterable{segment, segment.null_value_id()});
}
