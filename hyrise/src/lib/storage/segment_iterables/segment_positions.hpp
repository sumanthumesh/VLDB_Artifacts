 ...
 */
template <typename T> class AbstractSegmentPosition
{
  public:
    using Type = T;
...
class SegmentPosition final : public AbstractSegmentPosition<T>
{
  public:
    ...
    SegmentPosition(const T &value, const bool null_value,
  ...
  private:
    // The alignment improves the suitability of the iterator for
    // (auto-)vectorization
...
class IsNullSegmentPosition final : public AbstractSegmentPosition<boost::blank>
{
  public:
    ...
    IsNullSegmentPosition(const bool null_value,
                          const ChunkOffset &chunk_offset)
