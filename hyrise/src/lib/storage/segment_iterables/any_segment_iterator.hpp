 ...
 * a virtual interface.
 */
template <typename T> class AnySegmentIteratorWrapperBase
{
  public:
    ...
    distance_to(const AnySegmentIteratorWrapperBase<T> *other) const = 0;
...
{
  public:
    explicit AnySegmentIteratorWrapper(const Iterator &iterator)
        : _iterator{iterator}
    {
    }
    ...
    /**
     * Although `other` could have a different type, it is practically
     * impossible, since AnySegmentIterator is only used within
     * AnySegmentIterable.
     */
    bool equal(const AnySegmentIteratorWrapperBase<T> *other) const final
    {
        const auto casted_other =
            static_cast<const AnySegmentIteratorWrapper<T, Iterator> *>(other);
        return _iterator == casted_other->_iterator;
    }

    std::ptrdiff_t
    distance_to(const AnySegmentIteratorWrapperBase<T> *other) const final
    {
        const auto casted_other =
            static_cast<const AnySegmentIteratorWrapper<T, Iterator> *>(other);
    ...
    {
        return std::make_unique<AnySegmentIteratorWrapper<T, Iterator>>(
            _iterator);
    }
...
template <typename T> class AnySegmentIterable;
 ...
 *
 * For another example for type erasure see:
 * https://en.wikibooks.org/wiki/More_C%2B%2B_Idioms/Type_Erasure
 */
template <typename T>
class AnySegmentIterator
    : public AbstractSegmentIterator<AnySegmentIterator<T>, SegmentPosition<T>>
{
  public:
    using ValueType = T;
     ...
     * @{
     */
    template <typename U> friend class AnySegmentIterable;

    template <typename Iterator>
    explicit AnySegmentIterator(const Iterator &iterator)
        : _wrapper{
              std::make_unique<detail::AnySegmentIteratorWrapper<T, Iterator>>(
                  iterator)}
    {
    }
  ...
  public:
    AnySegmentIterator(const AnySegmentIterator &other)
