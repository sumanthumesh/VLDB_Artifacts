...
JoinNode::JoinNode(const JoinMode init_join_mode)
    : AbstractLQPNode(LQPNodeType::Join), join_mode(init_join_mode)
{
    Assert(join_mode == JoinMode::Cross,
           "Only Cross Joins can be constructed without predicate");
}

JoinNode::JoinNode(const JoinMode init_join_mode,
                   const std::shared_ptr<AbstractExpression> &join_predicate)
    : JoinNode(init_join_mode,
               std::vector<std::shared_ptr<AbstractExpression>>{join_predicate})
{
}

JoinNode::JoinNode(const JoinMode init_join_mode,
                   const std::vector<std::shared_ptr<AbstractExpression>>
                       &init_join_predicates)
...
JoinNode::output_expressions() const
{
    Assert(left_input() && right_input(),
           "Both inputs need to be set to determine a JoinNode's output "
           "expressions");

    /**
     * Update the JoinNode's output expressions every time they are requested.
     * An overhead, but keeps the LQP code simple. Previously we propagated
     * _input_changed() calls through the LQP every time a node changed and that
     * required a lot of feeble code.
     */
    ...
    auto output_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>{};
    output_expressions.resize(left_expressions.size() +
                              right_expressions.size());

    auto right_begin =
        std::copy(left_expressions.begin(), left_expressions.end(),
                  output_expressions.begin());
...
UniqueColumnCombinations JoinNode::unique_column_combinations() const
{
    ...
    // Semi- and Anti-Joins act as mere filters for input_left(). Thus, existing
    // unique column combinations remain valid.
    if (is_semi_or_anti_join(join_mode))
    {
        return _forward_left_unique_column_combinations();
    }

    const auto &left_unique_column_combinations =
        left_input()->unique_column_combinations();
    const auto &right_unique_column_combinations =
        right_input()->unique_column_combinations();

    return _output_unique_column_combinations(left_unique_column_combinations,
                                              right_unique_column_combinations);
}
...
{
    if (left_unique_column_combinations.empty() &&
    ...
    {
        // No guarantees implemented yet for Cross Joins and multi-predicate
        // joins.
        return UniqueColumnCombinations{};
    }

    DebugAssert(join_mode == JoinMode::Inner || join_mode == JoinMode::Left ||
    ...
    if (!join_predicate ||
        join_predicate->predicate_condition != PredicateCondition::Equals)
    {
        // Also, no guarantees implemented yet for other join predicates than
        // equals_() (Equi Join).
        return UniqueColumnCombinations{};
    }
    ...
    const auto left_operand_is_unique =
        ...
        contains_matching_unique_column_combination(
            left_unique_column_combinations, {join_predicate->left_operand()});
    ...
    {
        // Due to the one-to-one relationship, the UCCs of both sides remain
        // valid.
        auto unique_column_combinations =
            UniqueColumnCombinations{left_unique_column_combinations.begin(),
                                     left_unique_column_combinations.end()};
        unique_column_combinations.insert(
            right_unique_column_combinations.cbegin(),
...
{
    /**
     * In the case of Semi- & Anti-Joins, this node acts as a filter for the
     * left input node. The number of output expressions does not change and
     * therefore we should forward non-trivial FDs as follows:
     */
    if (is_semi_or_anti_join(join_mode))
    {
    ...
    /**
     * When joining tables, we usually lose some or even all unique column
     * combinations from both input tables. This leads to fewer trivial FDs that
     * we can generate from UCCs in upper nodes. To preserve all possible FDs,
     * we manually forward all FDs from input nodes with discarded UCCs.
     */
    ...
    const auto &output_unique_column_combinations =
        _output_unique_column_combinations(left_unique_column_combinations,
                                           right_unique_column_combinations);

    if (output_unique_column_combinations.empty() &&
        !left_unique_column_combinations.empty() &&
        !right_unique_column_combinations.empty())
    {
        // Left and right UCCs are discarded, so we have to manually forward all
        // FDs from the input nodes.
        ...
        fds_right = right_input()->functional_dependencies();
              ...
              output_unique_column_combinations ==
                  right_unique_column_combinations) &&
             !left_unique_column_combinations.empty())
    {
        // Left UCCs are discarded, so we have to manually forward all FDs of
        // the left input node.
        fds_left = left_input()->functional_dependencies();
        fds_right = right_input()->non_trivial_functional_dependencies();
              ...
              output_unique_column_combinations ==
                  left_unique_column_combinations) &&
             !right_unique_column_combinations.empty())
    {
        // Right UCCs are discarded, so we have to manually forward all FDs of
    ...
    else
    {
        ...
        DebugAssert(output_unique_column_combinations.size() ==
                        (left_unique_column_combinations.size() +
                         right_unique_column_combinations.size()),
    ...
    if (!fds_out.empty() &&
        (join_mode == JoinMode::FullOuter || join_mode == JoinMode::Left ||
         join_mode == JoinMode::Right))
    {
    ...
    /**
     * Future Work: In some cases, it is possible to create FDs from the join
     * columns. For example: a) {join_column_a} => {join_column_b} b)
     * {join_column_b} => {join_column_a}
     */
...
{
    Assert(left_input() && right_input(),
           "Need both inputs to determine nullability");

    const auto left_input_column_count =
        left_input()->output_expressions().size();
    ...
    const auto right_column_id = static_cast<ColumnID>(
        column_id - static_cast<ColumnID::base_type>(left_input_column_count));
    return right_input()->is_column_nullable(right_column_id);
}

const std::vector<std::shared_ptr<AbstractExpression>> &
JoinNode::join_predicates() const
{
    return node_expressions;
}

void JoinNode::mark_as_semi_reduction(
    const std::shared_ptr<JoinNode> &reduced_join_node)
{
    Assert(!_is_semi_reduction,
           "The semi reduction status should be set once only.");
    Assert(reduced_join_node, "Reduced JoinNode must be provided.");
    Assert(join_mode == JoinMode::Semi,
           "Semi join reductions require JoinMode::Semi.");
    DebugAssert(join_predicates().size() == 1,
                "Currently, semi join reductions are expected to have a single "
                "join predicate.");
    DebugAssert(std::any_of(reduced_join_node->join_predicates().cbegin(),
                            reduced_join_node->join_predicates().cend(),
                            [&](const auto predicate)
                            { return *predicate == *join_predicates()[0]; }),
                "Both semi join reduction node and the reduced join should "
                "have a common join predicate.");
...
{
    DebugAssert(!_is_semi_reduction || join_mode == JoinMode::Semi,
                "Non-semi join is marked as a semi reduction.");
...
std::shared_ptr<JoinNode> JoinNode::get_or_find_reduced_join_node() const
{
    ...
    {
        // In deep copies of the LQP, the weak pointer to the reduced join is
        // unset (lazy discovery). In such cases, find the reduced join by
        // traversing the LQP upwards.
        const auto &reduction_predicate = *join_predicates()[0];
        visit_lqp_upwards(
            std::const_pointer_cast<AbstractLQPNode>(shared_from_this()),
            [&](const auto &current_node)
            {
                if (current_node->type != LQPNodeType::Join ||
                    current_node.get() == this)
                {
                    return LQPUpwardVisitation::VisitOutputs;
                }
                const auto join_node =
                    std::static_pointer_cast<JoinNode>(current_node);
                if (std::none_of(join_node->join_predicates().begin(),
        ...
        Assert(!_reduced_join_node.expired(),
               "Could not find JoinNode that gets reduced by this semi join "
               "reduction.");
    }
...
JoinNode::_on_shallow_copy(LQPNodeMapping &node_mapping) const
{
    ...
    const auto copied_join_node =
        JoinNode::make(join_mode, expressions_copy_and_adapt_to_different_lqp(
                                      join_predicates(), node_mapping));
    copied_join_node->_is_semi_reduction = _is_semi_reduction;
    return copied_join_node;
}

bool JoinNode::_on_shallow_equals(const AbstractLQPNode &rhs,
                                  const LQPNodeMapping &node_mapping) const
{
    ...
    if (join_mode != join_node.join_mode ||
        _is_semi_reduction != join_node._is_semi_reduction)
    {
        return false;
    }
    return expressions_equal_to_expressions_in_different_lqp(
        join_predicates(), join_node.join_predicates(), node_mapping);
}
