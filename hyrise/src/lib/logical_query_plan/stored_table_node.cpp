...
bool contains_any_column_id(const std::set<ColumnID> &search_columns,
...
StoredTableNode::get_column(const std::string &name) const
{
...
void StoredTableNode::set_pruned_chunk_ids(
    const std::vector<ChunkID> &pruned_chunk_ids)
{
    DebugAssert(
        std::is_sorted(pruned_chunk_ids.begin(), pruned_chunk_ids.end()),
        "Expected sorted vector of ChunkIDs");
    DebugAssert(
        std::adjacent_find(pruned_chunk_ids.begin(), pruned_chunk_ids.end()) ==
            pruned_chunk_ids.end(),
...
void StoredTableNode::set_pruned_column_ids(
    const std::vector<ColumnID> &pruned_column_ids)
{
    DebugAssert(
        std::is_sorted(pruned_column_ids.begin(), pruned_column_ids.end()),
        "Expected sorted vector of ColumnIDs");
    DebugAssert(std::adjacent_find(pruned_column_ids.begin(),
                                   pruned_column_ids.end()) ==
                    pruned_column_ids.end(),
                "Expected vector of unique ColumnIDs");

    // It is valid for an LQP to not use any of the table's columns (e.g.,
    // SELECT 5 FROM t). We still need to include at least one column in the
    // output of this node, which is used by Table::size() to determine the
    // number of 5's.
    const auto stored_column_count =
        Hyrise::get().storage_manager.get_table(table_name)->column_count();
    Assert(pruned_column_ids.size() < static_cast<size_t>(stored_column_count),
           "Cannot exclude all columns from Table.");
...
StoredTableNode::prunable_subquery_predicates() const
{
    ...
    for (const auto &subquery_predicate_ref : _prunable_subquery_predicates)
    {
        ...
        Assert(subquery_predicate,
               "Referenced PredicateNode expired. LQP is invalid.");
...
std::string StoredTableNode::description(const DescriptionMode /*mode*/) const
{
    const auto &stored_table =
        Hyrise::get().storage_manager.get_table(table_name);

    auto stream = std::ostringstream{};
    stream << "[StoredTable] Name: '" << table_name << "' pruned: ";
    stream << _pruned_chunk_ids.size() << "/" << stored_table->chunk_count()
           << " chunk(s), ";
...
StoredTableNode::output_expressions() const
{
...
UniqueColumnCombinations StoredTableNode::unique_column_combinations() const
{
    ...
    for (const auto &table_key_constraint : table_key_constraints)
    {
        ...
        if (contains_any_column_id(table_key_constraint.columns(),
                                   _pruned_column_ids))
        {
            continue;
        }
        ...
        DebugAssert(column_expressions.size() ==
                        table_key_constraint.columns().size(),
...
std::vector<ChunkIndexStatistics>
StoredTableNode::chunk_indexes_statistics() const
{
    ...
    const auto column_id_mapping =
        pruned_column_id_mapping(table->column_count(), _pruned_column_ids);

    // Update index statistics
    // Note: The lambda also modifies statistics.column_ids. This is done
    // because a regular for loop runs into issues when remove(iterator)
    // invalidates the iterator.
    pruned_indexes_statistics.erase(
        std::remove_if(
            pruned_indexes_statistics.begin(), pruned_indexes_statistics.end(),
            [&](auto &statistics)
            {
                for (auto &original_column_id : statistics.column_ids)
                {
                    ...
                    {
                        // Indexed column was pruned - remove index from
                        // statistics
                        return true;
                    }
...
std::vector<TableIndexStatistics>
StoredTableNode::table_indexes_statistics() const
{
    const auto &table = Hyrise::get().storage_manager.get_table(table_name);
    ...
    const auto column_id_mapping =
        pruned_column_id_mapping(input_table_column_count, _pruned_column_ids);

    auto pruned_index_statistics = std::vector<TableIndexStatistics>{};
    pruned_index_statistics.reserve(input_table_column_count -
                                    _pruned_column_ids.size());

    for (const auto &index_statistic : index_statistics)
    {
        // TODO(anyone): When chunk indexes are removed, TableIndexStatistics
        // should no longer store a vector of ColumnIDs as multi-column indexes
        // are no longer supported.
        DebugAssert(index_statistic.column_ids.size() == 1,
                    "Unexpected multi-column index");

        const auto &updated_column_id =
            column_id_mapping[index_statistic.column_ids[0]];
        if (updated_column_id == INVALID_COLUMN_ID)
        {
...
size_t StoredTableNode::_on_shallow_hash() const
{
    ...
    // We intentionally force a hash collision for StoredTableNodes with the
    // same number of prunable subquery predicates even though these predicates
    // are different. Since we assume that (i) these predicates are not often
    // set and (ii) we hash LQPs often, this reduces the hash overhead, makes
    // the code simpler, and triggers an in-depth equality check for the rare
    // cases with (the same number of) prunable subquery predicates.
    boost::hash_combine(hash, _prunable_subquery_predicates.size());
    return hash;
}

std::shared_ptr<AbstractLQPNode>
StoredTableNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    // We cannot copy _prunable_subquery_predicated here since deep_copy()
    // recurses into the input nodes and the StoredTableNodes are the first ones
    // to be copied. Instead, AbstractLQPNode::deep_copy() sets the copied
...
bool StoredTableNode::_on_shallow_equals(
    const AbstractLQPNode &rhs, const LQPNodeMapping &node_mapping) const
{
    const auto &stored_table_node = static_cast<const StoredTableNode &>(rhs);
    if (table_name != stored_table_node.table_name ||
    ...
    // Check equality of prunable subquery predicates. For now, the order of the
    // predicates matters. Though this is a missed opportunity for LQP
    // deduplication, we do not consider this a problem for now.
    const auto &prunable_subquery_predicates =
        this->prunable_subquery_predicates();
    const auto &rhs_prunable_subquery_predicates =
        stored_table_node.prunable_subquery_predicates();
    ...
    for (auto predicate_idx = size_t{0};
         predicate_idx < subquery_predicate_count; ++predicate_idx)
    {
        // We cannot check that the PredicateNodes are equal since this equality
        // check recurses into the inputs und we do not terminate. We have to
        // compare the predicate expressions.
        if (!expressions_equal_to_expressions_in_different_lqp(
                prunable_subquery_predicates[predicate_idx]->node_expressions,
                rhs_prunable_subquery_predicates[predicate_idx]
                    ->node_expressions,
                node_mapping))
        {
            return false;
...
void StoredTableNode::_set_output_expressions() const
{
    ...
    // Create `_output_expressions` sized with respect to the
    // `_pruned_column_ids`
    const auto unpruned_column_count =
        stored_column_count - _pruned_column_ids.size();
    _output_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>(unpruned_column_count);

    auto pruned_column_ids_iter = _pruned_column_ids.begin();
    auto output_column_id = ColumnID{0};
    for (auto stored_column_id = ColumnID{0};
         stored_column_id < stored_column_count; ++stored_column_id)
    {
        ...
        (*_output_expressions)[output_column_id] =
            std::make_shared<LQPColumnExpression>(shared_from_this(),
                                                  stored_column_id);
        ++output_column_id;
    }
}
