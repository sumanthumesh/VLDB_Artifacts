 ...
 * It is useful in tests (e.g. general LQP tests, optimizer tests that just rely
 * on statistics and not actual data) and the playground
 */
class MockNode : public EnableMakeForLQPNode<MockNode>, public AbstractLQPNode
{
  public:
    ...
    get_column(const std::string &column_name) const;
    ...
    // Generates UCCs from table's key constraints and drops UCCs that include
    // pruned columns.
    ...
    // Pure container functionality: MockNode does not use key constraints
    // internally.
    void set_key_constraints(const TableKeyConstraints &key_constraints);
    const TableKeyConstraints &key_constraints() const;

    void
    set_non_trivial_functional_dependencies(const FunctionalDependencies &fds);
