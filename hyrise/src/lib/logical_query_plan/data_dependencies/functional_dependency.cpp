...
bool FunctionalDependency::operator==(const FunctionalDependency &other) const
{
    ...
    if (determinants.size() != other.determinants.size() ||
        dependents.size() != other.dependents.size())
    {
        return false;
    }
...
size_t FunctionalDependency::hash() const
{
    ...
    {
        // To make the hash independent of the expressions' order, we have to
        // use a commutative operator like XOR.
...
{
    stream << "{";
    auto determinants_vector = std::vector<std::shared_ptr<AbstractExpression>>{
        expression.determinants.begin(), expression.determinants.end()};
    stream << determinants_vector.at(0)->as_column_name();
    for (auto determinant_idx = size_t{1};
         determinant_idx < determinants_vector.size(); ++determinant_idx)
    {
        stream << ", "
               << determinants_vector[determinant_idx]->as_column_name();
    }

    stream << "} => {";
    auto dependents_vector = std::vector<std::shared_ptr<AbstractExpression>>{
        expression.dependents.begin(), expression.dependents.end()};
    stream << dependents_vector.at(0)->as_column_name();
    for (auto dependent_idx = size_t{1};
         dependent_idx < dependents_vector.size(); ++dependent_idx)
    {
...
FunctionalDependencies inflate_fds(const FunctionalDependencies &fds)
{
    ...
    for (const auto &fd : fds)
    {
        ...
        else
        {
            ...
            {
                inflated_fds.emplace(fd.determinants,
                                     ExpressionUnorderedSet{dependent});
            }
        }
    }
...
FunctionalDependencies deflate_fds(const FunctionalDependencies &fds)
{
    ...
    // We cannot use a set here as we want to add dependents to existing FDs and
    // objects in sets are immutable.
    auto existing_fds = std::vector<
        std::pair<ExpressionUnorderedSet, ExpressionUnorderedSet>>{};
    existing_fds.reserve(fds.size());

    for (const auto &fd_to_add : fds)
    {
        // Check if we have seen an FD with the same determinants.
        auto existing_fd = std::find_if(
            existing_fds.begin(), existing_fds.end(),
            [&](const auto &fd)
            {
                // Quick check for cardinality.
        ...
        // If we have found an FD with same determinants, add the dependents.
        // Otherwise, add a new FD.
        if (existing_fd != existing_fds.end())
        {
            existing_fd->second.insert(fd_to_add.dependents.cbegin(),
                                       fd_to_add.dependents.cend());
        }
        else
        {
            existing_fds.emplace_back(fd_to_add.determinants,
                                      fd_to_add.dependents);
        }
    }
...
size_t hash<hyrise::FunctionalDependency>::operator()(
    const hyrise::FunctionalDependency &fd) const
{
    return fd.hash();
}
