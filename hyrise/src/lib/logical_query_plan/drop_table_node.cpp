...
DropTableNode::DropTableNode(const std::string &init_table_name,
                             const bool init_if_exists)
    : AbstractNonQueryNode(LQPNodeType::DropTable), table_name(init_table_name),
...
DropTableNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    return DropTableNode::make(table_name, if_exists);
}

bool DropTableNode::_on_shallow_equals(
