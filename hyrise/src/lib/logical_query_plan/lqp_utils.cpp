...
void lqp_create_node_mapping_impl(LQPNodeMapping &mapping,
                                  const std::shared_ptr<AbstractLQPNode> &lhs,
                                  const std::shared_ptr<AbstractLQPNode> &rhs)
{
    ...
    Assert(lhs->type == rhs->type,
           "LQPs aren't equally structured, can't create mapping");

    // To avoid traversing subgraphs of ORs twice, check whether we've been here
    // already
    ...
    lqp_create_node_mapping_impl(mapping, lhs->right_input(),
                                 rhs->right_input());
}

std::optional<LQPMismatch>
lqp_find_structure_mismatch(const std::shared_ptr<const AbstractLQPNode> &lhs,
                            const std::shared_ptr<const AbstractLQPNode> &rhs)
{
    ...
    auto mismatch_left =
        lqp_find_structure_mismatch(lhs->left_input(), rhs->left_input());
    if (mismatch_left)
    {
...
std::optional<LQPMismatch> lqp_find_subplan_mismatch_impl(
    ...
    const std::shared_ptr<const AbstractLQPNode> &rhs)
{
    ...
    auto mismatch_left = lqp_find_subplan_mismatch_impl(
        node_mapping, lhs->left_input(), rhs->left_input());
    if (mismatch_left)
    {
        return mismatch_left;
    }

    return lqp_find_subplan_mismatch_impl(node_mapping, lhs->right_input(),
                                          rhs->right_input());
}

void lqp_find_subplan_roots_impl(
    std::vector<std::shared_ptr<AbstractLQPNode>> &root_nodes,
            ...
            for (const auto &expression : sub_node->node_expressions)
            {
                visit_expression(
                    ...
                    [&](const auto sub_expression)
                    {
                        ...
                        {
                            lqp_find_subplan_roots_impl(
                                root_nodes, visited_nodes,
...
void recursively_collect_lqp_subquery_expressions_by_lqp(
    SubqueryExpressionsByLQP &subquery_expressions_by_lqp,
    ...
    const bool only_correlated)
{
    ...
    for (const auto &expression : node->node_expressions)
    {
        visit_expression(
            ...
            [&](const auto &sub_expression)
            {
                ...
                {
                    for (auto &[lqp, subquery_expressions] :
                         subquery_expressions_by_lqp)
                    {
                        if (*lqp == *subquery_expression->lqp)
                        {
                            subquery_expressions.emplace_back(
                                subquery_expression);
                            return ExpressionVisitation::DoNotVisitArguments;
                        }
                    }
                    subquery_expressions_by_lqp.emplace(
                        subquery_expression->lqp,
                        std::vector{std::weak_ptr<LQPSubqueryExpression>(
                            subquery_expression)});
                }

                // Subqueries can be nested. We are also interested in the LQPs
                // from deeply nested subqueries.
                recursively_collect_lqp_subquery_expressions_by_lqp(
                    subquery_expressions_by_lqp, subquery_expression->lqp,
    ...
    recursively_collect_lqp_subquery_expressions_by_lqp(
        subquery_expressions_by_lqp, node->left_input(), visited_nodes,
        only_correlated);
    recursively_collect_lqp_subquery_expressions_by_lqp(
        subquery_expressions_by_lqp, node->right_input(), visited_nodes,
        only_correlated);
}
...
SubqueryExpressionsByLQP collect_lqp_subquery_expressions_by_lqp(
    const std::shared_ptr<AbstractLQPNode> &node, const bool only_correlated)
{
    auto visited_nodes = std::unordered_set<std::shared_ptr<AbstractLQPNode>>();
    auto subqueries_by_lqp = SubqueryExpressionsByLQP{};
    recursively_collect_lqp_subquery_expressions_by_lqp(
        subqueries_by_lqp, node, visited_nodes, only_correlated);

    return subqueries_by_lqp;
}

LQPNodeMapping
lqp_create_node_mapping(const std::shared_ptr<AbstractLQPNode> &lhs,
                        const std::shared_ptr<AbstractLQPNode> &rhs)
{
...
std::optional<LQPMismatch>
lqp_find_subplan_mismatch(const std::shared_ptr<const AbstractLQPNode> &lhs,
                          const std::shared_ptr<const AbstractLQPNode> &rhs)
{
    ...
    // For lqp_create_node_mapping() we need mutable pointers - but won't use
    // them to manipulate, promised. It's just that NodeMapping has takes a
    // mutable ptr in as the value type
                      ...
                      const std::shared_ptr<AbstractLQPNode> &replacement_node)
{
    ...
    DebugAssert(!replacement_node->left_input() &&
                    !replacement_node->right_input(),
    ...
    {
        outputs[output_idx]->set_input(input_sides[output_idx],
                                       replacement_node);
    }
...
{
    Assert(
        allow_right_input == AllowRightInput::Yes || !node->right_input(),
    ...
    /**
     * Hold left_input ptr in extra variable to keep the ref count up and untie
     * it from this node. left_input might be nullptr
     */
     ...
     * If left_input is nullptr, still call set_input so this node will get
     * untied from the LQP.
     */
    const auto output_count = outputs.size();
    for (auto output_idx = size_t{0}; output_idx < output_count; ++output_idx)
...
void lqp_insert_node(const std::shared_ptr<AbstractLQPNode> &parent_node,
                     const LQPInputSide input_side,
                     const std::shared_ptr<AbstractLQPNode> &node_to_insert,
                     const AllowRightInput allow_right_input)
{
    DebugAssert(!node_to_insert->left_input(),
                "Expected node without a left input.");
    DebugAssert(!node_to_insert->right_input() ||
                    allow_right_input == AllowRightInput::Yes,
                "Expected node without a right input.");
    DebugAssert(node_to_insert->output_count() == 0,
                "Expected node without outputs.");
...
bool lqp_is_validated(const std::shared_ptr<AbstractLQPNode> &lqp)
{
    ...
    return lqp_is_validated(lqp->left_input()) &&
           lqp_is_validated(lqp->right_input());
}

std::set<std::string>
lqp_find_modified_tables(const std::shared_ptr<AbstractLQPNode> &lqp)
{
    ...
    visit_lqp(
        ...
        [&](const auto &node)
        {
            ...
            case LQPNodeType::Insert:
                modified_tables.insert(
                    static_cast<InsertNode &>(*node).table_name);
                break;
            case LQPNodeType::Update:
                modified_tables.insert(
                    static_cast<UpdateNode &>(*node).table_name);
                break;
            case LQPNodeType::Delete:
            {
                visit_lqp(
                    node->left_input(),
                    [&](const auto &sub_delete_node)
                    {
                        if (const auto stored_table_node =
                                std::dynamic_pointer_cast<StoredTableNode>(
                                    sub_delete_node))
                        {
                            modified_tables.insert(
                                stored_table_node->table_name);
                        }
                        else if (const auto mock_node =
                                     std::dynamic_pointer_cast<MockNode>(
                                         sub_delete_node))
                        {
                            if (mock_node->name)
                            {
            ...
            case LQPNodeType::ChangeMetaTable:
                modified_tables.insert(
                    static_cast<ChangeMetaTableNode &>(*node).table_name);
                break;
...
{
/**
 * Function creates a boolean expression from an lqp. It traverses the passed
 * lqp from top to bottom. However, an lqp is evaluated from bottom to top. This
 * requires that the order in which the translated expressions are added to the
 * output expression is the reverse order of how the nodes are traversed. The
 * subsequent_expression parameter passes the translated expressions to the
 * translation of its children nodes, which allows to add the translated
 * expression of child node before its parent node to the output expression.
 */
std::shared_ptr<AbstractExpression> lqp_subplan_to_boolean_expression_impl(
    const std::shared_ptr<AbstractLQPNode> &begin,
    const std::optional<const std::shared_ptr<AbstractLQPNode>> &end,
    const std::optional<const std::shared_ptr<AbstractExpression>>
        &subsequent_expression)
{
    if (end && begin == *end)
    {
        return nullptr;
    ...
    case LQPNodeType::Predicate:
    {
        const auto predicate_node =
            std::dynamic_pointer_cast<PredicateNode>(begin);
        const auto predicate = predicate_node->predicate();
        auto expression = subsequent_expression
                              ? and_(predicate, *subsequent_expression)
                              : predicate;
        auto left_input_expression = lqp_subplan_to_boolean_expression_impl(
            begin->left_input(), end, expression);
    ...
    case LQPNodeType::Union:
    {
        ...
        const auto left_input_expression =
            lqp_subplan_to_boolean_expression_impl(begin->left_input(), end,
                                                   std::nullopt);
        const auto right_input_expression =
            lqp_subplan_to_boolean_expression_impl(begin->right_input(), end,
                                                   std::nullopt);
        if (left_input_expression && right_input_expression)
        {
            const auto or_expression =
                or_(left_input_expression, right_input_expression);
            return subsequent_expression
                       ? and_(or_expression, *subsequent_expression)
                       : or_expression;
        }
    ...
    case LQPNodeType::Validate:
    case LQPNodeType::Limit:
        return lqp_subplan_to_boolean_expression_impl(begin->left_input(), end,
                                                      subsequent_expression);
...
// Function wraps the call to the lqp_subplan_to_boolean_expression_impl()
// function to hide its third parameter, subsequent_predicate, which is only
// used internally.
...
lqp_find_subplan_roots(const std::shared_ptr<AbstractLQPNode> &lqp)
{
...
std::vector<std::shared_ptr<AbstractLQPNode>>
lqp_find_nodes_by_type(const std::shared_ptr<AbstractLQPNode> &lqp,
...
std::vector<std::shared_ptr<AbstractLQPNode>>
lqp_find_leaves(const std::shared_ptr<AbstractLQPNode> &lqp)
{
    std::vector<std::shared_ptr<AbstractLQPNode>> nodes;
    visit_lqp(lqp,
              [&](const auto &node)
...
{
    DebugAssert(lqp_node.type == LQPNodeType::StoredTable ||
                    lqp_node.type == LQPNodeType::StaticTable ||
                    lqp_node.type == LQPNodeType::Mock,
                "Did not expect other node types than StoredTableNode, "
                "StaticTableNode and MockNode.");
    ...
    for (const auto &output_expression : output_expressions)
    {
        const auto column_expression =
            dynamic_pointer_cast<LQPColumnExpression>(output_expression);
        ...
        if (std::find(column_ids.cbegin(), column_ids.cend(),
                      original_column_id) != column_ids.cend() &&
            *column_expression->original_node.lock() == lqp_node)
        {
            [[maybe_unused]] const auto [_, success] =
                column_expressions.emplace(column_expression);
            DebugAssert(success, "Did not expect multiple column expressions "
                                 "for the same column id.");
        }
    }
...
template ExpressionUnorderedSet
find_column_expressions(const AbstractLQPNode &lqp_node,
...
{
    DebugAssert(!unique_column_combinations.empty(),
                "Invalid input: Set of UCCs should not be empty.");
    DebugAssert(!expressions.empty(),
                "Invalid input: Set of expressions should not be empty.");

    // Look for a unique column combination that is based on a subset of the
    // given expressions.
...
FunctionalDependencies fds_from_unique_column_combinations(
    ...
    const UniqueColumnCombinations &unique_column_combinations)
{
    Assert(!unique_column_combinations.empty(),
           "Did not expect empty set of UCCs.");
    ...
    for (auto column_id = ColumnID{0}; column_id < output_expressions.size();
         ++column_id)
    {
        if (!lqp->is_column_nullable(column_id))
        {
            output_expressions_non_nullable.insert(
                output_expressions.at(column_id));
        }
    }
    ...
    for (const auto &ucc : unique_column_combinations)
    {
        ...
        // (1) Verify whether we can create an FD from the given UCC
        // (non-nullable determinant expressions).
        if (!std::all_of(determinants.cbegin(), determinants.cend(),
                         [&](const auto &determinant_expression) {
                             return output_expressions_non_nullable.contains(
...
void remove_invalid_fds(const std::shared_ptr<const AbstractLQPNode> &lqp,
                        FunctionalDependencies &fds)
{
    ...
    const auto &output_expressions = lqp->output_expressions();
    const auto &output_expressions_set = ExpressionUnorderedSet{
        output_expressions.cbegin(), output_expressions.cend()};

    // Adjust FDs: Remove dependents that are not part of the node's output
    // expressions.
    const auto not_part_of_output_expressions =
    ...
    const auto fd_is_invalid = [&](auto &fd)
    {
        ...
        for (const auto &fd_determinant_expression : fd.determinants)
        {
            ...
            const auto expression_idx = find_expression_idx(
                *fd_determinant_expression, output_expressions);
     ...
     *               - {a, b} => {c, SUM(d)}
     *               - {a}    => {b, c}
     *              Because we already have {a} => {c}, we do not need {a, b} =>
     * {c}. Therefore, we should change our set of FDs to the following:
     *               - {a, b} => {SUM(d)}
     *               - {a}    => {b, c}
     */
}
...
{
    Assert(union_root_node->type == LQPNodeType::Union,
           "Expecting UnionNode as the diamond's root node.");
    DebugAssert(union_root_node->input_count() > 1,
                "Diamond root node does not have two inputs.");
    bool is_diamond = true;
    std::optional<std::shared_ptr<AbstractLQPNode>> diamond_origin_node;
    visit_lqp(union_root_node,
              [&](const auto &diamond_node)
              {
                  ...
                  if (diamond_node->output_count() > 1)
                  {
                      ...
                      {
                          // The LQP traversal should always end in the same
                          // origin node having multiple outputs. Since we found
                          // two different origin nodes, we abort the traversal.
                          is_diamond = false;
                      }
                      return LQPVisitation::DoNotVisitInputs;
                  }
                  ...
                  {
                      // The traversal ends because we reached a MockNode,
                      // StoredTableNode or StaticTableNode. Since we did not
                      // find a node with multiple outputs yet, union_root_node
                      // cannot be considered the root of a diamond.
                      is_diamond = false;
                  }
