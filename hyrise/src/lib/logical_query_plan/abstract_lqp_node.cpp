...
void collect_lqps_in_plan(
    const AbstractLQPNode &lqp,
 ...
 */
void collect_lqps_from_expression(
    ...
    visit_expression(
        ...
        [&](const auto &sub_expression)
        {
            const auto subquery_expression =
                std::dynamic_pointer_cast<const LQPSubqueryExpression>(
                    sub_expression);
...
AbstractLQPNode::~AbstractLQPNode()
{
    Assert(_outputs.empty(), "There are outputs that should still reference "
                             "this node. Thus this node shouldn't get deleted");

    // We're in the destructor, thus we must make sure we're not calling any
    // virtual methods - so we're doing the removal directly instead of calling
    // set_left_input/right_input(nullptr)
    if (_inputs[0])
    {
...
size_t AbstractLQPNode::hash() const
{
    ...
    visit_lqp(shared_from_this(),
              [&hash](const auto &node)
              {
                  if (node)
                  {
...
void AbstractLQPNode::set_left_input(
    const std::shared_ptr<AbstractLQPNode> &left)
{
    set_input(LQPInputSide::Left, left);
}

void AbstractLQPNode::set_right_input(
    const std::shared_ptr<AbstractLQPNode> &right)
{
    DebugAssert(
        right == nullptr || type == LQPNodeType::Join ||
...
void AbstractLQPNode::set_input(LQPInputSide side,
                                const std::shared_ptr<AbstractLQPNode> &input)
{
    // We need a reference to _inputs[input_idx], so not calling
    // this->input(side)
...
size_t AbstractLQPNode::input_count() const
{
     ...
     */
    return _inputs.size() -
           std::count(_inputs.cbegin(), _inputs.cend(), nullptr);
}

LQPInputSide AbstractLQPNode::get_input_side(
    const std::shared_ptr<AbstractLQPNode> &output) const
{
...
// clang-tidy wants this to be const. Technically, it could be, but as this node
// will be modified via set_input, it is syntactically incorrect.
void AbstractLQPNode::remove_output(
    const std::shared_ptr<AbstractLQPNode> &output)
{ // NOLINT
...
{
    // Don't use for-each loop here, as remove_output manipulates the _outputs
    // vector
...
std::vector<LQPOutputRelation> AbstractLQPNode::output_relations() const
{
    ...
    for (auto output_idx = size_t{0}; output_idx < output_relation_count;
         ++output_idx)
    {
        output_relations[output_idx] =
            LQPOutputRelation{outputs[output_idx], input_sides[output_idx]};
    }
...
AbstractLQPNode::deep_copy(LQPNodeMapping node_mapping) const
{
    ...
    // copies of these PredicateNodes after copying the entire LQP (see
...
bool AbstractLQPNode::shallow_equals(const AbstractLQPNode &rhs,
...
AbstractLQPNode::output_expressions() const
{
    Assert(left_input() && !right_input(),
           "Can only forward input expressions iff there is a left input and "
           "no right input");
    return left_input()->output_expressions();
}

std::optional<ColumnID>
AbstractLQPNode::find_column_id(const AbstractExpression &expression) const
{
...
ColumnID
AbstractLQPNode::get_column_id(const AbstractExpression &expression) const
{
    const auto column_id = find_column_id(expression);
    Assert(column_id,
           "This node has no column '" + expression.as_column_name() + "'.");
    return *column_id;
}

bool AbstractLQPNode::has_output_expressions(
...
bool AbstractLQPNode::is_column_nullable(const ColumnID column_id) const
{
    ...
    Assert(left_input() && !right_input(),
           "Can forward nullability from input iff there is a left input and "
           "no right input");
    return left_input()->is_column_nullable(column_id);
}

bool AbstractLQPNode::has_matching_ucc(
    const ExpressionUnorderedSet &expressions) const
{
    ...
    DebugAssert(has_output_expressions(expressions),
                "The given expressions are not a subset of the LQP's output "
                "expressions.");
    ...
    return contains_matching_unique_column_combination(
        unique_column_combinations, expressions);
}
...
FunctionalDependencies AbstractLQPNode::functional_dependencies() const
{
    ...
    if constexpr (HYRISE_DEBUG)
    {
        ...
        for (const auto &fd : non_trivial_fds)
        {
            ...
            Assert(inserted, "FDs with the same set of determinant expressions "
                             "should be merged.");

            for (const auto &fd_determinant_expression : fd.determinants)
            {
                Assert(
                    output_expressions_set.contains(fd_determinant_expression),
                    "Expected FD's determinant expressions to be a subset of "
                    "the node's output expressions.");
                Assert(!is_column_nullable(
                           get_column_id(*fd_determinant_expression)),
                       "Expected FD's determinant expressions to be "
                       "non-nullable.");
            }
            Assert(std::all_of(fd.dependents.cbegin(), fd.dependents.cend(),
                               [&output_expressions_set](
                                   const auto &fd_dependent_expression) {
                                   return output_expressions_set.contains(
                                       fd_dependent_expression);
                               }),
                   "Expected the FD's dependent expressions to be a subset of "
                   "the node's output expressions.");
        }
    }
    ...
    const auto &trivial_fds = fds_from_unique_column_combinations(
        shared_from_this(), unique_column_combinations);
...
AbstractLQPNode::non_trivial_functional_dependencies() const
{
    if (left_input())
    {
        Assert(!right_input(), "Expected single input node for implicit FD "
                               "forwarding. Please override this function.");
        return left_input()->non_trivial_functional_dependencies();
    }

    // For instance, StoredTableNode or StaticTableNode cannot provide any
    // non-trivial FDs.
    return {};
}
...
bool AbstractLQPNode::operator==(const AbstractLQPNode &rhs) const
{
    ...
    return !lqp_find_subplan_mismatch(shared_from_this(),
...
std::shared_ptr<AbstractLQPNode>
AbstractLQPNode::_deep_copy_impl(LQPNodeMapping &node_mapping) const
{
...
std::shared_ptr<AbstractLQPNode>
AbstractLQPNode::_shallow_copy(LQPNodeMapping &node_mapping) const
{
...
{
    const auto iter = std::find_if(_outputs.begin(), _outputs.end(),
                                   [&](const auto &other)
                                   {
                                       /**
                                        * HACK!
                                        *  Normally we'd just check `&output ==
                                        * other.lock().get()` here. BUT (this is
                                        * the hacky part), we're checking for
                                        * `other.expired()` here as well and
                                        * accept an expired element as a match.
                                        * If nothing else breaks the only way we
                                        * might get an expired element is if
                                        * `other` is the expired weak_ptr<> to
                                        * `output` - and thus the element we're
                                        ...
                                        *
                                        * node_b.reset(); //
                                        * node_b::~AbstractLQPNode() will call
                                        * `node_a.remove_output_pointer(node_b)`
                                        *                 // But we can't lock
                                        * node_b anymore, since its ref count is
                                        * already 0
                                        */
                                       return &output == other.lock().get() ||
                                              other.expired();
                                   });
    DebugAssert(
        iter != _outputs.end(),
        "Specified output node is not actually a output node of this node.");

    /**
     * TODO(anybody) This is actually a O(n) operation, could be O(1) by just
     * swapping the last element into the deleted element.
     */
    _outputs.erase(iter);
}

void AbstractLQPNode::_add_output_pointer(
    const std::shared_ptr<AbstractLQPNode> &output)
{
...
AbstractLQPNode::_forward_left_unique_column_combinations() const
{
    Assert(left_input(),
           "Cannot forward unique column combinations without an input node.");
    const auto &input_unique_column_combinations =
        left_input()->unique_column_combinations();
    ...
    if constexpr (HYRISE_DEBUG)
    {
        ...
        for (const auto &ucc : input_unique_column_combinations)
        {
            Assert(has_output_expressions(ucc.expressions),
                   "Forwarding of UCC is illegal because node misses output "
                   "expressions.");
        }
    }
...
AbstractLQPNode::_expression_description_mode(const DescriptionMode mode)
{
    switch (mode)
    {
...
{
    // Recursively collect all LQPs in LQPSubqueryExpressions (and any anywhere
    // within those) in this LQP into a list and then print them
    ...
    const auto output_lqp_to_stream = [&](const auto &root)
    {
        ...
        const auto node_print_fn = [](const auto &node2, auto &stream2)
        {
            stream2 << node2->description(
                AbstractLQPNode::DescriptionMode::Detailed);
        ...
        print_directed_acyclic_graph<const AbstractLQPNode>(
            root.shared_from_this(), get_inputs_fn, node_print_fn, stream);
    };
