...
LimitNode::_on_shallow_copy(LQPNodeMapping &node_mapping) const
{
    return LimitNode::make(expression_copy_and_adapt_to_different_lqp(
        *num_rows_expression(), node_mapping));
}

bool LimitNode::_on_shallow_equals(const AbstractLQPNode &rhs,
                                   const LQPNodeMapping &node_mapping) const
{
    ...
    return expression_equal_to_expression_in_different_lqp(
        *num_rows_expression(), *limit_node.num_rows_expression(),
        node_mapping);
}
