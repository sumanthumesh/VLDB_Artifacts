...
CreateTableNode::CreateTableNode(const std::string &init_table_name,
...
CreateTableNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    return CreateTableNode::make(table_name, if_not_exists, left_input());
}

bool CreateTableNode::_on_shallow_equals(
    const AbstractLQPNode &rhs, const LQPNodeMapping & /*node_mapping*/) const
{
    ...
    return table_name == create_table_node.table_name &&
