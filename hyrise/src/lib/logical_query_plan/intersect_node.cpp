...
{
    /**
     * Because INTERSECT acts as a pure filter for both input tables, all unique
     * column combinations remain valid.
     *
     * Future Work: Merge unique column combinations from the left and right
     * input node.
     */
    DebugAssert(left_input()->unique_column_combinations() ==
...
IntersectNode::non_trivial_functional_dependencies() const
{
...
IntersectNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    return IntersectNode::make(set_operation_mode);
}

bool IntersectNode::_on_shallow_equals(
