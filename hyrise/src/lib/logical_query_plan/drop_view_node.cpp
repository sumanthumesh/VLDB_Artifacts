...
DropViewNode::DropViewNode(const std::string &init_view_name,
                           const bool init_if_exists)
    : AbstractNonQueryNode(LQPNodeType::DropView), view_name(init_view_name),
...
DropViewNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    return DropViewNode::make(view_name, if_exists);
}

bool DropViewNode::_on_shallow_equals(
    const AbstractLQPNode &rhs, const LQPNodeMapping & /*node_mapping*/) const
{
    ...
    return view_name == drop_view_node.view_name &&
