...
{
    Assert(expressions.size() == aliases.size(),
           "Number of expressions and number of aliases has to be equal.");
}
...
std::string AliasNode::description(const DescriptionMode mode) const
{
    ...
    stream << "[Alias] ";
    for (auto column_id = ColumnID{0}; column_id < node_expressions.size();
         ++column_id)
    {
        if (node_expressions[column_id]->description(expression_mode) ==
            aliases[column_id])
        {
        ...
        {
            stream << node_expressions[column_id]->description(expression_mode)
                   << " AS " << aliases[column_id];
        }
...
AliasNode::output_expressions() const
{
...
AliasNode::_on_shallow_copy(LQPNodeMapping &node_mapping) const
{
    return std::make_shared<AliasNode>(
        expressions_copy_and_adapt_to_different_lqp(node_expressions,
                                                    node_mapping),
        aliases);
}

bool AliasNode::_on_shallow_equals(const AbstractLQPNode &rhs,
                                   const LQPNodeMapping &node_mapping) const
{
    ...
    return expressions_equal_to_expressions_in_different_lqp(
               node_expressions, alias_node.node_expressions, node_mapping) &&
