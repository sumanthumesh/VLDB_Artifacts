...
std::string StaticTableNode::description(const DescriptionMode /*mode*/) const
{
    ...
    for (auto column_id = ColumnID{0};
         column_id < table->column_definitions().size(); ++column_id)
    {
...
StaticTableNode::output_expressions() const
{
...
StaticTableNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    return StaticTableNode::make(table);
}

bool StaticTableNode::_on_shallow_equals(
    const AbstractLQPNode &rhs, const LQPNodeMapping & /*node_mapping*/) const
{
    ...
    return table->column_definitions() ==
               static_table_node.table->column_definitions() &&
           table->soft_key_constraints() ==
               static_table_node.table->soft_key_constraints();
}
