...
ExceptNode::ExceptNode(const SetOperationMode init_operation_mode)
    : AbstractLQPNode(LQPNodeType::Except),
...
{
    // Because EXCEPT acts as a pure filter for the left input table, all unique
    // column combinations from the left input node remain valid.
    return _forward_left_unique_column_combinations();
}

FunctionalDependencies ExceptNode::non_trivial_functional_dependencies() const
{
    // The right input node is used for filtering only. It does not contribute
    // any FDs.
...
ExceptNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    return ExceptNode::make(set_operation_mode);
}

bool ExceptNode::_on_shallow_equals(
