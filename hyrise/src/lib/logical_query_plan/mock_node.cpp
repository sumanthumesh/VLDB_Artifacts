...
MockNode::MockNode(const ColumnDefinitions &column_definitions,
                   const std::optional<std::string> &init_name)
    : AbstractLQPNode(LQPNodeType::Mock), name(init_name),
      _column_definitions(column_definitions)
{
}

std::shared_ptr<LQPColumnExpression>
MockNode::get_column(const std::string &column_name) const
{
         ...
         ++column_id)
    {
        ...
        {
            return std::make_shared<LQPColumnExpression>(shared_from_this(),
                                                         column_id);
        }
    }
...
MockNode::output_expressions() const
{
    // Need to initialize the expressions lazily because they will have a
    // weak_ptr to this node and we can't obtain that in the constructor
    if (!_output_expressions)
    {
        _output_expressions.emplace(_column_definitions.size() -
...
void MockNode::set_pruned_column_ids(
...
std::string MockNode::description(const DescriptionMode /*mode*/) const
{
    ...
    {
        if (std::find(_pruned_column_ids.begin(), _pruned_column_ids.end(),
                      column_id) != _pruned_column_ids.end())
        {
            ++column_id;
            continue;
        }
    ...
    stream << " | pruned: " << _pruned_column_ids.size() << "/"
           << _column_definitions.size() << " columns";
...
UniqueColumnCombinations MockNode::unique_column_combinations() const
{
    ...
    const auto contains =
        [](const auto &column_ids, const auto search_column_id)
    {
        return std::find(column_ids.cbegin(), column_ids.cend(),
                         search_column_id) != column_ids.cend();
    };
    ...
    for (const auto &table_key_constraint : _table_key_constraints)
    {
        ...
        if (std::any_of(key_constraint_column_ids.cbegin(),
                        key_constraint_column_ids.cend(),
        ...
        // Search for output expressions that represent the TableKeyConstraint's
        // ColumnIDs.
        const auto &column_expressions =
            find_column_expressions(*this, key_constraint_column_ids);
...
void MockNode::set_table_statistics(
...
void MockNode::set_non_trivial_functional_dependencies(
    const FunctionalDependencies &fds)
{
...
MockNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    ...
    mock_node->set_non_trivial_functional_dependencies(
        _functional_dependencies);
    mock_node->set_pruned_column_ids(_pruned_column_ids);
    return mock_node;
}

bool MockNode::_on_shallow_equals(const AbstractLQPNode &rhs,
                                  const LQPNodeMapping & /*node_mapping*/) const
{
    ...
    return _column_definitions == mock_node._column_definitions &&
           _pruned_column_ids == mock_node._pruned_column_ids &&
           mock_node.name == name &&
           mock_node.key_constraints() == _table_key_constraints &&
