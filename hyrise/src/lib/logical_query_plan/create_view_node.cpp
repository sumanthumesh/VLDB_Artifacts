...
CreateViewNode::CreateViewNode(const std::string &init_view_name,
                               const std::shared_ptr<LQPView> &init_view,
                               const bool init_if_not_exists)
    : AbstractNonQueryNode(LQPNodeType::CreateView), view_name(init_view_name),
      view(init_view), if_not_exists(init_if_not_exists)
{
}
...
CreateViewNode::_on_shallow_copy(LQPNodeMapping & /*node_mapping*/) const
{
    return CreateViewNode::make(view_name, view->deep_copy(), if_not_exists);
}

bool CreateViewNode::_on_shallow_equals(
    const AbstractLQPNode &rhs, const LQPNodeMapping & /*node_mapping*/) const
{
    ...
    return view_name == create_view_node_rhs.view_name &&
           view->deep_equals(*create_view_node_rhs.view) &&
