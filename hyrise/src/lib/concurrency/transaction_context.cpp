...
TransactionContext::TransactionContext(const TransactionID transaction_id,
    ...
    : _transaction_id{transaction_id}, _snapshot_commit_id{snapshot_commit_id},
      _is_auto_commit{is_auto_commit}, _phase{TransactionPhase::Active},
    ...
    DebugAssert(
        (
            [this]()
            {
                ...
                const auto is_rolled_back_after_conflict =
                    _phase == TransactionPhase::RolledBackAfterConflict;
                return !an_operator_failed || is_rolled_back_after_conflict;
            }()),
        "A registered operator failed but the transaction has not been rolled "
        "back. You may also see this "
        "exception if an operator threw an uncaught exception.");

    DebugAssert((
                    [this]()
                    {
                        const auto has_registered_operators =
                            !_read_write_operators.empty();
                        const auto committed_or_rolled_back =
                            _phase == TransactionPhase::Committed ||
                            _phase == TransactionPhase::RolledBackByUser ||
                            _phase == TransactionPhase::RolledBackAfterConflict;
                        return !has_registered_operators ||
                               committed_or_rolled_back;
                        // Note: When thrown during stack unwinding, this
                        // exception might hide previous exceptions. If you are
                        // seeing this, either use a debugger and break on
                        // exceptions or disable this exception as a trial.
                    }()),
                "Has registered operators but has neither been committed nor "
                "rolled back (see comment in code!).");

    /**
     * Tell the TransactionManager, which keeps track of active
     * snapshot-commit-ids, that this transaction has finished.
     */
    Hyrise::get().transaction_manager._deregister_transaction(
        _snapshot_commit_id);
}
...
{
    Assert(_commit_context, "TransactionContext cid only available after "
                            "commit context has been created.");
...
bool TransactionContext::aborted() const
{
    ...
    return (phase == TransactionPhase::Conflicted) ||
           (phase == TransactionPhase::RolledBackAfterConflict);
}
...
void TransactionContext::rollback(RollbackReason rollback_reason)
{
    ...
    else
    {
        ...
        Assert(_num_active_operators == 0,
               "For a user-initiated rollback, no operators should be active");
    }
...
void TransactionContext::commit_async(
...
void TransactionContext::commit()
{
    Assert(_phase == TransactionPhase::Active,
           "TransactionContext must be active to be committed.");
...
void TransactionContext::_mark_as_rolled_back(RollbackReason rollback_reason)
{
    DebugAssert((
                    [this]()
                    {
                        ...
                        {
                            if (op->state() !=
                                ReadWriteOperatorState::RolledBack)
                            {
                                return false;
                            }
                        }
    ...
    {
        _transition(TransactionPhase::Active,
                    TransactionPhase::RolledBackByUser);
    }
    else
    {
        DebugAssert(rollback_reason == RollbackReason::Conflict,
                    "Invalid RollbackReason");
        _transition(TransactionPhase::Conflicted,
...
void TransactionContext::_prepare_commit()
{
    DebugAssert((
                ...
                "All read/write operators need to be in state Executed "
                "(especially not Failed).");
...
void TransactionContext::_mark_as_pending_and_try_commit(
    const std::function<void(TransactionID)> &callback)
{
    DebugAssert((
                    [this]()
                    {
                        ...
                        {
                            if (op->state() !=
                                ReadWriteOperatorState::Committed)
                            {
                                return false;
                            }
                        }
    ...
    auto context_weak_ptr =
        std::weak_ptr<TransactionContext>{this->shared_from_this()};
    _commit_context->make_pending(
        _transaction_id,
        [context_weak_ptr, callback](auto transaction_id)
        {
            // If the transaction context still exists, set its phase to
            // Committed.
            if (auto context_ptr = context_weak_ptr.lock())
            {
                context_ptr->_transition(TransactionPhase::Committing,
    ...
    Hyrise::get().transaction_manager._try_increment_last_commit_id(
        _commit_context);
}
...
void TransactionContext::_transition(TransactionPhase from_phase,
                                     TransactionPhase to_phase)
{
    DebugAssert(_is_auto_commit == AutoCommit::No ||
                    to_phase != TransactionPhase::RolledBackByUser,
