 ...
 *           |                          Finish Transaction
 * -----------------------+ |                                 | | | | IF (User
 * requests Commit         | ELSE (User requests Rollback)
 *     +------------+                          |     or Auto-Commit-Mode) | |
 * Conflicted |                    +------------+ +------------------+
 *     +------------+                    | Committing |                   |
 * RolledBackByUser | |                           +------------+
 * +------------------+ Rollback operators                        | | Commit
 * operators
 *  +-------------------------+                |
 ...
 *                                       +-----------+
 *
 *  RolledBackAfterConflict and RolledBackByUser have to be two different
 * transaction phases, because a final transaction state of
 * RolledBackAfterConflict is considered as a failure, while RolledBackByUser is
 * considered as a successful transaction. Among other things this has an
 * influence on the result message, the database client receives.
 */
enum class TransactionPhase
{
    Active,     // Transaction has just been created. Operators may be executed.
    Conflicted, // One of the operators ran into a conflict. Transaction needs
                // to be rolled back.
    RolledBackAfterConflict, // Transaction has been rolled back because an
                             // operator failed. (Considered a failure)
    RolledBackByUser,        // Transaction has been rolled back due to
                             // ROLLBACK;-statement. (Considered a success)
    ...
    : public std::enable_shared_from_this<TransactionContext>
{
  ...
  public:
    TransactionContext(TransactionID transaction_id,
                       CommitID snapshot_commit_id, AutoCommit is_auto_commit);
    ~TransactionContext();
    ...
    /**
     * The commit id that this transaction has once it is committed. This is the
     * one that is written to the begin/end commit ids of rows modified by this
     * transaction. Only available after TransactionManager::prepare_commit has
     * been called
     */
    CommitID commit_id() const;

    /**
     * Flag that indicates whether the context belongs to a transaction or
     * non-transaction (i.e., it auto-commits).
     */
    ...
    /**
     * Returns true if transaction has run into a conflict or was manually
     ...
     * @param rollback_reason specifies whether the rollback happens due to an
     * explicit ROLLBACK command by the database user or due to a transaction
     * conflict. We need to know this in order to transition into the correct
     * transaction phase.
     */
     ...
     */
    void
    register_read_write_operator(std::shared_ptr<AbstractReadWriteOperator> op)
    {
     ...
     */
    const std::vector<std::shared_ptr<AbstractReadWriteOperator>> &
    read_write_operators()
    {
    ...
    /**
     * Returns whether the transaction is created (and will also commit)
     * automatically. The alternative would be that it was created through a
     * user command (BEGIN). This information is used by the
  ...
  private:
    ...
    /**
     * Sets the transaction phase to Conflicted and waits for operators to
     * finish. Should be called if an operator fails.
     */
     ...
     */
    void _mark_as_pending_and_try_commit(
    ...
    /**
     * Throws an exception if the transition fails, i.e. if another thread has
  ...
  private:
    ...
    std::vector<std::shared_ptr<AbstractReadWriteOperator>>
        _read_write_operators;
