...
CommitContext::CommitContext(const CommitID commit_id)
    : _commit_id{commit_id}, _pending{false}
{
}
...
void CommitContext::make_pending(
    ...
    const std::function<void(TransactionID)> &callback)
{
    ...
    // This line MUST be AFTER setting the callback. Otherwise we run into a
    // race condition while committing, because this is 'pending' but the
    // callback is not there yet.
    _pending = true;
}
...
{
    DebugAssert(
        (next->commit_id() == commit_id() + 1u),
        "Next commit context's commit id needs to be incremented by 1.");
