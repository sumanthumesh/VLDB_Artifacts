...
LQPVisualizer::LQPVisualizer()
{
    ...
    // We can guarantee the LQP never changes during visualization and thus
    // avoid redundant estimations for subplans
    _cardinality_estimator.guarantee_bottom_up_construction();
}

LQPVisualizer::LQPVisualizer(GraphvizConfig graphviz_config,
                             VizGraphInfo graph_info, VizVertexInfo vertex_info,
                             VizEdgeInfo edge_info)
    : AbstractVisualizer(std::move(graphviz_config), std::move(graph_info),
                         std::move(vertex_info), std::move(edge_info))
{
}

void LQPVisualizer::_build_graph(
    const std::vector<std::shared_ptr<AbstractLQPNode>> &lqp_roots)
{
    std::unordered_set<std::shared_ptr<const AbstractLQPNode>> visualized_nodes;
    ExpressionUnorderedSet visualized_sub_queries;
    std::map<std::string, nlohmann::json::value_type> tree;
    for (const auto &root : lqp_roots)
    {
        tree = _build_subtree(root, visualized_nodes, visualized_sub_queries);
        std::string filename = "lqp_" + std::to_string(getpid()) + ".json";
        std::ofstream file(filename);
        if (file.is_open()) {
            file << nlohmann::json(tree).dump(4); // Serialize with 4 spaces of indentation
            file.close();
        }
    }
}

std::map<std::string, nlohmann::json::value_type> LQPVisualizer::_build_subtree(
    ...
    ExpressionUnorderedSet &visualized_sub_queries)
{
    ...
    std::map<std::string, nlohmann::json::value_type> nodejson;
    ...
    if (node->left_input())
    {
        ...
        nodejson["Leftchildren"] = _build_subtree(left_input, visualized_nodes, visualized_sub_queries);
        nodejson["LeftCard"] = _build_dataflow(left_input, node, InputSide::Left);
    }

    if (node->right_input())
    {
        auto right_input = node->right_input();
        nodejson["Rightchildren"] = _build_subtree(right_input, visualized_nodes, visualized_sub_queries);
        nodejson["RightCard"] = _build_dataflow(right_input, node, InputSide::Right);
    }
    ...
    for (const auto &expression : node->node_expressions)
    {
        visit_expression(
            ...
            [&](const auto &sub_expression)
            {
                ...
                _build_subtree(subquery_expression->lqp, visualized_nodes,
                               visualized_sub_queries);

                auto edge_info = _default_edge;
                auto correlated_str = std::string(
                    subquery_expression->is_correlated() ? "correlated"
                                                         : "uncorrelated");
...
std::string LQPVisualizer::_build_dataflow(
    const std::shared_ptr<AbstractLQPNode> &source_node,
    const std::shared_ptr<AbstractLQPNode> &target_node, const InputSide side)
{
    ...
    if (source_node->left_input())
    {
        try
        {
            float input_count = _cardinality_estimator.estimate_cardinality(
                source_node->left_input());

            // Include right side in cardinality estimation unless it is a
            // semi/anti join
            const auto join_node =
                std::dynamic_pointer_cast<JoinNode>(source_node);
            ...
            {
                input_count *= _cardinality_estimator.estimate_cardinality(
                    source_node->right_input());
            }
    ...
    // Use a copy of the stream's default locale with thousands separators:
    // Dynamically allocated raw pointers should be avoided whenever possible.
    // Unfortunately, std::locale stores pointers to the facets and does
    // internal reference counting. std::locale's destructor destructs the
    // locale and the facets whose reference count becomes zero. This forces us
    // to use a dynamically allocated raw pointer here.
    const auto &separate_thousands_locale =
        std::locale(label_stream.getloc(), new SeparateThousandsFacet);
    label_stream.imbue(separate_thousands_locale);

    if (!std::isnan(row_count))
    {
        label_stream << " " << std::fixed << std::setprecision(1) << row_count
                     << " row(s) | " << row_percentage << "% estd.";
    }
    else
    {
        label_stream << "no est.";
    ...
    if (!dynamic_pointer_cast<AbstractNonQueryNode>(source_node))
    {
        ...
        const auto &unique_column_combinations =
            source_node->unique_column_combinations();
        tooltip_stream << "\n"
        ...
        if (!unique_column_combinations.empty())
        {
            trivial_fds = fds_from_unique_column_combinations(
                source_node, unique_column_combinations);
        }
        tooltip_stream << "\n"
