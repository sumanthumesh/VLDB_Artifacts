...
PQPVisualizer::PQPVisualizer(GraphvizConfig graphviz_config,
...
void PQPVisualizer::_build_graph(
    const std::vector<std::shared_ptr<AbstractOperator>> &plans)
{
    std::unordered_set<std::shared_ptr<const AbstractOperator>> visualized_ops;
    nlohmann::json tree;
    for (const auto &plan : plans)
    {
        tree = _build_subtree(plan, visualized_ops);
        std::string filename = "pqp_" + std::to_string(getpid()) + ".json";
        std::ofstream file(filename);
        if (file.is_open()) {
            file << tree.dump(4); // Serialize with 4 spaces of indentation
            file.close();
        }
    }

    {
        // Print the "Total by operator" box using graphviz's record type. Using
        // HTML labels would be slightly nicer, but boost always encloses the
        // label in quotes, which breaks them.
        std::stringstream operator_breakdown_stream;
        operator_breakdown_stream << "{Total by operator|{";

        auto sorted_duration_by_operator_name =
            std::vector<std::pair<std::string, std::chrono::nanoseconds>>{
                _duration_by_operator_name.begin(),
                _duration_by_operator_name.end()};
        std::sort(sorted_duration_by_operator_name.begin(),
                  sorted_duration_by_operator_name.end(),
        ...
        {
            operator_breakdown_stream << " " << format_duration(nanoseconds)
                                      << " \\l";
            total_nanoseconds += nanoseconds;
        }
        operator_breakdown_stream << " " << format_duration(total_nanoseconds)
                                  << " \\l";
        ...
        {
            operator_breakdown_stream
                << round(std::chrono::duration<double, std::nano>{nanoseconds} /
...
nlohmann::json PQPVisualizer::_build_subtree(
    ...
    std::unordered_set<std::shared_ptr<const AbstractOperator>> &visualized_ops)
{
    ...
    nodejson["description"] = op->description();
    _add_operator(op);

    if (op->left_input())
    {
        auto left = op->left_input();
        nodejson["Leftchildren"] = _build_subtree(left, visualized_ops);
        nodejson["LeftCard"] =  _build_dataflow(left, op, InputSide::Left);
    }

    if (op->right_input())
    {
        auto right = op->right_input();
        nodejson["Rightchildren"] = _build_subtree(right, visualized_ops);
        nodejson["RightCard"] = _build_dataflow(right, op, InputSide::Right);
    }
    ...
    case OperatorType::Limit:
    {
        ...
        _visualize_subqueries(op, limit->row_count_expression(),
                              visualized_ops);
    }
    break;
...
void PQPVisualizer::_visualize_subqueries(
    const std::shared_ptr<const AbstractOperator> &op,
    ...
    visit_expression(
        ...
        [&](const auto &sub_expression)
        {
            ...
            auto correlated_str = std::string(
                pqp_subquery_expression->is_correlated() ? "correlated"
                                                         : "uncorrelated");
...
std::string PQPVisualizer::_build_dataflow(
    const std::shared_ptr<const AbstractOperator> &source_node,
    const std::shared_ptr<const AbstractOperator> &target_node,
    const InputSide side)
{
    ...
    if (source_node->executed() && performance_data.has_output)
    {
        ...
        // Dynamically allocated raw pointers should be avoided whenever
        // possible. Unfortunately, std::locale stores pointers to the facets
        // and does internal reference counting. std::locale's destructor
        ...
        const auto &separate_thousands_locale =
            std::locale(stream.getloc(), new SeparateThousandsFacet);
...
void PQPVisualizer::_add_operator(
    const std::shared_ptr<const AbstractOperator> &op)
{
    ...
    if (op->executed())
    {
        ...
        performance_data.output_to_stream(operator_performance_data_stream,
