...
void JoinGraphVisualizer::_build_graph(const std::vector<JoinGraph> &graphs)
{
    for (const auto &graph : graphs)
    {
        ...
        for (auto vertex_idx = size_t{0}; vertex_idx < vertex_count;
             ++vertex_idx)
        {
            ...
            // Don't wrap the label, we're using the record layout, which has
            // internal formatting. Randomly inserted newlines wouldn't help
            // here
        ...
        for (const auto &edge : graph.edges)
        {
            ...
            // Single-vertex edges are local predicates. We already visualized
            // those above
            if (vertex_count <= 1)
            {
                continue;
            ...
            // Binary vertex edges: render a simple edge between the two
            // vertices with the predicates as the edge label
            if (vertex_count == 2)
            {
            ...
            {
                // More than two vertices, i.e. we have a hyperedge (think
                // `SELECT * FROM x, y, z WHERE x.a + y.b + z.c = x.d`.) Render
                // a diamond vertex that contains all the Predicates and connect
                // all hyperedge vertices to that vertex.

                std::stringstream vertex_label_stream;
                const auto edge_predicate_count = edge.predicates.size();
                for (size_t predicate_idx{0};
                     predicate_idx < edge_predicate_count; ++predicate_idx)
                {
                ...
                for (auto current_vertex_idx = edge.vertex_set.find_first();
                     ...
                     current_vertex_idx =
                         edge.vertex_set.find_next(current_vertex_idx))
                {
                    VizEdgeInfo edge_info;
                    edge_info.dir = "none";
                    edge_info.font_color = vertex_info.color;
                    edge_info.color = vertex_info.color;
                    _add_edge(graph.vertices[current_vertex_idx], edge,
                              edge_info);
                }
            }
        }
...
std::string JoinGraphVisualizer::_create_vertex_description(
    const std::shared_ptr<AbstractLQPNode> &vertex)
{
    ...
    visit_lqp(vertex,
              [&](const auto &sub_node)
              {
                  if (const auto stored_table_node =
                          std::dynamic_pointer_cast<StoredTableNode>(sub_node))
                  {
    ...
    for (auto node_idx = size_t{0}; node_idx < stored_table_node_count;
         ++node_idx)
    {
