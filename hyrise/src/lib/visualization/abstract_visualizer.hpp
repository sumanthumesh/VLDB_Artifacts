...
// We do not want to create constants here because they would be rather
// restrictive compared to all possible options defined by graphviz.
...
struct VizGraphInfo
{
    ...
    std::string label = "For tooltips with advanced information, hover your "
                        "mouse over vertices or edge labels.\n\n\n";
...
template <typename GraphBase> class AbstractVisualizer
{
    //                                  Edge list    Vertex list   Directed
    //                                  graph
    using Graph =
    ...
    // No label in a node should be wider than this many characters. If it is
    // longer, line breaks should be added.
  ...
  public:
    ...
    AbstractVisualizer()
        : AbstractVisualizer(GraphvizConfig{}, VizGraphInfo{}, VizVertexInfo{},
                             VizEdgeInfo{})
    {
    }

    AbstractVisualizer(GraphvizConfig graphviz_config, VizGraphInfo graph_info,
    ...
    void visualize(const GraphBase &graph_base, const std::string &img_filename)
    {
        ...
        // mkstemp returns a file descriptor. Unfortunately, we cannot directly
        // create an ofstream from a file descriptor.
        close(file_descriptor);
        std::ofstream file(tmpname);

        // This unique_ptr serves as a scope guard that guarantees the deletion
        // of the temp file once we return from this method.
        const auto delete_temp_file = [&tmpname](auto ptr)
        {
            delete ptr;
            std::remove(tmpname);
        };
        const auto delete_guard =
            std::unique_ptr<char, decltype(delete_temp_file)>(new char,
                                                              delete_temp_file);

        // The caller set the pen widths to either the number of rows (for
        // edges) or the execution time in ns (for vertices). As some plans have
        // only operators that take microseconds and others take minutes,
        // normalize this so that the thickest pen has a width of
        // max_normalized_width and the thinnest one has a width of 1. Using a
        // logarithm makes the operators that follow the most expensive one more
        // visible. Not sure if this is what statisticians would do, but it
...
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
            ...
            {
                max_unnormalized_width =
                    std::max(max_unnormalized_width,
                             std::log(_graph[*iter].pen_width) / log_base);
            }

            double offset =
                max_unnormalized_width - (max_normalized_width - 1.0);
            ...
            for (auto iter = iter_pair.first; iter != iter_pair.second; ++iter)
            {
                ...
                else
                {
                    ...
                    pen_width =
                        1.0 +
                        std::max(0.0, std::log(pen_width) / log_base - offset);
                }
            }
#pragma GCC diagnostic pop
        ...
        auto cmd = renderer + " -T" + format + " \"" + tmpname + "\" > \"" +
                   img_filename + "\"";
        auto ret = system(cmd.c_str());

        Assert(ret == 0,
               "Calling graphviz' " + renderer +
                   " failed. Have you installed graphviz "
                   "(apt-get install graphviz / brew install graphviz)?");
        // We do not want to make graphviz a requirement for Hyrise as
        // visualization is just a gimmick
    }

  protected:
    virtual void _build_graph(const GraphBase &graph_base) = 0;

    template <typename T> static uintptr_t _get_id(const T &v)
    {
        return reinterpret_cast<uintptr_t>(&v);
    }

    template <typename T> static uintptr_t _get_id(const std::shared_ptr<T> &v)
    {
    ...
    void _add_vertex(const T &vertex, const std::string &label = "",
                     const WrapLabel wrap_label = WrapLabel::On)
    {
                     ...
                     const WrapLabel wrap_label = WrapLabel::On)
    {
        ...
        auto inserted =
            _id_to_position.insert({vertex_id, _id_to_position.size()}).second;
        if (!inserted)
        {
    ...
    template <typename T, typename K> void _add_edge(const T &from, const K &to)
    {
    ...
    {
        // Use this to add a global property to the graph. This results in a
        // config line in the graph file: property_name=value;
        _properties.property(property_name,
                             boost::make_constant_property<Graph *>(value));
    }

    template <typename T>
    void _add_property(const std::string &property_name, const T &value)
    {
        // Use this to add a property that is read from each vertex/edge
        // (depending on the value). This will result in: <node_id> [...,
        // property_name=value, ...];
    ...
    std::string _wrap_label(const std::string &label)
    {
        ...
        for (auto line_idx = size_t{0}; line_idx < line_count; ++line_idx)
        {
            ...
            // 2. Split line into words, so we don't break a line in the middle
            // of a word
    ...
    {
        // Favor a hand picked list of nice-to-look-at colors over random
        // generation for now.
        static std::vector<std::string> colors(
            {"#008A2A", "#005FAF", "#5F7E7E", "#9C2F2F", "#A0666C", "#9F9F00",
             "#9FC0CB", "#9F4C00", "#AF00AF"});
