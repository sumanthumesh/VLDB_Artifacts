 ...
 * To iterate over the cache in a thread-safe manner, use the copy provided by
 * snapshot(). Different cache implementations existed in the past, but were
 * retired with PR 2129.
 */
...
class GDFSCache : public AbstractCache<Key, Value>
{
  public:
    ...
    struct GDFSCacheEntry
    {
        ...
        // To have the item with lowest priority at the top, we invert the
        // comparison.
    ...
    using Handle =
        typename boost::heap::fibonacci_heap<GDFSCacheEntry>::handle_type;
    ...
    explicit GDFSCache(size_t capacity = DEFAULT_CACHE_CAPACITY)
        : AbstractCache<Key, Value>(capacity), _inflation(0.0)
    {
    }

    void set(const Key &key, const Value &value, double cost = 1.0,
             double size = 1.0) final
    {
        ...
        if (it != _map.end())
        {
            ...
            entry.frequency++;
            entry.priority =
                _inflation + static_cast<double>(entry.frequency) / entry.size;
  ...
  protected:
    ...
    /**
     * Locking this data structure is easier than using a concurrent data
     * structure (e.g. TBB) as (1) both the queue and the map would have to be
     * concurrent data structures, (2) their modifications would have to be
     * synchronized, and (3) TBB's concurrent_unordered_map does not provide
     * safe deletion and concurrent_hash_map is difficult to use.
     */
