...
// It guarantees the capacity to be thread-safe. Any other guarantees have to be
// fulfilled by the implementation.
template <typename Key, typename Value> class AbstractCache
{
  public:
    explicit AbstractCache(size_t capacity = DEFAULT_CACHE_CAPACITY)
        : _capacity(capacity)
    {
    }
    ...
    // Depending on the underlying strategy, the parameters for cost and size
    // may be used. If they are not intended to be used, we specify constant
    // default values here.
    virtual void set(const Key &key, const Value &value, double cost = 1.0,
                     double size = 1.0) = 0;

    // Tries to fetch a cache entry. We cannot return a reference since this
    // could not be thread-safe.
  ...
  protected:
    // Remove an element from the cache according to the cache algorithm's
    // strategy
