...
Cost AbstractCostEstimator::estimate_plan_cost(
    const std::shared_ptr<AbstractLQPNode> &lqp) const
{
    // Sum up the costs of all operators in the plan, while making sure to cost
    // each operator exactly once, even in the presence of diamond shapes.
    ...
    while (!bfs_queue.empty())
    {
        ...
        // Not every visited node has a cached cost. Costs are only cached for
        // the roots of the plans for which
        // `AbstractCostEstimator::estimate_plan_cost()` is called. For subplans
        // below the root, costs cannot be cached because their costs are never
        // aggregated due to the traversal being a breadth-first search.
        const auto cached_cost =
            _get_subplan_cost_from_cache(current_node, visited);
        if (cached_cost)
        {
            // Cost for subplan successfully retrieved from cache, no need to
            // look at the inputs of the current_node
            cost += *cached_cost;
            continue;
        }
    ...
    std::unordered_set<std::shared_ptr<AbstractLQPNode>> &visited) const
{
     ...
     * Needs to pay special attention to diamond shapes in the LQP to avoid
    ...
    const auto cost_estimation_cache_iter =
        cost_estimation_by_lqp_cache->find(lqp);
    if (cost_estimation_cache_iter == cost_estimation_by_lqp_cache->end())
    {
        return std::nullopt;
    }

    // Check whether the cache entry can be used: This is only the case if the
    // entire subplan has not yet been visited. If any node in it has already
    // been visited, and we'd use the cache entry anyway, costs would get
    // counted twice
    auto subplan_already_visited = false;
    auto subplan_nodes = std::vector<std::shared_ptr<AbstractLQPNode>>{};

    for (const auto &current_node_input :
         {lqp->left_input(), lqp->right_input()})
    {
        ...
        visit_lqp(current_node_input,
                  [&](const auto &node)
                  {
                      subplan_already_visited |=
                          visited.find(node) != visited.end();
                      subplan_nodes.emplace_back(node);
                      return subplan_already_visited
    ...
    // The subplan is "clean"/unvisited, mark all nodes in it as "visited" and
    // return the cost from the cache entry
