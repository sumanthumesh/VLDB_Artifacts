...
{
    constexpr auto TIME_UNIT_ORDER = boost::hana::to_tuple(
        boost::hana::tuple_t<std::chrono::minutes, std::chrono::seconds,
                             std::chrono::milliseconds,
                             std::chrono::microseconds,
                             std::chrono::nanoseconds>);
    const auto unit_strings =
        std::vector<std::string>{" min", " s", " ms", " µs", " ns"};
    ...
    boost::hana::for_each(
        TIME_UNIT_ORDER,
        [&](const auto duration_t)
        {
            using DurationType = typename decltype(duration_t)::type;
            auto floored_duration =
                std::chrono::floor<DurationType>(total_nanoseconds);
            floor_durations.emplace_back(floored_duration.count());
            round_durations.emplace_back(
                std::chrono::round<DurationType>(remaining_nanoseconds)
                    .count());
    ...
    for (auto unit_iterator = size_t{0}; unit_iterator < unit_string_count;
         ++unit_iterator)
    {
        ...
        if (floored_duration > 0 || is_last_element)
        {
            ...
            if (!is_last_element)
            {
                stream << " " << round_durations.at(unit_iterator + 1)
                       << unit_strings.at(unit_iterator + 1);
            }
            break;
        }
    }
