 ...
 *
 * Wrapper that implements the move constructor and assignment operator for
 * std::atomic<> Makes handling atomics in containers easier
 *
...
template <typename T> class copyable_atomic
{
  ...
  public:
    ...
    template <typename... Args> bool exchange(Args &&...args)
    {
        return _atomic.exchange(std::forward<Args>(args)...);
    }

    template <typename... Args> bool compare_exchange_weak(Args &&...args)
    {
        return _atomic.compare_exchange_weak(std::forward<Args>(args)...);
    }

    template <typename... Args> bool compare_exchange_strong(Args &&...args)
    {
