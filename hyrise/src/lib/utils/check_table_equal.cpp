...
Matrix table_to_matrix(const std::shared_ptr<const Table> &table)
{
    ...
    Matrix header(HEADER_SIZE,
                  std::vector<AllTypeVariant>(table->column_count()));
         ...
         ++column_id)
    {
        ...
        header[1][column_id] = pmr_string{
            data_type_to_string.left.at(table->column_data_type(column_id))};
        header[2][column_id] = pmr_string{
            table->column_is_nullable(column_id) ? "NULL" : "NOT NULL"};
    }
...
std::string matrix_to_string(
    const Matrix &matrix,
    const std::vector<std::pair<uint64_t, uint16_t>> &highlight_cells,
    const std::string &highlight_color, const std::string &highlight_color_bg)
{
    ...
    for (auto row_id = size_t{0}; row_id < matrix.size(); ++row_id)
    {
        ...
        {
            stream << coloring << std::setw(4)
                   << std::to_string(row_id - HEADER_SIZE) << ANSI_COLOR_RESET;
        }
        else
        {
        ...
        for (auto column_id = ColumnID{0}; column_id < matrix[row_id].size();
             ++column_id)
        {
            auto cell =
                boost::lexical_cast<std::string>(matrix[row_id][column_id]);
            coloring = "";
...
template <typename T>
bool almost_equals(T left_val, T right_val,
                   FloatComparisonMode float_comparison_mode)
{
    static_assert(std::is_floating_point_v<T>,
                  "Values must be of floating point type.");
    if (float_comparison_mode == FloatComparisonMode::AbsoluteDifference)
    {
        return std::fabs(left_val - right_val) < EPSILON;
    }

    return std::fabs(left_val - right_val) <
           std::max(EPSILON, std::fabs(right_val * EPSILON));
}
    ...
    FloatComparisonMode float_comparison_mode)
{
    ...
    const auto definitions =
        std::vector<TableColumnDefinition>{TableColumnDefinition(
            "single_column", actual_segment->data_type(), true)};

    auto table_type = [&](const std::shared_ptr<AbstractSegment> &segment)
    {
        if (const auto reference_segment =
    ...
    auto actual_table =
        std::make_shared<Table>(definitions, table_type(actual_segment));
    actual_table->append_chunk(
        pmr_vector<std::shared_ptr<AbstractSegment>>{actual_segment});
    auto expected_table =
        std::make_shared<Table>(definitions, table_type(expected_segment));
    expected_table->append_chunk(
        pmr_vector<std::shared_ptr<AbstractSegment>>{expected_segment});

    // If check_table_equal returns something other than std::nullopt, a
    // difference has been found.
    return !check_table_equal(actual_table, expected_table, order_sensitivity,
                  ...
                  IgnoreNullable ignore_nullable)
{
    ...
    const auto print_table_comparison =
        [&](const std::string &error_type, const std::string &error_msg,
            const std::vector<std::pair<uint64_t, uint16_t>>
                &highlighted_cells = {})
    {
        ...
        stream << "------------------------- Actual Result "
                  "------------------------"
               << std::endl;
        stream << matrix_to_string(actual_matrix, highlighted_cells,
                                   ANSI_COLOR_RED, ANSI_COLOR_BG_RED);
        stream << "------------------------------------------------------------"
                  "----"
               << std::endl
               << std::endl;
        stream << "------------------------ Expected Result "
                  "-----------------------"
               << std::endl;
        stream << matrix_to_string(expected_matrix, highlighted_cells,
                                   ANSI_COLOR_GREEN, ANSI_COLOR_BG_GREEN);
        stream << "------------------------------------------------------------"
                  "----"
               << std::endl;
    ...
    if (actual_table->column_count() != expected_table->column_count())
    {
        ...
        const std::string error_msg =
            ...
            std::to_string(actual_table->column_count()) + "\n" +
            "Expected number of columns: " +
            std::to_string(expected_table->column_count());
    ...
    for (auto column_id = ColumnID{0};
         column_id < expected_table->column_count(); ++column_id)
    {
        auto actual_column_type = actual_table->column_data_type(column_id);
        const auto actual_column_is_nullable =
            actual_table->column_is_nullable(column_id);
        auto expected_column_type = expected_table->column_data_type(column_id);
        const auto expected_column_is_nullable =
            expected_table->column_is_nullable(column_id);
        // This is needed for the SQLiteTestrunner, since SQLite does not
        // differentiate between float/double, and int/long. Also, as sqlite
        // returns a generic type for all-NULL columns, we need to adapt those
        // columns to our type.
        if (type_cmp_mode == TypeCmpMode::Lenient)
        {
            ...
            bool all_null = true;
            for (auto row_id = size_t{HEADER_SIZE};
                 row_id < expected_matrix.size(); ++row_id)
            {
        ...
        if (!boost::iequals(actual_table->column_name(column_id),
                            expected_table->column_name(column_id)))
        {
            const std::string error_type = "Column name mismatch (column " +
                                           std::to_string(column_id) + ")";
            const std::string error_msg =
                "Actual column name: " + actual_table->column_name(column_id) +
        ...
        {
            const std::string error_type = "Column type mismatch (column " +
                                           std::to_string(column_id) + ")";
            const std::string error_msg =
                "Actual column type: " +
                data_type_to_string.left.at(
                    actual_table->column_data_type(column_id)) +
                "\n" + "Expected column type: " +
                data_type_to_string.left.at(
                    expected_table->column_data_type(column_id));

            print_table_comparison(error_type, error_msg, {{1, column_id}});
            return stream.str();
        }

        if (ignore_nullable == IgnoreNullable::No &&
            actual_column_is_nullable != expected_column_is_nullable)
        {
            const std::string error_type = "Column NULLable mismatch (column " +
                                           std::to_string(column_id) + ")";
            const std::string error_msg =
                std::string{"Actual column is "} +
                (actual_column_is_nullable ? "" : "NOT ") + "NULL\n" +
    ...
    if (actual_table->row_count() != expected_table->row_count())
    {
        ...
        const std::string error_msg =
            ...
            std::to_string(actual_table->row_count()) + "\n" +
            "Expected number of rows: " +
            std::to_string(expected_table->row_count());
    ...
    const auto highlight_if =
        [&has_error, &mismatched_cells](bool statement, uint64_t row_id,
                                        uint16_t column_id)
    {
        if (statement)
        {
    ...
    for (auto row_id = size_t{HEADER_SIZE}; row_id < actual_matrix.size();
         ++row_id)
    {
        for (auto column_id = ColumnID{0};
             column_id < actual_matrix[row_id].size(); ++column_id)
        {
            if (variant_is_null(actual_matrix[row_id][column_id]) ||
                variant_is_null(expected_matrix[row_id][column_id]))
            {
                highlight_if(
                    !(variant_is_null(actual_matrix[row_id][column_id]) &&
                      variant_is_null(expected_matrix[row_id][column_id])),
                    row_id, column_id);
            }
            else if (actual_table->column_data_type(column_id) ==
                     DataType::Float)
            {
                auto left_val = static_cast<double>(
                    boost::get<float>(actual_matrix[row_id][column_id]));
                auto right_val = lossless_variant_cast<double>(
                    expected_matrix[row_id][column_id]);
                Assert(right_val,
                       "Expected double or float in expected_matrix");

                highlight_if(
                    !almost_equals(left_val, *right_val, float_comparison_mode),
                    row_id, column_id);
            }
            else if (actual_table->column_data_type(column_id) ==
                     DataType::Double)
            {
                auto left_val =
                    boost::get<double>(actual_matrix[row_id][column_id]);
                auto right_val = lossless_variant_cast<double>(
                    expected_matrix[row_id][column_id]);
                Assert(right_val,
                       "Expected double or float in expected_matrix");
            ...
            else
            {
                if (type_cmp_mode == TypeCmpMode::Lenient &&
                    (actual_table->column_data_type(column_id) ==
                     ...
                     actual_table->column_data_type(column_id) ==
                         DataType::Long))
                {
                    auto left_val = lossless_variant_cast<int64_t>(
                        actual_matrix[row_id][column_id]);
                    auto right_val = lossless_variant_cast<int64_t>(
                        expected_matrix[row_id][column_id]);
                    Assert(left_val && right_val,
                           "Expected int or long in actual_matrix and "
                           "expected_matrix");
                ...
                {
                    highlight_if(actual_matrix[row_id][column_id] !=
                                     expected_matrix[row_id][column_id],
                                 row_id, column_id);
                }
            }
    ...
    if (has_error)
    {
        ...
        {
            error_msg += "(" + std::to_string(cell.first - HEADER_SIZE) + "," +
