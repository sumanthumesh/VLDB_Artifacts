...
template <typename EnumType> struct enum_constant_tag
{
...
template <typename EnumType, EnumType enum_value> struct enum_constant
{
    static_assert(std::is_enum_v<EnumType>,
                  "EnumType must be an enum (class).");
 ...
 */
template <typename T> struct is_enum_constant : std::false_type
{
};
...
/**
 * Implementation of hana::value in order to meet requirements for concept
 * “Constant”
 */
template <typename E> struct value_impl<E, when<hyrise::is_enum_constant_v<E>>>
{
    template <typename C> static constexpr auto apply() { return C::value; }
};

/**
 * Implementation of hana::hash in order to meet requirements for concept
 * “Hashable”
 */
template <typename E> struct hash_impl<E, when<hyrise::is_enum_constant_v<E>>>
{
    template <typename X> static constexpr auto apply(const X & /*x*/)
    {
