...
std::vector<std::vector<AllTypeVariant>>
materialize_values(const std::shared_ptr<const Table> &values)
{
...
{
    return MetaTableManager::is_meta_table_name(table_name)
               ? table_name.substr(MetaTableManager::META_PREFIX.size())
               : table_name;
}
...
{
    const auto meta_tables = std::vector<std::shared_ptr<AbstractMetaTable>>{
...
bool MetaTableManager::is_meta_table_name(const std::string &name)
{
    ...
    return name.size() > prefix_len &&
           std::string_view{name.data(), prefix_len} ==
...
void MetaTableManager::add_table(
    const std::shared_ptr<AbstractMetaTable> &table)
{
    _meta_tables[table->name()] = table;
...
std::shared_ptr<AbstractMetaTable>
MetaTableManager::get_table(const std::string &table_name) const
{
    return _meta_tables.at(trim_table_name(table_name));
}

std::shared_ptr<Table>
MetaTableManager::generate_table(const std::string &table_name) const
{
...
void MetaTableManager::insert_into(const std::string &table_name,
...
void MetaTableManager::delete_from(const std::string &table_name,
                                   const std::shared_ptr<const Table> &values)
{
    const auto &rows = materialize_values(values);
...
void MetaTableManager::update(
    const std::string &table_name,
    const std::shared_ptr<const Table> &selected_values,
    const std::shared_ptr<const Table> &update_values)
{
    ...
    Assert(selected_rows.size() == update_rows.size(),
           "Selected and updated values need to have the same size.");

    const auto row_count = selected_rows.size();
    for (auto row = size_t{0}; row < row_count; ++row)
    {
        _meta_tables.at(table_name)
            ->_update(selected_rows[row], update_rows[row]);
    }
}
