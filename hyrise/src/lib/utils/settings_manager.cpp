...
{
    Assert(!_settings.contains(setting->name),
           "A setting with that name already exists.");
    _settings[setting->name] = std::move(setting);
}

void SettingsManager::_remove(const std::string &name)
{
    Assert(_settings.contains(name),
           "A setting with that name does not exist.");
    _settings.erase(name);
}

std::shared_ptr<AbstractSetting>
SettingsManager::get_setting(const std::string &name) const
{
    Assert(_settings.contains(name),
           "A setting with that name does not exist.");
