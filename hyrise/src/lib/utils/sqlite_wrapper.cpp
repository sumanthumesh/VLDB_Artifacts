...
/*
 * Creates columns in given Hyrise table according to an sqlite intermediate
 * statement (one result row).
 */
std::shared_ptr<Table>
create_hyrise_table_from_result(sqlite3_stmt *sqlite_statement,
                                int column_count)
{
    ...
    while ((return_code = sqlite3_step(sqlite_statement)) == SQLITE_ROW)
    {
        for (auto column_id = int{0}; column_id < column_count; ++column_id)
        {
            if (no_result)
            {
                column_names[column_id] =
                    sqlite3_column_name(sqlite_statement, column_id);
            }
        ...
        // We cannot break here, as we still need to check whether the columns
        // contain NULL values or not
    }

    // If this fails, this is likely because of a transaction conflict within
    // sqlite. This means that the Hyrise and the sqlite states have diverged.
    // When using the SQLite wrapper, the caller must ensure that parallel
    // operations do not conflict.
    Assert(return_code == SQLITE_ROW || return_code == SQLITE_DONE,
           "Unexpected sqlite state");
    ...
    if (!no_result)
    {
        ...
        for (auto column_id = int{0}; column_id < column_count; ++column_id)
        {
            ...
            const auto data_type =
                data_type_to_string.right.at(column_types[column_id]);
            column_definitions.emplace_back(column_names[column_id], data_type,
                                            column_nullable[column_id]);
        }
...
/*
 * Adds a single row to given Hyrise table according to an sqlite intermediate
 * statement (one result row).
 */
void copy_row_from_sqlite_to_hyrise(const std::shared_ptr<Table> &table,
                                    ...
                                    int column_count)
{
    ...
    for (auto column_id = int{0}; column_id < column_count; ++column_id)
    {
        ...
        {
            row.emplace_back(
                sqlite3_column_double(sqlite_statement, column_id));
            break;
        }

        case SQLITE_TEXT:
        {
            row.emplace_back(pmr_string(reinterpret_cast<const char *>(
                sqlite3_column_text(sqlite_statement, column_id))));
            break;
        }
...
// We include the address of this wrapper in the URI for the sqlite db so that
// two wrappers don't interfere See also https://www.sqlite.org/inmemorydb.html,
// starting at "If two or more distinct but shareable in-memory databases"
SQLiteWrapper::SQLiteWrapper()
    : _uri{std::string{"file:sqlitewrapper_"} +
           std::to_string(reinterpret_cast<uintptr_t>(this)) + // NOLINT
           "?mode=memory&cache=shared"},
      main_connection{_uri}
{
    Assert(sqlite3_threadsafe(),
           "Expected sqlite to be compiled with thread-safety");
}

SQLiteWrapper::Connection::Connection(const std::string &uri)
{
    // Explicity set parallel mode. On Linux it seems to be the default, on Mac,
    // it seems to make a difference.
    const auto ret =
        sqlite3_open_v2(uri.c_str(), &db,
                        SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE |
                            SQLITE_OPEN_FULLMUTEX, // NOLINT
                        nullptr);
    if (ret != SQLITE_OK)
    {
        sqlite3_close(db);
...
SQLiteWrapper::Connection::Connection(Connection &&other) noexcept
    : db(other.db)
{
    other.db = nullptr;
}
...
std::shared_ptr<Table>
SQLiteWrapper::Connection::execute_query(const std::string &sql) const
{
    ...
    // We need to split the queries such that we only create columns/add rows
    // from the final SELECT query
    const auto queries_before_select =
        std::vector<std::string>(queries.begin(), queries.end() - 1);
    const auto &select_query = queries.back();

    auto return_code = int{0};
    for (const auto &query : queries_before_select)
    {
        return_code = sqlite3_prepare_v2(db, query.c_str(), -1,
    ...
    return_code = sqlite3_prepare_v2(db, select_query.c_str(), -1,
                                     &sqlite_statement, nullptr);

    if (return_code != SQLITE_OK)
    {
        auto error_message = "Failed to execute query \"" + select_query +
                             "\": " + std::string(sqlite3_errmsg(db));
        sqlite3_finalize(sqlite_statement);
        Fail(error_message);
    }

    auto result_table = create_hyrise_table_from_result(
        sqlite_statement, sqlite3_column_count(sqlite_statement));

    if (!sqlite3_stmt_readonly(sqlite_statement))
    {
        // We need to make sure that we do not call sqlite3_reset below on a
        // modifying statement - otherwise, we would re-execute the modification
        Assert(!result_table,
               "Modifying statement was expected to return empty table");
        sqlite3_finalize(sqlite_statement);
        return nullptr;
    }
    ...
    while ((return_code = sqlite3_step(sqlite_statement)) == SQLITE_ROW)
    {
        copy_row_from_sqlite_to_hyrise(result_table, sqlite_statement,
                                       sqlite3_column_count(sqlite_statement));
    }
...
void SQLiteWrapper::create_sqlite_table(const Table &table,
                                        const std::string &table_name) const
{
    ...
    // SQLite doesn't like an unescaped "ORDER" as a table name, thus we escape
    // all table names.
    ...
    {
        create_table_query << table.column_definitions()[column_id].name << " "
    ...
    const auto sqlite3_prepare_return_code =
        sqlite3_prepare_v2(main_connection.db, insert_into_str.c_str(),
                           static_cast<int>(insert_into_str.size() + 1),
                           &insert_into_statement, nullptr);
    Assert(sqlite3_prepare_return_code == SQLITE_OK,
           "Failed to prepare statement: " +
               std::string(sqlite3_errmsg(main_connection.db)));
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
             ...
             ++chunk_offset)
        {
                 ...
                 ++column_id)
            {
                ...
                {
                    sqlite3_bind_return_code = sqlite3_bind_null(
                        insert_into_statement, sqlite_column_id);
                }
                else
                {
                    ...
                    case DataType::Int:
                    {
                        sqlite3_bind_return_code = sqlite3_bind_int(
                            insert_into_statement, sqlite_column_id,
                            boost::get<int32_t>(value));
                    }
                    break;
                    case DataType::Long:
                        sqlite3_bind_return_code = sqlite3_bind_int64(
                            insert_into_statement, sqlite_column_id,
                            boost::get<int64_t>(value));
                        break;
                    case DataType::Float:
                        sqlite3_bind_return_code = sqlite3_bind_double(
                            insert_into_statement, sqlite_column_id,
            ...
            const auto sqlite3_step_return_code =
                sqlite3_step(insert_into_statement);
            Assert(sqlite3_step_return_code == SQLITE_DONE,
                   "Failed to step INSERT: " +
                       std::string(sqlite3_errmsg(main_connection.db)));

            const auto sqlite3_reset_return_code =
...
void SQLiteWrapper::reset_table_from_copy(
    const std::string &table_name_to_reset,
    const std::string &table_name_to_copy_from) const
{
    ...
    command_sstream << "CREATE TABLE " << table_name_to_reset
                    << " AS SELECT * FROM " << table_name_to_copy_from << ";";
