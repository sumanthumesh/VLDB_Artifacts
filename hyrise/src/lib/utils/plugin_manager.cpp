...
bool PluginManager::_is_duplicate(
    const std::unique_ptr<AbstractPlugin> &plugin) const
{
...
std::unordered_map<std::pair<PluginName, PluginFunctionName>,
                   PluginFunctionPointer, plugin_name_function_name_hash>
...
void PluginManager::load_plugin(const std::filesystem::path &path)
{
    ...
    Assert(!_plugins.contains(plugin_name),
           "Loading plugin failed: A plugin with name " + plugin_name +
               " already exists.");
    ...
    PluginHandle plugin_handle =
        dlopen(path.c_str(), static_cast<uint8_t>(RTLD_NOW) |
                                 static_cast<uint8_t>(RTLD_LOCAL));
    // NOLINTNEXTLINE(concurrency-mt-unsafe) - dlerror is not thread-safe, but
    // it's guarded by dl_mutex.
    Assert(plugin_handle, std::string{"Loading plugin failed: "} + dlerror());

    // abstract_plugin.hpp defines a macro for exporting plugins which makes
    // them instantiable by providing a factory method. See the sources of
    // AbstractPlugin and TestPlugin for further details.
    void *factory = dlsym(plugin_handle, "factory");
    Assert(factory, "Instantiating plugin failed: Use the EXPORT_PLUGIN "
                    "(abstract_plugin.hpp) macro to export a factory method "
                    "for your plugin!");
    ...
    PluginHandleWrapper plugin_handle_wrapper = {plugin_handle,
                                                 std::move(plugin)};
    plugin = nullptr;

    Assert(!_is_duplicate(plugin_handle_wrapper.plugin),
           "Loading plugin failed: There can only be one instance of every "
           "plugin.");

    plugin_handle_wrapper.plugin->start();
    _plugins[plugin_name] = std::move(plugin_handle_wrapper);

    // Add the newly loaded plugin's user executable function and benchmark
    // hooks to our maps of functions.
    auto &plugin_ref = *_plugins[plugin_name].plugin;
    const auto &user_executable_functions =
        plugin_ref.provided_user_executable_functions();
    for (const auto &[function_name, function_pointer] :
         user_executable_functions)
    {
        _user_executable_functions[{plugin_name, function_name}] =
            function_pointer;
    }
...
void PluginManager::exec_user_function(const PluginName &plugin_name,
                                       const PluginFunctionName &function_name)
{
    Assert(_user_executable_functions.contains({plugin_name, function_name}),
           "There is no function '" + function_name + "' defined for plugin '" +
               plugin_name + "'.");

    const auto &user_executable_function =
        _user_executable_functions[{plugin_name, function_name}];
    user_executable_function();

    auto &log_manager = Hyrise::get().log_manager;
    log_manager.add_message("PluginManager",
                            "Called user executable function '" +
                                function_name + "' provided by plugin '" +
                                plugin_name + "'.",
                            LogLevel::Info);
}

void PluginManager::exec_pre_benchmark_hook(
    ...
    AbstractBenchmarkItemRunner &benchmark_item_runner)
{
    ...
    auto &log_manager = Hyrise::get().log_manager;
    log_manager.add_message("PluginManager",
                            "Called pre-benchmark hook provided by plugin '" +
                                plugin_name + "'.",
                            LogLevel::Info);
}
                                             ...
                                             nlohmann::json &report)
{
    Assert(_post_benchmark_hooks.contains({plugin_name}),
           "There is no post-benchmark hook defined for plugin '" +
               plugin_name + "'.");
    ...
    auto &log_manager = Hyrise::get().log_manager;
    log_manager.add_message("PluginManager",
                            "Called post-benchmark hook provided by plugin '" +
                                plugin_name + "'.",
                            LogLevel::Info);
}
...
void PluginManager::unload_plugin(const PluginName &plugin_name)
{
    ...
    Assert(plugin_iter != _plugins.cend(),
           "Unloading plugin failed: A plugin with name '" + plugin_name +
               "' does not exist.");

    _unload_and_erase_plugin(plugin_iter);
}

std::unordered_map<PluginName, PluginHandleWrapper>::iterator
PluginManager::_unload_and_erase_plugin(
    const std::unordered_map<PluginName, PluginHandleWrapper>::iterator
        plugin_iter)
{
    const PluginName plugin_name = plugin_iter->first;
    const auto &plugin_handle_wrapper = plugin_iter->second;

    // Delete user exectuable functions and benchmark hooks of the plugin to be
    // unloaded from the maps of functions.
    std::erase_if(_user_executable_functions,
                  [&](const auto &item)
                  {
