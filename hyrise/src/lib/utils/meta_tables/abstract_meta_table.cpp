...
void AbstractMetaTable::_update(
...
void AbstractMetaTable::_validate_data_types(
    const std::vector<AllTypeVariant> &values) const
{
    Assert(values.size() == column_definitions().size(),
           "Number of values must match column definitions.");
