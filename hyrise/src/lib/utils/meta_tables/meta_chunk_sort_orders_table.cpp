...
MetaChunkSortOrdersTable::MetaChunkSortOrdersTable()
    : AbstractMetaTable(
          TableColumnDefinitions{{"table_name", DataType::String, false},
...
/**
 * Chunks can be sorted independently by multiple columns (e.g., as the result
 * of an equi sort-merge join or accidentally). We do not track secondary sort
 * orders, meaning that for `ORDER BY a, b`, only a is reported.
 */
                ...
                for (const auto &[sorted_by_column_id, sort_mode] : sorted_by)
                {
                    ...
                    output_table->append(
                        {pmr_string{table_name}, static_cast<int32_t>(chunk_id),
                         static_cast<int32_t>(sorted_by_column_id),
                         pmr_string{sort_mode_stream.str()}});
                }
            }
        }
    }
