 ...
 * Information provided by the table is usually not persisted, but gathered on
 * the fly.
 *
 * A meta table may provide methods for retrieving, inserting, deleting or
 * updating. Not all of these operations are allowed for all meta tables.
 * Therefore, they also provide methods that indicate whether the particular
 * operation is allowed, e.g., can_insert().
 *
 * Modifications of a meta table's values usually trigger some actions, like
 * loading plugins.
 *
...
class AbstractMetaTable : public Noncopyable
{
  ...
  protected:
    ...
    void _update(const std::vector<AllTypeVariant> &selected_values,
    ...
    virtual void
    _on_update(const std::vector<AllTypeVariant> & /*selected_values*/,
               const std::vector<AllTypeVariant> & /*update_values*/);
