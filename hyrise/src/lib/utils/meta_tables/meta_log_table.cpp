...
std::shared_ptr<Table> MetaLogTable::_on_generate() const
{
    ...
    for (const auto &entry : Hyrise::get().log_manager.log_entries())
    {
        const auto timestamp_ns =
            std::chrono::nanoseconds{entry.timestamp.time_since_epoch()}
                .count();
        ...
        // components.", see https://en.cppreference.com/w/c/chrono/tm
        auto buffer = tm{};

        timestamp_stream << std::put_time(localtime_r(&timestamp, &buffer),
                                          "%F %T");
        output_table->append(
            {timestamp_ns, pmr_string{timestamp_stream.str()},
             pmr_string{magic_enum::enum_name(entry.log_level)},
