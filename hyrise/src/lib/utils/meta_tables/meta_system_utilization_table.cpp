...
MetaSystemUtilizationTable::MetaSystemUtilizationTable()
    : AbstractMetaTable(TableColumnDefinitions{
          {"cpu_system_time", DataType::Long, false},
...
std::shared_ptr<Table> MetaSystemUtilizationTable::_on_generate() const
{
    auto output_table = std::make_shared<Table>(
        _column_definitions, TableType::Data, std::nullopt, UseMvcc::Yes);
    ...
    const auto allocated_memory_variant =
        allocated_memory
            ? AllTypeVariant{static_cast<int64_t>(*allocated_memory)}
            : AllTypeVariant{NULL_VALUE};
    const auto cpu_affinity_count = Hyrise::get().topology.num_cpus();

    output_table->append(
        {static_cast<int64_t>(system_cpu_ticks),
         static_cast<int64_t>(process_cpu_ticks),
         static_cast<int64_t>(total_ticks), load_avg.load_1_min,
         load_avg.load_5_min, load_avg.load_15_min,
         ...
         allocated_memory_variant, static_cast<int32_t>(cpu_affinity_count)});
...
MetaSystemUtilizationTable::LoadAvg MetaSystemUtilizationTable::_get_load_avg()
{
    ...
    return {static_cast<float>(load_avg[0]), static_cast<float>(load_avg[1]),
            static_cast<float>(load_avg[2])};
}
...
/**
 * Returns the time in ns that ALL processes have spent on the CPU since an
 * arbitrary point in the past. This might be used to differentiate between CPU
 * time consumed by this process and by other processes on the same machine.
 */
uint64_t MetaSystemUtilizationTable::_get_system_cpu_time()
{
    ...
    const auto active_ns =
        (active_ticks * std::nano::den) / sysconf(_SC_CLK_TCK);
    ...
    // The amount of time from HOST_CPU_LOAD_INFO is measured in units of clock
    // ticks. sysconf(_SC_CLK_TCK) can be used to convert it to ns.
...
/**
 * Returns the time in ns that THIS process has spent on the CPU since an
...
uint64_t MetaSystemUtilizationTable::_get_process_cpu_time()
{
    ...
    const auto active_ns =
        (time_spec.tv_sec * std::nano::den + time_spec.tv_nsec);
 ...
 * could be made available (e.g. buffers, caches ...). This is not equivalent to
 * the total memory size, since certain data cannot be paged at any time.
 */
MetaSystemUtilizationTable::SystemMemoryUsage
MetaSystemUtilizationTable::_get_system_memory_usage()
{
#ifdef __linux__
        ...
        while (std::getline(meminfo_file, meminfo_line))
        {
            if (meminfo_line.starts_with("MemFree"))
            {
                memory_usage.free_memory =
                    _parse_value_string(meminfo_line)[0] * 1024;
            }
    ...
    ret = host_statistics64(mach_host_self(), HOST_VM_INFO,
                            reinterpret_cast<host_info64_t>(&vm_statistics),
                            &count);
    ...
    memory_usage.available_memory =
        (vm_statistics.inactive_count + vm_statistics.free_count) * page_size;
...
/**
 * Returns a struct that contains the virtual and physical memory used by this
 * process in bytes.
 * - Virtual Memory is the total memory usage of the process.
 * - Physical Memory is the resident set size (RSS), the portion of memory that
 * is held in RAM.
 */
MetaSystemUtilizationTable::ProcessMemoryUsage
MetaSystemUtilizationTable::_get_process_memory_usage()
{
#ifdef __linux__
        ...
        while (std::getline(self_status_file, self_status_line))
        {
            if (self_status_line.starts_with("VmSize"))
            {
                memory_usage.virtual_memory =
                    _parse_value_string(self_status_line)[0] * 1024;
            }
            else if (self_status_line.starts_with("VmRSS"))
            {
                memory_usage.physical_memory =
                    _parse_value_string(self_status_line)[0] * 1024;
            }
        }
    ...
    const auto ret = task_info(mach_task_self(), TASK_BASIC_INFO,
...
/**
 * This returns the actually allocated memory. It differs from
 * _get_process_memory_usage in that it only returns memory that has been
 * allocated. The reported virtual memory consumption is usually higher than the
 * amount of allocated memory as free'd memory is not immediately returned to
 * the system (either due to internal page fragmentation or because jemalloc
 * keeps empty pages for future use). In rare cases, it can also be higher than
 * the amount of virtual memory if memory has been allocated but not committed
 * yet.
 *
 * jemalloc's memory allocation uses different size classes, each with a
 * different number of bytes to allocate. If a data structure's allocator
 * requests a specific amount of bytes that exceeds a certain size class by only
 * one byte, the next larger size class is used and the full amount of bytes of
 * this class is allocated. The spacing between size classes doubles every 4th
 * class. Consequently, the larger the size class that the requested amount of
 * memory slightly exceeds, the larger the difference between the actually
 * allocated and the requested memory.
 *
 * Example:
...
std::optional<size_t> MetaSystemUtilizationTable::_get_allocated_memory()
{
    ...
    if constexpr (HYRISE_DEBUG)
    {
        ...
        const auto error_code = mallctl("config.stats", &stats_enabled,
                                        &stats_enabled_size, nullptr, 0);
        Assert(!error_code,
               "Cannot check if jemalloc was built with --stats_enabled.");
        Assert(stats_enabled,
               "Hyrise's jemalloc was not build with --stats_enabled.");
    }

    // Before retrieving the statistics, we need to update jemalloc's epoch to
    // get current values. See the mallctl documentation for details.
    {
    ...
    const auto error_code =
        mallctl("stats.allocated", &allocated, &allocated_size, nullptr, 0);
    Assert(!error_code, std::string{"mallctl failed with error code "} +
                            std::to_string(error_code));

    return allocated;
#else
    // Hyrise is compiled with jemalloc unless tsan is used (see
    // src/lib/CMakeLists.txt). To maintain compatibility with other allocators,
    // we return nullopt here.
    return std::nullopt;
#endif
}
...
#ifdef __linux__
std::vector<int64_t>
MetaSystemUtilizationTable::_parse_value_string(std::string &input_string)
{
