...
using NodeGetChildrenFn = std::function<std::vector<std::shared_ptr<Node>>(
    const std::shared_ptr<Node> &)>;

template <typename Node>
using NodePrintFn =
    std::function<void(const std::shared_ptr<Node> &, std::ostream &stream)>;
 ...
 *     \_Recurring Node --> [5]
 *
 * @param node              The root node originating from which the graph
 * should be printed.
 * @param get_children_fn   Callback that returns a nodes children.
 * @param print_node_fn     Callback that prints the information about a node,
 * e.g. "[Predicate] a = 42" in the example above.
 ...
 * @param stream            The stream to print on.
 * @param separator         String that is printed between different table key
 * constraints.
 */
