...
/**
 *
 * @param indentation   Helper structure for the indentation of a node. Its size
 * determines the level of indentation. For each position, `true` means a
 * vertical line "|", and `false` means a space " " should be used to increase
 * the indentation.
 * @param id_by_node    Mapping of nodes to their IDs. Used to determine whether
 * a node was already printed and which ID it had.
 * @param id_counter    ID of the last node. Used to generate the ID of the
 * current node.
 */
template <typename Node>
void print_directed_acyclic_graph_impl(
    ...
    size_t &id_counter)
{
    ...
    const auto max_indentation =
        indentation.empty() ? 0 : indentation.size() - 1;
    ...
    for (size_t child_idx = 0; child_idx < children.size(); ++child_idx)
    {
        ...
        print_directed_acyclic_graph_impl<Node>(
            children[child_idx], get_children_fn, node_print_fn, stream,
            indentation, id_by_node, id_counter);
    }
...
template <typename Node>
void print_directed_acyclic_graph(
    const std::shared_ptr<Node> &node,
    const NodeGetChildrenFn<Node> &get_children_fn,
    const NodePrintFn<Node> &print_node_fn, std::ostream &stream)
{
    ...
    print_directed_acyclic_graph_impl<Node>(node, get_children_fn,
                                            print_node_fn, stream, levels,
                                            id_by_node, id_counter);
}

void print_table_key_constraints(const std::shared_ptr<const Table> &table,
                                 std::ostream &stream,
                                 const std::string &separator)
{
    const auto &table_key_constraints =
        std::set<TableKeyConstraint>{table->soft_key_constraints().cbegin(),
                                     table->soft_key_constraints().cend()};
...
template void print_directed_acyclic_graph<const AbstractLQPNode>(
    ...
    const NodeGetChildrenFn<const AbstractLQPNode> &get_children_fn,
    const NodePrintFn<const AbstractLQPNode> &print_node_fn,
    std::ostream &stream);
template void print_directed_acyclic_graph<const AbstractOperator>(
    ...
    const NodePrintFn<const AbstractOperator> &print_node_fn,
    std::ostream &stream);

std::string all_encoding_options()
{
    return boost::algorithm::join(
        magic_enum::enum_names<EncodingType>() |
            boost::adaptors::transformed([](const auto it)
