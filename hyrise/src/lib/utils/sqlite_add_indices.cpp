                           ...
                           std::shared_ptr<SQLiteWrapper> &sqlite_wrapper)
{
    ...
    // SQLite does not support adding primary keys to non-empty tables, so we
    // rename the table, create an empty one from the provided schema and copy
    // the data.
    for (const auto &table_name : Hyrise::get().storage_manager.table_names())
    {
        // SQLite doesn't like an unescaped "ORDER" as a table name, thus we
        // escape it. No need to escape the
        // "..._unindexed" name.
        const auto escaped_table_name = std::string{"\""} + table_name + "\"";

        sqlite_wrapper->main_connection.raw_execute_query(
            std::string{"ALTER TABLE "}
    ...
    Assert(schema_file.good(), std::string{"Schema file "} + schema_file_path +
                                   " not found - try running the binary from "
                                   "the Hyrise root or the build directory");
    const auto schema_sql =
        std::string{std::istreambuf_iterator<char>(schema_file),
    ...
    if (!create_indices_file_path.empty())
    {
        ...
        Assert(create_indices_file.good(),
               std::string{"Index file "} + create_indices_file_path +
                   " not found - try running the binary from the Hyrise root "
                   "or the build directory");
        const auto create_indices_sql =
            std::string{std::istreambuf_iterator<char>(create_indices_file),
                        std::istreambuf_iterator<char>()};
    ...
    for (const auto &table_name : Hyrise::get().storage_manager.table_names())
    {
        ...
        std::cout << "-  Adding indexes to SQLite table " << table_name
                  << std::flush;
