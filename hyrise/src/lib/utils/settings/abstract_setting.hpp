 ...
 * If a component ends its lifecycle (e.g., a plugin is unloaded) and does not
 * deregister its settings, this might lead to undefined behavior.
 *
 * Settings provide a getter/setter that returns/requires a std::string.
 * When the set(...) method is called, a setting can invoke a method of its
 * owning component that triggers the immediate appliance of the changed value.
 *
 * Settings have a unique name that consists of the name of its parent and the
 * setting name (e.g. "TaskScheduler.workers")
 */
class AbstractSetting : public Noncopyable,
                        public std::enable_shared_from_this<AbstractSetting>
{
  public:
