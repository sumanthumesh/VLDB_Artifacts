...
// We discourage manual memory management in Hyrise (such as malloc, or new),
// but in case of allocator/memory resource implementations, it is fine.
...
class default_resource_impl : public memory_resource
{
  public:
    ...
    [[nodiscard]] bool
    do_is_equal(const memory_resource &other) const BOOST_NOEXCEPT override
    {
        return &other == this;
    }
};
...
memory_resource *get_default_resource() BOOST_NOEXCEPT
{
    ...
    // resource going out of scope before the other things were cleaned up that
    // we decided to live with the leak, rather than running into races over and
    // over again.
...
memory_resource *
set_default_resource(memory_resource * /*resource*/) BOOST_NOEXCEPT
{
    // Do nothing
    return get_default_resource();
}
