...
void Difference::_on_set_parameters(
...
{
    DebugAssert(left_input_table()->column_definitions() ==
    ...
    auto right_input_row_set =
        std::unordered_set<std::string>(right_input_table()->row_count());
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count_right; ++chunk_id)
    {
        ...
        // creating a temporary row representation with strings to be filled
        // segment-wise
        auto string_row_vector = std::vector<std::stringstream>(chunk->size());
        for (auto column_id = ColumnID{0}; column_id < column_count_right;
             ++column_id)
        {
            ...
            for (auto chunk_offset = ChunkOffset{0};
                 chunk_offset < segment_size; ++chunk_offset)
            {
                // Previously we called a virtual method of the AbstractSegment
                // interface here. It was replaced with a call to the subscript
                // operator as that is equally slow.
                const auto value = (*abstract_segment)[chunk_offset];
                _append_string_representation(string_row_vector[chunk_offset],
                                              value);
            }
        }
        ...
        std::transform(
            string_row_vector.cbegin(), string_row_vector.cend(),
            std::inserter(right_input_row_set, right_input_row_set.end()),
            [](auto &item) { return item.str(); });
    }

    // 2. Now we check for each chunk of the left input which rows can be added
    // to the output
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count_left; ++chunk_id)
    {
        ...
        Assert(in_chunk, "Physically deleted chunk should not reach this "
                         "point, see get_chunk / #1686.");

        Segments output_segments;

        // creating a map to share pos_lists (see table_scan.hpp)
        std::unordered_map<std::shared_ptr<const AbstractPosList>,
                           std::shared_ptr<RowIDPosList>>
            out_pos_list_map;

        const auto column_count_left = left_input_table()->column_count();
        for (auto column_id = ColumnID{0}; column_id < column_count_left;
             ++column_id)
        {
            ...
            {
                // if the input segment was a reference segment then the output
                // segment must reference the same values/objects
            ...
            std::shared_ptr<RowIDPosList> &pos_list_out =
                out_pos_list_map[in_pos_list];
            ...
            auto out_reference_segment = std::make_shared<ReferenceSegment>(
                out_referenced_table, out_column_id, pos_list_out);
        ...
        for (auto chunk_offset = ChunkOffset{0}; chunk_offset < in_chunk_size;
             ++chunk_offset)
        {
                 ...
                 ++column_id)
            {
                ...
                // Previously a virtual method of the AbstractSegment interface
                // was called here. It was replaced with a call to the subscript
                // operator as that is equally slow.
                const auto value = (*abstract_segment)[chunk_offset];
            ...
            // we check if the recently created row_string is contained in the
            // left_input_row_set
            ...
            if (search == right_input_row_set.end())
            {
                for (const auto &pos_list_pair : out_pos_list_map)
                {
                    ...
                    {
                        pos_list_pair.second->emplace_back(
                            (*pos_list_pair.first)[chunk_offset]);
                    }
                    else
                    {
                        pos_list_pair.second->emplace_back(chunk_id,
                                                           chunk_offset);
                    }
                }
            }
        ...
        {
            // The difference operator does not change the sorted_by property of
            // chunks. If the chunk was sorted before, it will be sorted after
            // the difference operation as well.
    ...
    return std::make_shared<Table>(left_input_table()->column_definitions(),
                                   TableType::References,
                                   std::move(output_chunks));
}

void Difference::_append_string_representation(std::ostream &row_string_buffer,
                                               const AllTypeVariant &value)
{
    ...
    row_string_buffer.write(reinterpret_cast<const char *>(&length),
                            sizeof(length));
}
