...
void AbstractReadWriteOperator::execute()
{
    Assert(static_cast<bool>(transaction_context()),
           "AbstractReadWriteOperator::execute() should never be called "
           "without having set the transaction context.");
    Assert(transaction_context()->phase() == TransactionPhase::Active,
           "Transaction is not active anymore.");
    Assert(_rw_state == ReadWriteOperatorState::Pending,
           "Operator needs to have state Pending in order to be executed.");

    transaction_context()->register_read_write_operator(
        std::static_pointer_cast<AbstractReadWriteOperator>(
            shared_from_this()));
    ...
    {
        // No matter what goes wrong, we need to mark the operators as failed.
        // Otherwise, when the transaction context gets destroyed, it will cause
        // another exception that hides the one that caused the actual error. We
        // are NOT trying to handle the exception here - just making sure that
        // we are not misled when we debug things.
        _mark_as_failed();
        throw;
    }
...
{
    Assert(_rw_state == ReadWriteOperatorState::Conflicted ||
               _rw_state == ReadWriteOperatorState::Executed,
           "Operator needs to have state Failed or Executed in order to be "
           "rolled back.");
...
{
    return _rw_state == ReadWriteOperatorState::Conflicted ||
           _rw_state == ReadWriteOperatorState::RolledBack;
}
...
{
    Assert(_rw_state == ReadWriteOperatorState::Pending,
           "Operator can only be marked as failed if pending.");
