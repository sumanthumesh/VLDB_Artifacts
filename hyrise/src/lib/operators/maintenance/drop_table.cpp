...
DropTable::DropTable(const std::string &init_table_name,
                     const bool init_if_exists)
    : AbstractReadOnlyOperator(OperatorType::DropTable),
      table_name(init_table_name), if_exists(init_if_exists)
{
}
...
{
    // If IF EXISTS is not set and the table is not found, StorageManager throws
    // an exception
...
void DropTable::_on_set_parameters(
