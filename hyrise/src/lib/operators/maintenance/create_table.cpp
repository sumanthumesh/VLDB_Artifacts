...
CreateTable::CreateTable(
    ...
    : AbstractReadWriteOperator(OperatorType::CreateTable, input_operator),
      table_name(init_table_name), if_not_exists(init_if_not_exists)
{
}
...
std::string CreateTable::description(DescriptionMode description_mode) const
{
    ...
    const auto *const separator =
        description_mode == DescriptionMode::SingleLine ? ", " : "\n";

    // If the input operator has already been cleared, we cannot retrieve its
    // columns anymore. However, since the table has been created, we can simply
    // pull the definitions from the new table.
...
CreateTable::_on_execute(std::shared_ptr<TransactionContext> context)
{
    const auto column_definitions =
        _left_input->get_output()->column_definitions();

    // If IF NOT EXISTS is not set and the table already exists, StorageManager
    // throws an exception
    if (!if_not_exists || !Hyrise::get().storage_manager.has_table(table_name))
    {
        ...
        for (const auto &table_key_constraint :
             _left_input->get_output()->soft_key_constraints())
        {
            table->add_soft_key_constraint(table_key_constraint);
        }

        // Insert table data (if no data is present, insertion makes no
        // difference)
...
std::shared_ptr<AbstractOperator> CreateTable::_on_deep_copy(
    ...
    const
{
    return std::make_shared<CreateTable>(table_name, if_not_exists,
                                         copied_left_input);
}

void CreateTable::_on_set_parameters(
