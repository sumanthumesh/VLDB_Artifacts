...
CreatePreparedPlan::CreatePreparedPlan(
    ...
    : AbstractReadOnlyOperator(OperatorType::CreatePreparedPlan),
      _prepared_plan_name(prepared_plan_name), _prepared_plan(prepared_plan)
{
}
...
std::string
CreatePreparedPlan::description(DescriptionMode description_mode) const
{
...
{
    Hyrise::get().storage_manager.add_prepared_plan(_prepared_plan_name,
                                                    _prepared_plan);
    return nullptr;
}

void CreatePreparedPlan::_on_set_parameters(
...
std::shared_ptr<AbstractOperator> CreatePreparedPlan::_on_deep_copy(
    ...
    const
{
    return std::make_shared<CreatePreparedPlan>(_prepared_plan_name,
                                                _prepared_plan->deep_copy());
}
