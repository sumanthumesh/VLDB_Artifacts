...
DropView::DropView(const std::string &init_view_name, const bool init_if_exists)
    : AbstractReadOnlyOperator(OperatorType::DropView),
      view_name(init_view_name), if_exists(init_if_exists)
{
}
...
void DropView::_on_set_parameters(
...
{
    // If IF EXISTS is not set and the view is not found, StorageManager throws
    // an exception
