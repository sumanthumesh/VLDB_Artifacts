...
CreateView::CreateView(const std::string &view_name,
                       const std::shared_ptr<LQPView> &view,
                       const bool if_not_exists)
    : AbstractReadOnlyOperator(OperatorType::CreateView), _view_name(view_name),
      _view(view), _if_not_exists(if_not_exists)
{
}
...
void CreateView::_on_set_parameters(
...
{
    // If IF NOT EXISTS is not set and the view already exists, StorageManager
    // throws an exception
