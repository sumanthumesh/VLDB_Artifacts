...
Export::Export(const std::shared_ptr<const AbstractOperator> &input_operator,
               const std::string &filename, const FileType &file_type)
    : AbstractReadOnlyOperator(OperatorType::Export, input_operator),
      _filename(filename), _file_type(file_type)
{
...
std::string Export::description(DescriptionMode description_mode) const
{
    ...
    return AbstractOperator::description(description_mode) + separator +
           "to '" + _filename + "'" + separator + "(" + file_type + ")";
}

std::shared_ptr<const Table> Export::_on_execute()
{
    if (_filename.empty() ||
        std::all_of(_filename.begin(), _filename.end(), isspace))
    {
...
void Export::_on_set_parameters(
