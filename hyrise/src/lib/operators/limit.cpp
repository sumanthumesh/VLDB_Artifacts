...
Limit::Limit(const std::shared_ptr<const AbstractOperator> &input_operator,
    ...
    : AbstractReadOnlyOperator(OperatorType::Limit, input_operator),
                       ...
                       std::shared_ptr<AbstractOperator>> &copied_ops) const
{
    return std::make_shared<Limit>(
        copied_left_input, _row_count_expression->deep_copy(copied_ops));
}
...
std::shared_ptr<const Table> Limit::_on_execute()
{
    ...
    /**
     * Evaluate the _row_count_expression to determine the actual number of rows
     * to "Limit" the output to
     */
    auto num_rows = size_t{};

    resolve_data_type(
        _row_count_expression->data_type(),
        [&](const auto data_type_t)
        {
            ...
            if constexpr (std::is_integral_v<LimitDataType>)
            {
                const auto num_rows_expression_result =
                    ExpressionEvaluator{}
                        .evaluate_expression_to_result<LimitDataType>(
                            *_row_count_expression);
                Assert(num_rows_expression_result->size() == 1,
                       "Expected exactly one row for Limit");
                Assert(!num_rows_expression_result->is_null(0),
                       "Expected non-null for Limit");

                const auto signed_num_rows =
                    num_rows_expression_result->value(0);
                Assert(signed_num_rows >= 0,
                       "Can't Limit to a negative number of Rows");
    ...
    for (auto index = size_t{0}; index < num_rows && chunk_id < chunk_count;
         ++chunk_id)
    {
        ...
        auto output_chunk_row_count = std::min<ChunkOffset>(
            input_chunk->size(), static_cast<ChunkOffset>(num_rows - index));

        const auto column_count = input_table->column_count();
        for (auto column_id = ColumnID{0}; column_id < column_count;
             ++column_id)
        {
            const auto input_abstract_segment =
                input_chunk->get_segment(column_id);
            auto output_pos_list =
                std::make_shared<RowIDPosList>(output_chunk_row_count);
            std::shared_ptr<const Table> referenced_table;
            ColumnID output_column_id = column_id;

            if (auto input_ref_segment =
                    std::dynamic_pointer_cast<const ReferenceSegment>(
                        input_abstract_segment))
            {
                ...
                std::copy(begin, begin + output_chunk_row_count,
                          output_pos_list->begin());
            }
            else
            {
                ...
                for (auto chunk_offset = ChunkOffset{0};
                     chunk_offset <
                     static_cast<ChunkOffset>(output_chunk_row_count);
                     ++chunk_offset)
                {
                    (*output_pos_list)[chunk_offset] =
                        RowID{chunk_id, chunk_offset};
                }
            }

            output_segments.push_back(std::make_shared<ReferenceSegment>(
                referenced_table, output_column_id, output_pos_list));
        }

        index += output_chunk_row_count;
        auto output_chunk = std::make_shared<Chunk>(std::move(output_segments));
        output_chunk->finalize();
        // The limit operator does not affect sorted_by property. If a chunk was
        // sorted before, it still is after the limit operator.
