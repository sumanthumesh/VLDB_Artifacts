...
Update::Update(const std::string &table_to_update_name,
               const std::shared_ptr<AbstractOperator> &fields_to_update_op,
               const std::shared_ptr<AbstractOperator> &update_values_op)
    : AbstractReadWriteOperator(OperatorType::Update, fields_to_update_op,
                                update_values_op),
...
Update::_on_execute(std::shared_ptr<TransactionContext> context)
{
    const auto table_to_update =
        Hyrise::get().storage_manager.get_table(_table_to_update_name);

    // 0. Validate input
    DebugAssert(context, "Update needs a transaction context");
    DebugAssert(left_input_table()->row_count() ==
                    right_input_table()->row_count(),
                "Update required identical layouts from its input tables");
    DebugAssert(left_input_table()->column_data_types() ==
                    right_input_table()->column_data_types(),
                "Update required identical layouts from its input tables");
...
std::shared_ptr<AbstractOperator> Update::_on_deep_copy(
    ...
    const
{
    return std::make_shared<Update>(_table_to_update_name, copied_left_input,
                                    copied_right_input);
}

void Update::_on_set_parameters(
