...
Import::Import(const std::string &init_filename, const std::string &tablename,
               const ChunkOffset chunk_size, const FileType file_type,
               const std::optional<CsvMeta> &csv_meta)
    : AbstractReadOnlyOperator(OperatorType::Import), filename(init_filename),
      _tablename(tablename), _chunk_size(chunk_size), _file_type(file_type),
      _csv_meta(csv_meta)
{
...
std::shared_ptr<AbstractOperator> Import::_on_deep_copy(
    ...
    const
{
    return std::make_shared<Import>(filename, _tablename, _chunk_size,
                                    _file_type, _csv_meta);
}

void Import::_on_set_parameters(
