...
using IndexRange =
    std::pair<AbstractChunkIndex::Iterator, AbstractChunkIndex::Iterator>;

/**
 * This operator joins two tables using one column of each table.
 * A speedup compared to the Nested Loop Join is achieved by avoiding the inner
 * loop, and instead finding the index side values utilizing the index.
 *
 * For index reference joins, only the join JoinMode::Inner is supported.
 * Additionally, if the join segments of the reference table don't provide the
 * guarantee of referencing one single chunk (of the original data table), then
 * the fallback solution (nested join loop) is used. Using the fallback solution
 * does not increment the number of chunks scanned with index in the performance
 * data.
 *
 * Note: An index needs to be present on the index side table in order to
...
class JoinIndex : public AbstractJoinOperator
{
  public:
    ...
    struct PerformanceData : public OperatorPerformanceData<OperatorSteps>
    {
        ...
        // Per default, the right input is the index side and the left side is
        // the probe side.
  ...
  protected:
    ...
    void _data_join_two_segments_using_index(
    ...
    void _reference_join_two_segments_using_index(
        ...
        const ChunkID probe_chunk_id, const ChunkID index_chunk_id,
        const std::shared_ptr<AbstractChunkIndex> &index,
        const std::shared_ptr<const AbstractPosList>
            &reference_segment_pos_list);

    template <typename SegmentPosition>
    std::vector<IndexRange> _index_ranges_for_value(
        const SegmentPosition probe_side_position,
        const std::shared_ptr<AbstractChunkIndex> &index) const;

    void _append_matches(const AbstractChunkIndex::Iterator &range_begin,
    ...
    void _append_matches_dereferenced(const ChunkID &probe_chunk_id,
                                      const ChunkOffset &probe_chunk_offset,
                                      const RowIDPosList &index_table_matches);

    void _append_matches_non_inner(const bool is_semi_or_anti_join);

    void _write_output_segments(Segments &output_segments,
