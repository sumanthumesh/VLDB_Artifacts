...
/**
 * Uses the ExpressionEvaluator::evaluate_expression_to_pos_list() for a
 * fallback implementation of the AbstractTableScanImpl. This is likely slower
 * than any specialized `AbstractTableScanImpl` and should thus only be used if
 * a particular expression type doesn't have a specialized
 * `AbstractTableScanImpl`.
 */
