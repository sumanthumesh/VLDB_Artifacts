...
ColumnIsNullTableScanImpl::ColumnIsNullTableScanImpl(
    const std::shared_ptr<const Table> &in_table, const ColumnID column_id,
    const PredicateCondition &predicate_condition)
    : _in_table(in_table), _column_id(column_id),
      _predicate_condition(predicate_condition)
{
    DebugAssert(predicate_condition == PredicateCondition::IsNull ||
...
ColumnIsNullTableScanImpl::scan_chunk(const ChunkID chunk_id)
{
    ...
    if (const auto value_segment =
            std::dynamic_pointer_cast<BaseValueSegment>(segment))
    {
    ...
    else
    {
        ...
        if (!chunk_sorted_by.empty())
        {
            for (const auto &sorted_by : chunk_sorted_by)
            {
                ...
                {
                    _scan_generic_sorted_segment(*segment, chunk_id, *matches,
                                                 sorted_by.sort_mode);
    ...
    RowIDPosList &matches) const
{
    segment_with_iterators(
        ...
        {
            // This may also be called for a ValueSegment if `segment` is a
            // ReferenceSegment pointing to a single ValueSegment.
            const auto invert =
                _predicate_condition == PredicateCondition::IsNotNull;
...
{
    const bool is_nulls_first =
        sorted_by == SortMode::Ascending || sorted_by == SortMode::Descending;
    const bool predicate_is_null =
        _predicate_condition == PredicateCondition::IsNull;
    segment_with_iterators(
        segment,
        [&](auto begin, auto end)
        {
            if (is_nulls_first)
            {
                const auto first_not_null = std::lower_bound(
                    begin, end, bool{},
                    [](const auto &segment_position, const auto & /*end*/)
            ...
            {
                matches[output_idx++] =
                    RowID{chunk_id, segment_it->chunk_offset()};
            }
        });
}

void ColumnIsNullTableScanImpl::_scan_value_segment(
    const BaseValueSegment &segment, const ChunkID chunk_id,
    RowIDPosList &matches)
{
    ...
    DebugAssert(segment.is_nullable(),
                "Columns that are not nullable should have been caught by edge "
                "case handling.");
...
bool ColumnIsNullTableScanImpl::_matches_all(
    const BaseValueSegment &segment) const
{
...
bool ColumnIsNullTableScanImpl::_matches_none(
    const BaseValueSegment &segment) const
{
    switch (_predicate_condition)
    {
...
void ColumnIsNullTableScanImpl::_add_all(const ChunkID chunk_id,
                                         RowIDPosList &matches,
                                         const size_t segment_size)
{
    const auto num_rows = segment_size;
    for (auto chunk_offset = ChunkOffset{0}; chunk_offset < num_rows;
         ++chunk_offset)
    {
