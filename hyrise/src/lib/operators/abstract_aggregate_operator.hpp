...
/*
The WindowFunctionBuilder is used to create the lambda function that will be
used by the AggregateVisitor. It is a separate class because methods cannot be
partially specialized. Therefore, we partially specialize the whole class and
define the get_aggregate_function anew every time.
*/
...
class WindowFunctionBuilder<ColumnDataType, AggregateType, WindowFunction::Min>
{
  public:
    auto get_aggregate_function()
    {
        ...
        {
            // We need to check if we have already seen a value before
            // (`aggregate_count > 0`) - otherwise, `accumulator` holds an
            // invalid value. While we might initialize `accumulator` with the
            // smallest possible numerical value, this approach does not work
            // for `max` on strings. To keep the code simple, we check
            // `aggregate_count` here.
  ...
  public:
    auto get_aggregate_function()
    {
        return [](const ColumnDataType &new_value, const size_t aggregate_count,
                  AggregateType &accumulator)
        {
            // Add new value to sum - no need to check if this is the first
            // value as `sum` is only defined on numerical values and the
            // accumulator is initialized with 0.
...
class WindowFunctionBuilder<ColumnDataType, AggregateType, WindowFunction::Avg>
{
  public:
    auto get_aggregate_function()
    {
        // We reuse Sum here, as updating an average value for every row is
        // costly and prone to problems regarding precision. To get the average,
        // the aggregate operator needs to count the number of elements
        // contributing to this sum, and divide the final sum by that number.
        return WindowFunctionBuilder<ColumnDataType, AggregateType,
                                     WindowFunction::Sum>{}
...
template <typename ColumnDataType, typename AggregateType>
class WindowFunctionBuilder<ColumnDataType, AggregateType,
    ...
    auto get_aggregate_function()
    {
                  ...
                  StandardDeviationSampleData &accumulator)
        {
            if constexpr (std::is_arithmetic_v<ColumnDataType>)
            {
                ...
                // For a new value, compute the new count, new mean and the new
                // squared_distance_from_mean.

                // get values
                auto &count = accumulator[0]; // We could probably reuse
                                              // aggregate_count here
                ...
                if (count > 1)
                {
                    // The SQL standard defines VAR_SAMP (which is the basis of
                    // STDDEV_SAMP) as NULL if the number of values is 1.
                    const auto variance =
            ...
            {
                Fail("StandardDeviationSample not available for non-arithmetic "
                     "types.");
            }
        };
    }
                            ...
                            WindowFunction::Count>
{
  public:
    auto get_aggregate_function()
    {
        return [](const ColumnDataType &, const size_t aggregate_count,
                  AggregateType &accumulator) {};
    }
};
