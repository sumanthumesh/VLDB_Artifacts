...
template <typename L, typename R, class Enable = void> struct JoinHashTraits
{
    using HashType = void;
};
...
struct JoinHashTraits<L, R,
                      std::enable_if_t<std::is_floating_point_v<L> &&
...
struct JoinHashTraits<
    L, R, std::enable_if_t<std::is_integral_v<L> && std::is_integral_v<R>>>
{
    using HashType = std::conditional_t<sizeof(L) < sizeof(R), R, L>; // NOLINT
};
...
// Joining with strings will use strings for hashing and a lexical cast if
// necessary
template <typename L, typename R>
struct JoinHashTraits<L, R,
                      std::enable_if_t<std::is_same_v<R, pmr_string> ||
                                       std::is_same_v<L, pmr_string>>>
{
