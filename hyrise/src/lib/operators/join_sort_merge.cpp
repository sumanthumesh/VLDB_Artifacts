...
{
    return (config.predicate_condition != PredicateCondition::NotEquals ||
...
// The sort merge join performs a join on two input tables on specific join
// columns. For usage notes, see the join_sort_merge.hpp. This is how the join
// works:
// -> The input tables are materialized and clustered into a specified number of
// clusters.
//    /utils/radix_cluster_sort.hpp for more info on the clustering phase.
// -> The join is performed per cluster. For the joining phase, runs of entries
// with the same value are identified
//    and handled at once. If a join-match is identified, the corresponding
//    row_ids are noted for the output.
// -> Using the join result, the output table is built using pos lists
// referencing the original tables.
JoinSortMerge::JoinSortMerge(
    const std::shared_ptr<const AbstractOperator> &left,
    const std::shared_ptr<const AbstractOperator> &right, const JoinMode mode,
    const OperatorJoinPredicate &primary_predicate,
    const std::vector<OperatorJoinPredicate> &secondary_predicates)
    : AbstractJoinOperator(
          OperatorType::JoinSortMerge, left, right, mode, primary_predicate,
    ...
    const
{
    return std::make_shared<JoinSortMerge>(
        copied_left_input, copied_right_input, _mode, _primary_predicate,
        _secondary_predicates);
}

void JoinSortMerge::_on_set_parameters(
...
std::shared_ptr<const Table> JoinSortMerge::_on_execute()
{
    ...
    std::shared_ptr<const Table> left_input_table_ptr =
        _left_input->get_output();
    std::shared_ptr<const Table> right_input_table_ptr =
        _right_input->get_output();

    // Check column types
    const auto &left_column_type = left_input_table()->column_data_type(
        _primary_predicate.column_ids.first);
    DebugAssert(left_column_type == right_input_table()->column_data_type(
                                        _primary_predicate.column_ids.second),
                "Left and right column types do not match. The sort merge join "
                "requires matching column types");
    ...
    resolve_data_type(
        ...
        [&](const auto type)
        {
            ...
            _impl = std::make_unique<JoinSortMergeImpl<ColumnDataType>>(
                *this, left_input_table_ptr, right_input_table_ptr,
                _primary_predicate.column_ids.first,
                _primary_predicate.column_ids.second,
                _primary_predicate.predicate_condition, _mode,
                _secondary_predicates,
                dynamic_cast<
                    OperatorPerformanceData<JoinSortMerge::OperatorSteps> &>(
                    *performance_data));
        });
...
class JoinSortMerge::JoinSortMergeImpl : public AbstractReadOnlyOperatorImpl
{
  public:
    JoinSortMergeImpl(
        ...
        : _sort_merge_join{sort_merge_join},
          ...
          _right_input_table{right_input_table}, _performance{performance_data},
          _primary_left_column_id{left_column_id},
          _primary_right_column_id{right_column_id},
          _primary_predicate_condition{op}, _mode{mode},
  ...
  protected:
    ...
    // These are used for outer joins with multiple predicates where the primary
    ...
    // The TablePosition is a utility struct that is used to define a specific
    // position in a sorted input table.
    struct TableRange;
    ...
    struct TablePosition
    {
        ...
        TablePosition(size_t init_cluster, size_t init_index)
            : cluster{init_cluster}, index{init_index}
        {
        }
    ...
    // The TableRange is a utility struct that is used to define ranges of rows
    // in a sorted input table spanning from a start position to an end
    // position.
    struct TableRange
    {
        TableRange(TablePosition start_position, TablePosition end_position)
            : start(start_position), end(end_position)
        {
        }

        TableRange(size_t cluster, size_t start_index, size_t end_index)
            : start{TablePosition(cluster, start_index)}, end{TablePosition(
                                                              cluster,
                                                              end_index)}
        {
        }
        ...
        // Executes the given action for every row id of the table in this
        // range.
        template <typename F>
        void for_every_row_id(const MaterializedSegmentList<T> &table,
                              const F &action)
        {
...
#endif
            for (auto cluster = start.cluster; cluster <= end.cluster;
                 ++cluster)
            {
                const auto start_index =
                    (cluster == start.cluster) ? start.index : 0;
                const auto end_index = (cluster == end.cluster)
                                           ? end.index
                                           : table[cluster].size();
...
#pragma GCC diagnostic pop
    ...
    // Determines the number of clusters to be used for the join. The number of
    // clusters must be a power of two.
    size_t _determine_number_of_clusters()
    {
        // We try to have a partition size of roughly 256 KB to avoid out-of-L2
        // cache sorts. This value has been determined by an array of
        // benchmarks. Ideally, it would incorporate hardware knowledge such as
        // the L2 cache size.
        const auto max_sort_items_count = 256'000 / sizeof(T);
        const size_t cluster_count_left =
            _sort_merge_join.left_input_table()->row_count() /
            max_sort_items_count;
        const size_t cluster_count_right =
            _sort_merge_join.right_input_table()->row_count() /
            max_sort_items_count;

        // Return the next larger power of two for the larger of the two cluster
        // counts.
        return static_cast<size_t>(std::pow(
            2, std::floor(std::log2(std::max(
                   {size_t{1}, cluster_count_left, cluster_count_right})))));
    }

    // Gets the table position corresponding to the end of the table, i.e. the
    // last entry of the last cluster.
    ...
    void _join_runs(TableRange left_run, TableRange right_run,
                    ...
                    const size_t cluster_id)
    {
        ...
        case PredicateCondition::Equals:
            ...
            {
                _emit_qualified_combinations(cluster_id, left_run, right_run,
                                             multi_predicate_join_evaluator);
            }
        ...
        case PredicateCondition::NotEquals:
            ...
            {
                _emit_qualified_combinations(
                    cluster_id, left_run.start.to(_end_of_left_table),
                    right_run, multi_predicate_join_evaluator);
            }
            else if (compare_result == CompareResult::Equal)
            {
                _emit_qualified_combinations(
                    cluster_id, left_run.end.to(_end_of_left_table), right_run,
                    multi_predicate_join_evaluator);
                _emit_qualified_combinations(
                    cluster_id, left_run, right_run.end.to(_end_of_right_table),
                    multi_predicate_join_evaluator);
            }
            else if (compare_result == CompareResult::Less)
            {
                _emit_qualified_combinations(
                    cluster_id, left_run,
                    right_run.start.to(_end_of_right_table),
                    multi_predicate_join_evaluator);
            }
            break;
    ...
    // Emits a combination of a left row id and a right row id to the join
    // output.
    void _emit_combination(size_t output_cluster, RowID left_row_id,
                           RowID right_row_id)
    {
    ...
    // Emits all the combinations of row ids from the left table range and the
    // right table range to the join output where also the secondary predicates
    // are satisfied.
    void _emit_qualified_combinations(size_t output_cluster,
                                      TableRange left_range,
                                          ...
                                          &multi_predicate_join_evaluator)
    {
        if (multi_predicate_join_evaluator)
        {
            ...
            {
                _emit_combinations_multi_predicated_inner(
                    output_cluster, left_range, right_range,
                    *multi_predicate_join_evaluator);
            }
            else if (_mode == JoinMode::Left)
            {
                _emit_combinations_multi_predicated_left_outer(
                    output_cluster, left_range, right_range,
                    *multi_predicate_join_evaluator);
            }
            ...
            {
                _emit_combinations_multi_predicated_full_outer(
                    output_cluster, left_range, right_range,
                    *multi_predicate_join_evaluator);
            }
        }
            ...
            left_range.for_every_row_id(
                ...
                [&](RowID left_row_id)
                {
                    right_range.for_every_row_id(
                        _sorted_right_table,
                        [&](RowID right_row_id) {
                            _emit_combination(output_cluster, left_row_id,
                                              right_row_id);
                        });
                });
        }
    ...
    // Emits all the combinations of row ids from the left table range and the
    // right table range to the join output where the secondary predicates are
    // satisfied.
    void _emit_combinations_multi_predicated_inner(
        size_t output_cluster, TableRange left_range, TableRange right_range,
        ...
        left_range.for_every_row_id(
            ...
            [&](RowID left_row_id)
            {
                right_range.for_every_row_id(
                    ...
                    {
                        if (multi_predicate_join_evaluator
                                .satisfies_all_predicates(left_row_id,
                                                          right_row_id))
                        {
                            _emit_combination(output_cluster, left_row_id,
                                              right_row_id);
    ...
    // Emits all the combinations of row ids from the left table range and the
    // right table range to the join output where the secondary predicates are
    // satisfied. For a left row id without a match, the combination [left row
    // id|NULL row id] is emitted.
    void _emit_combinations_multi_predicated_left_outer(
        size_t output_cluster, TableRange left_range, TableRange right_range,
        MultiPredicateJoinEvaluator &multi_predicate_join_evaluator)
    {
                    ...
                    right_range.for_every_row_id(
                        ...
                        [&](RowID right_row_id)
                        {
                                                              ...
                                                              right_row_id))
                            {
                                _emit_combination(output_cluster, left_row_id,
                                                  right_row_id);
                                _left_row_ids_emitted_per_chunk[output_cluster]
                                    .emplace(left_row_id);
                            }
                        });
                });
    ...
    // Emits all the combinations of row ids from the left table range and the
    // right table range to the join output where the secondary predicates are
    // satisfied. For a right row id without a match, the combination [NULL row
    // id|right row id] is emitted.
    void _emit_combinations_multi_predicated_right_outer(
        size_t output_cluster, TableRange left_range, TableRange right_range,
        MultiPredicateJoinEvaluator &multi_predicate_join_evaluator)
    {
        if (_primary_predicate_condition == PredicateCondition::Equals)
        {
            right_range.for_every_row_id(
                ...
                [&](RowID right_row_id)
                {
                    ...
                    {
                        _emit_combination(output_cluster, NULL_ROW_ID,
                                          right_row_id);
                    }
                });
        }
                    ...
                    left_range.for_every_row_id(
                        ...
                        [&](RowID left_row_id)
                        {
                                                              ...
                                                              right_row_id))
                            {
                                _emit_combination(output_cluster, left_row_id,
                                                  right_row_id);
                                _right_row_ids_emitted_per_chunk[output_cluster]
                                    .emplace(right_row_id);
                            }
                        });
                });
        }
    ...
    // Emits all the combinations of row ids from the left table range and the
    // right table range to the join output where the secondary predicates are
    // satisfied. For a left row id without a match, the combination [right row
    // id|NULL row id] is emitted. For a right row id without a match, the
    // combination [NULL row id|right row id] is emitted.
    void _emit_combinations_multi_predicated_full_outer(
        size_t output_cluster, TableRange left_range, TableRange right_range,
        MultiPredicateJoinEvaluator &multi_predicate_join_evaluator)
    {
        if (_primary_predicate_condition == PredicateCondition::Equals)
    ...
    // Emits all combinations of row ids from the left table range and a NULL
    // value on the right side (regarding the primary predicate) to the join
    // output.
    void _emit_right_primary_null_combinations(size_t output_cluster,
                                               TableRange left_range)
    {
        left_range.for_every_row_id(
            _sorted_left_table, [&](RowID left_row_id)
            { _emit_combination(output_cluster, left_row_id, NULL_ROW_ID); });
    }

    // Emits all combinations of row ids from the right table range and a NULL
    // value on the left side (regarding the primary predicate) to the join
    // output.
    void _emit_left_primary_null_combinations(size_t output_cluster,
                                              TableRange right_range)
    {
        right_range.for_every_row_id(
            _sorted_right_table, [&](RowID right_row_id)
            { _emit_combination(output_cluster, NULL_ROW_ID, right_row_id); });
    }

    // Determines the length of the run starting at start_index in the values
    // vector. A run is a series of the same value. Even though the input vector
    // is sorted, we first start by linearly scanning for the end of the run.
    // The reason is that we know with a high probability that the item we are
    // looking for is at the beginning of the input vector (the run value is the
    // first in the sequence and the sequence is sorted). If we do not find the
    // run's end within the linearly scanned part, we use a binary search. An
    // unsuccessful linear search should come with neglectable costs as we will
    // iterate over the values anyways.
    size_t _run_length(size_t start_index, const MaterializedSegment<T> &values)
    {
        ...
        {
            // Set end of linear search to end of input vector if we would
            // overshoot otherwise.
            end = values.end();
        }

        const auto linear_search_result = std::find_if(
            begin, end,
            [&](const auto &mat_value) { return mat_value.value > run_value; });
        ...
        {
            // We did not find a larger value in the linearly scanned part and
            // it spanned until the end of the input vector. That means all
            // values up to the end are part of the run.
            return std::distance(begin, end);
        }

        // Binary search in case the run did not end within the linearly scanned
        // part.
        const auto binary_search_result =
            std::upper_bound(end, values.end(), *end,
    ...
    // Performs the join on a single cluster. Runs of entries with the same
    // value are identified and handled together. This constitutes the merge
    // phase of the join. The output combinations of row ids are determined by
    // _join_runs.
                           ...
                           &multi_predicate_join_evaluator)
    {
        ...
        auto left_run_end =
            left_run_start + _run_length(left_run_start, left_cluster);
        auto right_run_end =
            right_run_start + _run_length(right_run_start, right_cluster);
        ...
        while (left_run_start < left_size && right_run_start < right_size)
        {
            ...
            const auto left_run =
                TableRange(cluster_id, left_run_start, left_run_end);
            const auto right_run =
                TableRange(cluster_id, right_run_start, right_run_end);
            _join_runs(left_run, right_run, compare_result,
                       multi_predicate_join_evaluator, cluster_id);
            ...
            case CompareResult::Equal:
                ...
                left_run_end =
                    left_run_start + _run_length(left_run_start, left_cluster);
                right_run_end = right_run_start +
                                _run_length(right_run_start, right_cluster);
                break;
        ...
        // Join the rest of the unfinished side, which is relevant for outer
        // joins and non-equi joins
        const auto right_rest =
            TableRange(cluster_id, right_run_start, right_size);
        const auto left_rest =
            TableRange(cluster_id, left_run_start, left_size);
    ...
    const T &_table_min_value(const MaterializedSegmentList<T> &sorted_table)
    {
        DebugAssert(_primary_predicate_condition != PredicateCondition::Equals,
                    "Complete table order is required for _table_min_value() "
                    "which is only available in the non-equi case");
    ...
    {
        DebugAssert(_primary_predicate_condition != PredicateCondition::Equals,
                    "The table needs to be sorted for _table_max_value() which "
                    "is only the case for non-equi joins");
        DebugAssert(!sorted_table.empty(), "Sorted table is empty");

        for (auto partition_iter = sorted_table.rbegin();
             partition_iter != sorted_table.rend(); ++partition_iter)
        {
    ...
    // Looks for the first value in a sorted materialized table that fulfills
    // the specified condition. Returns the TablePosition of this element and
    // whether a satisfying element has been found.
    template <typename Function>
    std::optional<TablePosition>
    _first_value_that_satisfies(const MaterializedSegmentList<T> &sorted_table,
                                const Function &condition)
    {
        const auto sorted_table_size = sorted_table.size();
        for (auto partition_id = size_t{0}; partition_id < sorted_table_size;
             ++partition_id)
        {
    ...
    // Looks for the first value in a sorted materialized table that fulfills
    // the specified condition, but searches the table in reverse order. Returns
    // the TablePosition of this element, and a satisfying element has been
    // found.
    template <typename Function>
    std::optional<TablePosition> _first_value_that_satisfies_reverse(
        const MaterializedSegmentList<T> &sorted_table,
        const Function &condition)
    {
        const auto sorted_table_size = sorted_table.size();
        for (auto partition_id = static_cast<int64_t>(sorted_table_size - 1);
             partition_id >= 0; --partition_id)
        {
            ...
            {
                const auto partition_size = partition.size();
                for (auto index = static_cast<int64_t>(partition_size - 1);
                     index >= 0; --index)
                {
                    if (condition(partition[index].value))
                    {
    ...
    // Adds the rows without matches for right outer joins for non-equi
    // operators (<, <=, >, >=). This method adds those rows from the right
    // table to the output that do not find a join partner. The outer join for
    // the equality operator is handled in _join_runs instead.
    void _right_outer_non_equi_join()
    {
        ...
        {
            _emit_left_primary_null_combinations(
                0, TablePosition(0, 0).to(end_of_right_table));
            return;
        }
        ...
        {
            // Look for the first right value that is bigger than the smallest
            // left value.
            auto result = _first_value_that_satisfies(
                _sorted_right_table,
                [&](const T &value) { return value > left_min_value; });
            if (result)
            {
                unmatched_range = TablePosition(0, 0).to(*result);
            }
        }
        else if (_primary_predicate_condition ==
                 PredicateCondition::LessThanEquals)
        {
            // Look for the first right value that is bigger or equal to the
            // smallest left value.
            auto result = _first_value_that_satisfies(
                _sorted_right_table,
                [&](const T &value) { return value >= left_min_value; });
        ...
        {
            // Look for the first right value that is smaller than the biggest
            // left value.
            auto result = _first_value_that_satisfies_reverse(
                _sorted_right_table,
        ...
        {
            // Look for the first right value that is smaller or equal to the
            // biggest left value.
            auto result = _first_value_that_satisfies_reverse(
                _sorted_right_table,
                [&](const T &value) { return value <= left_max_value; });
        ...
        if (unmatched_range)
        {
            ...
            unmatched_range->for_every_row_id(
                _sorted_right_table,
                [&](RowID right_row_id)
                {
                    // Mark as emitted so that it doesn't get emitted again
                    // below
                    _right_row_ids_emitted.emplace(right_row_id);
                });
        }

        // Add null-combinations for right row ids where the primary predicate
        // was satisfied but the secondary predicates were not.
    ...
    // (<, <=, >, >=). This method adds those rows from the left table to the
    // output that do not find a join partner. The outer join for the equality
    // operator is handled in _join_runs instead.
    void _left_outer_non_equi_join()
    {
        ...
        {
            _emit_right_primary_null_combinations(
                0, TablePosition(0, 0).to(end_of_left_table));
            return;
        }
        ...
        if (_primary_predicate_condition == PredicateCondition::LessThan)
        {
            // Look for the last left value that is smaller than the biggest
            // right value.
            auto result = _first_value_that_satisfies_reverse(
                _sorted_left_table,
            ...
            if (result)
            {
                unmatched_range = (*result).to(end_of_left_table);
                 ...
                 PredicateCondition::LessThanEquals)
        {
            // Look for the last left value that is smaller or equal than the
            // biggest right value.
            auto result = _first_value_that_satisfies_reverse(
                _sorted_left_table,
                [&](const T &value) { return value <= right_max_value; });
            if (result)
            {
                unmatched_range = (*result).to(end_of_left_table);
                 ...
                 PredicateCondition::GreaterThan)
        {
            // Look for the first left value that is bigger than the smallest
            // right value.
            auto result = _first_value_that_satisfies(
                _sorted_left_table,
                [&](const T &value) { return value > right_min_value; });
            if (result)
            {
                unmatched_range = TablePosition(0, 0).to(*result);
            }
                 ...
                 PredicateCondition::GreaterThanEquals)
        {
            // Look for the first left value that is bigger or equal to the
        ...
        if (unmatched_range)
        {
            ...
            unmatched_range->for_every_row_id(_sorted_left_table,
                                              [&](RowID left_row_id)
                                              {
                                                  // Mark as emitted so that it
                                                  // doesn't get emitted again
                                                  // below
                                                  _left_row_ids_emitted.emplace(
                                                      left_row_id);
                                              });
        }

        // Add null-combinations for left row ids where the primary predicate
        // was satisfied but the secondary predicates were not.
        if (!_secondary_join_predicates.empty())
        {
    ...
    void _perform_join()
    {
        ...
        for (auto cluster_id = size_t{0}; cluster_id < _cluster_count;
             ++cluster_id)
        {
            ...
            {
                if (_sorted_left_table[cluster_id].empty() ||
                    _sorted_right_table[cluster_id].empty())
                {
                    continue;
                }
            }
            ...
            const auto merge_row_count = _sorted_left_table[cluster_id].size() +
                                         _sorted_right_table[cluster_id].size();
            const auto join_cluster_task = [this, cluster_id]
            {
                ...
                if (!_secondary_join_predicates.empty())
                {
                    multi_predicate_join_evaluator.emplace(
                        *_sort_merge_join._left_input->get_output(),
        ...
        // Note: Equi outer joins can be integrated into the main algorithm,
        // while these can not.
        if ((_mode == JoinMode::Left || _mode == JoinMode::FullOuter) &&
            _primary_predicate_condition != PredicateCondition::Equals)
        {
  ...
  public:
    std::shared_ptr<const Table> _on_execute() override
    {
        const auto include_null_left =
            (_mode == JoinMode::Left || _mode == JoinMode::FullOuter);
        const auto include_null_right =
            (_mode == JoinMode::Right || _mode == JoinMode::FullOuter);
        auto radix_clusterer = RadixClusterSort<T>(
            _sort_merge_join.left_input_table(),
            _sort_merge_join.right_input_table(),
            _sort_merge_join._primary_predicate.column_ids,
            _primary_predicate_condition == PredicateCondition::Equals,
        ...
        if (include_null_left || include_null_right)
        {
            ...
            // Add the outer join rows which had a null value in their join
            // column
            if (include_null_left)
            {
                null_output_left.reserve(_null_rows_left.size());
                null_output_right.insert(null_output_right.end(),
                                         _null_rows_left.size(), NULL_ROW_ID);
            ...
            {
                null_output_left.insert(null_output_left.end(),
                                        _null_rows_right.size(), NULL_ROW_ID);
            ...
            DebugAssert(
                null_output_left.size() == null_output_right.size(),
        ...
        const auto create_left_side_pos_lists_by_segment =
            (_left_input_table->type() == TableType::References);
        const auto create_right_side_pos_lists_by_segment =
            (_right_input_table->type() == TableType::References);

        // A sort merge join's input can be heavily pre-filtered or the join
        // results in very few matches. In contrast to the hash join, we do not
        // (for now) merge small partitions to keep the sorted chunk guarantees,
        // which could be exploited by subsequent operators.
        constexpr auto ALLOW_PARTITION_MERGE = false;
        auto output_chunks = write_output_chunks(
            _output_pos_lists_left, _output_pos_lists_right, _left_input_table,
            _right_input_table, create_left_side_pos_lists_by_segment,
            create_right_side_pos_lists_by_segment,
            OutputColumnOrder::LeftFirstRightSecond, ALLOW_PARTITION_MERGE);

        const ColumnID left_join_column =
            _sort_merge_join._primary_predicate.column_ids.first;
        const ColumnID right_join_column = static_cast<ColumnID>(
            _sort_merge_join.left_input_table()->column_count() +
            _sort_merge_join._primary_predicate.column_ids.second);

        for (auto &chunk : output_chunks)
        {
            if (_sort_merge_join._primary_predicate.predicate_condition ==
                    PredicateCondition::Equals &&
                _mode == JoinMode::Inner)
            {
                chunk->finalize();
                // The join columns are sorted in ascending order (ensured by
                // radix_cluster_sort)
                chunk->set_individually_sorted_by(
                    {SortColumnDefinition(left_join_column,
                                          SortMode::Ascending),
                     SortColumnDefinition(right_join_column,
                                          SortMode::Ascending)});
            }
        }
        _performance.set_step_runtime(OperatorSteps::OutputWriting,
                                      timer.lap());

        auto result_table =
            _sort_merge_join._build_output_table(std::move(output_chunks));

        if (_mode != JoinMode::Left && _mode != JoinMode::Right &&
            _mode != JoinMode::FullOuter &&
            _sort_merge_join._primary_predicate.predicate_condition ==
                PredicateCondition::Equals)
        {
            // Table clustering is not defined for columns storing NULL values.
            // Additionally, clustering is not given for non-equal predicates.
            result_table->set_value_clustered_by(
