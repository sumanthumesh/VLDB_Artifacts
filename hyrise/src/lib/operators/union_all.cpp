...
std::shared_ptr<const Table> UnionAll::_on_execute()
{
    ...
    DebugAssert(left_input_table()->type() == right_input_table()->type(),
                "Input tables must have the same type");

    auto output_chunks = std::vector<std::shared_ptr<Chunk>>{
        left_input_table()->chunk_count() + right_input_table()->chunk_count()};
...
void UnionAll::_on_set_parameters(
