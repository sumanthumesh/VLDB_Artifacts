...
AbstractJoinOperator::AbstractJoinOperator(
    ...
    : AbstractReadOnlyOperator(type, left, right,
                               std::move(init_performance_data)),
      _mode(mode), _primary_predicate(primary_predicate),
      _secondary_predicates(secondary_predicates)
{
    Assert(mode != JoinMode::Cross, "Specified JoinMode not supported by an "
                                    "AbstractJoin, use Product etc. instead.");
    Assert(primary_predicate.predicate_condition ==
...
const std::vector<OperatorJoinPredicate> &
AbstractJoinOperator::secondary_predicates() const
{
    return _secondary_predicates;
}

std::string
AbstractJoinOperator::description(DescriptionMode description_mode) const
{
    const auto column_name = [&](const auto from_left, const auto column_id)
    {
        const auto state =
            from_left ? _left_input->state() : _right_input->state();
        if (state == OperatorState::ExecutedAndAvailable)
        {
            const auto &input_table = from_left ? _left_input->get_output()
        ...
        if (lqp_node)
        {
            ...
            const auto &input_lqp_node = lqp_node->input(
                from_left ? LQPInputSide::Left : LQPInputSide::Right);
            return input_lqp_node->output_expressions()[column_id]
                ->as_column_name();
        }
    ...
    if (_mode == JoinMode::Semi && lqp_node &&
        std::static_pointer_cast<const JoinNode>(lqp_node)->is_semi_reduction())
    {
    ...
    stream << column_name(true, _primary_predicate.column_ids.first) << " FromLeft Primary ";
    stream << _primary_predicate.predicate_condition << " ";
    stream << column_name(false, _primary_predicate.column_ids.second) << " FromRight Primary ";
    ...
    for (const auto &secondary_predicate : _secondary_predicates)
    {
        ...
        stream << column_name(false, secondary_predicate.column_ids.second) << " FromRight Secondary ";
    }

    return stream.str();
}

void AbstractJoinOperator::_on_set_parameters(
    const std::unordered_map<ParameterID, AllTypeVariant> &parameters)
{
}

std::shared_ptr<Table> AbstractJoinOperator::_build_output_table(
    ...
    const TableType table_type) const
{
    ...
    const bool left_may_produce_null =
        (_mode == JoinMode::Right || _mode == JoinMode::FullOuter);
    const bool right_may_produce_null =
    ...
    for (auto column_id = ColumnID{0};
         column_id < left_in_table->column_count(); ++column_id)
    {
        const auto nullable = (left_may_produce_null ||
                               left_in_table->column_is_nullable(column_id));
        output_column_definitions.emplace_back(
            left_in_table->column_name(column_id),
    ...
    {
        for (auto column_id = ColumnID{0};
             column_id < right_in_table->column_count(); ++column_id)
        {
            const auto nullable =
                (right_may_produce_null ||
                 right_in_table->column_is_nullable(column_id));
            output_column_definitions.emplace_back(
                right_in_table->column_name(column_id),
