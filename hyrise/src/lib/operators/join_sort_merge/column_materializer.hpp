...
template <typename T> struct MaterializedValue
{
...
// SubSamples are passed into the materialization phase. They are used to pass
// the number of requested samples and are filled by the materialization tasks.
// All SubSamples are later merged to gather a global sample list with which the
// split values for the radix partitioning are determined.
template <typename T> struct Subsample
{
    explicit Subsample(ChunkOffset sample_count)
        : samples_to_collect(sample_count), samples(sample_count)
    {
    }
...
template <typename T> class ColumnMaterializer
{
  public:
    explicit ColumnMaterializer(bool sort, bool materialize_null)
        : _sort{sort}, _materialize_null{materialize_null}
    {
    }

  public:
    // For sufficiently large chunks (number of rows > JOB_SPAWN_THRESHOLD), the
    // materialization is parallelized. Returns the materialized segments and a
    // list of null row ids if _materialize_null is true.
    std::tuple<MaterializedSegmentList<T>, RowIDPosList, std::vector<T>>
    materialize(const std::shared_ptr<const Table> &input,
                const ColumnID column_id)
    {
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
        {
            ...
            const auto samples_to_write =
                std::min(SAMPLES_PER_CHUNK, chunk_size);
            subsamples.push_back(Subsample<T>(samples_to_write));

            auto materialize_job = [&, chunk_id]
            {
                const auto &segment =
                    input->get_chunk(chunk_id)->get_segment(column_id);
                output[chunk_id] = _materialize_segment(
                    segment, chunk_id, null_rows_per_chunk[chunk_id],
                    subsamples[chunk_id]);
            };
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
        {
            ...
            gathered_samples.insert(gathered_samples.end(),
                                    subsample.samples.begin(),
                                    subsample.samples.end());

            const auto &chunk_null_rows = null_rows_per_chunk[chunk_id];
            null_rows.insert(null_rows.end(), chunk_null_rows.begin(),
                             chunk_null_rows.end());
        }
        gathered_samples.shrink_to_fit();

        return {std::move(output), std::move(null_rows),
                std::move(gathered_samples)};
    }

  private:
    // Sample values from a materialized segment. We collect samples locally and
    // write once to the global sample collection to limit non-local writes.
    void _gather_samples_from_segment(const MaterializedSegment<T> &segment,
                                      Subsample<T> &subsample) const
    {
        ...
        if (segment.size() > 0 && samples_to_collect > 0)
        {
            ...
            const auto step_width =
                segment.size() / std::max(ChunkOffset{1}, samples_to_collect);

            for (auto sample_count = size_t{0};
                 sample_count < samples_to_collect; ++sample_count)
            {
                // NULL values in passed `segment` vector have already been
                // removed, thus we do not have to check for NULL samples.
                collected_samples.push_back(
                    segment[static_cast<size_t>(sample_count * step_width)]
                        .value);
            }

            // Sequential write of result
            subsample.samples.insert(subsample.samples.end(),
                                     collected_samples.begin(),
                                     collected_samples.end());
        }
    }

    MaterializedSegment<T>
    _materialize_segment(const std::shared_ptr<AbstractSegment> &segment,
