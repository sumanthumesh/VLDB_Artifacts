...
process_match(RowID left_row_id, RowID right_row_id,
              const JoinNestedLoop::JoinParams &params)
{
    // Write out a pair of matching row_ids - except for Semi/Anti joins, who
    // build their output from params.left_matches after all pairs were compared
...
// __attribute__((noinline)) to reduce compile time. As the hotloop is within
// this function, no performance loss expected
template <typename BinaryFunctor, typename LeftIterator, typename RightIterator>
void __attribute__((noinline))
join_two_typed_segments(const BinaryFunctor &func, LeftIterator left_it,
                        ...
                        const JoinNestedLoop::JoinParams &params)
{
    for (; left_it != left_end; ++left_it)
    {
        ...
        for (auto right_it = right_begin; right_it != right_end; ++right_it)
        {
            ...
            const auto right_row_id =
                RowID{chunk_id_right, right_value.chunk_offset()};

            // AntiNullAsTrue is the only join mode where NULLs in any operand
            // lead to a match. For all other join modes, any NULL in the
            // predicate results in a non-match.
            if (params.mode == JoinMode::AntiNullAsTrue)
            {
                if ((left_value.is_null() || right_value.is_null() ||
                     func(left_value.value(), right_value.value())) &&
                    params.secondary_predicate_evaluator
                        .satisfies_all_predicates(left_row_id, right_row_id))
                {
...
JoinNestedLoop::JoinNestedLoop(
    const std::shared_ptr<const AbstractOperator> &left,
    const std::shared_ptr<const AbstractOperator> &right, const JoinMode mode,
    const OperatorJoinPredicate &primary_predicate,
    const std::vector<OperatorJoinPredicate> &secondary_predicates)
    : AbstractJoinOperator(OperatorType::JoinNestedLoop, left, right, mode,
...
std::shared_ptr<AbstractOperator> JoinNestedLoop::_on_deep_copy(
    ...
    const
{
    return std::make_shared<JoinNestedLoop>(
        copied_left_input, copied_right_input, _mode, _primary_predicate,
        _secondary_predicates);
}

void JoinNestedLoop::_on_set_parameters(
...
std::shared_ptr<const Table> JoinNestedLoop::_on_execute()
{
    ...
    auto maybe_flipped_predicate_condition =
    ...
    if (_mode == JoinMode::Right)
    {
        ...
        maybe_flipped_predicate_condition =
            flip_predicate_condition(_primary_predicate.predicate_condition);
    ...
    const auto is_outer_join = _mode == JoinMode::Left ||
    ...
    auto left_matches_by_chunk =
        std::vector<std::vector<bool>>(left_table->chunk_count());

    auto right_matches_by_chunk =
        std::vector<std::vector<bool>>(right_table->chunk_count());
    const auto chunk_count_right = right_table->chunk_count();
    for (ChunkID chunk_id_right = ChunkID{0};
         chunk_id_right < chunk_count_right; ++chunk_id_right)
    {
        const auto chunk_right = right_table->get_chunk(chunk_id_right);
        Assert(chunk_right, "Physically deleted chunk should not reach this "
    ...
    auto secondary_predicate_evaluator = MultiPredicateJoinEvaluator{
        *left_table, *right_table, _mode, maybe_flipped_secondary_predicates};

    // Scan all chunks from left input
    const auto chunk_count_left = left_table->chunk_count();
    for (ChunkID chunk_id_left = ChunkID{0}; chunk_id_left < chunk_count_left;
         ++chunk_id_left)
    {
        ...
        {
            const auto chunk_right = right_table->get_chunk(chunk_id_right);
            Assert(chunk_right, "Physically deleted chunk should not reach "
                                "this point, see get_chunk / #1686.");

            const auto segment_right =
                chunk_right->get_segment(right_column_id);
            ...
            JoinParams params{*pos_list_left,
                              *pos_list_right,
                              ...
                              !semi_or_anti_join};
            _join_two_untyped_segments(*segment_left, *segment_right,
        ...
        if (is_outer_join)
        {
            ...
            for (ChunkOffset chunk_offset{0};
                 chunk_offset < static_cast<ChunkOffset>(left_matches.size());
                 ++chunk_offset)
            {
    ...
    // results to the PosLists We use `left_matches_by_chunk` to determine
    // whether a tuple from the left side found a match.
    if (semi_or_anti_join)
    {
        const auto invert = _mode == JoinMode::AntiNullAsFalse ||
                            _mode == JoinMode::AntiNullAsTrue;

        for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count_left;
             ++chunk_id)
        {
...
void JoinNestedLoop::_join_two_untyped_segments(
    const AbstractSegment &abstract_segment_left,
    ...
    const ChunkID chunk_id_right, JoinNestedLoop::JoinParams &params)
{
     ...
     *
     * To reduce compile time, we erase the types of Segments and the
     * PredicateCondition/comparator if `abstract_segment_left.data_type() !=
     * abstract_segment_left.data_type()` or `LeftSegmentType !=
     * RightSegmentType`. This is the "SLOW PATH". If data types and segment
     * types are the same, we take the "FAST PATH", where only the SegmentType
     * of left segment is erased and inlining optimization can be performed by
     * the compiler for the inner loop.
     *
     * Having this SLOW PATH and erasing the SegmentType even for the FAST PATH
     * are essential for keeping the compile time of the JoinNestedLoop
     * reasonably low.
     */
    ...
    if (abstract_segment_left.data_type() == abstract_segment_right.data_type())
    {
        ...
        resolve_data_and_segment_type(
            abstract_segment_left,
            [&](const auto data_type_t, const auto &segment_left)
            {
                using ColumnDataType = typename decltype(data_type_t)::type;
                using LeftSegmentType = std::decay_t<decltype(segment_left)>;

                if (const auto *segment_right =
                        dynamic_cast<const LeftSegmentType *>(
                            &abstract_segment_right))
                {
                    const auto iterable_left =
                        create_any_segment_iterable<ColumnDataType>(
                            segment_left);
                    const auto iterable_right =
                        create_iterable_from_segment<ColumnDataType>(
                            *segment_right);

                    iterable_left.with_iterators(
                        [&](auto left_begin, const auto &left_end)
                        {
                            iterable_right.with_iterators(
                                [&](auto right_begin, const auto &right_end)
                                {
                                    with_comparator(
                                        params.predicate_condition,
                                        [&](auto comparator)
                                        {
                                            join_two_typed_segments(
                                                comparator, left_begin,
                                                left_end, right_begin,
                                                right_end, chunk_id_left,
...
void JoinNestedLoop::_write_output_chunk(
    Segments &segments, const std::shared_ptr<const Table> &input_table,
    const std::shared_ptr<RowIDPosList> &pos_list)
{
    // Add segments from table to output chunk
    for (auto column_id = ColumnID{0}; column_id < input_table->column_count();
         ++column_id)
    {
                    ...
                    else
                    {
                        auto reference_segment =
                            std::static_pointer_cast<const ReferenceSegment>(
                                input_table->get_chunk(row.chunk_id)
