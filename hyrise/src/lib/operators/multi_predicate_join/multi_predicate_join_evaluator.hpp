...
// This class is used to evaluate secondary join predicates. It is called by the
// join operators after the primary predicate has been handled by, e.g., probing
// the hash table. As accessors are not thread-safe, instances of this class
// should not be used in multiple threads.
class MultiPredicateJoinEvaluator
{
  public:
    MultiPredicateJoinEvaluator(
        const Table &left, const Table &right, const JoinMode join_mode,
        const std::vector<OperatorJoinPredicate> &join_predicates);

    bool satisfies_all_predicates(const RowID &left_row_id,
  ...
  protected:
    ...
    class FieldComparator : public BaseFieldComparator
    {
      public:
        FieldComparator(CompareFunctor compare_functor, JoinMode join_mode,
                        std::vector<std::unique_ptr<AbstractSegmentAccessor<L>>>
                            left_accessors,
                        std::vector<std::unique_ptr<AbstractSegmentAccessor<R>>>
                            right_accessors)
            : _compare_functor{std::move(compare_functor)},
              _join_mode{join_mode}, _left_accessors{std::move(left_accessors)},
        ...
        bool compare(const RowID &left, const RowID &right) const override
        {
            const auto left_value =
                _left_accessors[left.chunk_id]->access(left.chunk_offset);
            const auto right_value =
                _right_accessors[right.chunk_id]->access(right.chunk_offset);
            // NULL value handling:
            // If either left or right value is NULL, the comparison will
            // evaluate to TRUE for AntiNullAsTrue and to FALSE for all other
            // JoinModes.
    ...
    template <typename T>
    static std::vector<std::unique_ptr<AbstractSegmentAccessor<T>>>
    _create_accessors(const Table &table, const ColumnID column_id);
};
