...
template <typename T>
std::vector<T> concatenate(const std::vector<T> &left,
...
JoinVerification::JoinVerification(
    ...
    : AbstractJoinOperator(OperatorType::JoinVerification, left, right, mode,
...
std::shared_ptr<const Table> JoinVerification::_on_execute()
{
    ...
    // Tuples with NULLs used to fill up tuples in outer joins that do not find
    // a match
    const auto null_tuple_left =
        Tuple(left_input_table()->column_count(), NullValue{});
    const auto null_tuple_right =
        Tuple(right_input_table()->column_count(), NullValue{});
    ...
    case JoinMode::FullOuter:
    {
        ...
        for (auto left_tuple_idx = size_t{0}; left_tuple_idx < left_tuple_count;
             ++left_tuple_idx)
        {
            const auto &left_tuple = left_tuples[left_tuple_idx];

            for (auto right_tuple_idx = size_t{0};
                 right_tuple_idx < right_tuple_count; ++right_tuple_idx)
            {
        ...
        for (auto left_tuple_idx = size_t{0}; left_tuple_idx < left_row_count;
             ++left_tuple_idx)
        {
            if (!left_matches[left_tuple_idx])
            {
                output_table->append(
                    concatenate(left_tuples[left_tuple_idx], null_tuple_right));
            }
        }

        for (auto right_tuple_idx = size_t{0};
             right_tuple_idx < right_row_count; ++right_tuple_idx)
        {
            if (!right_matches[right_tuple_idx])
            {
                output_table->append(concatenate(
                    null_tuple_left, right_tuples[right_tuple_idx]));
            }
        }
    }
    break;
    ...
    case JoinMode::Semi:
    {
        for (const auto &left_tuple : left_tuples)
        {
            const auto has_match =
                std::any_of(right_tuples.begin(), right_tuples.end(),
    ...
    case JoinMode::AntiNullAsFalse:
    {
        for (const auto &left_tuple : left_tuples)
        {
            const auto has_no_match =
                std::none_of(right_tuples.begin(), right_tuples.end(),
                             [&](const auto &right_tuple) {
                                 return _tuples_match(left_tuple, right_tuple);
                             });
...
bool JoinVerification::_tuples_match(const Tuple &tuple_left,
                                     const Tuple &tuple_right) const
{
    ...
    return std::all_of(_secondary_predicates.begin(),
                       _secondary_predicates.end(),
                       [&](const auto &secondary_predicate) {
                           return _evaluate_predicate(secondary_predicate,
                                                      tuple_left, tuple_right);
                       });
}

bool JoinVerification::_evaluate_predicate(
    const OperatorJoinPredicate &predicate, const Tuple &tuple_left,
    const Tuple &tuple_right) const
{
    auto result = false;
    ...
    {
        // AntiNullAsTrue is the only JoinMode that treats null-booleans as
        // TRUE, all others treat it as FALSE
        return _mode == JoinMode::AntiNullAsTrue;
    }

    resolve_data_type(
        data_type_from_all_type_variant(variant_left),
        [&](const auto data_type_left_t)
        {
            using ColumnDataTypeLeft =
                typename decltype(data_type_left_t)::type;

            resolve_data_type(
                data_type_from_all_type_variant(variant_right),
                [&](const auto data_type_right_t)
                {
                    using ColumnDataTypeRight =
                        typename decltype(data_type_right_t)::type;

                    if constexpr (std::is_same_v<ColumnDataTypeLeft,
                                                 pmr_string> ==
                                  std::is_same_v<ColumnDataTypeRight,
                                                 pmr_string>)
                    {
                        with_comparator(predicate.predicate_condition,
                                        ...
                                        {
                                            result = comparator(
                                                boost::get<ColumnDataTypeLeft>(
                                                    variant_left),
                                                boost::get<ColumnDataTypeRight>(
                                                    variant_right));
                                        });
                    }
                    else
...
std::shared_ptr<AbstractOperator> JoinVerification::_on_deep_copy(
    ...
    const
{
    return std::make_shared<JoinVerification>(
        copied_left_input, copied_right_input, _mode, _primary_predicate,
        _secondary_predicates);
}

void JoinVerification::_on_set_parameters(
