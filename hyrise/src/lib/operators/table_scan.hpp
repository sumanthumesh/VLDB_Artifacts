...
class TableScan : public AbstractReadOnlyOperator
{
  ...
  public:
    ...
    /**
     * Create the TableScanImpl based on the predicate type. Public for testing
     * purposes. Resolves uncorrelated subqueries, caches their results in the
     * ExpressionEvaluator or a new predicate, and deregisters accordingly.
     */
                                 ...
                                 AbstractOperatorPerformanceData::NoSteps>
    {
                              ...
                              DescriptionMode description_mode) const override
        {
            OperatorPerformanceData<AbstractOperatorPerformanceData::NoSteps>::
                output_to_stream(stream, description_mode);

            const auto separator =
                (description_mode == DescriptionMode::SingleLine ? ' ' : '\n');
            stream << separator
                   << "Chunks: " << num_chunks_with_early_out.load()
                   << " skipped with no results, ";
            stream << separator << num_chunks_with_all_rows_matching.load()
                   << " skipped with all matching, ";
            stream << num_chunks_with_binary_search.load()
                   << " scanned using binary search.";
        }
    };
  ...
  protected:
    ...
    void _on_set_transaction_context(
        const std::weak_ptr<TransactionContext> &transaction_context) override;
    ...
    // Turns top-level uncorrelated subqueries into their value, e.g. `a =
    // (SELECT 123)` becomes `a = 123`. This makes it easier to avoid using the
    // more expensive ExpressionEvaluatorTableScanImpl.
  ...
  private:
    ...
    // The description of the impl, so that it still available after the _impl
    // is resetted in _on_cleanup()
