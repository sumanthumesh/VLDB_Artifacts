...
class JoinNestedLoop : public AbstractJoinOperator
{
  ...
  protected:
    ...
    // Having all these static methods and passing around the state of the
    // JoinNestedLoop is somewhat ugly, but it allows us to reuse the code in
    // JoinIndex as a fallback. __attribute__((noinline)) is simply magic -
    // otherwise the compiler would try to put the entire join for all types
    // into a single, monolithic function. For -O3 on clang, this reduces the
    // compile time to a fourth.
    ...
    static void
    _write_output_chunk(Segments &segments,
