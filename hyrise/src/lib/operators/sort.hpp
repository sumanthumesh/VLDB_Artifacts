...
/**
 * Operator to sort a table by one or multiple columns. This implements a stable
 * sort, i.e., rows that share the same value will maintain their relative
 * order. By passing multiple sort column definitions it is possible to sort
 * multiple columns with one operator run.
 */
class Sort : public AbstractReadOnlyOperator
{
  ...
  protected:
    ...
    template <typename SortColumnType> class SortImpl;

    template <typename SortColumnType> class SortImplMaterializeOutput;
