...
{
    return static_cast<uint32_t>(PrintFlags::Mvcc) &
           static_cast<uint32_t>(flags);
}

bool has_print_ignore_chunk_boundaries_flag(const PrintFlags flags)
{
    return static_cast<uint32_t>(PrintFlags::IgnoreChunkBoundaries) &
           static_cast<uint32_t>(flags);
}
...
Print::Print(const std::shared_ptr<const AbstractOperator> &input_operator,
             const PrintFlags flags, std::ostream &out)
    : AbstractReadOnlyOperator(OperatorType::Print, input_operator),
      _flags(flags), _out(out)
{
}
...
void Print::_on_set_parameters(
    const std::unordered_map<ParameterID, AllTypeVariant> &parameters)
{
}

void Print::print(const std::shared_ptr<const Table> &table,
                  const PrintFlags flags, std::ostream &out)
{
...
void Print::print(const std::shared_ptr<const AbstractOperator> &op,
                  const PrintFlags flags, std::ostream &out)
{
    Print(op, flags, out).execute();
}

void Print::print(const std::string &sql, const PrintFlags flags,
                  std::ostream &out)
{
    auto pipeline = SQLPipelineBuilder{sql}.create_pipeline();
    const auto [status, result_tables] = pipeline.get_result_tables();
    Assert(status == SQLPipelineStatus::Success,
           "SQL execution was unsuccessful.");
...
std::shared_ptr<const Table> Print::_on_execute()
{
    ...
    const auto widths = _column_string_widths(_min_cell_width, _max_cell_width,
                                              left_input_table());
    ...
    {
        _out << "|" << std::setw(widths[column_id])
             << left_input_table()->column_name(column_id) << std::setw(0);
    }
    ...
    {
        _out << "|" << std::setw(widths[column_id])
             << left_input_table()->column_data_type(column_id) << std::setw(0);
    }
    if (has_print_mvcc_flag(_flags))
    {
    ...
    for (auto column_id = ColumnID{0}; column_id < column_count; ++column_id)
    {
        ...
        _out << "|" << std::setw(widths[column_id])
             << (nullable ? "null" : "not null") << std::setw(0);
    }
    if (has_print_mvcc_flag(_flags))
    {
        _out << "||      |      |      ";
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
        ...
        if (!has_print_ignore_chunk_boundaries_flag(_flags))
        {
                 ...
                 ++column_id)
            {
                ...
                _out << "|" << std::setw(column_width) << std::left
                     << _segment_type(segment) << std::right << std::setw(0);
            }
            if (has_print_mvcc_flag(_flags))
            {
                _out << "|";
             ...
             ++chunk_offset)
        {
            ...
            for (auto column_id = ColumnID{0}; column_id < column_count;
                 ++column_id)
            {
                // well yes, we use AbstractSegment::operator[] here, but since
                // Print is not an operation that should be part of a regular
                // query plan, let's keep things simple here
                auto column_width = widths[column_id];
                auto cell = _truncate_cell(
                    (*chunk->get_segment(column_id))[chunk_offset],
                    column_width);
            ...
            if (has_print_mvcc_flag(_flags) && chunk->has_mvcc_data())
            {
                ...
                auto begin_string = begin == MvccData::MAX_COMMIT_ID
                                        ? ""
                                        : std::to_string(begin);
                auto end_string =
                    end == MvccData::MAX_COMMIT_ID ? "" : std::to_string(end);
...
// In order to print the table as an actual table, with columns being aligned,
// we need to calculate the number of characters in the printed representation
// of each column `min` and `max` can be used to limit the width of the columns
// - however, every column fits at least the column's name
std::vector<uint16_t>
Print::_column_string_widths(uint16_t min, uint16_t max,
                             const std::shared_ptr<const Table> &table) const
{
    ...
    for (auto column_id = ColumnID{0}; column_id < column_count; ++column_id)
    {
        widths[column_id] = std::max(
            min, static_cast<uint16_t>(table->column_name(column_id).size()));
    }

    // go over all rows and find the maximum length of the printed
    // representation of a value, up to max
    const auto chunk_count = left_input_table()->chunk_count();
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
        const auto chunk = left_input_table()->get_chunk(chunk_id);
             ...
             ++column_id)
        {
                 ...
                 ++chunk_offset)
            {
                ...
                widths[column_id] = std::max(
                    {min, widths[column_id], std::min(max, cell_length)});
            }
        }
    }
    return widths;
...
std::string Print::_truncate_cell(const AllTypeVariant &cell,
                                  uint16_t max_width)
{
    auto cell_string = boost::lexical_cast<std::string>(cell);
    DebugAssert(max_width > 3,
                "Cannot truncate string with '...' at end with max_width <= 3");
...
std::string
Print::_segment_type(const std::shared_ptr<AbstractSegment> &segment)
{
    ...
    else if (std::dynamic_pointer_cast<ReferenceSegment>(segment))
    {
        segment_type += "ReferS";
                 ...
                 std::dynamic_pointer_cast<AbstractEncodedSegment>(segment))
    {
