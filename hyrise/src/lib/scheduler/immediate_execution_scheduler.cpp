...
ImmediateExecutionScheduler::queues() const
{
    return _queues;
}

void ImmediateExecutionScheduler::schedule(std::shared_ptr<AbstractTask> task,
                                           NodeID /*preferred_node_id*/,
                                           SchedulePriority /*priority*/)
{
    DebugAssert(task->is_scheduled(),
                "Don't call ImmediateExecutionScheduler::schedule(), call "
                "schedule() on the task");

    if (task->is_ready())
    {
        // std::cout<<"ImmediateExecutionScheduler "
        // <<task->description()<<"\n";
        // std::cout<<std::this_thread::get_id()<<","<<getpid()<<"\n";
        task->execute();
    }
    else
    {
