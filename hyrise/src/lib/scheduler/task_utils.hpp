...
/**
 * Calls the passed @param visitor on @param task and recursively on its
 * PREDECESSORS. The visitor returns `TaskVisitation`, indicating whether the
 * current task's predecessors should be visited as well. The algorithm is
 * breadth-first search. Each task is visited exactly once.
 *
 * @tparam Visitor      Functor called with every task as a param. Returns
 * `TaskVisitation`.
 */
...
void visit_tasks(const std::shared_ptr<Task> &task, Visitor visitor)
{
    using AbstractTaskType =
        std::conditional_t<std::is_const_v<Task>, const AbstractTask,
                           AbstractTask>;
    ...
    auto visited_tasks =
        std::unordered_set<std::shared_ptr<AbstractTaskType>>{};
...
/**
 * Calls the passed @param visitor on @param task and recursively on its
 * SUCCESSORS. The visitor returns `TaskUpwardVisitation`, indicating whether
 * the current tasks's successors should be visited as well. Each node is
 * visited exactly once.
 *
 * @tparam Visitor      Functor called with every task as a param. Returns
 * `TaskUpwardVisitation`.
 */
template <typename Task, typename Visitor>
