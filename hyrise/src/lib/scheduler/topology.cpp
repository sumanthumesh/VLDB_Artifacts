...
#if HYRISE_NUMA_SUPPORT
const int Topology::_number_of_hardware_nodes =
    numa_num_configured_nodes(); // NOLINT
#else
...
void Topology::use_fake_numa_topology(uint32_t max_num_workers,
...
void Topology::_init_numa_topology(uint32_t max_num_cores)
{
    ...
    // We take the CPU affinity (set, e.g., by numactl) of our process into
    // account. Otherwise, we would always start with the first CPU, even if a
    // specific NUMA node was selected.
    ...
    for (auto node_id = 0; node_id <= max_node; ++node_id)
    {
        if (max_num_cores == 0 || core_count < max_num_cores)
        {
            ...
            {
                const auto cpu_is_part_of_node =
                    numa_bitmask_isbitset(this_node_cpu_bitmask, cpu_id);
                const auto cpu_is_part_of_affinity =
                    numa_bitmask_isbitset(affinity_cpu_bitmask, cpu_id);
...
void Topology::_init_fake_numa_topology(uint32_t max_num_workers,
                                        uint32_t workers_per_node)
{
    _clear();
    ...
    for (auto node_id = uint32_t{0}; node_id < num_nodes; ++node_id)
    {
        ...
        for (auto worker_id = uint32_t{0};
             worker_id < workers_per_node && cpu_id < num_workers; ++worker_id)
        {
            cpus.emplace_back(cpu_id);
            ++cpu_id;
        }
...
void Topology::set_num_cpus(uint32_t num_cores)
{
    _num_cpus = num_cores;
}
...
std::ostream &operator<<(std::ostream &stream, const Topology &topology)
{
    ...
    {
        stream << "Available CPUs / nodes were filtered by externally set CPU "
                  "affinity (e.g., numactl)."
               << std::endl;
    }
    for (auto node_idx = size_t{0}; node_idx < topology.nodes().size();
         ++node_idx)
    {
