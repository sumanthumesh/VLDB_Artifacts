                     ...
                     const SchedulePriority priority)
{
    ...
    [[maybe_unused]] const auto enqueue_successful =
...
std::shared_ptr<AbstractTask> TaskQueue::pull()
{
    ...
    // We waited for the semaphore to enter pull() but did not receive a task.
    // Ensure that queues are checked again.
    semaphore.signal();
    return nullptr;
}
...
std::shared_ptr<AbstractTask> TaskQueue::steal()
{
    ...
    for (auto &queue : _queues)
    {
        if (queue.try_dequeue(task))
        {
            ...
            [[maybe_unused]] const auto enqueue_successful =
                queue.enqueue(task);
            DebugAssert(enqueue_successful,
                        "Enqueuing stolen task did not succeed.");
            semaphore.signal();
        }
    }

    // We waited for the semaphore to enter steal() but did not receive a task.
    // Ensure that queues are checked again.
    semaphore.signal();
    return nullptr;
}
...
size_t TaskQueue::estimate_load() const
{
    ...
    // Simple heuristic to estimate the load: the higher the priority, the
    // higher the costs. We use powers of two (starting with 2^0) to calculate
    // the cost factor per priority level.
    for (auto queue_id = size_t{0}; queue_id < NUM_PRIORITY_LEVELS; ++queue_id)
    {
        // The lowest priority has a multiplier of 2^0, the next higher priority
        // 2^1, and so on.
        estimated_load += _queues[queue_id].size_approx() *
                          (size_t{1} << (NUM_PRIORITY_LEVELS - 1 - queue_id));
    }

    return estimated_load;
}

void TaskQueue::signal(const size_t count) { semaphore.signal(count); }
