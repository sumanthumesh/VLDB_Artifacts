...
void export_values(std::ofstream &ofstream,
                   const std::vector<T, Alloc> &values);

/* Writes the given strings to the ofstream. First an array of string lengths is
 * written. After that the strings are written without any gaps between them. In
 * order to reduce the number of memory allocations we iterate twice over the
 * string vector. After the first iteration we know the number of byte that must
 * be written to the file and can construct a buffer of this size. This approach
 * is indeed faster than a dynamic approach with a stringstream.
 */
void export_string_values(std::ofstream &ofstream,
...
{
    ofstream.write(reinterpret_cast<const char *>(values.data()),
                   values.size() * sizeof(T));
}

void export_values(std::ofstream &ofstream, const FixedStringVector &values)
{
    ofstream.write(values.data(), static_cast<int64_t>(values.size() *
                                                       values.string_length()));
}
                   ...
                   const std::vector<bool, Alloc> &values)
{
    ...
    const auto writable_bools =
        pmr_vector<BoolAsByteType>(values.begin(), values.end());
    export_values(ofstream, writable_bools);
}

// Writes a shallow copy of the given value to the ofstream
template <typename T> void export_value(std::ofstream &ofstream, const T &value)
{
    ofstream.write(reinterpret_cast<const char *>(&value), sizeof(T));
}

void export_compact_vector(std::ofstream &ofstream,
                           const pmr_compact_vector &values)
{
    export_value(ofstream, static_cast<uint8_t>(values.bits()));
    ofstream.write(reinterpret_cast<const char *>(values.get()),
...
{
    const auto target_chunk_size = table.type() == TableType::Data
                                       ? table.target_chunk_size()
                                       : Chunk::DEFAULT_SIZE;
    export_value(ofstream, static_cast<ChunkOffset>(target_chunk_size));
    export_value(ofstream,
                 static_cast<ChunkID::base_type>(table.chunk_count()));
    export_value(ofstream,
                 static_cast<ColumnID::base_type>(table.column_count()));
    ...
    // Transform column types and copy column names in order to write them to
    // the file.
    const auto column_count = table.column_count();
    for (auto column_id = ColumnID{0}; column_id < column_count; ++column_id)
    {
        column_types[column_id] =
...
void BinaryWriter::_write_chunk(const Table &table, std::ofstream &ofstream,
                                const ChunkID &chunk_id)
{
    const auto chunk = table.get_chunk(chunk_id);
    Assert(chunk, "Physically deleted chunk should not reach this point, see "
                  "get_chunk / #1686.");
    ...
    for (auto column_id = ColumnID{0}; column_id < column_count; ++column_id)
    {
        resolve_data_and_segment_type(
            *chunk->get_segment(column_id),
            ...
            {
                _write_segment(resolved_segment,
                               table.column_is_nullable(column_id), ofstream);
            });
    }
}

template <typename T>
void BinaryWriter::_write_segment(const ValueSegment<T> &value_segment,
...
void BinaryWriter::_write_segment(const ReferenceSegment &reference_segment,
                                  bool column_is_nullable,
                                  std::ofstream &ofstream)
{
    ...
    resolve_data_type(
        ...
        [&](auto type)
        {
            ...
            segment_iterate<SegmentDataType>(
                reference_segment,
                [&](const auto &position)
                {
                    if (position.is_null())
...
template <typename T>
void BinaryWriter::_write_segment(
    const DictionarySegment<T> &dictionary_segment, bool /*column_is_nullable*/,
    std::ofstream &ofstream)
{
    ...
    bool /*column_is_nullable*/, std::ofstream &ofstream)
{
    ...
    // Write attribute vector compression id
    const auto compressed_vector_type_id =
        _compressed_vector_type_id<T>(fixed_string_dictionary_segment);
    export_value(ofstream, compressed_vector_type_id);

    // Write the dictionary size, string length and dictionary
    const auto dictionary_size =
        fixed_string_dictionary_segment.fixed_string_dictionary()->size();
    const auto string_length =
        fixed_string_dictionary_segment.fixed_string_dictionary()
            ->string_length();
    ...
    export_values(ofstream,
                  *fixed_string_dictionary_segment.fixed_string_dictionary());

    // Write attribute vector
    _export_compressed_vector(
        ofstream, *fixed_string_dictionary_segment.compressed_vector_type(),
        *fixed_string_dictionary_segment.attribute_vector());
}

template <typename T>
void BinaryWriter::_write_segment(const RunLengthSegment<T> &run_length_segment,
                                  bool /*column_is_nullable*/,
                                  std::ofstream &ofstream)
{
    ...
    export_value(ofstream,
                 static_cast<uint32_t>(run_length_segment.values()->size()));
...
template <>
void BinaryWriter::_write_segment(
    const FrameOfReferenceSegment<int32_t> &frame_of_reference_segment,
    bool /*column_is_nullable*/, std::ofstream &ofstream)
{
    ...
    // Write attribute vector compression id
    const auto compressed_vector_type_id =
        _compressed_vector_type_id<int32_t>(frame_of_reference_segment);
    export_value(ofstream, compressed_vector_type_id);

    // Write number of blocks and block minima
    export_value(ofstream,
                 static_cast<uint32_t>(
                     frame_of_reference_segment.block_minima().size()));
    export_values(ofstream, frame_of_reference_segment.block_minima());

    // Write flag if optional NULL value vector is written
    export_value(ofstream,
                 static_cast<BoolAsByteType>(
                     frame_of_reference_segment.null_values().has_value()));
    ...
    // Write offset values
    _export_compressed_vector(
        ofstream, *frame_of_reference_segment.compressed_vector_type(),
        frame_of_reference_segment.offset_values());
}

template <typename T>
void BinaryWriter::_write_segment(const LZ4Segment<T> &lz4_segment,
                                  bool /*column_is_nullable*/,
                                  std::ofstream &ofstream)
{
    ...
    export_value(ofstream,
                 static_cast<uint32_t>(lz4_segment.lz4_blocks().size()));
    ...
    export_value(ofstream,
                 static_cast<uint32_t>(lz4_segment.last_block_size()));
    ...
    if (lz4_segment.null_values())
    {
        ...
        export_value(ofstream,
                     static_cast<uint32_t>(lz4_segment.null_values()->size()));
        // Write NULL values
    ...
    export_value(ofstream,
                 static_cast<uint32_t>(lz4_segment.dictionary().size()));
    ...
    if (lz4_segment.string_offsets())
    {
        ...
        export_value(ofstream, static_cast<uint32_t>(
                                   lz4_segment.string_offsets()->size()));
        // Write string_offset data_size
        export_compact_vector(ofstream, dynamic_cast<const BitPackingVector &>(
                                            *lz4_segment.string_offsets())
                                            .data());
    }
    else
    {
    ...
    const AbstractEncodedSegment &abstract_encoded_segment)
{
    ...
    resolve_encoded_segment_type<T>(
        abstract_encoded_segment,
        [&compressed_vector_type_id](auto &typed_segment)
        {
            const auto compressed_vector_type =
                typed_segment.compressed_vector_type();
            Assert(compressed_vector_type,
                   "Expected Segment to use vector compression");
            ...
            case CompressedVectorType::BitPacking:
                compressed_vector_type_id =
                    static_cast<uint8_t>(*compressed_vector_type);
                break;
            default:
                Fail("Export of specified CompressedVectorType is not yet "
                     "supported");
            }
        });
...
void BinaryWriter::_export_compressed_vector(
    std::ofstream &ofstream, const CompressedVectorType type,
    const BaseCompressedVector &compressed_vector)
{
    ...
    case CompressedVectorType::FixedWidthInteger4Byte:
        export_values(ofstream,
                      dynamic_cast<const FixedWidthIntegerVector<uint32_t> &>(
                          compressed_vector)
                          .data());
        return;
    case CompressedVectorType::FixedWidthInteger2Byte:
        export_values(ofstream,
                      dynamic_cast<const FixedWidthIntegerVector<uint16_t> &>(
                          compressed_vector)
                          .data());
        return;
    case CompressedVectorType::FixedWidthInteger1Byte:
        export_values(ofstream,
                      dynamic_cast<const FixedWidthIntegerVector<uint8_t> &>(
                          compressed_vector)
                          .data());
        return;
    case CompressedVectorType::BitPacking:
        export_compact_vector(
            ofstream,
            dynamic_cast<const BitPackingVector &>(compressed_vector).data());
        return;
    default:
