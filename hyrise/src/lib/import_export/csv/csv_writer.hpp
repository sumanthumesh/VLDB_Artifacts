...
class CsvWriter
{
  public:
     ...
     *
     * During this process, two files are created: <table_name>.csv and
     * <table_name>.csv.meta Currently, they are both csv files with a comma (,)
     * as delimiter and a quotation mark (") as quotation mark. As escape
     * character, also a quotation mark is used ("). This definition is in line
     * with RFC 4180
     *
     *
  ...
  protected:
    static void _generate_meta_info_file(const Table &table,
                                         const std::string &filename);
    static void _generate_content_file(const Table &table,
                                       const std::string &filename,
                                       const ParseConfig &config);
    static void _write(const AllTypeVariant &value, std::ofstream &ofstream,
                       const ParseConfig &config);
    static pmr_string _escape(const pmr_string &string,
                              const ParseConfig &config);
    static void _write_string_value(const pmr_string &value,
