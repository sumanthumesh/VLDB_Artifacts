...
void assign_if_exists(char &value, const nlohmann::json &json_object,
                      const std::string &key)
{
    if (json_object.find(key) != json_object.end())
    {
        ...
        Assert(character.size() == 1,
               "CSV meta file config: Character specifications can only be a "
               "single character.");
        value = character[0];
    }
}

void assign_if_exists(bool &value, const nlohmann::json &json_object,
                      const std::string &key)
{
    if (json_object.find(key) != json_object.end())
    {
...
void from_json(const nlohmann::json &json_string, NullHandling &null_handling)
{
    ...
    {
        Fail("Illegal value for null_handling: " +
...
{
    // Apply only parts of the ParseConfig that are provided, use default values
    // otherwise
    ParseConfig config{};
    if (json.find("config") != json.end())
    {
        Assert(json.at("config").is_object(),
               "CSV meta file,\"Config\" field has to be a json object.");
        ...
        assign_if_exists(config.delimiter_escape, config_json,
                         "delimiter_escape");
        assign_if_exists(config.reject_quoted_nonstrings, config_json,
                         "reject_quoted_nonstrings");
        ...
        if (config_json.find("null_handling") != config_json.end())
        {
            config.null_handling =
                config_json["null_handling"].get<NullHandling>();
        }
    }

    if (json.find("columns") != json.end())
    {
        Assert(json.at("columns").is_array(),
               "CSV meta file,\"Columns\" field has to be a json array.");
...
void to_json(nlohmann::json &json, const CsvMeta &meta)
{
    ...
    for (const auto &column_meta : meta.columns)
    {
        columns.emplace_back(
            nlohmann::json{{"name", column_meta.name},
...
{
    return std::tie(left.name, left.type, left.nullable) ==
           std::tie(right.name, right.type, right.nullable);
}

bool operator==(const ParseConfig &left, const ParseConfig &right)
{
    return std::tie(left.delimiter, left.separator, left.quote, left.escape,
                    left.delimiter_escape, left.reject_quoted_nonstrings,
                    left.null_handling, left.rfc_mode) ==
           std::tie(right.delimiter, right.separator, right.quote, right.escape,
                    right.delimiter_escape, right.reject_quoted_nonstrings,
                    right.null_handling, right.rfc_mode);
}
