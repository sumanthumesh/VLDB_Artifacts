...
void BaseCsvConverter::unescape(std::string &field, const ParseConfig &config)
{
    ...
    // 'escaped' holds the information whether the previous character was the
    // config.escape character
    bool escaped = false;
    // The start and end ranges leave out the surrounding quotes.
    std::copy_if(field.begin() + 1, field.end() - 1,
                 std::back_inserter(unescaped_string),
...
std::string BaseCsvConverter::unescape_copy(const std::string &field,
