                                        ...
                                        const std::optional<CsvMeta> &csv_meta)
{
    ...
    auto escaped_linebreak = std::string(1, meta.config.delimiter_escape) +
                             std::string(1, meta.config.delimiter);
    ...
    if (!csvfile || csvfile.peek() == EOF || csvfile.peek() == '\r' ||
        csvfile.peek() == '\n')
    {
        return table;
    }
    ...
    /**
     * Load the whole file(!) into a std::string using the, hopefully, fastest
     * method to do so.
     * TODO(anybody) Maybe use mmap() in the future. The current approach needs
     * to have the entire file in RAM, mmap might be cleverer, dunno.
     */
    ...
    while (_find_fields_in_chunk(content_view, *table, field_ends, meta))
    {
        ...
        // Only pass the part of the string that is actually needed to the
        // parsing task
        auto relevant_content =
            std::string_view{content_view.substr(0, field_ends.back())};
        ...
        tasks.emplace_back(std::make_shared<JobTask>(
            [relevant_content, field_ends, &table, &segments, &meta,
             &escaped_linebreak, &append_chunk_mutex]()
            {
                _parse_into_chunk(relevant_content, field_ends, *table,
                                  segments, meta, escaped_linebreak,
                                  append_chunk_mutex);
            }));
        tasks.back()->schedule();
    }
    ...
    {
        DebugAssert(!segments.empty(),
                    "Empty chunks shouldn't occur when importing CSV");
...
CsvParser::create_table_from_meta_file(const std::string &filename,
                                       const ChunkOffset chunk_size)
{
...
std::shared_ptr<Table>
CsvParser::_create_table_from_meta(const ChunkOffset chunk_size,
                                   const CsvMeta &meta)
{
    ...
    for (const auto &column_meta : meta.columns)
    {
        ...
        column_definitions.emplace_back(column_name, data_type,
...
bool CsvParser::_find_fields_in_chunk(std::string_view csv_content,
                                      const Table &table,
                                      ...
                                      const CsvMeta &meta)
{
    ...
    auto search_for = std::string{meta.config.separator, meta.config.delimiter,
                                  meta.config.quote};
    ...
    while (rows < table.target_chunk_size())
    {
        ...
        // Make sure to "toggle" in_quotes ONLY if the quotes are not part of
        // the string (i.e. escaped)
        if (elem == meta.config.quote)
        {
            ...
            {
                quote_is_escaped =
                    pos != 0 && csv_content[pos - 1] == meta.config.escape;
            }
            if (!quote_is_escaped)
            {
                in_quotes = !in_quotes;
        ...
        // Determine if delimiter marks end of row or is part of the (string)
        // value
        ...
        // Determine if separator marks end of field or is part of the (string)
        // value
...
size_t CsvParser::_parse_into_chunk(std::string_view csv_chunk,
                                    const std::vector<size_t> &field_ends,
                                    ...
                                    std::mutex &append_chunk_mutex)
{
    ...
    const auto row_count = ChunkOffset{
        static_cast<ChunkOffset::base_type>(field_ends.size() / column_count)};
        ...
        resolve_data_type(
            ...
            [&](const auto type)
            {
                ...
                converters.emplace_back(
                    std::make_unique<CsvConverter<ColumnDataType>>(
                        row_count, meta.config, is_nullable));
            });
    }

    Assert(field_ends.size() == static_cast<size_t>(row_count) * column_count,
    ...
    try
    {
        ...
        {
            for (column_id = ColumnID{0}; column_id < column_count;
                 ++column_id, ++field_idx)
            {
                ...
                {
                    // CSV fields not following RFC 4810 might need some
                    // preprocessing
                    _sanitize_field(field, meta, escaped_linebreak);
                }

                converters[column_id]->insert(field,
                                              static_cast<ChunkOffset>(row_id));
            }
        }
    }
    catch (const std::exception &exception)
    {
        throw std::logic_error(
            "Exception while parsing CSV, row " + std::to_string(row_id) +
...
void CsvParser::_sanitize_field(std::string &field, const CsvMeta &meta,
                                const std::string &escaped_linebreak)
{
