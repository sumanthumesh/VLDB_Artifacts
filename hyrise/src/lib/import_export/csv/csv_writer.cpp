...
void CsvWriter::_generate_meta_info_file(const Table &table,
...
void CsvWriter::_generate_content_file(const Table &table,
                                       const std::string &filename,
                                       const ParseConfig &config)
{
    /**
     ...
     * The advantages are that it is very straight forward to implement for any
     * segment type as it does not care about representation of values. Also,
     * probably, the main reason for this, it makes is very easy to load this
     * data into a different database. The disadvantage is that it can be quite
     * slow if the data has been compressed before. Also, it does not involve
     * the column-oriented style used in Hyrise.
     */
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
             ...
             ++chunk_offset)
        {
                 ...
                 ++column_id)
            {
                ...
                // The previous implementation did a double dispatch (at least
                // two virtual method calls) So the subscript operator cannot be
                // much slower.
...
void CsvWriter::_write(const AllTypeVariant &value, std::ofstream &ofstream,
                       const ParseConfig &config)
{
                              ...
                              const ParseConfig &config)
{
    ...
    while (std::string::npos !=
           (next_pos = result.find(config.quote, next_pos)))
    {
        result.insert(next_pos, 1, config.escape);
        // Has to jump 2 positions ahead because a new character had been
        // inserted.
        next_pos += 2;
    }
    return result;
