...
// Dedicated buffer for read operations. In contrast to the WriteBuffer integer
// types are converted from network to host byte order and data will only be
// read from the network device.
template <typename SocketType> class ReadBuffer
{
  public:
    ...
    // Extract numerical values from buffer. Values will be converted into the
    // correct byte order if type equals [u]int[16|32]_t.
    template <typename T> T get_value()
    {
        _receive_if_necessary(sizeof(T));
        T network_value = 0;
        std::copy_n(_start_position, sizeof(T),
  ...
  private:
    ...
    // This iterator points to the field after the last unread element of the
    // array.
