...
template <typename SocketType> size_t WriteBuffer<SocketType>::size() const
{
...
template <typename SocketType> bool WriteBuffer<SocketType>::full() const
{
    return size() == maximum_capacity();
}

template <typename SocketType>
void WriteBuffer<SocketType>::put_string(
    const std::string &value, const HasNullTerminator has_null_terminator)
{
    ...
    if (!full())
    {
        position_in_string = static_cast<uint32_t>(
            std::min(maximum_capacity() - size(), value.size()));
        std::copy_n(value.cbegin(), position_in_string, _current_position);
        std::advance(_current_position, position_in_string);
    }

    // Write to network device until string is complete. Ignore last character
    // since it is \0
    while (position_in_string < value.size())
    {
        const auto bytes_to_transfer =
            std::min(maximum_capacity(), value.size() - position_in_string);
        _flush_if_necessary(bytes_to_transfer);
        std::copy_n(value.cbegin() + position_in_string, bytes_to_transfer,
                    _current_position);
        ...
        bytes_sent = boost::asio::write(
            *_socket,
            std::array<boost::asio::mutable_buffer, 2>{
                boost::asio::buffer(
                    &*_start_position,
                    std::distance(&*_start_position, _data.end())),
                boost::asio::buffer(
                    _data.begin(),
                    std::distance(_data.begin(), &*_current_position))},
            boost::asio::transfer_at_least(bytes_to_send), error_code);
    }
    else
    {
        bytes_sent = boost::asio::write(
            *_socket, boost::asio::buffer(&*_start_position, size()),
            boost::asio::transfer_at_least(bytes_to_send), error_code);
    }

    // Socket was closed by client during execution
    if (error_code == boost::asio::error::broken_pipe ||
        error_code == boost::asio::error::connection_reset || bytes_sent == 0)
    {
        throw ClientDisconnectException(
            "Write operation failed. Client closed connection.");
    }
