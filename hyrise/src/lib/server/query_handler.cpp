...
std::pair<ExecutionInformation, std::shared_ptr<TransactionContext>>
QueryHandler::execute_pipeline(
    ...
    const std::shared_ptr<TransactionContext> &transaction_context)
{
    ...
    // See:
    // https://postgresql.org/docs/12/protocol-flow.html#PROTOCOL-FLOW-EXT-QUERY
    ...
    DebugAssert(!transaction_context || !transaction_context->is_auto_commit(),
                "Auto-commit transaction contexts should not be passed around "
                "this far");
    ...
    if (pipeline_status == SQLPipelineStatus::Success)
    {
        ...
        execution_info.root_operator_type =
            sql_pipeline.get_physical_plans().back()->type();
    ...
    {
        const std::string failed_statement =
            sql_pipeline.failed_pipeline_statement()->get_sql_string();
        execution_info.error_messages = {
            {PostgresMessageType::HumanReadableError,
             "Transaction conflict, transaction was rolled back. Following "
             "statements might "
...
void QueryHandler::setup_prepared_plan(const std::string &statement_name,
                                       const std::string &query)
{
    // Named prepared statements must be explicitly closed before they can be
    // redefined by another Parse message. An unnamed prepared statement lasts
    // only until the next Parse statement specifying the unnamed statement as
    ...
    if (Hyrise::get().storage_manager.has_prepared_plan(statement_name))
    {
        AssertInput(statement_name.empty(),
                    "Named prepared statements must be explicitly closed "
    ...
    // The PostgreSQL communication protocol does not allow more than one
    // prepared statement within the parse message. See note at:
    // https://www.postgresql.org/docs/12/protocol-flow.html#PROTOCOL-FLOW-EXT-QUERY
    AssertInput(lqps.size() == 1u,
                "Only a single statement allowed in prepared statement");
    ...
    auto parameter_ids_of_value_placeholders =
        translation_info.parameter_ids_of_value_placeholders;
    const auto prepared_plan = std::make_shared<PreparedPlan>(
        lqp, parameter_ids_of_value_placeholders);

    Hyrise::get().storage_manager.add_prepared_plan(statement_name,
                                                    prepared_plan);
}
...
{
    AssertInput(Hyrise::get().storage_manager.has_prepared_plan(
                    statement_details.statement_name),
                "The specified statement does not exist.");

    const auto prepared_plan = Hyrise::get().storage_manager.get_prepared_plan(
        statement_details.statement_name);

    const auto parameter_count = statement_details.parameters.size();
    auto parameter_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>{parameter_count};
    for (auto parameter_idx = size_t{0}; parameter_idx < parameter_count;
         ++parameter_idx)
    {
        parameter_expressions[parameter_idx] =
            std::make_shared<ValueExpression>(
                statement_details.parameters[parameter_idx]);
    }
    ...
    const std::shared_ptr<AbstractOperator> &physical_plan)
{
    const auto &[tasks, root_operator_task] =
        OperatorTask::make_tasks_from_operator(physical_plan);
    Hyrise::get().scheduler()->schedule_and_wait_for_tasks(tasks);
    return root_operator_task->get_operator()->get_output();
}

void QueryHandler::_handle_transaction_statement_message(
    ExecutionInformation &execution_info, SQLPipeline &sql_pipeline)
{
    // handle custom user feedback (command complete messages) for transaction
    // statements
    auto sql_statement = sql_pipeline.get_parsed_sql_statements().back();
    const auto &statements = sql_statement->getStatements();

    DebugAssert(statements.size() == 1,
                "SQL statements were not properly split");
    if (statements[0]->isType(hsql::StatementType::kStmtTransaction))
    {
        const auto &transaction_statement =
            dynamic_cast<hsql::TransactionStatement &>(*statements.front());
