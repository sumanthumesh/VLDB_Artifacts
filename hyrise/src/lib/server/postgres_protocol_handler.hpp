...
// This struct stores a prepared statement's name, its portal used and the
...
// This class extracts information from client messages and serializes the
// response data according to the PostgreSQL Wire Protocol.
template <typename SocketType> class PostgresProtocolHandler
{
  public:
    ...
    // Send query result
    void send_row_description_header(const uint32_t total_column_name_length,
                                     const uint16_t column_count);
    void send_row_description(const std::string &column_name,
    ...
    // Send error message to client if there is an error during parsing or
    // execution
    void send_error_message(const ErrorMessages &error_messages);

    // Additional (optional) message containing execution times of different
    // components (such as translator or optimizer)
    void send_execution_info(const std::string &execution_information);

    // This method is required for testing. Otherwise we cannot make the
    // protocol handler flush its data.
