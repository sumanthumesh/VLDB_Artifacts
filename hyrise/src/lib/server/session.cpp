...
Session::Session(boost::asio::io_service &io_service,
    ...
    : _socket(std::make_shared<Socket>(io_service)),
      _postgres_protocol_handler(
          std::make_shared<PostgresProtocolHandler<Socket>>(_socket)),
...
void Session::run()
{
    // Set TCP_NODELAY in order to disable Nagle's algorithm. It handles
    // congestion control in TCP networks. Therefore, small packets are buffered
    // and sent out later as one large packet. This might introduce a delay of
    // up to 40 ms which we have to avoid. Further reading:
    ...
    while (!_terminate_session)
    {
        ...
        {
            std::cerr << "Exception in session with client port "
                      << _socket->remote_endpoint().port() << ":" << std::endl
                      << e.what() << std::endl;
            const auto error_messages = ErrorMessages{
                {PostgresMessageType::HumanReadableError, e.what()}};
            _postgres_protocol_handler->send_error_message(error_messages);
            _postgres_protocol_handler->send_ready_for_query();
            // In case of an error, an error message has to be send to the
            // client followed by a "ReadyForQuery" message. Messages that have
            // already been received are processed further. A "sync" message
            // makes the server send another "ReadyForQuery" message. In order
            // to avoid this, we set this flag for further operations. As soon
            // as a new query arrives it must be set to false again to ensure
...
void Session::_establish_connection()
{
    const auto body_length =
        _postgres_protocol_handler->read_startup_packet_header();
...
void Session::_handle_request()
{
    ...
    {
        // The contents of this packet are not used for further processing. The
        // actual "describe" happens after executing the PQP.
...
void Session::_handle_simple_query()
{
    ...
    std::tie(execution_information, _transaction_context) =
        QueryHandler::execute_pipeline(query, _send_execution_info,
                                       _transaction_context);

    if (!execution_information.error_messages.empty())
    {
        _postgres_protocol_handler->send_error_message(
            execution_information.error_messages);
    }
    else
    {
        uint64_t row_count = 0;
        // If there is no result table, e.g. after an INSERT command, we cannot
        // send row data. Otherwise, the result table of the last statement will
        // be send back.
        if (execution_information.result_table)
        {
            ResultSerializer::send_table_description(
                execution_information.result_table, _postgres_protocol_handler);
            ResultSerializer::send_query_response(
                execution_information.result_table, _postgres_protocol_handler);
            row_count = execution_information.result_table->row_count();
        }
        if (_send_execution_info == SendExecutionInfo::Yes)
        {
            _postgres_protocol_handler->send_execution_info(
                execution_information.pipeline_metrics);
        }
        _postgres_protocol_handler->send_command_complete(
            ResultSerializer::build_command_complete_message(
                execution_information, row_count));
    }
...
{
    const auto [statement_name, query] =
        _postgres_protocol_handler->read_parse_packet();
    QueryHandler::setup_prepared_plan(statement_name, query);

    _postgres_protocol_handler->send_status_message(
        PostgresMessageType::ParseComplete);
...
void Session::_handle_bind_command()
{
    ...
    {
        AssertInput(parameters.portal.empty(),
                    "Named portals must be explicitly closed before they can "
                    "be redefined.");
        _portals.erase(portal_it);
    }

    // Since bind and execute packet usually arrive together, we still have to
    // handle the execute packet. Therefore, we first store a nullptr in the
    // portals map to signalize an error. However, if binding succeeds in the
    // next step this nullptr gets replaced by the correct pqp. Before executing
    // the prepared statement we make a check for errors.
    ...
    _postgres_protocol_handler->send_status_message(
        PostgresMessageType::BindComplete);
...
void Session::_handle_execute()
{
    const std::string &portal_name =
        _postgres_protocol_handler->read_execute_packet();

    auto portal_it = _portals.find(portal_name);
    AssertInput(portal_it != _portals.end(),
                "The specified portal does not exist.");

    // In case of an error occured during binding there is no pqp available.
    // Hence, early return here since there is nothing to execute.
    if (!portal_it->second)
    {
        _portals.erase(portal_it);
        return;
    ...
    const auto result_table =
        QueryHandler::execute_prepared_plan(physical_plan);
    ...
    if (result_table)
    {
        ResultSerializer::send_table_description(result_table,
                                                 _postgres_protocol_handler);
        ResultSerializer::send_query_response(result_table,
                                              _postgres_protocol_handler);
    ...
    {
        _postgres_protocol_handler->send_status_message(
            PostgresMessageType::NoDataResponse);
    }

    _postgres_protocol_handler->send_command_complete(
        ResultSerializer::build_command_complete_message(physical_plan->type(),
                                                         row_count));
    // Ready for query + flush will be done after reading sync message
}
} // namespace hyrise
