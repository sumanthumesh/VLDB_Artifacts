...
// Dedicated buffer for write operations. It flushes itself automatically if a
// new value does not fit into the available memory. Strings inserted use the
// remaining space first. The buffer can also be force flushed. In contrast to
// the ReadBuffer integer types are converted from host to network byte order
// and data will only be written to the network device.
template <typename SocketType> class WriteBuffer
{
  public:
    explicit WriteBuffer(const std::shared_ptr<SocketType> socket)
        : _socket(socket)
    {
    }
    ...
    // Put numerical values into the buffer. Values will be converted into the
    // correct byte order if necessary.
    template <typename T> void put_value(const T network_value)
    {
        ...
        std::copy_n(reinterpret_cast<const char *>(&converted_value), sizeof(T),
                    _current_position);
        std::advance(_current_position, sizeof(T));
    }

    // Put string into the buffer. If the string is longer than the buffer
    // itself the buffer will flush automatically.
    void put_string(
  ...
  private:
    ...
    // This iterator points to the field after the last unflushed element of the
    // array.
