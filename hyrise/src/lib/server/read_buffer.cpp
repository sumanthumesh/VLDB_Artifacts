...
template <typename SocketType> size_t ReadBuffer<SocketType>::size() const
{
    const auto current_size =
        std::distance(&*_start_position, &*_current_position);
...
template <typename SocketType> bool ReadBuffer<SocketType>::full() const
{
    return size() == maximum_capacity();
}

template <typename SocketType> std::string ReadBuffer<SocketType>::get_string()
{
    ...
    while (*string_end != '\0')
    {
        // We do not know how long this string is going to be. Hence, receive at
        // least 1 char.
...
template <typename SocketType>
std::string
ReadBuffer<SocketType>::get_string(const size_t string_length,
                                   const HasNullTerminator has_null_terminator)
{
    ...
    if (size() != 0)
    {
        std::copy_n(_start_position, std::min(string_length, size()),
                    std::back_inserter(result));
        std::advance(_start_position, result.size());
    }

    // Read from network device until string is complete or skip if string is
    // already complete.
    while (result.size() < string_length)
    {
        const auto substring_length =
            std::min(string_length - result.size(), maximum_capacity());
        _receive_if_necessary(substring_length);
        std::copy_n(_start_position, substring_length,
                    std::back_inserter(result));
    ...
    if (has_null_terminator == HasNullTerminator::Yes)
    {
        Assert(result.back() == '\0',
               "Last character is not a null terminator");
        result.pop_back();
    }
...
void ReadBuffer<SocketType>::_receive_if_necessary(const size_t bytes_required)
{
    ...
    // We cannot forward an iterator to the read system call. Hence, we need to
    // use raw pointers. Therefore, we need to distinguish between reading into
    // continuous memory or partially read the data.
    if (std::distance(&*_start_position, &*_current_position) < 0 ||
        &*_start_position == _data.data())
    {
        bytes_read = boost::asio::read(
            *_socket,
            boost::asio::buffer(&*_current_position, maximum_readable_size),
        ...
        bytes_read = boost::asio::read(
            *_socket,
            std::array<boost::asio::mutable_buffer, 2>{
                boost::asio::buffer(
                    &*_current_position,
                    std::distance(&*_current_position, _data.end())),
                boost::asio::buffer(
                    _data.begin(),
                    std::distance(_data.begin(), &*_start_position - 1))},
            boost::asio::transfer_at_least(bytes_required - size()),
            error_code);
    }
    ...
    if (error_code == boost::asio::error::broken_pipe ||
        error_code == boost::asio::error::connection_reset || bytes_read == 0)
    {
        throw ClientDisconnectException(
            "Read operation failed. Client closed connection.");
    }
