...
template <class From, class To> struct is_explicitly_convertible
{
    enum
    {
        value = std::is_constructible_v<To, From> &&
...
template <typename T, typename = void> struct is_lex_castable : std::false_type
{
};

template <typename T>
struct is_lex_castable<T, decltype(void(std::declval<std::ostream &>()
...
std::enable_if_t<std::is_convertible_v<L, R> && std::is_convertible_v<R, L>,
                 bool>
value_equal(L l, R r)
{
    return l == r;
...
template <typename L, typename R>
std::enable_if_t<std::is_arithmetic_v<L> && is_lex_castable_v<R> &&
...
template <typename L, typename R>
std::enable_if_t<std::is_arithmetic_v<R> && is_lex_castable_v<L> &&
...
// Function that calls a given functor with the correct std comparator. The
// light version is not instantiated for > and >=, reducing the number of
// instantiated templates by a third.
template <typename Functor>
void with_comparator_light(const PredicateCondition predicate_condition,
                           const Functor &func)
{
...
// Function that calls a functor with a functor that decides whether a value
// matches a Between-PredicateCondition. This function cannot be integrated into
// with_comparator, because the created function takes 3 instead of 2
// parameters.
template <typename Functor>
void with_between_comparator(const PredicateCondition predicate_condition,
                             const Functor &func)
{
    switch (predicate_condition)
    {
    case PredicateCondition::BetweenInclusive:
        return func([](const auto &value, const auto &lower_value,
...
template <typename Functor>
void with_comparator(const PredicateCondition predicate_condition,
                     const Functor &func)
{
    switch (predicate_condition)
    {
