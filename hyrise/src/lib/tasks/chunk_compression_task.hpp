 ...
 * it does not touch the segments. However, inserting records while
 * simultaneously compressing the chunk leads to inconsistent state. Therefore
 * only chunks where all insertion has been completed may be compressed. In
 * other words, they need to be full and all of their end-cids must be smaller
 * than infinity. This task calls those chunks “completed”.
 *
 * Note: Reference segments are not invalidated by this task because the order
 * in which records are stored does not change.
 */
class ChunkCompressionTask : public AbstractTask
{
  public:
    explicit ChunkCompressionTask(const std::string &table_name,
  ...
  private:
     ...
     */
    static bool _chunk_is_completed(const std::shared_ptr<Chunk> &chunk,
                                    const uint32_t target_chunk_size);
