...
TEST_F(SQLiteAddIndicesTest, AddIndexTest)
{
    ...
    EXPECT_THROW(sqlite_wrapper->main_connection.raw_execute_query(
                     "DROP INDEX index_1;"),
                 std::logic_error);
    auto sqlite_table =
        sqlite_wrapper->main_connection.execute_query("SELECT * FROM table_1");
    EXPECT_TABLE_EQ_ORDERED(stored_table, sqlite_table);
    const auto schema_file_path =
        "resources/test_data/sqlite_add_index_schema.sql";
    const auto create_index_file_path =
        "resources/test_data/sqlite_add_index_create_index.sql";
    add_indices_to_sqlite(schema_file_path, create_index_file_path,
                          sqlite_wrapper);
    sqlite_table =
        sqlite_wrapper->main_connection.execute_query("SELECT * FROM table_1");
    EXPECT_TABLE_EQ_ORDERED(stored_table, sqlite_table);
