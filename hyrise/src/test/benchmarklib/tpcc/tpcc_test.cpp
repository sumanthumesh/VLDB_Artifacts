  ...
  public:
    static void SetUpTestCase()
    {
        auto benchmark_config = std::make_shared<BenchmarkConfig>(
            BenchmarkConfig::get_default_config());
        auto table_generator =
            TPCCTableGenerator{NUM_WAREHOUSES, benchmark_config};
    ...
    void SetUp() override
    {
        for (const auto &[table_name, table_info] : tables)
        {
            ...
            auto isolated_table = std::make_shared<Table>(
                generated_table->column_definitions(), TableType::Data,
                std::nullopt, UseMvcc::Yes);
            Hyrise::get().storage_manager.add_table(table_name, isolated_table);

            auto table_wrapper =
                std::make_shared<TableWrapper>(generated_table);
    ...
    std::shared_ptr<const Table> get_validated(
    ...
    void
    verify_table_sizes(const std::unordered_map<std::string, size_t> &sizes)
    {
        for (const auto &[table_name, size] : sizes)
        {
            ...
            {
                // For ORDER_LINE, the number of lines per order is in [5, 15]
                // (see 4.3.3.1) and non-deterministic. If it is not
                // specifically calculated, we just check if it is within the
                // acceptable range.
                EXPECT_GE(table->row_count(), sizes.at("ORDER") * 5)
                    << "Failed table: " << table_name;
                EXPECT_LE(table->row_count(), sizes.at("ORDER") * 15)
                    << "Failed table: " << table_name;
            }
            else
            {
    ...
    const std::unordered_map<std::string, size_t> initial_sizes{
...
{
    // As the procedures have some internal logic that we do not want to
    // replicate in the tests (e.g., picking a W_ID), we first create a
    // transaction that has a view on the unmodified database, then execute the
    // procedure, and finally compare the changes between the final state of the
    // database to that seen by the initial transaction. Those changes should
    // reflect what we expect the procedure to have done.
    ...
    // These tests assume that updates / insertions into a table are done in the
    // same physical order as they were in the original table
    for (auto d_id = 1; d_id <= 10; ++d_id)
    {
        ...
        {
            // We have #ORDER (3000) - #NEW_ORDER (900) delivered orders per
            // warehouse, so 2101 is the first undelivered ID Fetch
            // num_order_lines of first undelivered order
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"SELECT * FROM ORDER_LINE WHERE OL_W_ID = "} +
                    std::to_string(delivery.w_id) + " AND OL_D_ID = " +
                    std::to_string(d_id) + " AND OL_O_ID = 2101"}
        ...
        // Fetch old c_balance and c_delivery_count as it is seen by
        // old_transaction_context
        {
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"SELECT * FROM CUSTOMER WHERE C_W_ID = "} +
                    std::to_string(delivery.w_id) +
                    " AND C_D_ID = " + std::to_string(d_id) +
                    " AND C_ID = (SELECT O_C_ID FROM \"ORDER\" WHERE O_W_ID "
                    "= " +
                    std::to_string(delivery.w_id) + " AND O_D_ID = " +
                    std::to_string(d_id) + " AND O_ID = 2101)"}
                    .with_transaction_context(old_transaction_context)
                    .create_pipeline();
            const auto [_, table] = pipeline.get_result_table();
            EXPECT_TRUE(table);
            ...
            old_c_delivery_cnt =
                *table->get_value<int32_t>("C_DELIVERY_CNT", 0);
        }
        ...
        // Verify that the order was deleted from the NEW_ORDER table:
        {
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"SELECT * FROM NEW_ORDER WHERE NO_W_ID = "} +
                    std::to_string(delivery.w_id) + " AND NO_D_ID = " +
                    std::to_string(d_id) + " AND NO_O_ID = 2101"}
                    .create_pipeline();
            const auto [_, table] = pipeline.get_result_table();
            EXPECT_TRUE(table);
        ...
        // Verify that O_CARRIER_ID was updated in the ORDER table - not
        // filtering by the district ID keeps it interesting:
        {
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"SELECT * FROM \"ORDER\" WHERE O_W_ID = "} +
                    std::to_string(delivery.w_id) + " AND O_ID = 2101"}
                    .create_pipeline();
            const auto [_, table] = pipeline.get_result_table();
            EXPECT_TRUE(table);
            // Ten rows because each district gets 3000 orders at startup
            // (compare 4.3.3.1)
            EXPECT_EQ(table->row_count(), 10);
            EXPECT_EQ(*table->get_value<int32_t>("O_CARRIER_ID", d_id - 1),
        ...
        // Check for delivery dates being set in ORDER_LINE:
        {
            ...
            for (auto ol_number = uint64_t{1}; ol_number <= num_order_lines;
                 ++ol_number)
            {
                EXPECT_EQ(
                    *table->get_value<int32_t>("OL_NUMBER", ol_number - 1),
                    ol_number);
                EXPECT_GE(
                    *table->get_value<int32_t>("OL_DELIVERY_D", ol_number - 1),
                    old_time);

                // Add the amount of the order, which was not seen by
                // old_transaction_context above, to expected_c_balance
                expected_c_balance +=
                    *table->get_value<float>(ColumnID{8}, ol_number - 1);
            }
        }
        ...
        // Check for updated customer balance and delivery count in CUSTOMER:
        {
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"SELECT C_BALANCE, C_DELIVERY_CNT FROM "
            ...
            EXPECT_TRUE(table);
            EXPECT_EQ(table->row_count(), 1);
            EXPECT_FLOAT_EQ(*table->get_value<float>("C_BALANCE", 0),
                            expected_c_balance);
            EXPECT_EQ(*table->get_value<int32_t>("C_DELIVERY_CNT", 0),
                      old_c_delivery_cnt + 1);
        }
    }
}
...
TEST_F(TPCCTest, NewOrder)
{
    auto old_transaction_context =
        Hyrise::get().transaction_manager.new_transaction_context(
            AutoCommit::No);
    const auto old_order_line_size = static_cast<int>(
        Hyrise::get().storage_manager.get_table("ORDER_LINE")->row_count());
    const auto old_time = time(nullptr);

    BenchmarkSQLExecutor sql_executor{nullptr, std::nullopt};
    auto new_order = TPCCNewOrder{NUM_WAREHOUSES, sql_executor};
    // Generate random NewOrders until we have one without unused item IDs and
    // where both local and remote order lines occur
    while (
        new_order.order_lines.back().ol_i_id == TPCCNewOrder::UNUSED_ITEM_ID ||
        std::find_if(new_order.order_lines.begin(), new_order.order_lines.end(),
                     [&](const auto &order_line) {
...

    {
        ...
        EXPECT_TRUE(table);
        EXPECT_EQ(table->row_count(), 1);
        EXPECT_EQ(*table->get_value<int32_t>("D_NEXT_O_ID", 0),
                  old_d_next_o_id + 1);
    }
    ...
    const auto new_order_row = Hyrise::get()
                                   .storage_manager.get_table("NEW_ORDER")
                                   ->get_row(new_sizes["NEW_ORDER"] - 1);
    EXPECT_EQ(new_order_row[0],
              AllTypeVariant{NUM_ORDERS_PER_DISTRICT + 1});      // NO_O_ID
    ...
    // Verify ORDER entry
    const auto order_row =
        Hyrise::get().storage_manager.get_table("ORDER")->get_row(
            new_sizes["ORDER"] - 1);
    EXPECT_EQ(order_row[0],
              AllTypeVariant{NUM_ORDERS_PER_DISTRICT + 1});  // O_ID
    ...
    EXPECT_GE(order_row[4],
              AllTypeVariant{static_cast<int32_t>(old_time)}); // O_ENTRY_D
    EXPECT_TRUE(variant_is_null(order_row[5]));                // O_CARRIER_ID
    EXPECT_EQ(order_row[6], AllTypeVariant{static_cast<int32_t>(
                                order_lines.size())}); // O_OL_CNT
    ...
    for (auto line_idx = size_t{0}; line_idx < order_lines.size(); ++line_idx)
    {
        ...
        const auto row_idx =
            new_sizes["ORDER_LINE"] - order_lines.size() + line_idx;
        const auto order_line_row =
            Hyrise::get()
                .storage_manager.get_table("ORDER_LINE")
                ->get_row(row_idx);
        EXPECT_EQ(order_line_row[0],
                  AllTypeVariant{NUM_ORDERS_PER_DISTRICT + 1});       // OL_O_ID
        ...
        if (ol_i_id != TPCCNewOrder::UNUSED_ITEM_ID)
        {
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"SELECT I_PRICE FROM ITEM WHERE I_ID = "} +
            ...
            const auto expected_ol_amount =
                i_price * static_cast<float>(order_lines[line_idx].ol_quantity);
            EXPECT_FLOAT_EQ(boost::get<float>(order_line_row[8]),
                            expected_ol_amount); // OL_AMOUNT
        }
        ...
        {
            auto pipeline =
                SQLPipelineBuilder{std::string{"SELECT S_DIST_"} +
                                   (new_order.d_id < 10 ? "0" : "") +
                                   std::to_string(new_order.d_id) +
                                   " FROM STOCK WHERE S_W_ID = " +
                                   std::to_string(ol_supply_w_id) +
                                   " AND S_I_ID = " + std::to_string(ol_i_id)}
                    .with_transaction_context(old_transaction_context)
                    .create_pipeline();
            const auto [_, table] = pipeline.get_result_table();
            EXPECT_TRUE(table);
            ...
            EXPECT_EQ(order_line_row[9],
                      AllTypeVariant{s_dist}); // OL_DIST_INFO
        }
    }
}
...
TEST_F(TPCCTest, NewOrderUnusedItemId)
{
    ...
    // NewOrder transactions with simulated user input errors (unused item ids)
    // are still counted as successful
    EXPECT_TRUE(new_order.execute());
...
TEST_F(TPCCTest, PaymentCustomerByName)
{
    ...
    // Generate random payments until we have one that identified the customer
    // by name
    ...
    pmr_string w_name;
    {
        ...
        EXPECT_TRUE(table);
        EXPECT_EQ(table->row_count(), 1);
        // As all warehouses start with W_YTD = 300'000, this should be the
        // first and only change
        EXPECT_FLOAT_EQ(*table->get_value<float>("W_YTD", 0),
                        300'000.0f + payment.h_amount);
    ...
    pmr_string d_name;
    {
        auto pipeline =
            SQLPipelineBuilder{
                std::string{
                    "SELECT D_YTD, D_NAME FROM DISTRICT WHERE D_W_ID = "} +
                std::to_string(payment.w_id) +
                " AND D_ID = " + std::to_string(payment.d_id)}
                .create_pipeline();
        const auto [_, table] = pipeline.get_result_table();
        EXPECT_TRUE(table);
        EXPECT_EQ(table->row_count(), 1);
        // As all districts start with D_YTD = 30'000, this should be the first
        // and only change
        EXPECT_FLOAT_EQ(*table->get_value<float>("D_YTD", 0),
                        30'000.0f + payment.h_amount);
    ...
    // Verify that the customer is updated
    {
        auto pipeline =
            SQLPipelineBuilder{
                std::string{"SELECT C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT "
                            "FROM CUSTOMER WHERE C_W_ID = "} +
                std::to_string(payment.w_id) +
                " AND C_D_ID = " + std::to_string(payment.d_id) +
                " AND C_ID = " + std::to_string(payment.c_id)}
                .create_pipeline();
        const auto [_, table] = pipeline.get_result_table();
        EXPECT_TRUE(table);
        EXPECT_EQ(table->row_count(), 1);
        // Customers start with C_BALANCE = -10, C_YTD_PAYMENT = 10,
        // C_PAYMENT_CNT = 1
        EXPECT_FLOAT_EQ(*table->get_value<float>("C_BALANCE", 0),
                        -10.0f - payment.h_amount);
        EXPECT_FLOAT_EQ(*table->get_value<float>("C_YTD_PAYMENT", 0),
                        10.0f + payment.h_amount);
    ...
    // Verify that a new history entry is written
    {
        auto pipeline =
            SQLPipelineBuilder{
                std::string{"SELECT H_DATE, H_AMOUNT, H_DATA FROM HISTORY "
                            "WHERE H_W_ID = "} +
                std::to_string(payment.w_id) +
                " AND H_D_ID = " + std::to_string(payment.d_id) +
        ...
        EXPECT_FLOAT_EQ(*table->get_value<float>("H_AMOUNT", row),
                        payment.h_amount);
        EXPECT_EQ(*table->get_value<pmr_string>("H_DATA", row),
                  w_name + "    " + d_name);
    }
}
...
TEST_F(TPCCTest, OrderStatusCustomerById)
{
    ...
    // As Order-Status has no externally visible changes, we create a new order
    // and test for correct return values
    ...
    while (new_order.order_lines.back().ol_i_id == TPCCNewOrder::UNUSED_ITEM_ID)
    {
        // Make sure that we do not have a TPCCNewOrder with an invalid ITEM_ID
    ...
    auto order_status =
        TPCCOrderStatus{NUM_WAREHOUSES, order_status_sql_executor};
...
// The dynamic nature of Stock-Level together with the random table generation
// makes this transaction hard to test.
