...
void create_test_data_directory(std::optional<std::string> &prefix)
{
    Assert(!std::filesystem::exists(hyrise::test_data_path),
           "Cannot create directory for test data: \"" +
               hyrise::test_data_path + "\" already exists.");

    if (prefix)
    {
        Assert(std::filesystem::exists("./" + *prefix),
               "Cannot create directory for test data because \"" + *prefix +
                   "\" does not exist");
    }
...
int main(int argc, char **argv)
{
    Assert(std::filesystem::exists("resources/test_data/tbl"),
           "Cannot find resources/test_data/tbl. Are you running the test "
           "suite from the main folder of the Hyrise "
           "repository?");
    ...
    if (argc > 1)
    {
        // If argv[1] is set after gtest extracted its commands, we interpret it
        // as directory name prefix for test data
        hyrise::test_data_path =
            "./" + std::string(argv[1]) + "/.hyrise_test_data/";
        prefix = argv[1];
    }
    else
    {
