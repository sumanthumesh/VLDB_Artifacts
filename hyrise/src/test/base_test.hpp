...
class BaseTestWithParam
    : public std::conditional_t<std::is_same_v<ParamType, void>,
                                ::testing::Test,
                                ::testing::TestWithParam<ParamType>>
{
  public:
    /**
     * Base test uses its destructor instead of TearDown() to clean up. This
     * way, derived test classes can override TearDown() safely without
     * preventing the BaseTest-cleanup from happening. GTest runs the destructor
...
// Converts a data table to a reference table. Note that this does not cover all
// variations of ReferenceSegments. Single-chunk guarantees (and the lack
// thereof), ordered and unordered PosLists, etc. should be tested individually
// where necessary.
...
inline auto segment_encoding_formatter =
    [](const ::testing::TestParamInfo<SegmentEncodingSpec> &info)
{
    ...
    if (spec.vector_compression_type)
    {
        result +=
            "_" +
            std::string{magic_enum::enum_name(*spec.vector_compression_type)};
    }
