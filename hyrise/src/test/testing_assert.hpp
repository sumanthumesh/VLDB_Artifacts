...
bool check_lqp_tie(const std::shared_ptr<const AbstractLQPNode> &output,
                   LQPInputSide input_side,
                   const std::shared_ptr<const AbstractLQPNode> &input);

template <typename Functor>
bool contained_in_lqp(const std::shared_ptr<AbstractLQPNode> &node,
                      Functor contains_fn)
{
    if (!node)
    {
    ...
    return contained_in_lqp(node->left_input(), contains_fn) ||
           contained_in_lqp(node->right_input(), contains_fn);
}

template <typename Functor>
bool contained_in_query_plan(
    const std::shared_ptr<const AbstractOperator> &node, Functor contains_fn)
{
    if (!node)
    {
        return false;
...
/**
 * Compare two tables with respect to OrderSensitivity, TypeCmpMode and
 * FloatComparisonMode
 */
#define EXPECT_TABLE_EQ(hyrise_table, expected_table, order_sensitivity,       \
                        type_cmp_mode, float_comparison_mode)                  \
    {                                                                          \
        if (const auto table_difference_message = check_table_equal(           \
                hyrise_table, expected_table, order_sensitivity,               \
                type_cmp_mode, float_comparison_mode, IgnoreNullable::No))     \
        {                                                                      \
...
#define EXPECT_TABLE_EQ_UNORDERED(hyrise_table, expected_table)                \
    EXPECT_TABLE_EQ(hyrise_table, expected_table, OrderSensitivity::No,        \
                    TypeCmpMode::Strict,                                       \
...
#define EXPECT_TABLE_EQ_ORDERED(hyrise_table, expected_table)                  \
    EXPECT_TABLE_EQ(hyrise_table, expected_table, OrderSensitivity::Yes,       \
                    TypeCmpMode::Strict,                                       \
                    FloatComparisonMode::AbsoluteDifference)

/**
 * Compare two segments with respect to OrderSensitivity, TypeCmpMode and
 * FloatComparisonMode
 */
// clang-format off
...
#define EXPECT_SEGMENT_EQ_UNORDERED(segment_to_test, expected_segment)         \
    EXPECT_SEGMENT_EQ(segment_to_test, expected_segment, OrderSensitivity::No, \
                      TypeCmpMode::Strict,                                     \
                      FloatComparisonMode::AbsoluteDifference)
...
#define EXPECT_SEGMENT_EQ_ORDERED(segment_to_test, expected_segment)           \
    EXPECT_SEGMENT_EQ(segment_to_test, expected_segment,                       \
...
#define EXPECT_LQP_EQ(lhs, rhs)                                                \
    {                                                                          \
        Assert(lhs != rhs, "Comparing an LQP with itself is always true. Did " \
                           "you mean to take a deep copy?");                   \
