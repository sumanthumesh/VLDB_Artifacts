...
class ExpressionResultTest : public BaseTest
{
  public:
    ...
    bool check_view(pmr_vector<typename ExpectedViewType::Type> values,
                    pmr_vector<bool> nulls)
    {
        auto match = false;
        ExpressionResult<typename ExpectedViewType::Type>(values, nulls)
            .as_view(
                [&](const auto &view) {
                    match = std::is_same_v<std::decay_t<decltype(view)>,
                                           ExpectedViewType>;
                });
        return match;
    }
...
TEST_F(ExpressionResultTest, AsView)
{
    ...
    EXPECT_TRUE(check_view<ExpressionResultNullableSeries<int32_t>>(
        {5, 6, 7}, {false, true, false}));
    EXPECT_TRUE(
        check_view<ExpressionResultNonNullSeries<int32_t>>({5, 6, 7}, {}));
}
