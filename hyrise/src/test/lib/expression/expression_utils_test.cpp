...
class ExpressionUtilsTest : public BaseTest
{
  public:
    void SetUp() override
    {
        ...
        node_b = MockNode::make(MockNode::ColumnDefinitions{
...
TEST_F(ExpressionUtilsTest, ExpressionFlattenAndInflate)
{
    ...
    const auto expression =
        and_(and_(greater_than_(a_a, 5), less_than_(a_b, 6)), equals_(a_c, 7));
    const auto flattened_expressions =
    ...
    const auto inflated_expression =
        inflate_logical_expressions(flattened_expressions, LogicalOperator::Or);
    EXPECT_EQ(
        *inflated_expression,
        *or_(or_(equals_(a_c, 7), greater_than_(a_a, 5)), less_than_(a_b, 6)));
}
...
TEST_F(ExpressionUtilsTest, ExpressionEvaluableOnLQPSimple)
{
  ...
  PredicateNode::make(equals_(a_a, 1),
    ...
    EXPECT_TRUE(expression_evaluable_on_lqp(
        count_(lqp_column_(node_a, INVALID_COLUMN_ID)), *input_lqp));
    EXPECT_FALSE(expression_evaluable_on_lqp(
        count_(lqp_column_(node_b, INVALID_COLUMN_ID)), *input_lqp));
}
...
TEST_F(ExpressionUtilsTest, ExpressionDeepReplace)
{
    ...
    const auto mapping =
        ExpressionUnorderedMap<std::shared_ptr<AbstractExpression>>{
            {a_a, a_b}, {value_(7), value_(8)}};

    expression_deep_replace(expression, mapping);

    EXPECT_EQ(*expression,
              *and_(and_(greater_than_(a_b, 5), less_than_(a_b, 6)),
                    equals_(a_c, 8)));
}

TEST_F(ExpressionUtilsTest, ExpressionCommonType)
{
    EXPECT_EQ(expression_common_type(DataType::Int, DataType::Int),
              DataType::Int);
    EXPECT_EQ(expression_common_type(DataType::Int, DataType::Float),
              DataType::Float);
    EXPECT_EQ(expression_common_type(DataType::Int, DataType::Null),
              DataType::Int);
    EXPECT_EQ(expression_common_type(DataType::Float, DataType::Float),
              DataType::Float);
    EXPECT_EQ(expression_common_type(DataType::Null, DataType::Float),
              DataType::Float);
    EXPECT_EQ(expression_common_type(DataType::Double, DataType::Int),
              DataType::Double);
    EXPECT_EQ(expression_common_type(DataType::Double, DataType::Double),
              DataType::Double);
    EXPECT_EQ(expression_common_type(DataType::Float, DataType::Double),
              DataType::Double);
    EXPECT_EQ(expression_common_type(DataType::Long, DataType::Long),
              DataType::Long);
    EXPECT_EQ(expression_common_type(DataType::String, DataType::String),
              DataType::String);
}
...
TEST_F(ExpressionUtilsTest, ExpressionContainsPlaceholders)
{
    EXPECT_FALSE(expression_contains_placeholder(
        and_(greater_than_(a_a, 5), equals_(a_c, 7))));
    EXPECT_TRUE(expression_contains_placeholder(and_(
        greater_than_(a_a, placeholder_(ParameterID{5})), equals_(a_c, 7))));
    EXPECT_FALSE(expression_contains_placeholder(
        and_(greater_than_(a_a, correlated_parameter_(ParameterID{5}, a_a)),
             equals_(a_c, 7))));
}
...
TEST_F(ExpressionUtilsTest, ExpressionContainsCorrelatedParameter)
{
    EXPECT_FALSE(expression_contains_correlated_parameter(
        and_(greater_than_(a_a, 5), equals_(a_c, 7))));
    EXPECT_FALSE(expression_contains_correlated_parameter(and_(
        greater_than_(a_a, placeholder_(ParameterID{5})), equals_(a_c, 7))));
...
{
    const auto pqp_subquery = std::make_shared<PQPSubqueryExpression>(
        std::make_shared<GetTable>("table_a"));
...
{
    const auto pqp_subquery_a = std::make_shared<PQPSubqueryExpression>(
        std::make_shared<GetTable>("table_a"));
    const auto pqp_subquery_b = std::make_shared<PQPSubqueryExpression>(
        std::make_shared<GetTable>("table_b"));
...
{
    // (1) Nested Subquery
    const auto nested_subquery = std::make_shared<PQPSubqueryExpression>(
        std::make_shared<GetTable>("table_a"));
    // (2) Root Subquery
    ...
    // Wrapping subqueries should not make a difference
    {
        ...
        auto found_pqp_subqueries =
            find_pqp_subquery_expressions(wrapped_subquery);
        EXPECT_EQ(found_pqp_subqueries.size(), 1);
        EXPECT_EQ(found_pqp_subqueries.at(0), subquery);
    }
}
...
TEST_F(ExpressionUtilsTest, GetValueOrParameter)
{
    ...
    const auto correlated_parameter_expression =
        correlated_parameter_(ParameterID{0}, value_expression);
    ...
    EXPECT_THROW(expression_get_value_or_parameter(*invalid_cast),
                 std::logic_error);
    // Casts as NULL are undefined
    EXPECT_THROW(expression_get_value_or_parameter(*cast_as_null),
                 std::logic_error);

    {
        const auto actual_value =
            expression_get_value_or_parameter(*value_expression);
        EXPECT_NE(actual_value, std::nullopt);
        EXPECT_EQ(*actual_value, expected_value);
    }
    {
        const auto actual_value =
            expression_get_value_or_parameter(*correlated_parameter_expression);
        EXPECT_NE(actual_value, std::nullopt);
        EXPECT_EQ(*actual_value, expected_value);
    }
    {
        const auto actual_value =
            expression_get_value_or_parameter(*cast_as_float);
        EXPECT_NE(actual_value, std::nullopt);
    ...
    }
    {
        ...
        const auto actual_value =
            expression_get_value_or_parameter(*cast_from_null);
        EXPECT_NE(actual_value, std::nullopt);
    ...
    }
    {
        ...
        const auto actual_value =
            expression_get_value_or_parameter(*cast_column);
...
{
    const auto expression_vector =
        std::vector<std::shared_ptr<AbstractExpression>>{a_a, a_b};
