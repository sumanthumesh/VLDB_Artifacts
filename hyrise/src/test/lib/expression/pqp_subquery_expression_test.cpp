  ...
  public:
    void SetUp() override
    {
        ...
        // Create PQPSubqueryExpression without parameters
        {
            ...
            _pqp_subquery_expression =
                std::make_shared<PQPSubqueryExpression>(_pqp);
        }

        // Create PQPSubqueryExpression with parameters
        {
            // Build a PQP with one parameter returning a single non-nullable
            // value
            const auto get_table = std::make_shared<GetTable>(_table_name);
            // Projection (a + ?)
            const auto parameter = placeholder_(ParameterID{0});
            const auto projection = std::make_shared<Projection>(
                get_table, expression_vector(add_(_a, parameter)));
            ...
            _pqp_subquery_expression_with_param =
                std::make_shared<PQPSubqueryExpression>(
                    _pqp_with_param, DataType::Int, false, _parameters);
        }
    }
...
{
    EXPECT_EQ(*_pqp_subquery_expression_with_param,
              *_pqp_subquery_expression_with_param);

    // different parameters:
    const auto parameters_b = PQPSubqueryExpression::Parameters{
        std::make_pair(ParameterID{0}, ColumnID{2})};
    const auto subquery_different_parameter =
        std::make_shared<PQPSubqueryExpression>(_pqp_with_param, DataType::Int,
                                                false, parameters_b);
    EXPECT_NE(*_pqp_subquery_expression_with_param,
              *subquery_different_parameter);

    // different PQP:
    const auto pqp_without_limit = _pqp_with_param->mutable_left_input();
    const auto subquery_different_lqp = std::make_shared<PQPSubqueryExpression>(
        pqp_without_limit, DataType::Int, false, _parameters);
...
TEST_F(PQPSubqueryExpressionTest, DeepCopy)
{
    ...
    EXPECT_EQ(_pqp_subquery_expression_with_param_copy->parameters[0].first,
              ParameterID{0});
    EXPECT_EQ(_pqp_subquery_expression_with_param_copy->parameters[0].second,
              ColumnID{0});
    EXPECT_NE(_pqp_subquery_expression_with_param_copy->pqp,
              _pqp_subquery_expression_with_param->pqp);
    EXPECT_EQ(_pqp_subquery_expression_with_param_copy->pqp->type(),
              OperatorType::Limit);

    // Without Parameter
    const auto _pqp_subquery_expression_copy =
        std::dynamic_pointer_cast<PQPSubqueryExpression>(
            _pqp_subquery_expression->deep_copy());
    ASSERT_TRUE(_pqp_subquery_expression_copy);

    ASSERT_EQ(_pqp_subquery_expression_copy->parameters.size(), 0u);
    EXPECT_NE(_pqp_subquery_expression_copy->pqp,
              _pqp_subquery_expression_with_param->pqp);
    EXPECT_EQ(_pqp_subquery_expression_copy->pqp->type(),
...
TEST_F(PQPSubqueryExpressionTest, DeepCopyPreservesPlanDeduplication)
{
    ...
    auto pqp_subquery_expression =
        std::make_shared<PQPSubqueryExpression>(union_positions);
    ...
    EXPECT_EQ(get_table->consumer_count(),
              copied_pqp->left_input()->left_input()->consumer_count());
}
...
TEST_F(PQPSubqueryExpressionTest, DataType)
{
    ...
    const auto subquery_float = std::make_shared<PQPSubqueryExpression>(
        _pqp_with_param, DataType::Float, true, _parameters);
...
TEST_F(PQPSubqueryExpressionTest, IsNullable)
{
    ...
    EXPECT_ANY_THROW(
        _pqp_subquery_expression->is_nullable_on_lqp(*DummyTableNode::make()));
}

TEST_F(PQPSubqueryExpressionTest, AsColumnName)
{
    EXPECT_TRUE(
        std::regex_search(_pqp_subquery_expression->as_column_name(),
                          std::regex{"SUBQUERY \\(PQP, 0x[0-9a-f]+\\)"}));
    EXPECT_TRUE(
        std::regex_search(_pqp_subquery_expression_with_param->as_column_name(),
                          std::regex{"SUBQUERY \\(PQP, 0x[0-9a-f]+\\)"}));
}
