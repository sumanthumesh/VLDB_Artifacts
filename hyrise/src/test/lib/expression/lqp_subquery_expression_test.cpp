...
{
    /**
     * Test that when comparing subquery expressions, the underlying LQPs get
     * compared and so does the Parameter signature
     */
  ...
  AggregateNode::make(expression_vector(), expression_vector(max_(add_(a, parameter_d))),
    ...
    const auto subquery_d =
        lqp_subquery_(lqp_d, std::make_pair(ParameterID{0}, a_2));
    const auto subquery_e =
        lqp_subquery_(lqp_e, std::make_pair(ParameterID{0}, b));
...
{
    // Can't determine the DataType of this Subquery, since it depends on a
    // parameter
...
TEST_F(LQPSubqueryExpressionTest, IsNullable)
{
    ...
    // clang-format of
    const auto lqp_c = AggregateNode::make(
        expression_vector(), expression_vector(max_(add_(a, null_()))),
