...
/**
 * Tests for most expression types, excluding Subqueries, since they have no
 * complex behaviour that would warrant their own test file.
 */
...
class ExpressionTest : public BaseTest
{
  public:
    void SetUp() override
    {
        ...
        table_int_float_with_null =
            load_table("resources/test_data/tbl/int_float_with_null.tbl");
        Hyrise::get().storage_manager.add_table("int_float", table_int_float);
        Hyrise::get().storage_manager.add_table("int_float_with_null",
                                                table_int_float_with_null);
...
TEST_F(ExpressionTest, Equals)
{
    ...
    EXPECT_EQ(*extract_(DatetimeComponent::Month, "1999-07-30"),
              *extract_(DatetimeComponent::Month, "1999-07-30"));
    EXPECT_NE(*extract_(DatetimeComponent::Day, "1999-07-30"),
              *extract_(DatetimeComponent::Month, "1999-07-30"));
    ...
    EXPECT_EQ(*interval_(3, DatetimeComponent::Day),
              *interval_(3, DatetimeComponent::Day));
    EXPECT_NE(*interval_(3, DatetimeComponent::Day),
              *interval_(3, DatetimeComponent::Month));
    EXPECT_NE(*interval_(3, DatetimeComponent::Day),
              *interval_(4, DatetimeComponent::Day));
}

TEST_F(ExpressionTest, DeepEquals)
{
    const auto expr_a_a =
        sub_(mul_(add_(1, 5), add_(13.3, 14.4)), mod_(12, 5.5));
    const auto expr_a_b =
        sub_(mul_(add_(1, 5), add_(13.3, 14.4)), mod_(12, 5.5));
    const auto expr_b =
        sub_(mul_(add_(1, 5), add_(13.3, 14.4)), mod_(12, null_()));
    ...
    const auto case_c =
        case_(equals_(a, 123), b, case_(equals_(a, 1234), a, null_()));
...
TEST_F(ExpressionTest, DeepCopy)
{
    ...
    const auto expr_b = and_(greater_than_equals_(15, 12),
                             or_(greater_than_(5, 3), less_than_(3, 5)));
...
TEST_F(ExpressionTest, DeepCopySubplanDeduplication)
{
    ...
    auto scan_less_than =
        std::make_shared<TableScan>(table_wrapper, less_than_(pqp_column_a, 5));
    auto scan_greater_than = std::make_shared<TableScan>(
        table_wrapper, greater_than_(pqp_column_a, 10));
    auto union_positions =
        std::make_shared<UnionPositions>(scan_less_than, scan_greater_than);

    auto projection_int1 = std::make_shared<Projection>(
        union_positions, expression_vector(pqp_column_a));
    auto projection_int2 = std::make_shared<Projection>(
        union_positions, expression_vector(pqp_column_a));
    auto projection_int3 = std::make_shared<Projection>(
        union_positions, expression_vector(pqp_column_a));
    auto projection_string = std::make_shared<Projection>(
        union_positions,
        expression_vector(cast_(pqp_column_a, DataType::String)));
    auto pqp_subquery_expression_int1 = std::make_shared<PQPSubqueryExpression>(
        projection_int1, DataType::Int, false,
        PQPSubqueryExpression::Parameters{});
    auto pqp_subquery_expression_int2 = std::make_shared<PQPSubqueryExpression>(
        projection_int2, DataType::Int, false,
        PQPSubqueryExpression::Parameters{});
    auto pqp_subquery_expression_int3 = std::make_shared<PQPSubqueryExpression>(
        projection_int3, DataType::Int, false,
        PQPSubqueryExpression::Parameters{});
    auto pqp_subquery_expression_string =
        std::make_shared<PQPSubqueryExpression>(
            projection_string, DataType::String, false,
            PQPSubqueryExpression::Parameters{});

    // We are going to check whether the following property survives
    // deep_copy(). We do not want to see duplicated operators in copied PQPs.
    ...
    expressions.emplace_back(
        add_(pqp_subquery_expression_int1, pqp_subquery_expression_int2));
    // Cast Expression
    expressions.emplace_back(
        cast_(pqp_subquery_expression_int1, DataType::String));
    // Case Expression
    expressions.emplace_back(case_(pqp_subquery_expression_int1,
                                   pqp_subquery_expression_int2,
    ...
    // List Expression
    expressions.emplace_back(list_(pqp_subquery_expression_int1,
                                   pqp_subquery_expression_int2,
                                   pqp_subquery_expression_int3));
    // Logical Expression
    expressions.emplace_back(
        and_(pqp_subquery_expression_int1, pqp_subquery_expression_int2));
    // Placeholder Expression           [never contains PQPSubqueryExpression]
    // Predicate Expressions
    // - InExpression
    expressions.emplace_back(
        in_(pqp_subquery_expression_int1, pqp_subquery_expression_int2));
    // - BetweenExpression
    expressions.emplace_back(between_inclusive_(pqp_subquery_expression_int1,
                                                pqp_subquery_expression_int2,
                                                pqp_subquery_expression_int3));
    // - BinaryPredicateExpression
    expressions.emplace_back(greater_than_(pqp_subquery_expression_int1,
                                           pqp_subquery_expression_int2));
    // - IsNullExpression
    ...
    for (const auto &expression : expressions)
    {
        ...
        for (auto argument_idx = size_t{0};
             argument_idx < copied_arguments.size(); ++argument_idx)
        {
            const auto &copied_pqp_subquery_expression =
                std::dynamic_pointer_cast<PQPSubqueryExpression>(
                    copied_arguments.at(argument_idx));
            ...
            // (1) Check for TableWrapper / subplan reuse inside a
            // PQPSubqueryExpression
            const auto &copied_projection = copied_pqp_subquery_expression->pqp;
            const auto &copied_union_positions =
                copied_projection->left_input();
            const auto &copied_scan_less_than =
                copied_union_positions->left_input();
            const auto &copied_table_wrapper =
                copied_scan_less_than->left_input();
            EXPECT_EQ(table_wrapper->consumer_count(),
                      copied_table_wrapper->consumer_count());

            // (2) Bonus: Check for reuse of UnionPositions among multiple
            // PQPSubqueryExpressions
            pqp_subquery_expression_count++;
            if (argument_idx == (copied_arguments.size() - 1))
            {
                EXPECT_EQ(copied_union_positions->consumer_count(),
                          pqp_subquery_expression_count);
            }
        }
    }
...
TEST_F(ExpressionTest, RequiresCalculation)
{
    ...
    EXPECT_FALSE(
        correlated_parameter_(ParameterID{5}, a)->requires_computation());
    EXPECT_FALSE(a->requires_computation());
    EXPECT_FALSE(PQPColumnExpression::from_table(*table_int_float, "a")
    ...
    const auto pqp_subquery_expression =
        std::make_shared<PQPSubqueryExpression>(get_table);
...
TEST_F(ExpressionTest, AsColumnName)
{
    ...
    EXPECT_EQ(between_inclusive_(5, 3, 4)->as_column_name(),
              "5 BETWEEN INCLUSIVE 3 AND 4");
    EXPECT_EQ(case_(1, 3, case_(0, 2, 1))->as_column_name(),
              "CASE WHEN 1 THEN 3 ELSE CASE WHEN 0 THEN 2 ELSE 1 END END");
    EXPECT_EQ(
        extract_(DatetimeComponent::Month, "1993-03-04")->as_column_name(),
        "EXTRACT(MONTH FROM '1993-03-04')");
    EXPECT_EQ(substr_("Hello", 1, 2)->as_column_name(), "SUBSTR('Hello',1,2)");
    EXPECT_EQ(concat_("Hello", "World")->as_column_name(),
              "CONCAT('Hello','World')");
    ...
    EXPECT_EQ(cast_("36", DataType::Float)->as_column_name(),
              "CAST('36' AS float)");
    EXPECT_EQ(placeholder_(ParameterID{0})->as_column_name(),
              "Placeholder[ParameterID=0]");
    EXPECT_EQ(correlated_parameter_(ParameterID{0}, a)->as_column_name(),
              "Parameter[name=a; ParameterID=0]");
    EXPECT_EQ(in_(5, list_(1, 2, 3))->as_column_name(), "(5) IN (1, 2, 3)");
    EXPECT_EQ(not_in_(5, list_(1, 2, 3))->as_column_name(),
              "(5) NOT IN (1, 2, 3)");
    EXPECT_EQ(interval_(30, DatetimeComponent::Day)->as_column_name(),
              "INTERVAL '30' Day");
}
...
{
    /**
     * Test that parentheses are placed correctly when generating column names
    ...
    EXPECT_EQ(case_(greater_than_(3, 2), mul_(2, 3), 2)->as_column_name(),
              "CASE WHEN 3 > 2 THEN 2 * 3 ELSE 2 END");
    EXPECT_EQ(case_(1, mul_(2, 3), div_(5, mul_(2, 3)))->as_column_name(),
              "CASE WHEN 1 THEN 2 * 3 ELSE 5 / (2 * 3) END");
    EXPECT_EQ(list_(1, sum_(a))->as_column_name(), "(1, SUM(a))");
    EXPECT_EQ(and_(1, 1)->as_column_name(), "1 AND 1");
    EXPECT_EQ(and_(1, greater_than_(add_(2, 3), 4))->as_column_name(),
              "1 AND 2 + 3 > 4");
    EXPECT_EQ(and_(1, or_(greater_than_(add_(2, 3), 4), 0))->as_column_name(),
              "1 AND (2 + 3 > 4 OR 0)");
    EXPECT_EQ(or_(1, and_(greater_than_(add_(2, 3), 4), 0))->as_column_name(),
              "1 OR (2 + 3 > 4 AND 0)");
    EXPECT_EQ(is_null_(1)->as_column_name(), "1 IS NULL");
    EXPECT_EQ(is_null_(and_(1, 1))->as_column_name(), "(1 AND 1) IS NULL");
    EXPECT_EQ(is_null_(sum_(add_(a, 2)))->as_column_name(),
              "SUM(a + 2) IS NULL");
    EXPECT_EQ(less_than_(a, b)->as_column_name(), "a < b");
    EXPECT_EQ(less_than_(add_(a, 5), b)->as_column_name(), "a + 5 < b");
    EXPECT_EQ(between_inclusive_(a, 2, 3)->as_column_name(),
              "a BETWEEN INCLUSIVE 2 AND 3");
    EXPECT_EQ(and_(greater_than_equals_(b, 5), between_inclusive_(a, 2, 3))
                  ->as_column_name(),
              "b >= 5 AND a BETWEEN INCLUSIVE 2 AND 3");
    EXPECT_EQ(not_equals_(between_inclusive_(a, 2, 3), 0)->as_column_name(),
              "(a BETWEEN INCLUSIVE 2 AND 3) != 0");

    EXPECT_EQ(mul_(less_than_(add_(a, 5), b), 3)->as_column_name(),
              "(a + 5 < b) * 3");
    EXPECT_EQ(add_(1, between_inclusive_(a, 2, 3))->as_column_name(),
              "1 + (a BETWEEN INCLUSIVE 2 AND 3)");

    // TODO(anybody) Omit redundant parentheses
    EXPECT_EQ(add_(5, add_(1, 3))->as_column_name(), "5 + (1 + 3)");
    EXPECT_EQ(add_(add_(2, 5), add_(1, 3))->as_column_name(),
              "(2 + 5) + (1 + 3)");
    EXPECT_EQ(mul_(mul_(2, 5), mul_(1, 3))->as_column_name(),
              "(2 * 5) * (1 * 3)");
    EXPECT_EQ(and_(and_(1, 0), and_(0, 1))->as_column_name(),
              "(1 AND 0) AND (0 AND 1)");
    EXPECT_EQ(and_(1, and_(1, or_(0, 1)))->as_column_name(),
              "1 AND (1 AND (0 OR 1))");
}
...
TEST_F(ExpressionTest, DataType)
{
    ...
    EXPECT_EQ(interval_(30, DatetimeComponent::Day)->data_type(),
              DataType::String);
...
TEST_F(ExpressionTest, IsNullable)
{
    ...
    EXPECT_TRUE(
        between_inclusive_(1, null_(), 3)->is_nullable_on_lqp(*dummy_lqp));
    ...
    EXPECT_TRUE(
        add_(greater_than_(2, null_()), 1)->is_nullable_on_lqp(*dummy_lqp));
    EXPECT_TRUE(
        and_(greater_than_(2, null_()), 1)->is_nullable_on_lqp(*dummy_lqp));
    EXPECT_FALSE(cast_(12, DataType::String)->is_nullable_on_lqp(*dummy_lqp));
    EXPECT_TRUE(
        cast_(null_(), DataType::String)->is_nullable_on_lqp(*dummy_lqp));
    ...
    // Division by zero could be nullable, thus division and modulo are always
    // nullable
...
TEST_F(ExpressionTest, StaticTableNode)
{
...

    {
        const auto static_table_node =
            StaticTableNode::make(table_int_float_with_null);
        const auto col_a = lqp_column_(static_table_node, ColumnID{0});
        EXPECT_EQ(col_a->as_column_name(), "a");
        EXPECT_EQ(col_a->data_type(), DataType::Int);
...
{
    std::vector<std::pair<int, std::shared_ptr<AbstractExpression>>>
        expressions;
    ...
    expressions.emplace_back(__LINE__,
                             correlated_parameter_(ParameterID{0}, a));
    expressions.emplace_back(__LINE__,
                             correlated_parameter_(ParameterID{1}, a));
    expressions.emplace_back(__LINE__,
                             correlated_parameter_(ParameterID{1}, b));

    // ExistsExpression
    expressions.emplace_back(__LINE__, exists_(lqp_subquery_(int_float_node)));
    expressions.emplace_back(__LINE__,
                             not_exists_(lqp_subquery_(int_float_node)));
    expressions.emplace_back(
        __LINE__, not_exists_(lqp_subquery_(int_float_node_nullable)));

    // ExtractExpression
    expressions.emplace_back(__LINE__,
                             extract_(DatetimeComponent::Month, "1993-03-04"));
    expressions.emplace_back(__LINE__,
                             extract_(DatetimeComponent::Month, "2019-03-04"));
    expressions.emplace_back(__LINE__,
                             extract_(DatetimeComponent::Year, "2019-03-04"));
    ...
    expressions.emplace_back(
        __LINE__,
        lqp_subquery_(int_float_node, std::make_pair(ParameterID{0}, a)));
    expressions.emplace_back(
        __LINE__,
        lqp_subquery_(int_float_node, std::make_pair(ParameterID{1}, a)));
    expressions.emplace_back(
        __LINE__,
        lqp_subquery_(int_float_node, std::make_pair(ParameterID{1}, b)));
    ...
    // PQPColumnExpression
    expressions.emplace_back(
        __LINE__, pqp_column_(ColumnID{0}, DataType::Float, false, "a + b"));
    expressions.emplace_back(
        __LINE__, pqp_column_(ColumnID{1}, DataType::Float, false, "a + b"));
    expressions.emplace_back(
        __LINE__, pqp_column_(ColumnID{1}, DataType::Int, false, "a + b"));
    expressions.emplace_back(
        __LINE__, pqp_column_(ColumnID{1}, DataType::Int, true, "a + b"));
    expressions.emplace_back(
        __LINE__, pqp_column_(ColumnID{1}, DataType::Int, true, "alias"));

    // PQPSubqueryExpression
    expressions.emplace_back(
        __LINE__,
        pqp_subquery_(std::make_shared<GetTable>("a"), DataType::Int, false));
    expressions.emplace_back(
        __LINE__,
        pqp_subquery_(std::make_shared<GetTable>("b"), DataType::Int, false));
    expressions.emplace_back(
        __LINE__,
        pqp_subquery_(std::make_shared<GetTable>("b"), DataType::Float, false));
    expressions.emplace_back(
        __LINE__,
        pqp_subquery_(std::make_shared<GetTable>("b"), DataType::Float, true));
    const auto parameter = std::make_pair(ParameterID{0}, ColumnID{0});
    expressions.emplace_back(__LINE__,
                             pqp_subquery_(std::make_shared<GetTable>("b"),
                                           DataType::Float, true, parameter));
    ...
    for (auto first_iter = expressions.begin(); first_iter != expressions.end();
         ++first_iter)
    {
        const auto &[first_line, first_expression] = *first_iter;
        SCOPED_TRACE(std::string{"First expression from line "} +
        ...
        {
            // The deep copy of a PQPSubqueryExpression is not equal to its
            // source, see the comment in _shallow_equals.
            deep_copy = first_expression->deep_copy();
            EXPECT_EQ(*first_expression, *deep_copy);
            EXPECT_EQ(first_expression->hash(), deep_copy->hash());
        }

        for (auto second_iter = first_iter + 1;
             second_iter != expressions.end(); ++second_iter)
        {
            const auto &[second_line, second_expression] = *second_iter;
            SCOPED_TRACE(std::string{"Second expression from line "} +
...
{
    EXPECT_TRUE(
        WindowFunctionExpression::is_count_star(*count_star_(int_float_node)));
