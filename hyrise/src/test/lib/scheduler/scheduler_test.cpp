...
class SchedulerTest : public BaseTest
{
  protected:
    void stress_linear_dependencies(std::atomic_uint32_t &counter)
    {
        const auto task1 = std::make_shared<JobTask>(
            [&]()
            {
                ...
                auto successful =
                    counter.compare_exchange_strong(current_value, 1u);
        ...
        const auto task2 = std::make_shared<JobTask>(
            [&]()
            {
                ...
                auto successful =
                    counter.compare_exchange_strong(current_value, 2u);
                ASSERT_TRUE(successful);
            });
        const auto task3 = std::make_shared<JobTask>(
            [&]()
            {
                ...
                auto successful =
                    counter.compare_exchange_strong(current_value, 3u);
                ASSERT_TRUE(successful);
            });
    ...
    void stress_multiple_dependencies(std::atomic_uint32_t &counter)
    {
        ...
        const auto task3 = std::make_shared<JobTask>(
            [&]()
            {
                ...
                auto successful =
                    counter.compare_exchange_strong(current_value, 4u);
                ASSERT_TRUE(successful);
            });
    ...
    void stress_diamond_dependencies(std::atomic_uint32_t &counter)
    {
        ...
        const auto task4 = std::make_shared<JobTask>(
            [&]()
            {
                ...
                auto successful =
                    counter.compare_exchange_strong(current_value, 7u);
                ASSERT_TRUE(successful);
            });
    ...
    void increment_counter_in_subtasks(std::atomic_uint32_t &counter)
    {
        ...
        for (auto outer_counter = size_t{0}; outer_counter < 10;
             ++outer_counter)
        {
            auto task = std::make_shared<JobTask>(
                [&]()
                {
                    ...
                    for (auto inner_counter = size_t{0}; inner_counter < 3;
                         ++inner_counter)
                    {
                        auto job =
                            std::make_shared<JobTask>([&]() { ++counter; });
...
{
    // Tests the grouping described in
    // AbstractScheduler::schedule_and_wait_for_tasks and
    // NodeQueueScheduler::_group_tasks. Also tests that successor tasks are
    // called immediately after their dependencies finish. Not really a
    // multi-threading test, though.
    ...
    for (auto task_id = 0; task_id < TASK_COUNT; ++task_id)
    {
        tasks.emplace_back(std::make_shared<JobTask>(
            [&output, task_id] { output.emplace_back(task_id); }));
    }
    Hyrise::get().scheduler()->schedule_and_wait_for_tasks(tasks);
    Hyrise::get().scheduler()->finish();

    // We expect NUM_GROUPS chains of tasks to be created. As tasks are added to
    // the chains by calling AbstractTask::set_predecessor_of, the first task in
    // the input vector ends up being the last task being called. This results
    // in [40 30 20 10 0 41 31 21 11 1 ...]
    ...
    for (auto group = 0; group < num_groups; ++group)
    {
        ...
        {
            expected_output.emplace_back(tasks.size() -
                                         (task_id + 1) * num_groups + group);
        }
    }
...
TEST_F(SchedulerTest, MultipleOperators)
{
    ...
    const auto ts =
        std::make_shared<TableScan>(gt, greater_than_equals_(a, 1234));
...
TEST_F(SchedulerTest, VerifyTaskQueueSetup)
{
    ...
    {
        // If the machine has less than 4 cores, the calls to
        // use_non_numa_topology() below will implicitly reduce the worker count
        // to the number of cores, therefore failing the assertions.
        GTEST_SKIP();
    }
...
TEST_F(SchedulerTest, TaskToNodeAssignment)
{
    ...
    const auto task_1 =
        std::make_shared<JobTask>([&]() {}, SchedulePriority::Default, false);
    const auto task_2 =
        std::make_shared<JobTask>([&]() {}, SchedulePriority::Default, false);
    const auto task_3 =
        std::make_shared<JobTask>([&]() {}, SchedulePriority::Default, false);
...
TEST_F(SchedulerTest, SingleWorkerGuaranteeProgress)
{
    ...
    auto task = std::make_shared<JobTask>(
        ...
        {
            const auto subtask =
                std::make_shared<JobTask>([&task_done]() { task_done = true; });

            subtask->schedule();
            Hyrise::get().scheduler()->wait_for_tasks(
                std::vector<std::shared_ptr<AbstractTask>>{subtask});
        });

    task->schedule();
    Hyrise::get().scheduler()->wait_for_tasks(
        std::vector<std::shared_ptr<AbstractTask>>{task});
...
TEST_F(SchedulerTest, DetermineQueueIDForTask)
{
    ...
    // For the case of no load on node ID 0 (which is the case here), tasks are
    // always scheduled on this node.
    EXPECT_EQ(node_queue_scheduler->determine_queue_id(CURRENT_NODE_ID),
              NodeID{0});

    // The distribution of tasks under high load is tested in the concurrency
    // stress tests.
}
...
template <typename Iterator> void merge_sort(Iterator first, Iterator last)
{
    ...
    auto tasks = std::vector<std::shared_ptr<AbstractTask>>{};
    tasks.emplace_back(
        std::make_shared<JobTask>([&]() { merge_sort(first, middle); }));
    tasks.emplace_back(
        std::make_shared<JobTask>([&]() { merge_sort(middle, last); }));
...
// Recursive merge sort. Creates a typical divide-and-conquer fan out pattern of
// tasks. We use the text book implementation that recurses until the vector
// length is 1 to increase the depth of the fan out.
TEST_F(SchedulerTest, MergeSort)
{
    // Sizes up to 20'000 works for MacOS (debug mode, more for release) with
    // its comparatively small stack size. If this test fails on a new platform,
    // check the system's stack size and if ITEM_COUNT needs to be reduced.
...
TEST_F(SchedulerTest, NodeQueueSchedulerCreationAndReset)
{
    ...
    Hyrise::get().topology.use_fake_numa_topology(thread_count,
                                                  thread_count / 4);
    for (auto loop_id = size_t{0}; loop_id < 256; ++loop_id)
    {
        auto node_queue_scheduler = std::make_shared<NodeQueueScheduler>();
        Hyrise::get().set_scheduler(node_queue_scheduler);
        EXPECT_EQ(node_queue_scheduler->active_worker_count().load(),
                  thread_count);
        Hyrise::get().set_scheduler(
            std::make_shared<ImmediateExecutionScheduler>());
...
TEST_F(SchedulerTest, ShutdownTaskDecrement)
{
    ...
    // Prepare job for execution (usually done when scheduled and obtained by
    // workers)
