  ...
  public:
    void SetUp() override
    {
        ...
        node_b = MockNode::make(MockNode::ColumnDefinitions{
            {DataType::Int, "a"}, {DataType::Int, "b"}});
        node_c = MockNode::make(MockNode::ColumnDefinitions{
            {DataType::Int, "a"}, {DataType::Int, "b"}});
        ...
        cache = create_cache({node_a, node_b, validate_c},
                             {equals_(a_a, b_a), equals_(b_a, c_a),
                              greater_than_(c_a, 5), less_than_(c_a, a_a),
                              equals_(a_a, b_b)});
    }
        ...
        const std::vector<std::shared_ptr<AbstractExpression>> &predicates)
    {
        ...
        for (auto predicate_idx = size_t{0}; predicate_idx < predicates.size();
             ++predicate_idx)
        {
            predicate_indices.emplace(predicates[predicate_idx], predicate_idx);
        }

        return std::make_shared<JoinGraphStatisticsCache>(
            std::move(vertex_indices), std::move(predicate_indices));
    }
    ...
    std::shared_ptr<BaseAttributeStatistics> statistics_a_a, statistics_a_b,
...
{
    EXPECT_EQ(cache->get(JoinGraphStatisticsCache::Bitmask{8, 0b00001011},
                         expression_vector(a_a, a_b, b_a, b_b, a_a)),
              nullptr);

    cache->set(JoinGraphStatisticsCache::Bitmask{8, 0b00001011},
               expression_vector(a_a, a_b, b_a, b_b, a_a),
               table_statistics_a_b);

    const auto cached_a_b =
        cache->get(JoinGraphStatisticsCache::Bitmask{8, 0b00001011},
                   expression_vector(a_a, a_b, b_a, b_b, a_a));
    ...
    const auto cached_b_a =
        cache->get(JoinGraphStatisticsCache::Bitmask{8, 0b00001011},
                   expression_vector(b_a, b_b, a_a, a_b, a_a));
