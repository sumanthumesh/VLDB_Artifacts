  ...
  public:
    void SetUp() override
    {
        ...
        node_a = create_mock_node_with_statistics(
            {{DataType::Int, "a"}, {DataType::Int, "b"}}, 100,
    ...
    const auto histogram_b_b = std::make_shared<GenericHistogram<int32_t>>(
      ...
      std::vector<HistogramCountType>{32}, std::vector<HistogramCountType>{10});
        ...
        node_b = create_mock_node_with_statistics(
            {{DataType::Int, "a"}, {DataType::Int, "b"}}, 32,
    ...
    const auto histogram_c_y = std::make_shared<GenericHistogram<int32_t>>(
      ...
      std::vector<HistogramCountType>{10});
        ...
        node_c = create_mock_node_with_statistics(
            {{DataType::Int, "x"}, {DataType::Int, "y"}}, 64,
         ...
         */
        // clang-format on
        node_e = create_mock_node_with_statistics(
            {{DataType::Int, "a"}, {DataType::Int, "b"}}, 100,
            {GenericHistogram<int32_t>::with_single_bin(1, 100, 10, 10),
             nullptr});
        // clang-format off
     ...
     */
        // clang-format on
        node_f = create_mock_node_with_statistics(
            {{DataType::Int, "a"}, {DataType::Float, "b"}}, 200,
            {GenericHistogram<int32_t>::with_single_bin(1, 200, 200, 10),
             GenericHistogram<float>::with_single_bin(1.0f, 100.0f, 200,
                                                      150.0f)});
        // clang-format off
     ...
     */
        // clang-format on
        node_g = create_mock_node_with_statistics(
            {{DataType::String, "a"}}, 100,
...
TEST_F(CardinalityEstimatorTest, Alias)
{
  ...
  AliasNode::make(expression_vector(a_b, a_a), std::vector<std::string>{"x", "y"},
    ...
    EXPECT_EQ(result_table_statistics->column_statistics.at(0),
              input_table_statistics->column_statistics.at(1));
    EXPECT_EQ(result_table_statistics->column_statistics.at(1),
...
TEST_F(CardinalityEstimatorTest, JoinNumericEquiInner)
{
  ...
  JoinNode::make(JoinMode::Inner, equals_(b_a, c_x),
    ...
    const auto column_statistics_b_a =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            result_statistics->column_statistics.at(0));
    ...
    const auto column_statistics_b_b =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            result_statistics->column_statistics[1]);
    ...
    const auto column_statistics_c_x =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            result_statistics->column_statistics[2]);
    ...
    const auto column_statistics_c_y =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            result_statistics->column_statistics[3]);
...
{
    // Test that joins on with non-equi predicate conditions are estimated as
    // cross joins (for now)
  ...
  const auto input_lqp_e = JoinNode::make(JoinMode::Inner, less_than_equals_(b_a, c_x), node_b, node_c);
    ...
    EXPECT_EQ(
        estimator.estimate_statistics(input_lqp_a)->column_statistics.size(),
        4u);
    EXPECT_EQ(estimator.estimate_statistics(input_lqp_b)->row_count, 32u * 64u);
    EXPECT_EQ(
        estimator.estimate_statistics(input_lqp_b)->column_statistics.size(),
        4u);
    EXPECT_EQ(estimator.estimate_statistics(input_lqp_c)->row_count, 32u * 64u);
    EXPECT_EQ(
        estimator.estimate_statistics(input_lqp_c)->column_statistics.size(),
        4u);
    EXPECT_EQ(estimator.estimate_statistics(input_lqp_d)->row_count, 32u * 64u);
    EXPECT_EQ(
        estimator.estimate_statistics(input_lqp_d)->column_statistics.size(),
        4u);
    EXPECT_EQ(estimator.estimate_statistics(input_lqp_e)->row_count, 32u * 64u);
    EXPECT_EQ(
        estimator.estimate_statistics(input_lqp_e)->column_statistics.size(),
        4u);
}

TEST_F(CardinalityEstimatorTest, JoinEquiInnerDifferentDataTypes)
{
    // Test that joins on columns with non-equal data types are estimated as
    // cross joins (for now)
...
TEST_F(CardinalityEstimatorTest, JoinInnerEquiHistograms)
{
    ...
    const auto right_histogram = GenericHistogram<int32_t>(
        std::vector<int32_t>{20, 30, 50}, std::vector<int32_t>{29, 39, 59},
        std::vector<HistogramCountType>{10, 5, 10},
        std::vector<HistogramCountType>{7, 2, 10});

    const auto join_histogram =
        CardinalityEstimator::estimate_inner_equi_join_with_histograms<int32_t>(
...
{
    // Test that left, right and full outer join operations are estimated the
    // same as an inner join (for now)

    const auto inner_join_lqp =
        JoinNode::make(JoinMode::Inner, equals_(a_a, b_a), node_a, node_b);
    const auto inner_join_cardinality =
        estimator.estimate_cardinality(inner_join_lqp);

    const auto left_join_lqp =
        JoinNode::make(JoinMode::Left, equals_(a_a, b_a), node_a, node_b);
    EXPECT_EQ(estimator.estimate_cardinality(left_join_lqp),
              inner_join_cardinality);

    const auto right_join_lqp =
        JoinNode::make(JoinMode::Right, equals_(a_a, b_a), node_a, node_b);
    EXPECT_EQ(estimator.estimate_cardinality(right_join_lqp),
              inner_join_cardinality);

    const auto full_join_lqp =
        JoinNode::make(JoinMode::FullOuter, equals_(a_a, b_a), node_a, node_b);
    EXPECT_EQ(estimator.estimate_cardinality(left_join_lqp),
              inner_join_cardinality);
}
...
{
    const auto left_join_column_histogram =
        std::make_shared<GenericHistogram<int32_t>>(
            std::vector<int32_t>{0, 10, 20, 30, 40, 50, 60},
            std::vector<int32_t>{9, 19, 29, 39, 49, 59, 69},
            std::vector<HistogramCountType>{10, 15, 10, 20, 5, 15, 5},
            std::vector<HistogramCountType>{1, 1, 3, 8, 1, 6, 1});
    const auto left_join_column_statistics =
        std::make_shared<AttributeStatistics<int32_t>>();
    left_join_column_statistics->set_statistics_object(
        left_join_column_histogram);

    const auto left_non_join_column_histogram =
        std::make_shared<GenericHistogram<int32_t>>(
            std::vector<int32_t>{0, 5, 10}, std::vector<int32_t>{4, 9, 14},
            std::vector<HistogramCountType>{20, 40, 30},
            std::vector<HistogramCountType>{1, 1, 3});
    const auto left_non_join_column_statistics =
        std::make_shared<AttributeStatistics<int32_t>>();
    left_non_join_column_statistics->set_statistics_object(
        left_non_join_column_histogram);

    const auto right_histogram = std::make_shared<GenericHistogram<int32_t>>(
        std::vector<int32_t>{20, 30, 50, 70},
        std::vector<int32_t>{29, 39, 69, 79},
        std::vector<HistogramCountType>{10, 5, 10, 8},
        std::vector<HistogramCountType>{7, 2, 6, 8});
    const auto right_statistics =
        std::make_shared<AttributeStatistics<int32_t>>();
    right_statistics->set_statistics_object(right_histogram);

    const auto left_table_statistics = TableStatistics{
        {left_join_column_statistics, left_non_join_column_statistics}, 90};
    const auto right_table_statistics = TableStatistics{{right_statistics}, 33};

    const auto join_estimation = CardinalityEstimator::estimate_semi_join(
        ColumnID{0}, ColumnID{0}, left_table_statistics,
        right_table_statistics);

    // Left Join Column Histogram:  |  Right Join Column Histogram:  | Resulting
    // Histogram
    //                              |                                |
    // Bin       Height  Distinct   |  Bin       Height  Distinct    |  Bin
    // Height  Distinct  Comment [ 0,  9]  10      1          | | [10, 19]  15
    // 1          |                                | [20, 29]  10      3 |  [20,
    // 29]  (10)    7           |  [20, 29]  10      3         100% as 3 <= 7
    // [30, 39]  20      8          |  [30, 39]  (5)     2           |  [30, 39]
    // 5       2         h: 20*(2/8) [40, 49]   5      1          | | [50, 59]
    // 15      6          |  [50,                          |  [50, 59]  7.5 3 h:
    // 15*((6/2)/6) [60, 69]   5      1          |       69]  (10)    6 |  [69,
    // 69]  5       1         100% as 1 <= (6/2)
    //                              |  [70, 79]  (8)     8           |
    //                              |                                |
    //                              |  (Height is ignored on right)  |
    //                              sum:  27.5
    ...
    const auto &first_column_histogram =
        *static_cast<const AttributeStatistics<int32_t> &>(
             *join_estimation->column_statistics[0])
             .histogram;
    EXPECT_EQ(first_column_histogram.bin_count(), 4);
    EXPECT_EQ(first_column_histogram.bin(0),
              HistogramBin<int32_t>(20, 29, 10, 3));
    EXPECT_EQ(first_column_histogram.bin(1),
              HistogramBin<int32_t>(30, 39, 5, 2));
    EXPECT_EQ(first_column_histogram.bin(2),
              HistogramBin<int32_t>(50, 59, 7.5, 3));
    EXPECT_EQ(first_column_histogram.bin(3),
              HistogramBin<int32_t>(60, 69, 5, 1));

    const auto &second_column_histogram =
        *static_cast<const AttributeStatistics<int32_t> &>(
             *join_estimation->column_statistics[1])
             .histogram;
    EXPECT_EQ(second_column_histogram.bin_count(), 3);
    const auto selectivity = 27.5f / 90;
    EXPECT_EQ(second_column_histogram.bin(0),
              HistogramBin<int32_t>(0, 4, 20 * selectivity, 1));
    EXPECT_EQ(second_column_histogram.bin(1),
              HistogramBin<int32_t>(5, 9, 40 * selectivity, 1));
    EXPECT_EQ(second_column_histogram.bin(2),
              HistogramBin<int32_t>(10, 14, 30 * selectivity, 3));
}

TEST_F(CardinalityEstimatorTest, JoinAnti)
{
    // Test that anti joins are estimated return the left input statistics (for
    // now)

    const auto anti_null_as_false_join_lqp = JoinNode::make(
        JoinMode::AntiNullAsFalse, equals_(a_a, b_a), node_a, node_b);
    EXPECT_EQ(estimator.estimate_statistics(anti_null_as_false_join_lqp),
              node_a->table_statistics());

    const auto anti_null_as_true_join_lqp = JoinNode::make(
        JoinMode::AntiNullAsTrue, equals_(a_a, b_a), node_a, node_b);
    EXPECT_EQ(estimator.estimate_statistics(anti_null_as_true_join_lqp),
              node_a->table_statistics());
}
...
TEST_F(CardinalityEstimatorTest, LimitWithValueExpression)
{
    ...
    const auto column_statistics_a =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            limit_statistics_a->column_statistics.at(0));
    const auto column_statistics_b =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            limit_statistics_a->column_statistics.at(1));
...
TEST_F(CardinalityEstimatorTest, LimitWithComplexExpression)
{
    ...
    // The "complex" expression `1+2` is evaluated and LimitNode is estimated to
    // have a selectivity of 1
    EXPECT_EQ(estimator.estimate_cardinality(limit_lqp_a), 100);
    EXPECT_EQ(estimator.estimate_statistics(limit_lqp_a),
              node_a->table_statistics());
}
...
TEST_F(CardinalityEstimatorTest, MockNode)
{
    ...
    EXPECT_EQ(estimator.estimate_statistics(node_a),
              node_a->table_statistics());
}
...
TEST_F(CardinalityEstimatorTest, PredicateWithOneSimplePredicate)
{
  ...
  PredicateNode::make(greater_than_(a_a, 50),
    ...
    const auto plan_output_statistics =
        estimator.estimate_statistics(input_lqp);
    ...
    const auto plan_output_statistics_a =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            plan_output_statistics->column_statistics.at(0));
    const auto plan_output_statistics_b =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            plan_output_statistics->column_statistics.at(1));
...
TEST_F(CardinalityEstimatorTest, PredicateWithOneBetweenPredicate)
{
    ...
    {
        const auto between_predicate = std::make_shared<BetweenExpression>(
            between_predicate_condition, a_a, value_(10), value_(89));
    ...
    const auto input_lqp =
      PredicateNode::make(between_predicate,
        ...
        const auto plan_output_statistics =
            estimator.estimate_statistics(input_lqp);
        EXPECT_FLOAT_EQ(plan_output_statistics->row_count,
                        80.0f); // Same as above
        ASSERT_EQ(plan_output_statistics->column_statistics.size(), 2u);
        ...
        ASSERT_EQ(plan_output_statistics_a->histogram->bin_count(), 1u);
        EXPECT_EQ(plan_output_statistics_a->histogram->bin_minimum(BinID{0}),
                  10);
        EXPECT_EQ(plan_output_statistics_a->histogram->bin_maximum(BinID{0}),
                  89);
        EXPECT_EQ(plan_output_statistics_a->histogram->bin_height(BinID{0}),
                  80);
    }
}
...
{
    // Same as PredicateTwoOnTheSameColumn, but the conditions are within one
    // predicate
    // clang-format off
  const auto input_lqp =
...
{
    // While we do not handle overlapping ranges yet, we at least want to make
    // sure that tables don't become bigger.
    // clang-format off
  const auto input_lqp =
...
TEST_F(CardinalityEstimatorTest, PredicateTwoOnDifferentColumns)
{
  ...
  PredicateNode::make(greater_than_(a_a, 50),
    ...
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(input_lqp->left_input()),
                    38.5f);
    EXPECT_FLOAT_EQ(
        estimator.estimate_cardinality(input_lqp->left_input()->left_input()),
        100.0f);
}
...
TEST_F(CardinalityEstimatorTest, PredicateMultiple)
{
  ...
  PredicateNode::make(equals_(d_a, 95),
    ...
    EXPECT_FLOAT_EQ(
        estimator.estimate_cardinality(input_lqp->left_input()->left_input()),
        4.7571f);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(
                        input_lqp->left_input()->left_input()->left_input()),
                    4.7571f);
    EXPECT_FLOAT_EQ(
        estimator.estimate_cardinality(
            input_lqp->left_input()->left_input()->left_input()->left_input()),
        39.3542f);
}

TEST_F(CardinalityEstimatorTest, PredicateWithValuePlaceholder)
{
    // 20 distinct values in column d_a and 100 values total. So == is assumed
    // to have a selectivity of 5%, != of 95%, and everything else is assumed to
    // hit 50%

    const auto lqp_a =
        PredicateNode::make(equals_(d_a, placeholder_(ParameterID{0})), node_d);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp_a), 5.0f);

    const auto lqp_b = PredicateNode::make(
        not_equals_(d_a, placeholder_(ParameterID{0})), node_d);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp_b), 95.0f);

    const auto lqp_c = PredicateNode::make(
        less_than_(d_a, placeholder_(ParameterID{0})), node_d);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp_c), 50.0f);

    const auto lqp_d = PredicateNode::make(
        greater_than_(d_a, placeholder_(ParameterID{0})), node_d);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp_d), 50.0f);

    // BETWEEN is split up into (a >= ? AND a <= ?), so it ends up with a
    // selecitivity of 25%
    const auto lqp_e = PredicateNode::make(
        between_inclusive_(d_a, placeholder_(ParameterID{0}),
...
TEST_F(CardinalityEstimatorTest, PredicateWithNull)
{
    ...
    const auto lqp_c =
        PredicateNode::make(between_inclusive_(a_a, 5, NullValue{}), node_a);
...
TEST_F(CardinalityEstimatorTest, PredicateEstimateColumnVsColumnEquiScan)
{
  ...
  const auto right_histogram = GenericHistogram<int32_t>(
    ...
    const auto result_histogram = CardinalityEstimator::
        estimate_column_vs_column_equi_scan_with_histograms(left_histogram,
                                                            right_histogram);

    ASSERT_EQ(result_histogram->bin_count(), 2u);
    EXPECT_EQ(result_histogram->bin(BinID{0}),
              HistogramBin<int32_t>(13, 14, 5, 2));
    EXPECT_EQ(result_histogram->bin(BinID{1}),
              HistogramBin<int32_t>(16, 20, 10, 2));
}

TEST_F(CardinalityEstimatorTest, PredicateEstimateColumnVsColumnNonEquiScan)
{
    // Test that a selectivity of 1 is used for ColumnVsColumn scans with a
    // non-equals PredicateCondition

    EXPECT_EQ(estimator.estimate_cardinality(
                  PredicateNode::make(not_equals_(a_a, a_b), node_a)),
              100);
    EXPECT_EQ(estimator.estimate_cardinality(
                  PredicateNode::make(greater_than_(a_a, a_b), node_a)),
              100);
    EXPECT_EQ(estimator.estimate_cardinality(
                  PredicateNode::make(greater_than_equals_(a_a, a_b), node_a)),
              100);
    EXPECT_EQ(estimator.estimate_cardinality(
                  PredicateNode::make(less_than_(a_a, a_b), node_a)),
              100);
    EXPECT_EQ(estimator.estimate_cardinality(
                  PredicateNode::make(less_than_equals_(a_a, a_b), node_a)),
              100);
}

TEST_F(CardinalityEstimatorTest, PredicateColumnVsColumnDifferentDataTypes)
{
    // Test that a selectivity of 1 is used for ColumnVsColumn scans on columns
    // with differing column data types
  ...
  PredicateNode::make(equals_(f_a, f_b),
    ...
    const auto estimated_column_statistics_a =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            estimated_statistics->column_statistics.at(0));
    const auto estimated_column_statistics_b =
        std::dynamic_pointer_cast<AttributeStatistics<float>>(
            estimated_statistics->column_statistics.at(1));
...
{
    // Test that a node, lacking statistical information on one column, produces
    // an estimation when scanning on the column WITH statistics and has a
    // selectivity of 1 on columns WITHOUT statistics
  ...
  PredicateNode::make(greater_than_(e_b, 50),
    ...
    const auto estimated_statistics_a =
        estimator.estimate_statistics(input_lqp_a);
    const auto estimated_statistics_b =
        estimator.estimate_statistics(input_lqp_a);

    const auto estimated_column_statistics_a_a =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            estimated_statistics_a->column_statistics.at(0));
    const auto estimated_column_statistics_a_b =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            estimated_statistics_a->column_statistics.at(1));
    const auto estimated_column_statistics_b_a =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            estimated_statistics_b->column_statistics.at(0));
    const auto estimated_column_statistics_b_b =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            estimated_statistics_b->column_statistics.at(1));
...
TEST_F(CardinalityEstimatorTest, PredicateString)
{
    ...
    const auto input_lqp_c =
        PredicateNode::make(greater_than_equals_(g_a, "a"), node_g);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(input_lqp_c), 100.0f);

    const auto input_lqp_d =
        PredicateNode::make(greater_than_equals_(g_a, "m"), node_g);
...
TEST_F(CardinalityEstimatorTest, Projection)
{
  ...
  ProjectionNode::make(expression_vector(a_b, add_(a_b, a_a), a_a),
    ...
    EXPECT_EQ(result_table_statistics->column_statistics.at(0),
              input_table_statistics->column_statistics.at(1));
    EXPECT_TRUE(result_table_statistics->column_statistics.at(1));
    EXPECT_EQ(result_table_statistics->column_statistics.at(2),
              input_table_statistics->column_statistics.at(0));
}
...
TEST_F(CardinalityEstimatorTest, StoredTable)
{
    Hyrise::get().storage_manager.add_table(
        "t", load_table("resources/test_data/tbl/int.tbl"));
    EXPECT_EQ(estimator.estimate_cardinality(StoredTableNode::make("t")), 3);
}

TEST_F(CardinalityEstimatorTest, Validate)
{
    // Test Validate doesn't break the TableStatistics. The CardinalityEstimator
    // is not estimating anything for Validate as there are no statistics
    // available atm to base such an estimation on.
...
{
    // Test that UnionNodes sum up the input row counts and return the left
    // input statistics (for now)
  ...
  UnionNode::make(SetOperationMode::Positions,
    ...
    ASSERT_EQ(result_statistics->column_statistics.at(0),
              node_a->table_statistics()->column_statistics.at(0));
    ASSERT_EQ(result_statistics->column_statistics.at(1),
              node_a->table_statistics()->column_statistics.at(1));
}

TEST_F(CardinalityEstimatorTest, NonQueryNodes)
{
    // Test that, basically, the CardinalityEstimator doesn't crash when
    // processing non-query nodes. There is not much more to test here
    ...
    const auto prepared_plan =
        std::make_shared<PreparedPlan>(node_a, std::vector<ParameterID>{});
    const auto create_prepared_plan_lqp =
        CreatePreparedPlanNode::make("t", prepared_plan);
    EXPECT_EQ(estimator.estimate_cardinality(create_prepared_plan_lqp), 0.0f);

    const auto lqp_view = std::make_shared<LQPView>(
        node_a, std::unordered_map<ColumnID, std::string>{{ColumnID{0}, "x"},
                                                          {ColumnID{1}, "y"}});
    ...
    EXPECT_EQ(estimator.estimate_cardinality(DropViewNode::make("v", false)),
              0.0f);
    EXPECT_EQ(estimator.estimate_cardinality(DropTableNode::make("t", false)),
              0.0f);
    EXPECT_EQ(estimator.estimate_cardinality(DummyTableNode::make()), 0.0f);
}

// Usually, we assume that we know nothing about predicates getting their values
// from uncorrelated subqueries and we simply forward the input estimations.
// However, special predicates with subqueries can stem from join rewrites
// (e.g., JoinToPredicateRewriteRule). We estimate the resulting plans just like
// we would have done it for a semi-join. The following two test cases ensure
// the correct behavior for the possible rewrites.
TEST_F(CardinalityEstimatorTest, ValueScanWithUncorrelatedSubquery)
{
    ...
    // In this case, SELECT r_regionkey ... is a selection on the primary key.
    // We know it will result in a single value, so using an uncorrelated
    // subquery to get this value is feasible.
    // clang-format off
  const auto subquery =
  ProjectionNode::make(expression_vector(c_y),
    ...
    auto lqp =
        PredicateNode::make(equals_(a_a, lqp_subquery_(subquery)), node_a);

    const auto semi_join_lqp =
        JoinNode::make(JoinMode::Semi, equals_(a_a, c_y), node_a, subquery);

    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(semi_join_lqp));
    EXPECT_LT(estimator.estimate_cardinality(lqp),
              estimator.estimate_cardinality(node_a));

    // Ensure correlated subqueries are not considered and we forward the input
    // estimates.
    lqp = PredicateNode::make(
        equals_(a_a,
                lqp_subquery_(subquery, std::make_pair(ParameterID{0}, a_b))),
        node_a);

    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(node_a));

    // Ensure that any predicate condition other than equals and between still
    // lead to forwarding the input estimates.
    for (const auto predicate_condition :
         {PredicateCondition::LessThan, PredicateCondition::LessThanEquals,
          PredicateCondition::NotEquals, PredicateCondition::GreaterThan,
          PredicateCondition::GreaterThanEquals})
    {
        const auto predicate = std::make_shared<BinaryPredicateExpression>(
            predicate_condition, a_a, lqp_subquery_(subquery));
...
TEST_F(CardinalityEstimatorTest, BetweenScanWithUncorrelatedSubquery)
{
    ...
    //                                WHERE d_year = 2000);
    const auto subquery = PredicateNode::make(
        between_inclusive_(c_y, value_(0), value_(2)), node_c);

    const auto min_c_y = AggregateNode::make(
        expression_vector(), expression_vector(min_(c_y)), subquery);

    const auto max_c_y = AggregateNode::make(
        expression_vector(), expression_vector(max_(c_y)), subquery);

    auto lqp = PredicateNode::make(
        between_inclusive_(a_a, lqp_subquery_(min_c_y), lqp_subquery_(max_c_y)),
        node_a);
    ...
    for (const auto predicate_condition :
         {PredicateCondition::BetweenLowerExclusive,
          ...
          PredicateCondition::BetweenExclusive})
    {
        const auto predicate = std::make_shared<BetweenExpression>(
            predicate_condition, a_a, lqp_subquery_(min_c_y),
            lqp_subquery_(max_c_y));
        lqp = PredicateNode::make(predicate, node_a);
        EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                        estimator.estimate_cardinality(semi_join_lqp));
    }
  ...
  PredicateNode::make(between_inclusive_(a_a, lqp_subquery_(min_c_y, std::make_pair(ParameterID{0}, a_b)), lqp_subquery_(max_c_y, std::make_pair(ParameterID{0}, a_b))),  // NOLINT(whitespace/line_length)
    ...
    // Ensure we do not consider the wrong aggregate functions and forward the
    // input estimates.
    lqp = PredicateNode::make(
        between_inclusive_(a_a, lqp_subquery_(max_c_y), lqp_subquery_(max_c_y)),
        node_a);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(node_a));

    lqp = PredicateNode::make(
        between_inclusive_(a_a, lqp_subquery_(min_c_y), lqp_subquery_(min_c_y)),
        node_a);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(node_a));

    // Aggregate functions must be performed on the same column.
    const auto min_c_x = AggregateNode::make(
        expression_vector(), expression_vector(min_(c_x)), subquery);
    lqp = PredicateNode::make(
        between_inclusive_(a_a, lqp_subquery_(min_c_x), lqp_subquery_(max_c_y)),
        node_a);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(node_a));

    // Aggregate functions must not group the tuples. Otherwise, we do not get
    // the minimum and maximum value for the join key for the underlying node.
    const auto min_c_y_grouped = AggregateNode::make(
        expression_vector(c_x), expression_vector(min_(c_y)), subquery);
    lqp = PredicateNode::make(between_inclusive_(a_a,
                                                 lqp_subquery_(min_c_y_grouped),
                                                 lqp_subquery_(max_c_y)),
                              node_a);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(node_a));
    ...
    // Otherwise, further join keys might be filtered and the predicate is not
    // equivalent to a semi-join with the origin node anymore.
    // clang-format off
  ...
  AggregateNode::make(expression_vector(), expression_vector(min_(c_y)),
    ...
    // clang-format on
    lqp = PredicateNode::make(
        between_inclusive_(a_a, lqp_subquery_(min_c_y_filtered),
                           lqp_subquery_(max_c_y)),
        node_a);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(node_a));
...
TEST_F(CardinalityEstimatorTest,
       BetweenScanWithUncorrelatedSubqueryAndProjections)
{
    // Similar to BetweenScanWithUncorrelatedSubquery, but with a single
    // AggregateNode for MIN/MAX and two projections. Example query is the same
    // as for that test case:
    //     SELECT SUM(ws_ext_sales_price) FROM web_sales
    //      WHERE ws_sold_date_sk BETWEEN (SELECT MIN(d_date_sk) FROM date_dim
    //      WHERE d_year = 2000)
    //                                AND (SELECT MAX(d_date_sk) FROM date_dim
  ...
  AggregateNode::make(expression_vector(), expression_vector(min_(c_y), max_(c_y)),
    ...
    const auto min_c_y =
        ProjectionNode::make(expression_vector(min_(c_y)), subquery);
    const auto max_c_y =
        ProjectionNode::make(expression_vector(max_(c_y)), subquery);

    auto lqp = PredicateNode::make(
        between_inclusive_(a_a, lqp_subquery_(min_c_y), lqp_subquery_(max_c_y)),
        node_a);

    const auto semi_join_lqp = JoinNode::make(JoinMode::Semi, equals_(a_a, c_y),
                                              node_a, subquery->left_input());
    ...
    const auto min_c_y_alias =
        AliasNode::make(expression_vector(min_(c_y)),
                        std::vector<std::string>{"foo"}, subquery);
    lqp = PredicateNode::make(between_inclusive_(a_a,
                                                 lqp_subquery_(min_c_y_alias),
                                                 lqp_subquery_(max_c_y)),
                              node_a);
    EXPECT_FLOAT_EQ(estimator.estimate_cardinality(lqp),
                    estimator.estimate_cardinality(node_a));
    ...
    const auto union_node =
        UnionNode::make(SetOperationMode::All, subquery, subquery);
    ...
    const auto aggregate_node = AggregateNode::make(
        expression_vector(c_x), expression_vector(min_(c_y), max_(c_y)));
...
TEST_F(CardinalityEstimatorTest, WindowNode)
{
    ...
    const auto window =
        window_(expression_vector(), expression_vector(),
                std::vector<SortMode>{}, std::move(frame_description));
