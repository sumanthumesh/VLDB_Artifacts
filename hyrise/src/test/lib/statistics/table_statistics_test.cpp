...
TEST_F(TableStatisticsTest, FromTable)
{
    const auto table = load_table(
        "resources/test_data/tbl/int_with_nulls_large.tbl", ChunkOffset{20});
    ...
    const auto column_statistics_a =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            table_statistics->column_statistics.at(0));
    ASSERT_TRUE(column_statistics_a);

    const auto histogram_a =
        std::dynamic_pointer_cast<AbstractHistogram<int32_t>>(
            column_statistics_a->histogram);
    ASSERT_TRUE(histogram_a);

    // The 24 nulls values should be represented in the compact statistics as
    // well
    ...
    const auto column_statistics_b =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            table_statistics->column_statistics.at(1));
    ASSERT_TRUE(column_statistics_b);

    const auto histogram_b =
        std::dynamic_pointer_cast<AbstractHistogram<int32_t>>(
            column_statistics_b->histogram);
