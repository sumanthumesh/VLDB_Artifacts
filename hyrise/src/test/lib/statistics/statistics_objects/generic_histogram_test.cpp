...
/**
 * As well as testing GenericHistogram, we also test a lot of the functionally
 * implemented in AbstractHistogram here. We do this here since GenericHistogram
 * allows us to easily construct interesting edge cases.
 */
...
template <typename T> T next_value(T v)
{
    return HistogramDomain<T>{}.next_value_clamped(v);
}

template <typename T> T previous_value(T v)
{
...
class GenericHistogramTest : public BaseTest
{
  public:
    ...
    template <typename T>
    void test_sliced_with_predicates(
        const std::vector<std::shared_ptr<AbstractHistogram<T>>> &histograms,
        ...
        for (const auto &predicate : predicates)
        {
            ...
            for (const auto &histogram : histograms)
            {
                ...
                const auto sliced_statistics_object =
                    histogram->sliced(predicate.predicate_condition,
                                      predicate.value, predicate.value2);
                const auto cardinality = histogram->estimate_cardinality(
                    predicate.predicate_condition, predicate.value,
                    predicate.value2);

                if (const auto sliced_histogram =
                        std::dynamic_pointer_cast<AbstractHistogram<T>>(
                            sliced_statistics_object))
                {
                    SCOPED_TRACE(sliced_histogram->description());
                    EXPECT_NEAR(sliced_histogram->total_count(), cardinality,
                                0.005f);
                }
                else
                {
...
TEST_F(GenericHistogramTest, EstimateCardinalityAndPruningBasicInt)
{
    const auto histogram =
        GenericHistogram<int32_t>{{12, 12345}, {123, 123456}, {2, 5}, {2, 2}};
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 0), 0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 12), 1.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 1'234), 0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 123'456),
        2.5f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 1'000'000),
        0.f);
}

TEST_F(GenericHistogramTest, EstimateCardinalityAndPruningBasicFloat)
{
    const auto histogram = GenericHistogram<float>{
        {0.5f, 2.5f, 3.6f}, {2.2f, 3.3f, 6.1f}, {4, 6, 4}, {4, 3, 3}};

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 0.4f), 0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 0.5f),
        4 / 4.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 1.1f),
        4 / 4.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 1.3f),
        4 / 4.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 2.2f),
        4 / 4.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 2.3f), 0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 2.5f),
        6 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 2.9f),
        6 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 3.3f),
        6 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 3.5f), 0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 3.6f),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 3.9f),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 6.1f),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, 6.2f), 0.f);
}

TEST_F(GenericHistogramTest, EstimateCardinalityAndPruningBasicString)
{
    const auto histogram =
        GenericHistogram<pmr_string>{{"aa", "bla", "uuu", "yyy"},
                                     {"birne", "ttt", "xxx", "zzz"},
                                     {3, 4, 4, 4},
                                     {3, 3, 3, 2},
                                     StringHistogramDomain{}};
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "a"), 0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "aa"),
        3 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "ab"),
        3 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "b"),
        3 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "birne"),
        3 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "biscuit"),
        0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "bla"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "blubb"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "bums"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "ttt"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "turkey"),
        0.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "uuu"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "vvv"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "www"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "xxx"),
        4 / 3.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "yyy"),
        4 / 2.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "zzz"),
        4 / 2.f);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::Equals, "zzzzzz"),
        0.f);
}
...
TEST_F(GenericHistogramTest, FloatLessThan)
{
    ...
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 0.5f),
        0.f);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 1.0f),
        (1.0f - 0.5f) /
            std::nextafter(2.2f - 0.5f,
                           std::numeric_limits<float>::infinity()) *
            4);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 1.7f),
        (1.7f - 0.5f) /
            std::nextafter(2.2f - 0.5f,
                           std::numeric_limits<float>::infinity()) *
            4);
    ...
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 2.5f),
        4.f);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 3.0f),
        4.f + (3.0f - 2.5f) /
                  std::nextafter(3.3f - 2.5f,
                                 std::numeric_limits<float>::infinity()) *
                  6);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 3.3f),
        4.f + (3.3f - 2.5f) /
                  std::nextafter(3.3f - 2.5f,
                                 std::numeric_limits<float>::infinity()) *
                  6);
    ...
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 3.6f),
        4.f + 6.f);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 3.9f),
        4.f + 6.f +
            (3.9f - 3.6f) /
                std::nextafter(6.1f - 3.6f,
                               std::numeric_limits<float>::infinity()) *
                4);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, 5.9f),
        4.f + 6.f +
            (5.9f - 3.6f) /
                std::nextafter(6.1f - 3.6f,
                               std::numeric_limits<float>::infinity()) *
...
{
    const auto histogram =
        GenericHistogram<pmr_string>{{"abcd", "ijkl", "oopp", "uvwx"},
                                     {"efgh", "mnop", "qrst", "yyzz"},
                                     {4, 6, 3, 3},
                                     {3, 3, 3, 3},
    ...
    // "abcd"
    const auto bin_1_lower =
        0 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        1 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        2 * (ipow(26, 1) + ipow(26, 0)) + 1 + 3 * (ipow(26, 0)) + 1;
    // "efgh"
    const auto bin_1_upper =
        4 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        5 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        6 * (ipow(26, 1) + ipow(26, 0)) + 1 + 7 * (ipow(26, 0)) + 1;
    // "ijkl"
    const auto bin_2_lower =
        8 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        9 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        10 * (ipow(26, 1) + ipow(26, 0)) + 1 + 11 * (ipow(26, 0)) + 1;
    // "mnop"
    const auto bin_2_upper =
        12 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        13 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        14 * (ipow(26, 1) + ipow(26, 0)) + 1 + 15 * (ipow(26, 0)) + 1;
    // "oopp"
    const auto bin_3_lower =
        14 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        14 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        15 * (ipow(26, 1) + ipow(26, 0)) + 1 + 15 * (ipow(26, 0)) + 1;
    // "qrst"
    const auto bin_3_upper =
        16 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        17 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        18 * (ipow(26, 1) + ipow(26, 0)) + 1 + 19 * (ipow(26, 0)) + 1;
    // "uvwx"
    const auto bin_4_lower =
        20 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        21 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        22 * (ipow(26, 1) + ipow(26, 0)) + 1 + 23 * (ipow(26, 0)) + 1;
    // "yyzz"
    const auto bin_4_upper =
        24 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        24 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
        25 * (ipow(26, 1) + ipow(26, 0)) + 1 + 25 * (ipow(26, 0)) + 1;

    const auto bin_1_width =
        (static_cast<float>(bin_1_upper - bin_1_lower) + 1.f);
    const auto bin_2_width =
        (static_cast<float>(bin_2_upper - bin_2_lower) + 1.f);
    const auto bin_3_width =
        (static_cast<float>(bin_3_upper - bin_3_lower) + 1.f);
    const auto bin_4_width =
        (static_cast<float>(bin_4_upper - bin_4_lower) + 1.f);

    constexpr auto bin_1_count = 4.f;
    constexpr auto bin_2_count = 6.f;
    constexpr auto bin_3_count = 3.f;
    constexpr auto bin_4_count = 3.f;
    constexpr auto total_count =
        bin_1_count + bin_2_count + bin_3_count + bin_4_count;

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "aaaa"),
        0.f);
    ...
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "abce"),
        1 / bin_1_width * bin_1_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "abcf"),
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            ...
            2 * (ipow(26, 1) + ipow(26, 0)) + 1 + 2 * (ipow(26, 0)) + 1 -
            bin_1_lower) /
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            3 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
            3 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
            3 * (ipow(26, 1) + ipow(26, 0)) + 1 + 3 * (ipow(26, 0)) + 1 -
            bin_1_lower) /
            bin_1_width * bin_1_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "efgg"),
        (bin_1_width - 2) / bin_1_width * bin_1_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "efgh"),
        (bin_1_width - 1) / bin_1_width * bin_1_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "efgi"),
        bin_1_count);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "ijkl"),
        bin_1_count);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "ijkm"),
        1 / bin_2_width * bin_2_count + bin_1_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "ijkn"),
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            ...
            9 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
            9 * (ipow(26, 1) + ipow(26, 0)) + 1 + 9 * (ipow(26, 0)) + 1 -
            bin_2_lower) /
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            ...
            10 * (ipow(26, 1) + ipow(26, 0)) + 1 + 10 * (ipow(26, 0)) + 1 -
            bin_2_lower) /
                bin_2_width * bin_2_count +
            bin_1_count);
    ...
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "mnoo"),
        (bin_2_width - 2) / bin_2_width * bin_2_count + bin_1_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "mnop"),
        (bin_2_width - 1) / bin_2_width * bin_2_count + bin_1_count);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "mnoq"),
        bin_1_count + bin_2_count);
    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "oopp"),
        bin_1_count + bin_2_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "oopq"),
        1 / bin_3_width * bin_3_count + bin_1_count + bin_2_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "oopr"),
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            ...
            16 * (ipow(26, 1) + ipow(26, 0)) + 1 + 16 * (ipow(26, 0)) + 1 -
            bin_3_lower) /
                bin_3_width * bin_3_count +
            bin_1_count + bin_2_count);
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            ...
            11 * (ipow(26, 1) + ipow(26, 0)) + 1 + 14 * (ipow(26, 0)) + 1 -
            bin_3_lower) /
                bin_3_width * bin_3_count +
            bin_1_count + bin_2_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "qrss"),
        (bin_3_width - 2) / bin_3_width * bin_3_count + bin_1_count +
            bin_2_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "qrst"),
        (bin_3_width - 1) / bin_3_width * bin_3_count + bin_1_count +
            bin_2_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "qrsu"),
        bin_1_count + bin_2_count + bin_3_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "uvwx"),
        bin_1_count + bin_2_count + bin_3_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "uvwy"),
        1 / bin_4_width * bin_4_count + bin_1_count + bin_2_count +
            bin_3_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "uvwz"),
        2 / bin_4_width * bin_4_count + bin_1_count + bin_2_count +
            bin_3_count);
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            ...
            21 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
            21 * (ipow(26, 1) + ipow(26, 0)) + 1 + 21 * (ipow(26, 0)) + 1 -
            bin_4_lower) /
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            ...
            23 * (ipow(26, 1) + ipow(26, 0)) + 1 + 23 * (ipow(26, 0)) + 1 -
            bin_4_lower) /
                bin_4_width * bin_4_count +
            bin_1_count + bin_2_count + bin_3_count);
    ...
    EXPECT_FLOAT_EQ(
        ...
        static_cast<float>(
            24 * (ipow(26, 3) + ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
            2 * (ipow(26, 2) + ipow(26, 1) + ipow(26, 0)) + 1 +
            8 * (ipow(26, 1) + ipow(26, 0)) + 1 + 15 * (ipow(26, 0)) + 1 -
            bin_4_lower) /
                bin_4_width * bin_4_count +
            bin_1_count + bin_2_count + bin_3_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "yyzy"),
        (bin_4_width - 2) / bin_4_width * bin_4_count + bin_1_count +
            bin_2_count + bin_3_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "yyzz"),
        (bin_4_width - 1) / bin_4_width * bin_4_count + bin_1_count +
            bin_2_count + bin_3_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "yz"),
        total_count);

    EXPECT_FLOAT_EQ(
        histogram.estimate_cardinality(PredicateCondition::LessThan, "zzzz"),
        total_count);
}

TEST_F(GenericHistogramTest, StringLikeEstimation)
{
    const auto histogram = GenericHistogram<pmr_string>::with_single_bin(
        "a", "z", 100, 40, StringHistogramDomain{'a', 'z', 4u});

    // (NOT) LIKE is not estimated and has a selectivity of 1
    EXPECT_FLOAT_EQ(
        histogram->estimate_cardinality(PredicateCondition::Like, "aa_zz%"),
        100.f);
    EXPECT_FLOAT_EQ(
        histogram->estimate_cardinality(PredicateCondition::NotLike, "aa_zz%"),
        100.f);
}
...
TEST_F(GenericHistogramTest, DoesNotContain)
{
  ...
  std::vector<HistogramCountType>{ 5,     3,                5,                 2,      1}};
    ...
    EXPECT_TRUE(
        histogram_a.does_not_contain(PredicateCondition::Equals, 22.5f));
    EXPECT_TRUE(
        histogram_a.does_not_contain(PredicateCondition::Equals, 33.0f));
    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::Equals, 24.0f));
    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::Equals, 32.0f));

    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::NotEquals, 1.0f));
    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::NotEquals, 22.5f));
    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::NotEquals, 33.0f));

    EXPECT_TRUE(
        histogram_a.does_not_contain(PredicateCondition::GreaterThan, 32.0f));
    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::GreaterThan, 31.0f));

    EXPECT_TRUE(histogram_a.does_not_contain(
        PredicateCondition::GreaterThanEquals, 32.1f));
    EXPECT_FALSE(histogram_a.does_not_contain(
        PredicateCondition::GreaterThanEquals, 32.0f));

    EXPECT_TRUE(
        histogram_a.does_not_contain(PredicateCondition::LessThan, 2.0f));
    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::LessThan, 22.5f));

    EXPECT_TRUE(
        histogram_a.does_not_contain(PredicateCondition::LessThanEquals, 1.9f));
    EXPECT_FALSE(
        histogram_a.does_not_contain(PredicateCondition::LessThanEquals, 2.0f));
}
...
TEST_F(GenericHistogramTest, EstimateCardinalityString)
{
  ...
  StringHistogramDomain{'a', 'z', 2u});
    ...
    auto estimate =
        histogram->estimate_cardinality(PredicateCondition::Equals, "a");
    ...
    estimate =
        histogram->estimate_cardinality(PredicateCondition::Equals, "ay");
    EXPECT_FLOAT_EQ(estimate, 0.f);
}
...
TEST_F(GenericHistogramTest, SliceReturnsNullptr)
{
    const auto histogram = std::make_shared<GenericHistogram<int32_t>>(
        std::vector<int32_t>{1}, std::vector<int32_t>{25},
        std::vector<HistogramCountType>{40},
        std::vector<HistogramCountType>{10});

    // Check that histogram returns a nullptr if predicate will not match any
    // data.
    EXPECT_EQ(histogram->sliced(PredicateCondition::LessThan, 1), nullptr);
    EXPECT_EQ(histogram->sliced(PredicateCondition::LessThanEquals, 0),
              nullptr);
    EXPECT_EQ(histogram->sliced(PredicateCondition::GreaterThanEquals, 26),
              nullptr);
...
TEST_F(GenericHistogramTest, SlicedFloat)
{
  ...
  const auto histogram_d = std::make_shared<GenericHistogram<float>>(
    ...
    std::vector<std::shared_ptr<AbstractHistogram<float>>> histograms{
        histogram_a, histogram_b, histogram_c, histogram_d};
    ...
    std::vector<Predicate> predicates{
        ...
        Predicate{PredicateCondition::NotEquals, next_value(2.0f),
                  std::nullopt},
        ...
        Predicate{PredicateCondition::LessThanEquals, previous_value(0.0f),
                  std::nullopt},
        Predicate{PredicateCondition::LessThanEquals, previous_value(1.0f),
                  std::nullopt},
        Predicate{PredicateCondition::LessThanEquals, 1.0f, std::nullopt},
        Predicate{PredicateCondition::LessThanEquals, previous_value(2.0f),
                  std::nullopt},
        Predicate{PredicateCondition::LessThanEquals, 2.0f, std::nullopt},
        Predicate{PredicateCondition::LessThanEquals, previous_value(6.0f),
                  std::nullopt},
        Predicate{PredicateCondition::LessThanEquals, previous_value(11.0f),
                  std::nullopt},
        Predicate{PredicateCondition::LessThanEquals, previous_value(12.25f),
                  std::nullopt},
        ...
        Predicate{PredicateCondition::LessThanEquals, 1'000'100.0f,
                  std::nullopt},
        ...
        Predicate{PredicateCondition::GreaterThan, previous_value(1'000'100.0f),
                  std::nullopt},
        ...
        Predicate{PredicateCondition::GreaterThanEquals, previous_value(1.0f),
                  std::nullopt},
        Predicate{PredicateCondition::GreaterThanEquals, previous_value(2.0f),
                  std::nullopt},
        ...
        Predicate{PredicateCondition::GreaterThanEquals,
                  previous_value(1'000'100.0f), std::nullopt},
        Predicate{PredicateCondition::GreaterThanEquals, 1'000'100.0f,
                  std::nullopt},
...
TEST_F(GenericHistogramTest, PrunedInt)
{
  ...
  const auto histogram = std::make_shared<GenericHistogram<int32_t>>(
    ...
    {
        // Prune all values from the first bin and half of the values from the
        // second bin
        const auto pruned = std::static_pointer_cast<GenericHistogram<int32_t>>(
    ...
    {
        // Prune three values from the last bin with the boundary being outside
        // of any bin
        const auto pruned = std::static_pointer_cast<GenericHistogram<int32_t>>(
    ...
    {
        // Prune eight values from the last two bins with the boundary being on
        // a single bin's value
        const auto pruned = std::static_pointer_cast<GenericHistogram<int32_t>>(
...

    {
        ...
        const auto pruned = std::static_pointer_cast<GenericHistogram<int32_t>>(
            histogram->pruned(14, PredicateCondition::BetweenInclusive, 41,
                              50));
        // clang-format off
    const auto expected_histogram = GenericHistogram<int32_t>{
      std::vector<int32_t>            { 1, 31, 60, 80},
    ...
    {
        // Prune by Between spanning three bins, covering half of the bins on
        // both ends
        const auto pruned = std::static_pointer_cast<GenericHistogram<int32_t>>(
            histogram->pruned(24, PredicateCondition::BetweenExclusive, 41,
                              89));
        // clang-format off
    const auto expected_histogram = GenericHistogram<int32_t>{
      std::vector<int32_t>            { 1, 31, 60, 80},
    ...
    {
        // Prune by Between spanning two bins, with the begin value being
        // outside of the histogram and the end value in the middle of the
        // second bin
        const auto pruned = std::static_pointer_cast<GenericHistogram<int32_t>>(
            histogram->pruned(12, PredicateCondition::BetweenLowerExclusive,
                              -10, 40));
        // clang-format off
    const auto expected_histogram = GenericHistogram<int32_t>{
      std::vector<int32_t>            { 1, 31,   60, 80},
    ...
    {
        // Prune from the first and second bin. Prune more values than the bins
        // contain.
        const auto pruned = std::static_pointer_cast<GenericHistogram<int32_t>>(
...
{
    // Same cardinalities as in PrunedInt, just with string values. Strings are
    // chosen so that their numeric representation is the same as the numbers
    // used in PrunedInt, too.
    // clang-format off
  const auto histogram = std::make_shared<GenericHistogram<pmr_string>>(std::vector<pmr_string>        { "a", "bc", "ce", "cy"},  // NOLINT
...

    {
        // Prune half of the values from the first bin
        const auto pruned =
            std::static_pointer_cast<GenericHistogram<pmr_string>>(
                histogram->pruned(20, PredicateCondition::GreaterThanEquals,
                                  "ay"));
...

    {
        ...
        const auto pruned =
            std::static_pointer_cast<GenericHistogram<pmr_string>>(
                histogram->pruned(14, PredicateCondition::BetweenInclusive,
                                  "bl", "bu"));
        // clang-format off
    const auto expected_histogram = GenericHistogram<pmr_string>{
      std::vector<pmr_string>        { "a", "bc", "ce", "cy"},
...

    {
        ...
        const auto pruned =
            std::static_pointer_cast<GenericHistogram<pmr_string>>(
                histogram->pruned(24, PredicateCondition::BetweenExclusive,
                                  "bm", "dg"));
        // clang-format off
    const auto expected_histogram = GenericHistogram<pmr_string>{
      std::vector<pmr_string>        { "a", "bc", "ce", "cy"},
    ...
    {
        // Same as the last test, but this time, the search values are longer
        // than the domain's common prefix. Remember that string_to_number adds
        // 1 for strings that are longer than the prefix.
        const auto pruned =
            std::static_pointer_cast<GenericHistogram<pmr_string>>(
                histogram->pruned(24, PredicateCondition::BetweenExclusive,
                                  "blaba", "dfqfqs"));
    ...
    {
        // Same as the last test, but this time, the search values contain
        // characters outside of the domain. cX should be converted to ca. This
        // way, the third bin should be included fully and the fourth bin
        // partially.
        const auto pruned =
            std::static_pointer_cast<GenericHistogram<pmr_string>>(
                histogram->pruned(15, PredicateCondition::BetweenExclusive,
                                  "cX", "dg"));
        // clang-format off
    const auto expected_histogram = GenericHistogram<pmr_string>{
      std::vector<pmr_string>        { "a", "bc", "ce", "cy"},
...
TEST_F(GenericHistogramTest, SplitAtEmptyBinBounds)
{
  ...
  const auto histogram = GenericHistogram<int32_t>(
    ...
    const auto new_hist = histogram.split_at_bin_bounds(
    ...
    for (auto bin_id = BinID{0}; bin_id < expected_minima.size(); bin_id++)
    {
        ...
        EXPECT_FLOAT_EQ(new_hist->bin_distinct_count(bin_id),
                        expected_distinct_counts[bin_id]);
    }
}
...
TEST_F(GenericHistogramTest, SplitAtBinBounds)
{
  ...
  const auto histogram = GenericHistogram<int32_t>(
    ...
    const auto expected_minima =
        std::vector<int32_t>{1, 10, 16, 30, 36, 60, 80};
    const auto expected_maxima =
        std::vector<int32_t>{9, 15, 25, 35, 50, 75, 100};
    const auto expected_heights = std::vector<HistogramCountType>{
        14.4f, 9.6f, 16.0f, 8.57143f, 21.42857f, 20.0f, 10};
    const auto expected_distinct_counts = std::vector<HistogramCountType>{
        3.6f, 2.4f, 4.0f, 5.7142859f, 14.285714f, 15, 5};

    const auto new_hist = histogram.split_at_bin_bounds(
        std::vector<std::pair<int32_t, int32_t>>{{10, 15}, {28, 35}});
...
TEST_F(GenericHistogramTest, SplitAtBinBoundsTwoHistograms)
{
  ...
  const auto histogram_2_expected_maxima = std::vector<int32_t>{   4, 7,     14, 18, 19, 25,         40, 42, 48, 51, 52};  // NOLINT
    ...
    const auto new_histogram_1 =
        histogram_1.split_at_bin_bounds(histogram_2.bin_bounds());
    const auto new_histogram_2 =
        histogram_2.split_at_bin_bounds(histogram_1.bin_bounds());
    ...
    for (auto bin_id = BinID{0}; bin_id < histogram_1_expected_minima.size();
         bin_id++)
    {
        EXPECT_EQ(new_histogram_1->bin_minimum(bin_id),
                  histogram_1_expected_minima[bin_id]);
        EXPECT_EQ(new_histogram_1->bin_maximum(bin_id),
                  histogram_1_expected_maxima[bin_id]);
    }

    for (auto bin_id = BinID{0}; bin_id < histogram_2_expected_minima.size();
         bin_id++)
    {
        EXPECT_EQ(new_histogram_2->bin_minimum(bin_id),
                  histogram_2_expected_minima[bin_id]);
        EXPECT_EQ(new_histogram_2->bin_maximum(bin_id),
                  histogram_2_expected_maxima[bin_id]);
    }
}
...
TEST_F(GenericHistogramTest, ScaledWithSelectivity)
{
  ...
  const auto histogram = GenericHistogram<int32_t>(
    ...
    const auto scaled_histogram_05 =
        std::dynamic_pointer_cast<AbstractHistogram<int32_t>>(
            scaled_statistics_object_05);
