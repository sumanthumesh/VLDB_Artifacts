...
template <typename T> class MinMaxFilterTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _in_between2 = static_cast<T>(
            _in_between +
            0.5 * (_max_value -
                   _in_between));       // value in between _in_between and max
...
template <> class MinMaxFilterTest<pmr_string> : public BaseTest
{
  protected:
    void SetUp() override
    {
        _values = pmr_vector<pmr_string>{"aa",     "bb",     "b", "bbbbba",
                                         "bbbbbb", "bbbbbc", "c"};
        _min_value = *std::min_element(std::begin(_values), std::end(_values));
        _max_value = *std::max_element(std::begin(_values), std::end(_values));
...
TEST_F(MinMaxFilterTestLike, CanPruneLike)
{
    ...
    // For the predicate condition of Like, we expect only values where the
    // lower_bound is bigger than the max value or the upper_bound is smaller or
    // equal to the min value to be prunable. In the following tests `b` would
    // be the min value and `c` the max.
    ...
    EXPECT_FALSE(
        filter->does_not_contain(PredicateCondition::Like, max_ascii_value));
    ...
    // For the predicate condition of NotLike, we expect only values where the
    // lower_bound is smaller or equal than the min value and the upper_bound is
    // bigger to the max value to be prunable. In the following tests `b` would
    // be the min value and `c` the max.
    ...
    const auto filter_equal_values =
        std::make_unique<MinMaxFilter<pmr_string>>("a", "a");
    EXPECT_TRUE(filter_equal_values->does_not_contain(
        PredicateCondition::NotLike, "a"));

    const auto filter_max_ascii = std::make_unique<MinMaxFilter<pmr_string>>(
        pmr_string(1, static_cast<char>(127)),
        pmr_string(1, static_cast<char>(127)));
    // The following test should make sure, that if the last character of the
    // upper bound would be an ASCII overflow, `does_not_contain` returns false.
    EXPECT_FALSE(filter_max_ascii->does_not_contain(PredicateCondition::NotLike,
                                                    max_ascii_value));
    EXPECT_FALSE(filter_max_ascii->does_not_contain(PredicateCondition::Like,
                                                    max_ascii_value));
    EXPECT_FALSE(filter_max_ascii->does_not_contain(PredicateCondition::NotLike,
                                                    max_ascii_value));

    // We use the ascii collation for min/max filters. This means that lower
    // case letters are considered larger than upper case letters. In this test
    // USA% is not pruned since t is a lower case value.
    const auto filter_max_lower_case =
        std::make_unique<MinMaxFilter<pmr_string>>("T", "t");
    EXPECT_FALSE(filter_max_lower_case->does_not_contain(
        PredicateCondition::Like, "USA%"));
}

using MixMaxFilterTypes = ::testing::Types<int, float, double, pmr_string>;
TYPED_TEST_SUITE(MinMaxFilterTest,
                 MixMaxFilterTypes, ); // NOLINT(whitespace/parens)

TYPED_TEST(MinMaxFilterTest, CanPruneOnBounds)
{
    auto filter = std::make_unique<MinMaxFilter<TypeParam>>(
        this->_values.front(), this->_values.back());
    ...
    // for the predicate condition of <, we expect only values smaller or equal
    // to the minimum to be prunable
    ...
    // for the predicate condition of <=, we expect only values smaller than the
    // minimum to be prunable
    ...
    // for the predicate condition of ==, we expect only values outside the
    // max/max range to be prunable
    ...
    EXPECT_FALSE(
        filter->does_not_contain(PredicateCondition::BetweenLowerExclusive,
                                 {this->_before_range}, {this->_min_value}));
    EXPECT_FALSE(
        filter->does_not_contain(PredicateCondition::BetweenLowerExclusive,
                                 {this->_min_value}, {this->_in_between}));
    EXPECT_FALSE(
        filter->does_not_contain(PredicateCondition::BetweenLowerExclusive,
                                 {this->_in_between}, {this->_in_between2}));
    EXPECT_FALSE(
        filter->does_not_contain(PredicateCondition::BetweenLowerExclusive,
                                 {this->_in_between2}, {this->_max_value}));
    EXPECT_TRUE(
        filter->does_not_contain(PredicateCondition::BetweenLowerExclusive,
                                 {this->_max_value}, {this->_after_range}));
    ...
    EXPECT_TRUE(filter->does_not_contain(PredicateCondition::BetweenExclusive,
                                         {this->_before_range},
                                         {this->_min_value}));
    EXPECT_FALSE(filter->does_not_contain(PredicateCondition::BetweenExclusive,
                                          {this->_min_value},
                                          {this->_in_between}));
    EXPECT_FALSE(filter->does_not_contain(PredicateCondition::BetweenExclusive,
                                          {this->_in_between},
    ...
    EXPECT_FALSE(filter->does_not_contain(PredicateCondition::IsNull,
                                          {this->_in_between}));
    EXPECT_FALSE(filter->does_not_contain(
        PredicateCondition::IsNull, {this->_min_value}, {this->_in_between}));
    EXPECT_FALSE(
        filter->does_not_contain(PredicateCondition::IsNotNull, NULL_VALUE));
    EXPECT_FALSE(filter->does_not_contain(PredicateCondition::IsNotNull,
                                          {this->_in_between}));
    EXPECT_FALSE(filter->does_not_contain(PredicateCondition::IsNotNull,
                                          {this->_min_value},
                                          {this->_in_between}));
...
TYPED_TEST(MinMaxFilterTest, Sliced)
{
    ...
    const auto filter = std::make_unique<MinMaxFilter<TypeParam>>(
        this->_values.front(), this->_values.back());
    ...
    new_filter =
        std::static_pointer_cast<MinMaxFilter<TypeParam>>(filter->sliced(
            PredicateCondition::GreaterThanEquals, this->_in_between));
    EXPECT_EQ(new_filter->min, this->_in_between);
    EXPECT_EQ(new_filter->max, this->_max_value);

    new_filter = std::static_pointer_cast<MinMaxFilter<TypeParam>>(
        filter->sliced(PredicateCondition::BetweenInclusive, this->_in_between,
                       this->_in_between2));
    EXPECT_EQ(new_filter->min, this->_in_between);
...
TYPED_TEST(MinMaxFilterTest, SliceWithPredicateReturnsNullptr)
{
    ...
    EXPECT_EQ(
        ...
        nullptr);
    EXPECT_NE(filter->sliced(PredicateCondition::LessThanEquals,
                             this->_values.front()),
              nullptr);
    EXPECT_NE(filter->sliced(PredicateCondition::GreaterThanEquals,
                             this->_values.back()),
              nullptr);
    EXPECT_EQ(
