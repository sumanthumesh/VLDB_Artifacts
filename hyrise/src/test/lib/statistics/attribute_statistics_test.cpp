...
TEST_F(AttributeStatisticsTest, SetStatisticsObject)
{
    ...
    attribute_statistics.set_statistics_object(
        GenericHistogram<int32_t>::with_single_bin(0, 100, 40, 20));
    EXPECT_NE(attribute_statistics.histogram, nullptr);

    attribute_statistics.set_statistics_object(
        std::make_shared<MinMaxFilter<int32_t>>(0, 100));
    EXPECT_NE(attribute_statistics.min_max_filter, nullptr);

    attribute_statistics.set_statistics_object(
        std::make_shared<RangeFilter<int32_t>>(
            std::vector{std::pair<int32_t, int32_t>(0, 100)}));
    EXPECT_NE(attribute_statistics.range_filter, nullptr);

    attribute_statistics.set_statistics_object(
        std::make_shared<NullValueRatioStatistics>(0.2f));
    EXPECT_NE(attribute_statistics.null_value_ratio, nullptr);

    attribute_statistics.set_statistics_object(
        std::make_shared<DistinctValueCount>(123));
...
TEST_F(AttributeStatisticsTest, Scaled)
{
    ...
    attribute_statistics.histogram =
        GenericHistogram<int32_t>::with_single_bin(0, 100, 40, 20);
    attribute_statistics.min_max_filter =
        std::make_shared<MinMaxFilter<int32_t>>(0, 100);
    attribute_statistics.range_filter = std::make_shared<RangeFilter<int32_t>>(
        std::vector{std::pair<int32_t, int32_t>(0, 100)});
    attribute_statistics.null_value_ratio =
        std::make_shared<NullValueRatioStatistics>(0.2f);
    ...
    const auto scaled_attribute_statistics =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            attribute_statistics.scaled(0.5f));
    ...
    EXPECT_EQ(scaled_attribute_statistics->range_filter->ranges.at(0).second,
              100);
...
TEST_F(AttributeStatisticsTest, Sliced)
{
    auto attribute_statistics = AttributeStatistics<int32_t>{};
    attribute_statistics.histogram =
        GenericHistogram<int32_t>::with_single_bin(1, 100, 40, 20);
    attribute_statistics.min_max_filter =
        std::make_shared<MinMaxFilter<int32_t>>(0, 100);
    attribute_statistics.range_filter = std::make_shared<RangeFilter<int32_t>>(
        std::vector{std::pair<int32_t, int32_t>(0, 100)});
    ...
    const auto sliced_attribute_statistics =
        std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(
            attribute_statistics.sliced(PredicateCondition::GreaterThanEquals,
                                        51));
    ...
    EXPECT_EQ(sliced_attribute_statistics->range_filter->ranges.at(0).first,
              51);
    EXPECT_EQ(sliced_attribute_statistics->range_filter->ranges.at(0).second,
              100);
