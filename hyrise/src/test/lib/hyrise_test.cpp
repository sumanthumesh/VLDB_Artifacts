...
{
  protected:
    // This wrapper method is needed to access the plugins vector since it is a
    // private member of PluginManager
...
TEST_F(HyriseTest, GetAndResetHyrise)
{
    ...
    // We need to do some honest work so that the commit id is actually
    // incremented
    ...
    const auto transaction_context =
        hyrise.transaction_manager.new_transaction_context(AutoCommit::No);
