...
TEST_F(PredicateReorderingTest, SameOrderingForStoredTable)
{
    std::shared_ptr<Table> table_a =
        load_table("resources/test_data/tbl/int_float4.tbl", ChunkOffset{2});
...
{
    /**
     * Check that Reordering predicates works if a predicate chain is both on
     * the left and right side of a node. This is particularly interesting
     * because the PredicateReorderingRule needs to re-attach the ordered chain
     * of predicates to the output (the cross node in this case). This test
     * checks whether the attachment happens as the correct input.
     *
     ...
     */
    auto table_0 = create_mock_node_with_statistics(
        MockNode::ColumnDefinitions{{DataType::Int, "a"}}, 100.0f,
        {GenericHistogram<int32_t>::with_single_bin(0, 100, 100.0f, 100.0f)});
    auto table_1 = create_mock_node_with_statistics(
        MockNode::ColumnDefinitions{{DataType::Int, "a"}}, 100.0f,
        {GenericHistogram<int32_t>::with_single_bin(0, 100, 100.0f, 100.0f)});
...
{
    /**
     * If a PredicateNode has multiple outputs, it should only be considered for
     * reordering with lower predicates.
     */
    /**
     *        ____Union___
     *       /            \
     ...
     *           Table
     *
     * Predicate_a has a lower selectivity than Predicate_b - but since
     * Predicate_b has two outputs, Predicate_a cannot be reordered since it
     * does not belong to the predicate chain (Predicate_b and Predicate_c).
     * However, Predicate_b and Predicate_c can be reordered inside their chain.
     */

    /**
     * The mocked table has one column of int32_ts with the value range 0..100
     */
    auto table_node = create_mock_node_with_statistics(
        MockNode::ColumnDefinitions{{DataType::Int, "a"}}, 100.0f,
        {GenericHistogram<int32_t>::with_single_bin(0, 100, 100.0f, 100.0f)});
