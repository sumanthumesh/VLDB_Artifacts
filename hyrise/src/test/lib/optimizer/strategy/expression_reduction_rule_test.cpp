...
class ExpressionReductionRuleTest : public StrategyBaseTest
{
  public:
    void SetUp() override
    {
        ...
        // Create two objects for each expression to make sure the algorithm
        // tests for expression equality, not for pointer equality
...
TEST_F(ExpressionReductionRuleTest, ReduceDistributivity)
{
  ...
  auto expression_j = std::shared_ptr<AbstractExpression>(and_(value_(1), or_(and_(a, b), and_(a2, b2))));
    ...
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_a),
              *and_(a, b));
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_b),
              *and_(b, a2));

    // (a AND c AND b) OR (d AND a AND e) -->
    // a AND (c AND b) OR (e AND d)
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_c),
              *and_(a, or_(and_(c, b), and_(e, d))));

    // (a AND b AND c) OR (a AND b) OR (a AND d AND b AND a) OR (b AND a AND e)
    // --> a AND b AND (c OR d OR e)
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_d),
              *and_(and_(a2, b), or_(or_(c, d), e)));

    // Expressions that aren't modified
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_e),
              *or_(or_(and_(a2, b), and_(c, d)), c));
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_f),
              *and_(and_(or_(a, b), a), b));
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_g),
              *a);
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_h),
              *and_(a, b));
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_i),
              *or_(a, b));

    // Reduction works recursively
    EXPECT_EQ(*ExpressionReductionRule::reduce_distributivity(expression_j),
              *and_(value_(1), and_(a, b)));
}

TEST_F(ExpressionReductionRuleTest, ReduceConstantExpression)
{
    auto expression_a =
        std::shared_ptr<AbstractExpression>(add_(1, add_(3, 4)));
    ExpressionReductionRule::reduce_constant_expression(expression_a);
    EXPECT_EQ(*expression_a, *value_(8));

    auto expression_b = std::shared_ptr<AbstractExpression>(
        not_equals_(in_(a, list_(1, 2, add_(5, 3), 4)), 0));
...
TEST_F(ExpressionReductionRuleTest, RewriteLikePrefixWildcard)
{
    ...
    auto expression_i =
        std::shared_ptr<AbstractExpression>(not_like_(s, "RED%"));
    ExpressionReductionRule::rewrite_like_prefix_wildcard(expression_i);
    EXPECT_EQ(*expression_i,
              *or_(less_than_(s, "RED"), greater_than_equals_(s, "REE")));

    auto expression_b =
        std::shared_ptr<AbstractExpression>(like_(concat_(s, s), "RED%"));
    ExpressionReductionRule::rewrite_like_prefix_wildcard(expression_b);
    EXPECT_EQ(*expression_b,
              *between_upper_exclusive_(concat_(s, s), "RED", "REE"));

    // Test LIKE patterns where a rewrite to BETWEEN UPPER EXCLUSIVE is NOT
    // possible
    ...
    auto expression_g =
        std::shared_ptr<AbstractExpression>(like_(s, "RE\x7F%"));
    ...
    auto expression_h =
        std::shared_ptr<AbstractExpression>(greater_than_(s, "RED%"));
...
{
    const auto table_definition = TableColumnDefinitions{
        {"a", DataType::Int, false}, {"b", DataType::Int, true}};
    ...
    {
        // SELECT SUM(a), COUNT(a), AVG(a) -> SUM(a), COUNT(a), SUM(a) /
        // COUNT(a) AS AVG(a)
        // clang-format off
    ...
    {
        // SELECT SUM(a), COUNT(*), AVG(a) -> SUM(a), COUNT(*), SUM(a) /
        // COUNT(*) AS AVG(a) as a is not NULLable
        // clang-format off
    ...
    {
        // SELECT SUM(a), COUNT(a), AVG(a) GROUP BY b -> SUM(a), COUNT(a),
        // SUM(a) / COUNT(a) AS AVG(a) GROUP BY b
        // clang-format off
    ...
    {
        // SELECT SUM(a), COUNT(b), AVG(a) stays unmodified as COUNT(b) is
        // unrelated
        // clang-format off
    ...
    {
        // SELECT SUM(a), COUNT(a) + 1, AVG(a) + 2 -> SUM(a), COUNT(a) + 1,
        // SUM(a) / COUNT(a) + 2 AS AVG(a) + 2
        // clang-format off
    ...
    {
        // SELECT SUM(a), COUNT(a) AS foo, AVG(a) AS bar -> SUM(a), COUNT(a) AS
        // foo, SUM(a) / COUNT(a) AS bar
