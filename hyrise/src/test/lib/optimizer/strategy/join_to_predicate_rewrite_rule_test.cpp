...
class JoinToPredicateRewriteRuleJoinModeTest
    : public JoinToPredicateRewriteRuleTest,
      public ::testing::WithParamInterface<JoinMode>
{
};

INSTANTIATE_TEST_SUITE_P(
    JoinToPredicateRewriteRuleJoinModeTestInstance,
    JoinToPredicateRewriteRuleJoinModeTest,
...
TEST_P(JoinToPredicateRewriteRuleJoinModeTest, PerformRewrite)
{
    ...
    key_constraints.emplace(std::set<ColumnID>{u->original_column_id},
                            KeyConstraintType::UNIQUE);
    key_constraints.emplace(std::set<ColumnID>{v->original_column_id},
                            KeyConstraintType::UNIQUE);
    node_b->set_key_constraints(key_constraints);

    const auto join_node = GetParam() == JoinMode::Cross
                               ? JoinNode::make(GetParam())
                               : JoinNode::make(GetParam(), equals_(a, u));
  ...
  ProjectionNode::make(expression_vector(b),
    ...
    const auto annotated_lqp =
        apply_rule(std::make_shared<ColumnPruningRule>(), lqp->deep_copy());
    const auto actual_lqp = apply_rule(rule, annotated_lqp);
    expected_lqp = std::static_pointer_cast<ProjectionNode>(apply_rule(
        std::make_shared<ColumnPruningRule>(), expected_lqp->deep_copy()));
...
TEST_F(JoinToPredicateRewriteRuleTest, Union)
{
    ...
    auto key_constraints = TableKeyConstraints{};
    key_constraints.emplace(
        TableKeyConstraint({u->original_column_id}, KeyConstraintType::UNIQUE));
    key_constraints.emplace(
        TableKeyConstraint({v->original_column_id}, KeyConstraintType::UNIQUE));
    node_b->set_key_constraints(key_constraints);
