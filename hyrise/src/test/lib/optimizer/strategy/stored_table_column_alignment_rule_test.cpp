  ...
  public:
    void SetUp() override
    {
        Hyrise::get().storage_manager.add_table(
            "t_a", load_table("resources/test_data/tbl/int_int_float.tbl",
                              ChunkOffset{1}));
        Hyrise::get().storage_manager.add_table(
        ...
        _stored_table_node_right = std::static_pointer_cast<StoredTableNode>(
            _stored_table_node_left->deep_copy());
...
TEST_F(StoredTableColumnAlignmentRuleTest, EqualTableEqualChunksEqualColumns)
{
    ...
    EXPECT_EQ(_stored_table_node_left->pruned_column_ids(),
              (std::vector{ColumnID{0}}));
    EXPECT_EQ(_stored_table_node_right->pruned_column_ids(),
              (std::vector{ColumnID{0}}));
    EXPECT_EQ(_stored_table_node_left->hash(),
              _stored_table_node_right->hash());
    EXPECT_EQ(*_stored_table_node_left, *_stored_table_node_right);
}

TEST_F(StoredTableColumnAlignmentRuleTest,
       EqualTableEqualChunksDifferentColumns)
{
...
TEST_F(StoredTableColumnAlignmentRuleTest,
       EqualTableDifferentChunksDifferentColumns)
{
    ...
    EXPECT_NE(_stored_table_node_left->hash(),
              _stored_table_node_right->hash());
    EXPECT_NE(*_stored_table_node_left, *_stored_table_node_right);
}

TEST_F(StoredTableColumnAlignmentRuleTest,
       DifferentTableEqualChunksDifferentColumns)
{
...
TEST_F(StoredTableColumnAlignmentRuleTest, CoverSubqueries)
{
    ...
    auto stn_subquery = std::static_pointer_cast<StoredTableNode>(
        _stored_table_node_left->deep_copy());
    auto column_c = lqp_column_(stn_subquery, ColumnID{2});
    auto projection_subquery =
        ProjectionNode::make(expression_vector(column_c), stn_subquery);
    auto subquery = lqp_subquery_(projection_subquery);

    auto projection_root =
        ProjectionNode::make(expression_vector(subquery), _union_node);
    ...
    // Prerequisites
    ASSERT_EQ(_stored_table_node_left->pruned_column_ids(),
              pruned_column_set_a);
    ASSERT_EQ(_stored_table_node_right->pruned_column_ids(),
              pruned_column_set_a);
    ASSERT_EQ(stn_subquery->pruned_column_ids(),
              pruned_column_set_a_b); // differs
