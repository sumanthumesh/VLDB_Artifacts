...
{
    // SELECT * FROM a WHERE a < 'foo' OR a >= 'fop' - should identify the
    // conditions as mutually exclusive and use SetOperationMode::All. The
    // expression could have been the result of the ExpressionReductionRule
    // being applied to `a NOT LIKE 'foo%'`.
...
{
    // SELECT * FROM a WHERE c < 'foo' OR c >= 'bar' - looks like
    // SplitUpNotLike, but the optimization does not apply
    // clang-format off
  const auto input_lqp =
...
{
    // SELECT * FROM a WHERE c < 'foo' OR d >= 'fop' - looks like
    // SplitUpNotLike, but the optimization does not apply
    // clang-format off
  const auto input_lqp =
...
{
    // SELECT * FROM a WHERE a < 5 OR b >= 8 - should identify the conditions as
    // mutually exclusive and use SetOperationMode::All
    // clang-format off
  const auto input_lqp =
...
TEST_F(PredicateSplitUpRuleTest,
       SplitUpComplexNestedConjunctionsAndDisjunctions)
{
    // SELECT * FROM (
    //   SELECT a, b FROM a WHERE a = b AND a = 3
    // ) WHERE ((a > 10 OR a < 8) AND (b <= 7 OR 11 = b)) OR ((a = 5 AND b > 7)
    // AND 13 = 13)
    // clang-format off
  const auto input_lqp =
