      ...
      auto lqp_c =
        AggregateNode::make(expression_vector(), expression_vector(max_(add_(_a_a, parameter_c))),
          ProjectionNode::make(expression_vector(add_(_a_a, parameter_c)), _stored_table_a));
            // clang-format on
            _subquery_c =
                lqp_subquery_(lqp_c, std::make_pair(ParameterID{0}, _a_a));
        }

        const auto parameter_a_a = correlated_parameter_(ParameterID{0}, _a_a);
        const auto subquery_lqp =
            PredicateNode::make(equals_(parameter_a_a, _b_a), _stored_table_b);
        _subquery =
            lqp_subquery_(subquery_lqp, std::make_pair(ParameterID{0}, _a_a));
    }

    inline static std::shared_ptr<Table> _table_a, _table_b, _table_c, _table_d,
        _table_e;
    std::shared_ptr<PredicatePlacementRule> _rule;
    std::shared_ptr<StoredTableNode> _stored_table_a, _stored_table_b,
        _stored_table_c, _stored_table_d, _stored_table_e;
...
{
    // We expect predicates to get pushed below UnionNode-based diamonds if they
    // continue to be evaluable.
    // clang-format off
...
{
    // Derived from SimpleDiamondPushdownTest. In this test, the diamond's
    // origin node is used by another LQP node, which is not part of the
    // diamond. As a result, the predicate pushdown must be blocked because it
    // would incorrectly filter the other LQP node not part of the diamond.
    // clang-format off
  const auto input_common_node =
  ProjectionNode::make(expression_vector(_a_a, _a_b, cast_(11, DataType::Float)),
    _stored_table_a);
...
{
    // Derived from SimpleDiamondPushdownTest. In this test, the diamond's
    // origin node is an AggregateNode which blocks one predicate from getting
    // pushed below the diamond.
    // clang-format off
  const auto input_common_node =
...
{
    // In this test, two predicates sit on top of two consecutive Union
    // diamonds. While both predicates can be pushed below the first diamond,
    // only one predicate can also be pushed below the second diamond.
    // clang-format off
  const auto input_common_node =
...
{
    // If the predicate cannot be pushed down and is effectively re-inserted at
    // the same position, make sure that its outputs are correctly restored.
    // clang-format off
...
{
    // We must stop pushing down predicates when reaching a node with multiple
    // outputs.
    // clang-format off
  const auto input_common_node =
    ProjectionNode::make(expression_vector(_a_a, _a_b, cast_(11, DataType::Float)),
...
TEST_F(PredicatePlacementRuleTest,
       AllowedValuePredicatePushdownThroughProjectionTest)
{
    // We can push `a > 4` under the projection because it does not depend on
    // the sub-query.
    // clang-format off
  const auto input_lqp =
...
TEST_F(PredicatePlacementRuleTest,
       AllowedColumnPredicatePushdownThroughProjectionTest)
{
    // We can push `a > b` under the projection because it does not depend on
    // the sub-query.
    // clang-format off
  const auto input_lqp =
  PredicateNode::make(greater_than_(_a_a, _a_b),
...
TEST_F(PredicatePlacementRuleTest,
       ForbiddenPredicatePushdownThroughProjectionTest)
{
    // We can't push `(SELECT ...) > a.b` under the projection because the
    // projection is responsible for the SELECT.
    // clang-format off
  const auto input_lqp =
...
{
    // In this test, the PredicatePlacementRule cannot push down the top two
    // predicates because of a predicate having multiple outputs (the barrier
    // node). The purpose of this test is to check whether a barrier node
    // becomes pushed down by the rule, if it is a predicate eligible for
    // pushdown.
  ...
  PredicateNode::make(greater_than_(_c_a, 150),
    ...
    // After the pushdown, the inner join should have the two outputs that
    // barrier_predicate_node previously had.
    const auto &inner_join = std::dynamic_pointer_cast<JoinNode>(
        actual_lqp->left_input()->left_input());
...
{
    /**
     * Test that if all columns required for the evaluation of a PredicateNode
     * are still available above a ProjectionNode, then the PredicateNode can be
     * pulled up
     */
    // clang-format off
  const auto input_lqp =
...
{
    /**
     * Test that if a ProjectionNode prunes columns necessary for the evaluation
     * of a PredicateNode, PredicateNodes can not be pulled above it
     */
    // clang-format off
  const auto input_lqp =
...
{
    /**
     * Test that SortNodes are treated as barriers and nothing is pulled up
     * above them
     */
    // clang-format off
  const auto input_lqp =
...
TEST_F(PredicatePlacementRuleTest,
       NoPullUpPastNodeWithMultipleOutputsNoPullUpPastUnion)
{
    /**
     * Test that Nodes with multiple outputs are treated as barriers, and so are
     * UnionNodes
     */
    // clang-format off
...
{
    // For now, the PredicatePlacementRule doesn't touch uncorrelated (think 6 >
    // (SELECT...)) predicates
...
{
    // SELECT * FROM d JOIN e on d.a = e.a WHERE (d.b = 1 AND e.a = 2) OR (d.b =
    // 2) should lead to (b = 1 OR b = 2) being created on the left side of the
    // join. We cannot filter the right side, because all tuples qualify for the
    // second part of the disjunction.
...
{
    // SELECT * FROM d JOIN e on d.a = e.a WHERE (d.b = 1 AND e.a = 10) OR (d.b
    // = 2 AND e.a = 3) should lead to (b = 1 OR b = 2) being created on the
    // left and (a = 10 OR a = 3) on the right side of the join
...
TEST_F(PredicatePlacementRuleTest, CreatePreJoinPredicateOnlyWhereBeneficial)
{
    // SELECT * FROM d JOIN e on d.a = e.a WHERE (d.b = 1 AND e.a < 10) OR (d.b
    // = 2 AND e.a = 3) should lead to (b = 1 OR b = 2) being created on the
    // left side of the join, but no predicate on the right side, as it only
...
{
    // Similar to the previous test, but we do not do anything (yet) because it
    // uses a non-inner join
...
{
    // SELECT * FROM a JOIN b on a.a = b.a WHERE (a.b = 1 AND ? = 2) OR (b.a = 2
    // AND ? = 1) should not lead to a pre-join being created, as we cannot make
    // any assumptions about the two predicates that do not belong to any table
