...
/**
 * We can't actually test much about the JoinOrderingRule, since it is highly
 * dependent on the underlying algorithms which are separately tested.
 */
...
class JoinOrderingRuleTest : public StrategyBaseTest
{
  public:
    void SetUp() override
    {
        ...
        // This test only makes sure THAT something gets reordered, not what the
        // result of this reordering is - so the stats are just dummies.
        const auto histogram =
            GenericHistogram<int32_t>::with_single_bin(1, 50, 20, 10);

        node_a = create_mock_node_with_statistics({{DataType::Int, "a"}}, 20,
                                                  {histogram});
        node_b = create_mock_node_with_statistics({{DataType::Int, "b"}}, 20,
                                                  {histogram});
        node_c = create_mock_node_with_statistics({{DataType::Int, "c"}}, 20,
                                                  {histogram});
        node_d = create_mock_node_with_statistics({{DataType::Int, "d"}}, 20,
                                                  {histogram});
...
{
    // Test that the JoinOrderingRule works when there are multiple parts in the
    // plan that need isolated optimization e.g., when there is a barrier in the
    // form of an outer join
