  ...
  public:
    void SetUp() override
    {
        ...
        ChunkEncoder::encode_all_chunks(
            Hyrise::get().storage_manager.get_table("a"));
...
// Same test as before, but placing the predicate with a high selectivity first,
// which does not trigger an index scan. The seoond predicate has a very low
// selectivity, but does not follow a stored table node.
