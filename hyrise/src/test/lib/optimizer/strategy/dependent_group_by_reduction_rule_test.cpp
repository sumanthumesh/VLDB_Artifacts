  ...
  public:
    void SetUp() override
    {
        ...
        TableColumnDefinitions column_definitions{
            {"column0", DataType::Int, false},
            {"column1", DataType::Int, false},
            {"column2", DataType::Int, false},
            {"column3", DataType::Int, false}};

        table_a = std::make_shared<Table>(column_definitions, TableType::Data,
                                          ChunkOffset{2}, UseMvcc::Yes);
        table_a->add_soft_key_constraint(
        ...
        table_b = std::make_shared<Table>(column_definitions, TableType::Data,
                                          ChunkOffset{2}, UseMvcc::Yes);
        table_b->add_soft_key_constraint(
            {{ColumnID{0}, ColumnID{1}}, KeyConstraintType::UNIQUE});
        ...
        table_c = std::make_shared<Table>(column_definitions, TableType::Data,
                                          ChunkOffset{2}, UseMvcc::Yes);
        table_c->add_soft_key_constraint(
            {{ColumnID{0}, ColumnID{2}}, KeyConstraintType::PRIMARY_KEY});
        ...
        table_d = std::make_shared<Table>(
            TableColumnDefinitions{{"column0", DataType::Int, false}},
            TableType::Data, ChunkOffset{2}, UseMvcc::Yes);
        ...
        table_e = std::make_shared<Table>(column_definitions, TableType::Data,
                                          ChunkOffset{2}, UseMvcc::Yes);
        table_e->add_soft_key_constraint(
            {{ColumnID{0}, ColumnID{1}}, KeyConstraintType::PRIMARY_KEY});
        table_e->add_soft_key_constraint(
            {{ColumnID{2}}, KeyConstraintType::UNIQUE});
    ...
    std::shared_ptr<StoredTableNode> stored_table_node_a, stored_table_node_b,
        stored_table_node_c, stored_table_node_d, stored_table_node_e;
...
TEST_F(DependentGroupByReductionRuleTest, SimpleCases)
{
    ...
    // Early out for LQP where table does not have a unique column combination.
    {
        const auto lqp = AggregateNode::make(
            expression_vector(column_d_0), expression_vector(sum_(column_d_0)),
            stored_table_node_d);
...
// Test that a removable column is removed when a unary UCC is present. Check
// for the restored column order.
...
// Test that a non-UCC column is not removed if the full UCC is not present in
// the group by list.
...
// Test adaption of multi-column but inconsecutive column order of the primary
// key columns (table_c with UCC {0,2}).
...
// Test whether we remove the correct columns after joining (one column of a can
// be moved, none of b). No projection added, as root already is a projection.
...
// Test that the plan stays the same (no alias, no projection) for a table with
// a primary key but no removable columns.
...
// A simple aggregate follows an optimized aggregate, column order of root node
// should not change. Thus, no projection.
TEST_F(DependentGroupByReductionRuleTest,
       SimpleAggregateFollowsAdaptedAggregate)
{
    // clang-format off
  const auto lqp =
...
// A sort follows an optimized aggregate. Operator following the aggregate does
// not change the column order itself, but aggregate does. Hence, we need to add
// a projection.
...
// When a primary key column is nullable after an outer join, check that we do
// not modify the aggregate.
...
// Check that we reduce using the shortest UCCs (in terms of the number of
// columns).
...
{
    // Since this is a special FD-scenario that can not be generated from
    // UniqueConstraints and StoredTableNodes at the moment, we have to use a
    // custom MockNode:
    auto mock_node =
...
TEST_F(DependentGroupByReductionRuleTest,
       RemoveSuperfluousDistinctAggregateSimple)
{
    // To guarantee distinct results for SELECT DISTINCT clauses, the
    // SQLTranslator adds an AggregateNode with the selected attributes as group
    // by columns. If the AggregateNode's input is already unique for these
    // columns, remove the entire node.

    // Basic case: Column is unique due to a constraint (e.g., it is a primary
    // key). Example query: SELECT DISTINCT column0 FROM table_a;
    {
        // clang-format off
    const auto lqp =
    AggregateNode::make(expression_vector(column_a_0), expression_vector(),
      stored_table_node_a);
        ...
        stored_table_node_a->set_pruned_column_ids(
    ...
    // More advanced case: Column is unique due to another operation (e.g., we
    // grouped by it before). Example query: SELECT DISTINCT column1,
    // MIN(column2) FROM table_a GROUP BY column1;
    {
        // clang-format off
    const auto lqp =
...
TEST_F(DependentGroupByReductionRuleTest,
       RemoveSuperfluousDistinctAggregateProjectColumns)
{
    // Remove the AggregateNode completely when it is used to for SELECT
    // DISTINCT on a unique column, but add a ProjectionNode to output only the
    // desired column.
    {
        // clang-format off
    const auto lqp =
    ...
    // All columns are part of the grouped columns, but the order changes. Thus,
    // we need a ProjectionNode that changes the order.
    {
        // clang-format off
    const auto lqp =
...
{
    // Do not remove the AggregateNode when the grouped column is not unique
    // (only a combination of the two columns column0 and column1 is unique).
    {
        // clang-format off
    const auto lqp =
    ...
    // Do not remove the AggregateNode when the grouped column is unique but
    // there are further aggregates.
    {
        // clang-format off
    const auto lqp =
