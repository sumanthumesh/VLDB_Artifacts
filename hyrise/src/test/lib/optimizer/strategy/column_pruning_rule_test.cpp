...
class ColumnPruningRuleTest : public StrategyBaseTest
{
  public:
    ...
    pruned(const std::shared_ptr<MockNode> node,
           const std::vector<ColumnID> &column_ids)
    {
        const auto pruned_node =
            std::static_pointer_cast<MockNode>(node->deep_copy());
...
TEST_F(ColumnPruningRuleTest, WithUnion)
{
    ...
    {
        SCOPED_TRACE(std::string{"union_mode: "} +
                     std::string{magic_enum::enum_name(union_mode)});
...
TEST_F(ColumnPruningRuleTest, ProjectionDoesNotRecompute)
{
  ...
  ProjectionNode::make(expression_vector(add_(add_(pruned_a, 2), 1)),
    ...
    // We can be sure that the top projection node does not recompute a+2
    // because a is not available.
...
{
    // Do not prune away input columns to ChangeMetaTable, ChangeMetaTable needs
    // them all.
...
{
    // Do not prune away the window function argument, the PARTITION BY columns,
    // and the ORDER BY columns.
...
{
    // Do not prune away the window the PARTITION BY columns and the ORDER BY
    // columns, but prune additional columns.
    auto frame_description = FrameDescription{
        FrameType::Range, FrameBound{0, FrameBoundType::Preceding, true},
        FrameBound{0, FrameBoundType::CurrentRow, false}};
    const auto window = window_(expression_vector(a), expression_vector(b),
  ...
  ProjectionNode::make(expression_vector(1),
    ...
    apply_rule(rule, lqp);
    EXPECT_EQ(node_abc->pruned_column_ids(),
              std::vector<ColumnID>{ColumnID{2}});
}

TEST_F(ColumnPruningRuleTest, AnnotatePrunableJoinInput)
{
    // Join inputs where no expressions are used later in the query plan should
    // be marked as prunable to enable further optimization, such as Join to
    // Semi-Join rewrite. We skip Semi- and Anti-Joins since their right input
    // is always prunable.
    for (const auto join_mode :
         {JoinMode::Inner, JoinMode::Left, JoinMode::Right, JoinMode::FullOuter,
          JoinMode::Cross})
    {
        for (const auto prunable_input_side :
             {LQPInputSide::Left, LQPInputSide::Right})
        {
            const auto join_node =
                join_mode == JoinMode::Cross
                    ? JoinNode::make(join_mode)
                    : JoinNode::make(join_mode, equals_(a, u));
            ...
            const auto projections = prunable_input_side == LQPInputSide::Left
                                         ? expression_vector(u, v)
                                         : expression_vector(a, b);
            const auto lqp = ProjectionNode::make(projections, join_node);

            apply_rule(rule, lqp);

            SCOPED_TRACE("With JoinMode::" +
