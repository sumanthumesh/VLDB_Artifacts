...
class SubqueryToJoinRuleTest : public StrategyBaseTest
{
  public:
    void SetUp() override
    {
        ...
        const auto string_histogram =
            GenericHistogram<pmr_string>::with_single_bin("a", "z", 100, 10);

        node_a = create_mock_node_with_statistics(
            {{DataType::Int, "a"}, {DataType::Int, "b"}, {DataType::Int, "c"}},
            100, {histogram, histogram, histogram});
        ...
        small_node = create_mock_node_with_statistics(
            {{DataType::Int, "a"}, {DataType::Int, "b"}}, 100,
            {histogram, histogram});
        small_node_a = small_node->get_column("a");

        large_node = create_mock_node_with_statistics(
            {{DataType::Int, "a"}, {DataType::Int, "b"}}, 10000,
            {histogram, histogram});
        large_node_a = large_node->get_column("a");

        lineitem = create_mock_node_with_statistics(
            {{DataType::Int, "extendedprice"},
             {DataType::Int, "quantity"},
             {DataType::Int, "partkey"},
             {DataType::String, "shipdate"},
             {DataType::Int, "suppkey"}},
            100,
            {histogram, histogram, histogram, string_histogram, histogram});
        ...
        nation = create_mock_node_with_statistics(
            {{DataType::Int, "nationkey"}, {DataType::Int, "name"}}, 100,
            {histogram, histogram});
        n_nationkey = nation->get_column("nationkey");
        n_name = nation->get_column("name");

        part = create_mock_node_with_statistics(
            {{DataType::Int, "partkey"},
             {DataType::String, "brand"},
             {DataType::String, "container"},
             {DataType::String, "name"}},
            100,
            {histogram, string_histogram, string_histogram, string_histogram});
        ...
        supplier = create_mock_node_with_statistics(
            {{DataType::Int, "suppkey"},
    ...
    std::shared_ptr<MockNode> node_a, node_b, node_c, node_d, node_e,
        small_node, large_node, lineitem, nation, part, partsupp, supplier;
    std::shared_ptr<LQPColumnExpression> a_a, a_b, a_c, b_a, b_b, c_a, d_a, d_b,
        d_c, e_a, e_b, e_c, small_node_a, large_node_a, l_extendedprice,
        l_quantity, l_partkey, l_shipdate, l_suppkey, n_nationkey, n_name,
        p_partkey, p_brand, p_container, p_name, ps_availqty, ps_partkey,
        ps_suppkey, s_suppkey, s_address, s_name, s_nationkey;
};

// HELPER FUNCTION TESTS

TEST_F(SubqueryToJoinRuleTest,
       AssessCorrelatedParameterUsageCountsNodesNotUsages)
{
    ...
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}, {ParameterID{1}, a_b}};
  ...
  PredicateNode::make(equals_(b_a, parameter0),
    ...
    const auto result = SubqueryToJoinRule::assess_correlated_parameter_usage(
        lqp, parameter_map);
    EXPECT_EQ(result, std::pair(true, size_t{2}));
}

TEST_F(SubqueryToJoinRuleTest,
       AssessCorrelatedParameterUsageIgnoresUnrelatedParameters)
{
...
TEST_F(SubqueryToJoinRuleTest,
       AssessCorrelatedParameterUsageFindsUsagesInSubqueries)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
  ...
  PredicateNode::make(exists_(lqp_subquery_(subquery_lqp)),
    ...
    const auto &[optimizable, _] =
        SubqueryToJoinRule::assess_correlated_parameter_usage(lqp,
                                                              parameter_map);
    EXPECT_FALSE(optimizable);
}

TEST_F(SubqueryToJoinRuleTest,
       AssessCorrelatedParameterUsageReportsUnoptimizableUsageInProjection)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       AssessCorrelatedParameterUsageReportsUnoptimizableUsageInJoin)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest, AdaptAggregateNode)
{
    ...
    const auto adapted_aggregate_node =
        SubqueryToJoinRule::adapt_aggregate_node(aggregate_node, {a_a});
    ...
    EXPECT_EQ(std::find(original_expressions.cbegin(),
                        original_expressions.cend(), a_b),
              original_expressions.cend());
    const auto adapted_aggregate_node_2 =
        SubqueryToJoinRule::adapt_aggregate_node(aggregate_node, {a_b});
    const auto &expressions = adapted_aggregate_node_2->output_expressions();
    EXPECT_NE(std::find(expressions.cbegin(), expressions.cend(), a_b),
              expressions.cend());
}

TEST_F(SubqueryToJoinRuleTest, AdaptAliasNode)
{
    const auto alias_node = AliasNode::make(
        expression_vector(a_a, a_a, a_b),
        std::vector<std::string>{"a_a", "alias_a_a", "alias_a_b"});
    const auto &original_expressions = alias_node->output_expressions();

    // no added duplicates, preserve multiple names for same column,
    const auto adapted_alias_node =
        SubqueryToJoinRule::adapt_alias_node(alias_node, {a_a});
    EXPECT_EQ(adapted_alias_node->output_expressions().size(), size_t{3});

    // no additional aliases
    const auto adapted_alias_node2 =
        SubqueryToJoinRule::adapt_alias_node(alias_node, {a_b});
    EXPECT_EQ(adapted_alias_node2->output_expressions().size(), size_t{3});

    // add if necessary
    EXPECT_EQ(std::find(original_expressions.cbegin(),
                        original_expressions.cend(), a_c),
              original_expressions.cend());
    const auto adapted_alias_node3 =
        SubqueryToJoinRule::adapt_alias_node(alias_node, {a_c});
    const auto &expressions = adapted_alias_node3->output_expressions();
    EXPECT_NE(std::find(expressions.cbegin(), expressions.cend(), a_c),
              expressions.cend());
}
...
TEST_F(SubqueryToJoinRuleTest, AdaptProjectionNode)
{
    const auto projection_node =
        ProjectionNode::make(expression_vector(a_a, a_a));
    const auto &original_expressions = projection_node->output_expressions();

    // no added duplicates, preserve original duplicates
    const auto adapted_projection_node =
        SubqueryToJoinRule::adapt_projection_node(projection_node, {a_a});
    ...
    EXPECT_EQ(std::find(original_expressions.cbegin(),
                        original_expressions.cend(), a_b),
              original_expressions.cend());
    const auto adapted_projection_node2 =
        SubqueryToJoinRule::adapt_projection_node(projection_node, {a_b});
...
TEST_F(SubqueryToJoinRuleTest,
       TryToExtractJoinPredicatesUnsupportedPredicateTypes)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};

    auto unsupported_predicates =
...
TEST_F(SubqueryToJoinRuleTest,
       TryToExtractJoinPredicatesNonEqualsPredicateBelowAggregate)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const auto predicate_node =
    ...
    const auto &[extracted_predicates, remaining_expression] =
        SubqueryToJoinRule::try_to_extract_join_predicates(
            predicate_node, {{ParameterID{0}, a_a}}, true);
    EXPECT_TRUE(extracted_predicates.empty());
    ASSERT_TRUE(remaining_expression);
...
TEST_F(SubqueryToJoinRuleTest,
       TryToExtractJoinPredicatesNonParameterSideMustBeAColumnExpression)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_b);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};

    const auto rejected_predicate_node =
        PredicateNode::make(equals_(add_(b_a, 2), parameter), node_b);
    const auto &[rejected_extracted, rejected_remaining] =
        SubqueryToJoinRule::try_to_extract_join_predicates(
            rejected_predicate_node, parameter_map, false);
  ...
  PredicateNode::make(equals_(add_(b_a, 2), parameter),
    ...
    const auto &[accepted_extracted, accepted_remaining] =
        SubqueryToJoinRule::try_to_extract_join_predicates(
            accepted_predicate_node, parameter_map, false);
...
TEST_F(SubqueryToJoinRuleTest, TryToExtractJoinPredicatesUnrelatedParameter)
{
    ...
    const auto predicate_node =
        PredicateNode::make(equals_(unrelated_parameter, b_a), node_b);
    const auto &[extracted_predicates, remaining_expression] =
        SubqueryToJoinRule::try_to_extract_join_predicates(
            predicate_node, parameter_map, false);
    EXPECT_TRUE(extracted_predicates.empty());
...
TEST_F(SubqueryToJoinRuleTest,
       TryToExtractJoinPredicatesHandlesAndedExpressions)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_b);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
    ...
    const auto expression =
        and_(equals_(parameter, b_a),
             and_(equals_(b_a, b_b), less_than_(b_b, parameter)));
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCanPullEqualsFromBelowAggregate)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCannotPullNonEqualsFromBelowAggregate)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCanPullFromBelowProjection)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCanPullFromBelowValidate)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCanPullFromBothSidesOfInnerJoin)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCanPullFromBothSidesOfCrossJoin)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCanPullFromNonNullProducingSidesOfOuterJoins)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
    ...
    const auto left_predicate_node =
        PredicateNode::make(greater_than_(b_a, parameter), node_b);
  ...
  const auto right_outer_expected_lqp = JoinNode::make(JoinMode::Right, join_predicate, left_predicate_node, node_c);
    ...
    const auto full_outer_result =
        SubqueryToJoinRule::pull_up_correlated_predicates(full_outer_lqp,
                                                          parameter_map);
    EXPECT_TRUE(full_outer_result.join_predicates.empty());
    EXPECT_LQP_EQ(full_outer_result.adapted_lqp, full_outer_expected_lqp);

    const auto left_outer_result =
        SubqueryToJoinRule::pull_up_correlated_predicates(left_outer_lqp,
                                                          parameter_map);
    ASSERT_EQ(left_outer_result.join_predicates.size(), 1);
    EXPECT_EQ(*left_outer_result.join_predicates.front(),
              *less_than_(a_a, b_a));
    EXPECT_LQP_EQ(left_outer_result.adapted_lqp, left_outer_expected_lqp);

    const auto right_outer_result =
        SubqueryToJoinRule::pull_up_correlated_predicates(right_outer_lqp,
                                                          parameter_map);
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCanPullFromLeftSideOfSemiAntiJoins)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
    ...
    JoinNode::make(join_mode, equals_(b_a, c_a),
      ...
      PredicateNode::make(less_than_equals_(c_a, parameter),
        ...
        const auto result = SubqueryToJoinRule::pull_up_correlated_predicates(
            lqp, parameter_map);
        ASSERT_EQ(result.join_predicates.size(), 1);
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesCannotPullFromBelowLimits)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesRemovesPullablePredicates)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       PullUpCorrelatedPredicatesDoesNotChangeOriginalLQPNodes)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const std::map<ParameterID, std::shared_ptr<AbstractExpression>>
        parameter_map = {{ParameterID{0}, a_a}};
...
TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateHandlesCorrelatedExists)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const auto subquery_lqp =
        PredicateNode::make(equals_(b_a, parameter), node_b);
    const auto subquery_expression =
        lqp_subquery_(subquery_lqp, std::pair{ParameterID{0}, a_a});
    const auto lqp = PredicateNode::make(exists_(subquery_expression), node_a);

    const auto input_info =
        SubqueryToJoinRule::is_predicate_node_join_candidate(*lqp);
...
TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateHandlesCorrelatedNotExists)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
    const auto subquery_lqp =
        PredicateNode::make(equals_(b_a, parameter), node_b);
    const auto subquery_expression =
        lqp_subquery_(subquery_lqp, std::pair{ParameterID{0}, a_a});
    const auto lqp =
        PredicateNode::make(not_exists_(subquery_expression), node_a);
...
TEST_F(SubqueryToJoinRuleTest, IsPredicateNodeJoinCandidateHandlesIn)
{
    ...
    const auto mapping = lqp_create_node_mapping(subquery_expression->lqp,
                                                 input_info->subquery->lqp);
...
TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateHandlesUncorrelatedNotIn)
{
    const auto subquery_lqp =
        ProjectionNode::make(expression_vector(b_a), node_b);
    const auto subquery_expression = lqp_subquery_(subquery_lqp);
    const auto lqp =
        PredicateNode::make(not_in_(a_a, subquery_expression), node_a);
...
TEST_F(SubqueryToJoinRuleTest, IsPredicateNodeJoinCandidateHandlesComparison)
{
    ...
    const auto subquery_expression = lqp_subquery_(subquery_lqp);
    const auto lqp =
        PredicateNode::make(less_than_(a_a, subquery_expression), node_a);
...
TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateRejectsCorrelatedNotIn)
{
    const auto parameter = correlated_parameter_(ParameterID{0}, a_a);
...
TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateRejectsInWithConstantList)
{
    // See #1546
    const auto lqp =
        PredicateNode::make(in_(a_a, list_(value_(1), value_(2))), node_a);
    const auto input_info =
        SubqueryToJoinRule::is_predicate_node_join_candidate(*lqp);
    EXPECT_FALSE(input_info);
}
...
TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateRejectsUncorrelatedExists)
{
    const auto subquery_expression = lqp_subquery_(node_b);
    for (const auto &predicate :
         {exists_(subquery_expression), not_exists_(subquery_expression)})
    {
...
TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateLeftInOperandMustBeAColumnExpression)
{
    // See #1547
    const auto subquery_lqp =
        ProjectionNode::make(expression_vector(b_a), node_b);
    ...
    EXPECT_FALSE(
        SubqueryToJoinRule::is_predicate_node_join_candidate(*reject_lqp));
  ...
  PredicateNode::make(predicate,
    ...
    EXPECT_TRUE(
        SubqueryToJoinRule::is_predicate_node_join_candidate(*accept_lqp));
}

TEST_F(SubqueryToJoinRuleTest,
       IsPredicateNodeJoinCandidateLeftComparisonOperandMustBeAColumnExpression)
{
    // See #1547
    const auto subquery_lqp =
        ProjectionNode::make(expression_vector(b_a), node_b);
...
{
    // SELECT * FROM d WHERE EXISTS (SELECT e.a AS b, e.b AS a FROM e WHERE e.b
    // = d.b)
...
{
    // SELECT * FROM d WHERE EXISTS (SELECT * FROM e WHERE e.b = d.b AND e.c <
    // d.c)
...
{
    // SELECT * FROM a WHERE a.a IN (SELECT b.a FROM b WHERE b.a IN (SELECT c.a
    // FROM c))
...
{
    // SELECT * FROM d WHERE d.a IN (SELECT e.a FROM e WHERE e.b = d.b AND e.c <
    // d.c)
...
{
    // SELECT * FROM d WHERE d.a > (SELECT SUM(e.a) FROM e WHERE e.b = d.b AND
    // e.c = d.c)
...
TEST_F(SubqueryToJoinRuleTest,
       SubqueryUsesConjunctionOfCorrelatedAndLocalPredicates)
{
    // SELECT * FROM d WHERE d.a > (SELECT SUM(e.a) FROM e WHERE e.b = d.b AND
    // e.a IN (1, 2, 3))
...
{
    // Optimize a pre-optimized LQP version of TPC-H 17, as the
    // SubqueryToJoinRule would receive it
...
{
    // Optimize a pre-optimized LQP version of TPC-H 20, as the
    // SubqueryToJoinRule would receive it
...
// The reformulation requires OR support in the MultiPredicateJoinOperator
// (#1580).
TEST_F(SubqueryToJoinRuleTest, NoRewriteOfOr)
{
    // SELECT * FROM d WHERE d.a IN (SELECT e.a FROM e WHERE e.b = d.b OR e.c <
    // d.c)
...
// The reformulation requires semi-/anti-join support in the SortMergeJoin
// operator (#1497).
...
{
    // SELECT * FROM a WHERE a.a > (SELECT SUM(a.a) FROM a WHERE a.b = a.b) +
    // (SELECT SUM(b.a) FROM b WHERE b.b = a.b)
