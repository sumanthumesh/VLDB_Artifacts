...
TEST_F(JoinToSemiJoinRuleTest, InnerJoinToSemiJoin)
{
  ...
  ProjectionNode::make(expression_vector(add_(a, 2)),
    ...
    static_cast<JoinNode &>(*lqp->left_input())
        .mark_input_side_as_prunable(LQPInputSide::Right);
...
{
    // Same as InnerJoinToSemiJoin, but with an additional join predicate that
...
{
    /**
     * Defines a multi-column UCC (column0, column1) and two inner join
     * predicates of type Equals covering these two columns. We expect to see a
     * semi join reformulation because the resulting unique column combination
     * matches the inner join's predicate expressions.
     */
    {
        auto column_definitions = TableColumnDefinitions{};
        column_definitions.emplace_back("column0", DataType::Int, false);
...
TEST_F(JoinToSemiJoinRuleTest, DoNotTouchInnerJoinWithoutMatchingUcc)
{
     ...
     * Based on the InnerJoinToSemiJoin test.
     *
     * We define a multi-column UCC (column0, column1), but only a single
     * Equals-predicate for the inner join (a == column0). Hence, the resulting
     * unique column combination does not match the expressions of the single
