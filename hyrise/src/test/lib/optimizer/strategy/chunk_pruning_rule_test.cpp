...
class ChunkPruningRuleTest : public StrategyBaseTest
{
  protected:
    void SetUp() override
    {
        ...
        auto compressed_table = load_table(
            "resources/test_data/tbl/int_float2.tbl", ChunkOffset{2});
        ChunkEncoder::encode_all_chunks(
            compressed_table, SegmentEncodingSpec{EncodingType::Dictionary});
        storage_manager.add_table("compressed", compressed_table);

        auto long_compressed_table = load_table(
            "resources/test_data/tbl/25_ints_sorted.tbl", ChunkOffset{25});
        ChunkEncoder::encode_all_chunks(
            long_compressed_table,
            SegmentEncodingSpec{EncodingType::Dictionary});
        storage_manager.add_table("long_compressed", long_compressed_table);

        auto run_length_compressed_table =
            load_table("resources/test_data/tbl/10_ints.tbl", ChunkOffset{5});
        ChunkEncoder::encode_all_chunks(
            run_length_compressed_table,
            SegmentEncodingSpec{EncodingType::RunLength});
        storage_manager.add_table("run_length_compressed",
                                  run_length_compressed_table);

        auto string_compressed_table =
            load_table("resources/test_data/tbl/string.tbl", ChunkOffset{3});
        ChunkEncoder::encode_all_chunks(
            string_compressed_table,
            SegmentEncodingSpec{EncodingType::Dictionary});
        storage_manager.add_table("string_compressed", string_compressed_table);

        auto fixed_string_compressed_table =
            load_table("resources/test_data/tbl/string.tbl", ChunkOffset{3});
        ChunkEncoder::encode_all_chunks(
            fixed_string_compressed_table,
            SegmentEncodingSpec{EncodingType::FixedStringDictionary});
        storage_manager.add_table("fixed_string_compressed",
                                  fixed_string_compressed_table);

        auto int_float4 = load_table("resources/test_data/tbl/int_float4.tbl",
                                     ChunkOffset{2});
        ChunkEncoder::encode_all_chunks(
            int_float4, SegmentEncodingSpec{EncodingType::Dictionary});
        ...
        storage_manager.add_table(
            "uncompressed", load_table("resources/test_data/tbl/int_float2.tbl",
                                       ChunkOffset{10}));
    }
...
TEST_F(ChunkPruningRuleTest, SimplePruningTest)
{
    ...
    const auto predicate_node = PredicateNode::make(
        greater_than_(lqp_column_(stored_table_node, ColumnID{0}), 200));
  ...
  const auto expected_histogram = GenericHistogram<int32_t>{
    ...
    const auto &column_statistics =
        dynamic_cast<AttributeStatistics<int32_t> &>(
            *stored_table_node->table_statistics->column_statistics[0]);
    const auto &actual_histogram =
        dynamic_cast<GenericHistogram<int32_t> &>(*column_statistics.histogram);
...
TEST_F(ChunkPruningRuleTest, SimpleChunkPruningTestWithColumnPruning)
{
    ...
    const auto predicate_node = PredicateNode::make(
        less_than_(lqp_column_(stored_table_node, ColumnID{1}), 400.0f));
    predicate_node->set_left_input(stored_table_node);
...
{
    // If a temporary table is used more than once, only prune for the
    // predicates that apply to all paths.
  ...
  UnionNode::make(SetOperationMode::All,
    ...
    const auto expected_chunk_ids =
        std::vector<ChunkID>{ChunkID{0}, ChunkID{2}};
    EXPECT_EQ(stored_table_node->pruned_chunk_ids(), expected_chunk_ids);
}

TEST_F(ChunkPruningRuleTest, MultipleOutputs2)
{
    // Similar to MultipleOutputs1, but b > 700 is now part of one of the
    // branches and can't be used for pruning anymore.
    const auto stored_table_node = StoredTableNode::make("int_float4");
...
TEST_F(ChunkPruningRuleTest, BetweenPruningTest)
{
    ...
    const auto predicate_node = PredicateNode::make(between_inclusive_(
        lqp_column_(stored_table_node, ColumnID{1}), 350.0f, 351.0f));
    predicate_node->set_left_input(stored_table_node);
...
TEST_F(ChunkPruningRuleTest, TwoOperatorPruningTest)
{
    ...
    const auto predicate_node_0 = PredicateNode::make(
        greater_than_(lqp_column_(stored_table_node, ColumnID{0}), 200));
    predicate_node_0->set_left_input(stored_table_node);

    const auto predicate_node_1 = PredicateNode::make(
        less_than_equals_(lqp_column_(stored_table_node, ColumnID{1}), 400.0f));
    predicate_node_1->set_left_input(predicate_node_0);

    const auto actual_lqp =
        StrategyBaseTest::apply_rule(_rule, predicate_node_1);

    EXPECT_EQ(actual_lqp, predicate_node_1);
    const auto expected_chunk_ids =
        std::vector<ChunkID>{ChunkID{0}, ChunkID{1}};
    EXPECT_EQ(stored_table_node->pruned_chunk_ids(), expected_chunk_ids);
}
...
TEST_F(ChunkPruningRuleTest, IntersectionPruningTest)
{
    ...
    const auto predicate_node_0 = PredicateNode::make(
        less_than_(lqp_column_(stored_table_node, ColumnID{0}), 10));
    predicate_node_0->set_left_input(stored_table_node);
...
TEST_F(ChunkPruningRuleTest, ComparatorEdgeCasePruningTest_GreaterThan)
{
    ...
    const auto predicate_node = PredicateNode::make(
        greater_than_(lqp_column_(stored_table_node, ColumnID{0}), 12345));
    predicate_node->set_left_input(stored_table_node);
...
TEST_F(ChunkPruningRuleTest, ComparatorEdgeCasePruningTest_Equals)
{
    ...
    const auto predicate_node = PredicateNode::make(
        equals_(lqp_column_(stored_table_node, ColumnID{1}), 458.7f));
    predicate_node->set_left_input(stored_table_node);
...
TEST_F(ChunkPruningRuleTest, RangeFilterTest)
{
    ...
    const auto predicate_node = PredicateNode::make(
        equals_(lqp_column_(stored_table_node, ColumnID{0}), 50));
    predicate_node->set_left_input(stored_table_node);
...
TEST_F(ChunkPruningRuleTest, LotsOfRangesFilterTest)
{
    ...
    const auto predicate_node = PredicateNode::make(
        equals_(lqp_column_(stored_table_node, ColumnID{0}), 2500));
    predicate_node->set_left_input(stored_table_node);
...
TEST_F(ChunkPruningRuleTest, RunLengthSegmentPruningTest)
{
    const auto stored_table_node =
        StoredTableNode::make("run_length_compressed");

    const auto predicate_node = PredicateNode::make(
        equals_(lqp_column_(stored_table_node, ColumnID{0}), 2));
    predicate_node->set_left_input(stored_table_node);
...
TEST_F(ChunkPruningRuleTest, GetTablePruningTest)
{
    ...
    const auto get_table_operator = std::dynamic_pointer_cast<GetTable>(
        lqp_translator.translate_node(stored_table_node));
...
TEST_F(ChunkPruningRuleTest, StringPruningTest)
{
    ...
    const auto predicate_node = PredicateNode::make(
        equals_(lqp_column_(stored_table_node, ColumnID{0}), "zzz"));
    predicate_node->set_left_input(stored_table_node);
...
{
    const auto stored_table_node =
        StoredTableNode::make("fixed_string_compressed");
...
TEST_F(ChunkPruningRuleTest, PrunePastJoinNodes)
{
  ...
  PredicateNode::make(less_than_(table_2_a, 10000),                              // Prune chunk 0 and 1 of table 2.
    ...
    EXPECT_EQ(stored_table_node_1->pruned_chunk_ids(),
              expected_chunk_ids_table_1);

    const auto expected_chunk_ids_table_2 =
        std::vector<ChunkID>{ChunkID{0}, ChunkID{1}, ChunkID{3}};
    EXPECT_EQ(stored_table_node_2->pruned_chunk_ids(),
              expected_chunk_ids_table_2);
}

TEST_F(ChunkPruningRuleTest, ValueOutOfRange)
{
    // Filters are not required to handle values out of their data type's range
    // and the ColumnPruningRule currently doesn't convert out-of-range values
    // into the type's range.
    // TODO(anybody) In the test LQP below, the ChunkPruningRule could convert
    // the -3'000'000'000 to MIN_INT (but ONLY
...
{
    // Predicates that have uncorrelated subqueries as arguments cannot be used
    // for pruning as we do not know the predicate values yet. However, such
    // predicates should not prevent us from pruning by other predicates.
    const auto stored_table_node_1 = StoredTableNode::make("compressed");
    const auto stored_table_node_2 = StoredTableNode::make("compressed");

    const auto subquery_1 = ProjectionNode::make(expression_vector(value_(1.1)),
                                                 DummyTableNode::make());
    const auto subquery_2 = subquery_1->deep_copy();
    const auto subquery_3 = ProjectionNode::make(expression_vector(value_(3.3)),
                                                 DummyTableNode::make());
...
{
    // Similar to PredicateWithUncorrelatedSubquery, we cannot use predicates
    // with correlated subqueries as arguments for pruning because their values
    // are not known before execution. However, pruning by other predicates is
    // still possible.
    const auto stored_table_node_1 = StoredTableNode::make("compressed");
    const auto stored_table_node_2 = StoredTableNode::make("int_float4");
...
TEST_F(ChunkPruningRuleTest, SetPrunableSubqueryScans)
{
  ...
  PredicateNode::make(greater_than_(stored_table_node_1_col_a, lqp_subquery_(subquery)),
    ...
    EXPECT_EQ(stored_table_node_1->prunable_subquery_predicates().front(),
              input_lqp);
}
