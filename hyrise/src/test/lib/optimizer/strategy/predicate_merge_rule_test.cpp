...
class PredicateMergeRuleTest : public StrategyBaseTest
{
  public:
    void SetUp() override
    {
        ...
        // Reducing the minimum_union_count so that plans are merged earlier,
        // making the test cases shorter
...
{
    // Pure predicate chains won't be merged since this is unlikely to improve
    // the performance.
    // clang-format off
  const auto input_lqp =
  PredicateNode::make(equals_(a_a, 5),
