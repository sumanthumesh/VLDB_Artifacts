  ...
  public:
    void SetUp() override
    {
        ...
        Hyrise::get().storage_manager.add_table(
            "nullable_table",
            load_table("resources/test_data/tbl/int_float_null_1.tbl",
                       ChunkOffset{2}));
        Hyrise::get().storage_manager.add_table(
            "table", load_table("resources/test_data/tbl/int_float4_or_1.tbl",
                                ChunkOffset{2}));
        nullable_table_node = StoredTableNode::make("nullable_table");
        table_node = StoredTableNode::make("table");
        nullable_table_node_column =
            lqp_column_(nullable_table_node, ColumnID{0});
    ...
    std::shared_ptr<LQPColumnExpression> mock_node_column,
        nullable_table_node_column, table_node_column;
...
TEST_F(NullScanRemovalRuleTest, PredicateIsNotNullExpression)
{
    ...
    const auto input_lqp =
        PredicateNode::make(equals_(mock_node_column, 42), mock_node);
...
TEST_F(NullScanRemovalRuleTest, PredicateConditionIsNotNull)
{
    ...
    const auto input_lqp =
        PredicateNode::make(is_null_(mock_node_column), mock_node);
    const auto expected_lqp = input_lqp->deep_copy();
    const auto actual_lqp = apply_rule(rule, input_lqp);
...
{
    // The rule can't apply where the predicate operand is not a LQP Column
    // expression.
...
{
    // The rule can't apply where the original node of the LQP Column expression
    // is not a storage table node.
    const auto input_lqp =
        PredicateNode::make(is_not_null_(mock_node_column), mock_node);
    const auto expected_lqp = input_lqp->deep_copy();
    const auto actual_lqp = apply_rule(rule, input_lqp);
...
TEST_F(NullScanRemovalRuleTest, TableColumnDefinitionIsNullable)
{
    ...
    const auto input_lqp = PredicateNode::make(
        is_not_null_(nullable_table_node_column), nullable_table_node);
    const auto expected_lqp = input_lqp->deep_copy();
    const auto actual_lqp = apply_rule(rule, input_lqp);
...
TEST_F(NullScanRemovalRuleTest, TableColumnDefinitionIsNotNullable)
{
    ...
    // 5. The original node of the LQP Column expression needs to be a storage
    // table node
    // 6. The column (referenced by the LQP Column expression) is not nullable
    const auto input_lqp =
        PredicateNode::make(is_not_null_(table_node_column), table_node);
