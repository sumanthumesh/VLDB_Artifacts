...
bool equals(
    const std::pair<boost::dynamic_bitset<>, boost::dynamic_bitset<>> &lhs,
    const std::pair<unsigned long, unsigned long> &rhs)
{ // NOLINT - doesn't like unsigned long
    Assert(lhs.first.size() == lhs.second.size() &&
               lhs.first.size() <= sizeof(unsigned long) * 8, // NOLINT
           "Bitset has too many bits for comparison");
    return lhs.first.to_ulong() == rhs.first &&
           lhs.second.to_ulong() == rhs.second;
}
} // namespace
...
/**
 * Test that the correct CCPs are enumerated for _very_ simple graphs and that
 * they are enumerated in the correct order
 */
