  ...
  public:
    void SetUp() override
    {
        ...
        node_b = MockNode::make(
            MockNode::ColumnDefinitions{{DataType::Int, "a"}}, "b");
        node_c = MockNode::make(
            MockNode::ColumnDefinitions{{DataType::Int, "a"}}, "c");
        node_d = MockNode::make(
            MockNode::ColumnDefinitions{{DataType::Int, "a"}}, "d");
...
{
    const auto edge_a_b =
        JoinGraphEdge{JoinGraphVertexSet{4u, 0b0011}, {equals_(a_a, b_a)}};
    const auto edge_b_c =
        JoinGraphEdge{JoinGraphVertexSet{4u, 0b0110},
                      {equals_(b_a, c_a), less_than_(b_a, c_a)}};
    const auto edge_b =
        JoinGraphEdge{JoinGraphVertexSet{4u, 0b0010}, {equals_(b_a, 3)}};
    const auto edge_c_d = JoinGraphEdge{JoinGraphVertexSet{4u, 0b1100}, {}};

    const auto join_graph = JoinGraph{{node_a, node_b, node_c, node_d},
    ...
    const auto predicates_ab_c =
        join_graph.find_join_predicates({4, 0b1100}, {4, 0b0011});
    ...
    const auto predicates_a_bcd =
        join_graph.find_join_predicates({4, 0b1110}, {4, 0b0001});
