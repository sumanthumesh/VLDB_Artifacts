  ...
  public:
    void SetUp() override
    {
        ...
        node_c = MockNode::make(
            MockNode::ColumnDefinitions{{DataType::Int, "a"}}, "c");
...
TEST_F(JoinGraphBuilderTest, Basic)
{
  ...
  PredicateNode::make(equals_(b_a, c_a),
    ...
    EXPECT_EQ(*join_graph->edges.at(1).predicates.at(1),
              *greater_than_(b_b, a_b));
}
...
TEST_F(JoinGraphBuilderTest, ComputedExpression)
{
  ...
  JoinNode::make(JoinMode::Inner, equals_(a_a, group_by_expression),
    ...
    EXPECT_EQ(*join_graph->edges.at(0).predicates.at(0),
              *equals_(a_a, group_by_expression));
...
{
    // Test that components in the join graph get merged with a cross join, even
    // if they are connected by a hyperedge DPccp needs the JoinGraph to be
    // connected without relying on the hyperedges
  ...
  PredicateNode::make(equals_(add_(a_a, b_a), c_a),
    ...
    EXPECT_EQ(*join_graph->edges.at(0).predicates.at(0),
              *equals_(add_(a_a, b_a), c_a));

    EXPECT_EQ(join_graph->edges.at(1).vertex_set, JoinGraphVertexSet(3, 0b011));
    ASSERT_EQ(join_graph->edges.at(1).predicates.size(), 1u);
    EXPECT_EQ(*join_graph->edges.at(1).predicates.at(0), *equals_(a_a, b_a));

    // The edge connecting the components of the JoinGraph can either be AC or
    // BC. Depending on the hash function used by the stdlib, either could
    // happen
...
TEST_F(JoinGraphBuilderTest, MultiPredicateJoin)
{
    const auto join_predicates =
        expression_vector(equals_(a_a, b_a), greater_than_(a_b, b_b),
  ...
  JoinNode::make(JoinMode::Inner, join_predicates,
    ...
    EXPECT_EQ(*join_graph->edges.at(0).predicates.at(0), *equals_(a_a, b_a));
    EXPECT_EQ(*join_graph->edges.at(0).predicates.at(1),
              *greater_than_(a_b, b_b));
    EXPECT_EQ(*join_graph->edges.at(0).predicates.at(2),
              *less_than_equals_(a_c, b_c));
}
