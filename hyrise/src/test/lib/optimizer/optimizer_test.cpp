  ...
  public:
    void SetUp() override
    {
        ...
        node_c = MockNode::make(
            MockNode::ColumnDefinitions{{DataType::Int, "u"}}, "node_c");
        u = node_c->get_column("u");

        subquery_lqp_a = LimitNode::make(to_expression(1), node_b);
        subquery_a = lqp_subquery_(subquery_lqp_a);
        const auto correlated_parameter =
            correlated_parameter_(ParameterID{0}, b);
        // clang-format off
    subquery_lqp_b =
...
TEST_F(OptimizerTest, VerifiesResults)
{
    ...
    // While the Asserts* tests checked the different features of validate_lqp,
    // this test checks that a rule that breaks an LQP throws an assertion.
    // clang-format off
  auto lqp =
  ProjectionNode::make(expression_vector(add_(b, subquery_a)),
    PredicateNode::make(greater_than_(a, subquery_b),
    ...
    {
      public:
        explicit LQPBreakingRule(
            const std::shared_ptr<AbstractExpression> &out_of_plan_expression)
      ...
      protected:
        ...
        {
            // Change the `b` expression in the projection to `u`, which is not
            // part of the input LQP.
            const auto projection_node =
                std::dynamic_pointer_cast<ProjectionNode>(
                    lqp_root->left_input());
            if (!projection_node)
            {
                return;
...
TEST_F(OptimizerTest, OptimizesSubqueries)
{
    ...
    class MockRule : public AbstractRule
    {
      public:
        explicit MockRule(
            std::unordered_set<std::shared_ptr<AbstractLQPNode>> &init_nodes)
            : nodes(init_nodes)
        {
        }
  ...
  ProjectionNode::make(expression_vector(add_(b, subquery_a)),
    ...
    // Test that the optimizer has reached all nodes (the number includes all
    // nodes created above and the root nodes created by the optimizer for the
    // lqp and each subquery)
...
{
    /**
     * This one is important. An LQP can contain the same Subquery multiple
     * times (see the LQP constructed below). We need make sure that each LQP is
     * only optimized once and that all SubqueryExpressions point to the same
     * LQP after optimization.
     *
     * This is not just "nice to have". If we do not do this properly and
     * multiple SubqueryExpressions point to the same LQP then we would
     * potentially break all the other SubqueryExpressions while moving nodes
     * around when optimizing a single one of them.
     */
  ...
  PredicateNode::make(greater_than_(add_(b, subquery_a), 2),
    ...
    /**
     * 1. Optional: Deep copy the LQP, thereby making its SubqueryExpressions
     * point to different copies of the same LQP (The optimizer should
     * incidentally make them point to the same LQP again)
     */
    lqp = lqp->deep_copy();
    {
        ...
        auto projection_node =
            std::dynamic_pointer_cast<ProjectionNode>(lqp->left_input());
        ASSERT_TRUE(projection_node);
        auto subquery_a_b = std::dynamic_pointer_cast<LQPSubqueryExpression>(
            projection_node->node_expressions.at(0)->arguments.at(1));
    ...
    // to check whether SubqueryExpressions pointing to the same LQP before
    // optimization still point to the same (though deep_copy()ed) LQP
    // afterwards
    size_t counter{0};
    ...
    /**
     * 3. Check that the rule was invoked 3 times. Once for the main LQP, once
     * for subquery_a and once for subquery_b
     */
    EXPECT_EQ(counter, 3u);

    /**
     * 4. Check that now - after optimizing - both SubqueryExpressions using
     * subquery_lqp_a point to the same LQP object again
     */
    {
        auto predicate_node_a =
            std::dynamic_pointer_cast<PredicateNode>(optimized_lqp);
        ASSERT_TRUE(predicate_node_a);
        auto subquery_a_a = std::dynamic_pointer_cast<LQPSubqueryExpression>(
            predicate_node_a->predicate()->arguments.at(0)->arguments.at(1));
        ASSERT_TRUE(subquery_a_a);
        auto projection_node = std::dynamic_pointer_cast<ProjectionNode>(
            optimized_lqp->left_input());
        ASSERT_TRUE(projection_node);
        auto subquery_a_b = std::dynamic_pointer_cast<LQPSubqueryExpression>(
            projection_node->node_expressions.at(0)->arguments.at(1));
        ASSERT_TRUE(subquery_a_b);
        auto predicate_node_b = std::dynamic_pointer_cast<PredicateNode>(
            optimized_lqp->left_input()->left_input());
        ASSERT_TRUE(predicate_node_b);
        auto subquery_b_a = std::dynamic_pointer_cast<LQPSubqueryExpression>(
            predicate_node_b->predicate()->arguments.at(1));
