...
TEST_F(LosslessCastTest, IdenticalSourceAndTarget)
{
    ...
    EXPECT_EQ(lossless_cast<int64_t>(int64_t(5'000'000'000)),
              int64_t(5'000'000'000));
    EXPECT_EQ(lossless_cast<float>(3.2f), 3.2f);
    EXPECT_EQ(lossless_cast<double>(3.2), 3.2);
    EXPECT_EQ(lossless_cast<pmr_string>(pmr_string{"hello"}),
              pmr_string("hello"));

    // Cannot check for NULL == NULL, as that's not TRUE. So simply check
    // whether non-std::nullopt was returned.
...
TEST_F(LosslessCastTest, IntegralToFloatingPoint)
{
    ...
    EXPECT_EQ(lossless_cast<float>(int64_t(20'123'456'789'012'345ll)),
              std::nullopt);
    EXPECT_EQ(lossless_cast<double>(int64_t(20'123'456'789'012'345ll)),
              std::nullopt);
}
...
TEST_F(LosslessCastTest, FloatingPointToIntegral)
{
    ...
    // The _float literal_ and the integer are obviously not the same, but the
    // _stored float_ and the integer are.
    ...
    // The _float literal_ and the integer are obviously not the same, but the
    // _stored float_ and the integer are.
    EXPECT_EQ(lossless_cast<int64_t>(1234567890123456.0f),
              1'234'567'948'140'544);

    // Test the largest double losslessly convertible to int64 - as well as the
    // next bigger/smaller double, which is not representable anymore
    EXPECT_EQ(lossless_cast<int64_t>(9'223'372'036'854'774'784.0),
              9'223'372'036'854'774'784ll);
    EXPECT_EQ(lossless_cast<int64_t>(9'223'372'036'854'775'808.0),
              std::nullopt);
    EXPECT_EQ(lossless_cast<int64_t>(-9'223'372'036'854'775'808.0),
              std::numeric_limits<int64_t>::min());
    EXPECT_EQ(lossless_cast<int64_t>(-9'223'372'036'854'777'856.0),
              std::nullopt);
    ...
    EXPECT_EQ(lossless_cast<int32_t>(std::numeric_limits<float>::quiet_NaN()),
              std::nullopt);
    EXPECT_EQ(lossless_cast<int32_t>(std::numeric_limits<double>::quiet_NaN()),
              std::nullopt);
    EXPECT_EQ(lossless_cast<int32_t>(std::numeric_limits<float>::infinity()),
              std::nullopt);
    EXPECT_EQ(lossless_cast<int32_t>(std::numeric_limits<double>::infinity()),
              std::nullopt);
}
...
TEST_F(LosslessCastTest, FloatingPointToDifferentFloatingPoint)
{
    ...
    // Maximum double that can losslessly represented as a float and the next
    // higher double
    EXPECT_EQ(lossless_cast<float>(340282346638528859811704183484516925440.0),
              std::numeric_limits<float>::max());
    EXPECT_EQ(lossless_cast<float>(340282346638528897590636046441678635008.0),
              std::nullopt);

    // Minimum double that can losslessly represented as a float and the next
    // lower double
    EXPECT_EQ(lossless_cast<float>(-340282346638528859811704183484516925440.0),
...
TEST_F(LosslessCastTest, VariantCastSafe)
{
    ...
    EXPECT_EQ(lossless_variant_cast(2.0f, DataType::Int),
              AllTypeVariant{int32_t{2}});
    EXPECT_EQ(lossless_variant_cast(2.5f, DataType::Int), std::nullopt);
    EXPECT_EQ(lossless_variant_cast(int32_t{2}, DataType::Float),
              AllTypeVariant{2.0f});
