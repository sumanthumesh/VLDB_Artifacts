...
class JoinNodeTest : public BaseTest
{
  protected:
    ...
    {
        _mock_node_a =
            MockNode::make(MockNode::ColumnDefinitions{{DataType::Int, "a"},
                                                       {DataType::Int, "b"},
                                                       {DataType::Int, "c"}},
                           "t_a");
        _mock_node_b =
            MockNode::make(MockNode::ColumnDefinitions{{DataType::Int, "x"},
                                                       {DataType::Float, "y"}},
                           "t_b");
        ...
        _inner_join_node =
            JoinNode::make(JoinMode::Inner, equals_(_t_a_a, _t_b_y),
                           _mock_node_a, _mock_node_b);
        _semi_join_node =
            JoinNode::make(JoinMode::Semi, equals_(_t_a_a, _t_b_y),
                           _mock_node_a, _mock_node_b);
        _semi_join_reduction_node =
            JoinNode::make(JoinMode::Semi, equals_(_t_a_a, _t_b_y),
                           _mock_node_a, _mock_node_b);
        _semi_join_reduction_node->mark_as_semi_reduction(_inner_join_node);
        _anti_join_node =
            JoinNode::make(JoinMode::AntiNullAsTrue, equals_(_t_a_a, _t_b_y),
                           _mock_node_a, _mock_node_b);

        // Prepare constraint definitions.
        _key_constraint_a = TableKeyConstraint{{_t_a_a->original_column_id},
                                               KeyConstraintType::UNIQUE};
        _key_constraint_b_c = TableKeyConstraint{
            {_t_a_b->original_column_id, _t_a_c->original_column_id},
            KeyConstraintType::UNIQUE};
        _key_constraint_x = TableKeyConstraint{{_t_b_x->original_column_id},
                                               KeyConstraintType::UNIQUE};
        _key_constraint_y = TableKeyConstraint{{_t_b_y->original_column_id},
                                               KeyConstraintType::UNIQUE};
    }

    std::shared_ptr<MockNode> _mock_node_a, _mock_node_b, _mock_node_c;
    std::shared_ptr<JoinNode> _inner_join_node, _semi_join_node,
        _semi_join_reduction_node, _anti_join_node, _cross_join_node;
    std::shared_ptr<LQPColumnExpression> _t_a_a, _t_a_b, _t_a_c, _t_b_x, _t_b_y,
        _t_b_z;
    std::optional<TableKeyConstraint> _key_constraint_a, _key_constraint_b_c,
        _key_constraint_x, _key_constraint_y;
};

class JoinNodeMultiJoinModeTest : public JoinNodeTest,
                                  public ::testing::WithParamInterface<JoinMode>
{
};

INSTANTIATE_TEST_SUITE_P(
    JoinNodeMultiJoinModeTestInstance, JoinNodeMultiJoinModeTest,
    ::testing::ValuesIn(magic_enum::enum_values<JoinMode>()),
    enum_formatter<JoinMode>);
...
{
    EXPECT_EQ(_anti_join_node->description(),
              "[Join] Mode: AntiNullAsTrue [a = y]");
}
...
TEST_F(JoinNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto other_join_node_a = JoinNode::make(
        JoinMode::Inner, equals_(_t_a_a, _t_b_x), _mock_node_a, _mock_node_b);
    const auto other_join_node_b = JoinNode::make(
        JoinMode::Inner, not_like_(_t_a_a, _t_b_y), _mock_node_a, _mock_node_b);
    const auto other_join_node_c =
        JoinNode::make(JoinMode::Cross, _mock_node_a, _mock_node_b);
    const auto other_join_node_d = JoinNode::make(
        JoinMode::Inner, equals_(_t_a_a, _t_b_y), _mock_node_a, _mock_node_b);
...
TEST_F(JoinNodeTest, Copy)
{
    ...
    EXPECT_EQ(*_semi_join_reduction_node,
              *_semi_join_reduction_node->deep_copy());
...
TEST_F(JoinNodeTest, NodeExpressions)
{
    ...
    EXPECT_EQ(*_inner_join_node->node_expressions.at(0u),
              *equals_(_t_a_a, _t_b_y));
    ASSERT_EQ(_cross_join_node->node_expressions.size(), 0u);
}

TEST_F(JoinNodeTest, IsColumnNullableWithoutOuterJoin)
{
    // Test that for LQPs without (Left,Right)Outer Joins,
    // lqp_column_is_nullable() is equivalent to expression.is_nullable()
...
{
    // Test that columns on the "null-supplying" side of an outer join are
    // always nullable. Test that is_null_(<nullable>) is never nullable
...
TEST_P(JoinNodeMultiJoinModeTest, FunctionalDependenciesForwardNonTrivialLeft)
{
    ...
    {
        join_node = JoinNode::make(join_mode, equals_(_t_a_a, _t_b_x),
                                   _mock_node_a, _mock_node_b);
    }
    ...
    else
    {
        ...
        EXPECT_TRUE(
            join_node->non_trivial_functional_dependencies().contains(fd_a));
    }
}
...
TEST_P(JoinNodeMultiJoinModeTest, FunctionalDependenciesForwardNonTrivialRight)
{
    ...
    case JoinMode::Cross:
        EXPECT_EQ(join_node->non_trivial_functional_dependencies().size(), 1);
        EXPECT_TRUE(
            join_node->non_trivial_functional_dependencies().contains(fd_x));
    }
}
...
TEST_P(JoinNodeMultiJoinModeTest,
       FunctionalDependenciesForwardNonTrivialBothAndDerive)
{
    const auto join_mode = GetParam();
    auto join_node = std::shared_ptr<JoinNode>();
    if (join_mode == JoinMode::Cross)
    ...
    /**
     * Test whether non-trivial FD forwarding still works when deriving FDs from
     * unique column combinations:
     *  - We specify UCCs for both input tables.
     *  - We enforce the dismissal of UCCs for all join modes by making none of
     * the join columns unique. Consequently, we expect non-trivial FDs that
     * were derived from the input nodes' unique column combinations.
     *  - Semi- and Anti-Joins preserve the UCCs, so they should not generate
     * non-trivial FDs but keep the left input's UCCs of the left input.
     */
    const auto fd_a = FunctionalDependency{{_t_a_a}, {_t_a_b}};
    const auto fd_x = FunctionalDependency{{_t_b_x}, {_t_b_y}};
    _mock_node_a->set_non_trivial_functional_dependencies({fd_a});
    ...
    const auto generated_fd_b_c =
        FunctionalDependency{{_t_a_b, _t_a_c}, {_t_a_a}};
    ...
    case JoinMode::AntiNullAsFalse:
        ...
        EXPECT_EQ(join_node->unique_column_combinations(),
                  _mock_node_a->unique_column_combinations());
        break;
    case JoinMode::Left:
        EXPECT_EQ(join_node->non_trivial_functional_dependencies().size(), 2);
        EXPECT_TRUE(
            join_node->non_trivial_functional_dependencies().contains(fd_a));
        EXPECT_TRUE(join_node->non_trivial_functional_dependencies().contains(
            generated_fd_b_c));
        break;
    case JoinMode::Right:
        EXPECT_EQ(join_node->non_trivial_functional_dependencies().size(), 2);
...
{
    /**
     * Set UCCs for both join columns of an Inner Join, so that UCCs from both
     * sides are forwarded. Consequently, we do not expect non-trivial FDs from
     * the left or right input nodes' UCCs to be derived.
     */
  ...
  JoinNode::make(JoinMode::Inner, equals_(_t_a_a, _t_b_x),
    ...
    EXPECT_EQ(join_node->non_trivial_functional_dependencies().size(), 2);
    EXPECT_TRUE(
        join_node->non_trivial_functional_dependencies().contains(fd_b));
    EXPECT_TRUE(
        join_node->non_trivial_functional_dependencies().contains(fd_y));
    ...
    const auto generated_fd_a =
        FunctionalDependency{{_t_a_a}, {_t_a_b, _t_a_c, _t_b_x, _t_b_y}};
    EXPECT_TRUE(join_node->functional_dependencies().contains(generated_fd_a));
    const auto generated_fd_x =
        FunctionalDependency{{_t_b_x}, {_t_b_y, _t_a_a, _t_a_b, _t_a_c}};
...
{
    /**
     * We set a UCC for the left, but not for the right join column of the Inner
     * Join. Consequently, UCCs of the left input node are discarded, whereas
     * the UCCs of the right input node survive. Therefore, we have to check
     * whether left input node's trivial FDs are forwarded as non-trivial ones.
     */
    _mock_node_a->set_key_constraints({*_key_constraint_a});
    _mock_node_b->set_key_constraints({*_key_constraint_x});
    // clang-format off
  ...
  JoinNode::make(JoinMode::Inner, equals_(_t_a_a, _t_b_y),
    ...
    // Tests
    const auto generated_fd_a =
        FunctionalDependency{{_t_a_a}, {_t_a_b, _t_a_c}};
    EXPECT_EQ(join_node->non_trivial_functional_dependencies().size(), 1);
    EXPECT_TRUE(join_node->non_trivial_functional_dependencies().contains(
        generated_fd_a));

    const auto generated_fd_x =
        FunctionalDependency{{_t_b_x}, {_t_a_a, _t_a_b, _t_a_c, _t_b_y}};
...
{
    const auto key_constraint_a_b = TableKeyConstraint{
        {_t_a_a->original_column_id, _t_a_b->original_column_id},
        KeyConstraintType::PRIMARY_KEY};
    const auto key_constraint_c = TableKeyConstraint{
        {_t_a_c->original_column_id}, KeyConstraintType::UNIQUE};
    _mock_node_a->set_key_constraints({key_constraint_a_b, key_constraint_c});
    _mock_node_b->set_key_constraints({*_key_constraint_x});

    // The following FD is trivial since it can be derived from a UCC (PRIMARY
    // KEY across a & b). However, we define it as non-trivial anyway, to verify
    // the conflict resolution when merging FDs later on.
  ...
  JoinNode::make(JoinMode::Inner, equals_(_t_a_c, _t_b_x),
    ...
    const auto expected_fd_a_b =
        FunctionalDependency{{_t_a_a, _t_a_b}, {_t_a_c, _t_b_x, _t_b_y}};
    const auto expected_fd_c =
        FunctionalDependency{{_t_a_c}, {_t_a_a, _t_a_b, _t_b_x, _t_b_y}};
    const auto expected_fd_x =
        FunctionalDependency{{_t_b_x}, {_t_a_a, _t_a_b, _t_a_c, _t_b_y}};

    // Prerequisites
    const auto &non_trivial_fds =
        join_node->non_trivial_functional_dependencies();
    EXPECT_EQ(non_trivial_fds.size(), 1);
    EXPECT_TRUE(non_trivial_fds.contains(fd_a_b));

    const auto &trivial_fds = fds_from_unique_column_combinations(
        join_node, join_node->unique_column_combinations());
    ...
    /**
     * After unifiying the two FD sets above, we expect three and instead of
     * four FDs since the following FD objects {a, b} => {c} and {a, b} => {c,
     * x, y} can be merged into one.
     */
...
/**
 * The following four tests check the forwarding of UCCs given the uniqueness of
 * the input nodes' join columns. See the table for a brief overview of the test
 * cases:
 *
 *                      |                              Join column of left input
 * is unique
 *                      +-----------------------------------------------+-----------------------------------------------
 *                      |                      Yes                      | No
 * ---------------------+-----------------------------------------------+-----------------------------------------------
 * Join column of | Yes | UniqueColumnCombinationsBothJoinColumnsUnique |
 * UniqueColumnCombinationsRightJoinColumnUnique right input is
 * +-----+-----------------------------------------------+-----------------------------------------------
 * unique         | No  | UniqueColumnCombinationsLeftJoinColumnUnique  |
 * UniqueColumnCombinationsNoJoinColumnUnique
 *
 */
TEST_P(JoinNodeMultiJoinModeTest, UniqueColumnCombinationsNoJoinColumnUnique)
{
    ...
    const auto &join_unique_column_combinations =
        join_node->unique_column_combinations();
    if (is_semi_or_anti_join(join_mode))
    {
        // Semi- and Anti-Joins act as filters for the left input and preserve
        // its unique column combinations.
        EXPECT_EQ(join_unique_column_combinations.size(), 1);
        EXPECT_TRUE(find_ucc_by_key_constraint(
            *_key_constraint_b_c, join_unique_column_combinations));
    }
    else
    {
        // Not unique join columns might lead to row/value duplication in the
        // opposite relation. No UCCs remain valid.
...
TEST_P(JoinNodeMultiJoinModeTest, UniqueColumnCombinationsLeftJoinColumnUnique)
{
    ...
    _mock_node_a->set_key_constraints(
        {*_key_constraint_a, *_key_constraint_b_c});
    _mock_node_b->set_key_constraints({*_key_constraint_x});
    ...
    case JoinMode::AntiNullAsFalse:
        ...
        EXPECT_EQ(join_unique_column_combinations,
                  _mock_node_a->unique_column_combinations());
        break;
    case JoinMode::Inner:
    case JoinMode::Left:
    case JoinMode::Right:
    case JoinMode::FullOuter:
        // UCCs of the right input should be forwarded. Since our current UCC
        // implementation is compatible with NULL values, outer joins are
        // handled like inner joins.
        EXPECT_EQ(join_unique_column_combinations.size(), 1);
        EXPECT_EQ(join_unique_column_combinations,
                  _mock_node_b->unique_column_combinations());
        break;
    }
}
...
TEST_P(JoinNodeMultiJoinModeTest, UniqueColumnCombinationsBothJoinColumnsUnique)
{
    ...
    EXPECT_TRUE(find_ucc_by_key_constraint(*_key_constraint_a,
                                           join_unique_column_combinations));
    EXPECT_TRUE(find_ucc_by_key_constraint(*_key_constraint_b_c,
                                           join_unique_column_combinations));
    ...
    EXPECT_TRUE(find_ucc_by_key_constraint(*_key_constraint_x,
                                           join_unique_column_combinations));
    EXPECT_TRUE(find_ucc_by_key_constraint(*_key_constraint_y,
                                           join_unique_column_combinations));
}

TEST_P(JoinNodeMultiJoinModeTest, UniqueColumnCombinationsNonEquiJoin)
{
    // Currently, we do not support UCC forwarding for Non-Equi- or Theta-Joins.
    // Semi-, Cross-, and Anti-Joins only support Equi-Joins.
    const auto join_mode = GetParam();
...
{
    // Except for Semi- and Anti-Joins, we do not support forwarding of UCCs for
...
TEST_F(JoinNodeTest, GetOrFindReducedJoinNode)
{
    ...
    auto semi_reduction_node = JoinNode::make(JoinMode::Semi, join_predicate,
                                              _mock_node_a, _mock_node_b);
    // clang-format off
  const auto lqp =
  JoinNode::make(JoinMode::Inner, join_predicate,
    ...
    // The semi join reduction node should use the stored weak pointer to create
    // and return a shared pointer to the reduced join.
    EXPECT_EQ(semi_reduction_node->get_or_find_reduced_join_node(), join_node);

    // In a deep-copied LQP, the semi reduction node's weak pointer to the
    // reduced join is unset (lazy approach). Therefore,
    // get_or_find_reduced_join_node must utilize an LQP upwards traversal for
    // discovering the reduced join.
    const auto copied_lqp = lqp->deep_copy();
    const auto copied_join_node =
        std::static_pointer_cast<JoinNode>(copied_lqp);
    const auto copied_semi_reduction_node = std::static_pointer_cast<JoinNode>(
        copied_lqp->left_input()->left_input());
    EXPECT_EQ(copied_semi_reduction_node->get_or_find_reduced_join_node(),
              copied_join_node);

    // If there is no reduced join, we expect to fail.
    EXPECT_THROW(
        std::static_pointer_cast<JoinNode>(semi_reduction_node->deep_copy())
...
{
    // In contrast to the test GetOrFindReducedJoinNode, we define a semi
    // reduction for a join with multiple predicates.
  ...
  JoinNode::make(JoinMode::Inner, expression_vector(less_than_(_t_a_b, _t_b_y), semi_reduction_predicate),
    ...
    const auto copied_lqp = lqp->deep_copy();
    const auto copied_join_node =
        std::dynamic_pointer_cast<JoinNode>(copied_lqp);
    const auto copied_semi_reduction_node = std::dynamic_pointer_cast<JoinNode>(
        copied_lqp->left_input()->left_input());
    EXPECT_EQ(copied_semi_reduction_node->get_or_find_reduced_join_node(),
              copied_join_node);
}
