...
TEST_F(PredicateNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto other_predicate_node_a =
        PredicateNode::make(equals_(_i, 5), _table_node);
    const auto other_predicate_node_b =
        PredicateNode::make(equals_(_f, 5), _table_node);
    const auto other_predicate_node_c =
        PredicateNode::make(not_equals_(_i, 5), _table_node);
    const auto other_predicate_node_d =
        PredicateNode::make(equals_(_i, 6), _table_node);
    const auto other_predicate_node_e =
        PredicateNode::make(equals_(equal_i, 5), equal_table_node);
...
TEST_F(PredicateNodeTest, ForwardUniqueColumnCombinations)
{
    ...
    const auto &unique_column_combinations =
        _predicate_node->unique_column_combinations();
