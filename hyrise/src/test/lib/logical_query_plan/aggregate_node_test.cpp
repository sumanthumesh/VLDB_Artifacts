  ...
  protected:
    void SetUp() override
    {
        ...
        _aggregate_expressions =
            expression_vector(sum_(add_(_a, _b)), sum_(add_(_a, _c)));
        _aggregate_node = AggregateNode::make(
            _group_by_expressions, _aggregate_expressions, _mock_node);
    }
...
TEST_F(AggregateNodeTest, InvalidAggregates)
{
    ...
    // Aggregate expression must be an aggregate function (i.e., a
    // WindowFunctionExpression).
    EXPECT_THROW(AggregateNode::make(expression_vector(),
                                     expression_vector(add_(_a, _b))),
                 std::logic_error);

    // WindowFunctionExpression used as aggregate function must not be a pure
    // window function.
    const auto window_function =
        std::make_shared<WindowFunctionExpression>(WindowFunction::Rank, _a);
    EXPECT_THROW(AggregateNode::make(expression_vector(),
                                     expression_vector(window_function)),
                 std::logic_error);

    // WindowFunctionExpression used as aggregate function must not define a
    // window.
    ...
    EXPECT_THROW(AggregateNode::make(expression_vector(),
                                     expression_vector(min_(_a, window))),
                 std::logic_error);
}
...
TEST_F(AggregateNodeTest, OutputColumnExpressions)
{
    ...
    EXPECT_EQ(*_aggregate_node->output_expressions().at(2),
              *sum_(add_(_a, _b)));
    EXPECT_EQ(*_aggregate_node->output_expressions().at(3),
              *sum_(add_(_a, _c)));
}
...
TEST_F(AggregateNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto different_aggregate_node_a = AggregateNode::make(
        expression_vector(_a),
        expression_vector(sum_(add_(_a, _b)), sum_(add_(_a, _c))), _mock_node);
    const auto different_aggregate_node_b = AggregateNode::make(
        expression_vector(_a, _c),
        expression_vector(sum_(add_(_a, 2)), sum_(add_(_a, _c))), _mock_node);
    const auto different_aggregate_node_c = AggregateNode::make(
        expression_vector(_a, _c),
        expression_vector(sum_(add_(_a, _b)), sum_(add_(_a, _c)), min_(_a)),
        _mock_node);
    const auto different_aggregate_node_d = AggregateNode::make(
        expression_vector(_a, _a),
        expression_vector(sum_(add_(_a, _b)), sum_(add_(_a, _c))), _mock_node);
    ...
    // _aggregate_node and different_aggregate_node_b are known to conflict
    // because we do not recurse deep enough to identify the difference in the
    // aggregate expressions. That is acceptable, as long as the comparison
    // identifies the two nodes as non-equal.
...
TEST_F(AggregateNodeTest, UniqueColumnCombinationsAdd)
{
    ...
    const auto agg_node_a = AggregateNode::make(
        expression_vector(_a), expression_vector(aggregate1, aggregate2),
        _mock_node);
    const auto agg_node_b = AggregateNode::make(
        expression_vector(_a, _b), expression_vector(aggregate1, aggregate2),
        _mock_node);

    // Check whether AggregateNode adds a new UCC for its group-by column(s).
    {
        const auto &unique_column_combinations =
            agg_node_a->unique_column_combinations();
        EXPECT_EQ(unique_column_combinations.size(), 1);
        EXPECT_TRUE(
            unique_column_combinations.contains(UniqueColumnCombination{{_a}}));
    }
    {
        const auto &unique_column_combinations =
            agg_node_b->unique_column_combinations();
        EXPECT_EQ(unique_column_combinations.size(), 1);
        EXPECT_TRUE(unique_column_combinations.contains(
            UniqueColumnCombination{{_a, _b}}));
    }
}
...
TEST_F(AggregateNodeTest, UniqueColumnCombinationsForwardingSimple)
{
    const auto key_constraint_b =
        TableKeyConstraint{{_b->original_column_id}, KeyConstraintType::UNIQUE};
    const auto key_constraint_c =
        TableKeyConstraint{{_c->original_column_id}, KeyConstraintType::UNIQUE};
    ...
    _aggregate_node = AggregateNode::make(
        expression_vector(_a, _b), expression_vector(aggregate_c), _mock_node);
    const auto &unique_column_combinations =
        _aggregate_node->unique_column_combinations();

    /**
     * Expected behaviour:
     *  - UCC from key_constraint_b remains valid since _b is part of the
     * group-by columns.
     *  - UCC from key_constraint_c, however, should be discarded because _c
     * gets aggregated.
     */
    ...
    EXPECT_EQ(unique_column_combinations.size(), 1);
    // In-depth check.
    EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_b,
...
TEST_F(AggregateNodeTest, UniqueColumnCombinationsForwardingAnyAggregates)
{
    ...
    const auto aggregate_c = sum_(_c);
    _aggregate_node = AggregateNode::make(
        expression_vector(_a), expression_vector(aggregate_b, aggregate_c),
        _mock_node);
    const auto &unique_column_combinations =
        _aggregate_node->unique_column_combinations();
    ...
    /**
     * Expected behaviour:
     *  - UCC from key_constraint_b remains valid because _b is aggregated via
     * ANY(), a pseudo aggregate function used by the
     * DependentGroupByReductionRule to optimize group-bys.
     *  - UCC from key_constraint_c should be discarded because _c is
     * aggregated.
    ...
    EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_b,
                                           unique_column_combinations));
    EXPECT_FALSE(find_ucc_by_key_constraint(key_constraint_c,
                                            unique_column_combinations));
...
TEST_F(AggregateNodeTest, UniqueColumnCombinationsNoDuplicates)
{
    ...
    /**
     * AggregateNode should try to create a new UCC from its group-by-column _a.
     * It is the same as MockNode's UCC, which is forwarded.
     *
    ...
    EXPECT_EQ(unique_column_combinations.size(), 1);
    // In-depth check.
    EXPECT_TRUE(find_ucc_by_key_constraint(table_key_constraint,
                                           unique_column_combinations));
}
...
TEST_F(AggregateNodeTest, UniqueColumnCombinationsNoSupersets)
{
    ...
    _aggregate_node = AggregateNode::make(
        expression_vector(_a, _b), expression_vector(aggregate), _mock_node);

    /**
     * AggregateNode should try to create a new UCC from both group-by-columns
     * _a and _b. However, MockNode already has a UCC for _a, which is
     * forwarded. It is shorter, and thus preferred over the UCC covering both
     * _a and _b.
     *
...
TEST_F(AggregateNodeTest, FunctionalDependenciesForwarding)
{
    ...
    // All determinant and dependent expressions are part of the output ->
    // expect FD forwarding
    const auto &agg_node_b = AggregateNode::make(
        expression_vector(_a, _c), expression_vector(aggregate1, aggregate2),
        _mock_node);
    EXPECT_EQ(agg_node_b->non_trivial_functional_dependencies().size(), 1);
    EXPECT_TRUE(
        agg_node_b->non_trivial_functional_dependencies().contains(fd_a));

    // Special case: All determinant expressions, but only some of the dependent
    // expressions are part of the output
    const auto &agg_node_c = AggregateNode::make(
        expression_vector(_b, _c), expression_vector(aggregate1, aggregate2),
        _mock_node);
    const auto expected_fd = FunctionalDependency{{_b}, {_c}};
    EXPECT_EQ(agg_node_c->non_trivial_functional_dependencies().size(), 1);
    EXPECT_TRUE(agg_node_c->non_trivial_functional_dependencies().contains(
        expected_fd));
}

TEST_F(AggregateNodeTest, FunctionalDependenciesAdd)
{
    // The group-by columns form a new candidate key / UCC from which we should
    // derive a trivial FD.
    ...
    const auto expected_determinants = ExpressionUnorderedSet{
        _group_by_expressions.cbegin(), _group_by_expressions.cend()};
    const auto expected_dependents = ExpressionUnorderedSet{
        _aggregate_expressions.cbegin(), _aggregate_expressions.cend()};
