...
TEST_F(LQPUtilsTest, LQPSubplanToBooleanExpression_A)
{
  ...
  PredicateNode::make(greater_than_(a_a, 5),
    ...
    const auto expected_expression =
        and_(less_than_(a_b, 4), greater_than_(a_a, 5));
...
TEST_F(LQPUtilsTest, LQPSubplanToBooleanExpression_C)
{
  ...
  PredicateNode::make(greater_than_(a_a, 5),
    ...
    const auto actual_expression = lqp_subplan_to_boolean_expression(lqp);
    const auto expected_expression =
        and_(less_than_(b_x, 4), greater_than_(a_a, 5));
...
TEST_F(LQPUtilsTest, LQPSubplanToBooleanExpressionBeginEndNode)
{
    ...
    const auto begin_node =
        PredicateNode::make(greater_than_(a_a, 5), end_node);

    const auto actual_expression =
        lqp_subplan_to_boolean_expression(begin_node, end_node);
...
TEST_F(LQPUtilsTest, VisitLQP)
{
  ...
  const auto expected_nodes = std::vector<std::shared_ptr<AbstractLQPNode>>{
...

    {
        // Visit PredicateNode
        auto actual_nodes = std::vector<std::shared_ptr<AbstractLQPNode>>{};
        visit_lqp(std::static_pointer_cast<PredicateNode>(expected_nodes[0]),
                  [&](const auto &node)
                  {
                      actual_nodes.emplace_back(node);
                      return LQPVisitation::VisitInputs;
    ...
    ProjectionNode::make(expression_vector(add_(a_a, a_b)),
      PredicateNode::make(in_(a_b, x_greater_than_max_a_subquery),
        ...
        const auto subquery_expressions_by_lqp =
            collect_lqp_subquery_expressions_by_lqp(root_lqp,
                                                    only_correlated_subqueries);
        const auto expected_expression_count =
            only_correlated_subqueries ? 1 : 3;
        EXPECT_EQ(subquery_expressions_by_lqp.size(),
                  expected_expression_count);

        ASSERT_TRUE(
            subquery_expressions_by_lqp.contains(correlated_subquery->lqp));
        EXPECT_EQ(
            subquery_expressions_by_lqp.at(correlated_subquery->lqp).size(), 1);
        EXPECT_EQ(subquery_expressions_by_lqp.at(correlated_subquery->lqp)
                      .at(0)
                      .lock(),
                  correlated_subquery);
        ...
        ASSERT_TRUE(subquery_expressions_by_lqp.contains(
            x_greater_than_max_a_subquery->lqp));
        EXPECT_EQ(
            subquery_expressions_by_lqp.at(x_greater_than_max_a_subquery->lqp)
                .size(),
            1);
        EXPECT_EQ(
            subquery_expressions_by_lqp.at(x_greater_than_max_a_subquery->lqp)
        ...
        EXPECT_EQ(subquery_expressions_by_lqp.at(max_a_subquery->lqp).size(),
                  1);
        EXPECT_EQ(
            subquery_expressions_by_lqp.at(max_a_subquery->lqp).at(0).lock(),
            max_a_subquery);
    }
}
...
TEST_F(LQPUtilsTest, FindDiamondOriginNode)
{
    ...
    // Test for null pointer, because the diamond from above no longer has a
    // common origin node.
    {
        // clang-format off
    const auto lqp =
...
{
    // Test if the diamond's origin node is returned in case of multiple
    // UnionNodes.
    {
        // clang-format off
    const auto lqp =
...
{
    // Edge case: In this test, the diamond's root node has the same number of
    // outputs as the diamond's origin node.
    // clang-format off
  ...
  UnionNode::make(SetOperationMode::Positions,
    ...
    const auto bottom_diamond_origin_node =
        find_diamond_origin_node(bottom_diamond_root_node);
    EXPECT_EQ(bottom_diamond_origin_node, node_a);

    const auto top_diamond_origin_node =
        find_diamond_origin_node(top_diamond_root_node);
