...
class LQPFindSubplanMismatchTest : public BaseTest
{
  protected:
    ...
    void _init_query_nodes(QueryNodes &query_nodes) const
    {
        ...
        query_nodes.mock_node_a =
            MockNode::make(MockNode::ColumnDefinitions{{DataType::Int, "a"}});
        query_nodes.mock_node_b = MockNode::make(MockNode::ColumnDefinitions{
            {DataType::Int, "b"}, {DataType::Int, "c"}});

        query_nodes.table_a_a =
            query_nodes.stored_table_node_a->get_column("a");
        ...
        query_nodes.predicate_node_a =
            PredicateNode::make(less_than_(query_nodes.table_a_a, 41));
        query_nodes.predicate_node_b = PredicateNode::make(
            between_inclusive_(query_nodes.table_a_a, 42, 45));
        ...
        query_nodes.join_node =
            JoinNode::make(JoinMode::Inner, equals_(query_nodes.table_a_a,
                                                    query_nodes.table_c_c));

        query_nodes.aggregate_node =
            AggregateNode::make(expression_vector(query_nodes.table_c_c),
                                expression_vector(sum_(query_nodes.table_c_b)));

        query_nodes.sort_node =
            SortNode::make(expression_vector(query_nodes.table_c_c),
                           std::vector<SortMode>{SortMode::Ascending});
        query_nodes.projection_node =
            ProjectionNode::make(expression_vector(query_nodes.table_a_a));
    }

    std::shared_ptr<AbstractLQPNode> _build_query_lqp(QueryNodes &query_nodes)
    {
        query_nodes.validate_node->set_left_input(
            query_nodes.stored_table_node_a);
        query_nodes.predicate_node_a->set_left_input(query_nodes.validate_node);
        query_nodes.predicate_node_b->set_left_input(
            query_nodes.stored_table_node_a);
...
TEST_F(LQPFindSubplanMismatchTest, EqualsTest)
{
    ...
    EXPECT_LQP_EQ(_query_nodes_lhs.predicate_node_a,
                  _query_nodes_rhs.predicate_node_a);
    EXPECT_LQP_EQ(_query_nodes_lhs.join_node, _query_nodes_rhs.join_node);
}

TEST_F(LQPFindSubplanMismatchTest, SubplanMismatch)
{
    _query_nodes_rhs.predicate_node_b = PredicateNode::make(
        between_inclusive_(_query_nodes_rhs.table_a_a, 42, 46));
...
{
    const auto additional_predicate_node = PredicateNode::make(
        between_inclusive_(_query_nodes_rhs.table_a_a, 42, 45));

    _build_query_lqps();

    _query_nodes_rhs.predicate_node_b->set_left_input(
        additional_predicate_node);
    additional_predicate_node->set_left_input(
        _query_nodes_rhs.stored_table_node_a);
...
{
    const auto first_node = PredicateNode::make(between_inclusive_(
        _query_nodes_rhs.table_a_a, int32_t{42}, int32_t{45}));
    const auto second_node = PredicateNode::make(between_inclusive_(
        _query_nodes_rhs.table_a_a, int64_t{42}, int64_t{45}));
