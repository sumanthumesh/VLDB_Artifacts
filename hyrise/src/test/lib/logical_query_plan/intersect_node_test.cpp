...
{
    EXPECT_EQ(_intersect_node->description(),
              "[IntersectNode] Mode: Positions");
}

TEST_F(IntersectNodeTest, OutputColumnExpressions)
{
    EXPECT_TRUE(_intersect_node->output_expressions() ==
...
TEST_F(IntersectNodeTest, HashingAndEqualityCheck)
{
    auto same_intersect_node = IntersectNode::make(SetOperationMode::Positions,
                                                   _mock_node1, _mock_node1);
    auto different_intersect_node =
        IntersectNode::make(SetOperationMode::All, _mock_node1, _mock_node1);
    auto different_intersect_node_1 = IntersectNode::make(
        SetOperationMode::Positions, _mock_node1, _mock_node2);
    auto different_intersect_node_2 = IntersectNode::make(
        SetOperationMode::Positions, _mock_node2, _mock_node1);
    auto different_intersect_node_3 = IntersectNode::make(
        SetOperationMode::Positions, _mock_node2, _mock_node2);
...
TEST_F(IntersectNodeTest, ForwardUniqueColumnCombinations)
{
    ...
    const auto &unique_column_combinations =
        _intersect_node->unique_column_combinations();
    ...
    if constexpr (HYRISE_DEBUG)
    {
        ...
        EXPECT_THROW(_intersect_node->unique_column_combinations(),
                     std::logic_error);
    }
}
