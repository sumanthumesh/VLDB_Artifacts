  ...
  public:
    void SetUp() override
    {
        Hyrise::get().storage_manager.add_table(
            "int_int", load_table("resources/test_data/tbl/int_int.tbl"));
        Hyrise::get().storage_manager.add_table(
            "int_int_int",
        ...
        _projection_node =
            ProjectionNode::make(expression_vector(_t_a_a, _t_a_b));
        ...
        _nodes[6] =
            MockNode::make(MockNode::ColumnDefinitions{{{DataType::Int, "a"}}});
        _nodes[7] =
            MockNode::make(MockNode::ColumnDefinitions{{{DataType::Int, "b"}}});
        _nodes[0] = JoinNode::make(JoinMode::Cross);
        _nodes[1] = JoinNode::make(JoinMode::Cross);
        _nodes[2] = PredicateNode::make(
            equals_(lqp_column_(_nodes[6], ColumnID{0}), 42));
...
TEST_F(LogicalQueryPlanTest, LQPColumnExpressionHash)
{
    ...
    const auto expression_a =
        std::make_shared<LQPColumnExpression>(node_int_int_1, ColumnID{0});
    const auto expression_a_1 =
        std::make_shared<LQPColumnExpression>(node_int_int_1, ColumnID{0});
    const auto expression_a_2 =
        std::make_shared<LQPColumnExpression>(node_int_int_2, ColumnID{0});
...
TEST_F(LogicalQueryPlanTest, SimpleOutputTest)
{
    ...
    ASSERT_EQ(_mock_node_a->outputs(),
              std::vector<std::shared_ptr<AbstractLQPNode>>{_predicate_node_a});
    ...
    ASSERT_EQ(_predicate_node_a->outputs(),
              std::vector<std::shared_ptr<AbstractLQPNode>>{_projection_node});
...
TEST_F(LogicalQueryPlanTest, ChainSameNodesTest)
{
    ...
    ASSERT_EQ(_predicate_node_a->outputs(),
              std::vector<std::shared_ptr<AbstractLQPNode>>{_predicate_node_b});
    ...
    ASSERT_EQ(_predicate_node_b->outputs(),
              std::vector<std::shared_ptr<AbstractLQPNode>>{_projection_node});
...
TEST_F(LogicalQueryPlanTest, TwoInputsTest)
{
    ...
    ASSERT_EQ(_mock_node_a->outputs(),
              std::vector<std::shared_ptr<AbstractLQPNode>>{_join_node});
    ASSERT_EQ(_mock_node_b->outputs(),
              std::vector<std::shared_ptr<AbstractLQPNode>>{_join_node});
}
...
TEST_F(LogicalQueryPlanTest, ComplexGraphRemoveFromTree)
{
    ...
    // Make sure _node[1], _node[3] and _node[4] are the only outputs _nodes[5]
    // has
...
TEST_F(LogicalQueryPlanTest, ComplexGraphReplaceWith)
{
    ...
    // Make sure new_node, _nodes[3] and _nodes[4] are the only outputs of
    // _nodes[7]
...
TEST_F(LogicalQueryPlanTest, CreateNodeMapping)
{
    const auto projection_node =
        ProjectionNode::make(node_int_int->output_expressions(), node_int_int);
    const auto lqp = projection_node;

    const auto copied_lqp = lqp->deep_copy();
    const auto copied_projection_node =
        std::dynamic_pointer_cast<ProjectionNode>(copied_lqp);
    const auto copied_node_int_int =
        std::dynamic_pointer_cast<StoredTableNode>(copied_lqp->left_input());
...
{
    const auto expression_a =
        std::make_shared<LQPColumnExpression>(node_int_int, ColumnID{0});
    const auto expression_b =
        std::make_shared<LQPColumnExpression>(node_int_int, ColumnID{1});
    ...
    // Check that expressions in copied LQP point to StoredTableNode in their
    // LQP, not into the original LQP
    const auto copied_expression_a =
        std::dynamic_pointer_cast<LQPColumnExpression>(
            copied_projection_node->node_expressions.at(0));
    const auto copied_expression_b =
        std::dynamic_pointer_cast<LQPColumnExpression>(
            copied_projection_node->node_expressions.at(1));
...
TEST_F(LogicalQueryPlanTest, DeepCopySubqueries)
{
  ...
  ProjectionNode::make(expression_vector(a1, subquery),
    ...
    EXPECT_LQP_EQ(copied_lqp, lqp);
    const auto copied_projection_a =
        std::dynamic_pointer_cast<ProjectionNode>(copied_lqp);
    const auto copied_predicate_a =
        std::dynamic_pointer_cast<PredicateNode>(copied_lqp->left_input());

    const auto copied_subquery_a =
        std::dynamic_pointer_cast<LQPSubqueryExpression>(
            copied_lqp->output_expressions().at(1));
    const auto copied_subquery_b =
        std::dynamic_pointer_cast<LQPSubqueryExpression>(
            copied_predicate_a->predicate()->arguments.at(1));
...
TEST_F(LogicalQueryPlanTest, OutputResetOnNodeDelete)
{
    auto mock_node_a =
        MockNode::make(MockNode::ColumnDefinitions{{DataType::Int, "x"}});
    auto mock_node_b =
        MockNode::make(MockNode::ColumnDefinitions{{DataType::Int, "y"}});
