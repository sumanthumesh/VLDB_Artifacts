...
{
    // _limit_node has _mock_node as input, so it would not be equal to or have
    // the same hash value as newly created, equivalent nodes.
...
TEST_F(LimitNodeTest, ForwardUniqueColumnCombinations)
{
    ...
    const auto &unique_column_combinations =
        _limit_node->unique_column_combinations();
