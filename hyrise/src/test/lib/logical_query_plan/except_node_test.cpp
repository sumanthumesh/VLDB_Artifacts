    ...
    void SetUp() override
    {
        ...
        _mock_node2 =
            MockNode::make(MockNode::ColumnDefinitions{{DataType::Int, "u"},
                                                       {DataType::Int, "v"}},
                           "t_b");
        _mock_node3 = MockNode::make(
            MockNode::ColumnDefinitions{{DataType::Int, "x"}}, "t_v");
        ...
        _except_node_without_right_input =
...
TEST_F(ExceptNodeTest, IsColumnNullable)
{
    ...
    EXPECT_THROW(
        _except_node_without_right_input->is_column_nullable(ColumnID{0}),
        std::logic_error);
}
...
TEST_F(ExceptNodeTest, NonTrivialFunctionalDependencies)
{
    ...
    const auto non_trivial_dependencies =
        FunctionalDependencies{fd_a, fd_b_two_dependents};
    _mock_node1->set_non_trivial_functional_dependencies(
        non_trivial_dependencies);

    EXPECT_EQ(_except_node->non_trivial_functional_dependencies(),
              non_trivial_dependencies);
    // Also works if right_input is not set.
    EXPECT_EQ(
        _except_node_without_right_input->non_trivial_functional_dependencies(),
        non_trivial_dependencies);
}

TEST_F(ExceptNodeTest, OutputColumnExpressions)
{
    EXPECT_TRUE(_except_node->output_expressions() ==
                _mock_node1->output_expressions());
}

TEST_F(ExceptNodeTest, HashingAndEqualityCheck)
{
    auto same_except_node =
        ExceptNode::make(SetOperationMode::Positions, _mock_node1, _mock_node1);
    auto different_except_node =
        ExceptNode::make(SetOperationMode::All, _mock_node1, _mock_node1);
    auto different_except_node_1 =
        ExceptNode::make(SetOperationMode::Positions, _mock_node1, _mock_node2);
    auto different_except_node_2 =
        ExceptNode::make(SetOperationMode::Positions, _mock_node2, _mock_node1);
    auto different_except_node_3 =
        ExceptNode::make(SetOperationMode::Positions, _mock_node2, _mock_node2);
...
TEST_F(ExceptNodeTest, ForwardUniqueColumnCombinations)
{
    ...
    const auto &unique_column_combinations =
        _except_node->unique_column_combinations();
