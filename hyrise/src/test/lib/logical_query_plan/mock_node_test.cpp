...
{
    EXPECT_EQ(_mock_node_a->description(),
              "[MockNode 'Unnamed'] Columns: a b c d | pruned: 0/4 columns");
    EXPECT_EQ(_mock_node_b->description(),
              "[MockNode 'mock_name'] Columns: a b | pruned: 0/2 columns");

    _mock_node_a->set_pruned_column_ids({ColumnID{2}});
    EXPECT_EQ(_mock_node_a->description(),
              "[MockNode 'Unnamed'] Columns: a b d | pruned: 1/4 columns");
}

TEST_F(MockNodeTest, OutputColumnExpression)
{
    ASSERT_EQ(_mock_node_a->output_expressions().size(), 4u);
    EXPECT_EQ(*_mock_node_a->output_expressions().at(0),
              *lqp_column_(_mock_node_a, ColumnID{0}));
    EXPECT_EQ(*_mock_node_a->output_expressions().at(1),
              *lqp_column_(_mock_node_a, ColumnID{1}));
    EXPECT_EQ(*_mock_node_a->output_expressions().at(2),
              *lqp_column_(_mock_node_a, ColumnID{2}));
    EXPECT_EQ(*_mock_node_a->output_expressions().at(3),
              *lqp_column_(_mock_node_a, ColumnID{3}));

    ASSERT_EQ(_mock_node_b->output_expressions().size(), 2u);
    EXPECT_EQ(*_mock_node_b->output_expressions().at(0),
              *lqp_column_(_mock_node_b, ColumnID{0}));
    EXPECT_EQ(*_mock_node_b->output_expressions().at(1),
              *lqp_column_(_mock_node_b, ColumnID{1}));
...
TEST_F(MockNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto different_mock_node_1 =
        MockNode::make(MockNode::ColumnDefinitions{{DataType::Long, "a"},
                                                   {DataType::String, "b"}},
                       "mock_name");
...
TEST_F(MockNodeTest, UniqueColumnCombinations)
{
    ...
    const auto table_key_constraints =
        TableKeyConstraints{key_constraint_a_b, key_constraint_c};
...
TEST_F(MockNodeTest, UniqueColumnCombinationsPrunedColumns)
{
    ...
    const auto key_constraint_a =
        TableKeyConstraint{{ColumnID{0}}, KeyConstraintType::UNIQUE};
    const auto key_constraint_a_b = TableKeyConstraint{
        {ColumnID{0}, ColumnID{1}}, KeyConstraintType::UNIQUE};
    const auto key_constraint_c =
        TableKeyConstraint{{ColumnID{2}}, KeyConstraintType::UNIQUE};
    _mock_node_a->set_key_constraints(
        {key_constraint_a, key_constraint_a_b, key_constraint_c});
...

    {
        ...
        EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_a,
                                               unique_column_combinations));
        EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_a_b,
                                               unique_column_combinations));
        EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_c,
    ...
    }
    {
        ...
        EXPECT_FALSE(find_ucc_by_key_constraint(key_constraint_a,
                                                unique_column_combinations));
        EXPECT_FALSE(find_ucc_by_key_constraint(key_constraint_a_b,
                                                unique_column_combinations));
        EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_c,
                                               unique_column_combinations));
    }
