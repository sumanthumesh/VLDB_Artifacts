...
class UnionNodeTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _union_node = UnionNode::make(SetOperationMode::Positions, _mock_node1,
                                      _mock_node1);

        _mock_node2 =
            MockNode::make(MockNode::ColumnDefinitions{{DataType::Int, "u"},
                                                       {DataType::Int, "v"}},
                           "t_b");
...
{
    EXPECT_EQ(*_union_node->output_expressions().at(0),
              *_mock_node1->output_expressions().at(0));
    EXPECT_EQ(*_union_node->output_expressions().at(1),
              *_mock_node1->output_expressions().at(1));
    EXPECT_EQ(*_union_node->output_expressions().at(2),
              *_mock_node1->output_expressions().at(2));
}
...
TEST_F(UnionNodeTest, HashingAndEqualityCheck)
{
    ...
    EXPECT_NE(_union_node->hash(),
              UnionNode::make(SetOperationMode::Positions)->hash());
    EXPECT_NE(_union_node->hash(),
              UnionNode::make(SetOperationMode::All)->hash());
}
...
{
    // Ensure to forbid a union of nodes with different expressions, i.e.,
    // different columns.
    {
        const auto union_node = UnionNode::make(SetOperationMode::Positions,
                                                _mock_node1, _mock_node2);
        EXPECT_THROW(union_node->output_expressions(), std::logic_error);
    }
    {
        const auto union_node =
            UnionNode::make(SetOperationMode::All, _mock_node1, _mock_node2);
        EXPECT_THROW(union_node->output_expressions(), std::logic_error);
    }
}
...
TEST_F(UnionNodeTest, FunctionalDependenciesUnionAllSimple)
{
    ...
    // Set FDs
    _mock_node1->set_key_constraints(
        {{{_a->original_column_id}, KeyConstraintType::UNIQUE}});
    _mock_node1->set_non_trivial_functional_dependencies(
        {non_trivial_fd_b, non_trivial_fd_c});
    EXPECT_EQ(_mock_node1->functional_dependencies().size(), 3);
    EXPECT_TRUE(
        _mock_node1->functional_dependencies().contains(non_trivial_fd_b));
    EXPECT_TRUE(
        _mock_node1->functional_dependencies().contains(non_trivial_fd_c));
    EXPECT_TRUE(_mock_node1->functional_dependencies().contains(trivial_fd_a));

    // Create PredicateNodes & UnionPositionsNode
    const auto &predicate_node_a =
        PredicateNode::make(greater_than_(_a, 5), _mock_node1);
    const auto &predicate_node_b =
        PredicateNode::make(greater_than_(_b, 5), _mock_node1);
    ...
    // We expect all FDs to be forwarded since both input nodes have the same
    // non-trivial FDs & UCCs.
    const auto &union_node_fds = union_all_node->functional_dependencies();
    const auto &union_node_non_trivial_fds =
        union_all_node->non_trivial_functional_dependencies();
...
TEST_F(UnionNodeTest, FunctionalDependenciesUnionAllIntersect)
{
     ...
     */
    const auto &projection_node_a =
        ProjectionNode::make(expression_vector(_a, _b, _c), _mock_node1);
    const auto &aggregate_node = AggregateNode::make(
        expression_vector(_a, _b), expression_vector(any_(_c)), _mock_node1);
    const auto &projection_node_b =
        ProjectionNode::make(expression_vector(_a, _b, _c), aggregate_node);
    ...
    EXPECT_TRUE(projection_node_a->functional_dependencies().contains(
        non_trivial_fd_b));
    EXPECT_EQ(projection_node_b->functional_dependencies().size(), 2);
    EXPECT_TRUE(projection_node_b->functional_dependencies().contains(
        non_trivial_fd_b));
    EXPECT_TRUE(
        projection_node_b->functional_dependencies().contains(expected_fd_a_b));

    // Test: We expect both input FD-sets to be intersected. Therefore, only one
    // FD should survive.
    EXPECT_EQ(union_all_node->functional_dependencies().size(), 1);
    EXPECT_TRUE(
        union_all_node->functional_dependencies().contains(non_trivial_fd_b));
}
...
TEST_F(UnionNodeTest, FunctionalDependenciesUnionPositions)
{
    ...
    const auto &predicate_node_b =
        PredicateNode::make(greater_than_(_b, 5), _mock_node1);
    const auto &union_positions_node =
    ...
    // Positive Tests
    EXPECT_EQ(
        union_positions_node->non_trivial_functional_dependencies().size(), 1);
    EXPECT_TRUE(
        union_positions_node->non_trivial_functional_dependencies().contains(
            non_trivial_fd_b));
    EXPECT_EQ(union_positions_node->functional_dependencies().size(), 2);
    EXPECT_TRUE(union_positions_node->functional_dependencies().contains(
        non_trivial_fd_b));
    EXPECT_TRUE(
        union_positions_node->functional_dependencies().contains(trivial_fd_a));
}

TEST_F(UnionNodeTest, FunctionalDependenciesUnionPositionsInvalidInput)
{
    // This test verifies a DebugAssert condition. Therefore, we do not want
    // this test to run in release mode.
    ...
    const auto &predicate_node_a =
        PredicateNode::make(greater_than_(_a, 5), _mock_node1);
    const auto &predicate_node_b =
        PredicateNode::make(greater_than_(_b, 5), _mock_node2);
    const auto &union_positions_node =
        UnionNode::make(SetOperationMode::Positions);
    union_positions_node->set_left_input(predicate_node_a);
    union_positions_node->set_right_input(predicate_node_b);
    ...
    EXPECT_THROW(union_positions_node->non_trivial_functional_dependencies(),
                 std::logic_error);
    EXPECT_THROW(union_positions_node->functional_dependencies(),
                 std::logic_error);
}
...
TEST_F(UnionNodeTest, UniqueColumnCombinationsUnionPositions)
{
    ...
    const auto key_constraint_b =
        TableKeyConstraint{{ColumnID{2}}, KeyConstraintType::UNIQUE};
    ...
    EXPECT_TRUE(_union_node->left_input() == _mock_node1 &&
                _union_node->right_input() == _mock_node1);
    EXPECT_EQ(_union_node->unique_column_combinations(),
              _mock_node1->unique_column_combinations());
}
...
TEST_F(UnionNodeTest, UniqueColumnCombinationsUnionPositionsInvalidInput)
{
    ...
    const auto key_constraint_b =
        TableKeyConstraint{{ColumnID{2}}, KeyConstraintType::UNIQUE};
    _mock_node1->set_key_constraints(
        TableKeyConstraints{key_constraint_a_b, key_constraint_b});

    // Input nodes are not allowed to have differing UCCs. The cross join does
    // not forward any UCCs.
    // clang-format off
...
TEST_F(UnionNodeTest, UniqueColumnCombinationsUnionAll)
{
    ...
    const auto union_node =
        UnionNode::make(SetOperationMode::All, _mock_node1, _mock_node1);
