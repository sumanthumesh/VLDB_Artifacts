...
TEST_F(StaticTableNodeTest, Description)
{
    EXPECT_EQ(static_table_node->description(),
              "[StaticTable]: (a int not nullable, b float nullable)");

    dummy_table->add_soft_key_constraint(
...
TEST_F(StaticTableNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto same_static_table_node =
    ...
    const auto different_static_table_node_by_definitions =
        StaticTableNode::make(
            Table::create_dummy_table(different_column_definitions));
    ...
    EXPECT_NE(different_static_table_node_by_definitions->hash(),
              static_table_node->hash());
    EXPECT_NE(different_static_table_node_by_constraints->hash(),
              static_table_node->hash());
}
