...
class WindowNodeTest : public BaseTest
{
  public:
    void SetUp() override
    {
        _mock_node = MockNode::make(MockNode::ColumnDefinitions{
...
TEST_F(WindowNodeTest, InvalidExpressions)
{
    ...
    EXPECT_THROW(window_(expression_vector(), expression_vector(_a),
                         std::vector<SortMode>{}, std::move(frame_description)),
                 std::logic_error);

    if constexpr (HYRISE_DEBUG)
    {
        EXPECT_THROW(WindowNode::make(std::shared_ptr<AbstractExpression>{}),
                     std::logic_error);
...
{
    EXPECT_EQ(_window_node->description(),
              "[Window] MIN(a) OVER (RANGE BETWEEN UNBOUNDED PRECEDING AND "
              "CURRENT ROW)");
    auto frame_description = FrameDescription{
        FrameType::Rows, FrameBound{1, FrameBoundType::Preceding, false},
        FrameBound{2, FrameBoundType::Following, false}};
    auto window = window_(expression_vector(_a), expression_vector(_b),
                          std::vector<SortMode>{SortMode::Descending},
                          std::move(frame_description));
    auto window_node = WindowNode::make(rank_(window));
    EXPECT_EQ(window_node->description(),
              "[Window] RANK() OVER (PARTITION BY a ORDER BY b Descending ROWS "
              "BETWEEN 1 PRECEDING AND 2 FOLLOWING)");
    frame_description = FrameDescription{
        FrameType::Rows, FrameBound{0, FrameBoundType::CurrentRow, false},
        FrameBound{0, FrameBoundType::Following, true}};
    window = window_(expression_vector(), expression_vector(),
                     std::vector<SortMode>{}, std::move(frame_description));
    window_node = WindowNode::make(cume_dist_(window));
    EXPECT_EQ(window_node->description(),
              "[Window] CUME_DIST() OVER (ROWS BETWEEN CURRENT ROW AND "
              "UNBOUNDED FOLLOWING)");
}
...
TEST_F(WindowNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto window_node_different_function =
        WindowNode::make(rank_(_window), _mock_node);
    EXPECT_NE(*window_node_different_function, *_window_node);

    auto frame_description = _window->frame_description;
    const auto window =
        window_(expression_vector(_a), expression_vector(),
                std::vector<SortMode>{}, std::move(frame_description));
    const auto window_node_different_window =
        WindowNode::make(rank_(window), _mock_node);
...
TEST_F(WindowNodeTest, UniqueColumnCombinationsForwarding)
{
    ...
    const auto &unique_column_combinations =
        _window_node->unique_column_combinations();
