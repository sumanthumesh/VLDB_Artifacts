...
{
    // A DummyTableNode is just a wrapper for a single value and should not
    // provide meaningful data dependencies (though a single row is obviously
    // unique).
...
{
    EXPECT_THROW(_dummy_table_node->is_column_nullable(ColumnID{0}),
                 std::logic_error);
}
