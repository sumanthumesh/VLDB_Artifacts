  ...
  protected:
    void SetUp() override
    {
        _mock_node = MockNode::make(
            MockNode::ColumnDefinitions({{DataType::String, "foo"}}));
        _change_meta_table_node = ChangeMetaTableNode::make(
            "meta_table", MetaTableChangeType::Insert, _mock_node, _mock_node);
    }
...
{
    EXPECT_EQ(_change_meta_table_node->description(),
              "[Change] Meta Table: 'meta_table'");
}

TEST_F(ChangeMetaTableNodeTest, HashingAndEqualityCheck)
{
    const auto another_change_meta_table_node = ChangeMetaTableNode::make(
        "meta_table", MetaTableChangeType::Insert, _mock_node, _mock_node);
    EXPECT_EQ(*_change_meta_table_node, *another_change_meta_table_node);

    EXPECT_EQ(_change_meta_table_node->hash(),
              another_change_meta_table_node->hash());
}
...
TEST_F(ChangeMetaTableNodeTest, NoUniqueColumnCombinations)
{
    ...
    EXPECT_THROW(_change_meta_table_node->unique_column_combinations(),
                 std::logic_error);
}
