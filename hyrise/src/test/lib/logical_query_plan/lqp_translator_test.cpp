...
class LQPTranslatorTest : public BaseTest
{
  public:
    void SetUp() override
    {
        ...
        table_alias_name =
            load_table("resources/test_data/tbl/table_alias_name.tbl");

        Hyrise::get().storage_manager.add_table("table_int_float",
                                                table_int_float);
        Hyrise::get().storage_manager.add_table("table_int_string",
                                                table_int_string);
        Hyrise::get().storage_manager.add_table("table_int_float2",
                                                table_int_float2);
        Hyrise::get().storage_manager.add_table("table_int_float5",
                                                table_int_float5);
        Hyrise::get().storage_manager.add_table("table_alias_name",
                                                table_alias_name);
        Hyrise::get().storage_manager.add_table(
            "int_float_chunked",
            load_table("resources/test_data/tbl/int_float.tbl",
                       ChunkOffset{1}));
        ChunkEncoder::encode_all_chunks(
            Hyrise::get().storage_manager.get_table("int_float_chunked"));
    ...
    std::shared_ptr<Table> table_int_float, table_int_float2, table_int_float5,
        table_int_string, table_alias_name;
    std::shared_ptr<StoredTableNode> int_float_node, int_float2_node,
        int_float5_node, int_string_node;
    std::shared_ptr<LQPColumnExpression> int_float_a, int_float_b, int_float2_a,
        int_float2_b, int_float5_a, int_float5_d, int_string_a, int_string_b;
};
...
TEST_F(LQPTranslatorTest, StoredTableNodeThrowsWhenNotLeaf)
{
    ...
    EXPECT_THROW(LQPTranslator{}.translate_node(int_float_node),
                 std::logic_error);
}
...
TEST_F(LQPTranslatorTest, ArithmeticExpression)
{
     ...
     */
    const auto a_plus_b_lqp = std::make_shared<ArithmeticExpression>(
        ArithmeticOperator::Addition, int_float_a, int_float_b);
    const auto projection_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>({a_plus_b_lqp});
    const auto projection_node =
        ProjectionNode::make(projection_expressions, int_float_node);
    ...
    const auto a_plus_b_pqp = std::dynamic_pointer_cast<ArithmeticExpression>(
        projection_op->expressions.at(0));
    ASSERT_TRUE(a_plus_b_pqp);
    EXPECT_EQ(a_plus_b_pqp->arithmetic_operator, ArithmeticOperator::Addition);

    const auto a_pqp = std::dynamic_pointer_cast<PQPColumnExpression>(
        a_plus_b_pqp->left_operand());
    ASSERT_TRUE(a_pqp);
    EXPECT_EQ(a_pqp->column_id, ColumnID{0});

    const auto b_pqp = std::dynamic_pointer_cast<PQPColumnExpression>(
        a_plus_b_pqp->right_operand());
    ASSERT_TRUE(b_pqp);
    EXPECT_EQ(b_pqp->column_id, ColumnID{1});

    const auto get_table_op =
        std::dynamic_pointer_cast<const GetTable>(pqp->left_input());
    ASSERT_TRUE(get_table_op);
    EXPECT_EQ(get_table_op->table_name(), "table_int_float");
}
...
TEST_F(LQPTranslatorTest, PredicateNodeSimpleBinary)
{
    ...
    const auto predicate_node =
        PredicateNode::make(greater_than_(5, int_float_b), int_float_node);
    ...
    const auto b =
        PQPColumnExpression::from_table(*table_int_float, ColumnID{1});
...
TEST_F(LQPTranslatorTest, PredicateNodeLike)
{
     ...
     */
    const auto lqp =
        PredicateNode::make(like_(int_string_b, "hello%"), int_string_node);
    ...
    const auto table_scan_op = std::dynamic_pointer_cast<TableScan>(pqp);
    const auto b =
        PQPColumnExpression::from_table(*table_int_string, ColumnID{1});
    ASSERT_TRUE(table_scan_op);
...
TEST_F(LQPTranslatorTest, PredicateNodeUnary)
{
     ...
     */
    const auto predicate_node =
        PredicateNode::make(is_not_null_(int_float_b), int_float_node);
    const auto pqp = LQPTranslator{}.translate_node(predicate_node);
...
TEST_F(LQPTranslatorTest, PredicateNodeBetween)
{
    ...
    const auto predicate_node = PredicateNode::make(
        between_inclusive_(5, int_float_a, int_float_b), int_float_node);
    const auto pqp = LQPTranslator{}.translate_node(predicate_node);
    ...
    const auto between_scan_op =
        std::dynamic_pointer_cast<const TableScan>(pqp);
...
TEST_F(LQPTranslatorTest, SubqueryExpressionCorrelated)
{
     ...
     *
     * LQP resembles:
     *   SELECT (SELECT MIN(a + int_float5.d + int_float5.a) FROM int_float), a
     * FROM int_float5;
     */
    const auto parameter_a =
        correlated_parameter_(ParameterID{0}, int_float5_a);
    const auto parameter_d =
        correlated_parameter_(ParameterID{1}, int_float5_d);

    const auto a_plus_a_plus_d =
        add_(int_float_a, add_(parameter_a, parameter_d));
  ...
  ProjectionNode::make(expression_vector(subquery, int_float5_a), int_float5_node);
    ...
    const auto expression_a = std::dynamic_pointer_cast<PQPSubqueryExpression>(
        projection->expressions.at(0));
    ...
    const auto expression_b = std::dynamic_pointer_cast<PQPColumnExpression>(
        projection->expressions.at(1));
...
TEST_F(LQPTranslatorTest, Sort)
{
    ...
    const auto sort_modes =
        std::vector<SortMode>{{SortMode::Ascending, SortMode::Descending}};
  ...
  ProjectionNode::make(expression_vector(int_float_a, int_float_b),
    ...
    const auto projection_b =
        std::dynamic_pointer_cast<const Projection>(sort->left_input());
    ASSERT_TRUE(projection_b);

    const auto get_table =
        std::dynamic_pointer_cast<const GetTable>(projection_b->left_input());
...
TEST_F(LQPTranslatorTest, LimitLiteral)
{
     ...
     */
    const auto lqp =
        LimitNode::make(value_(static_cast<int64_t>(1337)), int_float_node);
    const auto pqp = LQPTranslator{}.translate_node(lqp);
    ...
    ASSERT_TRUE(limit);
    const auto value_expression = std::dynamic_pointer_cast<ValueExpression>(
        limit->row_count_expression());
    ASSERT_TRUE(value_expression);
    ASSERT_EQ(value_expression->value,
              AllTypeVariant(static_cast<int64_t>(1337)));

    const auto get_table =
        std::dynamic_pointer_cast<const GetTable>(limit->left_input());
    ASSERT_TRUE(get_table);
...
TEST_F(LQPTranslatorTest, PredicateNodeBetweenScan)
{
     ...
     * Build LQP and translate to PQP.
     */
    auto predicate_node = PredicateNode::make(
        between_inclusive_(int_float_a, 42, 1337), int_float_node);
    const auto op = LQPTranslator{}.translate_node(predicate_node);
...
TEST_F(LQPTranslatorTest, LQPNodeAccess)
{
    ...
    auto aggregate_node = AggregateNode::make(
        expression_vector(int_float_a, int_float_b),
...

    {
        ...
        const auto recovered_node =
            std::dynamic_pointer_cast<const AggregateNode>(lqp_node);
        EXPECT_EQ(recovered_node, aggregate_node);
    }
    {
        const auto lqp_node = op->left_input()->lqp_node;
        const auto recovered_node =
            std::dynamic_pointer_cast<const JoinNode>(lqp_node);
        EXPECT_EQ(recovered_node, join_node);
    }
    {
        const auto lqp_node_left = op->left_input()->left_input()->lqp_node;
        const auto recovered_node_left =
            std::dynamic_pointer_cast<const ValidateNode>(lqp_node_left);
        EXPECT_EQ(recovered_node_left, validate_node);
        const auto lqp_node_right = op->left_input()->right_input()->lqp_node;
        const auto recovered_node_right =
            std::dynamic_pointer_cast<const StoredTableNode>(lqp_node_right);
        EXPECT_EQ(recovered_node_right, int_float2_node);
    }
    {
        const auto lqp_node =
            op->left_input()->left_input()->left_input()->lqp_node;
        const auto recovered_node =
            std::dynamic_pointer_cast<const PredicateNode>(lqp_node);
        EXPECT_EQ(recovered_node, predicate_node);
    }
    {
        const auto lqp_node = op->left_input()
                                  ->left_input()
                                  ->left_input()
                                  ->left_input()
                                  ->lqp_node;
        const auto recovered_node =
            std::dynamic_pointer_cast<const StoredTableNode>(lqp_node);
        EXPECT_EQ(recovered_node, int_float_node);
    }
}

// Check if the LQP that is referenced in the PQP is really cleaned up. This
// test is intended to check that no cyclic references are accidentally
// introduced a later point in time.
TEST_F(LQPTranslatorTest, PQPReferencedLQPNodeCleanUp)
{
    std::weak_ptr<const AbstractLQPNode> lqp_node;
    {
        auto pipeline_statement =
            SQLPipelineBuilder{"SELECT a FROM table_int_float WHERE a < 42"}
                .create_pipeline();
...
TEST_F(LQPTranslatorTest, PredicateNodeIndexScan)
{
    ...
    auto predicate_node =
        PredicateNode::make(equals_(stored_table_node->get_column("b"), 42));
    ...
    const auto index_scan_op =
        std::dynamic_pointer_cast<const IndexScan>(op->left_input());
    ASSERT_TRUE(index_scan_op);
    EXPECT_EQ(*index_scan_op->included_chunk_ids, index_chunk_ids);

    const auto table_scan_op =
        std::dynamic_pointer_cast<const TableScan>(op->right_input());
...
TEST_F(LQPTranslatorTest, PredicateNodePrunedIndexScan)
{
    ...
    // As the vector of indexed chunks contains the chunk ids {0, 2} and the
    // vector of pruned chunks contains the chunk id {0}, the first indexed
    // chunk is pruned. Correspondingly, the ids of the indexed chunks have to
    // be adapted, so that the indexed chunk with the id 2 now has the id 1.
...
// All indexed chunks are pruned. The LQP translator should not instantiate an
// index scan and a union.
...
TEST_F(LQPTranslatorTest, PredicateNodeBinaryIndexScan)
{
    ...
    auto predicate_node = PredicateNode::make(
        between_inclusive_(stored_table_node->get_column("b"), 42, 1337));
    predicate_node->set_left_input(stored_table_node);
    predicate_node->scan_type = ScanType::IndexScan;
    const auto op = LQPTranslator{}.translate_node(predicate_node);
...
TEST_F(LQPTranslatorTest, PredicateNodeIndexScanFailsWhenNotApplicable)
{
    ...
    const auto index_chunk_ids = std::vector<ChunkID>{ChunkID{0}, ChunkID{2}};
    table->get_chunk(index_chunk_ids[0])
        ->create_index<GroupKeyIndex>(index_column_ids);
    table->get_chunk(index_chunk_ids[1])
        ->create_index<GroupKeyIndex>(index_column_ids);
    ...
    auto predicate_node2 =
        PredicateNode::make(less_than_(stored_table_node->get_column("a"), 42));
    ...
    EXPECT_THROW(LQPTranslator{}.translate_node(predicate_node2),
                 std::logic_error);
}
...
TEST_F(LQPTranslatorTest, ProjectionNode)
{
    ...
    auto projection_node =
        ProjectionNode::make(expression_vector(int_float_a), int_float_node);
    ...
    EXPECT_EQ(*projection_op->expressions[0],
              *PQPColumnExpression::from_table(*table_int_float, "a"));
}
...
TEST_F(LQPTranslatorTest, JoinNodeToJoinHash)
{
     ...
     */
    auto join_node =
        JoinNode::make(JoinMode::Inner, equals_(int_float2_b, int_float_b),
                       int_float_node, int_float2_node);
    ...
    ASSERT_TRUE(join_op);
    EXPECT_EQ(join_op->primary_predicate().column_ids,
              ColumnIDPair(ColumnID{1}, ColumnID{1}));
    EXPECT_EQ(join_op->primary_predicate().predicate_condition,
...
TEST_F(LQPTranslatorTest, JoinNodeToJoinSortMerge)
{
     ...
     */
    auto join_node =
        JoinNode::make(JoinMode::Inner, less_than_(int_float_b, int_float2_b),
                       int_float_node, int_float2_node);
    const auto op = LQPTranslator{}.translate_node(join_node);

    /**
     * Check PQP: JoinHash doesn't support non-equi joins, thus we fall back to
     * JoinSortMerge.
     */
...
TEST_F(LQPTranslatorTest, JoinNodeToJoinNestedLoop)
{
    ...
    /**
     * Check PQP: Neither JoinHash nor JoinSortMerge support non-equi joins on
     * different column types. So we fall back to JoinNestedLoop.
     */
    const auto join_op = std::dynamic_pointer_cast<JoinNestedLoop>(op);
    ASSERT_TRUE(join_op);
    EXPECT_EQ(join_op->primary_predicate().column_ids,
              ColumnIDPair(ColumnID{0}, ColumnID{1}));
    EXPECT_EQ(join_op->primary_predicate().predicate_condition,
              PredicateCondition::LessThan);
    EXPECT_EQ(join_op->mode(), JoinMode::Inner);
}
...
TEST_F(LQPTranslatorTest, AggregateNodeSimple)
{
  ...
  AggregateNode::make(expression_vector(int_float_a), expression_vector(sum_(add_(int_float_b, int_float_a)), count_star_(int_float_node)),  // NOLINT
    ...
    EXPECT_EQ(*sum,
              *sum_(pqp_column_(ColumnID{2}, DataType::Float, false, "b + a")));
...
TEST_F(LQPTranslatorTest, JoinAndPredicates)
{
    ...
    auto predicate_node_left =
        PredicateNode::make(equals_(int_float_a, 42), int_float_node);
    auto predicate_node_right =
        PredicateNode::make(greater_than_(int_float2_b, 30.0), int_float2_node);
    ...
    const auto predicate_op_left =
        std::dynamic_pointer_cast<const TableScan>(join_op->left_input());
    ASSERT_TRUE(predicate_op_left);
    ASSERT_EQ(*predicate_op_left->predicate(), *equals_(a, 42));

    const auto predicate_op_right =
        std::dynamic_pointer_cast<const TableScan>(join_op->right_input());
    ASSERT_TRUE(predicate_op_right);
    ASSERT_EQ(*predicate_op_right->predicate(), *greater_than_(b, 30.0));

    const auto get_table_op_left = std::dynamic_pointer_cast<const GetTable>(
        predicate_op_left->left_input());
    ASSERT_TRUE(get_table_op_left);
    EXPECT_EQ(get_table_op_left->table_name(), "table_int_float");

    const auto get_table_op_right = std::dynamic_pointer_cast<const GetTable>(
        predicate_op_right->left_input());
...
TEST_F(LQPTranslatorTest, DiamondShapeSimple)
{
     ...
     *     table_int_float2
     *
     * has a diamond shape in the PQP as well. If it wouldn't have it might look
     * like this:
     *
     *    _____union____
     *   /              \
     ...
     *
     * which is still semantically correct, but would mean predicate_c gets
     * executed twice.
     */
    ...
    EXPECT_EQ(pqp->left_input()->left_input(),
              pqp->right_input()->left_input());
    EXPECT_EQ(pqp->left_input()->left_input()->left_input(),
              pqp->right_input()->left_input()->left_input());
}

TEST_F(LQPTranslatorTest, DiamondShapeIncludeUncorrelatedSubqueries)
{
    // Tests that PQP parts that are shared between an uncorrelated subquery and
    // the outer plan are deduplicated.
  ...
  PredicateNode::make(greater_than_(int_float_a, lqp_subquery_expression),
    ...
    const auto get_table_int_float =
        std::static_pointer_cast<const GetTable>(pqp->left_input());

    // Get operators of subquery PQP.
    const auto greater_than_predicate =
        std::dynamic_pointer_cast<const BinaryPredicateExpression>(
            table_scan->predicate());
    ASSERT_TRUE(greater_than_predicate &&
                greater_than_predicate->predicate_condition ==
                    PredicateCondition::GreaterThan);
    ASSERT_EQ(greater_than_predicate->left_operand()->type,
              ExpressionType::PQPColumn);
    ASSERT_EQ(greater_than_predicate->right_operand()->type,
              ExpressionType::PQPSubquery);

    const auto pqp_subquery_expression =
        std::static_pointer_cast<const PQPSubqueryExpression>(
    ...
    const auto subquery_projection = std::static_pointer_cast<const Projection>(
        pqp_subquery_expression->pqp);
    ASSERT_EQ(subquery_projection->left_input()->type(),
              OperatorType::JoinHash);

    const auto subquery_join = std::static_pointer_cast<const JoinHash>(
        subquery_projection->left_input());
    ...
    // Compare addresses to check if the uncorrelated PQP subquery reuses the
    // GetTable instance from its owning PQP.
    EXPECT_EQ(subquery_join->left_input(), get_table_int_float);
}

TEST_F(LQPTranslatorTest, DiamondShapeExcludeCorrelatedSubqueries)
{
    // Tests that PQP parts that are shared between a correlated subquery and
    // the outer plan are NOT deduplicated.
  ...
  PredicateNode::make(greater_than_(int_float_a, lqp_subquery_expression),
    ...
    // Compare addresses to check if the correlated PQP subquery uses a
    // different GetTable instance than its owning PQP.
...
TEST_F(LQPTranslatorTest, ReusingPQPSelfJoin)
{
  ...
  ProjectionNode::make(expression_vector(int_float_b_1, int_float_b_2),
    ...
    const auto product =
        std::dynamic_pointer_cast<const Product>(projection->left_input());
    ASSERT_NE(product, nullptr);

    const auto table_scan_b_1 =
        std::dynamic_pointer_cast<const TableScan>(product->left_input());
    const auto table_scan_b_2 =
        std::dynamic_pointer_cast<const TableScan>(product->right_input());
    ASSERT_NE(table_scan_b_1, nullptr);
    ASSERT_NE(table_scan_b_2, nullptr);
    ASSERT_NE(table_scan_b_1, table_scan_b_2);

    const auto table_scan_a_1 = std::dynamic_pointer_cast<const TableScan>(
        table_scan_b_1->left_input());
    const auto table_scan_a_2 = std::dynamic_pointer_cast<const TableScan>(
        table_scan_b_2->left_input());
    ASSERT_NE(table_scan_a_1, nullptr);
    ASSERT_NE(table_scan_a_2, nullptr);
    ASSERT_EQ(table_scan_a_1, table_scan_a_2);

    const auto get_table =
        std::dynamic_pointer_cast<const GetTable>(table_scan_a_1->left_input());
...
{
    // If the result of a (sub)expression is available in an input column, the
    // expression should not be redundantly evaluated.
  ...
  PredicateNode::make(greater_than_(add_(add_(int_float_a, int_float_b), 3), 2),
    ...
    const auto projection_a =
        std::dynamic_pointer_cast<const Projection>(pqp->left_input());
    const auto projection_b = std::dynamic_pointer_cast<const Projection>(
        pqp->left_input()->left_input());

    ASSERT_NE(table_scan, nullptr);
    ASSERT_NE(projection_a, nullptr);
    ASSERT_NE(projection_b, nullptr);

    const auto a_plus_b_in_temporary_column =
        pqp_column_(ColumnID{1}, DataType::Float, false, "a + b");
    const auto scan_column_expression =
        std::dynamic_pointer_cast<PQPColumnExpression>(
            table_scan->predicate()->arguments.at(0));

    ASSERT_TRUE(scan_column_expression);
    EXPECT_EQ(scan_column_expression->column_id, ColumnID{0});
    EXPECT_EQ(*projection_a->expressions.at(0),
              *add_(a_plus_b_in_temporary_column, 3));
}

TEST_F(LQPTranslatorTest, ReuseSubqueryExpression)
{
    // Test that subquery expressions whose result is available in an output
    // column of the input operator are not evaluated redundantly.
  ...
  ProjectionNode::make(expression_vector(add_(subquery_a, 3)),
    ...
    // As subquery columns without an explicit alias get the LQP/PQP address as
    // their name, we need to retrieve it first.
    const auto column_name = subquery_a->as_column_name();
    const auto subquery_in_temporary_column =
        pqp_column_(ColumnID{1}, DataType::Int, false, column_name);

    EXPECT_EQ(*projection_a->expressions.at(0),
              *add_(subquery_in_temporary_column, 3));
}
...
TEST_F(LQPTranslatorTest, CreateTable)
{
    ...
    const auto lqp = CreateTableNode::make(
        "t", false,
    ...
    // CreateTable input must be executed to enable access to column
    // definitions.
...
TEST_F(LQPTranslatorTest, CreatePreparedPlan)
{
    const auto prepared_plan = std::make_shared<PreparedPlan>(
        DummyTableNode::make(), std::vector<ParameterID>{});
...
TEST_F(LQPTranslatorTest, ChangeMetaTable)
{
  ...
  ChangeMetaTableNode::make("meta_table", MetaTableChangeType::Insert,
    ...
    const auto change_meta_table =
        std::dynamic_pointer_cast<ChangeMetaTable>(pqp);

    EXPECT_EQ(change_meta_table->type(), OperatorType::ChangeMetaTable);
    EXPECT_EQ(change_meta_table->left_input()->type(),
              OperatorType::TableWrapper);
    EXPECT_EQ(change_meta_table->right_input()->type(),
              OperatorType::TableWrapper);
}
...
TEST_F(LQPTranslatorTest, TranslatePrunableSubqueries)
{
  ...
  PredicateNode::make(equals_(int_float2_a, lqp_subquery_(subquery)),
    ...
    const auto &get_table =
        std::dynamic_pointer_cast<const GetTable>(pqp->left_input());
    ASSERT_TRUE(get_table);
    const auto &prunable_subquery_scans =
        get_table->prunable_subquery_predicates();
...
TEST_F(LQPTranslatorTest, WindowNode)
{
    ...
    const auto window_function =
        rank_(window_(expression_vector(), expression_vector(),
                      std::vector<SortMode>{}, std::move(frame)));
