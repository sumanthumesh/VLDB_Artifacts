...
class ProjectionNodeTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _projection_node = ProjectionNode::make(
            expression_vector(_c, _a, _b, add_(_b, _c), add_(_a, _c)),
            _mock_node);

        _key_constraint_a_b_pk = TableKeyConstraint{
...
TEST_F(ProjectionNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto different_projection_node_a = ProjectionNode::make(
        expression_vector(_a, _c, _b, add_(_b, _c), add_(_a, _c)), _mock_node);
    const auto different_projection_node_b = ProjectionNode::make(
        expression_vector(_c, _a, _b, add_(_b, _c)), _mock_node);
...
TEST_F(ProjectionNodeTest, UniqueColumnCombinationsReorderedColumns)
{
    ...
    _mock_node->set_key_constraints(
        {*_key_constraint_a_b_pk, *_key_constraint_b});
    EXPECT_EQ(_mock_node->unique_column_combinations().size(), 2);

    {
        // Reorder columns: (a, b, c) -> (c, a, b).
        _projection_node =
            ProjectionNode::make(expression_vector(_c, _a, _b), _mock_node);

        // Basic check.
        const auto &unique_column_combinations =
            _projection_node->unique_column_combinations();
        EXPECT_EQ(unique_column_combinations.size(), 2);
        // In-depth check.
        EXPECT_TRUE(find_ucc_by_key_constraint(*_key_constraint_a_b_pk,
                                               unique_column_combinations));
        EXPECT_TRUE(find_ucc_by_key_constraint(*_key_constraint_b,
                                               unique_column_combinations));
    }
...
TEST_F(ProjectionNodeTest, UniqueColumnCombinationsRemovedColumns)
{
    ...
    _projection_node =
        ProjectionNode::make(expression_vector(_a, _c), _mock_node);
    EXPECT_TRUE(_projection_node->unique_column_combinations().empty());
...

    {
        ...
        _projection_node =
            ProjectionNode::make(expression_vector(_a, _b), _mock_node);
...

    {
        ...
        _projection_node =
            ProjectionNode::make(expression_vector(_b), _mock_node);
