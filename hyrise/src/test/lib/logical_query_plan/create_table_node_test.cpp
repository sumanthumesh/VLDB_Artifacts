...
class CreateTableNodeTest : public BaseTest
{
  public:
    void SetUp() override
    {
        ...
        input_node = std::make_shared<StaticTableNode>(
            Table::create_dummy_table(column_definitions));
        create_table_node =
            CreateTableNode::make("some_table", false, input_node);
    }
...
TEST_F(CreateTableNodeTest, Description)
{
    EXPECT_EQ(create_table_node->description(),
              "[CreateTable] Name: 'some_table'");
    auto create_table_node_2 =
        CreateTableNode::make("some_table", true, input_node);
...
TEST_F(CreateTableNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto different_create_table_node_a =
        CreateTableNode::make("some_table2", false, input_node);
    const auto different_create_table_node_b =
        CreateTableNode::make("some_table", true, input_node);

    TableColumnDefinitions different_column_definitions;
    different_column_definitions.emplace_back("a", DataType::Int, false);
    const auto different_input_node = std::make_shared<StaticTableNode>(
        Table::create_dummy_table(different_column_definitions));
    const auto different_create_table_node_c =
        CreateTableNode::make("some_table", false, different_input_node);
...
{
    EXPECT_THROW(create_table_node->unique_column_combinations(),
                 std::logic_error);
}
