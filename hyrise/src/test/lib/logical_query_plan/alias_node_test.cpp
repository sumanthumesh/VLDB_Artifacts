...
class AliasNodeTest : public BaseTest
{
  public:
    void SetUp() override
    {
        ...
        partial_alias_node =
            AliasNode::make(expressions, partial_aliases, mock_node);
    }
...
TEST_F(AliasNodeTest, ShallowEqualsAndCopy)
{
    ...
    const auto node_mapping =
        lqp_create_node_mapping(alias_node, alias_node_copy);
...
TEST_F(AliasNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto alias_node_other_aliases = AliasNode::make(
        expressions, std::vector<std::string>{"a", "b"}, mock_node);
    ...
    const auto other_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>{expr_a, expr_b};
    const auto alias_node_other_expressions =
        AliasNode::make(other_expressions, aliases, mock_node);
    EXPECT_NE(*alias_node, *alias_node_other_expressions);
    const auto alias_node_other_left_input =
        AliasNode::make(expressions, aliases, other_mock_node);
    ...
    // alias_node == alias_node_other_left_input is false but the hash codes of
    // these nodes are equal. The reason for this is in the
    // LQPColumnExpressions: Semantically equal LQPColumnExpressions are not
    // equal if they refer to different original_nodes. This allows, e.g., for
    // self-joins. The hash function does not take the actual pointer into
    // account, so the hashes of semantically equal LQPColumnExpressions are
    // equal. The following lines show this fact in detail:
    EXPECT_NE(*a, *expr_a);
    EXPECT_NE(*b, *expr_b);
