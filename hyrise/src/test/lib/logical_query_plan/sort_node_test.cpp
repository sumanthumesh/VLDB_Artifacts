...
class SortNodeTest : public BaseTest
{
  protected:
    ...
    {
        _table_a =
            load_table("resources/test_data/tbl/int_float_double_string.tbl",
                       ChunkOffset{2});
        ...
        _sort_node = SortNode::make(expression_vector(_a_i),
...
TEST_F(SortNodeTest, Descriptions)
{
    ...
    auto sort_b = SortNode::make(expression_vector(_a_i),
    ...
    EXPECT_EQ(sort_c->description(),
              "[Sort] d (Descending), f (Ascending), i (Descending)");
}
...
TEST_F(SortNodeTest, HashingAndEqualityCheck)
{
    ...
    const auto sort_a = SortNode::make(
        expression_vector(_a_i), std::vector<SortMode>{SortMode::Descending},
        _table_node);
    const auto sort_b = SortNode::make(
        expression_vector(_a_d, _a_f, _a_i),
        std::vector<SortMode>{SortMode::Descending, SortMode::Ascending,
                              SortMode::Descending});
    const auto sort_c =
        SortNode::make(expression_vector(_a_i),
                       std::vector<SortMode>{SortMode::Ascending}, _table_node);
...
TEST_F(SortNodeTest, ForwardUniqueColumnCombinations)
{
    ...
    _table_a->add_soft_key_constraint(
    ...
    const auto &unique_column_combinations =
        _sort_node->unique_column_combinations();
