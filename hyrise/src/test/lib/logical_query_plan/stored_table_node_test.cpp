...
class StoredTableNodeTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        _table_a = load_table("resources/test_data/tbl/int_int_float.tbl",
                              ChunkOffset{1});
        ...
        Hyrise::get().storage_manager.add_table(
            "t_b", load_table("resources/test_data/tbl/int_int_float.tbl",
                              ChunkOffset{1}));

        _table_a->create_chunk_index<GroupKeyIndex>({ColumnID{0}}, "i_a1");
        _table_a->create_chunk_index<GroupKeyIndex>({ColumnID{1}}, "i_b");
        _table_a->create_chunk_index<CompositeGroupKeyIndex>(
            {ColumnID{0}, ColumnID{1}}, "i_a2");
        _table_a->create_chunk_index<CompositeGroupKeyIndex>(
            {ColumnID{1}, ColumnID{0}}, "i_a3");
...
TEST_F(StoredTableNodeTest, Description)
{
    ...
    EXPECT_EQ(stored_table_node_a->description(),
              "[StoredTable] Name: 't_a' pruned: 0/4 chunk(s), 0/3 column(s)");
    ...
    EXPECT_EQ(stored_table_node_b->description(),
              "[StoredTable] Name: 't_a' pruned: 1/4 chunk(s), 1/3 column(s)");
}
...
{
    const auto different_node_a = std::static_pointer_cast<StoredTableNode>(
        _stored_table_node->deep_copy());
    const auto different_node_b = std::static_pointer_cast<StoredTableNode>(
        _stored_table_node->deep_copy());
    const auto different_node_c = std::static_pointer_cast<StoredTableNode>(
        _stored_table_node->deep_copy());
    const auto different_node_d = std::static_pointer_cast<StoredTableNode>(
        _stored_table_node->deep_copy());
  ...
  PredicateNode::make(equals_(different_node_d->get_column("a"), lqp_subquery_(subquery_d)),
    ...
    // We force hash collisions for nodes with the same number of prunable
    // subquery predicates.
...
TEST_F(StoredTableNodeTest, Copy)
{
  ...
  PredicateNode::make(equals_(_a, lqp_subquery_(subquery)),
    ...
    const auto &prunable_subquery_predicates =
        static_cast<StoredTableNode &>(*lqp_deep_copy->left_input())
    ...
    // Do not allow deep copies where prunable subquery predicates are not part
    // of the LQP.
...
TEST_F(StoredTableNodeTest, GetStatisticsPruneFirstColumn)
{
    ...
    auto expected_statistics =
        _stored_table_node->chunk_indexes_statistics().at(1u);

    _stored_table_node->set_pruned_column_ids({ColumnID{0}});

    // column with ColumnID{0} was pruned, therefore the column has to be left
    // shifted
    expected_statistics.column_ids[0] -= 1;

    EXPECT_EQ(_stored_table_node->chunk_indexes_statistics().size(), 1u);
    EXPECT_EQ(_stored_table_node->chunk_indexes_statistics().at(0u),
              expected_statistics);
}
...
TEST_F(StoredTableNodeTest, GetStatisticsPruneSecondColumn)
{
    ...
    auto expected_statistics =
        _stored_table_node->chunk_indexes_statistics().at(0u);
...
TEST_F(StoredTableNodeTest, GetStatisticsPruneBothColumns)
{
    ...
    // All indexed columns were pruned, therefore the index statistics should be
    // empty
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesNone)
{
    ...
    // Constraint across all columns => No more columns available to create a
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesPrunedLeftColumnSet)
{
    ...
    // Prune unique column "a", which would be part of the left column set in
    // the resulting FD: {a} => {b, c}
    _stored_table_node->set_pruned_column_ids({ColumnID{0}});
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesPrunedLeftColumnSet2)
{
    const auto table = Hyrise::get().storage_manager.get_table("t_a");
    table->add_soft_key_constraint(
        {{_b->original_column_id}, KeyConstraintType::UNIQUE});
    ...
    EXPECT_TRUE(
        _stored_table_node->functional_dependencies().contains(fd_expected));
}
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesPrunedRightColumnSet)
{
    ...
    // Prune column "b", which would be part of the right column set in the
    // resulting FD: {a} => {b, c}
    _stored_table_node->set_pruned_column_ids({ColumnID{1}});
...
{
    const auto table =
        Hyrise::get().storage_manager.get_table("t_a"); // int_int_float.tbl
    table->add_soft_key_constraint(
        {{_a->original_column_id}, KeyConstraintType::UNIQUE});
    table->add_soft_key_constraint(
        {{_b->original_column_id}, KeyConstraintType::UNIQUE});
...
{
    // Create four identical tables of 3 columns (a, b, c), where the second
    // column of which is nullable (b)
    ...
    // Test {a} => {b, c}
    {
        ...
        table->add_soft_key_constraint(
            {{ColumnID{0}}, KeyConstraintType::UNIQUE});
    ...
    // Test {a, c} => {b}
    {
        const auto table =
            std::make_shared<Table>(column_definitions, TableType::Data);
        table->add_soft_key_constraint(
            {{ColumnID{0}, ColumnID{2}}, KeyConstraintType::UNIQUE});
    ...
    // Test {b} => {a, c}
    {
        const auto table =
            std::make_shared<Table>(column_definitions, TableType::Data);
        table->add_soft_key_constraint(
            {{ColumnID{1}}, KeyConstraintType::UNIQUE});
...
TEST_F(StoredTableNodeTest, UniqueColumnCombinations)
{
    ...
    const auto &unique_column_combinations =
        _stored_table_node->unique_column_combinations();

    // Basic check.
    EXPECT_EQ(unique_column_combinations.size(), 2);
    // In-depth check.
    EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_a_b,
                                           unique_column_combinations));
    EXPECT_TRUE(find_ucc_by_key_constraint(key_constraint_c,
                                           unique_column_combinations));
    ...
    for (const auto &ucc : unique_column_combinations)
    {
        for (const auto &expression : ucc.expressions)
        {
            ...
            EXPECT_TRUE(column_expression &&
                        !column_expression->original_node.expired());
            EXPECT_TRUE(column_expression->original_node.lock() ==
                        _stored_table_node);
        }
    }
}
