...
{
    auto table = load_table("resources/test_data/tbl/compression_input.tbl",
                            ChunkOffset{12});
    Hyrise::get().storage_manager.add_table("table", table);

    auto table_dict = load_table(
        "resources/test_data/tbl/compression_input.tbl", ChunkOffset{3});
    Hyrise::get().storage_manager.add_table("table_dict", table_dict);

    auto compression_task1 =
        std::make_shared<ChunkCompressionTask>("table_dict", ChunkID{0});
    compression_task1->set_done_callback(
        []()
        {
            auto compression_task2 = std::make_shared<ChunkCompressionTask>(
                "table_dict", std::vector<ChunkID>{ChunkID{1}, ChunkID{2}});
            Hyrise::get().scheduler()->schedule_and_wait_for_tasks(
                {compression_task2});
        });
    auto compression_task3 =
        std::make_shared<ChunkCompressionTask>("table_dict", ChunkID{3});

    Hyrise::get().scheduler()->schedule_and_wait_for_tasks(
        {compression_task1, compression_task3});
...
{
    auto table_dict = load_table(
        "resources/test_data/tbl/compression_input.tbl", ChunkOffset{6});
    Hyrise::get().storage_manager.add_table("table_dict", table_dict);
    ...
    auto dictionary_sizes =
        std::array<std::vector<size_t>, chunk_count>{{{3u, 3u}, {2u, 3u}}};
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
             ...
             ++column_id)
        {
            ...
            EXPECT_EQ(dict_segment->unique_values_count(),
                      dictionary_sizes[chunk_id][column_id]);
        }
    }
}
...
TEST_F(ChunkCompressionTaskTest, CompressionWithAbortedInsert)
{
    ...
    auto compression = std::make_shared<ChunkCompressionTask>(
        "table_insert",
        std::vector<ChunkID>{ChunkID{0}, ChunkID{1}, ChunkID{2}, ChunkID{3}});
    Hyrise::get().scheduler()->schedule_and_wait_for_tasks({compression});
