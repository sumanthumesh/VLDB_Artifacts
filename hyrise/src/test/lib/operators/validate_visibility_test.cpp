...
class OperatorsValidateVisibilityTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        t = std::make_shared<Table>(column_definitions, TableType::Data,
...
// taken from:
// https://github.com/hyrise/hyrise/blob/master/docs/documentation/queryexecution/tx.rst

// yes, yes, yes
TEST_F(OperatorsValidateVisibilityTest, Impossible)
{
    auto context = std::make_shared<TransactionContext>(
        TransactionID{2}, CommitID{2}, AutoCommit::No);

    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_tid(ChunkOffset{0}, TransactionID{2});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_begin_cid(ChunkOffset{0}, CommitID{2});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_end_cid(ChunkOffset{0}, CommitID{2});
...
TEST_F(OperatorsValidateVisibilityTest, PastDelete)
{
    ...
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_tid(ChunkOffset{0}, TransactionID{42});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_begin_cid(ChunkOffset{0}, CommitID{2});
    t->get_chunk(ChunkID{0})
...
TEST_F(OperatorsValidateVisibilityTest, Impossible2)
{
    ...
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_begin_cid(ChunkOffset{0}, CommitID{4});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_end_cid(ChunkOffset{0}, CommitID{1});
...
TEST_F(OperatorsValidateVisibilityTest, OwnDeleteUncommitted)
{
    ...
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_begin_cid(ChunkOffset{0}, CommitID{1});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_end_cid(ChunkOffset{0}, CommitID{6});
...
TEST_F(OperatorsValidateVisibilityTest, Impossible3)
{
    ...
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_tid(ChunkOffset{0}, TransactionID{50});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_begin_cid(ChunkOffset{0}, CommitID{3});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_end_cid(ChunkOffset{0}, CommitID{1});
...
TEST_F(OperatorsValidateVisibilityTest, OwnInsert)
{
    ...
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_end_cid(ChunkOffset{0}, CommitID{3});
...
TEST_F(OperatorsValidateVisibilityTest, PastInsertOrFutureDelete)
{
    ...
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_tid(ChunkOffset{0}, TransactionID{99});
    t->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_begin_cid(ChunkOffset{0}, CommitID{2});
    t->get_chunk(ChunkID{0})
