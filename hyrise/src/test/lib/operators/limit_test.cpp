...
class OperatorsLimitTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        _table_wrapper = std::make_shared<TableWrapper>(
    ...
    {
        auto limit =
            std::make_shared<Limit>(_input_operator, to_expression(int64_t{1}));
        limit->execute();

        auto expected_result = load_table(
            "resources/test_data/tbl/int_int3_limit_1.tbl", ChunkOffset{3});
    ...
    {
        auto limit =
            std::make_shared<Limit>(_input_operator, to_expression(int64_t{2}));
        limit->execute();

        auto expected_result = load_table(
            "resources/test_data/tbl/int_int3_limit_2.tbl", ChunkOffset{3});
        EXPECT_TABLE_EQ_ORDERED(limit->get_output(), expected_result);
    }
    ...
    {
        auto limit =
            std::make_shared<Limit>(_input_operator, to_expression(int64_t{4}));
        limit->execute();

        auto expected_result = load_table(
            "resources/test_data/tbl/int_int3_limit_4.tbl", ChunkOffset{3});
        EXPECT_TABLE_EQ_ORDERED(limit->get_output(), expected_result);
    }
    ...
    void test_limit_10()
    {
        auto limit = std::make_shared<Limit>(_input_operator,
                                             to_expression(int64_t{10}));
        limit->execute();
...
TEST_F(OperatorsLimitTest, Limit1ReferenceSegment)
{
    ...
    auto table_scan = create_table_scan(_table_wrapper, ColumnID{0},
...
{
    auto limit =
        std::make_shared<Limit>(_table_wrapper, to_expression(int64_t{4}));
    limit->execute();

    const auto result_table_unsorted = limit->get_output();
    const auto chunk_count_unsorted = result_table_unsorted->chunk_count();
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count_unsorted;
         ++chunk_id)
    {
...
{
    const auto limit_string =
        std::make_shared<Limit>(_table_wrapper, value_("test_string"));
    EXPECT_THROW(limit_string->execute(), std::logic_error);

    const auto limit_float =
        std::make_shared<Limit>(_table_wrapper, value_(2.0f));
