...
namespace hyrise
{
// This is the base class for all tests that should run on every possible
// combination of data type, order and encoding. A sample table is generate in
// TableScanBetweenTest. Depending on how other tests will use this, it might
// make sense to have a single instantiation here
class TypedOperatorBaseTest
    : public BaseTestWithParam<std::tuple<
          DataType, EncodingType, std::optional<SortMode>, bool /*nullable*/>>
{
  public:
    static std::string format(testing::TestParamInfo<ParamType> info)
    {
        ...
        output << data_type << encoding
               << (sort_mode ? magic_enum::enum_name(*sort_mode) : "Unsorted")
