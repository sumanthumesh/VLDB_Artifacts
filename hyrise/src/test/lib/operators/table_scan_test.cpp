      ...
      public ::testing::WithParamInterface<EncodingType>
{
  protected:
    void SetUp() override
    {
        ...
        auto int_int_7 = load_table(
            "resources/test_data/tbl/int_int_shuffled.tbl", ChunkOffset{7});
        auto int_int_5 = load_table(
            "resources/test_data/tbl/int_int_shuffled_2.tbl", ChunkOffset{5});

        ChunkEncoder::encode_chunks(int_int_7, {ChunkID{0}, ChunkID{1}},
                                    SegmentEncodingSpec{_encoding_type});
        // partly compressed table
        ChunkEncoder::encode_chunks(int_int_5, {ChunkID{0}, ChunkID{1}},
                                    SegmentEncodingSpec{_encoding_type});

        _int_int_compressed =
            std::make_shared<TableWrapper>(std::move(int_int_7));
        _int_int_compressed->never_clear_output();
        _int_int_compressed->execute();
        _int_int_partly_compressed =
            std::make_shared<TableWrapper>(std::move(int_int_5));
    ...
    std::shared_ptr<TableWrapper> load_and_encode_table(
        ...
        const std::optional<std::vector<SortColumnDefinition>> sorted_by =
            std::nullopt)
    {
        ...
        {
            if (encoding_supports_data_type(_encoding_type,
    ...
    std::shared_ptr<TableWrapper> get_int_sorted_op()
    {
        return load_and_encode_table(
            ...
            std::make_optional(std::vector<SortColumnDefinition>{
                SortColumnDefinition(ColumnID(0), SortMode::Ascending)}));
    }
    ...
    std::shared_ptr<TableWrapper>
    get_int_float_with_null_op(const ChunkOffset chunk_size = ChunkOffset{2})
    {
        return load_and_encode_table(
            "resources/test_data/tbl/int_float_with_null.tbl", chunk_size);
    }
    ...
    std::shared_ptr<TableWrapper> get_table_op_filtered()
    {
        ...
        const auto table = std::make_shared<Table>(table_column_definitions,
                                                   TableType::References);

        const auto test_table_part_compressed =
            _int_int_partly_compressed->get_output();
        ...
        auto segment_a = std::make_shared<ReferenceSegment>(
            test_table_part_compressed, ColumnID{0}, pos_list);
        auto segment_b = std::make_shared<ReferenceSegment>(
            test_table_part_compressed, ColumnID{1}, pos_list);
    ...
    std::shared_ptr<TableWrapper>
    get_table_op_with_n_dict_entries(const int num_entries)
    {
        // Set up dictionary encoded table with a dictionary consisting of
        // num_entries entries.
        auto table_column_definitions = TableColumnDefinitions{};
        table_column_definitions.emplace_back("a", DataType::Int, false);

        const auto table = std::make_shared<Table>(
            table_column_definitions, TableType::Data, ChunkOffset{100'000});
    ...
    create_referencing_table_w_null_row_id(const bool references_dict_segment)
    {
        const auto table =
            load_table("resources/test_data/tbl/int_int_w_null_8_rows.tbl",
                       ChunkOffset{4});

        if (references_dict_segment)
        {
            ChunkEncoder::encode_all_chunks(
                table, SegmentEncodingSpec{_encoding_type});
        }

        auto pos_list_a = std::make_shared<RowIDPosList>(
            RowIDPosList{RowID{ChunkID{0}, ChunkOffset{1}},
                         RowID{ChunkID{1}, ChunkOffset{0}},
                         RowID{ChunkID{0}, ChunkOffset{2}},
                         RowID{ChunkID{0}, ChunkOffset{3}}});
        auto ref_segment_a =
            std::make_shared<ReferenceSegment>(table, ColumnID{0}, pos_list_a);

        auto pos_list_b = std::make_shared<RowIDPosList>(
            RowIDPosList{NULL_ROW_ID, RowID{ChunkID{0}, ChunkOffset{0}},
                         RowID{ChunkID{1}, ChunkOffset{2}},
                         RowID{ChunkID{0}, ChunkOffset{1}}});
        auto ref_segment_b =
            std::make_shared<ReferenceSegment>(table, ColumnID{1}, pos_list_b);

        TableColumnDefinitions column_definitions;
        column_definitions.emplace_back("a", DataType::Int, true);
        column_definitions.emplace_back("b", DataType::Int, true);
        auto ref_table =
            std::make_shared<Table>(column_definitions, TableType::References);
    ...
    void scan_for_null_values(
        const std::shared_ptr<AbstractOperator> in,
        const std::map<PredicateCondition, std::vector<AllTypeVariant>> &tests)
    {
        for (const auto &test : tests)
        {
            ...
            auto scan = create_table_scan(in, ColumnID{1}, predicate_condition,
                                          NULL_VALUE);
            scan->execute();
    ...
    void ASSERT_COLUMN_EQ(std::shared_ptr<const Table> table,
                          ...
                          std::vector<AllTypeVariant> expected)
    {
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
        {
            ...
            for (auto chunk_offset = ChunkOffset{0}; chunk_offset < chunk_size;
                 ++chunk_offset)
            {
                ...
                const auto comparator =
                    [found_value](const AllTypeVariant expected_value)
                {
                    // returns equivalency, not equality to simulate
                    // std::multiset. multiset cannot be used because it
                    // triggers a compiler / lib bug when built in CI
                    return !(found_value < expected_value) &&
                           !(expected_value < found_value);
                };

                auto search =
                    std::find_if(expected.begin(), expected.end(), comparator);
    ...
    void scan_and_check_sorted_by(
        const std::shared_ptr<TableWrapper> table_wrapper) const
    {
        const auto scan_sorted =
            create_table_scan(table_wrapper, ColumnID{0},
                              PredicateCondition::GreaterThanEquals, 1234);
        scan_sorted->execute();
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
        {
            const auto &actual_sorted_by =
                result_table_sorted->get_chunk(chunk_id)
                    ->individually_sorted_by();
            ASSERT_TRUE(!actual_sorted_by.empty());
            const auto expected_sorted_by = std::vector<SortColumnDefinition>{
                SortColumnDefinition(ColumnID{0}, SortMode::Ascending)};
...
{
    const auto expected_result = load_table(
        "resources/test_data/tbl/int_float_filtered.tbl", ChunkOffset{2});

    auto scan_1 =
        create_table_scan(get_int_float_op(), ColumnID{0},
                          PredicateCondition::GreaterThanEquals, 1234);
    scan_1->execute();

    auto scan_2 = create_table_scan(scan_1, ColumnID{1},
                                    PredicateCondition::LessThan, 457.9);
    scan_2->execute();
...
{
    auto scan_1 = create_table_scan(get_int_float_op(), ColumnID{0},
                                    PredicateCondition::GreaterThan, 90000);
    scan_1->execute();
...
{
    const auto expected_result = load_table(
        "resources/test_data/tbl/int_float_filtered2.tbl", ChunkOffset{1});

    auto scan = create_table_scan(get_int_float_op(), ColumnID{0},
                                  PredicateCondition::GreaterThanEquals, 1234);
    scan->execute();
...
{
    const auto expected_result = load_table(
        "resources/test_data/tbl/int_sorted_filtered.tbl", ChunkOffset{1});

    auto scan = create_table_scan(get_int_sorted_op(), ColumnID{0},
...
TEST_P(OperatorsTableScanTest,
       SingleScanWithSortedSegmentEqualsAllElementsEqualNull)
{
    const auto expected_result = load_table(
        "resources/test_data/tbl/int_empty_nullable.tbl", ChunkOffset{1});

    auto scan = create_table_scan(get_int_only_null_op(), ColumnID{0},
                                  PredicateCondition::Equals, 2);
    scan->execute();
...
TEST_P(OperatorsTableScanTest,
       SingleScanWithSortedSegmentEqualsAllPredicateValueLarger)
{
    const auto expected_result =
        load_table("resources/test_data/tbl/int_empty.tbl", ChunkOffset{1});

    auto scan = create_table_scan(get_int_sorted_op(), ColumnID{0},
                                  PredicateCondition::Equals, 6);
    scan->execute();
...
{
    const auto expected_result = load_table(
        "resources/test_data/tbl/int_sorted_filtered2.tbl", ChunkOffset{1});

    auto scan = create_table_scan(get_int_sorted_op(), ColumnID{0},
                                  PredicateCondition::NotEquals, 2);
    scan->execute();
...
TEST_P(OperatorsTableScanTest, SingleScanWithSubquery)
{
    ...
    const auto subquery_pqp = std::make_shared<Projection>(
        std::make_shared<TableWrapper>(Projection::dummy_table()),
    ...
    const auto scan = std::make_shared<TableScan>(
        get_int_float_op(),
        greater_than_equals_(
            pqp_column_(ColumnID{0}, DataType::Int, false, "a"),
            pqp_subquery_(subquery_pqp, DataType::Int, false)));
    EXPECT_TRUE(
        dynamic_cast<ColumnVsValueTableScanImpl *>(scan->create_impl().get()));
    scan->execute();
    EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
{
    auto expected_result = load_table(
        "resources/test_data/tbl/int_float_filtered2.tbl", ChunkOffset{1});
    ...
    const auto scan = std::make_shared<TableScan>(
        ...
        between_inclusive_(pqp_column_(ColumnID{0}, DataType::Int, false, "a"),
                           ...
                           value_(12345)));
    EXPECT_TRUE(
        dynamic_cast<ColumnBetweenTableScanImpl *>(scan->create_impl().get()));
    scan->execute();
    EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
{
    const auto expected_result = Table::create_dummy_table(
        get_int_float_op()->table->column_definitions());
    const auto dummy_table =
        Table::create_dummy_table({{"dummy", DataType::Int, false}});
...
TEST_P(OperatorsTableScanTest, SingleScanWithInvalidSubquery)
{
    ...
    const auto subquery_pqp = std::make_shared<Projection>(
        get_int_string_op(), expression_vector(value_(1234)));
    subquery_pqp->execute();
...
{
    // We do not need to check for a non existing value, because that happens
    // automatically when we scan the second chunk.

    auto tests = std::map<PredicateCondition, std::vector<AllTypeVariant>>{};
    tests[PredicateCondition::Equals] = {106, 106};
    tests[PredicateCondition::NotEquals] = {100, 102, 104, 108, 110, 112,
                                            100, 102, 104, 108, 110, 112};
    tests[PredicateCondition::LessThan] = {100, 102, 104, 100, 102, 104};
    tests[PredicateCondition::LessThanEquals] = {100, 102, 104, 106,
                                                 100, 102, 104, 106};
    tests[PredicateCondition::GreaterThan] = {108, 110, 112, 108, 110, 112};
    tests[PredicateCondition::GreaterThanEquals] = {106, 108, 110, 112,
                                                    106, 108, 110, 112};
    tests[PredicateCondition::IsNull] = {};
    tests[PredicateCondition::IsNotNull] = {100, 102, 104, 106, 108, 110, 112,
                                            100, 102, 104, 106, 108, 110, 112};

    for (const auto &test : tests)
    {
        auto scan_int =
            create_table_scan(_int_int_compressed, ColumnID{0}, test.first, 6);
        auto scan_int_partly = create_table_scan(_int_int_partly_compressed,
                                                 ColumnID{0}, test.first, 6);
        ...
        ASSERT_COLUMN_EQ(scan_int_partly->get_output(), ColumnID{1},
                         test.second);
    }
}
...
TEST_P(OperatorsTableScanTest, ScanOnReferencedCompressedSegments)
{
    ...
    {
        // the first scans will match all values since the predicate condition
        // evaluates to true for all
        auto scan1 = create_table_scan(_int_int_compressed, ColumnID{1},
                                       PredicateCondition::LessThan, 108);
        scan1->execute();

        auto scan_partly1 =
            create_table_scan(_int_int_partly_compressed, ColumnID{1},
                              PredicateCondition::LessThan, 108);
        scan_partly1->execute();

        auto scan2 =
            create_table_scan(scan1, ColumnID{0}, predicate_condition, 4);
        auto scan_partly2 = create_table_scan(scan_partly1, ColumnID{0},
                                              predicate_condition, 4);
...
TEST_P(OperatorsTableScanTest, ScanWeirdPosList)
{
    ...
    {
        auto scan_partly =
            create_table_scan(table_wrapper, ColumnID{0}, test.first, 10);
        scan_partly->execute();
...
TEST_P(OperatorsTableScanTest,
       ScanOnCompressedSegmentsValueGreaterThanMaxDictionaryValue)
{
    const auto all_rows = std::vector<AllTypeVariant>{
        100, 102, 104, 106, 108, 110, 112, 100, 102, 104, 106, 108, 110, 112};
    ...
    {
        auto scan =
            create_table_scan(_int_int_compressed, ColumnID{0}, test.first, 30);
        scan->execute();

        auto scan_partly = create_table_scan(_int_int_partly_compressed,
                                             ColumnID{0}, test.first, 30);
        scan_partly->execute();
...
TEST_P(OperatorsTableScanTest,
       ScanOnCompressedSegmentsValueLessThanMinDictionaryValue)
{
    const auto all_rows = std::vector<AllTypeVariant>{
        100, 102, 104, 106, 108, 110, 112, 100, 102, 104, 106, 108, 110, 112};
    const auto no_rows = std::vector<AllTypeVariant>{};
    ...
    for (const auto &test : tests)
    {
        auto scan = create_table_scan(_int_int_compressed,
                                      ColumnID{0} /* "a" */, test.first, -10);
        scan->execute();

        auto scan_partly = create_table_scan(
            _int_int_partly_compressed, ColumnID{0} /* "a" */, test.first, -10);
        scan_partly->execute();
...
TEST_P(OperatorsTableScanTest,
       ScanOnIntValueSegmentWithFloatColumnWithNullValues)
{
    auto table = load_table("resources/test_data/tbl/int_int_w_null_8_rows.tbl",
                            ChunkOffset{4});
    ...
    const auto predicate =
        greater_than_(get_column_expression(table_wrapper, ColumnID{0}),
...
TEST_P(OperatorsTableScanTest,
       ScanOnReferencedIntValueSegmentWithFloatColumnWithNullValues)
{
    auto table = load_table("resources/test_data/tbl/int_int_w_null_8_rows.tbl",
                            ChunkOffset{4});

    auto table_wrapper =
        std::make_shared<TableWrapper>(to_simple_reference_table(table));
    table_wrapper->execute();
...
TEST_P(OperatorsTableScanTest,
       ScanOnIntCompressedSegmentsWithFloatColumnWithNullValues)
{
    auto table = load_table("resources/test_data/tbl/int_int_w_null_8_rows.tbl",
                            ChunkOffset{4});
...
TEST_P(OperatorsTableScanTest,
       ScanOnReferencedIntCompressedSegmentsWithFloatColumnWithNullValues)
{
    auto table = load_table("resources/test_data/tbl/int_int_w_null_8_rows.tbl",
                            ChunkOffset{4});
    ChunkEncoder::encode_all_chunks(table, SegmentEncodingSpec{_encoding_type});
...
TEST_P(OperatorsTableScanTest, ScanOnCompressedSegmentsAroundBounds)
{
    ...
    tests[PredicateCondition::GreaterThan] = {102, 104, 106, 108, 110, 112,
                                              102, 104, 106, 108, 110, 112};
    tests[PredicateCondition::GreaterThanEquals] = {
        100, 102, 104, 106, 108, 110, 112, 100, 102, 104, 106, 108, 110, 112};
    tests[PredicateCondition::NotEquals] = {102, 104, 106, 108, 110, 112,
                                            102, 104, 106, 108, 110, 112};
    tests[PredicateCondition::IsNull] = {};
    tests[PredicateCondition::IsNotNull] = {100, 102, 104, 106, 108, 110, 112,
                                            100, 102, 104, 106, 108, 110, 112};
    ...
    {
        auto scan =
            create_table_scan(_int_int_compressed, ColumnID{0}, test.first, 0);
        scan->execute();
...
TEST_P(OperatorsTableScanTest, ScanWithEmptyInput)
{
    auto scan_1 = std::make_shared<TableScan>(
        get_int_float_op(),
        greater_than_(get_column_expression(get_int_float_op(), ColumnID{0}),
                      12345));
    scan_1->execute();
    ...
    auto scan_2 = std::make_shared<TableScan>(
        scan_1, equals_(get_column_expression(scan_1, ColumnID{1}), 456.7));
    scan_2->execute();
...
TEST_P(OperatorsTableScanTest, ScanOnWideDictionarySegment)
{
    ...
    const auto table_wrapper_dict_16 =
        get_table_op_with_n_dict_entries((1 << 8) + 1);
    auto scan_1 = std::make_shared<TableScan>(
        table_wrapper_dict_16,
        greater_than_(get_column_expression(table_wrapper_dict_16, ColumnID{0}),
                      200));
    scan_1->execute();
    ...
    const auto table_wrapper_dict_32 =
        get_table_op_with_n_dict_entries((1 << 16) + 1);
    auto scan_2 = std::make_shared<TableScan>(
        table_wrapper_dict_32,
        greater_than_(get_column_expression(table_wrapper_dict_32, ColumnID{0}),
                      65500));
    scan_2->execute();
...
{
    auto table_wrapper = std::make_shared<TableWrapper>(load_table(
        "resources/test_data/tbl/int_int_w_null_8_rows.tbl", ChunkOffset{4}));
    table_wrapper->never_clear_output();
    table_wrapper->execute();

    const auto tests =
        std::map<PredicateCondition, std::vector<AllTypeVariant>>{
            {PredicateCondition::IsNull, {12, 123}},
            {PredicateCondition::IsNotNull,
             {12345, NULL_VALUE, 1234, 12345, 12, 1234}}};
...
TEST_P(OperatorsTableScanTest, ScanForNullValuesOnCompressedSortedSegments)
{
    const auto table = load_table(
        "resources/test_data/tbl/int_null_sorted_asc_2.tbl", ChunkOffset{4});
    table->get_chunk(ChunkID{0})
        ->set_individually_sorted_by(
...
TEST_P(OperatorsTableScanTest,
       ScanForNullValuesOnCompressedDescendingSortedSegments)
{
    const auto table = load_table(
        "resources/test_data/tbl/int_null_sorted_desc_2.tbl", ChunkOffset{4});
    table->get_chunk(ChunkID{0})
        ->set_individually_sorted_by(
...
{
    auto table =
        load_table("resources/test_data/tbl/int_float.tbl", ChunkOffset{4});
...
TEST_P(OperatorsTableScanTest,
       ScanForNullValuesOnReferencedValueSegmentWithoutNulls)
{
    auto table =
        load_table("resources/test_data/tbl/int_float.tbl", ChunkOffset{4});
...
TEST_P(OperatorsTableScanTest,
       ScanForNullValuesWithNullRowIDOnReferencedValueSegment)
{
    ...
    const auto tests =
        std::map<PredicateCondition, std::vector<AllTypeVariant>>{
            {PredicateCondition::IsNull, {123, 1234}},
...
TEST_P(OperatorsTableScanTest,
       ScanForNullValuesWithNullRowIDOnReferencedCompressedSegments)
{
...
TEST_P(OperatorsTableScanTest, ComparisonOfIntColumnAndNullValue)
{
    ...
    {
        auto scan = create_table_scan(get_int_float_with_null_op(), ColumnID{0},
                                      predicate_condition, NullValue{});
        scan->execute();

        EXPECT_EQ(scan->get_output()->row_count(), 0u);

        for (auto chunk_id = ChunkID{0};
             chunk_id < scan->get_output()->chunk_count(); chunk_id++)
        {
            EXPECT_EQ(scan->get_output()->get_chunk(chunk_id)->column_count(),
                      2u);
        }
    }
}
...
TEST_P(OperatorsTableScanTest, ComparisonOfStringColumnAndNullValue)
{
    ...
    for (auto predicate_condition : predicate_conditions)
    {
        auto scan = create_table_scan(get_int_string_op(), ColumnID{1},
                                      predicate_condition, NullValue{});
        scan->execute();
...
{
    // Scan implementations will potentially optimize the scan if they can
    // detect that all values in the column match the predicate. Test that if
    // the predicate matches all values in the Chunk, rows with NULL values in
    // the scanned column are nevertheless excluded from the result. E.g. [NULL,
    // 1, 2, NULL] < 5 = [1, 2]

    /**
     * Binary Predicates
     */
    auto predicates = std::vector<
        std::tuple<ColumnID, PredicateCondition, AllTypeVariant,
                   std::vector<AllTypeVariant>>>{
        {ColumnID{0},
         PredicateCondition::Equals,
         1234,
         {1234}}, // Matches all in second chunk
    ...
    for (const auto &[column_id, predicate_condition, value, expected_values] :
         predicates)
    {
        const auto scan =
            create_table_scan(table, column_id, predicate_condition, value);
        scan->execute();
     ...
     * BETWEEN
     */
    const auto between_scan = create_table_scan(
        table, ColumnID{0}, PredicateCondition::BetweenInclusive, 0, 15'000);
    between_scan->execute();
    ASSERT_COLUMN_EQ(between_scan->get_output(), ColumnID{0},
                     {12345, 123, 1234});
    ...
    const auto table_with_null_chunk =
        load_and_encode_table("resources/test_data/tbl/int_int_int_null.tbl");
    const auto is_null_scan =
        create_table_scan(table_with_null_chunk, ColumnID{1},
                          PredicateCondition::IsNull, NullValue{});
    is_null_scan->execute();
    ASSERT_COLUMN_EQ(is_null_scan->get_output(), ColumnID{0}, {11, 9});
    ASSERT_COLUMN_EQ(is_null_scan->get_output(), ColumnID{1},
                     {NullValue{}, NullValue{}});
    ...
    const auto is_not_null_scan = create_table_scan(
        table, ColumnID{0}, PredicateCondition::IsNotNull, NullValue{});
    is_not_null_scan->execute();
    ASSERT_COLUMN_EQ(is_not_null_scan->get_output(), ColumnID{0},
                     {12345, 123, 1234});
}

TEST_P(OperatorsTableScanTest, ScanWithExcludedFirstChunk)
{
    const auto expected = std::vector<AllTypeVariant>{100, 102, 104, 106, 108,
                                                      110, 112, 102, 104};

    auto scan = std::make_shared<TableScan>(
        _int_int_partly_compressed,
        greater_than_equals_(
            get_column_expression(_int_int_partly_compressed, ColumnID{0}), 0));
    scan->excluded_chunk_ids = std::make_shared<std::vector<ChunkID>>(
        std::initializer_list<ChunkID>{ChunkID{0}});
    scan->execute();
...
{
    auto predicates =
        std::vector<std::tuple<ColumnID, PredicateCondition, AllTypeVariant,
                               std::vector<AllTypeVariant>>>{
...
TEST_P(OperatorsTableScanTest, SetParameters)
{
    ...
    //       Therefore, we deep copy the TableWrapper _int_int_compressed to
    //       reset its execution state.
    const auto scan_a = std::make_shared<TableScan>(
        _int_int_compressed->deep_copy(), greater_than_equals_(column, 4));
    ...
    const auto scan_b = std::make_shared<TableScan>(
        _int_int_compressed->deep_copy(),
        greater_than_equals_(column, placeholder_(ParameterID{2})));
    scan_b->set_parameters(parameters);
    EXPECT_EQ(*scan_b->predicate(),
              *greater_than_equals_(column, parameter_expression_with_value));
...
TEST_P(OperatorsTableScanTest, GetImpl)
{
    ...
    const auto column_s =
        pqp_column_(ColumnID{1}, DataType::String, false, "c");
    const auto column_an =
        pqp_column_(ColumnID{0}, DataType::String, true, "a");
...
{
    // To stress-test the SIMD scan, which only operates on bigger tables, the
    // generated table holds 1'000 rows. For each fifth row, column a is NULL.
    // Otherwise, a is 100'000 + i, b is the index in the list of non-NULL
    // values.

    auto column_definitions = TableColumnDefinitions{
        {"a", DataType::Int, true}, {"b", DataType::Int, true}};
    const auto data_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{13});
    ...
    // We have two full chunks and one open chunk, we only encode the full
    // chunks
    for (auto chunk_id = ChunkID{0}; chunk_id < 2; ++chunk_id)
    {
        ChunkEncoder::encode_chunk(
            data_table->get_chunk(chunk_id), {DataType::Int, DataType::Int},
            {SegmentEncodingSpec{_encoding_type},
    ...
    // Scan for a >= 100'100
    const auto scan_a = std::make_shared<TableScan>(
        data_table_wrapper, greater_than_equals_(column_a, 100'100));
    scan_a->execute();

    // Now scan for b <= 100'700
    const auto scan_b = std::make_shared<TableScan>(
        scan_a, less_than_equals_(column_a, 100'700));
    scan_b->execute();

    // Try the same with a between scan
    const auto between_scan = std::make_shared<TableScan>(
        data_table_wrapper, between_inclusive_(column_a, 100'100, 100'700));
    between_scan->execute();
...
{
    // Verify that the sorted_by flag is not set when it's not present in left
    // input.
    ...
    EXPECT_TRUE(result_table_unsorted->get_chunk(ChunkID{0})
                    ->individually_sorted_by()
                    .empty());
    EXPECT_TRUE(result_table_unsorted->get_chunk(ChunkID{1})
                    ->individually_sorted_by()
                    .empty());
}
...
{
    // In the current implementation, scans on tables that do not contain
    // ReferenceSegments are executed chunk-by-chunk. Each input chunk will lead
    // to one output chunk (assuming there are qualifying tuples). The position
    // list thus references only a single chunk, allowing the scan to simply
    // forward the initial sorting order.
    const auto scan_sorted =
        create_table_scan(get_int_sorted_op(), ColumnID{0},
                          PredicateCondition::LessThanEquals, 1234);
    scan_sorted->execute();
    const auto &result_chunk_sorted =
        scan_sorted->get_output()->get_chunk(ChunkID{0});

    const auto ref_segment = std::dynamic_pointer_cast<const ReferenceSegment>(
        result_chunk_sorted->get_segment(ColumnID{0}));
    EXPECT_TRUE(ref_segment->pos_list()->references_single_chunk());

    const auto expected_sorted_by = std::vector<SortColumnDefinition>{
        SortColumnDefinition(ColumnID{0}, SortMode::Ascending)};
    EXPECT_EQ(result_chunk_sorted->individually_sorted_by(),
              expected_sorted_by);
}

// Reference segments that only reference a single chunk allow forwarding the
// sorting flags.
TEST_P(OperatorsTableScanTest, SortedFlagReferenceSegments)
{
    const auto table = get_int_sorted_op()->table;

    const auto ref_table = std::make_shared<Table>(table->column_definitions(),
                                                   TableType::References);
    ...
    const auto segment_1 =
        std::make_shared<ReferenceSegment>(table, ColumnID{0}, pos_list_1);
    ...
    const auto segment_2 =
        std::make_shared<ReferenceSegment>(table, ColumnID{0}, pos_list_2);
    ...
    const auto scan_sorted = create_table_scan(
        table_wrapper, ColumnID{0}, PredicateCondition::LessThanEquals, 4);
    scan_sorted->execute();
    ...
    {
        const auto &result_chunk_sorted =
            scan_sorted->get_output()->get_chunk(chunk_id);

        const auto ref_segment =
            std::dynamic_pointer_cast<const ReferenceSegment>(
                result_chunk_sorted->get_segment(ColumnID{0}));
        EXPECT_TRUE(ref_segment->pos_list()->references_single_chunk());

        const auto &chunk_sorted_by =
            result_chunk_sorted->individually_sorted_by();
        ASSERT_EQ(chunk_sorted_by, std::vector{SortColumnDefinition(
                                       ColumnID{0}, SortMode::Ascending)});
    }
}

// Scanning two sorted reference segments. Currently, the sorting information is
// not passed forward when reference segments do not guarantee that they only
// reference a single physical chunk (see table_scan.cpp). An exception is if
// the entire chunk matches and is simply forwarded.
TEST_P(OperatorsTableScanTest, SortedFlagSingleChunkNotGuaranteed)
{
    ...
    const auto scan_sorted = create_table_scan(
        table_wrapper, ColumnID{0}, PredicateCondition::GreaterThanEquals, 2);
    scan_sorted->execute();
    ...
    {
        // Chunk 1 matches entirely and the chunk (including the sorting
        // information) is forwarded.
        const auto &result_chunk_sorted =
            scan_sorted->get_output()->get_chunk(ChunkID{1});
        const auto &chunk_sorted_by =
            result_chunk_sorted->individually_sorted_by();
        ASSERT_EQ(chunk_sorted_by.size(), 1);
        ASSERT_EQ(chunk_sorted_by.at(0),
                  SortColumnDefinition(ColumnID(0), SortMode::Ascending));
    }
}

// Tables with ReferenceSegments that reference multiple chunks are first
// separated by the ChunkID (see
// AbstractDereferencedColumnTableScanImpl::_scan_reference_segment) and thus
// the order may be destroyed. Due to the missing single chunk guarantee, no
// sorting information is forwarded. An exception is if the entire chunk matches
// and is simply forwarded. This is NOT tested here.
...
TEST_P(OperatorsTableScanTest, DeepCopyRetainsExcludedChunks)
{
    const auto table_scan =
        create_table_scan(get_int_float_op(), ColumnID{0},
                          PredicateCondition::GreaterThanEquals, 1234);
    table_scan->excluded_chunk_ids = std::make_shared<std::vector<ChunkID>>(
        std::initializer_list<ChunkID>{ChunkID{0}});
    const auto new_table_scan =
        std::dynamic_pointer_cast<TableScan>(table_scan->deep_copy());
    EXPECT_EQ(*table_scan->excluded_chunk_ids,
              *new_table_scan->excluded_chunk_ids);
    EXPECT_EQ(table_scan->excluded_chunk_ids->data(),
              new_table_scan->excluded_chunk_ids
                  ->data()); // Should be the same object.
}
