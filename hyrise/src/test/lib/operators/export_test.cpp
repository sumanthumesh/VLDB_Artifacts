...
class OperatorsExportTest : public BaseTest
{
  protected:
    ...
    const std::map<FileType, std::string> reference_filenames{
        {FileType::Binary, "bin/float.bin"}, {FileType::Csv, "csv/float.csv"}};
    const std::map<FileType, std::string> file_extensions{
        {FileType::Binary, ".bin"}, {FileType::Csv, ".csv"}};
};

class OperatorsExportMultiFileTypeTest
    : public OperatorsExportTest,
      public ::testing::WithParamInterface<FileType>
{
};

INSTANTIATE_TEST_SUITE_P(FileTypes, OperatorsExportMultiFileTypeTest,
                         ::testing::Values(FileType::Csv, FileType::Binary),
...
TEST_P(OperatorsExportMultiFileTypeTest, ExportWithFileType)
{
    ...
    auto exporter =
        std::make_shared<Export>(table_wrapper, test_filename, GetParam());
    exporter->execute();
...
{
    const auto exporter =
        std::make_shared<Export>(nullptr, "a/file/path", GetParam());
...
TEST_F(OperatorsExportTest, NonsensePath)
{
    ...
    table_wrapper->execute();
    auto exporter =
        std::make_shared<Export>(table_wrapper, "this/path/does/not/exist.tbl");
...
TEST_F(OperatorsExportTest, UnknownFileExtension)
{
    ...
    EXPECT_THROW(
        std::make_shared<Export>(table_wrapper, "not_existing_file.mp3"),
        std::exception);
}
