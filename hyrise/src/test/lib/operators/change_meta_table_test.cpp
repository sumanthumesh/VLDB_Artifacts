...
class ChangeMetaTableTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        auto other_mock_table = std::make_shared<Table>(
            column_definitions, TableType::Data, ChunkOffset{2});
        other_mock_table->append({pmr_string{"bar"}});
        right_input =
            std::make_shared<TableWrapper>(std::move(other_mock_table));
...
{
    auto change_meta_table = std::make_shared<ChangeMetaTable>(
        "meta_mock", MetaTableChangeType::Insert, left_input, right_input);
    ...
    EXPECT_EQ(meta_mock_table->insert_values(),
              right_input->get_output()->get_row(0));
}

TEST_F(ChangeMetaTableTest, Delete)
{
    auto change_meta_table = std::make_shared<ChangeMetaTable>(
        "meta_mock", MetaTableChangeType::Delete, left_input, right_input);
    ...
    EXPECT_EQ(meta_mock_table->remove_values(),
              left_input->get_output()->get_row(0));
}

TEST_F(ChangeMetaTableTest, Update)
{
    auto change_meta_table = std::make_shared<ChangeMetaTable>(
        "meta_mock", MetaTableChangeType::Update, left_input, right_input);
    ...
    EXPECT_EQ(meta_mock_table->update_selected_values(),
              left_input->get_output()->get_row(0));
    EXPECT_EQ(meta_mock_table->update_updated_values(),
              right_input->get_output()->get_row(0));
}
