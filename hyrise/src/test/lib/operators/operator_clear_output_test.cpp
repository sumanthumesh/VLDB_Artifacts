...
class OperatorClearOutputTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        _table = load_table("resources/test_data/tbl/int_int_int.tbl",
                            ChunkOffset{2});
        ...
        _ta_context = Hyrise::get().transaction_manager.new_transaction_context(
            AutoCommit::No);
    }
...
TEST_F(OperatorClearOutputTest, ConsumerTrackingTableScanUncorrelatedSubquery)
{
    ...
    // Subquery: (SELECT 9 + 2)
    auto dummy_table_wrapper =
        std::make_shared<TableWrapper>(Projection::dummy_table());
    auto projection_literals = std::make_shared<Projection>(
        dummy_table_wrapper, expression_vector(add_(value_(9), value_(2))));

    // b > (SELECT 9 + 2)
    auto validate = std::make_shared<Validate>(_gt);
    validate->set_transaction_context(_ta_context);
    auto pqp_subquery_expression =
        pqp_subquery_(projection_literals, DataType::Int, false);
    auto table_scan = std::make_shared<TableScan>(
        validate,
        greater_than_(pqp_column_(ColumnID{1}, DataType::Int, false, "b"),
                      pqp_subquery_expression));

    // a IN (SELECT 9 + 2)
    const auto semi_join_predicate = OperatorJoinPredicate{
        {ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals};
    const auto semi_join = std::make_shared<JoinHash>(
        table_scan, projection_literals, JoinMode::Semi, semi_join_predicate);

    // SELECT a, b
    const auto projection =
        std::make_shared<Projection>(semi_join, expression_vector(_a, _b));
...
TEST_F(OperatorClearOutputTest,
       ConsumerTrackingTableScanUncorrelatedSubqueryNested)
{
    /**
     * Based on ConsumerTrackingTableScanUncorrelatedSubquery, but with
     * additional nesting of the subquery. Models a PQP for the following SQL
     * query: SELECT a, b FROM int_int_int WHERE a IN (SELECT 9 + 2) AND b IN
     * (9, 10, (SELECT 9 + 2))
     */
    ...
    auto pqp_subquery_expression =
        pqp_subquery_(projection_literals, DataType::Int, false);
    auto table_scan = std::make_shared<TableScan>(
        validate, in_(pqp_column_(ColumnID{1}, DataType::Int, false, "b"),
                      list_(9, 10, pqp_subquery_expression)));
...
TEST_F(OperatorClearOutputTest, ConsumerTrackingProjectionUncorrelatedSubquery)
{
    ...
    auto aggregate_count_star =
        std::make_shared<WindowFunctionExpression>(WindowFunction::Count, star);
    ...
    auto aggregate_max_a =
        std::make_shared<WindowFunctionExpression>(WindowFunction::Max, _a);
    auto aggregate_hash_max_a = std::make_shared<AggregateHash>(
        validate,
        std::vector<std::shared_ptr<WindowFunctionExpression>>{aggregate_max_a},
        std::vector<ColumnID>{});

    // Projection: COUNT(*), (SELECT MAX(a) FROM int_int_int)
    auto pqp_count_star =
        pqp_column_(ColumnID{0}, aggregate_count_star->data_type(), false,
                    aggregate_count_star->as_column_name());
    auto pqp_subquery_max_id =
        pqp_subquery_(aggregate_hash_max_a, DataType::Int, false);
    auto projection = std::make_shared<Projection>(
        aggregate_hash_count_star,
    ...
    EXPECT_EQ(aggregate_hash_count_star->state(),
              OperatorState::ExecutedAndCleared);
    EXPECT_EQ(aggregate_hash_max_a->state(), OperatorState::ExecutedAndCleared);
}

TEST_F(OperatorClearOutputTest,
       ConsumerTrackingProjectionUncorrelatedSubqueryNested)
{
    /**
     * Based on ConsumerTrackingProjectionUncorrelatedSubquery, but with
     * additional nesting of the subquery. Models a PQP for the following SQL
     * query: SELECT (COUNT(*) + (SELECT MAX(a) FROM int_int_int)) FROM
     * int_int_int
     */
