...
class TableScanBetweenTest : public TypedOperatorBaseTest
{
  protected:
    ...
    void SetUp() override
    {
        // For the test, we create a table with the data type that is to be
        // scanned as the first column and a control int in the second column:
        //
        // a<DataType>  b<int>
        // 10.25         0
        // 12.25         1
        // 14.25 / NULL  2       (each third row is nulled if the table is
        // marked as nullable and no ordering is expected) 16.25         3
        // ...
        // 30.25         10
        //
        // If the table is sorted, NULL values are added up front or to the
        // back.
        //
        // As the first column is TYPE CASTED, it contains 10 for an int column,
        // the string "10.25" for a string column etc. We chose .25 because that
        // can be exactly expressed in a float.
        ...
        auto column_definitions = TableColumnDefinitions{
            {"a", data_type, nullable}, {"b", DataType::Int, nullable}};

        const auto data_table = std::make_shared<Table>(
            column_definitions, TableType::Data, ChunkOffset{6});

        // `nullable=nullable` is a dirty hack to work around C++ defect 2313.
        resolve_data_type(
            data_type,
            [&, nullable = nullable, sort_mode = sort_mode](const auto type)
            {
                ...
                for (auto i = 0; i <= 10; ++i)
                {
                    ...
                    else
                    {
                        ...
                        {
                            data_table->append(
                                {pmr_string{std::to_string(double_value)},
                                 i + number_of_nulls});
                        }
                        else
                        {
                            data_table->append({static_cast<Type>(double_value),
                                                i + number_of_nulls});
                        }
                    }
                }
        ...
        if (sort_mode)
        {
            for (ChunkID chunk_id{0}; chunk_id < data_table->chunk_count();
                 ++chunk_id)
            {
                data_table->get_chunk(chunk_id)->set_individually_sorted_by(
                    SortColumnDefinition(ColumnID{0}, *sort_mode));
            }
        }
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < 2; ++chunk_id)
        {
            ChunkEncoder::encode_chunk(
                data_table->get_chunk(chunk_id), {data_type, DataType::Int},
    ...
    // This is a helper function, which runs a between table scan on the data
    // defined in SetUp above. It takes the boundaries and the expected result
    // index positions from the given tests parameter. To test all
    // functionality, it is also necessary to consider the left/right
    // inclusiveness. It cannot be done automatically, because the results
    // differ depending on the chosen inclusiveness. Standard SQL only supports
    // double-inclusiveness, therefore this is the default. a sample tests
    // structure looks like this: std::vector<std::tuple<AllTypeVariant,
    // AllTypeVariant, std::vector<int>>>{
    //    {12.25, 16.25, {1, 2, 3}},
    //    {12.0, 16.25, {1, 2, 3}},
    //    {12.0, 16.75, {1, 2, 3}},
    // }
    void
    _test_between_scan(std::vector<std::tuple<AllTypeVariant, AllTypeVariant,
                       ...
                       PredicateCondition predicate_condition)
    {
        ...
        resolve_data_type(
            data_type,
            [&, nullable = nullable](const auto data_type_t)
            {
                ...
                for (const auto &[left, right, expected_with_null] : tests)
                {
                    SCOPED_TRACE(
                        std::string("BETWEEN ") +
                        std::to_string(boost::get<double>(left)) +
                    ...
                    // Float-with-String comparison not supported. We have to
                    // manually convert all floats to Strings if we're scanning
                    // on a String column.
                    if constexpr (std::is_same_v<ColumnDataType, pmr_string>)
                    {
                        left_casted = pmr_string{
                            std::to_string(boost::get<double>(left))};
                        right_casted = pmr_string{
                            std::to_string(boost::get<double>(right))};
                    }
                    else
                    {
                        left_casted = static_cast<ColumnDataType>(
                            boost::get<double>(left));
                        right_casted = static_cast<ColumnDataType>(
                            boost::get<double>(right));
                    }

                    auto scan = create_between_table_scan(
                        _data_table_wrapper, ColumnID{0}, left_casted,
                        right_casted, predicate_condition);
                    scan->execute();
                         ...
                         ++chunk_id)
                    {
                        ...
                        for (auto offset = ChunkOffset{0};
                             offset < segment_b->size(); ++offset)
                        {
                            result_ints.emplace_back(
                                boost::get<int>((*segment_b)[offset]));
                        }
                    }
                    ...
                    if (descending)
                    {
                        // Since the data is stored in reverse order, we expect
                        // inverted indices (e.g. highest index instead of
                        // lowest)
                        // We need to substract number_of_nulls as well because
                        // the expected values need to be shifted towards the
                        // added nulls.

                        const int max_index = 10 + number_of_nulls;
                        std::transform(expected.begin(), expected.end(),
                                       expected.begin(),
                                       [max_index](int expected_index) -> int
                    ...
                    if (ascending)
                    {
                        // Since we prepended three Null values we need to
                        // correct our indices
                        std::transform(
                    ...
                    {
                        // Remove the positions that should not be included
                        // because they are meant to be NULL In this case,
...
{
    auto inclusive_tests = std::vector<
        std::tuple<AllTypeVariant, AllTypeVariant, std::vector<int>>>{
...
{
    auto left_exclusive_tests = std::vector<
        std::tuple<AllTypeVariant, AllTypeVariant, std::vector<int>>>{
...
INSTANTIATE_TEST_SUITE_P(TableScanBetweenTestInstances, TableScanBetweenTest,
                         testing::ValuesIn(create_test_params()),
