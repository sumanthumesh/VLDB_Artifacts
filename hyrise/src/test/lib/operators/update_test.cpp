...
class OperatorsUpdateTest : public BaseTest
{
  public:
    ...
    void SetUp() override
    {
        const auto table = load_table("resources/test_data/tbl/int_float2.tbl",
                                      ChunkOffset{2});
                ...
                const std::string &expected_result_path)
    {
        ...
        const auto where_scan =
            std::make_shared<TableScan>(get_table, where_predicate);
        where_scan->never_clear_output();
        const auto updated_values_projection =
            std::make_shared<Projection>(where_scan, update_expressions);
        ...
        const auto transaction_context =
            Hyrise::get().transaction_manager.new_transaction_context(
                AutoCommit::No);
        const auto update = std::make_shared<Update>(
            table_to_update_name, where_scan, updated_values_projection);
        ...
        const auto post_update_transaction_context =
            Hyrise::get().transaction_manager.new_transaction_context(
                AutoCommit::No);
        const auto post_update_get_table =
            std::make_shared<GetTable>(table_to_update_name);
        ...
        EXPECT_TABLE_EQ_UNORDERED(validate->get_output(),
                                  load_table(expected_result_path));
    }
...
{
    helper(greater_than_(column_a, 0), expression_vector(column_a, column_b),
           "resources/test_data/tbl/int_float2.tbl");
}
...
{
    helper(greater_than_(column_a, 1000),
           expression_vector(column_a,
                             cast_(add_(column_a, 100), DataType::Float)),
           "resources/test_data/tbl/int_float2_updated_1.tbl");
}

TEST_F(OperatorsUpdateTest, UpdateNone)
{
    helper(greater_than_(column_a, 100'000), expression_vector(1, 1.5f),
           "resources/test_data/tbl/int_float2.tbl");
}
