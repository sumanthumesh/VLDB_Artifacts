...
std::shared_ptr<std::vector<ChunkID>>
shared_chunk_id_vector(std::vector<ChunkID> &&chunk_vector)
{
...
class OperatorsIndexScanTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _chunk_ids_partly_compressed =
            std::vector<ChunkID>(int_int_5->chunk_count());
        std::iota(_chunk_ids_partly_compressed.begin(),
                  _chunk_ids_partly_compressed.end(), ChunkID{0});

        _column_id = ColumnID{0};

        int_int_7->create_partial_hash_index(_column_id, _chunk_ids);
        int_int_5->create_partial_hash_index(_column_id,
                                             _chunk_ids_partly_compressed);
        ...
        const auto partially_indexed_table = load_table(
            "resources/test_data/tbl/int_int_shuffled.tbl", ChunkOffset{7});
        ChunkEncoder::encode_all_chunks(partially_indexed_table);
        const auto second_chunk =
            partially_indexed_table->get_chunk(ChunkID{1});
        Hyrise::get().storage_manager.add_table("index_test_table",
                                                partially_indexed_table);
    }
...
TEST_F(OperatorsIndexScanTest, SingleColumnScanOnDataTable)
{
    ...
    auto tests =
        std::vector<std::pair<PredicateCondition, std::vector<AllTypeVariant>>>{
            {PredicateCondition::Equals, {104, 104}},
            {PredicateCondition::NotEquals,
             {100, 102, 106, 108, 110, 112, 100, 102, 106, 108, 110, 112}}};

    for (const auto &[predicate, test_rows] : tests)
    {
        auto scan = std::make_shared<IndexScan>(_int_int, _column_id, predicate,
                                                right_value);
        scan->included_chunk_ids =
            shared_chunk_id_vector({ChunkID{0}, ChunkID{1}});

        scan->execute();

        auto scan_small_chunk = std::make_shared<IndexScan>(
            _int_int_small_chunk, _column_id, predicate, right_value);
        scan_small_chunk->included_chunk_ids =
            shared_chunk_id_vector({ChunkID{0}, ChunkID{1}, ChunkID{2}});

        scan_small_chunk->execute();

        ASSERT_COLUMN_EQ(scan->get_output(), ColumnID{1u}, test_rows);
        ASSERT_COLUMN_EQ(scan_small_chunk->get_output(), ColumnID{1u},
                         test_rows);
    }
}
...
TEST_F(OperatorsIndexScanTest, ScanWithNoChunkIDsIncluded)
{
    ...
    auto scan = std::make_shared<IndexScan>(
        _int_int, _column_id, PredicateCondition::Equals, AllTypeVariant{17});
    scan->included_chunk_ids = std::make_shared<std::vector<ChunkID>>();
    EXPECT_THROW(scan->execute(), std::logic_error);
}

TEST_F(OperatorsIndexScanTest,
       SingleColumnScanValueGreaterThanMaxDictionaryValue)
{
    ...
    auto tests =
        std::vector<std::pair<PredicateCondition, std::vector<AllTypeVariant>>>{
            {PredicateCondition::Equals, {}},
            {PredicateCondition::NotEquals,
             {100, 102, 104, 106, 108, 110, 112, 100, 102, 104, 106, 108, 110,
              112}}};
...
{
    auto table =
        load_table("resources/test_data/tbl/int_string.tbl", ChunkOffset{1});
    ...
    const auto pruned_chunk_id_tests = std::vector<
        std::pair<std::vector<ChunkID>, std::vector<AllTypeVariant>>>{
        // Prunes chunks with values 6, 10, 12, and 14.
        {{ChunkID{2}, ChunkID{4}, ChunkID{6}, ChunkID{7}}, {2, 4, 8, 19}},
        // Prunes all chunks (also the ones that are included in the scan). This
        // can happen with dynamic pruning, where
        // chunks are pruned (and added to list of pruned chunks of the GetTable
        // operator) at runtime.
        {{ChunkID{0}, ChunkID{1}, ChunkID{2}, ChunkID{3}, ChunkID{4},
          ChunkID{5}, ChunkID{6}, ChunkID{7}, ChunkID{8}, ChunkID{9},
          ChunkID{10}, ChunkID{11}},
         {}}};

    for (const auto &[pruned_chunk_ids, result] : pruned_chunk_id_tests)
    {
        auto get_table = std::make_shared<GetTable>("table", pruned_chunk_ids,
                                                    std::vector<ColumnID>{});
        get_table->never_clear_output();
        get_table->execute();

        auto index_scan = std::make_shared<IndexScan>(
            get_table, ColumnID{0}, PredicateCondition::NotEquals,
            AllTypeVariant{-17});
        // We include chunks with values 2, 4, and 6.
        index_scan->included_chunk_ids = shared_chunk_id_vector(
            {ChunkID{0}, ChunkID{1}, ChunkID{2}, ChunkID{5}});
...
{
    const auto scan = std::make_shared<IndexScan>(
        _int_int, _column_id, PredicateCondition::GreaterThanEquals,
        AllTypeVariant{0});
...
TEST_F(OperatorsIndexScanTest, DeepCopyRetainsIncludedChunks)
{
    ...
    const auto new_index_scan =
        std::dynamic_pointer_cast<IndexScan>(index_scan->deep_copy());
    EXPECT_EQ(*index_scan->included_chunk_ids,
              *new_index_scan->included_chunk_ids);
