  ...
  protected:
    static void SetUpTestCase()
    {
        _table_wrapper_small = std::make_shared<TableWrapper>(
            load_table("resources/test_data/tbl/join_operators/anti_int4.tbl",
                       ChunkOffset{2}));
        _table_wrapper_small->execute();

        _table_tpch_orders = std::make_shared<TableWrapper>(
            load_table("resources/test_data/tbl/tpch/sf-0.001/orders.tbl",
                       ChunkOffset{10}));
        _table_tpch_orders->execute();

        _table_tpch_lineitems = std::make_shared<TableWrapper>(
            load_table("resources/test_data/tbl/tpch/sf-0.001/lineitem.tbl",
                       ChunkOffset{10}));
        _table_tpch_lineitems->execute();

        _table_with_nulls = std::make_shared<TableWrapper>(load_table(
        ...
        _table_tpch_orders_scanned =
            create_table_scan(_table_tpch_orders, ColumnID{0},
                              PredicateCondition::GreaterThan, 0);
        _table_tpch_orders_scanned->execute();
        _table_tpch_lineitems_scanned =
            create_table_scan(_table_tpch_lineitems, ColumnID{0},
                              PredicateCondition::GreaterThan, 0);
    ...
    inline static std::shared_ptr<TableWrapper> _table_wrapper_small,
        _table_tpch_orders, _table_tpch_lineitems, _table_with_nulls;
    inline static std::shared_ptr<TableScan> _table_tpch_orders_scanned,
        _table_tpch_lineitems_scanned;
};
...
// Once we bring in the PosList optimization flag REFERS_TO_SINGLE_CHUNK_ONLY,
// this test will ensure that the join does not unnecessarily add chunks (e.g.,
// discussed in #698).
TEST_F(OperatorsJoinHashTest, DISABLED_ChunkCount /* #698 */)
{
    auto join = std::make_shared<JoinHash>(
        _table_tpch_orders_scanned, _table_tpch_lineitems_scanned,
        JoinMode::Inner,
    ...
    // While radix clustering is well-suited for very large tables, it also
    // yields many output tables. This test checks whether we create more chunks
    // that existing in the input (which should not be the case).
    EXPECT_TRUE(
        join->get_output()->chunk_count() <=
...
TEST_F(OperatorsJoinHashTest, DescriptionAndName)
{
    ...
    const auto join_operator = std::make_shared<JoinHash>(
        dummy_input, dummy_input, JoinMode::Inner, primary_predicate,
        std::vector<OperatorJoinPredicate>{secondary_predicate});
    const auto join_operator_with_radix = std::make_shared<JoinHash>(
        dummy_input, dummy_input, JoinMode::Inner, primary_predicate,
        std::vector<OperatorJoinPredicate>{secondary_predicate}, 4);

    EXPECT_EQ(
        join_operator->description(DescriptionMode::SingleLine),
        "JoinHash (Inner) Column #0 = Column #0 AND Column #0 != Column #0");
    EXPECT_EQ(
        join_operator->description(DescriptionMode::MultiLine),
        "JoinHash (Inner)\nColumn #0 = Column #0\nAND Column #0 != Column #0");
    EXPECT_EQ(
        join_operator_with_radix->description(DescriptionMode::MultiLine),
        "JoinHash (Inner)\nColumn #0 = Column #0\nAND Column #0 != Column #0");
...
TEST_F(OperatorsJoinHashTest, DeepCopy)
{
    ...
    const auto join_operator_copy =
        std::dynamic_pointer_cast<JoinHash>(join_operator);
...
TEST_F(OperatorsJoinHashTest, RadixBitCalculation)
{
    ...
    EXPECT_GT(
        JoinHash::calculate_radix_bits(std::numeric_limits<size_t>::max(),
