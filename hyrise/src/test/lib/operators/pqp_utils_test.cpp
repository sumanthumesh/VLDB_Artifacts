...
TEST_F(PQPUtilsTest, VisitPQPStreamlinePQP)
{
    ...
    const auto expected_nodes = std::vector<std::shared_ptr<AbstractOperator>>{
        node_d, node_c, node_b, node_a};
...
TEST_F(PQPUtilsTest, VisitPQPNonConstOperators)
{
    ...
    const auto expected_nodes =
        std::vector<std::shared_ptr<AbstractOperator>>{node_b, node_a};
    auto actual_nodes = std::vector<std::shared_ptr<AbstractOperator>>{};
...
TEST_F(PQPUtilsTest, VisitPQPConstOperators)
{
    ...
    const auto expected_nodes =
        std::vector<std::shared_ptr<const AbstractOperator>>{node_c, node_b};
...
TEST_F(PQPUtilsTest, ResolveUncorrelatedSubqueryWrongOperatorState)
{
    ...
    EXPECT_THROW(resolve_uncorrelated_subquery(table_wrapper),
                 std::logic_error);
...
{
    // Since this function is used by the TableScan to resolve uncorrelated
    // subqueries that can stem from a join rewrite, returning NULL_VALUE for
    // empty results leads to no matching tuples for the scan predicate, which
    // is the same as a join with an empty relation.
    const auto dummy_table =
        Table::create_dummy_table({{"a", DataType::Int, false}});
...
{
    const auto dummy_table =
        Table::create_dummy_table({{"a", DataType::Int, true}});
...
{
    const auto table_too_wide = Table::create_dummy_table(
        {{"a", DataType::Int, false}, {"b", DataType::Int, false}});
    table_too_wide->append({1, 2});
    const auto table_wrapper_too_wide =
        std::make_shared<TableWrapper>(table_too_wide);
    table_wrapper_too_wide->execute();

    EXPECT_THROW(resolve_uncorrelated_subquery(table_wrapper_too_wide),
                 std::logic_error);

    const auto table_too_long =
        Table::create_dummy_table({{"a", DataType::Int, false}});
    table_too_long->append({1});
    table_too_long->append({2});
    const auto table_wrapper_too_long =
        std::make_shared<TableWrapper>(table_too_long);
    table_wrapper_too_long->execute();

    EXPECT_THROW(resolve_uncorrelated_subquery(table_wrapper_too_long),
                 std::logic_error);
}
