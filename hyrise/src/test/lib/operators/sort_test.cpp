...
class SortTest : public BaseTestWithParam<SortTestParam>
{
  public:
    ...
    {
        input_table = load_table("resources/test_data/tbl/sort/input.tbl",
                                 ChunkOffset{20});
...
TEST_P(SortTest, Sort)
{
    ...
    if (param.input_is_empty)
    {
        ...
        else
        {
            ...
            auto empty_table =
                Table::create_dummy_table(input_table->column_definitions());
            input = std::make_shared<TableWrapper>(empty_table);
            input->execute();
        }
    }

    auto sort = Sort{input, param.sort_columns, param.output_chunk_size,
                     param.force_materialization};
    sort.execute();

    const auto expected_table = load_table(
        std::string{"resources/test_data/tbl/sort/"} + param.expected_filename);
    ...
    {
        for (auto chunk_id = ChunkID{0}; chunk_id < result->chunk_count() - 1;
             ++chunk_id)
        {
            EXPECT_EQ(result->get_chunk(chunk_id)->size(),
                      param.output_chunk_size);
        }
    }
}
...
TEST_P(SortTest, UnchangedNullability)
{
    ...
    auto segment_nullability_implies_column_nullablility =
                ...
                resolve_data_and_segment_type(
                    *abstract_segment,
                    [&](const auto data_type_t, const auto &segment)
                    {
                        ...
                        {
                            // if the segment is nullable, the column must be
                            // nullable as well
    ...
    auto sort = Sort{input_table_wrapper, param.sort_columns,
                     param.output_chunk_size, param.force_materialization};
    sort.execute();
...
inline std::string
sort_test_formatter(const testing::TestParamInfo<SortTestParam> &param_info)
{
...
{
    // Even though not all columns in a join result refer to the same table, the
    // output should use references
    const auto right_wrapper = std::make_shared<TableWrapper>(
        load_table("resources/test_data/tbl/int3.tbl"));
    right_wrapper->execute();

    const auto join_predicate = OperatorJoinPredicate{
        ColumnIDPair{ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals};
    auto join = std::make_shared<JoinHash>(input_table_wrapper, right_wrapper,
                                           JoinMode::Inner, join_predicate);
    join->execute();

    auto sort =
        Sort{join, {SortColumnDefinition{ColumnID{1}, SortMode::Descending}}};
    sort.execute();
...
{
    // When a single column in a table references different tables, we cannot
    // output sorted ReferenceSegments. This test simulates the output of a
    // union on the first column.

    const auto second_table =
        load_table("resources/test_data/tbl/sort/a_asc.tbl", ChunkOffset{10});
    const auto second_table_wrapper =
        std::make_shared<TableWrapper>(second_table);
    second_table_wrapper->execute();

    const auto union_table = std::make_shared<Table>(
        TableColumnDefinitions{TableColumnDefinition{"a", DataType::Int, true}},
    ...
    auto first_reference_segment =
        std::make_shared<ReferenceSegment>(input_table, ColumnID{0}, pos_list);
    union_table->append_chunk(Segments{first_reference_segment});

    auto second_reference_segment =
        std::make_shared<ReferenceSegment>(second_table, ColumnID{0}, pos_list);
    union_table->append_chunk(Segments{second_reference_segment});

    const auto union_table_wrapper =
        std::make_shared<TableWrapper>(union_table);
    union_table_wrapper->execute();

    auto sort = Sort{union_table_wrapper,
...
{
    // Similarly to InputReferencesDifferentTables, we cannot build a
    // ReferenceSegment that references different columns in the same table.

    // This is not just a normal union_table but something weird that you
    // probably won't see in the wild
    ...
    auto second_reference_segment =
        std::make_shared<ReferenceSegment>(input_table, ColumnID{1}, pos_list);
    weird_table->append_chunk(Segments{second_reference_segment});

    const auto weird_table_wrapper =
        std::make_shared<TableWrapper>(weird_table);
    weird_table_wrapper->execute();

    auto sort = Sort{weird_table_wrapper,
                     {SortColumnDefinition{ColumnID{0}, SortMode::Descending}}};
    sort.execute();
