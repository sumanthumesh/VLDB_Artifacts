...
class OperatorsUnionAllTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _table_wrapper_c = std::make_shared<TableWrapper>(
...
TEST_F(OperatorsUnionAllTest, UnionOfValueTables)
{
    std::shared_ptr<Table> expected_result = load_table(
        "resources/test_data/tbl/int_float_union.tbl", ChunkOffset{2});

    auto union_all =
        std::make_shared<UnionAll>(_table_wrapper_a, _table_wrapper_b);
    union_all->execute();
...
TEST_F(OperatorsUnionAllTest, UnionOfValueReferenceTables)
{
    ...
    auto projection = std::make_shared<Projection>(
        _table_wrapper_a,
        std::vector<std::shared_ptr<AbstractExpression>>{{a, b}});
    projection->execute();
...
TEST_F(OperatorsUnionAllTest, ThrowWrongColumnNumberException)
{
    ...
    std::shared_ptr<Table> test_table_c =
...
TEST_F(OperatorsUnionAllTest, ThrowWrongColumnOrderException)
{
    ...
    std::shared_ptr<Table> test_table_d =
...
{
    auto union_all =
        std::make_shared<UnionAll>(_table_wrapper_a, _table_wrapper_c);
