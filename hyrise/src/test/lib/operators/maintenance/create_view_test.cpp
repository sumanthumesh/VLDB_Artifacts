...
TEST_F(CreateViewTest, OperatorName)
{
    ...
    const auto view = std::make_shared<LQPView>(
        view_lqp, std::unordered_map<ColumnID, std::string>{});

    const auto create_view =
        std::make_shared<CreateView>("view_name", view, false);
...
TEST_F(CreateViewTest, Execute)
{
    const auto view_lqp =
        MockNode::make(MockNode::ColumnDefinitions{{{DataType::Int, "x"}}});
    const auto view_in = std::make_shared<LQPView>(
        view_lqp, std::unordered_map<ColumnID, std::string>{});

    const auto create_view =
        std::make_shared<CreateView>("view_name", view_in, false);
    create_view->execute();
...
TEST_F(CreateViewTest, ExecuteWithIfNotExists)
{
    ...
    const auto create_view =
        std::make_shared<CreateView>("view_name", view_in, true);
    create_view->execute();
