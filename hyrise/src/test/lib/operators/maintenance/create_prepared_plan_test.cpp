...
class CreatePreparedPlanTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        prepared_plan = std::make_shared<PreparedPlan>(
            DummyTableNode::make(), std::vector<ParameterID>{});
        create_prepared_plan = std::make_shared<CreatePreparedPlan>(
            "prepared_plan_a", prepared_plan);
    }
...
{
    EXPECT_EQ(replace_addresses(create_prepared_plan->description(
...
{
    const auto copy = std::dynamic_pointer_cast<CreatePreparedPlan>(
        create_prepared_plan->deep_copy());
...
{
    EXPECT_FALSE(
        Hyrise::get().storage_manager.has_prepared_plan("prepared_plan_a"));
