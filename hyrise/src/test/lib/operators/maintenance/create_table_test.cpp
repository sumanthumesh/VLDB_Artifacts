...
class CreateTableTest : public BaseTest
{
  public:
    void SetUp() override
    {
        ...
        dummy_table_wrapper = std::make_shared<TableWrapper>(
            Table::create_dummy_table(column_definitions));
        dummy_table_wrapper->never_clear_output();
        dummy_table_wrapper->execute();

        create_table =
            std::make_shared<CreateTable>("t", false, dummy_table_wrapper);
    }
...
TEST_F(CreateTableTest, NameAndDescriptionWithConstraints)
{
    const auto &input_table =
        std::const_pointer_cast<Table>(dummy_table_wrapper->table);
    input_table->add_soft_key_constraint({{input_table->column_id_by_name("a")},
                                          KeyConstraintType::PRIMARY_KEY});
    input_table->add_soft_key_constraint(
        {{input_table->column_id_by_name("b")}, KeyConstraintType::UNIQUE});

    EXPECT_EQ(create_table->name(), "CreateTable");
    // Case (i): CreateTable operator can retrieve the columns' information from
    // the input operator's output table.
    EXPECT_EQ(create_table->description(DescriptionMode::SingleLine),
              "CreateTable 't' ('a' int NOT NULL, 'b' float NULL, "
              "PRIMARY_KEY(a), UNIQUE(b))");
    ...
    // Case (ii): CreateTable operator has to retrieve the columns' information
    // from the created and stored table.
    EXPECT_EQ(create_table->description(DescriptionMode::MultiLine),
              "CreateTable 't' ('a' int NOT NULL\n'b' float "
              "NULL\nPRIMARY_KEY(a)\nUNIQUE(b))");
}
...
{
    const auto input_table =
        const_pointer_cast<Table>(dummy_table_wrapper->table);
...
TEST_F(CreateTableTest, TableAlreadyExists)
{
    ...
    const auto create_different_table =
        std::make_shared<CreateTable>("t2", false, dummy_table_wrapper);
    const auto create_same_table =
        std::make_shared<CreateTable>("t", false, dummy_table_wrapper);

    const auto context_2 =
        Hyrise::get().transaction_manager.new_transaction_context(
            AutoCommit::No);
    const auto context_3 =
        Hyrise::get().transaction_manager.new_transaction_context(
            AutoCommit::No);
...
TEST_F(CreateTableTest, ExecuteWithIfNotExists)
{
    const auto context =
        Hyrise::get().transaction_manager.new_transaction_context(
            AutoCommit::No);
    const auto ct_if_not_exists_1 =
        std::make_shared<CreateTable>("t", true, dummy_table_wrapper);
...
TEST_F(CreateTableTest, CreateTableAsSelectWithProjection)
{
    ...
    const std::shared_ptr<AbstractExpression> expr =
        add_(PQPColumnExpression::from_table(*table, "a"),
             PQPColumnExpression::from_table(*table, "b"));
    const auto projection =
        std::make_shared<Projection>(validate, expression_vector(expr));
    projection->set_transaction_context(context);
    projection->execute();

    const auto create_table_as =
        std::make_shared<CreateTable>("test_2", false, projection);
    create_table_as->set_transaction_context(context);
    EXPECT_NO_THROW(create_table_as->execute());
...
TEST_F(CreateTableTest, CreateTableWithDifferentTransactionContexts)
{
    ...
    const auto create_table_as_1 =
        std::make_shared<CreateTable>("test_2", false, validate_1);
    create_table_as_1->set_transaction_context(context_2);
    EXPECT_NO_THROW(create_table_as_1->execute());

    // Create table 2 with first context, which should not see the rows of table
    // 1
    ...
    const auto create_table_as_2 =
        std::make_shared<CreateTable>("test_3", false, validate_2);
