...
class OperatorsImportTest : public BaseTest
{
  protected:
    ...
    const std::map<FileType, std::string> reference_filenames{
        {FileType::Binary, "bin/float"},
        {FileType::Tbl, "tbl/float"},
        {FileType::Csv, "csv/float"}};
...
class OperatorsImportMultiFileTypeTest
    : public OperatorsImportTest,
      public ::testing::WithParamInterface<FileType>
{
};

INSTANTIATE_TEST_SUITE_P(FileTypes, OperatorsImportMultiFileTypeTest,
                         ::testing::Values(FileType::Csv, FileType::Tbl,
                                           FileType::Binary),
                         enum_formatter<FileType>);
...
TEST_P(OperatorsImportMultiFileTypeTest, ImportWithFileType)
{
    ...
    const auto reference_filename =
        reference_filepath + reference_filenames.at(GetParam());
    auto importer = std::make_shared<Import>(reference_filename, "a",
                                             Chunk::DEFAULT_SIZE, GetParam());
    importer->execute();

    EXPECT_TABLE_EQ_ORDERED(Hyrise::get().storage_manager.get_table("a"),
                            expected_table);
}
...
TEST_P(OperatorsImportMultiFileTypeTest, ImportWithoutFileType)
{
    ...
    const auto reference_filename = reference_filepath +
                                    reference_filenames.at(GetParam()) +
                                    file_extensions.at(GetParam());
    auto importer = std::make_shared<Import>(reference_filename, "a");
    importer->execute();
...
TEST_P(OperatorsImportMultiFileTypeTest, HasCorrectMvccData)
{
    ...
    EXPECT_EQ(table->get_chunk(ChunkID{0})->mvcc_data()->max_begin_cid.load(),
              CommitID{0});
}
...
{
    EXPECT_THROW(std::make_shared<Import>("not_existing_file.mp3", "a"),
                 std::exception);
}
...
TEST_F(OperatorsImportTest, ReplaceExistingTable)
{
    ...
    auto importer =
        std::make_shared<Import>("resources/test_data/tbl/int.tbl", "a");
    importer->execute();
...
{
    auto importer = std::make_shared<Import>(
        "resources/test_data/csv/float_int_large.csv", "a", ChunkOffset{20});
    importer->execute();

    // check if chunk_size property is correct
    EXPECT_EQ(Hyrise::get().storage_manager.get_table("a")->target_chunk_size(),
              20U);

    // check if actual chunk_size is correct
    EXPECT_EQ(Hyrise::get()
                  .storage_manager.get_table("a")
                  ->get_chunk(ChunkID{0})
                  ->size(),
              20U);
    EXPECT_EQ(Hyrise::get()
...
{
    auto importer = std::make_shared<Import>(
        "resources/test_data/csv/float_int_large_chunksize_max.csv", "a",
        Chunk::DEFAULT_SIZE);
    importer->execute();
    ...
    EXPECT_EQ(Hyrise::get()
                  ...
                  ->size(),
              100U);
    EXPECT_EQ(Hyrise::get().storage_manager.get_table("a")->chunk_count(),
              ChunkID{1});
