...
TEST_F(OperatorsJoinVerificationTest, DescriptionAndName)
{
    ...
    const auto join_operator = std::make_shared<JoinVerification>(
    ...
    EXPECT_EQ(join_operator->description(DescriptionMode::SingleLine),
              "JoinVerification (Inner) Column #0 = Column #0 AND Column #0 != "
              "Column #0");
    ...
    dummy_input->execute();
    EXPECT_EQ(join_operator->description(DescriptionMode::SingleLine),
              "JoinVerification (Inner) a = a AND a != a");
    EXPECT_EQ(join_operator->description(DescriptionMode::MultiLine),
              "JoinVerification (Inner)\na = a\nAND a != a");
...
TEST_F(OperatorsJoinVerificationTest, DeepCopy)
{
    ...
    const auto join_operator_copy =
        std::dynamic_pointer_cast<JoinVerification>(join_operator);
