...
class OperatorsValidateTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        _test_table = load_table("resources/test_data/tbl/validate_input.tbl",
                                 ChunkOffset{2});
        set_all_records_visible(*_test_table);
        invalidate_record(*_test_table, RowID{ChunkID{1}, ChunkOffset{0}},
                          CommitID{2});

        const auto _test_table2 =
    ...
    static bool
    forward_is_entire_chunk_visible(std::shared_ptr<Validate> validate,
...
void OperatorsValidateTest::invalidate_record(Table &table, RowID row,
                                              CommitID end_cid)
{
...
TEST_F(OperatorsValidateTest, SimpleValidate)
{
    ...
    std::shared_ptr<Table> expected_result =
        load_table("resources/test_data/tbl/validate_output_validated.tbl",
                   ChunkOffset{2});
...
TEST_F(OperatorsValidateTest, ScanValidate)
{
    ...
    std::shared_ptr<Table> expected_result = load_table(
        "resources/test_data/tbl/validate_output_validated_scanned.tbl",
        ChunkOffset{2});

    auto a = PQPColumnExpression::from_table(*_test_table, "a");
    auto table_scan =
        std::make_shared<TableScan>(_table_wrapper, greater_than_equals_(a, 2));
...
TEST_F(OperatorsValidateTest, ValidateAfterDelete)
{
    ...
    auto table_scan =
        create_table_scan(_gt, ColumnID{0}, PredicateCondition::Equals, "13");
    table_scan->execute();
...
TEST_F(OperatorsValidateTest, ChunkEntirelyVisibleThrowsOnRefChunk)
{
    ...
    auto pos_list = std::make_shared<RowIDPosList>(
        std::initializer_list<RowID>({RowID{ChunkID{0}, ChunkOffset{0}}}));
    auto segments = Segments{
        std::make_shared<ReferenceSegment>(_test_table, ColumnID{0}, pos_list)};
    auto chunk = std::make_shared<Chunk>(segments);

    auto validate = std::make_shared<Validate>(nullptr);

    EXPECT_THROW(forward_is_entire_chunk_visible(validate, chunk, snapshot_cid),
                 std::logic_error);
}
...
TEST_F(OperatorsValidateTest, ChunkNotEntirelyVisibleWithoutMaxBeginCid)
{
    ...
    auto chunk = std::make_shared<Chunk>(
        Segments{vs_int}, std::make_shared<MvccData>(1, CommitID{0}));
    // We explicitly do not finalize the chunk so that max_begin_cid remains
    // emtpy

    auto validate = std::make_shared<Validate>(nullptr);

    EXPECT_FALSE(
        forward_is_entire_chunk_visible(validate, chunk, snapshot_cid));
}
...
TEST_F(OperatorsValidateTest, ChunkNotEntirelyVisibleWithLowerSnapshotCid)
{
    ...
    auto chunk = std::make_shared<Chunk>(
        Segments{vs_int}, std::make_shared<MvccData>(1, begin_cid));
    chunk->finalize();
...
{
    // If Validate has a reference table as input, it can usually optimize the
    // evaluation of the MVCC data. This optimization is possible if a PosList
    // of a reference segment references only one chunk. Here, the fallback
    // implementation for a PosList with multiple chunks is tested.
    ...
    for (auto column_id = ColumnID{0}; column_id < column_count; ++column_id)
    {
        segments.emplace_back(std::make_shared<ReferenceSegment>(
            _test_table, column_id, pos_list));
    }

    auto reference_table = std::make_shared<Table>(
        _test_table->column_definitions(), TableType::References);
...
{
    const auto context = std::make_shared<TransactionContext>(
        TransactionID{1}, CommitID{3}, AutoCommit::No);
    ...
    // Since Validate can not be executed after Sort, we need to load a sorted
    // table.
    const auto sorted_table =
        load_table("resources/test_data/tbl/int_sorted.tbl", ChunkOffset{2});
    const auto sort_column_definition =
    ...
    const auto sorted_table_wrapper =
        std::make_shared<TableWrapper>(sorted_table);
    sorted_table_wrapper->execute();

    const auto validate_sorted =
        std::make_shared<Validate>(sorted_table_wrapper);
