...
/*
  The purpose of this test case is to test the single steps of the Hash Join
  (e.g., build(), probe(), etc.).
*/
...
class JoinHashStepsTest : public BaseTest
{
  protected:
    static void SetUpTestCase()
    {
        ...
        _table_zero_one = std::make_shared<Table>(
            column_definitions, TableType::Data, _chunk_size_zero_one);
        for (auto i = size_t{0}; i < _table_size_zero_one; ++i)
        {
            _table_zero_one->append({static_cast<int>(i % 2)});
        }

        _table_int_with_nulls = std::make_shared<TableWrapper>(
        ...
        _table_with_nulls_and_zeros = std::make_shared<TableWrapper>(load_table(
            "resources/test_data/tbl/int_int4_with_null.tbl", ChunkOffset{10}));
        ...
        _table_with_nulls_and_zeros_scanned =
            create_table_scan(_table_with_nulls_and_zeros, ColumnID{0},
                              PredicateCondition::GreaterThan, 0);
    ...
    // Accumulates the RowIDs hidden behind the iterator element (hash map
    // stores PosLists, not RowIDs)
    template <typename Iter> size_t get_row_count(Iter begin, Iter end)
    {
        size_t row_count = 0;
    ...
    inline static std::shared_ptr<TableWrapper> _table_int_with_nulls,
        _table_with_nulls_and_zeros;
    inline static std::shared_ptr<TableScan>
        _table_with_nulls_and_zeros_scanned;
};
...
TEST_F(JoinHashStepsTest, SmallHashTableAllPositions)
{
    ...
    {
        table.emplace(int64_t{index},
                      RowID{ChunkID{100u + index}, ChunkOffset{200u + index}});
        table.emplace(int64_t{index}, RowID{ChunkID{100u + index},
                                            ChunkOffset{200u + index + 1}});
    }
    const auto expected_pos_list =
        RowIDPosList{RowID{ChunkID{105}, ChunkOffset{205}},
...
TEST_F(JoinHashStepsTest, LargeHashTableExistenceOnly)
{
    ...
    const auto expected_pos_list =
        RowIDPosList{RowID{ChunkID{150}, ChunkOffset{250}}};
    {
        EXPECT_TRUE(table.contains(5));
        EXPECT_FALSE(table.contains(1000));
    }
...
TEST_F(JoinHashStepsTest, MaterializeAndBuildWithKeepNulls)
{
    ...
    // We materialize the table twice, once with keeping NULL values and once
    // without
    auto materialized_with_nulls = materialize_input<int, int, true>(
        _table_with_nulls_and_zeros->get_output(), ColumnID{0}, histograms,
        radix_bit_count, bloom_filter_with_nulls);
    auto materialized_without_nulls = materialize_input<int, int, false>(
        _table_with_nulls_and_zeros->get_output(), ColumnID{0}, histograms,
        radix_bit_count, bloom_filter_without_nulls);

    // Partition count should be equal to chunk count
    EXPECT_EQ(materialized_with_nulls.size(),
              static_cast<size_t>(
                  _table_with_nulls_and_zeros->get_output()->chunk_count()));
    EXPECT_EQ(materialized_without_nulls.size(),
              static_cast<size_t>(
                  _table_with_nulls_and_zeros->get_output()->chunk_count()));

    // Sum of partition sizes should be equal to row count if NULL values are
    // contained and lower if they are not
    ...
    EXPECT_EQ(materialized_with_nulls_size,
              _table_with_nulls_and_zeros->get_output()->row_count());
    ...
    EXPECT_LT(materialized_without_nulls_size,
              _table_with_nulls_and_zeros->get_output()->row_count());
    ...
    // For materialized_with_nulls, NULLs should be set according to the data in
    // the segment
    for (ChunkID chunk_id = ChunkID{0};
         chunk_id < _table_with_nulls_and_zeros->get_output()->chunk_count();
         ++chunk_id)
    {
        const auto segment = _table_with_nulls_and_zeros->get_output()
                                 ->get_chunk(chunk_id)
                                 ->get_segment(ColumnID{0});

        resolve_data_and_segment_type(
            *segment,
            [&](auto type, auto &typed_segment)
            {
                using Type = typename decltype(type)::type;
                auto iterable =
                    create_iterable_from_segment<Type>(typed_segment);
                ...
                iterable.with_iterators(
                    [&](auto it, auto end)
                    {
                        for (; it != end; ++it)
                        {
                            const bool null_flag =
                                materialized_with_nulls[chunk_id]
                                    .null_values[counter++];
...
{
    {
        ...
        materialize_input<int, int, false>(
            _table_with_nulls_and_zeros->get_output(), ColumnID{0}, histograms,
            1, bloom_filter);

        // For all input values, their position in the bloom filter should be
        // correctly set to true
        for (auto value : std::vector<int>{0, 6, 7, 9, 13, 18})
        {
            EXPECT_TRUE(bloom_filter[value]);

            // Set to false for easier checking of the values that should be
            // false
...
{
    {
        ...
        auto container = materialize_input<int, int, false>(
            _table_with_nulls_and_zeros->get_output(), ColumnID{0}, histograms,
            1, output_bloom_filter, input_bloom_filter);
        ...
        for (const auto &partition : container)
        {
            for (const auto &element : partition.elements)
            {
                ...
                chunk_offsets.emplace_back(
                    static_cast<int>(element.row_id.chunk_offset));
            }
        }
...
{
    {
        ...
        // When using 1 bit for radix partitioning, we have two radix clusters
        // determined on the least significant bit. For the 0/1 table, we should
        // thus cluster the ones and the zeros.
        materialize_input<int, int, false>(_table_zero_one, ColumnID{0},
                                           histograms, 1, bloom_filter);
        size_t histogram_offset_sum = 0;
        EXPECT_EQ(histograms.size(),
                  this->_table_size_zero_one / this->_chunk_size_zero_one);
...

    {
        ...
        // When using 2 bits for radix partitioning, we have four radix clusters
        // determined on the two least significant bits. For the 0/1 table, we
        // expect two non-empty clusters (00/01) and two empty ones (10/11).
        // Since the radix clusters are determine by hashing the value, we do
        // not know in which cluster the values are going to be stored.
        size_t empty_cluster_count = 0;
        materialize_input<int, int, false>(_table_zero_one, ColumnID{0},
                                           histograms, 2, bloom_filter);
        for (const auto &radix_count_per_chunk : histograms)
        {
            for (auto count : radix_count_per_chunk)
            {
                // Again, due to the hashing, we do not know which cluster holds
                // the value But we know that two buckets have
                // _table_size_zero_one/2 items and two have none items.
                EXPECT_TRUE(count == this->_chunk_size_zero_one / 2 ||
                            count == 0);
                if (count == 0)
                {
                    ++empty_cluster_count;
        ...
        EXPECT_EQ(empty_cluster_count,
                  2 * this->_table_size_zero_one / this->_chunk_size_zero_one);
    }
}
...
TEST_F(JoinHashStepsTest, RadixClusteringOfNulls)
{
    ...
    const auto materialized_without_null_handling =
        materialize_input<int, int, true>(_table_int_with_nulls->get_output(),
                                          ColumnID{0}, histograms,
                                          radix_bit_count, bloom_filter);
...
TEST_F(JoinHashStepsTest, BuildRespectsBloomFilter)
{
    ...
    auto container = materialize_input<int, int, false>(
        _table_with_nulls_and_zeros->get_output(), ColumnID{0}, histograms, 1,
        output_bloom_filter);

    auto hash_tables = build<int, int>(
        container, JoinHashBuildMode::AllPositions, 0, input_bloom_filter);
...
TEST_F(JoinHashStepsTest, ThrowWhenNoNullValuesArePassed)
{
    ...
    const auto materialized_without_null_handling =
        materialize_input<int, int, false>(
            _table_with_nulls_and_zeros->get_output(), ColumnID{0}, histograms,
            radix_bit_count, bloom_filter);
    // We want to test a non-NULL-considering Radix Container, ensure we did it
    // correctly
    ...
    // Note, the extra parantheses are required for Gtest since otherwise the
    // preprocessor has problems resolving this code line (see
    // https://stackoverflow.com/a/35957776/1147726)
    EXPECT_THROW(
        (partition_by_radix<int, int, true>)(materialized_without_null_handling,
                                             histograms, radix_bit_count),
        std::logic_error);
}
