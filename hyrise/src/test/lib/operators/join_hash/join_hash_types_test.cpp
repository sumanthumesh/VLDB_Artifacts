...
/**
 * The purpose of these tests it to check the construction of hash maps which
 * are not mapping value to value but value to vector of RowIDs (i.e., PosList).
 * We have two tests with sparse data (no value occurs twice) and dense data
 * (all rows share the same value). All these tests are executed for the main
 * numeric types.
 */
template <typename T> class JoinHashTypesTest : public BaseTest
{
};
...
void test_hash_map(const std::vector<T> &values)
{
    ...
    for (ChunkOffset i = ChunkOffset{0}; i < values.size(); ++i)
    {
        ...
        partition.elements.emplace_back(
            PartitionedElement<T>{row_id, static_cast<T>(values.at(i))});
        partition.null_values.emplace_back(false);
    }

    // Build a BloomFilter that cannot be used to skip any entries by creating a
    // BloomFilter with every value being false and using bitwise negation (~x).
    auto bloom_filter = ~BloomFilter(BLOOM_FILTER_SIZE);

    auto hash_maps =
        build<T, HashType>(RadixContainer<T>{partition},
                           JoinHashBuildMode::AllPositions, 0, bloom_filter);
    ...
    for (const auto &element : partition.elements)
    {
        ...
        EXPECT_TRUE(std::find(result_list.begin(), result_list.end(),
                              probe_row_id) != result_list.end());
        ++offset;
    }
}

using JoinHashTypesTestDataTypes = ::testing::Types<int, float, double>;
TYPED_TEST_SUITE(JoinHashTypesTest,
                 JoinHashTypesTestDataTypes, ); // NOLINT(whitespace/parens)
