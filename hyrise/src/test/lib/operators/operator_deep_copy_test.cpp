...
// At the moment, all the deep_copy() methods just call the constructor again.
// At first sight, these tests do not seem to add too much value because. This
// might change in the future. Then, these tests will make much more sense.
...
class OperatorDeepCopyTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _table_wrapper_a = std::make_shared<TableWrapper>(load_table(
            "resources/test_data/tbl/int_float.tbl", ChunkOffset{2}));
        _table_wrapper_b = std::make_shared<TableWrapper>(load_table(
            "resources/test_data/tbl/int_float2.tbl", ChunkOffset{2}));
        _table_wrapper_c = std::make_shared<TableWrapper>(load_table(
            "resources/test_data/tbl/int_float3.tbl", ChunkOffset{2}));
        _table_wrapper_d = std::make_shared<TableWrapper>(
            load_table("resources/test_data/tbl/int_int3.tbl", ChunkOffset{2}));
    ...
    std::shared_ptr<TableWrapper> _table_wrapper_a, _table_wrapper_b,
        _table_wrapper_c, _table_wrapper_d;
};

template <typename T> class DeepCopyTestJoin : public OperatorDeepCopyTest
{
};
...
{
    std::shared_ptr<Table> expected_result = load_table(
        "resources/test_data/tbl/join_operators/int_left_join_equals.tbl",
        ChunkOffset{1});
    EXPECT_NE(expected_result, nullptr)
        << "Could not load expected result table";

    // build and execute join
    auto join = std::make_shared<TypeParam>(
        this->_table_wrapper_a, this->_table_wrapper_b, JoinMode::Left,
...
{
    std::shared_ptr<Table> expected_result = load_table(
        "resources/test_data/tbl/int_float_filtered2.tbl", ChunkOffset{2});

    // build and execute difference
    auto difference =
        std::make_shared<Difference>(_table_wrapper_a, _table_wrapper_c);
    difference->execute();
...
TEST_F(OperatorDeepCopyTest, DeepCopyPrint)
{
    ...
    auto print1 = std::make_shared<Print>(
        _table_wrapper_a, PrintFlags::IgnoreChunkBoundaries, ostream);
    print1->execute();
...
{
    std::shared_ptr<Table> expected_result = load_table(
        "resources/test_data/tbl/int_int3_limit_1.tbl", ChunkOffset{1});

    // build and execute limit
    auto limit =
        std::make_shared<Limit>(_table_wrapper_d, to_expression(int64_t{1}));
    limit->execute();
...
{
    std::shared_ptr<Table> expected_result = load_table(
        "resources/test_data/tbl/int_float_sorted.tbl", ChunkOffset{1});
...
TEST_F(OperatorDeepCopyTest, DeepCopyTableScan)
{
    ...
    auto scan = create_table_scan(_table_wrapper_a, ColumnID{0},
...
{
    auto scan_a =
        create_table_scan(_table_wrapper_a, ColumnID{0},
                          PredicateCondition::GreaterThanEquals, 1234);
    scan_a->execute();

    auto scan_b = create_table_scan(scan_a, ColumnID{1},
                                    PredicateCondition::LessThan, 1000);
    auto scan_c = create_table_scan(scan_a, ColumnID{1},
                                    PredicateCondition::GreaterThan, 2000);
    auto union_positions = std::make_shared<UnionPositions>(scan_b, scan_c);

    auto copied_pqp = union_positions->deep_copy();

    EXPECT_EQ(copied_pqp->left_input()->left_input(),
              copied_pqp->right_input()->left_input());
}

TEST_F(OperatorDeepCopyTest, Subquery)
{
    // Due to the nested structure of the subquery, it makes sense to keep this
    // more high level than the other tests in this suite. The test would be
    // very confusing and error-prone with explicit operators as above.
    const auto table =
        load_table("resources/test_data/tbl/int_int_int.tbl", ChunkOffset{2});
    Hyrise::get().storage_manager.add_table("table_3int", table);

    const std::string subquery_query =
        "SELECT * FROM table_3int WHERE a = (SELECT MAX(b) FROM table_3int)";
    ...
    auto sql_pipeline =
        SQLPipelineBuilder{subquery_query}.disable_mvcc().create_pipeline();
    const auto [pipeline_status, first_result] =
...
TEST_F(OperatorDeepCopyTest, DeduplicationAmongRootAndSubqueryPQPs)
{
     ...
     *  uncorrelated subqueries that share a part of the root PQP. Similar to
     * TPC-H Q11.
     */
    ...
    auto join = std::make_shared<JoinHash>(
        ...
        OperatorJoinPredicate{ColumnIDPair(ColumnID{0}, ColumnID{0}),
                              PredicateCondition::Equals});
    auto projection =
        std::make_shared<Projection>(join, expression_vector(_a_a));
    auto pqp_subquery_expression =
        std::make_shared<PQPSubqueryExpression>(projection);

    // Prepare Root PQP
    auto table_scan = std::make_shared<TableScan>(
        get_table_a, less_than_(_a_a, pqp_subquery_expression));
    ...
    EXPECT_EQ(copied_get_table_a->consumer_count(),
              get_table_a->consumer_count());
}
...
TEST_F(OperatorDeepCopyTest, DeduplicationAmongSubqueries)
{
    /**
     * In this test, we check whether deep copies preserve deduplication for
     * uncorrelated subqueries that share parts of their PQP among each other.
    ...
    auto count_star =
        count_(pqp_column_(INVALID_COLUMN_ID, DataType::Long, false, "*"));
    auto aggregates =
        std::vector<std::shared_ptr<WindowFunctionExpression>>{count_star};
    auto aggregate_hash = std::make_shared<AggregateHash>(
        get_table_a, aggregates, group_by_columns);
    auto when_subquery =
        std::make_shared<PQPSubqueryExpression>(aggregate_hash);

    auto table_scan1 = std::make_shared<TableScan>(
        get_table_b, between_inclusive_(_b_b, 1, 20));
    auto then_subquery = std::make_shared<PQPSubqueryExpression>(table_scan1);

    auto table_scan2 = std::make_shared<TableScan>(
        get_table_b, between_inclusive_(_b_b, 21, 40));
    auto otherwise_subquery =
        std::make_shared<PQPSubqueryExpression>(table_scan2);

    // Root PQP
    auto projection = std::make_shared<Projection>(
        get_table_b, expression_vector(_a_a, case_(when_subquery, then_subquery,
                                                   otherwise_subquery)));
    ...
    EXPECT_EQ(copied_get_table_b->consumer_count(),
              get_table_b->consumer_count());
}
