...
TEST_F(OperatorsJoinSortMergeTest, DescriptionAndName)
{
    ...
    const auto join_operator = std::make_shared<JoinSortMerge>(
    ...
    EXPECT_EQ(join_operator->description(DescriptionMode::SingleLine),
              "JoinSortMerge (Inner) Column #0 = Column #0 AND Column #0 != "
              "Column #0");
    ...
    dummy_input->execute();
    EXPECT_EQ(join_operator->description(DescriptionMode::SingleLine),
              "JoinSortMerge (Inner) a = a AND a != a");
    EXPECT_EQ(join_operator->description(DescriptionMode::MultiLine),
              "JoinSortMerge (Inner)\na = a\nAND a != a");
...
TEST_F(OperatorsJoinSortMergeTest, DeepCopy)
{
    ...
    const auto join_operator_copy =
        std::dynamic_pointer_cast<JoinSortMerge>(join_operator);
...
TEST_F(OperatorsJoinSortMergeTest, ValueClusteringFlag)
{
    ...
    test_input->execute();
    const auto primary_predicate = OperatorJoinPredicate{
        {ColumnID{0}, ColumnID{1}}, PredicateCondition::Equals};

    // For inner joins, both join columns are clustered
    {
        const auto join_operator = std::make_shared<JoinSortMerge>(
            test_input, test_input, JoinMode::Inner, primary_predicate);
        join_operator->execute();

        const std::vector<ColumnID> expected_value_clustering{ColumnID{0},
                                                              ColumnID{4}};
        const auto &actual_value_clustering =
            join_operator->get_output()->value_clustered_by();
        EXPECT_EQ(actual_value_clustering, expected_value_clustering);
    }

    // For outer joins, the table cannot be clustered as table clustering is not
    // defined for NULL values
    {
        const auto join_operator = std::make_shared<JoinSortMerge>(
            test_input, test_input, JoinMode::Left, primary_predicate);
        join_operator->execute();
...
TEST_F(OperatorsJoinSortMergeTest, SetSortedFlagOnJoinColumns)
{
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < output_table->chunk_count();
         ++chunk_id)
    {
        const auto &actual_sorted_columns =
            output_table->get_chunk(chunk_id)->individually_sorted_by();
