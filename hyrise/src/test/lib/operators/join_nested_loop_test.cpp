...
TEST_F(OperatorsJoinNestedLoopTest, DescriptionAndName)
{
    ...
    const auto join_operator = std::make_shared<JoinNestedLoop>(
    ...
    EXPECT_EQ(join_operator->description(DescriptionMode::SingleLine),
              "JoinNestedLoop (Inner) Column #0 = Column #0 AND Column #0 != "
              "Column #0");
    ...
    dummy_input->execute();
    EXPECT_EQ(join_operator->description(DescriptionMode::SingleLine),
              "JoinNestedLoop (Inner) a = a AND a != a");
    EXPECT_EQ(join_operator->description(DescriptionMode::MultiLine),
              "JoinNestedLoop (Inner)\na = a\nAND a != a");
...
TEST_F(OperatorsJoinNestedLoopTest, DeepCopy)
{
    ...
    const auto join_operator_copy =
        std::dynamic_pointer_cast<JoinNestedLoop>(abstract_join_operator_copy);
