...
class OperatorsJoinIndexTest : public BaseTest
{
  public:
    ...
    { // called ONCE before the tests
        const auto dummy_table = std::make_shared<Table>(
        ...
        _table_wrapper_a = load_table_with_index(
            "resources/test_data/tbl/int_float.tbl", ChunkOffset{2});
        _table_wrapper_b = load_table_with_index(
            "resources/test_data/tbl/int_float2.tbl", ChunkOffset{2});
        _table_wrapper_c = load_table_with_index(
            "resources/test_data/tbl/int_string.tbl", ChunkOffset{4});
        _table_wrapper_d = load_table_with_index(
            "resources/test_data/tbl/string_int.tbl", ChunkOffset{3});
        _table_wrapper_e = load_table_with_index(
            "resources/test_data/tbl/int_int2.tbl", ChunkOffset{4});
        _table_wrapper_f = load_table_with_index(
            "resources/test_data/tbl/int_int3.tbl", ChunkOffset{4});
        _table_wrapper_g = load_table_with_index(
            "resources/test_data/tbl/int_int4.tbl", ChunkOffset{4});
        _table_wrapper_h = load_table_with_index(
            "resources/test_data/tbl/int_float_null_1.tbl", ChunkOffset{20});
        _table_wrapper_h_no_index = std::make_shared<TableWrapper>(load_table(
            "resources/test_data/tbl/int_float_null_1.tbl", ChunkOffset{20}));
        _table_wrapper_i = load_table_with_index(
            "resources/test_data/tbl/int_float_null_2.tbl", ChunkOffset{20});
        _table_wrapper_i_no_index = std::make_shared<TableWrapper>(load_table(
            "resources/test_data/tbl/int_float_null_2.tbl", ChunkOffset{20}));
  ...
  protected:
    static std::shared_ptr<TableWrapper>
    load_table_with_index(const std::string &filename,
                          const ChunkOffset chunk_size)
    {
        auto table = load_table(filename, chunk_size);

        ChunkEncoder::encode_all_chunks(
            table, SegmentEncodingSpec{EncodingType::Dictionary});
        ...
        for (ChunkID chunk_id{0}; chunk_id < table->chunk_count(); ++chunk_id)
        {
            ...
            for (auto column_id = ColumnID{0};
                 column_id < chunk->column_count(); ++column_id)
            {
                columns[0] = column_id;
    ...
    static void
    test_join_output(const std::shared_ptr<AbstractOperator> &left,
                     const std::shared_ptr<AbstractOperator> &right,
                     const OperatorJoinPredicate &primary_predicate,
                     const JoinMode mode, const size_t chunk_size,
                     const bool using_index = true,
                     ...
                     const bool single_chunk_reference_guarantee = true)
    {
        ...
        const auto join_verification = std::make_shared<JoinVerification>(
            left, right, mode, primary_predicate);
    ...
    inline static std::shared_ptr<TableWrapper> dummy_input, _table_wrapper_a,
        _table_wrapper_b, _table_wrapper_c, _table_wrapper_d, _table_wrapper_e,
        _table_wrapper_f, _table_wrapper_g, _table_wrapper_h, _table_wrapper_i,
...
TEST_F(OperatorsJoinIndexTest, Supports)
{
    ...
    auto join_operator = std::make_shared<JoinIndex>(
        dummy_input, dummy_input, JoinMode::Inner, primary_predicate,
...
TEST_F(OperatorsJoinIndexTest, DescriptionAndName)
{
    const auto primary_predicate = OperatorJoinPredicate{
        {ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals};
    const auto secondary_predicate = OperatorJoinPredicate{
        {ColumnID{0}, ColumnID{0}}, PredicateCondition::NotEquals};

    // INDEX SIDE LEFT
    const auto join_operator_index_left = std::make_shared<JoinIndex>(
        dummy_input, dummy_input, JoinMode::Inner, primary_predicate,
        std::vector<OperatorJoinPredicate>{secondary_predicate},
        IndexSide::Left);
    // INDEX SIDE RIGHT
    const auto join_operator_index_right = std::make_shared<JoinIndex>(
        dummy_input, dummy_input, JoinMode::Inner, primary_predicate,
        std::vector<OperatorJoinPredicate>{secondary_predicate},
        IndexSide::Right);

    EXPECT_EQ(
        join_operator_index_left->description(DescriptionMode::SingleLine),
        "JoinIndex (Inner) Column #0 = Column #0 AND Column #0 != Column #0 "
        "Index side: Left");
    ...
    EXPECT_EQ(
        join_operator_index_right->description(DescriptionMode::SingleLine),
        "JoinIndex (Inner) Column #0 = Column #0 AND Column #0 != Column #0 "
        "Index side: Right");
    EXPECT_EQ(
        join_operator_index_right->description(DescriptionMode::MultiLine),
        "JoinIndex (Inner)\nColumn #0 = Column #0\nAND Column #0 != Column "
        "#0\nIndex side: Right");
...
TEST_F(OperatorsJoinIndexTest, DeepCopy)
{
    const auto primary_predicate = OperatorJoinPredicate{
        {ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals};
    const auto secondary_predicates = std::vector<OperatorJoinPredicate>{
        {{ColumnID{1}, ColumnID{1}}, PredicateCondition::NotEquals}};
    const auto join_operator = std::make_shared<JoinIndex>(
        dummy_input, dummy_input, JoinMode::Left, primary_predicate,
        secondary_predicates, IndexSide::Left);
    const auto abstract_join_operator_copy = join_operator->deep_copy();
    const auto join_operator_copy =
        std::dynamic_pointer_cast<JoinIndex>(abstract_join_operator_copy);
...
TEST_F(OperatorsJoinIndexTest, PerformanceDataOutputToStream)
{
    ...
    performance_data.step_runtimes[static_cast<size_t>(
        JoinIndex::OperatorSteps::IndexJoining)] = std::chrono::nanoseconds{17};
...

    {
        ...
        EXPECT_EQ(stream.str(), "Output: 2 rows in 1 chunk, 999 ns. Operator "
                                "step runtimes: IndexJoining 17 ns, "
...
TEST_F(OperatorsJoinIndexTest, InnerRefJoinNoIndex)
{
    ...
    auto scan_a = create_table_scan(_table_wrapper_h_no_index, ColumnID{0},
                                    PredicateCondition::GreaterThanEquals, 0);
    scan_a->execute();
    auto scan_b = create_table_scan(_table_wrapper_i_no_index, ColumnID{0},
                                    PredicateCondition::GreaterThanEquals, 0);
    scan_b->execute();

    test_join_output(scan_a, scan_b,
                     {{ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals},
...
{
    // scan that returns all rows
    auto scan_a = create_table_scan(_table_wrapper_e, ColumnID{0},
                                    PredicateCondition::GreaterThanEquals, 0);
    scan_a->execute();
    auto scan_b = create_table_scan(_table_wrapper_f, ColumnID{0},
                                    PredicateCondition::GreaterThanEquals, 0);
    scan_b->execute();
    auto scan_c = create_table_scan(_table_wrapper_g, ColumnID{0},
                                    PredicateCondition::GreaterThanEquals, 0);
    scan_c->execute();

    auto join = std::make_shared<JoinIndex>(
        scan_a, scan_b, JoinMode::Inner,
        OperatorJoinPredicate{{ColumnID{0}, ColumnID{0}},
                              PredicateCondition::Equals});
    join->execute();

    // Referencing single chunk guarantee is not given since the left input of
    // the index join is also an index join and the IndexSide is left. The
    // execution of the index join does not provide single chunk reference
    // guarantee.
    test_join_output(join, scan_c,
                     {{ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals},
                     JoinMode::Inner, 1, true, IndexSide::Left, false);
}

TEST_F(OperatorsJoinIndexTest,
       RightJoinPruneInputIsRefIndexInputIsDataIndexSideIsRight)
{
    // scan that returns all rows
    auto scan_a = create_table_scan(_table_wrapper_a, ColumnID{0},
                                    PredicateCondition::GreaterThanEquals, 0);
    scan_a->execute();

    test_join_output(scan_a, _table_wrapper_b,
                     {{ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals},
