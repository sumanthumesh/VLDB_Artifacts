...
TEST_F(OperatorPerformanceDataTest, ElementsAreSet)
{
    ...
    auto table_scan = std::make_shared<TableScan>(
        table_wrapper,
        greater_than_(get_column_expression(table_wrapper, ColumnID{0}), 1));
    table_scan->execute();
...
// Check for correct counting of skipped chunks/segment (this is different to
// chunk pruning, which happens within the optimizer) due to early exists via
// the dictionary and counting of chunks/segments that are sorted and have thus
// been scanned with a sorted search (i.e., binary search).
TEST_F(OperatorPerformanceDataTest, TableScanPerformanceData)
{
    ...
    {
        auto table_scan =
            std::make_shared<TableScan>(table_wrapper, equals_(column_a, 2));
        table_scan->execute();

        auto &performance_data = dynamic_cast<TableScan::PerformanceData &>(
            *table_scan->performance_data);
        EXPECT_GT(performance_data.walltime.count(), 0ul);
...

    {
        const auto table_scan =
            std::make_shared<TableScan>(table_wrapper, equals_(column_a, 1));
        table_scan->execute();

        const auto &performance_data =
            dynamic_cast<TableScan::PerformanceData &>(
                *table_scan->performance_data);
        EXPECT_GT(performance_data.walltime.count(), 0ul);
    ...
    // Between scan
    {
        const auto table_scan = std::make_shared<TableScan>(
            table_wrapper, between_inclusive_(column_a, 3, 4));
        table_scan->execute();
    ...
    const TableColumnDefinitions nullable_column_definition = {
        {"a", DataType::Int, true}};
    table = std::make_shared<Table>(nullable_column_definition, TableType::Data,
                                    ChunkOffset{2});
    table->append({1});
    table->append({2});
    table->append({2});
    ...
    // ColumnVsValue scan
    {
        const auto table_scan = std::make_shared<TableScan>(
            table_wrapper,
            equals_(get_column_expression(table_wrapper, ColumnID{0}), 2));
        table_scan->execute();
    ...
    // Between scan on nullable columns
    {
        const auto table_scan = std::make_shared<TableScan>(
            table_wrapper,
            between_inclusive_(
                get_column_expression(table_wrapper, ColumnID{0}), 1, 4));
        table_scan->execute();
...
TEST_F(OperatorPerformanceDataTest, JoinHashStepRuntimes)
{
    ...
    const auto &perf =
        dynamic_cast<const OperatorPerformanceData<JoinHash::OperatorSteps> &>(
    ...
    for (const auto step : magic_enum::enum_values<JoinHash::OperatorSteps>())
    {
        ...
        {
            // Clustering step (i.e., radix partitioning) is not executed for
            // small joins.
...
{
    const auto table_a =
        load_table("resources/test_data/tbl/int_int2.tbl", ChunkOffset{2});
    const auto table_wrapper_a = std::make_shared<TableWrapper>(table_a);
    table_wrapper_a->never_clear_output();
    table_wrapper_a->execute();

    const auto table_b =
        load_table("resources/test_data/tbl/int_int_shuffled.tbl",
                   ChunkOffset{2}); // larger than int_int.tbl
    const auto table_wrapper_b = std::make_shared<TableWrapper>(table_b);
    table_wrapper_b->never_clear_output();
    table_wrapper_b->execute();

    // Check that std's hash for integer values is still the identity funtion.
    // This assumption might break in the future. In this case, some of the
    // following tests might break as well.
    ASSERT_EQ(std::hash<uint32_t>{}(17), 17);

    // Inner join case: We check that the number of stored positions (required
    // for all join types except semi/anti). Depending on the Bloom filter
    // implementation (which is currently depending on the implementation of
    // std::hash), we might store values and positions in the hash map that do
    // not have a matching partner.
    ...
    const auto &inner_perf = dynamic_cast<JoinHash::PerformanceData &>(
        *inner_join->performance_data);
    EXPECT_EQ(inner_perf.build_side_materialized_value_count,
              4ul); // matching values 2,6,2
    EXPECT_EQ(inner_perf.probe_side_materialized_value_count,
              4ul); // matching values 2,6,2
    ...
    // Semi join case: We check that no positions are stored (see explanation
    // for "AllPositions" mode in hash map). Further, we force the larger input
    // to be the build side. As we first materialize the smaller side (i.e., the
    // probe side in this case) and create the initial bloom filter with that,
    // there will be no reduction due to bloom filtering on the probe side.
    ...
    const auto &semi_perf =
        dynamic_cast<JoinHash::PerformanceData &>(*semi_join->performance_data);
    EXPECT_EQ(semi_perf.build_side_materialized_value_count, 4ul);
    EXPECT_EQ(semi_perf.probe_side_materialized_value_count,
              table_a->row_count());
...
// Check that steps of IndexJoin (indexed chunks/unindexed chunks) are executed
// as expected.
TEST_F(OperatorPerformanceDataTest, JoinIndexStepRuntimes)
{
    // This test modifies a table. Hence, we create a new one for this test
    // only.
    auto table =
        load_table("resources/test_data/tbl/int_int.tbl", ChunkOffset{2});
    auto table_wrapper = std::make_shared<TableWrapper>(table);
...

    {
        auto join = std::make_shared<JoinIndex>(
            ...
            OperatorJoinPredicate{ColumnIDPair(ColumnID{0}, ColumnID{0}),
                                  PredicateCondition::Equals});
        join->execute();
        auto &perf = dynamic_cast<const JoinIndex::PerformanceData &>(
            *join->performance_data);

        EXPECT_EQ(perf.get_step_runtime(JoinIndex::OperatorSteps::IndexJoining)
                      .count(),
                  0);
        EXPECT_GT(
            perf.get_step_runtime(JoinIndex::OperatorSteps::NestedLoopJoining)
                .count(),
            0);
        EXPECT_GT(perf.get_step_runtime(JoinIndex::OperatorSteps::OutputWriting)
                      .count(),
                  0);
...

    {
        ...
        EXPECT_GT(perf.get_step_runtime(JoinIndex::OperatorSteps::IndexJoining)
                      .count(),
                  0);
        EXPECT_EQ(
            perf.get_step_runtime(JoinIndex::OperatorSteps::NestedLoopJoining)
...
TEST_F(OperatorPerformanceDataTest, AggregateHashStepRuntimes)
{
    ...
    auto &performance_data = dynamic_cast<
        const OperatorPerformanceData<AggregateHash::OperatorSteps> &>(
        *aggregate->performance_data);

    for (const auto step :
         magic_enum::enum_values<AggregateHash::OperatorSteps>())
    {
...
TEST_F(OperatorPerformanceDataTest, OperatorPerformanceDataHasOutputMarkerSet)
{
    ...
    // also be known to the StorageManager
    ...
    auto table_scan_2 =
        create_table_scan(gt, ColumnID{0}, PredicateCondition::LessThan, "0");
    ...
    // Committing to avoid error "Has registered operators but has neither been
...
TEST_F(OperatorPerformanceDataTest, JoinHashPerformanceToOutputStream)
{
    ...
    performance_data.set_step_runtime(
        JoinHash::OperatorSteps::BuildSideMaterializing,
        std::chrono::nanoseconds{17});
    performance_data.set_step_runtime(JoinHash::OperatorSteps::Probing,
                                      std::chrono::nanoseconds{17});
    ...
    if constexpr (HYRISE_DEBUG)
    {
        ...
        // output_to_stream() throws when cumulative step runtimes are larger
        // than operator runtime.
        EXPECT_THROW(performance_data.output_to_stream(
                         stringstream_throw, DescriptionMode::SingleLine),
                     std::logic_error);
    }
    ...
    EXPECT_TRUE(stringstream.str().starts_with(
        "Output: 1 row in 1 chunk, 35 ns. Operator step runtimes: "
        "BuildSideMaterializing"
        " 17 ns, ProbeSideMaterializing 0 ns, Clustering 0 ns, Building 0 ns, "
        "Probing"
...
TEST_F(OperatorPerformanceDataTest, OutputToStream)
{
    auto performance_data =
        OperatorPerformanceData<AbstractOperatorPerformanceData::NoSteps>{};
    {
