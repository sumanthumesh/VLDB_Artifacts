...
auto table_scan_sorted_segment_search_test_formatter =
...
class OperatorsTableScanSortedSegmentSearchTest
    : public BaseTest,
...
TEST_P(OperatorsTableScanSortedSegmentSearchTest, ScanSortedSegment)
{
    ...
    iterable.with_iterators(
        [&](auto input_begin, auto input_end)
        {
            auto sorted_segment_search =
                _second_search_value
                    ? SortedSegmentSearch(input_begin, input_end, _sorted_by,
                                          _nullable, _predicate_condition,
                                          _search_value, *_second_search_value)
                    : SortedSegmentSearch(input_begin, input_end, _sorted_by,
                                          _nullable, _predicate_condition,
                                          _search_value);
            auto matches = RowIDPosList{};
            const auto initial_chunk_id = ChunkID{0};
            sorted_segment_search.scan_sorted_segment(initial_chunk_id, matches,
                                                      nullptr);
            ...
            for (auto index = size_t{0}; index < _expected.size(); ++index)
            {
                ...
                EXPECT_FALSE(_segment->is_null(chunk_offset))
                    << "row " << index << " is null";
                EXPECT_EQ(_segment->get(chunk_offset), _expected[index])
