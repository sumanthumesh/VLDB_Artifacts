...
{
    const auto predicate_a = OperatorJoinPredicate::from_expression(
        *equals_(a_a, b_b), *node_a, *node_b);
    ...
    const auto predicate_b = OperatorJoinPredicate::from_expression(
        *less_than_(b_a, a_b), *node_a, *node_b);
    ...
    EXPECT_EQ(predicate_b->predicate_condition,
...
{
    const auto predicate_a = OperatorJoinPredicate::from_expression(
        *equals_(a_a, a_b), *node_a, *node_b);
    ASSERT_FALSE(predicate_a);

    const auto predicate_b = OperatorJoinPredicate::from_expression(
        *less_than_(add_(b_a, 5), a_b), *node_a, *node_b);
    ASSERT_FALSE(predicate_b);
}

TEST_F(OperatorJoinPredicateTest, Flip)
{
    auto predicate = OperatorJoinPredicate{{ColumnID{0}, ColumnID{1}},
    ...
    EXPECT_EQ(predicate.predicate_condition,
