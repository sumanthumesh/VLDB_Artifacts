...
class OperatorsPrintTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _t = std::make_shared<Table>(column_definitions, TableType::Data,
                                     _chunk_size);
...
class PrintWrapper : public Print
{
  ...
  public:
    PrintWrapper(const std::shared_ptr<AbstractOperator> in,
                 const PrintFlags flags = PrintFlags::None,
    ...
    std::string test_truncate_cell(const AllTypeVariant &cell,
                                   uint16_t max_width)
    {
    ...
    bool is_printing_mvcc_information()
    {
        return static_cast<uint32_t>(_flags) &
               static_cast<uint32_t>(PrintFlags::Mvcc);
    }
};
...
TEST_F(OperatorsPrintTest, FilledTable)
{
    ...
    for (size_t i = 0; i < _chunk_size * chunk_count; i++)
    {
        ...
        tab->append(
            {static_cast<int>(i % _chunk_size),
             pmr_string(1, 97 + static_cast<int>(i / _chunk_size) % 26)});
    }
    ...
    size_t line_count =
        std::count(output_string.begin(), output_string.end(), '\n');
    size_t expected_line_count =
        4 + 12 * chunk_count; // 4 header lines + all 10-line chunks with chunk
                              // header
    EXPECT_EQ(line_count, expected_line_count);

    EXPECT_TRUE(output_string.find("Chunk 0") != std::string::npos);
    auto non_existing_chunk_header =
        std::string("Chunk ").append(std::to_string(chunk_count));
    EXPECT_TRUE(output_string.find(non_existing_chunk_header) ==
                std::string::npos);

    // remove spaces for some simple tests
    output_string.erase(
        remove_if(output_string.begin(), output_string.end(), isspace),
        output_string.end());
...
TEST_F(OperatorsPrintTest, GetColumnWidths)
{
...

    {
        ...
        // with empty columns and short column names, we should see the minimal
        // lengths
    ...
    tab->append(
        {ten_digits_ints, "quite a long string with more than $max chars"});
...
TEST_F(OperatorsPrintTest, TruncateLongValueInOutput)
{
    ...
    pmr_string cell_string =
        "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz";
    auto input = AllTypeVariant{cell_string};

    tab->append({0, input});

    auto substr_length = std::min(static_cast<int>(cell_string.length()),
                                  print_wrap->get_max_cell_width() - 3);
...
TEST_F(OperatorsPrintTest, MVCCTableLoad)
{
    ...
    // This test passes the flag for printing MVCC information, which is not
    // printed by default.
    std::shared_ptr<TableWrapper> table = std::make_shared<TableWrapper>(
...
TEST_F(OperatorsPrintTest, NullableColumnPrinting)
{
    ...
    nullable_column_definitions.emplace_back("l_returnflag", DataType::String,
                                             false);
    nullable_column_definitions.emplace_back("l_linestatus", DataType::String,
                                             false);
    nullable_column_definitions.emplace_back("sum_qty", DataType::Double, true);
    nullable_column_definitions.emplace_back("sum_base_price", DataType::Double,
                                             true);
    const auto tab = std::make_shared<Table>(nullable_column_definitions,
                                             TableType::Data, _chunk_size);
...
TEST_F(OperatorsPrintTest, SegmentType)
{
    ...
    ChunkEncoder::encode_chunks(table, {ChunkID{1}},
