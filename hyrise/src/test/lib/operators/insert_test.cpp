...
TEST_F(OperatorsInsertTest, SelfInsert)
{
    ...
    // Insert Operator works with the Storage Manager, so the test table must
    ...
    EXPECT_EQ((*table->get_chunk(ChunkID{0})
                    ->get_segment(ColumnID{1}))[ChunkOffset{0}],
              AllTypeVariant(12345));
    EXPECT_EQ((*table->get_chunk(ChunkID{0})
                    ->get_segment(ColumnID{0}))[ChunkOffset{0}],
              AllTypeVariant(458.7f));
    EXPECT_EQ((*table->get_chunk(ChunkID{1})
                    ->get_segment(ColumnID{1}))[ChunkOffset{0}],
              AllTypeVariant(12345));
    EXPECT_EQ((*table->get_chunk(ChunkID{1})
                    ->get_segment(ColumnID{0}))[ChunkOffset{0}],
    ...
    EXPECT_EQ(table->get_chunk(ChunkID{0})->get_segment(ColumnID{0})->size(),
              3u);
    EXPECT_EQ(table->get_chunk(ChunkID{0})->get_segment(ColumnID{1})->size(),
              3u);
    EXPECT_EQ(table->get_chunk(ChunkID{1})->get_segment(ColumnID{0})->size(),
              3u);
    EXPECT_EQ(table->get_chunk(ChunkID{1})->get_segment(ColumnID{1})->size(),
              3u);
}
...
TEST_F(OperatorsInsertTest, RollbackIncreaseInvalidRowCount)
{
    ...
    EXPECT_EQ(Hyrise::get().storage_manager.get_table(t_name)->row_count(),
              row_count * 2);
    EXPECT_EQ(Hyrise::get().storage_manager.get_table(t_name)->chunk_count(),
              2); // load_table() has finalized first chunk
    EXPECT_EQ(Hyrise::get()
                  .storage_manager.get_table(t_name)
                  ->get_chunk(ChunkID{0})
                  ->invalid_row_count(),
              uint32_t{0});
    EXPECT_EQ(Hyrise::get()
...
TEST_F(OperatorsInsertTest, InsertStringNullValue)
{
    ...
    auto table = load_table("resources/test_data/tbl/string_with_null.tbl",
                            ChunkOffset{4});
    Hyrise::get().storage_manager.add_table(table_name, table);

    auto table2 = load_table("resources/test_data/tbl/string_with_null.tbl",
                             ChunkOffset{4});
    Hyrise::get().storage_manager.add_table(table_name2, table2);
    ...
    auto null_val = (*(table->get_chunk(ChunkID{1})
                           ->get_segment(ColumnID{0})))[ChunkOffset{2}];
...
TEST_F(OperatorsInsertTest, InsertIntFloatNullValues)
{
    ...
    auto table = load_table("resources/test_data/tbl/int_float_with_null.tbl",
                            ChunkOffset{3});
    Hyrise::get().storage_manager.add_table(table_name, table);

    auto table2 = load_table("resources/test_data/tbl/int_float_with_null.tbl",
                             ChunkOffset{4});
    Hyrise::get().storage_manager.add_table(table_name2, table2);
    ...
    auto null_val_int = (*(table->get_chunk(ChunkID{2})
                               ->get_segment(ColumnID{0})))[ChunkOffset{2}];
    EXPECT_TRUE(variant_is_null(null_val_int));

    auto null_val_float = (*(table->get_chunk(ChunkID{2})
                                 ->get_segment(ColumnID{1})))[ChunkOffset{1}];
...
TEST_F(OperatorsInsertTest, InsertNullIntoNonNull)
{
    ...
    auto table =
        load_table("resources/test_data/tbl/int_float.tbl", ChunkOffset{3});
    Hyrise::get().storage_manager.add_table(table_name, table);
...
TEST_F(OperatorsInsertTest, InsertSingleNullFromDummyProjection)
{
    ...
    auto table = load_table("resources/test_data/tbl/float_with_null.tbl",
                            ChunkOffset{4});
    Hyrise::get().storage_manager.add_table(table_name, table);
    ...
    auto projection = std::make_shared<Projection>(
        dummy_wrapper, expression_vector(add_(0.0f, null_())));
    projection->execute();
...
TEST_F(OperatorsInsertTest, InsertIntoEmptyTable)
{
    ...
    const auto target_table = std::make_shared<Table>(
...
TEST_F(OperatorsInsertTest, SetMaxBeginCID)
{
    ...
    EXPECT_EQ(chunk->mvcc_data()->max_begin_cid.load(),
