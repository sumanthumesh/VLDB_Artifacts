  ...
  protected:
    void SetUp() override
    {
        Hyrise::get().storage_manager.add_table(
            "int_int_float",
            load_table("resources/test_data/tbl/int_int_float.tbl",
                       ChunkOffset{1}));
        Hyrise::get().storage_manager.add_table(
            "int_int_float_aliased",
            load_table("resources/test_data/tbl/int_int_float_aliased.tbl",
                       ChunkOffset{2}));

        const auto &table =
            Hyrise::get().storage_manager.get_table("int_int_float");
...
TEST_F(OperatorsGetTableTest, ThrowsUnknownTableName)
{
    ...
    EXPECT_THROW(get_table->execute(), std::exception)
        << "Should throw unknown table name exception";
}
...
TEST_F(OperatorsGetTableTest, Description)
{
    ...
    EXPECT_EQ(
        get_table_a->description(DescriptionMode::MultiLine),
        "GetTable\n(int_int_float)\npruned:\n0/4 chunk(s)\n0/3 column(s)");

    const auto get_table_b = std::make_shared<GetTable>(
        "int_int_float", std::vector{ChunkID{0}}, std::vector{ColumnID{1}});
    EXPECT_EQ(get_table_b->description(DescriptionMode::SingleLine),
              "GetTable (int_int_float) pruned: 1/4 chunk(s) (1 static, 0 "
              "dynamic), 1/3 column(s)");
    EXPECT_EQ(get_table_b->description(DescriptionMode::MultiLine),
              "GetTable\n(int_int_float)\npruned:\n1/4 chunk(s) (1 static, 0 "
              "dynamic)\n1/3 column(s)");
}
...
TEST_F(OperatorsGetTableTest, PassThroughInvalidRowCount)
{
    ...
    auto table_scan = create_table_scan(get_table_1, ColumnID{0},
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < result_table->chunk_count();
         ++chunk_id)
    {
        total_invalid_row_count +=
            result_table->get_chunk(chunk_id)->invalid_row_count();
    }
...
{
    auto get_table = std::make_shared<GetTable>(
        "int_int_float", std::vector{ChunkID{0}, ChunkID{2}},
        std::vector<ColumnID>{});
    ...
    EXPECT_EQ(table->get_value<int32_t>(ColumnID(0), 0),
              original_table->get_value<int32_t>(ColumnID(0), 1));
    EXPECT_EQ(table->get_value<int32_t>(ColumnID(0), 1),
              original_table->get_value<int32_t>(ColumnID(0), 3));
    const auto column_ids_0 = std::vector<ColumnID>{ColumnID{0}};
    const auto column_ids_1 = std::vector<ColumnID>{ColumnID{1}};
    EXPECT_EQ(table->get_chunk(ChunkID{0})->get_indexes(column_ids_0).size(),
              1);
    EXPECT_EQ(table->get_chunk(ChunkID{0})->get_indexes(column_ids_1).size(),
              2);
    EXPECT_EQ(table->get_chunk(ChunkID{1})->get_indexes(column_ids_0).size(),
              1);
    EXPECT_EQ(table->get_chunk(ChunkID{1})->get_indexes(column_ids_1).size(),
              2);
}
...
{
    auto get_table = std::make_shared<GetTable>(
        "int_int_float", std::vector<ChunkID>{}, std::vector{ColumnID{1}});
    ...
    EXPECT_EQ(table->get_chunk(ChunkID{1})->get_indexes(column_ids_1).size(),
              0);
    EXPECT_EQ(table->get_chunk(ChunkID{2})->get_indexes(column_ids_0).size(),
              1);
    EXPECT_EQ(table->get_chunk(ChunkID{2})->get_indexes(column_ids_1).size(),
              0);
    EXPECT_EQ(table->get_chunk(ChunkID{3})->get_indexes(column_ids_0).size(),
              1);
    EXPECT_EQ(table->get_chunk(ChunkID{3})->get_indexes(column_ids_1).size(),
              0);
}
...
TEST_F(OperatorsGetTableTest, ExcludeCleanedUpChunk)
{
    ...
    auto context = std::make_shared<TransactionContext>(
        TransactionID{1}, CommitID{3}, AutoCommit::No);
...
TEST_F(OperatorsGetTableTest, ExcludePhysicallyDeletedChunks)
{
    ...
    auto context = std::make_shared<TransactionContext>(
        TransactionID{1}, CommitID{1}, AutoCommit::No);
    auto get_table = std::make_shared<GetTable>("int_int_float");
    get_table->set_transaction_context(context);
    get_table->execute();
    ...
    // Delete all rows from table so calling original_table->remove_chunk()
    // below is legal
    ...
    auto context2 = std::make_shared<TransactionContext>(
        TransactionID{2}, CommitID{1}, AutoCommit::No);
    auto get_table_2 = std::make_shared<GetTable>("int_int_float");
...
TEST_F(OperatorsGetTableTest, PrunedChunksCombined)
{
    ...
    // Not setting cleanup commit ids is intentional, because we delete the
    // chunks manually for this test.
    ...
    auto get_table_2 = std::make_shared<GetTable>(
        "int_int_float", std::vector{ChunkID{0}}, std::vector<ColumnID>{});
...
TEST_F(OperatorsGetTableTest, Copy)
{
    ...
    const auto &get_table_a_copy =
        std::static_pointer_cast<GetTable>(get_table_a->deep_copy());
    ...
    const auto get_table_b = std::make_shared<GetTable>(
        "int_int_float", std::vector{ChunkID{1}}, std::vector{ColumnID{0}});
    const auto &get_table_b_copy =
        std::static_pointer_cast<GetTable>(get_table_b->deep_copy());
    ...
    const auto get_table_d =
        std::make_shared<GetTable>("int_int_float_aliased");
    const auto table_scan_a = std::make_shared<TableScan>(
        get_table_d,
        equals_(pqp_column_(ColumnID{1}, DataType::Int, false, "x"), 10));
    const auto projection = std::make_shared<Projection>(
        table_scan_a, expression_vector(pqp_column_(
                          ColumnID{0}, DataType::Float, false, "z")));

    const auto table_scan_b = std::make_shared<TableScan>(
        get_table_c,
        equals_(pqp_column_(ColumnID{2}, DataType::Float, false, "c"),
    ...
    const auto &get_table_c_copy =
        std::static_pointer_cast<const GetTable>(pqp_copy->left_input());
    const auto &prunable_subquery_scans =
        get_table_c_copy->prunable_subquery_predicates();
    ASSERT_EQ(prunable_subquery_scans.size(), 1);
    EXPECT_EQ(prunable_subquery_scans.front(), pqp_copy);

    // Do not allow deep copies where prunable subquery scans are not part of
    // the PQP.
...
TEST_F(OperatorsGetTableTest, AdaptOrderByInformation)
{
    ...
    // single column pruned
    {
        ...
        EXPECT_EQ(get_table_output->get_chunk(ChunkID{0})
                      ->individually_sorted_by()
                      .front()
                      .column,
                  ColumnID{0});
        EXPECT_EQ(get_table_output->get_chunk(ChunkID{1})
                      ->individually_sorted_by()
                      .front()
                      .column,
                  ColumnID{1});
        ...
        EXPECT_EQ(get_table_output->get_chunk(ChunkID{1})
                      ...
                      .sort_mode,
                  SortMode::Descending);
        EXPECT_TRUE(get_table_output->get_chunk(ChunkID{2})
                        ->individually_sorted_by()
                        .empty());
    }
    ...
    {
        auto get_table =
            std::make_shared<GetTable>("int_int_float", std::vector<ChunkID>{},
                                       std::vector{ColumnID{0}, ColumnID{1}});
        get_table->execute();

        const auto &get_table_output = get_table->get_output();
        EXPECT_EQ(get_table_output->column_count(), 1);
        EXPECT_TRUE(get_table_output->get_chunk(ChunkID{0})
                        ->individually_sorted_by()
                        .empty());
        EXPECT_EQ(get_table_output->get_chunk(ChunkID{1})
                      ->individually_sorted_by()
    ...
    {
        auto get_table = std::make_shared<GetTable>(
            "int_int_float", std::vector<ChunkID>{}, std::vector<ColumnID>{});
        get_table->execute();
...
{
    // Prune table with the predicates of linked TableScan using subquery
    // results.
    const auto get_table = std::make_shared<GetTable>(
        "int_int_float", std::vector{ChunkID{0}}, std::vector{ColumnID{1}});
    const auto dummy_table =
        Table::create_dummy_table({{"x", DataType::Int, false}});
    ...
    const auto table_scan = std::make_shared<TableScan>(
        get_table,
        not_equals_(pqp_column_(ColumnID{0}, DataType::Int, false, "a"),
                    pqp_subquery_(table_wrapper, DataType::Int, false)));
    const auto mock_node =
    ...
    const auto predicate_node = PredicateNode::make(not_equals_(
        stored_table_node->get_column("a"), lqp_subquery_(mock_node)));
    ...
    EXPECT_EQ(get_table->description(DescriptionMode::SingleLine),
              "GetTable (int_int_float) pruned: 2/4 chunk(s) (1 static, 1 "
              "dynamic), 1/3 column(s)");
    EXPECT_EQ(get_table->description(DescriptionMode::MultiLine),
              "GetTable\n(int_int_float)\npruned:\n2/4 chunk(s) (1 static, 1 "
              "dynamic)\n1/3 column(s)");
}

TEST_F(OperatorsGetTableTest, DynamicSubqueryPruningSubqueryNotExecuted)
{
    // Same as DynamicSubqueryPruning, but the subquery is not executed. Thus,
    // we fail.
    const auto get_table = std::make_shared<GetTable>(
        "int_int_float", std::vector{ChunkID{0}}, std::vector{ColumnID{1}});
    const auto dummy_table =
...
TEST_F(OperatorsGetTableTest, FinalizedChunks)
{
    ...
    // Test without and with pruned columns. In the first case, GetTable can
    // just forward the stored chunks. In the second case, it has to build the
    // chunks on its own.
    for (const auto &pruned_column_ids :
         {std::vector<ColumnID>{}, std::vector{ColumnID{1}, ColumnID{2}}})
    {
        const auto get_table = std::make_shared<GetTable>(
            "int_int_float", std::vector<ChunkID>{}, pruned_column_ids);
        get_table->execute();
        ...
        for (auto chunk_id = ChunkID{0}; chunk_id < immutable_chunk_count;
             ++chunk_id)
        {
