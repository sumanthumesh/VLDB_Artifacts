...
TEST_F(PluginManagerTest, LoadUnloadPlugin)
{
    ...
    EXPECT_EQ(plugins["hyriseTestPlugin"].plugin->description(),
              "This is the Hyrise TestPlugin");
    ...
    // The test plugin removes the dummy table from the storage manager when it
    // is unloaded
    EXPECT_FALSE(sm.has_table("DummyTable"));
    EXPECT_FALSE(plugins.contains("hyriseTestPlugin"));
}

// Plugins are unloaded when the PluginManager's destructor is called, this is
// simulated and tested here.
TEST_F(PluginManagerTest, LoadPluginAutomaticUnload)
{
    ...
    // The PluginManager's destructor calls _clean_up(), we call it here
    // explicitly to simulate the destructor being called, which in turn should
    // unload all loaded plugins.
    call_clean_up();

    // The test plugin removes the dummy table from the storage manager when it
    // is unloaded (implicitly by the destructor of the PluginManager).
    EXPECT_FALSE(sm.has_table("DummyTable"));
}
...
TEST_F(PluginManagerTest, LoadingUnloadingUserExecutableFunctions)
{
...

    {
        ...
        EXPECT_TRUE(user_executable_functions.contains(
            {"hyriseSecondTestPlugin", "OurFreelyChoosableFunctionName"}));
        EXPECT_TRUE(user_executable_functions.contains(
            {"hyriseTestPlugin", "OurFreelyChoosableFunctionName"}));
        EXPECT_TRUE(user_executable_functions.contains(
            {"hyriseTestPlugin", "SpecialFunction17"}));
    }
...
TEST_F(PluginManagerTest, CallUserExecutableFunctions)
{
    ...
    // The test plugin creates the below table when the called function is
    // executed
    EXPECT_TRUE(sm.has_table("TableOfTestPlugin_0"));

    // The PluginManager adds log messages when user executable functions are
    // called
    ASSERT_EQ(lm.log_entries().size(), 1);
    {
        ...
        EXPECT_EQ(
            entry.message,
            "Called user executable function 'OurFreelyChoosableFunctionName' "
            "provided by plugin 'hyriseTestPlugin'.");
        EXPECT_EQ(entry.log_level, LogLevel::Info);
    }

    pm.exec_user_function("hyriseSecondTestPlugin",
                          "OurFreelyChoosableFunctionName");
    // The second test plugin creates the below table when the called function
    // is executed
    ...
    ASSERT_EQ(lm.log_entries().size(), 2);
    {
        ...
        EXPECT_EQ(entry.message,
                  "Called user executable function "
                  "'OurFreelyChoosableFunctionName' provided by plugin "
...
TEST_F(PluginManagerTest, CallNotCallableUserExecutableFunctions)
{
    ...
    EXPECT_THROW(pm.exec_user_function("hyriseUnknownPlugin",
                                       "OurFreelyChoosableFunctionName"),
                 std::exception);
    ...
    pm.load_plugin(build_dylib_path("libhyriseSecondTestPlugin"));
    EXPECT_THROW(
        pm.exec_user_function("hyriseSecondTestPlugin", "SpecialFunction17"),
        std::exception);

    // Call function exposed by plugin but plugin has been unloaded before
    pm.unload_plugin("hyriseSecondTestPlugin");
    EXPECT_THROW(pm.exec_user_function("hyriseSecondTestPlugin",
                                       "OurFreelyChoosableFunctionName"),
                 std::exception);
}
...
TEST_F(PluginManagerTest, CallBenchmarkHooks)
{
    ...
    const std::unique_ptr<AbstractBenchmarkItemRunner> benchmark_item_runner =
        std::make_unique<TPCHBenchmarkItemRunner>(
            std::make_shared<BenchmarkConfig>(
                BenchmarkConfig::get_default_config()),
            false, 1, ClusteringConfiguration::None);

    pm.load_plugin(build_dylib_path("libhyriseTestPlugin"));

    pm.exec_pre_benchmark_hook("hyriseTestPlugin", *benchmark_item_runner);
    // The test plugin creates the below table when the hook is executed and
    // adds a row for each benchmark item.
    ...
    ASSERT_EQ(lm.log_entries().size(), 1);
    {
        ...
        EXPECT_EQ(
            entry.message,
            "Called pre-benchmark hook provided by plugin 'hyriseTestPlugin'.");
        EXPECT_EQ(entry.log_level, LogLevel::Info);
    }
    ...
    ASSERT_EQ(lm.log_entries().size(), 2);
    {
        const auto &entry = lm.log_entries()[1];
        EXPECT_EQ(entry.reporter, "PluginManager");
        EXPECT_EQ(entry.message, "Called post-benchmark hook provided by "
                                 "plugin 'hyriseTestPlugin'.");
        EXPECT_EQ(entry.log_level, LogLevel::Info);
    }
}
...
TEST_F(PluginManagerTest, CallNotExistingBenchmarkHooks)
{
    ...
    EXPECT_THROW(pm.exec_pre_benchmark_hook("hyriseUnknownPlugin",
                                            *benchmark_item_runner),
                 std::logic_error);
    EXPECT_THROW(pm.exec_post_benchmark_hook("hyriseUnknownPlugin", report),
                 std::logic_error);

    // Call existing, loaded plugin without hooks.
    pm.load_plugin(build_dylib_path("libhyriseSecondTestPlugin"));
    EXPECT_THROW(pm.exec_pre_benchmark_hook("libhyriseSecondTestPlugin",
                                            *benchmark_item_runner),
                 std::logic_error);
    EXPECT_THROW(
        pm.exec_post_benchmark_hook("libhyriseSecondTestPlugin", report),
        std::logic_error);
    pm.unload_plugin("hyriseSecondTestPlugin");
    ...
    pm.load_plugin(build_dylib_path("libhyriseTestPlugin"));
    pm.unload_plugin("hyriseTestPlugin");
    EXPECT_THROW(pm.exec_pre_benchmark_hook("libhyriseTestPlugin",
                                            *benchmark_item_runner),
                 std::logic_error);
    EXPECT_THROW(pm.exec_post_benchmark_hook("libhyriseTestPlugin", report),
                 std::logic_error);
}
...
TEST_F(PluginManagerTest, LoadingSameName)
{
    ...
    EXPECT_THROW(pm.load_plugin(build_dylib_path("libhyriseTestPlugin")),
                 std::exception);
}
...
TEST_F(PluginManagerTest, LoadingNotExistingLibrary)
{
    ...
    EXPECT_THROW(pm.load_plugin(build_dylib_path("libNotExisting")),
                 std::exception);
}
...
TEST_F(PluginManagerTest, LoadingNonInstantiableLibrary)
{
    ...
    EXPECT_THROW(
        pm.load_plugin(build_dylib_path("libhyriseTestNonInstantiablePlugin")),
        std::exception);
}
