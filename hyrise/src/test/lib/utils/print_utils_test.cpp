...
TEST_F(PrintUtilsTest, print_directed_acyclic_graph)
{
    ...
    const auto recurring_node =
        MockNode::make(column_definitions, "recurring_node");
    // clang-format off
  const auto lqp =
 ...
 \_[3] node_c
    ...
    print_directed_acyclic_graph<const AbstractLQPNode>(lqp, get_inputs_fn,
...
TEST_F(PrintUtilsTest, print_table_key_constraints)
{
    ...
    table->add_soft_key_constraint(
        {{ColumnID{1}, ColumnID{0}}, KeyConstraintType::UNIQUE});
    table->add_soft_key_constraint(
...
TEST_F(PrintUtilsTest, all_encoding_options)
{
    EXPECT_EQ(all_encoding_options(),
              "Unencoded, Dictionary, RunLength, FixedStringDictionary, "
              "FrameOfReference, LZ4");
}
