...
{
    const auto tbl_header_table =
        create_table_from_header("resources/test_data/tbl/float_int.tbl");
...
{
    EXPECT_THROW(load_table("resources/test_data/tbl/float_int_crlf.tbl",
                            ChunkOffset{2}),
                 std::exception);
}
