...
class ListDirectoryTest : public BaseTest
{
  protected:
    ...
    void TearDown() override { std::filesystem::remove_all(_directory_path); }
...
{
    const auto files = std::unordered_set<std::string>{
        _directory_path + "/one.test", _directory_path + "/two.test",
