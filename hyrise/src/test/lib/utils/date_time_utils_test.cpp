...
TEST_F(DateTimeUtilsTest, StringToDateTime)
{
    ...
    const auto timestamp_without_microseconds =
        string_to_timestamp("2000-01-31 12:13:14");
    ...
    EXPECT_EQ(
        timestamp_without_microseconds->time_of_day().fractional_seconds(), 0);

    const auto timestamp_with_microseconds =
        string_to_timestamp("2000-01-31 12:13:14.5");
    ...
    const auto timestamp_without_time =
        string_to_timestamp("2000-01-01 00:00:00");
    EXPECT_EQ(*string_to_timestamp("2000-01-01 00:"), *timestamp_without_time);
    EXPECT_EQ(*string_to_timestamp("2000-01-01 00:00"),
              *timestamp_without_time);
    EXPECT_EQ(*string_to_timestamp("2000-01-01 00:00:00.000000"),
              *timestamp_without_time);
    EXPECT_EQ(*string_to_timestamp("2000-01-01 0:0:0"),
              *timestamp_without_time);

    const auto timestamp_with_overflow =
        string_to_timestamp("2000-12-31 25:61:61");
...
TEST_F(DateTimeUtilsTest, DateInterval)
{
    ...
    EXPECT_THROW(date_interval(date, 0, DatetimeComponent::Second),
                 std::logic_error);
    EXPECT_THROW(date_interval(date, 0, DatetimeComponent::Minute),
                 std::logic_error);
    EXPECT_THROW(date_interval(date, 0, DatetimeComponent::Hour),
                 std::logic_error);
    EXPECT_EQ(date_interval(date, 1, DatetimeComponent::Day),
              (boost::gregorian::date{2000, 2, 1}));
    EXPECT_EQ(date_interval(date, 1, DatetimeComponent::Month), leap_year_date);
    EXPECT_EQ(date_interval(boost::gregorian::date{1999, 11, 30}, 2,
                            DatetimeComponent::Month),
              date);
    EXPECT_EQ(date_interval(date, 1, DatetimeComponent::Year),
              (boost::gregorian::date{2001, 1, 31}));
    EXPECT_EQ(date_interval(leap_year_date, 1, DatetimeComponent::Year),
              (boost::gregorian::date{2001, 2, 28}));
    EXPECT_EQ(date_interval(boost::gregorian::date{1999, 2, 28}, 1,
...
{
    const auto date = boost::gregorian::date{2000, 1, 31};
    const auto time_without_microseconds =
        boost::posix_time::time_duration{1, 1, 1, 0};
    const auto time_with_microseconds =
        boost::posix_time::time_duration{1, 1, 1, 500000};
    const auto timestamp_without_microseconds =
        boost::posix_time::ptime{date, time_without_microseconds};
    const auto timestamp_with_microseconds =
        boost::posix_time::ptime{date, time_with_microseconds};
    EXPECT_EQ(timestamp_to_string(timestamp_without_microseconds),
              "2000-01-31 01:01:01");
    EXPECT_EQ(timestamp_to_string(timestamp_with_microseconds),
              "2000-01-31 01:01:01.500000");
}
