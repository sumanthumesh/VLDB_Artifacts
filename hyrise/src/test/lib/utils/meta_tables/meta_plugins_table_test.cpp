  ...
  protected:
    ...
    void SetUp() override
    {
        ...
        const auto column_definitions =
            meta_plugins_table->column_definitions();
    ...
    void delete_from(const std::shared_ptr<AbstractMetaTable> &table,
                     const std::vector<AllTypeVariant> &values)
    {
        return table->_remove(values);
    }

    void insert_into(const std::shared_ptr<AbstractMetaTable> &table,
                     const std::vector<AllTypeVariant> &values)
    {
...
TEST_F(MetaPluginsTest, TableGeneration)
{
    Hyrise::get().plugin_manager.load_plugin(
        build_dylib_path("libhyriseTestPlugin"));
...
TEST_F(MetaPluginsTest, Insert)
{
    ...
    const auto meta_table = generate_meta_table(meta_plugins_table);
    EXPECT_TABLE_EQ_UNORDERED(meta_table, table_wrapper->get_output());
    EXPECT_EQ(Hyrise::get().plugin_manager.loaded_plugins().at(0),
              "hyriseTestPlugin");
}
...
{
    SQLPipelineBuilder{"INSERT INTO meta_plugins VALUES ('" +
                       build_dylib_path("libhyriseTestPlugin") + "')"}
        .create_pipeline()
        .get_result_table();
    ...
    SQLPipelineBuilder{
        "DELETE FROM meta_plugins WHERE name ='hyriseTestPlugin'"}
        .create_pipeline()
        .get_result_table();
