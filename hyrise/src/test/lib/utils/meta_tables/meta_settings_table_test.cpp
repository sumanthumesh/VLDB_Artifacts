  ...
  protected:
    ...
    void SetUp() override
    {
        ...
        const auto column_definitions =
            meta_settings_table->column_definitions();
        const auto table = std::make_shared<Table>(
            column_definitions, TableType::Data, ChunkOffset{2});
        table->append(
            {pmr_string{"mock_setting"}, pmr_string{"bar"}, pmr_string{"baz"}});
        auto table_wrapper = std::make_shared<TableWrapper>(std::move(table));
        table_wrapper->execute();
        mock_manipulation_values = table_wrapper->get_output();

        expected_table = std::make_shared<Table>(
            TableColumnDefinitions{{"name", DataType::String, false},
    ...
    void updateTable(const std::shared_ptr<AbstractMetaTable> &table,
                     const std::vector<AllTypeVariant> &selected_values,
...
{
    expected_table->append({pmr_string{"mock_setting"},
                            pmr_string{"mock_value"},
                            pmr_string{"mock_description"}});
    auto table_wrapper =
...
{
    updateTable(meta_settings_table, mock_manipulation_values->get_row(0),
                mock_manipulation_values->get_row(0));

    expected_table->append({pmr_string{"mock_setting"}, pmr_string{"bar"},
                            pmr_string{"mock_description"}});
    auto table_wrapper =
        std::make_shared<TableWrapper>(std::move(expected_table));
    table_wrapper->execute();
