...
TEST_F(MetaLogTest, TableGeneration)
{
    const auto column_definitions =
        TableColumnDefinitions{{"timestamp", DataType::Long, false},
    ...
    // Log entry should be created less than a minute ago but before the meta
    // table was generated.
    EXPECT_GT(boost::get<int64_t>(values[0]),
              timestamp_ns - static_cast<int64_t>(60e9));
    // Since the clock is not necessarily precise enough, we use "less or
    // equals" instead of "less than" here.
    EXPECT_LE(boost::get<int64_t>(values[0]), timestamp_ns);

    // We only check if the human-readable timestamp has the format YYYY-MM-DD
    // HH:MM:SS.
    const auto timestamp_regex =
        std::regex{"\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}"};
    auto match = std::smatch{};
    const auto timestamp_string =
        std::string{boost::get<pmr_string>(values[1])};
