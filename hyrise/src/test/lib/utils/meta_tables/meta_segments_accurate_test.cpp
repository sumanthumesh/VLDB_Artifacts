...
// General behavior of MetaSegmentsAccurateTable is tested in
// meta_table_test.cpp.
...
TEST_F(MetaSegmentsAccurateTest, UsesDistinctCountStatistics)
{
    resolve_data_type(
        ...
        [&](auto type)
        {
            ...
            attribute_statistics->set_statistics_object(
                std::make_shared<DistinctValueCount>(1234));
            auto mock_pruning_statistics = ChunkPruningStatistics{
                attribute_statistics, attribute_statistics};

            int_int->get_chunk(ChunkID{0})
                ->set_pruning_statistics(mock_pruning_statistics);
            int_int->get_chunk(ChunkID{1})
                ->set_pruning_statistics(mock_pruning_statistics);
        });

    const auto expected_table = Table::create_dummy_table(
    ...
    const auto &result_table =
        SQLPipelineBuilder{
            "SELECT distinct_value_count FROM meta_segments_accurate"}
            .create_pipeline()
            .get_result_table()
            .second;
...
TEST_F(MetaSegmentsAccurateTest, FallBackValueAccess)
{
    ...
    expected_table->append(
        {pmr_string{"int_int"}, pmr_string{"a"}, int32_t{0}, int64_t{2}});
    expected_table->append(
        {pmr_string{"int_int"}, pmr_string{"a"}, int32_t{1}, int64_t{1}});
    expected_table->append(
        {pmr_string{"int_int"}, pmr_string{"b"}, int32_t{0}, int64_t{2}});
    expected_table->append(
        {pmr_string{"int_int"}, pmr_string{"b"}, int32_t{1}, int64_t{1}});

    {
        ...
        const auto &result_table =
            SQLPipelineBuilder{
                "SELECT table_name, column_name, chunk_id, "
                "distinct_value_count FROM meta_segments_accurate"}
                .create_pipeline()
                .get_result_table()
                .second;
        EXPECT_TABLE_EQ_UNORDERED(result_table, expected_table);
...

    {
        ...
        int_int->get_chunk(ChunkID{0})
            ->set_pruning_statistics(ChunkPruningStatistics{2});
        int_int->get_chunk(ChunkID{1})
            ->set_pruning_statistics(ChunkPruningStatistics{2});
