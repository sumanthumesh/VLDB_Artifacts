...
class MetaTableTest : public BaseTest
{
  public:
    ...
    generate_meta_table(const MetaTable &table) const
    {
  ...
  protected:
    const std::string test_file_path{
        "resources/test_data/tbl/meta_tables/meta_"};
    ...
    void SetUp() override
    {
        ...
        int_int_int_null = load_table(
            "resources/test_data/tbl/int_int_int_null.tbl", ChunkOffset{100});

        ChunkEncoder::encode_chunk(
            int_int_int_null->get_chunk(ChunkID{0}),
            int_int_int_null->column_data_types(),
    ...
    void
    _insert_into_meta_table(const MetaTable &table,
    ...
    void
    _delete_from_meta_table(const MetaTable &table,
                            const std::vector<AllTypeVariant> &values) const
    {
        return table->_remove(values);
    }

    void _update_meta_table(const MetaTable &table,
                            const std::vector<AllTypeVariant> &old_values,
...
class MultiMetaTablesTest : public MetaTableTest,
                            public ::testing::WithParamInterface<MetaTable>
{
};

auto meta_table_test_formatter =
    [](const ::testing::TestParamInfo<MetaTable> info)
{
...
INSTANTIATE_TEST_SUITE_P(MetaTable, MultiMetaTablesTest,
                         ::testing::ValuesIn(MetaTableTest::meta_tables()),
...
TEST_P(MultiMetaTablesTest, IsImmutable)
{
    ...
    const auto mock_values =
        std::vector<AllTypeVariant>(GetParam()->column_definitions().size());
    EXPECT_THROW(_insert_into_meta_table(GetParam(), mock_values),
                 std::logic_error);
    EXPECT_THROW(_delete_from_meta_table(GetParam(), mock_values),
                 std::logic_error);
    EXPECT_THROW(_update_meta_table(GetParam(), mock_values, mock_values),
                 std::logic_error);
}

TEST_P(MultiMetaTablesTest, MetaTableGeneration)
{
    std::string suffix = GetParam()->name() == "segments" ||
                                 GetParam()->name() == "segments_accurate"
                             ? lib_suffix
                             : "";
    ...
    const auto expected_table =
        load_table(test_file_path + GetParam()->name() + suffix + ".tbl");

    // The values in the AccessCounters depend on how the segments are accessed
    // during the test. As such, the values in meta_segments*.tbl are fragile
...
TEST_P(MultiMetaTablesTest, IsDynamic)
{
    ...
    SQLPipelineBuilder{"UPDATE int_int SET a = a + 1000 WHERE a < 1000"}
        ...
        .get_result_table();
    SQLPipelineBuilder{
        "INSERT INTO int_int_int_null (a, b, c) VALUES (NULL, 1, 2)"}
        .create_pipeline()
        .get_result_table();
...

    {
        const auto expected_table = load_table(
            test_file_path + GetParam()->name() + suffix + "_updated.tbl");
        const auto meta_table = generate_meta_table(GetParam());
        EXPECT_TABLE_EQ_UNORDERED(meta_table, expected_table);
    }
}
...
{
    // Meta tables that access stored tables without going through GetTable need
    // to handle nullptr explicitly. We do not check the actual results in order
    // to avoid the number of test tables (that would have to be updated if the
    // memory consumption changes) low. Instead, we simply ensure that the meta
    // table is generated without dereferencing said nullptr.

    const auto int_int = Hyrise::get().storage_manager.get_table("int_int");

    SQLPipelineBuilder{"DELETE FROM int_int"}
        .create_pipeline()
        .get_result_table();
...
TEST_P(MultiMetaTablesTest, SQLFeatures)
{
    ...
    const auto result =
        SQLPipelineBuilder{"SELECT COUNT(*) FROM " +
                           MetaTableManager::META_PREFIX + GetParam()->name()}
            .create_pipeline()
            .get_result_table();
...
TEST_F(MetaTableTest, IsNotCached)
{
    ...
    SQLPipelineBuilder{
        "UPDATE meta_mock SET mock='bar' WHERE mock='mock_value'"}
        .create_pipeline()
        .get_result_table();
