...
TEST_F(MetaExecTest, SelectUserExecutableFunctions)
{
    ...
    const auto expected_table = std::make_shared<Table>(
        TableColumnDefinitions{{"plugin_name", DataType::String, false},
                               {"function_name", DataType::String, false}},
        TableType::Data, ChunkOffset{5});

    expected_table->append({pmr_string{"hyriseSecondTestPlugin"},
                            pmr_string{"OurFreelyChoosableFunctionName"}});
    expected_table->append({pmr_string{"hyriseTestPlugin"},
                            pmr_string{"OurFreelyChoosableFunctionName"}});
    expected_table->append(
        {pmr_string{"hyriseTestPlugin"}, pmr_string{"SpecialFunction17"}});

    auto table_wrapper =
        std::make_shared<TableWrapper>(std::move(expected_table));
    table_wrapper->execute();

    const auto &[_, result_table] =
        SQLPipelineBuilder{"SELECT * FROM meta_exec"}
            .create_pipeline()
            .get_result_table();
...
TEST_F(MetaExecTest, CallUserExecutableFunctions)
{
    ...
    const auto exec_query = "INSERT INTO meta_exec (plugin_name, "
                            "function_name) VALUES ('hyriseTestPlugin', "
...
TEST_F(MetaExecTest, CallNotCallableUserExecutableFunctions)
{
    ...
    // We have to manually rollback the transaction contexts below because
    // otherwise their destruction would cause an exeception to be thrown. This
    // is due to an assert in the TransactionContext's destructor checking for
    // failed operators. See ~TransactionContext for details.
    ...
    // Call non-existing plugin (with non-existing function)
    {
        auto transaction_context =
            Hyrise::get().transaction_manager.new_transaction_context(
                AutoCommit::No);
        auto sql_pipeline =
            SQLPipelineBuilder{"INSERT INTO meta_exec (plugin_name, "
                               "function_name) VALUES ('hyriseUnknownPlugin', "
                               "'OurFreelyChoosableFunctionName')"}
...

    {
        ...
        auto sql_pipeline =
            SQLPipelineBuilder{
                "INSERT INTO meta_exec (plugin_name, function_name) VALUES "
                "('hyriseSecondTestPlugin', 'SpecialFunction17')"}
                .with_transaction_context(transaction_context)
                .create_pipeline();
        EXPECT_THROW(sql_pipeline.get_result_table(), std::logic_error);
        sql_pipeline.transaction_context()->rollback(RollbackReason::Conflict);
