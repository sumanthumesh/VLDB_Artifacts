...
class SQLTranslatorTest : public BaseTest
{
  public:
    ...
    std::pair<std::shared_ptr<AbstractLQPNode>, SQLTranslationInfo>
    sql_to_lqp_helper(const std::string &query,
                      const UseMvcc use_mvcc = UseMvcc::No)
    {
        hsql::SQLParserResult parser_result;
        hsql::SQLParser::parseSQLString(query, &parser_result);
        Assert(parser_result.isValid(),
               create_sql_parser_error_message(query, parser_result));

        const auto translation_result =
            SQLTranslator{use_mvcc}.translate_parser_result(parser_result);
    ...
    static inline std::shared_ptr<Table> int_float, int_string, int_float2,
        int_float5, int_int_int;
    static inline std::shared_ptr<StoredTableNode> stored_table_node_int_float,
        stored_table_node_int_string, stored_table_node_int_float2,
        stored_table_node_int_float5, stored_table_node_int_int_int;
    std::shared_ptr<LQPColumnExpression> int_float_a, int_float_b, int_string_a,
        int_string_b, int_float5_a, int_float5_d, int_float2_a, int_float2_b,
        int_int_int_a, int_int_int_b, int_int_int_c;
};

TEST_F(SQLTranslatorTest, NoFromClause)
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT 1 + 2;");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE a = 'b'");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a FROM int_float;");

    const auto expected_lqp = ProjectionNode::make(
        expression_vector(int_float_a), stored_table_node_int_float);
...
TEST_F(SQLTranslatorTest, SelectStar)
{
    const auto [actual_lqp_no_table, translation_info_no_table] =
        sql_to_lqp_helper("SELECT * FROM int_float;");
    const auto [actual_lqp_table, translation_info] =
        sql_to_lqp_helper("SELECT int_float.* FROM int_float;");
...
TEST_F(SQLTranslatorTest, SimpleArithmeticExpression)
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT a * b FROM int_float;");
...
TEST_F(SQLTranslatorTest, NestedArithmeticExpression)
{
    ...
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a * b + a / b % 5 FROM int_float;");
...
TEST_F(SQLTranslatorTest, CaseExpressionSimple)
{
  ...
  const auto expression = case_(equals_(a_plus_b_times_3, 123), "Hello",
    ...
    const auto expected_lqp = ProjectionNode::make(
        expression_vector(expression), stored_table_node_int_float);
...
TEST_F(SQLTranslatorTest, SelectListAlias)
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT a AS column_a, b, b + a AS sum_column FROM int_float;");

    const auto aliases =
        std::vector<std::string>{{"column_a", "b", "sum_column"}};
    const auto expressions = expression_vector(int_float_a, int_float_b,
                                               add_(int_float_b, int_float_a));
...
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT a AS x FROM int_float WHERE a > 5");
    const auto [actual_lqp_b, translation_info_b] =
        sql_to_lqp_helper("SELECT a AS x FROM int_float WHERE x > 5");
...
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT a AS x FROM int_float GROUP BY a");
    const auto [actual_lqp_b, translation_info_b] =
        sql_to_lqp_helper("SELECT a AS x FROM int_float GROUP BY x");
    const auto [actual_lqp_c, translation_info_c] =
...
TEST_F(SQLTranslatorTest, SelectListAliasUsedInGroupByAndHaving)
{
    const auto [actual_lqp_a, translation_info_a] = sql_to_lqp_helper(
        "SELECT a AS x FROM int_float GROUP BY x HAVING a > 5");
    const auto [actual_lqp_b, translation_info_b] = sql_to_lqp_helper(
        "SELECT a AS x FROM int_float GROUP BY x HAVING x > 5");
...
TEST_F(SQLTranslatorTest, SelectListAliasUsedInOrderBy)
{
    ...
    const auto sort_modes =
        std::vector<SortMode>({SortMode::Ascending, SortMode::Ascending});
...
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT R.a, R.b FROM (SELECT a AS c, b AS d FROM "
                          "int_float) AS R JOIN int_float2 AS S ON R.b = S.b");
    const auto [actual_lqp_b, translation_info_b] =
        sql_to_lqp_helper("SELECT R.c, R.d FROM (SELECT a AS c, b AS d FROM "
                          "int_float) AS R JOIN int_float2 AS S ON R.d = S.b");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a AS a1, b AS b2, b AS b3, a AS a3, b AS b1, "
                          "a AS a2 FROM int_float");

    const auto aliases =
        std::vector<std::string>({"a1", "b2", "b3", "a3", "b1", "a2"});
    const auto expressions =
        expression_vector(int_float_a, int_float_b, int_float_b, int_float_a,
                          int_float_b, int_float_a);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT COUNT(*) AS cnt1, COUNT(*) AS cnt2, COUNT(*) "
                          "AS cnt3 FROM int_float");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT a1, b2, a3 FROM (SELECT a AS a1, b AS b2, b AS b3, a AS a3, b "
        "AS b1, a AS a2 FROM int_float) AS R");

    const auto outer_aliases = std::vector<std::string>({"a1", "b2", "a3"});
    const auto inner_aliases =
        std::vector<std::string>({"a1", "b2", "b3", "a3", "b1", "a2"});
    const auto outer_expressions =
        expression_vector(int_float_a, int_float_b, int_float_a);
...
TEST_F(SQLTranslatorTest,
       SelectListAliasesDifferentForSimilarAggregatesInSubquery)
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM (SELECT COUNT(*) AS cnt1, COUNT(*) AS "
                          "cnt2, COUNT(*) AS cnt3 FROM int_float) AS R");
...
{
    // See #1902. Optimally, these queries would produce correct results. Until
    // then, at least make sure they don't produce any wrong ones.
    ...
    EXPECT_THROW(sql_to_lqp_helper("SELECT COUNT(a) FROM (SELECT a, COUNT(a) "
                                   "AS b FROM t GROUP BY a) t2"),
                 InvalidInputException);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT COUNT(*) FROM int_float");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT COUNT(*), SUM(a + b) FROM int_float");
...
TEST_F(SQLTranslatorTest,
       SelectListAliasesDifferentForSimilarColumnsAndFromColumnAliasing)
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT x AS x1, x AS x2, x AS x3, y AS y1, y AS y2, "
                          "y AS y3 FROM int_float AS R (x, y)");

    const auto aliases =
        std::vector<std::string>({"x1", "x2", "x3", "y1", "y2", "y3"});
    const auto expressions =
        expression_vector(int_float_a, int_float_a, int_float_a, int_float_b,
                          int_float_b, int_float_b);
...
TEST_F(
    SQLTranslatorTest,
    SelectListAliasesDifferentForSimilarColumnsInSubqueryAndFromColumnAliasing)
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT u, z, w FROM (SELECT a AS a1, b AS b2, b AS "
                          "b3, a AS a3, b AS b1, a AS a2 FROM int_float)"
                          "AS R (y, x, v, w, u, z)");
    ...
    const auto inner_aliases =
        std::vector<std::string>({"a1", "b2", "b3", "a3", "b1", "a2"});
    const auto outer_expressions =
        expression_vector(int_float_b, int_float_a, int_float_a);
    const auto inner_expressions =
        expression_vector(int_float_a, int_float_b, int_float_b, int_float_a,
                          int_float_b, int_float_a);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT l.a1, l.a2, r.a1 FROM ("
    ...
    const auto expressions =
        expression_vector(int_float_a, int_float_a, int_float2_a);
...
TEST_F(SQLTranslatorTest,
       SelectListAliasesDifferentForSimilarColumnsUsedInCorrelatedSubquery)
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT b AS b1, b AS b2 FROM int_float WHERE a < (SELECT MAX(b) FROM "
        "int_float2 WHERE int_float2.b > b1)");
...
TEST_F(SQLTranslatorTest, SelectListAliasesUsedInView)
{
    const auto [result_node, translation_info] =
        sql_to_lqp_helper("CREATE VIEW alias_view AS SELECT a AS x, b as y "
                          "FROM int_float WHERE a > 10");
...
{
    const auto [result_node, translation_info] =
        sql_to_lqp_helper("CREATE VIEW alias_view AS SELECT a AS a1, a AS a2 "
                          "FROM int_float WHERE a > 10");
...
TEST_F(SQLTranslatorTest,
       SelectListManyAliasesDifferentForSimilarColumnsUsedInView)
{
    const auto [result_node, translation_info] =
        sql_to_lqp_helper("CREATE VIEW alias_view (a3, a4) AS SELECT a AS a1, "
                          "a AS a2 FROM int_float WHERE a > 10");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a FROM int_float WHERE a < 200;");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a FROM int_float WHERE a * b >= b + a;");
...
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT * FROM int_string WHERE b LIKE '%test1%';");
    const auto [actual_lqp_b, translation_info_b] = sql_to_lqp_helper(
        "SELECT * FROM int_string WHERE b NOT LIKE '%test1%';");
    const auto [actual_lqp_c, translation_info_c] = sql_to_lqp_helper(
        "SELECT * FROM int_string WHERE b NOT LIKE CONCAT('%test1', '%');");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT a FROM int_float WHERE 5 >= b + a OR (a > 2 AND b > 2);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a FROM int_float WHERE a BETWEEN b and 5;");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT b FROM int_float WHERE a + b IS NULL;");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT b FROM int_float WHERE a IS NOT NULL;");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE EXISTS(SELECT * FROM "
                          "int_float2 WHERE int_float.a = int_float2.a);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE NOT EXISTS(SELECT * "
                          "FROM int_float2 WHERE int_float.a = int_float2.a);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE a > (SELECT MIN(a + "
                          "int_float.b) FROM int_float2);");
...
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE NOT (a = b);");
    const auto [actual_lqp_b, translation_info_b] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE NOT (a != b);");
    const auto [actual_lqp_c, translation_info_c] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE NOT (a > b);");
    ...
    const auto [actual_lqp_e, translation_info_e] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE NOT (a >= b);");
    const auto [actual_lqp_f, translation_info_f] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE NOT (a <= b);");
    const auto [actual_lqp_g, translation_info_g] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE NOT (a IS NULL);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT SUM(a * 3) * b FROM int_float GROUP BY b");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT b, SUM(a) AS s FROM int_float GROUP BY b HAVING s > 1000");

    const auto select_list_expressions =
        expression_vector(int_float_b, sum_(int_float_a));
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT b, COUNT(a) FROM int_float GROUP BY b HAVING SUM(a) > 1000");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT DISTINCT b FROM int_float");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT DISTINCT * FROM int_float");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT DISTINCT b FROM int_float GROUP BY b");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT DISTINCT a, b FROM int_float ORDER BY b");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT DISTINCT a + b FROM int_float ORDER BY a + b");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT DISTINCT a + b, a + c FROM int_int_int ORDER BY a + b");
...
{
    // ORDER BY is executed after DISTINCT. For SELECT DISTINCT, expressions in
    // the ORDER BY clause must also be in the SELECT list.
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT DISTINCT a FROM int_float ORDER BY b"),
        InvalidInputException);
}
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT DISTINCT b, SUM(a * 3) * b FROM int_float GROUP BY b");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT DISTINCT MIN(a) FROM int_float GROUP BY b");
...
TEST_F(SQLTranslatorTest, AggregateCount)
{
  ...
  AggregateNode::make(expression_vector(int_float_b), expression_vector(count_star_(stored_table_node_int_float)),
    ...
    const auto [actual_lqp_count_distinct_a_plus_b, translation_info_3] =
        sql_to_lqp_helper(
            "SELECT a, b, COUNT(DISTINCT a + b) FROM int_float GROUP BY a, b");
    // clang-format off
  ...
  AggregateNode::make(expression_vector(int_float_a, int_float_b), expression_vector(count_distinct_(add_(int_float_a, int_float_b))),  // NOLINT(whitespace/line_length)
    ProjectionNode::make(expression_vector(add_(int_float_a, int_float_b), int_float_a, int_float_b),
      stored_table_node_int_float));
    // clang-format on
    EXPECT_LQP_EQ(actual_lqp_count_distinct_a_plus_b,
                  expected_lqp_count_distinct_a_plus_b);
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float GROUP BY b + 3, a / b, a, b");

    const auto b_plus_3 = add_(int_float_b, 3);
    const auto a_divided_by_b = div_(int_float_a, int_float_b);
    const auto group_by_expressions =
        expression_vector(b_plus_3, a_divided_by_b, int_float_b);
...
{
    // - y is an alias assigned in the SELECT list and can be used in the GROUP
    // BY list
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT int_float.*, b+3 AS y, SUM(a+b) FROM "
                          "int_float GROUP BY a, y, b");

    const auto sum_a_plus_b = sum_(add_(int_float_a, int_float_b));
    const auto b_plus_3 = add_(int_float_b, 3);

    const auto aliases =
        std::vector<std::string>({"a", "b", "y", "SUM(a + b)"});
    const auto select_list_expressions =
        expression_vector(int_float_a, int_float_b, b_plus_3,
                          sum_(add_(int_float_a, int_float_b)));
...
TEST_F(SQLTranslatorTest, AggregateAndGroupByWildcardTwoTables)
{
    ...
    // BY list
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT t1.*, t2.a, SUM(t2.b) FROM int_float t1, "
                          "int_float t2 GROUP BY t1.a, t1.b, t2.a");
...
{
    // Test that a referenced Aggregate does not result in redundant (and
    // illegal!) AggregateNodes

    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT x + 3 FROM (SELECT MIN(a) as x FROM int_float) AS t;");
...
TEST_F(SQLTranslatorTest, ProjectedAggregateForwarding)
{
    ...
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT x + 3 FROM (SELECT MIN(a) - 1 as x FROM int_float) AS t;");
...
{
    const auto [actual_lqp_a, translation_info_a] = sql_to_lqp_helper(
        "SELECT z.x, z.a, z.b FROM (SELECT a + b AS x, * FROM int_float) AS z");
    const auto [actual_lqp_b, translation_info_b] = sql_to_lqp_helper(
        "SELECT * FROM (SELECT a + b AS x, * FROM int_float) AS z");
    const auto [actual_lqp_c, translation_info_c] = sql_to_lqp_helper(
        "SELECT z.* FROM (SELECT a + b AS x, * FROM int_float) AS z");

    const auto expressions = expression_vector(add_(int_float_a, int_float_b),
                                               int_float_a, int_float_b);
...
TEST_F(SQLTranslatorTest, SubquerySelectList)
{
  ...
  AggregateNode::make(expression_vector(), expression_vector(min_(a_plus_d)),
    ...
    const auto subquery = lqp_subquery_(
        subquery_lqp, std::make_pair(ParameterID{0}, int_float5_d));
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float ORDER BY a, a+b DESC, b ASC");

    const auto sort_modes = std::vector<SortMode>(
        {SortMode::Ascending, SortMode::Descending, SortMode::Ascending});
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM int_float WHERE a + 7 IN (1+2,3,4)");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float WHERE a + 7 NOT IN (1+2,3,4)");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float WHERE a + 7 IN (SELECT * FROM int_float2)");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float WHERE a IN (SELECT * FROM int_float2 WHERE "
        "int_float.b * int_float.a * int_float.a > "
        "b)");

    const auto parameter_a = correlated_parameter_(ParameterID{1}, int_float_a);
    const auto parameter_b = correlated_parameter_(ParameterID{0}, int_float_b);

    const auto b_times_a_times_a =
        mul_(mul_(parameter_b, parameter_a), parameter_a);
  ...
  PredicateNode::make(greater_than_(b_times_a_times_a, int_float2_b),
    ...
    const auto subquery =
        lqp_subquery_(subquery_lqp, std::make_pair(ParameterID{1}, int_float_a),
...
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT * FROM int_float JOIN int_float2 ON "
                          "int_float2.a > int_float.a");
    const auto [actual_lqp_b, translation_info_b] =
        sql_to_lqp_helper("SELECT * FROM int_float LEFT JOIN int_float2 ON "
                          "int_float2.a > int_float.a");
    const auto [actual_lqp_c, translation_info_c] =
        sql_to_lqp_helper("SELECT * FROM int_float RIGHT JOIN int_float2 ON "
                          "int_float2.a > int_float.a");
    const auto [actual_lqp_d, translation_info_d] =
        sql_to_lqp_helper("SELECT * FROM int_float FULL OUTER JOIN int_float2 "
                          "ON int_float2.a > int_float.a");
    ...
    const auto expected_lqp_a =
        JoinNode::make(JoinMode::Inner, a_gt_a, node_a, node_b);
    const auto expected_lqp_b =
        JoinNode::make(JoinMode::Left, a_gt_a, node_a, node_b);
    const auto expected_lqp_c =
        JoinNode::make(JoinMode::Right, a_gt_a, node_a, node_b);
    const auto expected_lqp_d =
        JoinNode::make(JoinMode::FullOuter, a_gt_a, node_a, node_b);
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float, int_float2 AS t, int_float5 WHERE t.a < 2");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT int_float5.d, t.* FROM int_float, int_float2 "
                          "AS t, int_float5 WHERE t.a < 2");
...
TEST_F(SQLTranslatorTest, JoinLeftRightFullOuter)
{
    const auto [actual_lqp_left, translation_info_1] = sql_to_lqp_helper(
        "SELECT * FROM int_float AS a LEFT JOIN int_float2 AS b ON a.a = b.a;");
  ...
  JoinNode::make(JoinMode::Left, equals_(int_float_a, int_float2_a),
    ...
    const auto [actual_lqp_right, translation_info_2] =
        sql_to_lqp_helper("SELECT * FROM int_float AS a RIGHT JOIN int_float2 "
                          "AS b ON a.a = b.a;");
  ...
  JoinNode::make(JoinMode::Right, equals_(int_float_a, int_float2_a),
    ...
    const auto [actual_lqp_full, translation_info_3] = sql_to_lqp_helper(
        "SELECT * FROM int_float AS a FULL JOIN int_float2 AS b ON a.a = b.a;");
...
{
    // Test that predicates in the JOIN condition that reference only the
    // null-supplying side are pushed down
...
TEST_F(SQLTranslatorTest, JoinOuterPredicatesForNullPreservingSide)
{
    ...
    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM int_float AS a LEFT JOIN "
                                   "int_float2 AS b ON a.a > 5 AND a.a = b.a"),
                 InvalidInputException);

    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM int_float AS a RIGHT JOIN "
                                   "int_float2 AS b ON b.a > 5 AND a.a = b.a"),
                 InvalidInputException);

    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM int_float AS a FULL JOIN "
                                   "int_float2 AS b ON a.a > 5 AND a.a = b.a"),
                 InvalidInputException);
...
{
    // Test that the Natural join can work with column aliases and that the
    // output columns have the correct name

    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT "
                          "  * "
                          "FROM "
                          "  int_float AS a NATURAL JOIN (SELECT a AS d, b AS "
                          "a, c FROM int_int_int) AS b");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float JOIN int_float2 ON int_float.a + int_float2.a "
        "= int_float2.b * int_float.a;");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM int_float AS m1 JOIN int_float AS m2 "
                          "ON m1.a * 3 = m2.a - 5 OR m1.a > 20;");
...
{
    const auto [actual_lqp_a, translation_info_a] = sql_to_lqp_helper(
        "SELECT t.x FROM int_float AS t (x, y) WHERE x = t.y");
    const auto [actual_lqp_b, translation_info_b] = sql_to_lqp_helper(
        "SELECT t.x FROM (SELECT * FROM int_float) AS t (x, y) WHERE x = t.y");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT foo + 1 FROM (SELECT a, MIN(b) FROM "
                          "int_float WHERE a > 10 GROUP BY a) AS t (bar, foo)");
...
TEST_F(SQLTranslatorTest, FromColumnAliasingColumnsSwitchNames)
{
    ...
    const auto [actual_lqp_b, translation_info_b] = sql_to_lqp_helper(
        "SELECT * FROM (SELECT * FROM int_float) AS t (b, a) WHERE b = t.a");
...
{
    // Tricky: Tables "switch names". int_float becomes int_float2 and
    // int_float2 becomes int_float
    ...
    const auto [actual_lqp_b, translation_info_b] =
        sql_to_lqp_helper("SELECT int_float.y, int_float2.* "
                          "FROM (SELECT * FROM int_float) AS int_float2 (a, "
                          "b), (SELECT * FROM int_float2) AS int_float(x,y) "
                          "WHERE int_float.x = int_float2.b");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT R.a, S.a FROM int_float AS R, int_float AS S");
...
TEST_F(SQLTranslatorTest, LimitLiteral)
{
    ...
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM int_float LIMIT 1;");
    const auto expected_lqp =
        LimitNode::make(value_(1), stored_table_node_int_float);
    EXPECT_LQP_EQ(actual_lqp, expected_lqp);
}

TEST_F(SQLTranslatorTest, LimitExpression)
{
    // Uncommon: LIMIT to the result of an Expression (which has to be
    // uncorrelated)
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float LIMIT 3 + (SELECT MIN(b) FROM int_float2);");
...
TEST_F(SQLTranslatorTest, Extract)
{
    std::vector<DatetimeComponent> components{
        DatetimeComponent::Year,   DatetimeComponent::Month,
    ...
    for (const auto &component : components)
    {
        ...
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper(query_str.str());
        expected_lqp = ProjectionNode::make(
            expression_vector(extract_(component, "1993-08-01")),
            DummyTableNode::make());
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a + ?, ? FROM int_float WHERE a > ?");
...
TEST_F(SQLTranslatorTest, ParameterIDAllocationSimple)
{
    ...
    const auto query = "SELECT (SELECT (SELECT int_float2.a + int_float.b) "
                       "FROM int_float2) FROM int_float";
    ...
    const auto actual_lqp =
        sql_translator.translate_parser_result(parser_result).lqp_nodes.at(0);
...
{
    /**
     * Test that ParameterIDs are correctly allocated to ValuePlaceholders and
     * External Parameters
     */
    // clang-format off
  const auto query =
...
{
    const auto query =
        "SELECT * FROM int_float, int_float2 WHERE int_float.a = int_float2.b";
    ...
    const auto lqp_a = SQLTranslator{UseMvcc::No}
                           .translate_parser_result(parser_result)
                           .lqp_nodes.at(0);
...
{
    const auto [actual_lqp_a, translation_info_a] =
        sql_to_lqp_helper("SELECT SUBSTR('Hello', 3, 2 + 3)");
    const auto [actual_lqp_b, translation_info_b] =
        sql_to_lqp_helper("SELECT substr('Hello', 3, 2 + 3)");
    const auto [actual_lqp_c, translation_info] =
        sql_to_lqp_helper("SELECT substring('Hello', 3, 2 + 3)");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT EXISTS(SELECT * FROM int_float);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT NOT EXISTS(SELECT * FROM int_float);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT EXISTS(SELECT * FROM int_float WHERE "
                          "int_float.a > int_float2.b) FROM int_float2");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SHOW TABLES");

    const auto meta_table =
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SHOW COLUMNS int_float");

    const auto meta_table =
        Hyrise::get().meta_table_manager.generate_table("columns");
    const auto static_table_node = StaticTableNode::make(meta_table);
    const auto table_name_column = std::make_shared<LQPColumnExpression>(
        static_table_node, meta_table->column_id_by_name("table_name"));
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM " + MetaTableManager::META_PREFIX + "tables");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT table_name FROM (SELECT table_name, column_count FROM " +
    ...
    const auto table_name_column = std::make_shared<LQPColumnExpression>(
        static_table_node, meta_table->column_id_by_name("table_name"));
    const auto column_count_column = std::make_shared<LQPColumnExpression>(
        static_table_node, meta_table->column_id_by_name("column_count"));
  ...
  ProjectionNode::make(expression_vector(table_name_column, column_count_column),
    ...
    // clang-format on
    const auto expected_lqp = ProjectionNode::make(
        expression_vector(table_name_column), expected_subquery_lqp);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * from meta_tables, (SELECT 1) as subquery");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM int_float WHERE a in (SELECT column_count FROM " +
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "SELECT * FROM " + MetaTableManager::META_PREFIX +
        "tables WHERE column_count in (SELECT * FROM int_float2)");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT 1 WHERE EXISTS (SELECT * FROM " +
                          MetaTableManager::META_PREFIX + "tables)");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM  " + MetaTableManager::META_PREFIX +
                          "tables WHERE EXISTS (SELECT 1)");
...
TEST_F(SQLTranslatorTest, SelectMetaTableMultipleAccess)
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT * FROM " + MetaTableManager::META_PREFIX +
                          "tables AS a JOIN " + MetaTableManager::META_PREFIX +
                          "tables AS b ON a.table_name = b.table_name");
    ...
    const auto expected_lqp = JoinNode::make(
        JoinMode::Inner, a_equals_b, static_table_node, static_table_node);

    const auto table_1 =
        std::static_pointer_cast<StaticTableNode>(actual_lqp->left_input())
            ->table;
    const auto table_2 =
        std::static_pointer_cast<StaticTableNode>(actual_lqp->right_input())
            ->table;
...
TEST_F(SQLTranslatorTest, ExportMetaTable)
{
    ...
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("COPY " + meta_table_name + " TO 'foo.csv'");
...
{
    EXPECT_THROW(
        sql_to_lqp_helper("UPDATE meta_tables SET table_name = 'foo';"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("DELETE FROM meta_tables;"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("INSERT INTO meta_tables SELECT * FROM meta_tables;"),
        InvalidInputException);
}
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("INSERT INTO int_float VALUES (10, 12.5);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("INSERT INTO int_float (b, a) VALUES (12.5, 10);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("INSERT INTO int_float (b) VALUES (12.5);");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("INSERT INTO int_float (b, a) VALUES (12.5, NULL);");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "INSERT INTO int_float SELECT a, b FROM int_float2 WHERE a > 5;");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("INSERT INTO int_float VALUES (5.5, 12)");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("INSERT INTO meta_plugins VALUES ('foo');");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("DELETE FROM int_float", UseMvcc::Yes);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("DELETE FROM int_float WHERE a > 5", UseMvcc::Yes);
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "DELETE FROM meta_plugins WHERE name = 'foo'", UseMvcc::Yes);

    const auto meta_table =
        Hyrise::get().meta_table_manager.generate_table("plugins");
...
{
    EXPECT_THROW(sql_to_lqp_helper("UPDATE int_float SET b = 3.2 WHERE a > 1;"),
                 std::logic_error);
}
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("UPDATE int_float SET b = b + 1", UseMvcc::Yes);
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "UPDATE int_float SET b = 3.2 WHERE a > 1;", UseMvcc::Yes);
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "UPDATE int_float SET a = b, b = 3 WHERE a > 1;", UseMvcc::Yes);
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "UPDATE meta_settings SET value = 'foo' WHERE name = 'bar';",
        UseMvcc::Yes);

    const auto meta_table =
        Hyrise::get().meta_table_manager.generate_table("settings");
    const auto select_node = StaticTableNode::make(meta_table);
...
{
    const auto query = "CREATE VIEW my_first_view AS SELECT a, b, a + b, a*b "
                       "AS t FROM int_float WHERE a = 'b';";
  ...
  const auto view_columns = std::unordered_map<ColumnID, std::string>({
    ...
    const auto expected_lqp =
        CreateViewNode::make("my_first_view", view, false);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("CREATE VIEW my_second_view (c, d) AS SELECT * FROM "
                          "int_float WHERE a = 'b';");
  ...
  const auto view_lqp = PredicateNode::make(equals_(int_float_a, "b"), stored_table_node_int_float);
    ...
    const auto expected_lqp =
        CreateViewNode::make("my_second_view", view, false);
...
{
    const auto query = "CREATE VIEW IF NOT EXISTS my_first_view AS SELECT b, a "
                       "FROM int_float WHERE a = 'b';";
    const auto [result_node, translation_info] = sql_to_lqp_helper(query);

    const auto select_list_expressions =
        expression_vector(int_float_b, int_float_a);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("CREATE TABLE a_table ("
                          "  a_smallint SMALLINT,"
                          "  a_int INTEGER,"
                          "  a_long LONG,"
    ...
    const auto column_definitions =
        TableColumnDefinitions{{"a_smallint", DataType::Int, true},
                               {"a_int", DataType::Int, false},
                               {"a_long", DataType::Long, false},
                               {"a_bigint", DataType::Long, true},
                               {"a_decimal", DataType::Float, true},
                               {"a_real", DataType::Float, true},
                               {"a_float", DataType::Float, true},
                               {"a_double", DataType::Double, true},
                               {"a_varchar", DataType::String, false},
                               {"a_char_varying", DataType::String, true},
                               {"a_date", DataType::String, true},
                               {"a_time", DataType::String, true},
    ...
    static_table_node->table->add_soft_key_constraint(
        {{ColumnID{3}}, KeyConstraintType::UNIQUE});
    static_table_node->table->add_soft_key_constraint(
        {{ColumnID{1}, ColumnID{2}}, KeyConstraintType::PRIMARY_KEY});
    const auto expected_lqp =
        CreateTableNode::make("a_table", false, static_table_node);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("CREATE TABLE IF NOT EXISTS a_table ("
                          "  a_smallint SMALLINT,"
                          "  a_int INTEGER,"
                          "  a_long LONG,"
                          "  a_decimal DECIMAL(5,2),"
    ...
    const auto column_definitions =
        TableColumnDefinitions{{"a_smallint", DataType::Int, true},
                               {"a_int", DataType::Int, true},
                               {"a_long", DataType::Long, true},
                               {"a_decimal", DataType::Float, true},
                               {"a_real", DataType::Float, true},
                               {"a_float", DataType::Float, true},
                               {"a_double", DataType::Double, true},
    ...
    const auto static_table_node =
        StaticTableNode::make(Table::create_dummy_table(column_definitions));
    const auto expected_lqp =
        CreateTableNode::make("a_table", true, static_table_node);
...
TEST_F(SQLTranslatorTest, InvalidConstraints)
{
    ...
    EXPECT_THROW(
        sql_to_lqp_helper(
            "CREATE TABLE a_table (a_int INTEGER NULL, PRIMARY KEY(a_int))"),
        InvalidInputException);
    // Constraints must not contain unknown columns.
    EXPECT_THROW(sql_to_lqp_helper(
                     "CREATE TABLE a_table (a_int INTEGER, UNIQUE(b_int))"),
                 InvalidInputException);
}
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("CREATE TABLE a_table AS SELECT * FROM int_float");

    const auto stored_table_node = StoredTableNode::make("int_float");
    const auto expected_lqp =
        CreateTableNode::make("a_table", false, stored_table_node);
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("DROP TABLE a_table");
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("DROP TABLE IF EXISTS a_table");
...
{
    const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
        "PREPARE some_prepared_plan FROM 'SELECT a AS x FROM int_float'");
  ...
  AliasNode::make(expression_vector(int_float_a), std::vector<std::string>{"x"},
    ...
    const auto prepared_plan = std::make_shared<PreparedPlan>(
        statement_lqp, std::vector<ParameterID>{});

    const auto expected_lqp =
        CreatePreparedPlanNode::make("some_prepared_plan", prepared_plan);
...
TEST_F(SQLTranslatorTest, PrepareWithParameters)
{
  ...
  PredicateNode::make(greater_than_(int_float_a, placeholder_(ParameterID{0})),
    ...
    const auto prepared_plan = std::make_shared<PreparedPlan>(
        statement_lqp,
        std::vector<ParameterID>{ParameterID{0}, ParameterID{1}});
...
{
    // Correlated subqueries and prepared statement's parameters both use the
    // ParameterID system, so let's test that they cooperate

    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("PREPARE some_prepared_plan FROM 'SELECT * FROM "
                          "int_float WHERE a > ? AND"
                          " a < (SELECT MIN(a) FROM int_string WHERE "
                          "int_float.a = int_string.a) AND"
                          " b < ?'");
  ...
  PredicateNode::make(less_than_(int_float_a, subquery),
    ...
    const auto prepared_plan = std::make_shared<PreparedPlan>(
        statement_lqp,
        std::vector<ParameterID>{ParameterID{0}, ParameterID{2}});
...
TEST_F(SQLTranslatorTest, Execute)
{
  ...
  PredicateNode::make(greater_than_(int_string_a, placeholder_(ParameterID{1})),
    ...
    const auto prepared_plan = std::make_shared<PreparedPlan>(
        prepared_plan_lqp, std::vector<ParameterID>{
                               ParameterID{0}, ParameterID{1}, ParameterID{3}});

    Hyrise::get().storage_manager.add_prepared_plan("some_prepared_plan",
                                                    prepared_plan);

    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("EXECUTE some_prepared_plan ('Hello', 1, 42)");
...
TEST_F(SQLTranslatorTest, ExecuteWithoutParams)
{
  ...
  AggregateNode::make(expression_vector(), expression_vector(min_(int_float_a)),
    ...
    const auto prepared_plan = std::make_shared<PreparedPlan>(
        prepared_lqp, std::vector<ParameterID>{});

    Hyrise::get().storage_manager.add_prepared_plan("another_prepared_plan",
                                                    prepared_plan);

    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("EXECUTE another_prepared_plan ()");
...
{
    /**
     * Though the operator precedence is handled by the sql-parser, do some
     * checks here as well that it works as expected. SQLite is our reference:
     * https://www.sqlite.org/lang_expr.html For operators with the same
     * precedence, we evaluate left-to-right
     */
...
{
    EXPECT_THROW(sql_to_lqp_helper("SELECT no_such_table.* FROM int_float;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT no_such_function(5+3);"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT no_such_column FROM int_float;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM no_such_table;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT SUM(b) FROM int_string;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper(
                     "SELECT a FROM int_string GROUP BY a HAVING SUM(b) > 2;"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT b, SUM(b) AS s FROM table_a GROUP BY a;"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM int_float GROUP BY a;"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT t1.*, t2.*, SUM(t2.b) FROM int_float t1, "
                          "int_float t2 GROUP BY t1.a, t1.b, t2.a"),
        InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT * FROM table_a JOIN table_b ON a = b;"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM table_a JOIN table_b ON "
                                   "table_a.a = table_b.a AND a = 3;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM int_float WHERE 3 + 4;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT a AS b FROM int_float WHERE b > 5"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT a AS b FROM int_float GROUP BY int_float.b"),
        InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT a AS b, b AS a FROM int_float WHERE a > 5"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("INSERT INTO int_float VALUES (1, 2, 3, 4)"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper(
            "SELECT a, SUM(b) FROM int_float GROUP BY a HAVING b > 10;"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM int_float LIMIT 1 OFFSET 1;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("INSERT INTO no_such_table (a) VALUES (1);"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("DELETE FROM no_such_table"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("DELETE FROM no_such_table WHERE a = 1"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("UPDATE no_such_table SET a = 1"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("UPDATE no_such_table SET a = 1 WHERE a = 1"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("WITH q AS (SELECT * FROM int_float), q AS "
                                   "(SELECT b FROM q) SELECT * FROM q;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("COPY no_such_table TO 'a_file.tbl';"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT * FROM meta_unknown;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("UPDATE meta_unknown SET a = 1;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("DELETE FROM meta_unknown;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("INSERT INTO meta_unknown (a) VALUES(1);"),
                 InvalidInputException);
}
...
TEST_F(SQLTranslatorTest, WithClauseTableMasking)
{
    ...
    const auto [pre_condition_lqp_actual, translation_info_1] =
        sql_to_lqp_helper("SELECT * FROM int_float;");
    ...
    const auto [actual_lqp, translation_info_2] =
        sql_to_lqp_helper("WITH "
...
{
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT a FROM int_int_int "
                          "INTERSECT "
...
TEST_F(SQLTranslatorTest, CopyStatementImport)
{
    {
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("COPY a_table FROM 'a_file.tbl';");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Auto);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY a_table FROM 'a_file.tbl' WITH FORMAT TBL;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Tbl);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY a_table FROM 'a_file.tbl' WITH FORMAT CSV;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Csv);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY a_table FROM 'a_file.tbl' WITH FORMAT BINARY;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Binary);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY a_table FROM 'a_file.tbl' WITH FORMAT BIN;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Binary);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    ...
    EXPECT_THROW(
        sql_to_lqp_helper("COPY a_table FROM 'a_file.tbl' WHERE foo = 'bar';"),
        InvalidInputException);
}
...
TEST_F(SQLTranslatorTest, CopyStatementExport)
{
    {
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("COPY int_float TO 'a_file.tbl';");
        const auto expected_lqp = ExportNode::make("a_file.tbl", FileType::Auto,
                                                   stored_table_node_int_float);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("COPY int_float TO 'a_file.tbl';", UseMvcc::Yes);
        const auto expected_lqp =
    ...
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY int_float TO 'a_file.tbl' WITH FORMAT TBL;");
        const auto expected_lqp = ExportNode::make("a_file.tbl", FileType::Tbl,
                                                   stored_table_node_int_float);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY int_float TO 'a_file.tbl' WITH FORMAT CSV;");
        const auto expected_lqp = ExportNode::make("a_file.tbl", FileType::Csv,
                                                   stored_table_node_int_float);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY int_float TO 'a_file.tbl' WITH FORMAT BINARY;");
        const auto expected_lqp = ExportNode::make(
            "a_file.tbl", FileType::Binary, stored_table_node_int_float);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "COPY int_float TO 'a_file.tbl' WITH FORMAT BIN;");
        const auto expected_lqp = ExportNode::make(
            "a_file.tbl", FileType::Binary, stored_table_node_int_float);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
...
TEST_F(SQLTranslatorTest, ImportStatement)
{
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "IMPORT FROM TBL FILE 'a_file.tbl' INTO a_table;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Tbl);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "IMPORT FROM CSV FILE 'a_file.tbl' INTO a_table;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Csv);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "IMPORT FROM BINARY FILE 'a_file.tbl' INTO a_table;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Binary);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] = sql_to_lqp_helper(
            "IMPORT FROM BIN FILE 'a_file.tbl' INTO a_table;");
        const auto expected_lqp =
            ImportNode::make("a_table", "a_file.tbl", FileType::Binary);
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
...
{
    EXPECT_THROW(sql_to_lqp_helper("SELECT DATE '2001-01-35';"),
                 InvalidInputException);

    const auto value_expression =
        expression_vector(value_(pmr_string{"2000-01-31"}));
    // clang-format off
  const auto expected_lqp =
  AliasNode::make(value_expression, std::vector<std::string>{"2000-01-31"},
    ...
    // clang-format on
    const auto [actual_lqp, translation_info] =
        sql_to_lqp_helper("SELECT DATE '2000-01-31';");
    EXPECT_LQP_EQ(actual_lqp, expected_lqp);
}

TEST_F(SQLTranslatorTest, IntervalLiteral)
{
    // Though most of these queries are valid SQL, we want to ensure to reject
    // expressions Hyrise cannot handle
    EXPECT_THROW(sql_to_lqp_helper("SELECT INTERVAL '3' day from int_string;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper(
                     "SELECT * from int_string WHERE b = INTERVAL '3' day;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT 'abc' + 3 days;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT 1 + 3 days;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT DATE '2001-01-01' / 3 days;"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT DATE '2001-01-01' + 1 second;"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT INTERVAL '1' day + INTERVAL '1' day;"),
        InvalidInputException);
  ...
  ProjectionNode::make(expression_vector(value_(pmr_string{"2000-01-31"})),
    ...
    {
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("SELECT '2000-01-01' + INTERVAL '30' day;");
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("SELECT '2002-01-31' - INTERVAL '2 years';");
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("SELECT '1999-12-31' + 1 month;");
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
}
...
{
    EXPECT_THROW(sql_to_lqp_helper("SELECT CAST('abc' AS DATE)"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT CAST(1 AS DATE)"),
                 InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT CAST(a AS DATE) FROM int_string"),
                 InvalidInputException);
    // We do not have a real Date data type, so we cannot pass this cast
    // anywhere else.
    EXPECT_THROW(sql_to_lqp_helper("SELECT CAST(b AS DATE) FROM int_string"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT CAST('2000-01-01 00:00:x' AS DATETIME)"),
        InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT CAST('10-01-01 00:00:00' AS DATETIME)"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT CAST('not_a_datetime' AS DATETIME)"),
                 InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT CAST(b AS DATETIME) FROM int_string"),
        InvalidInputException);

    {
        const auto cast_expression =
            expression_vector(cast_(value_(pmr_string{'1'}), DataType::Int));
        // clang-format off
    const auto expected_lqp =
    ProjectionNode::make(cast_expression,
      DummyTableNode::make());
        // clang-format on
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("SELECT CAST('1' as INT);");
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        // Omit the cast when source and target data type are identical
        const auto value_expression =
            expression_vector(value_(pmr_string{'1'}));
        // clang-format off
    const auto expected_lqp =
    ProjectionNode::make(value_expression,
      DummyTableNode::make());
        // clang-format on
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("SELECT CAST('1' as TEXT);");
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        // Dates need to be translated to a value expression, as we do not have
        // a Date DataType
        const auto value_expression =
            expression_vector(value_(pmr_string{"2000-01-01"}));
        // clang-format off
    const auto expected_lqp =
    ProjectionNode::make(value_expression,
      DummyTableNode::make());
        // clang-format on
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("SELECT CAST('2000-01-01' as DATE);");
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
    {
        // Date times need to be translated to a value expression, as we do not
        // have a DateTime DataType
        const auto value_expression =
            expression_vector(value_(pmr_string{"2000-01-02 02:02:01"}));
        // clang-format off
    const auto expected_lqp =
    ProjectionNode::make(value_expression,
      DummyTableNode::make());
        // clang-format on
        const auto [actual_lqp_1, translation_info_1] = sql_to_lqp_helper(
            "SELECT CAST('2000-01-02 02:02:01' as DATETIME);");
        EXPECT_LQP_EQ(actual_lqp_1, expected_lqp);
        // Resolve overflowing time components
        const auto [actual_lqp_2, translation_info_2] = sql_to_lqp_helper(
            "SELECT CAST('2000-01-01 25:61:61' as DATETIME);");
        EXPECT_LQP_EQ(actual_lqp_2, expected_lqp);
    }
    {
        // BIGINT is an alias for LONG
        const auto cast_expression =
            expression_vector(cast_(value_(int32_t{1}), DataType::Long));
        // clang-format off
    const auto expected_lqp =
    ProjectionNode::make(cast_expression,
      DummyTableNode::make());
        // clang-format on
        const auto [actual_lqp, translation_info] =
            sql_to_lqp_helper("SELECT CAST(1 as BIGINT);");
        EXPECT_LQP_EQ(actual_lqp, expected_lqp);
    }
}
...
{
    const auto &[actual_lqp, _] = sql_to_lqp_helper(
        "SELECT a, b, row_number() OVER (ORDER BY a DESC) FROM int_float;");

    auto frame = FrameDescription{
        FrameType::Range, FrameBound{0, FrameBoundType::Preceding, true},
        FrameBound{0, FrameBoundType::CurrentRow, false}};
    const auto window =
        window_(expression_vector(), expression_vector(int_float_a),
                std::vector<SortMode>{SortMode::Descending}, std::move(frame));
    const auto expected_lqp =
        WindowNode::make(row_number_(window), stored_table_node_int_float);
...
TEST_F(SQLTranslatorTest, WindowFunctionFrames)
{
    ...
    const auto frame_types = std::vector<std::pair<std::string, FrameType>>{
        {"RANGE", FrameType::Range}, {"ROWS", FrameType::Rows}};
    const auto frame_descriptions = std::vector<std::string>{
        "UNBOUNDED PRECEDING",
        "BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING",
        "BETWEEN 1 PRECEDING AND 2 FOLLOWING",
        "BETWEEN CURRENT ROW AND CURRENT ROW"};
    const auto expected_starts = std::vector<FrameBound>{
        FrameBound{0, FrameBoundType::Preceding, true},
        FrameBound{0, FrameBoundType::Preceding, true},
        FrameBound{1, FrameBoundType::Preceding, false},
        FrameBound{0, FrameBoundType::CurrentRow, false}};
    ...
    {
        for (auto frame_bound_id = size_t{0};
             frame_bound_id < frame_descriptions.size(); ++frame_bound_id)
        {
            const auto query =
                "SELECT a, b, row_number() OVER (ORDER BY a " + frame_type_str +
                " " + frame_descriptions[frame_bound_id] + " ) FROM int_float;";
            const auto &[actual_lqp, _] = sql_to_lqp_helper(query);

            auto frame =
                FrameDescription{frame_type, expected_starts[frame_bound_id],
                                 expected_ends[frame_bound_id]};
            const auto window = window_(
                expression_vector(), expression_vector(int_float_a),
                std::vector<SortMode>{SortMode::Ascending}, std::move(frame));
            const auto expected_lqp = WindowNode::make(
                row_number_(window), stored_table_node_int_float);
...
TEST_F(SQLTranslatorTest, WindowDifferentFunctions)
{
    ...
    const auto window = window_(
        std::move(partition_by_expressions), std::move(order_by_expressions),
        std::vector<SortMode>{SortMode::Ascending}, std::move(frame));
    const auto expected_functions = expression_vector(
        rank_(window), dense_rank_(window), percent_rank_(window),
        cume_dist_(window), row_number_(window), min_(int_float_a, window),
        max_(int_float_a, window), avg_(int_float_a, window),
        count_(int_float_a, window), count_distinct_(int_float_a, window));

    for (auto window_function_id = size_t{0};
         window_function_id < functions.size(); ++window_function_id)
    {
        const auto query = "SELECT a, b, " + functions[window_function_id] +
                           " OVER (PARTITION BY b ORDER BY a) FROM int_float;";
        const auto [actual_lqp, _] = sql_to_lqp_helper(query);

        const auto expected_lqp =
            WindowNode::make(expected_functions[window_function_id],
                             stored_table_node_int_float);
...
{
    // We need one ProjectionNode before the WindowNode to calculate a + b and
    // one after it to compute 1.0 / row_number().
    const auto [actual_lqp, _] = sql_to_lqp_helper(
        "SELECT 1.0 / row_number() OVER (ORDER BY a + b) FROM int_float;");

    auto frame = FrameDescription{
        FrameType::Range, FrameBound{0, FrameBoundType::Preceding, true},
        FrameBound{0, FrameBoundType::CurrentRow, false}};
    const auto window = window_(
        expression_vector(), expression_vector(add_(int_float_a, int_float_b)),
        std::vector<SortMode>{SortMode::Ascending}, std::move(frame));
...
TEST_F(SQLTranslatorTest, MultipleWindowFunctions)
{
    const auto [actual_lqp, _] =
        sql_to_lqp_helper("SELECT a, b, rank() OVER (), row_number() OVER "
                          "(ORDER BY a) FROM int_float;");

    auto frame_a = FrameDescription{
        FrameType::Range, FrameBound{0, FrameBoundType::Preceding, true},
        FrameBound{0, FrameBoundType::CurrentRow, false}};
    auto frame_b = frame_a;
    const auto window_a = window_(expression_vector(), expression_vector(),
                                  std::vector<SortMode>{}, std::move(frame_a));
    const auto window_b =
        window_(expression_vector(), expression_vector(int_float_a),
...
TEST_F(SQLTranslatorTest, InvalidWindowFunctions)
{
    ...
    EXPECT_THROW(sql_to_lqp_helper("SELECT rank() FROM int_float;"),
                 InvalidInputException);
    // Pure window functions must not have an argument.
    EXPECT_THROW(sql_to_lqp_helper("SELECT rank(a) OVER () FROM int_float;"),
                 InvalidInputException);
    // Aggregate functions used as window functions must have an argument.
    EXPECT_THROW(sql_to_lqp_helper("SELECT avg() OVER () FROM int_float;"),
                 InvalidInputException);
    // Window functions can only be used in the SELECT list.
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT a FROM int_float WHERE avg(a) OVER () > a;"),
        InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT a FROM int_float GROUP BY avg(a) OVER ();"),
        InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT a FROM int_float ORDER BY avg(a) OVER ();"),
        InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT a FROM int_float LIMIT min(a) OVER ();"),
        InvalidInputException);
    // Window functions must not be nested.
    EXPECT_THROW(
        sql_to_lqp_helper(
            "SELECT rank() OVER(ORDER BY avg(a) OVER ()) FROM int_float;"),
        InvalidInputException);
    EXPECT_THROW(sql_to_lqp_helper("SELECT rank() OVER(ORDER BY a) rnk, avg() "
                                   "OVER(ORDER BY rnk) FROM int_float;"),
                 InvalidInputException);
    // Arguments of window functions must be available or derivable.
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT avg(a) OVER () FROM int_float GROUP BY b;"),
        InvalidInputException);
    // PATITION BY and ORDER BY expressions must be available or derivable.
    EXPECT_THROW(
        sql_to_lqp_helper(
            "SELECT rank() OVER(PARTITION BY a) FROM int_float GROUP BY b;"),
        InvalidInputException);
    EXPECT_THROW(
        sql_to_lqp_helper(
            "SELECT rank() OVER(ORDER BY a) FROM int_float GROUP BY b;"),
        InvalidInputException);
    // GROUPS frames are not supported.
    EXPECT_THROW(
        sql_to_lqp_helper(
            "SELECT rank() OVER (GROUPS UNBOUNDED PRECEDING) FROM int_float;"),
        InvalidInputException);
    // Frame begin must not be UNBOUNDED FOLLOWING.
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT rank() OVER (ROWS BETWEEN UNBOUNDED "
                          "FOLLOWING AND UNBOUNDED FOLLOWING) FROM int_float;"),
        InvalidInputException);
    // Frame end must not be UNBOUNDED PRECEDING.
    EXPECT_THROW(sql_to_lqp_helper("SELECT rank() OVER (ROWS BETWEEN UNBOUNDED "
                                   "PRECEDING AND UNBOUNDED PRECEDING) FROM "
                                   "int_float;"),
                 InvalidInputException);
    // Frame end must not be before start.
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT rank() OVER (ROWS BETWEEN 1 FOLLOWING AND "
                          "CURRENT ROW) FROM int_float;"),
        InvalidInputException);
    // Function must be a window function.
    EXPECT_THROW(
        sql_to_lqp_helper("SELECT substr(b, 1, 3) OVER () FROM int_string;"),
        InvalidInputException);
}
