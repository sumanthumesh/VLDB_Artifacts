...
class SQLPipelineTest : public BaseTest
{
  protected:
    ...
    void SetUp() override
    {
        // We reload table_a every time since it is modified during the test
        // case.
        _table_a =
            load_table("resources/test_data/tbl/int_float.tbl", ChunkOffset{2});
        Hyrise::get().storage_manager.add_table("table_a", _table_a);

        Hyrise::get().storage_manager.add_table("table_a_multi",
                                                _table_a_multi);
    ...
    const std::string _multi_statement_dependent =
        "CREATE VIEW foo AS SELECT * FROM table_a; SELECT * FROM foo;";
    // VIEW --> VIE
    const std::string _multi_statement_invalid =
        "CREATE VIE foo AS SELECT * FROM table_a; SELECT * FROM foo;";
};
...
{
    auto sql_pipeline =
        SQLPipelineBuilder{_multi_statement_query}.create_pipeline();
...
TEST_F(SQLPipelineTest, SimpleCreationInvalid)
{
    EXPECT_THROW(
        auto sql_pipeline =
            SQLPipelineBuilder{_multi_statement_invalid}.create_pipeline(),
        std::exception);
}
...
{
    auto sql_pipeline = SQLPipelineBuilder{_select_query_a}.create_pipeline();
    const auto &parsed_sql_statements =
        sql_pipeline.get_parsed_sql_statements();
...
{
    auto sql_pipeline =
        SQLPipelineBuilder{_multi_statement_dependent}.create_pipeline();
...
TEST_F(SQLPipelineTest, GetTasksExecutionRequired)
{
    ...
    catch (const std::exception &e)
    {
        const auto error_msg = std::string(e.what());
        // Check that the correct error was thrown
        EXPECT_TRUE(error_msg.find("Cannot generate tasks") !=
                    std::string::npos);
    }
}
...
TEST_F(SQLPipelineTest, UpdateWithTransactionFailure)
{
    ...
    const auto sql = "UPDATE table_a SET a = 1 WHERE a = 12345; UPDATE table_a "
                     "SET a = 1 WHERE a = 123; "
    ...
    EXPECT_EQ(sql_pipeline.failed_pipeline_statement()->get_sql_string(),
              "UPDATE table_a SET a = 1 WHERE a = 123;");
    ...
    EXPECT_EQ(first_chunk_mvcc_data->get_end_cid(ChunkOffset{0}),
              MvccData::MAX_COMMIT_ID);

    EXPECT_EQ(first_chunk_mvcc_data->get_tid(ChunkOffset{1}),
              TransactionID{17});
    EXPECT_EQ(first_chunk_mvcc_data->get_end_cid(ChunkOffset{1}),
              MvccData::MAX_COMMIT_ID);

    auto second_chunk_mvcc_data = _table_a->get_chunk(ChunkID{1})->mvcc_data();

    EXPECT_EQ(second_chunk_mvcc_data->get_tid(ChunkOffset{0}),
              TransactionID{0});
    EXPECT_EQ(second_chunk_mvcc_data->get_end_cid(ChunkOffset{0}),
              MvccData::MAX_COMMIT_ID);
}

TEST_F(SQLPipelineTest, UpdateWithTransactionFailureAutoCommit)
{
    // Similar to UpdateWithTransactionFailure, but without explicit transaction
    // context
    ...
    // This time, the first row should have been updated before the second
    // statement failed
    EXPECT_EQ(first_chunk_mvcc_data->get_tid(ChunkOffset{0}), TransactionID{1});
    EXPECT_EQ(first_chunk_mvcc_data->get_end_cid(ChunkOffset{0}),
              CommitID{2}); // initial commit ID + 1

    // This row was being modified by a different transaction, so it should not
    // have been touched
    EXPECT_EQ(first_chunk_mvcc_data->get_tid(ChunkOffset{1}),
              TransactionID{17});
    EXPECT_EQ(first_chunk_mvcc_data->get_end_cid(ChunkOffset{1}),
...
{
    EXPECT_FALSE(SQLPipelineBuilder{_select_query_a}
                     .create_pipeline()
                     .requires_execution());
    EXPECT_FALSE(
        SQLPipelineBuilder{_join_query}.create_pipeline().requires_execution());
    EXPECT_FALSE(SQLPipelineBuilder{_multi_statement_query}
                     .create_pipeline()
                     .requires_execution());
    ...
    const std::string create_view_single =
        "CREATE VIEW blub AS SELECT * FROM foo;";
    EXPECT_FALSE(SQLPipelineBuilder{create_view_single}
                     .create_pipeline()
                     .requires_execution());

    const std::string create_view_multi_reverse =
        "SELECT * FROM blub; " + create_view_single;
    EXPECT_TRUE(SQLPipelineBuilder{create_view_multi_reverse}
                    .create_pipeline()
                    .requires_execution());

    const std::string create_view_multi_middle =
        create_view_multi_reverse + " SELECT * FROM foo;";
    EXPECT_TRUE(SQLPipelineBuilder{create_view_multi_reverse}
                    .create_pipeline()
                    .requires_execution());

    const std::string create_table_single = "CREATE TABLE foo2 (c int);";
    EXPECT_FALSE(SQLPipelineBuilder{create_table_single}
                     .create_pipeline()
                     .requires_execution());

    const std::string create_table_multi =
        create_table_single + "SELECT * FROM foo2;";
    EXPECT_TRUE(SQLPipelineBuilder{create_table_multi}
                    .create_pipeline()
                    .requires_execution());

    const std::string drop_table_single = "DROP TABLE foo;";
    EXPECT_FALSE(SQLPipelineBuilder{drop_table_single}
                     .create_pipeline()
                     .requires_execution());

    const std::string drop_table_multi =
        "SELECT * FROM foo; " + drop_table_single;
    EXPECT_TRUE(SQLPipelineBuilder{drop_table_multi}
                    .create_pipeline()
                    .requires_execution());

    const std::string multi_no_exec =
        "SELECT * FROM foo; INSERT INTO foo VALUES (2); SELECT * FROM blub; "
        "DELETE FROM foo WHERE a = 2;";
    EXPECT_FALSE(SQLPipelineBuilder{multi_no_exec}
                     .create_pipeline()
                     .requires_execution());
}
...
{
    // Tests that the string passed into the pipeline is correctly split into
    // the statement substrings
    auto select_pipeline =
        SQLPipelineBuilder{_select_query_a}.create_pipeline();
    ...
    auto dependent_pipeline =
        SQLPipelineBuilder{_multi_statement_query}.create_pipeline();
    const auto &dependent_strings = dependent_pipeline.get_sql_per_statement();
    EXPECT_EQ(dependent_strings.size(), 2u);
    // "INSERT INTO table_a VALUES (11, 11.11); SELECT * FROM table_a";
    EXPECT_EQ(dependent_strings.at(0),
              "INSERT INTO table_a VALUES (11, 11.11);");
    EXPECT_EQ(dependent_strings.at(1),
              "SELECT * FROM table_a"); // leading whitespace should be removed
    ...
    EXPECT_EQ(spacing_strings.at(0),
              "SELECT\na, b, c,d,e FROM\t(SELECT * FROM foo);"); // internal
                                                                 // formatting
                                                                 // is not done
  ...
  ORDER BY foo.x ASC
    ...
    auto multi_line_pipeline =
        SQLPipelineBuilder{multi_line_sql}.create_pipeline();
    const auto &multi_line_strings =
        multi_line_pipeline.get_sql_per_statement();
...
TEST_F(SQLPipelineTest, CacheQueryPlanTwice)
{
    ...
    // The second part of _multi_statement_query is _select_query_a, which is
    // already cached
...
{
    auto sql_pipeline_1 =
        SQLPipelineBuilder{"CREATE TABLE t (a_int INTEGER)"}.create_pipeline();
    ...
    auto sql_pipeline_5 =
        SQLPipelineBuilder{"DROP TABLE IF EXISTS t"}.create_pipeline();
    EXPECT_NO_THROW(sql_pipeline_5.get_result_table());

    auto sql_pipeline_6 =
        SQLPipelineBuilder{"CREATE TABLE t2 (a_int INTEGER); DROP TABLE t2"}
            .create_pipeline();
...
TEST_F(SQLPipelineTest, GetResultTableNoReexecuteOnConflict)
{
...

    {
        ...
        auto conflicting_sql_pipeline =
            SQLPipelineBuilder{conflicting_sql}.create_pipeline();
    ...
    const auto sql = "INSERT INTO table_a (a, b) VALUES (1, 2.0); UPDATE "
                     "table_a SET a = a + 1 WHERE b < 457";
    auto sql_pipeline = SQLPipelineBuilder{sql}
                            .with_transaction_context(transaction_context)
                            .create_pipeline();
    const auto [pipeline_status, table] = sql_pipeline.get_result_table();
    ...
    // The INSERT could not be committed, but still created a row that never
    ...
    const auto verify_table_contents = []()
    {
        ...
        const auto [verification_status, verification_table] =
            verification_pipeline.get_result_table();
