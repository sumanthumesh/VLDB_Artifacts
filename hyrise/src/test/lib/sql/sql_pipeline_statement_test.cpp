...
namespace
{
// This function is a slightly hacky way to check whether an LQP was optimized.
// This relies on JoinOrderingRule and could break if something is changed
// within the optimizer. It assumes that for the query: SELECT * from a, b WHERE
// a.a = b.a will be translated to a Cross Join with a filter predicate and then
// optimized to a Join.
std::function<bool(const std::shared_ptr<hyrise::AbstractLQPNode> &)>
    contains_cross = [](const std::shared_ptr<hyrise::AbstractLQPNode> &node)
{
...
class SQLPipelineStatementTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _table_int = load_table("resources/test_data/tbl/int_int_int.tbl",
                                ChunkOffset{2});
        ...
        _multi_statement_parse_result =
            std::make_shared<hsql::SQLParserResult>();
        hsql::SQLParser::parse(_multi_statement_dependant,
                               _multi_statement_parse_result.get());
    ...
    get_sql_pipeline_statements(SQLPipeline &sql_pipeline)
    {
    ...
    const std::string _join_query =
        "SELECT table_a.a, table_a.b, table_b.b AS bb FROM table_a, table_b "
        "WHERE table_a.a = table_b.a AND table_a.a "
        "> 1000";
    const std::string _multi_statement_query =
        "INSERT INTO table_a VALUES (11, 11.11); SELECT * FROM table_a";
    const std::string _multi_statement_dependant =
        "CREATE VIEW foo AS SELECT * FROM table_a; SELECT * FROM foo;";
    ...
    static bool
    _contains_validate(const std::vector<std::shared_ptr<AbstractTask>> &tasks)
    {
        for (const auto &task : tasks)
        {
            if (auto op_task = std::dynamic_pointer_cast<OperatorTask>(task))
            {
                if (std::dynamic_pointer_cast<Validate>(
                        op_task->get_operator()))
                {
                    return true;
                }
            }
...
{
    auto sql_pipeline =
        SQLPipelineBuilder{_join_query}.disable_mvcc().create_pipeline();
    auto statement = get_sql_pipeline_statements(sql_pipeline).at(0);
...
TEST_F(SQLPipelineStatementTest, SimpleCreationWithCustomTransactionContext)
{
    ...
    // Execute SQLPipeline so that the transaction context gets set in the
    // statement(s)
...
TEST_F(SQLPipelineStatementTest, ConstructorCombinations)
{
    ...
    auto sql_pipeline1 = SQLPipelineBuilder{_select_query_a}
    ...
    auto sql_pipeline2 =
        SQLPipelineBuilder{_select_query_a}.disable_mvcc().create_pipeline();
...
TEST_F(SQLPipelineStatementTest, GetCachedOptimizedLQPValidated)
{
    ...
    auto validated_sql_pipeline = SQLPipelineBuilder{_select_query_a}
                                      .with_lqp_cache(_lqp_cache)
                                      .create_pipeline();
    auto &validated_statement =
        get_sql_pipeline_statements(validated_sql_pipeline).at(0);

    const auto &validated_lqp =
        validated_statement->get_optimized_logical_plan();
    ...
    auto not_validated_sql_pipeline = SQLPipelineBuilder{_select_query_a}
                                          ...
                                          .disable_mvcc()
                                          .create_pipeline();
    auto &not_validated_statement =
        get_sql_pipeline_statements(not_validated_sql_pipeline).at(0);
    const auto &not_validated_lqp =
        not_validated_statement->get_optimized_logical_plan();
...
TEST_F(SQLPipelineStatementTest, GetOptimizedLQPDoesNotInfluenceUnoptimizedLQP)
{
    ...
    // The optimizer works on the original LQP nodes which could be modified
    // during optimization. Copy the structure to check that it is equal after
    // optimizing.
    std::shared_ptr<AbstractLQPNode> unoptimized_copy =
        unoptimized_lqp->deep_copy();
...
TEST_F(SQLPipelineStatementTest, GetQueryPlanJoinWithFilter)
{
    ...
    auto is_join_op = [](const std::shared_ptr<const AbstractOperator> &node)
    {
        return static_cast<bool>(
            std::dynamic_pointer_cast<const AbstractJoinOperator>(node));
    };
...
TEST_F(SQLPipelineStatementTest, ClearOperators)
{
    ...
    {
        const auto &executed_operator =
            static_cast<const OperatorTask &>(*task).get_operator();
        EXPECT_EQ(executed_operator->state(),
...
TEST_F(SQLPipelineStatementTest, GetResultTableNoOutputNoReexecution)
{
    ...
    const auto verify_table_contents = [this]()
    {
        ...
        auto verification_pipeline =
            SQLPipelineBuilder{verification_sql}.create_pipeline();
        auto &verification_statement =
            get_sql_pipeline_statements(verification_pipeline).at(0);
        const auto [verification_status, verification_table] =
            verification_statement->get_result_table();
    ...
    // Check that this doesn't crash. This should not modify the table a second
    // time.
...
TEST_F(SQLPipelineStatementTest,
       GetResultTableTransactionFailureExplicitTransaction)
{
    // Mark a row as modified by a different transaction
    _table_a->get_chunk(ChunkID{0})
        ->mvcc_data()
        ->set_tid(ChunkOffset{0}, TransactionID{17});
...
{
    const auto subquery_query =
        "SELECT * FROM table_int WHERE a = (SELECT MAX(b) FROM table_int)";

    auto first_subquery_sql_pipeline =
        SQLPipelineBuilder{subquery_query}.create_pipeline();
    auto &first_subquery_statement =
        get_sql_pipeline_statements(first_subquery_sql_pipeline).at(0);

    const auto [first_subquery_status, first_subquery_result] =
        first_subquery_statement->get_result_table();
    EXPECT_EQ(first_subquery_status, SQLPipelineStatus::Success);

    auto expected_first_result = std::make_shared<Table>(
        _int_int_int_column_definitions, TableType::Data);
    ...
    auto second_subquery_sql_pipeline =
        SQLPipelineBuilder{subquery_query}.create_pipeline();
    auto &second_subquery_statement =
        get_sql_pipeline_statements(second_subquery_sql_pipeline).at(0);
    const auto [second_subquery_status, second_subquery_result] =
        second_subquery_statement->get_result_table();
    EXPECT_EQ(second_subquery_status, SQLPipelineStatus::Success);

    auto expected_second_result = std::make_shared<Table>(
        _int_int_int_column_definitions, TableType::Data);
...
TEST_F(SQLPipelineStatementTest, DefaultPlanCaches)
{
    ...
    // No caches
    auto sql_pipeline_0 =
        SQLPipelineBuilder{"SELECT * FROM table_a"}.create_pipeline();
...
{
    {
        auto sql_pipeline =
            SQLPipelineBuilder{"SELECT * FROM table_a"}.create_pipeline();
        auto translation_info = get_sql_pipeline_statements(sql_pipeline)
                                    .at(0)
                                    ->get_sql_translation_info();

        EXPECT_TRUE(translation_info.cacheable);
        EXPECT_TRUE(
            translation_info.parameter_ids_of_value_placeholders.empty());
    }

    {
        auto sql_pipeline =
            SQLPipelineBuilder{"SELECT * FROM meta_tables"}.create_pipeline();
        auto translation_info = get_sql_pipeline_statements(sql_pipeline)
                                    .at(0)
                                    ->get_sql_translation_info();
...

    {
        auto sql_pipeline =
            SQLPipelineBuilder{
                "SELECT * FROM table_a WHERE a > ? AND b BETWEEN 5 AND ?"}
                .create_pipeline();
        auto translation_info = get_sql_pipeline_statements(sql_pipeline)
                                    .at(0)
                                    ->get_sql_translation_info();
        ...
        EXPECT_EQ(translation_info.parameter_ids_of_value_placeholders,
                  parameters);
    }

    {
        auto sql_pipeline =
            SQLPipelineBuilder{
                "SELECT * FROM table_a t1 WHERE a > ? AND b = (SELECT MAX(b) "
                "FROM table_a t2 WHERE t2.a = t1.a AND b > ?)"}
                .create_pipeline();
        auto translation_info = get_sql_pipeline_statements(sql_pipeline)
                                    .at(0)
                                    ->get_sql_translation_info();
