...
class QueryPlanCacheTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        auto table_b = load_table("resources/test_data/tbl/int_float2.tbl",
                                  ChunkOffset{2});
    ...
    {
        auto pipeline =
            SQLPipelineBuilder{query}.with_pqp_cache(cache).create_pipeline();
...
// Check access to PQP cache. When set, check the underlying cache
// implementation, and verify that it is a GDFS cache that supports retrieving
// the cache frequency count.
TEST_F(QueryPlanCacheTest, CachedPQPFrequencyCount)
{
    // Create pipeline and pass pqp cache. Verify that this does not change
    // default_pqp_cache.
    auto sql_pipeline =
        SQLPipelineBuilder{Q1}.with_pqp_cache(cache).create_pipeline();
    ...
    // Create new pipeline, without setting a cache (default cache set
    // previously). Execute pipeline and check if frequency of query is as
    // expected.
