...
class SQLIdentifierResolverTest : public BaseTest
{
  public:
    void SetUp() override
    {
        ...
        expression_a =
            std::make_shared<LQPColumnExpression>(node_a, ColumnID{0});
        expression_b =
            std::make_shared<LQPColumnExpression>(node_a, ColumnID{1});
        expression_c =
            std::make_shared<LQPColumnExpression>(node_a, ColumnID{2});
        expression_unnamed =
            std::make_shared<LQPColumnExpression>(node_a, ColumnID{3});
    ...
    std::shared_ptr<AbstractExpression> expression_a, expression_b,
        expression_c, expression_unnamed;
...
{
    auto expression_a2 =
        std::make_shared<LQPColumnExpression>(node_c, ColumnID{2});
...
{
    /**
     * Simulate a scenario in which a Sub-Subquery accesses an Identifier from
     * the outermost and intermediate queries
     */

    /**
     * Create context and context proxy for the outermost query
     */
    const auto outermost_expression_a =
        std::make_shared<LQPColumnExpression>(node_b, ColumnID{0});
    const auto outermost_expression_b =
        std::make_shared<LQPColumnExpression>(node_b, ColumnID{1});
    const auto outermost_expression_c =
        std::make_shared<LQPColumnExpression>(node_b, ColumnID{2});
    ...
    outermost_context->add_column_name(outermost_expression_b,
                                       "b"); // Intentionally named just "b"
    outermost_context->add_column_name(outermost_expression_c,
                                       "c"); // Intentionally named just "c"
    outermost_context->set_table_name(outermost_expression_b, "Outermost");

    const auto outermost_context_proxy =
        std::make_shared<SQLIdentifierResolverProxy>(outermost_context,
                                                     parameter_id_allocator);
    ...
    const auto intermediate_expression_a =
        std::make_shared<LQPColumnExpression>(node_c, ColumnID{0});
    const auto intermediate_expression_b =
        std::make_shared<LQPColumnExpression>(node_c, ColumnID{1});
    intermediate_context->add_column_name(intermediate_expression_a,
                                          "intermediate_a");
    intermediate_context->add_column_name(intermediate_expression_b,
                                          "b"); // Intentionally named just "b"
    intermediate_context->set_table_name(intermediate_expression_b,
                                         "Intermediate");

    const auto intermediate_context_proxy =
        std::make_shared<SQLIdentifierResolverProxy>(intermediate_context,
                                                     parameter_id_allocator,
                                                     outermost_context_proxy);
    ...
    EXPECT_EQ(
        *intermediate_context_proxy->resolve_identifier_relaxed(
            {"b", "Intermediate"}),
    ...
    EXPECT_EQ(
        *intermediate_context_proxy->resolve_identifier_relaxed({"b"}),
        *correlated_parameter_(ParameterID{0}, intermediate_expression_b));
    EXPECT_EQ(intermediate_context_proxy->resolve_identifier_relaxed(
                  {"intermediate_a", "Intermediate"}),
              nullptr);
    ...
    EXPECT_EQ(outermost_context_proxy->accessed_expressions().count(
                  outermost_expression_a),
              1u);
    EXPECT_EQ(outermost_context_proxy->accessed_expressions().count(
                  outermost_expression_b),
              1u);

    ASSERT_EQ(intermediate_context_proxy->accessed_expressions().size(), 2u);
    EXPECT_EQ(intermediate_context_proxy->accessed_expressions().count(
                  intermediate_expression_b),
              1u);
    EXPECT_EQ(intermediate_context_proxy->accessed_expressions().count(
                  intermediate_expression_a),
              1u);
}

TEST_F(SQLIdentifierResolverTest, GetExpressionIdentifiers)
{
    EXPECT_EQ(context.get_expression_identifiers(expression_a),
              std::vector<SQLIdentifier>{SQLIdentifier("a", "T1")});
    EXPECT_EQ(context.get_expression_identifiers(expression_unnamed),
              std::vector<SQLIdentifier>{});
}

TEST_F(SQLIdentifierResolverTest, DeepEqualsIsUsed)
{
    /**
     * Test that we can use equivalent Expression objects that are stored in
     * different Objects
     */

    const auto expression_a2 =
        std::make_shared<LQPColumnExpression>(node_a, ColumnID{0});
    const std::vector<SQLIdentifier> expressions = {SQLIdentifier("a"s, "T2"),
                                                    SQLIdentifier("a2"s, "T2")};
...
TEST_F(SQLIdentifierResolverTest, ResolveTableName)
{
    ...
    const auto t1_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>(
            {expression_a, expression_b});
    const auto t2_expressions =
        std::vector<std::shared_ptr<AbstractExpression>>({expression_c});
