...
{
    // This checks if the EntireChunkPosList correctly handles rows that are
    // added to the table after the PosList was created. These later added rows
    // should not be contained in the PosList

    auto table_name = "test_table";
    auto table = load_table("resources/test_data/tbl/float_int.tbl",
                            ChunkOffset{10}, FinalizeLastChunk::No);
    EXPECT_EQ(table->chunk_count(), 1);
    auto table_to_add_name = "test_table_to_add";
    auto table_to_add =
        load_table("resources/test_data/tbl/float_int.tbl", ChunkOffset{10});
    ...
    // TODO(anyone): Maybe add a better check than just size, cause the returned
    // iterators should also handle. this case, which we right now don't check.
}

TEST_F(EntireChunkPosListTest, InsertDoesNotAffectIterators)
{
    // This checks that the EntireChunkPosList does not change its iterators
    // after rows were added to the table. These added rows should not be
    // contained in the PosList.
    ...
    // Currently, the EntireChunkPosList is not linked to the table. Therefore,
    // we do not expect changes to the PosList when new tuples are appended to
    // the table. Nevertheless, we added this test to ensure this assumption is
    // still true even when the EntireChunkPosList might later be aware of the
    // referenced table.
    EXPECT_EQ(table->chunk_count(), 1);
    EXPECT_EQ(table->row_count(), 3);

    const auto entire_chunk_pos_list =
        std::make_shared<const EntireChunkPosList>(ChunkID{0}, ChunkOffset{3});
    ...
    EXPECT_EQ(entire_chunk_pos_list->size(), 3);
    EXPECT_EQ(entire_chunk_pos_list->begin().distance_to(
                  entire_chunk_pos_list->end()),
              3);
    EXPECT_EQ(entire_chunk_pos_list->cbegin().distance_to(
                  entire_chunk_pos_list->cend()),
              3);
}
