...
class MaterializeTest : public EncodingTest
{
  public:
    void SetUp() override
    {
        _data_table = load_table_with_encoding(
            "resources/test_data/tbl/int_float.tbl", ChunkOffset{2});
        _data_table_with_nulls = load_table_with_encoding(
            "resources/test_data/tbl/int_float_with_null.tbl", ChunkOffset{2});
        ...
        const auto table_scan =
            std::make_shared<TableScan>(table_wrapper, greater_than_(a, 0));
    ...
    materialize_values_and_nulls_to_vector(const AbstractSegment &segment)
    {
    ...
    template <typename T>
    std::vector<bool>
    materialize_nulls_to_vector(const AbstractSegment &segment)
    {
...
{
    EXPECT_EQ(
        std::vector<int32_t>({12345, 123}),
        materialize_values_to_vector<int32_t>(
            *_data_table->get_chunk(ChunkID(0))->get_segment(ColumnID(0))));
    EXPECT_EQ(
        std::vector<int32_t>({1234}),
        materialize_values_to_vector<int32_t>(
            *_data_table->get_chunk(ChunkID(1))->get_segment(ColumnID(0))));
}
...
TEST_P(MaterializeTest, MaterializeTwoIntSegments)
{
    ...
    materialize_values(
        *_data_table->get_chunk(ChunkID(0))->get_segment(ColumnID(0)), values);
    materialize_values(
        *_data_table->get_chunk(ChunkID(1))->get_segment(ColumnID(0)), values);
...
TEST_P(MaterializeTest, MaterializeIntReferences)
{
    EXPECT_EQ(std::vector<int32_t>({12345, 123}),
              materialize_values_to_vector<int32_t>(
                  *_references_table->get_chunk(ChunkID(0))
                       ->get_segment(ColumnID(0))));
    EXPECT_EQ(std::vector<int32_t>({1234}),
              materialize_values_to_vector<int32_t>(
                  *_references_table->get_chunk(ChunkID(1))
                       ->get_segment(ColumnID(0))));
}
...
TEST_P(MaterializeTest, MaterializeTwoIntReferenceSegments)
{
    std::vector<int32_t> values;
    materialize_values(
        *_references_table->get_chunk(ChunkID(1))->get_segment(ColumnID(0)),
        values);
    materialize_values(
        *_references_table->get_chunk(ChunkID(0))->get_segment(ColumnID(0)),
        values);
...
{
    const auto floats_0 = materialize_values_to_vector<float>(
        *_data_table->get_chunk(ChunkID(0))->get_segment(ColumnID(1)));
    ASSERT_EQ(floats_0.size(), 2u);
    EXPECT_FLOAT_EQ(458.7, floats_0.at(0));
    EXPECT_FLOAT_EQ(456.7f, floats_0.at(1));

    const auto floats_1 = materialize_values_to_vector<float>(
        *_data_table->get_chunk(ChunkID(1))->get_segment(ColumnID(1)));
...
{
    const auto floats_0 = materialize_values_to_vector<float>(
        *_references_table->get_chunk(ChunkID(0))->get_segment(ColumnID(1)));
    ASSERT_EQ(floats_0.size(), 2u);
    EXPECT_FLOAT_EQ(458.7, floats_0.at(0));
    EXPECT_FLOAT_EQ(456.7f, floats_0.at(1));

    const auto floats_1 = materialize_values_to_vector<float>(
        *_references_table->get_chunk(ChunkID(1))->get_segment(ColumnID(1)));
    ASSERT_EQ(floats_1.size(), 1u);
    EXPECT_FLOAT_EQ(457.7, floats_1.at(0));
}
...
{
    const auto values_and_nulls =
        materialize_values_and_nulls_to_vector<int32_t>(
            *_data_table_with_nulls->get_chunk(ChunkID(1))
                 ->get_segment(ColumnID(0)));
...
TEST_P(MaterializeTest, MaterializeValuesAndNullsTwoSegments)
{
    ...
    materialize_values_and_nulls(*_data_table_with_nulls->get_chunk(ChunkID(0))
                                      ->get_segment(ColumnID(0)),
                                 values_and_nulls);
    materialize_values_and_nulls(*_data_table_with_nulls->get_chunk(ChunkID(1))
                                      ->get_segment(ColumnID(0)),
                                 values_and_nulls);
...
{
    const auto nulls = materialize_nulls_to_vector<float>(
        *_data_table_with_nulls->get_chunk(ChunkID(0))
...
{
    std::vector<bool> nulls;
    materialize_nulls<float>(*_data_table_with_nulls->get_chunk(ChunkID(0))
                                  ->get_segment(ColumnID(1)),
                             nulls);
    materialize_nulls<float>(*_data_table_with_nulls->get_chunk(ChunkID(1))
                                  ->get_segment(ColumnID(1)),
                             nulls);
