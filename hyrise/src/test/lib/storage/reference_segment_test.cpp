...
class ReferenceSegmentTest : public BaseTest
{
    void SetUp() override
    {
        ...
        _test_table = std::make_shared<Table>(column_definitions,
                                              TableType::Data, ChunkOffset{3});
        ...
        _test_table_dict = std::make_shared<Table>(
            column_definitions2, TableType::Data, ChunkOffset{5}, UseMvcc::Yes);
        ...
        Hyrise::get().storage_manager.add_table("test_table_dict",
                                                _test_table_dict);
    }
...
TEST_F(ReferenceSegmentTest, RetrievesValues)
{
    ...
    auto &segment =
        *(_test_table->get_chunk(ChunkID{0})->get_segment(ColumnID{0}));
...
TEST_F(ReferenceSegmentTest, RetrievesValuesFromChunks)
{
    ...
    auto pos_list = std::make_shared<RowIDPosList>(std::initializer_list<RowID>(
        {RowID{ChunkID{0}, ChunkOffset{2}}, RowID{ChunkID{1}, ChunkOffset{0}},
    ...
    auto &segment_1 =
        *(_test_table->get_chunk(ChunkID{0})->get_segment(ColumnID{0}));
    auto &segment_2 =
        *(_test_table->get_chunk(ChunkID{1})->get_segment(ColumnID{0}));

    EXPECT_EQ(ref_segment[ChunkOffset{0}], segment_1[ChunkOffset{2}]);
    EXPECT_TRUE(variant_is_null(ref_segment[ChunkOffset{1}]) &&
                variant_is_null(segment_2[ChunkOffset{0}]));
...
TEST_F(ReferenceSegmentTest, RetrieveNullValueFromNullRowID)
{
    ...
    auto pos_list = std::make_shared<RowIDPosList>(std::initializer_list<RowID>(
        {RowID{ChunkID{0}, ChunkOffset{0}}, RowID{ChunkID{0}, ChunkOffset{1}},
         NULL_ROW_ID, RowID{ChunkID{0}, ChunkOffset{2}}}));

    auto ref_segment = ReferenceSegment(_test_table, ColumnID{0}, pos_list);

    auto &segment =
        *(_test_table->get_chunk(ChunkID{0})->get_segment(ColumnID{0u}));
...
TEST_F(ReferenceSegmentTest, MemoryUsageEstimation)
{
    ...
    EXPECT_EQ(
        reference_segment_a.memory_usage(MemoryUsageCalculationMode::Sampled),
        reference_segment_b.memory_usage(MemoryUsageCalculationMode::Sampled) +
            2 * sizeof(RowID));
}
