  ...
  public:
    static void SetUpTestCase()
    {
        ...
        auto row_ids = RowIDPosList{
            {ChunkID{0}, ChunkOffset{0}}, {ChunkID{0}, ChunkOffset{8}},
            {ChunkID{0}, ChunkOffset{7}}, {ChunkID{0}, ChunkOffset{1}},
            {ChunkID{0}, ChunkOffset{1}}, {ChunkID{0}, ChunkOffset{5}}};
    ...
    void SetUp() override
    {
        ...
        const auto value_int_segment =
            std::make_shared<ValueSegment<int32_t>>(std::move(int_values_copy));

        int_segment = ChunkEncoder::encode_segment(
            std::dynamic_pointer_cast<ValueSegment<int32_t>>(value_int_segment),
...
{
    auto any_segment_iterable_int =
        create_any_segment_iterable<int32_t>(*int_segment);

    auto values = pmr_vector<int32_t>{};
    any_segment_iterable_int.for_each(
        [&](const auto &position) { values.emplace_back(position.value()); });
...
TEST_P(AnySegmentIterableTest, IntWithPositionFilter)
{
    ...
    any_segment_iterable_int.for_each(
        ...
        [&](const auto &position)
        {
            EXPECT_EQ(position.value(),
                      int_values[(*position_filter)[index].chunk_offset]);
            ++index;
        });
