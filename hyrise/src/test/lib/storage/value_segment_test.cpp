...
{
    /**
     * As ValueSegments are pre-allocated, their size should not change when
     * inserting data, except for strings placed on the heap.
     */

    const auto empty_usage_int =
        vs_int.memory_usage(MemoryUsageCalculationMode::Sampled);
    const auto empty_usage_double =
        vs_double.memory_usage(MemoryUsageCalculationMode::Sampled);
    const auto empty_usage_str =
        vs_str.memory_usage(MemoryUsageCalculationMode::Sampled);

    vs_int.append(1);
    vs_int.append(2);

    const auto short_str = pmr_string{"Hello"};
    const auto longer_str =
        pmr_string{"HelloWorldHaveANiceDayWithSunshineAndGoodCofefe"};
    ...
    EXPECT_EQ(empty_usage_int,
              vs_int.memory_usage(MemoryUsageCalculationMode::Sampled));
    EXPECT_EQ(empty_usage_double,
              vs_double.memory_usage(MemoryUsageCalculationMode::Sampled));

    EXPECT_GE(vs_str.memory_usage(MemoryUsageCalculationMode::Sampled),
              empty_usage_str);
    // The short string will fit within the SSO capacity of a string and the
    // long string will be placed on the heap.
    EXPECT_EQ(vs_str.memory_usage(MemoryUsageCalculationMode::Full),
              empty_usage_str + longer_str.capacity() + 1);
}
