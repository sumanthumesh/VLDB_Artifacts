...
class StorageDictionarySegmentTest
    : public BaseTestWithParam<VectorCompressionType>
{
  protected:
    std::shared_ptr<ValueSegment<int>> vs_int =
        std::make_shared<ValueSegment<int>>();
    std::shared_ptr<ValueSegment<pmr_string>> vs_str =
        std::make_shared<ValueSegment<pmr_string>>();
    std::shared_ptr<ValueSegment<double>> vs_double =
        std::make_shared<ValueSegment<double>>();
};

INSTANTIATE_TEST_SUITE_P(
    VectorCompressionTypes, StorageDictionarySegmentTest,
    ::testing::Values(VectorCompressionType::FixedWidthInteger,
...
TEST_P(StorageDictionarySegmentTest, LowerUpperBound)
{
    ...
    auto dict_segment =
        std::dynamic_pointer_cast<DictionarySegment<int>>(segment);
...
TEST_P(StorageDictionarySegmentTest, CompressSegmentString)
{
    ...
    auto segment = ChunkEncoder::encode_segment(
        ...
        SegmentEncodingSpec{EncodingType::Dictionary, GetParam()});
    auto dict_segment =
        std::dynamic_pointer_cast<DictionarySegment<pmr_string>>(segment);
...
TEST_P(StorageDictionarySegmentTest, CompressSegmentDouble)
{
    ...
    auto segment = ChunkEncoder::encode_segment(
        vs_double, DataType::Double,
        SegmentEncodingSpec{EncodingType::Dictionary, GetParam()});
    auto dict_segment =
        std::dynamic_pointer_cast<DictionarySegment<double>>(segment);
...
TEST_F(StorageDictionarySegmentTest, FixedWidthIntegerVectorSize)
{
    ...
    auto attribute_vector_uint8_t =
        std::dynamic_pointer_cast<const FixedWidthIntegerVector<uint8_t>>(
            dict_segment->attribute_vector());
    auto attribute_vector_uint16_t =
        std::dynamic_pointer_cast<const FixedWidthIntegerVector<uint16_t>>(
            dict_segment->attribute_vector());
...
TEST_F(StorageDictionarySegmentTest, FixedWidthIntegerMemoryUsageEstimation)
{
    ...
    const auto dictionary_segment =
        std::dynamic_pointer_cast<DictionarySegment<int>>(compressed_segment);

    static constexpr auto size_of_attribute = 1u;

    EXPECT_GE(
        dictionary_segment->memory_usage(MemoryUsageCalculationMode::Sampled),
