...
class FixedStringVectorTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        fixed_string_vector = std::make_shared<FixedStringVector>(
            FixedStringVector(strings.begin(), strings.end(), 6u));
    }
...
TEST_F(FixedStringVectorTest, Iterator)
{
    ...
    for (auto it = fixed_string_vector->begin();
         it != fixed_string_vector->end(); ++it)
    {
        *it = fixed_string;
    }
...
{
    const auto &fixed_string_vector2 =
        FixedStringVector(std::move(*fixed_string_vector));
...
{
    EXPECT_EQ(fixed_string_vector->data_size(),
              sizeof(*fixed_string_vector) + 3u * 6u);
}
...
{
    std::vector<pmr_string> strings = {"Larry", "Bill", "Alexander", "Mark",
                                       "Hasso"};
    auto fixed_string_vector1 =
        FixedStringVector(strings.begin(), strings.end(), 10u);
...
// FixedStringsVectors of empty strings have a special handling which needs to
// be tested.
TEST_F(FixedStringVectorTest, StringLengthZero)
{
    std::vector<pmr_string> strings = {"", ""};
    auto fixed_string_vector1 =
        FixedStringVector(strings.begin(), strings.end(), 0u);
...
TEST_F(FixedStringVectorTest, ThrowOnOversizedStrings)
{
    ...
    EXPECT_THROW((FixedStringVector{v.cbegin(), v.cend(), 3u}),
                 std::logic_error);
