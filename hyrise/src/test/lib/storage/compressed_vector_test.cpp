...
class CompressedVectorTest : public BaseTestWithParam<VectorCompressionType>
{
  protected:
    ...
    encode(const pmr_vector<uint32_t> &vector)
    {
        const auto compression_type = GetParam();

        auto encoded_vector =
            compress_vector(vector, compression_type, {}, {max()});
        EXPECT_EQ(encoded_vector->size(), vector.size());
        EXPECT_EQ(parent_vector_compression_type(encoded_vector->type()),
                  compression_type);
                                ...
                                const pmr_vector<uint32_t> &expected_values)
    {
        ...
        for (; encoded_seq_it != encoded_seq_end;
             expected_it++, encoded_seq_it++)
        {
        ...
        encoded_seq_it =
            encoded_sequence.cbegin() + (expected_values.size() / 2);
        EXPECT_EQ(*encoded_seq_it, *expected_it);
...
INSTANTIATE_TEST_SUITE_P(
    VectorCompressionTypes, CompressedVectorTest,
...
TEST_P(CompressedVectorTest, DecodeIncreasingSequenceUsingIterators)
{
    ...
    resolve_compressed_vector_type(
        *encoded_sequence_base, [&](auto &encoded_sequence)
