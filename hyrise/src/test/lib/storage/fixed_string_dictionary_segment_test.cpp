...
TEST_F(StorageFixedStringDictionarySegmentTest, CompressSegmentString)
{
    ...
    auto dict_segment =
        std::dynamic_pointer_cast<FixedStringDictionarySegment<pmr_string>>(
            segment);
...
TEST_F(StorageFixedStringDictionarySegmentTest, Decode)
{
    ...
    EXPECT_EQ(dict_segment->encoding_type(),
              EncodingType::FixedStringDictionary);
    EXPECT_EQ(dict_segment->compressed_vector_type(),
              CompressedVectorType::FixedWidthInteger1Byte);
...
{
    vs_str->append(
        "ThisIsAVeryLongStringThisIsAVeryLongStringThisIsAVeryLongString");
...
TEST_F(StorageFixedStringDictionarySegmentTest, MemoryUsageEstimation)
{
    ...
    // We have to substract 1 since the empty FixedStringSegment actually
    // contains one null terminator
    EXPECT_EQ(dictionary_segment->memory_usage(), empty_memory_usage - 1u +
                                                      3 * size_of_attribute +
                                                      size_of_dictionary);
}
