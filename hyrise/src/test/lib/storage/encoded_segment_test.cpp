...
class EncodedSegmentTest : public BaseTestWithParam<SegmentEncodingSpec>
{
  protected:
    ...
    // 0, 1, -1, _max_value, -_max_value and 20 randomly generated int32_t
    // values from the interval
    // [-_max_value, _max_value]
    const std::array<int32_t, 25> _generated_value_pool{
        0,   1,    -1,   _max_value, -_max_value, 750, -978, -350, -885,
  ...
  protected:
    ...
    size_t _row_count(EncodingType encoding_type)
    {
        ...
        case EncodingType::FrameOfReference:
            ...
            return static_cast<size_t>(
                FrameOfReferenceSegment<int32_t>::block_size * (3.3));
        default:
    ...
    std::shared_ptr<ValueSegment<int32_t>>
    _create_int_value_segment(size_t row_count)
    {
        ...
        {
            elem =
                _generated_value_pool[counter % _generated_value_pool.size()];
            ++counter;
        }
    ...
    std::shared_ptr<ValueSegment<int32_t>>
    _create_int_with_null_value_segment(size_t row_count)
    {
        auto values = pmr_vector<int32_t>(row_count);
    ...
    static std::shared_ptr<RowIDPosList>
    _create_sequential_position_filter(size_t row_count)
    {
        auto position_list = std::make_shared<RowIDPosList>();
        position_list->guarantee_single_chunk();

        for (auto offset_in_referenced_chunk = ChunkOffset{0};
             offset_in_referenced_chunk < row_count;
             ++offset_in_referenced_chunk)
        {
            if (offset_in_referenced_chunk % 2)
            {
                position_list->emplace_back(ChunkID{0},
                                            offset_in_referenced_chunk);
            }
        }
    ...
    static std::shared_ptr<RowIDPosList>
    _create_random_access_position_filter(size_t row_count)
    {
        ...
        // Let one iterator run from the beginning and one from the end of the
        // list in each other's direction. Add the iterators' elements
        // alternately to the skewed list.
    ...
    _encode_segment(const std::shared_ptr<AbstractSegment> &abstract_segment,
                    const DataType data_type)
    {
        auto segment_encoding_spec = GetParam();
        return this->_encode_segment(abstract_segment, data_type,
                    ...
                    const SegmentEncodingSpec &segment_encoding_spec)
    {
        return std::dynamic_pointer_cast<AbstractEncodedSegment>(
            ChunkEncoder::encode_segment(abstract_segment, data_type,
                                         segment_encoding_spec));
    }
};
...
TEST_P(EncodedSegmentTest, EncodeEmptyIntSegment)
{
    ...
    auto abstract_encoded_segment =
        this->_encode_segment(value_segment, DataType::Int);

    EXPECT_EQ(value_segment->size(), abstract_encoded_segment->size());

    // Trying to iterate over the empty segments should not cause any errors or
    // crashes.
    resolve_encoded_segment_type<int32_t>(
        *abstract_encoded_segment,
        [&](const auto &encoded_segment)
        {
            auto value_segment_iterable =
                create_iterable_from_segment(*value_segment);
            auto encoded_segment_iterable =
                create_iterable_from_segment(encoded_segment);

            value_segment_iterable.with_iterators(
                [&](auto value_segment_it, auto value_segment_end)
                {
                    encoded_segment_iterable.with_iterators(
                        [&](auto encoded_segment_it, auto encoded_segment_end)
                        {
            ...
            value_segment_iterable.with_iterators(
                [&](auto value_segment_it, auto value_segment_end)
                {
                    encoded_segment_iterable.with_iterators(
                        [&](auto encoded_segment_it, auto encoded_segment_end)
                        {
                            for (; encoded_segment_it != encoded_segment_end;
                                 ++encoded_segment_it, ++value_segment_it)
                            {
                                EXPECT_EQ(value_segment_it->value(),
                                          encoded_segment_it->value());
                            }
                        });
                });
        });
                    ...
                    encoded_segment_iterable.with_iterators(
                        [&](auto encoded_segment_it, auto encoded_segment_end)
                        {
                                 ...
                                 ++row_idx)
                            {
                                ...
                                {
                                    EXPECT_TRUE(variant_is_null(
                                        encoded_segment[row_idx]));
                                }
                                else
                                {
                                    EXPECT_EQ((*value_segment)[row_idx],
                                              encoded_segment[row_idx]);
                                }

                                // This covers the point access iterator
                                EXPECT_EQ(value_segment_it->is_null(),
                                          encoded_segment_it->is_null());
...
TEST_P(EncodedSegmentTest,
       SequentiallyReadNullableIntSegmentWithChunkOffsetsList)
{
    auto value_segment = _create_int_with_null_value_segment();
    auto abstract_encoded_segment =
        this->_encode_segment(value_segment, DataType::Int);
...
TEST_P(EncodedSegmentTest,
       SequentiallyReadNullableIntSegmentWithShuffledChunkOffsetsList)
{
    auto value_segment = _create_int_with_null_value_segment();
    auto abstract_encoded_segment =
        this->_encode_segment(value_segment, DataType::Int);
...
TEST_P(EncodedSegmentTest, SequentiallyReadEmptyIntSegment)
{
    ...
    // Even if no actual reading happens here, iterators are created and we can
    // test that they do not crash on empty segments
    resolve_encoded_segment_type<int32_t>(
        *abstract_encoded_segment,
        [&](const auto &encoded_segment)
        {
...
TEST_F(EncodedSegmentTest, SeqIncreasingAccessValueSegmentCounters)
{
    ...
    EXPECT_EQ(
        0, value_segment
               ->access_counter[SegmentAccessCounter::AccessType::Sequential]);
    ...
    resolve_encoded_segment_type<int32_t>(
        *abstract_encoded_segment,
        [&](const auto &encoded_segment)
        {
            ...
            EXPECT_EQ(0, encoded_segment.access_counter
                             [SegmentAccessCounter::AccessType::Sequential]);
            iterable.for_each([](const auto) {});
            EXPECT_EQ(encoded_segment.size(),
                      encoded_segment.access_counter
                          [SegmentAccessCounter::AccessType::Sequential]);
        });
}
...
TEST_P(EncodedSegmentTest, IncreasingSegmentAccessCountersWithPosList)
{
    ...
    EXPECT_EQ(
        0, value_segment
               ->access_counter[SegmentAccessCounter::AccessType::Monotonic]);
    ...
    resolve_encoded_segment_type<int32_t>(
        *abstract_encoded_segment,
        [&](const auto &encoded_segment)
        {
            auto abstract_encoded_segment_iterable =
                create_iterable_from_segment(encoded_segment);
            EXPECT_EQ(0, encoded_segment.access_counter
                             [SegmentAccessCounter::AccessType::Monotonic]);
            abstract_encoded_segment_iterable.for_each(pos_filter,
                                                       [](const auto) {});
            EXPECT_EQ(pos_filter->size(),
                      encoded_segment.access_counter
                          [SegmentAccessCounter::AccessType::Monotonic]);
        });
}
...
TEST_P(EncodedSegmentTest, RandomSegmentAccessCountersWithPosList)
{
    ...
    EXPECT_EQ(0,
              value_segment
                  ->access_counter[SegmentAccessCounter::AccessType::Random]);
    value_segment_iterable.for_each(pos_filter, [](const auto) {});
    EXPECT_EQ(pos_filter->size(),
              value_segment
                  ->access_counter[SegmentAccessCounter::AccessType::Random]);
...
TEST_P(EncodedSegmentTest, ScansIncrementCounters)
{
    const auto input_table =
        load_table("resources/test_data/tbl/int_int_int.tbl");
    ...
    const auto verify_access_counters = [&](const auto line)
    {
        ...
        for (auto column_id = ColumnID{0};
             column_id < input_table->column_count(); ++column_id)
        {
            SCOPED_TRACE(std::string{"Chunk "} + std::to_string(chunk_id) +
                         ", column " + std::to_string(column_id) + " at line " +
                         std::to_string(line));
            EXPECT_EQ(chunk->get_segment(column_id)->access_counter,
                      expected_access_counters.at(column_id));
        }
    };

    // ValueSegment::append as called by load_table increases the access
    // counters
    ...
    // For all but dictionary encoding, generate_chunk_pruning_statistics
    // iterated over the data to create the filters
    const auto is_dictionary =
        GetParam().encoding_type == EncodingType::Dictionary ||
        GetParam().encoding_type == EncodingType::FixedStringDictionary;
    if (!is_dictionary)
    {
        expected_access_counters
            [0][SegmentAccessCounter::AccessType::Sequential] += 4;
        expected_access_counters
            [1][SegmentAccessCounter::AccessType::Sequential] += 4;
        expected_access_counters
            [2][SegmentAccessCounter::AccessType::Sequential] += 4;
    }
    verify_access_counters(__LINE__);
    ...
    // Perform a scan
    auto scan_1 = create_table_scan(table_wrapper, ColumnID{2},
                                    PredicateCondition::NotEquals, 10);
    scan_1->execute();

    // Expect the segment for column c to have been read sequentially
    expected_access_counters[2][SegmentAccessCounter::AccessType::Sequential] +=
        4;
    // For dictionary encoding, the dictionaries are accessed, too. The exact
    // number of accesses depends on the number of unique values and the
    // implementation of the binary search.
    if (is_dictionary)
    {
        expected_access_counters
            [2][SegmentAccessCounter::AccessType::Dictionary] += 4;
    }
    verify_access_counters(__LINE__);

    // Perform a scan
    auto scan_2 = create_table_scan(scan_1, ColumnID{0},
                                    PredicateCondition::GreaterThan, 10);
    scan_2->execute();

    // Expect the segment for column a to have been read sequentially with a gap
    // caused by the first scan
    expected_access_counters[0][SegmentAccessCounter::AccessType::Monotonic] +=
        3;
    if (is_dictionary)
    {
        expected_access_counters
            [0][SegmentAccessCounter::AccessType::Dictionary] += 2;
    }
    verify_access_counters(__LINE__);
    ...
    // Perform a scan
    auto scan_3 =
        create_table_scan(scan_2, ColumnID{0}, PredicateCondition::Equals, 11);
    scan_3->execute();
    ...
    {
        expected_access_counters
            [0][SegmentAccessCounter::AccessType::Dictionary] += 4;
    }
    verify_access_counters(__LINE__);
...
TEST_F(EncodedSegmentTest, SegmentReencoding)
{
    ...
    auto value_segment = _create_int_with_null_value_segment(
        _row_count(EncodingType::FrameOfReference));

    // Dictionary
    auto encoded_segment = this->_encode_segment(
    ...
    // LZ4 (only testing bitpacking compression as fixed-width integer is not
    // supported by LZ4)
    encoded_segment = this->_encode_segment(
        value_segment, DataType::Int,
...
// Testing the internal data structures of Run Length-encoded segments for
...
TEST_F(EncodedSegmentTest, RunLengthEncodingMonotonicallyIncreasing)
{
    ...
    const auto value_segment =
        std::make_shared<ValueSegment<int32_t>>(std::move(values));
    const auto encoded_segment =
        this->_encode_segment(value_segment, DataType::Int,
                              SegmentEncodingSpec{EncodingType::RunLength});

    const auto run_length_segment =
        std::dynamic_pointer_cast<const RunLengthSegment<int32_t>>(
            encoded_segment);
...
TEST_F(EncodedSegmentTest, RunLengthEncodingVaryingRuns)
{
    ...
    constexpr auto single_value_runs_increasing =
        row_count - single_value_runs_decreasing -
        long_value_runs * long_value_element_repititions;
    auto values = pmr_vector<int32_t>(row_count);

    // fill first ten values with decreasing values
    for (auto row_id = int32_t{0u}; row_id < single_value_runs_decreasing;
         ++row_id)
    {
    ...
    {
        for (auto repetition = 0; repetition < long_value_element_repititions;
             ++repetition)
        {
            values[row_id] = value;
            ++row_id;
    ...
    EXPECT_EQ(run_length_segment->values()->size(),
              run_length_segment->end_positions()->size());
    EXPECT_EQ(run_length_segment->values()->size(),
              run_length_segment->null_values()->size());
    EXPECT_EQ(run_length_segment->values()->size(),
              single_value_runs_decreasing + long_value_runs +
                  single_value_runs_increasing);

    EXPECT_EQ(run_length_segment->end_positions()->front(), 0ul);
    EXPECT_EQ(run_length_segment->end_positions()->back(), 99ul);

    // first longer run has the value 10, starts at position 10, and ends at
    // position 19
...
// Testing the internal data structures of Run Length-encoded segments for runs
// and NULL values where NULL values are fully covering single- and
// multi-element value runs.
TEST_F(EncodedSegmentTest, RunLengthEncodingNullValues)
{
    ...
    {
        for (auto repetition = 0; repetition < elements_per_long_run;
             ++repetition)
        {
            values[row_id] = value;
    ...
    const auto value_segment = std::make_shared<ValueSegment<int32_t>>(
    ...
    // The handling of NULL values is unintuitive. When two runs of values are
    // both NULL, they are merged into a single run, only keeping the first
    // value of both runs in values() as a placeholder for the merged NULL run.
    // That means the runs of values 3/4 and the single-value runs of 95/96 are
    // stored as two value runs 3 and 95 in the values() vector (thus, we
    // substract two from the number of expected runs).
    EXPECT_EQ(run_length_segment->values()->size(),
              run_length_segment->end_positions()->size());
    EXPECT_EQ(run_length_segment->values()->size(),
              run_length_segment->null_values()->size());
    ...
    EXPECT_EQ(run_length_segment->values()->at(13),
              95); // 9 + 5 - 1 (succeeding run of 4 removed from values())
    EXPECT_EQ(run_length_segment->values()->at(14),
              97); // no value 96 in values()

    // Check that successive NULL runs are merged to single position
    EXPECT_FALSE(run_length_segment->null_values()->at(2));
    EXPECT_TRUE(run_length_segment->null_values()->at(
        3)); // NULLs of values 3/4 are merged into single run
...
// Testing the internal data structures of Run Length-encoded segments for runs
// and NULL values where NULL values are either at the very first or last
// position, or a NULL value run spans two value runs.
TEST_F(EncodedSegmentTest, RunLengthEncodingNullValuesInRun)
{
    ...
    // Adding four NULL value runs (three single element runs, one two element
    // run)
    ...
    // NULL value runs are runs as well. We have four NULL runs and two value
    // runs, where one run is split in two due to a NULL value in between.
    EXPECT_EQ(run_length_segment->values()->size(),
              run_length_segment->end_positions()->size());
    EXPECT_EQ(run_length_segment->values()->size(),
              run_length_segment->null_values()->size());
    ...
    EXPECT_EQ(run_length_segment->end_positions()->front(),
              0); // value run longer, but first position is NULL
    ...
    EXPECT_EQ(run_length_segment->values()->at(1),
              run_length_segment->values()->at(2));

    // NULL value run spans two elements of different value runs (last value of
    // first run, first of second run). Hence, second value run starts at
    // position 11 instead of 10 due to NULL value.
    EXPECT_EQ(run_length_segment->end_positions()->at(3), 8);
    EXPECT_EQ(run_length_segment->end_positions()->at(4), 10);
}

// Testing the internal data structures of Frame of Reference-encoded segments.
// In particular, the determination of the reference value (i.e., the block
// minimum) as well as the difference to the reference value are checked. This
// test does not test the creation of multiple frames as the required vectors
// are too large for unit testing.
TEST_F(EncodedSegmentTest, FrameOfReference)
{
    ...
    const auto value_segment = std::make_shared<ValueSegment<int32_t>>(
        std::move(values), std::move(null_values));
    const auto encoded_segment = this->_encode_segment(
        value_segment, DataType::Int,
        SegmentEncodingSpec{EncodingType::FrameOfReference});

    const auto for_segment =
        std::dynamic_pointer_cast<const FrameOfReferenceSegment<int32_t>>(
            encoded_segment);
    ...
    resolve_compressed_vector_type(
        ...
        [&](const auto &offset_values)
        {
            ...
            for (; offset_iter != offset_values.cend();
                 ++offset_iter, ++values_iter, ++null_values_iter)
            {
                if (!*null_values_iter)
                {
    ...
    // Check that NULLs are not stored for FoR segment that does not contain any
    // NULLs
    const auto value_segment_no_nulls =
        std::make_shared<ValueSegment<int32_t>>(std::move(values_copy));
    const auto encoded_segment_no_nulls = this->_encode_segment(
        value_segment_no_nulls, DataType::Int,
        SegmentEncodingSpec{EncodingType::FrameOfReference});

    const auto for_segment_no_nulls =
        std::dynamic_pointer_cast<const FrameOfReferenceSegment<int32_t>>(
            encoded_segment_no_nulls);
