...
class ChunkEncoderTest : public BaseTest
{
  public:
    ...
    create_test_table(const size_t row_count,
                      const ChunkOffset target_chunk_size,
                      const ColumnCount column_count)
    {
        TableColumnDefinitions column_definitions;

        for (auto column_id = ColumnCount{0}; column_id < column_count;
             ++column_id)
        {
        ...
        auto table = std::make_shared<Table>(
            column_definitions, TableType::Data, target_chunk_size);

        for (auto row_id = 0u; row_id < row_count; ++row_id)
        {
            const auto row = std::vector<AllTypeVariant>(
                column_count, AllTypeVariant{static_cast<int32_t>(row_id)});
            table->append(row);
        }
...
TEST_F(ChunkEncoderTest, EncodeSingleChunk)
{
    const auto chunk_encoding_spec =
        ChunkEncodingSpec{SegmentEncodingSpec{EncodingType::Dictionary},
...
TEST_F(ChunkEncoderTest, ThrowOnEncodingAMutableChunk)
{
    ...
    EXPECT_THROW(ChunkEncoder::encode_chunk(chunk, types, chunk_encoding_spec),
                 std::logic_error);
}
...
TEST_F(ChunkEncoderTest, ThrowOnEncodingReferenceSegments)
{
    ...
    auto table_scan =
        std::make_shared<TableScan>(table_wrapper, greater_than_equals_(a, 0));
    table_scan->execute();
    ...
    const auto chunk_encoding_spec =
        ChunkEncodingSpec{SegmentEncodingSpec{EncodingType::Dictionary},
                          SegmentEncodingSpec{EncodingType::Dictionary},
                          SegmentEncodingSpec{EncodingType::Dictionary}};
    auto chunk = std::const_pointer_cast<Chunk>(
        table_scan->get_output()->get_chunk(ChunkID{0u}));
    const auto column_data_types = _table->column_data_types();
    EXPECT_THROW(ChunkEncoder::encode_chunk(chunk, column_data_types,
                                            chunk_encoding_spec),
                 std::logic_error);
}
...
TEST_F(ChunkEncoderTest, EncodeWholeTable)
{
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < _table->chunk_count();
         ++chunk_id)
    {
...
{
    const auto segment_encoding_spec =
        SegmentEncodingSpec{EncodingType::Dictionary};
    const auto chunk_encoding_spec =
        ChunkEncodingSpec{3u, segment_encoding_spec};

    _table->last_chunk()->finalize();
    ChunkEncoder::encode_all_chunks(_table, segment_encoding_spec);

    for (auto chunk_id = ChunkID{0u}; chunk_id < _table->chunk_count();
         ++chunk_id)
    {
        const auto chunk = _table->get_chunk(chunk_id);
        assert_chunk_encoding(chunk, chunk_encoding_spec);
...
TEST_F(ChunkEncoderTest, EncodeMultipleChunksUsingSameEncoding)
{
    ...
    const auto unencoded_chunk_spec =
        ChunkEncodingSpec{3u, SegmentEncodingSpec{EncodingType::Unencoded}};
...
{
    // Encoding specifications which will be applied one after another to the
    // chunk.
    const auto chunk_encoding_specs = std::vector<ChunkEncodingSpec>{
        {SegmentEncodingSpec{EncodingType::Unencoded},
         SegmentEncodingSpec{EncodingType::RunLength},
    ...
    for (auto const &chunk_encoding_spec : chunk_encoding_specs)
    {
        ...
        {
            assert_chunk_encoding(_table->get_chunk(chunk_id),
                                  chunk_encoding_spec);
        }
    }
}
...
TEST_F(ChunkEncoderTest, ReencodeNotNullableSegment)
{
    ...
    auto reencoded_segment = ChunkEncoder::encode_segment(
        dict_segment, DataType::Int,
        SegmentEncodingSpec{EncodingType::Unencoded});
    const auto casted_reencoded_segment =
        std::dynamic_pointer_cast<const ValueSegment<int>>(reencoded_segment);
