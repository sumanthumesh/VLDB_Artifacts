...
TEST_F(TableOrderConstraintTest, OrderedColumnIDs)
{
    ...
    const auto order_constraint = TableOrderConstraint(
        {{ColumnID{2}, ColumnID{1}}, {{ColumnID{3}, ColumnID{4}}}});
...
{
    _table->add_soft_order_constraint(
        {{ColumnID{0}, ColumnID{1}}, {ColumnID{2}, ColumnID{3}}});

    // Invalid because the column id is out of range.
    EXPECT_THROW(
        _table->add_soft_order_constraint({{ColumnID{5}}, {ColumnID{1}}}),
        std::logic_error);

    // Invalid because the ordered column id is out of range.
    EXPECT_THROW(
        _table->add_soft_order_constraint({{ColumnID{1}}, {ColumnID{5}}}),
        std::logic_error);

    // Invalid because the same order constraint already exists.
    EXPECT_THROW(_table->add_soft_order_constraint(
                     {{ColumnID{0}, ColumnID{1}}, {ColumnID{2}, ColumnID{3}}}),
                 std::logic_error);

    // Invalid because the order constraint with same ordered columns, but more
    // required columns already exists.
    EXPECT_THROW(_table->add_soft_order_constraint(
                     {{ColumnID{0}}, {ColumnID{2}, ColumnID{3}}}),
                 std::logic_error);

    // Invalid because the order constraint with same ordered columns, but fewer
    // required columns already exists.
    EXPECT_THROW(_table->add_soft_order_constraint(
                     {{ColumnID{0}, ColumnID{1}, ColumnID{4}},
                      {ColumnID{2}, ColumnID{3}}}),
                 std::logic_error);

    // Invalid because the order constraint with same required columns, but more
    // ordered columns already exists.
    EXPECT_THROW(_table->add_soft_order_constraint(
                     {{ColumnID{0}, ColumnID{1}}, {ColumnID{2}}}),
                 std::logic_error);

    // Invalid because the order constraint with same required columns, but
    // fewer ordered columns already exists.
    EXPECT_THROW(_table->add_soft_order_constraint(
    ...
    if constexpr (HYRISE_DEBUG)
    {
        ...
        EXPECT_THROW(TableOrderConstraint({ColumnID{0}, ColumnID{1}},
...
{
    const auto order_constraint_a =
        TableOrderConstraint{{ColumnID{0}, ColumnID{2}}, {ColumnID{1}}};
    const auto order_constraint_a_copy =
        TableOrderConstraint{{ColumnID{0}, ColumnID{2}}, {ColumnID{1}}};
    const auto order_constraint_a_reordered =
        TableOrderConstraint{{ColumnID{2}, ColumnID{0}}, {ColumnID{1}}};

    const auto order_constraint_b =
        TableOrderConstraint{{ColumnID{0}, ColumnID{3}}, {ColumnID{1}}};
    const auto order_constraint_c =
        TableOrderConstraint{{ColumnID{0}}, {ColumnID{1}}};
    const auto order_constraint_d =
        TableOrderConstraint{{ColumnID{0}, ColumnID{2}}, {ColumnID{3}}};
...
TEST_F(TableOrderConstraintTest, Hash)
{
    ...
    EXPECT_FALSE(order_constraint_a.hash() ==
                 order_constraint_a_reordered.hash());
    EXPECT_FALSE(order_constraint_a_reordered.hash() ==
                 order_constraint_a.hash());
