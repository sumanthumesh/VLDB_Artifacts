...
TEST_F(TableKeyConstraintTest, DuplicateColumnIDs)
{
    ...
    const auto key_constraint = TableKeyConstraint(
        {{ColumnID{1}, ColumnID{1}}, KeyConstraintType::UNIQUE});
...
TEST_F(TableKeyConstraintTest, AddKeyConstraints)
{
    ...
    const auto key_constraint_2 = TableKeyConstraint{{ColumnID{1}, ColumnID{2}},
...
{
    _table_nullable->add_soft_key_constraint(
    ...
    EXPECT_THROW(_table->add_soft_key_constraint(
                     {{ColumnID{5}}, KeyConstraintType::UNIQUE}),
                 std::logic_error);

    // Invalid because the column must be non nullable for a PRIMARY KEY.
    EXPECT_THROW(_table_nullable->add_soft_key_constraint(
                     {{ColumnID{1}}, KeyConstraintType::PRIMARY_KEY}),
                 std::logic_error);
    ...
    _table->add_soft_key_constraint(
        {{ColumnID{2}}, KeyConstraintType::PRIMARY_KEY});
...
{
    const auto key_constraint_a = TableKeyConstraint{{ColumnID{0}, ColumnID{2}},
                                                     KeyConstraintType::UNIQUE};
    const auto key_constraint_a_reordered = TableKeyConstraint{
        {ColumnID{2}, ColumnID{0}}, KeyConstraintType::UNIQUE};
    const auto primary_key_constraint_a = TableKeyConstraint{
        {ColumnID{0}, ColumnID{2}}, KeyConstraintType::PRIMARY_KEY};

    const auto key_constraint_b = TableKeyConstraint{{ColumnID{2}, ColumnID{3}},
                                                     KeyConstraintType::UNIQUE};
...
{
    const auto key_constraint_1 = TableKeyConstraint{{ColumnID{0}, ColumnID{2}},
                                                     KeyConstraintType::UNIQUE};
    const auto key_constraint_2 = TableKeyConstraint{{ColumnID{2}, ColumnID{3}},
                                                     KeyConstraintType::UNIQUE};
    ...
    EXPECT_EQ(*std::next(key_constraints_a.begin()),
              *std::next(key_constraints_b.begin()));
}
