...
class SegmentAccessCounterTest : public BaseTest
{
  protected:
    ...
    _access_pattern(const std::shared_ptr<const RowIDPosList> &positions)
    {
...
TEST_F(SegmentAccessCounterTest, CopyConstructor)
{
    ...
    for (auto access_type = 0ul;
         access_type < static_cast<size_t>(AccessType::Count); ++access_type)
    {
        EXPECT_EQ(counter1[static_cast<AccessType>(access_type)],
                  counter2[static_cast<AccessType>(access_type)]);
    }
}
...
TEST_F(SegmentAccessCounterTest, AccessPattern1)
{
    ...
    EXPECT_EQ(_access_pattern(positions),
              AccessPattern::SequentiallyIncreasing);
    positions->push_back({ChunkID{0}, ChunkOffset{3}});
    EXPECT_EQ(_access_pattern(positions),
              AccessPattern::MonotonicallyIncreasing);
    positions->push_back({ChunkID{0}, ChunkOffset{3}});
    EXPECT_EQ(_access_pattern(positions),
              AccessPattern::MonotonicallyIncreasing);
    positions->push_back({ChunkID{0}, ChunkOffset{1}});
...
TEST_F(SegmentAccessCounterTest, AccessPattern3)
{
    ...
    EXPECT_EQ(_access_pattern(positions),
              AccessPattern::SequentiallyDecreasing);
    positions->push_back({ChunkID{0}, ChunkOffset{665}});
    EXPECT_EQ(_access_pattern(positions),
              AccessPattern::SequentiallyDecreasing);
    positions->push_back({ChunkID{0}, ChunkOffset{660}});
    EXPECT_EQ(_access_pattern(positions),
              AccessPattern::MonotonicallyDecreasing);
