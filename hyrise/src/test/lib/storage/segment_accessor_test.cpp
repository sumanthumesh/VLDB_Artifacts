...
class SegmentAccessorTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        dict_seg_int = ChunkEncoder::encode_segment(
            val_seg_int, DataType::Int,
            SegmentEncodingSpec{EncodingType::Dictionary});
        dict_seg_str = ChunkEncoder::encode_segment(
            val_seg_str, DataType::String,
            SegmentEncodingSpec{EncodingType::Dictionary});

        tbl = std::make_shared<Table>(
            TableColumnDefinitions{
                TableColumnDefinition{"val_seg_int", DataType::Int, false},
        ...
        ref_seg_int =
            std::make_shared<ReferenceSegment>(tbl, ColumnID{0}, pos_list);
        ref_seg_str =
            std::make_shared<ReferenceSegment>(tbl, ColumnID{1}, pos_list);
    }
...
{
    auto val_seg_str_accessor =
        create_segment_accessor<pmr_string>(val_seg_str);
...
{
    auto dict_seg_str_accessor =
        create_segment_accessor<pmr_string>(dict_seg_str);
...
{
    auto ref_seg_str_accessor =
        create_segment_accessor<pmr_string>(ref_seg_str);
...
TEST_F(SegmentAccessorTest, TestSegmentAccessCounterIncrementing)
{
    ...
    // Create segment accessor in a new scope to ensure its destructor, which
    // writes the access counters, is called.
    {
        auto val_seg_int_base_accessor =
            create_segment_accessor<int>(val_seg_int);
