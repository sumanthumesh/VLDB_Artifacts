...
class StorageChunkTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        ds_int = ChunkEncoder::encode_segment(
            vs_int, DataType::Int,
            SegmentEncodingSpec{EncodingType::Dictionary});
        ds_str = ChunkEncoder::encode_segment(
            vs_str, DataType::String,
            SegmentEncodingSpec{EncodingType::Dictionary});
...
TEST_F(StorageChunkTest, AddIndexByColumnID)
{
    ...
    auto index_int =
        chunk->create_index<GroupKeyIndex>(std::vector<ColumnID>{ColumnID{0}});
    auto index_str =
        chunk->create_index<GroupKeyIndex>(std::vector<ColumnID>{ColumnID{0}});
    auto index_int_str = chunk->create_index<CompositeGroupKeyIndex>(
...
{
    chunk = std::make_shared<Chunk>(Segments({ds_int, ds_str}));
    auto index_int = chunk->create_index<GroupKeyIndex>(
        std::vector<std::shared_ptr<const AbstractSegment>>{ds_int});
    auto index_str = chunk->create_index<GroupKeyIndex>(
...
TEST_F(StorageChunkTest, GetIndexByColumnID)
{
    ...
    EXPECT_EQ(chunk->get_index(ChunkIndexType::GroupKey,
...
TEST_F(StorageChunkTest, GetIndexBySegmentPointer)
{
    ...
    EXPECT_EQ(chunk->get_index(
                  ChunkIndexType::GroupKey,
                  std::vector<std::shared_ptr<const AbstractSegment>>{ds_int}),
              index_int);
    EXPECT_EQ(chunk->get_index(
              ...
              index_int_str);
    EXPECT_EQ(
        chunk->get_index(ChunkIndexType::CompositeGroupKey,
...
TEST_F(StorageChunkTest, GetIndexesByColumnIDs)
{
    ...
    auto indexes_for_segment_0 =
        chunk->get_indexes(std::vector<ColumnID>{ColumnID{0}});
    // Make sure it finds both the single-column index as well as the
    // multi-column index
    EXPECT_NE(std::find(indexes_for_segment_0.cbegin(),
                        indexes_for_segment_0.cend(), index_int),
              indexes_for_segment_0.cend());
    EXPECT_NE(std::find(indexes_for_segment_0.cbegin(),
                        indexes_for_segment_0.cend(), index_int_str),
              indexes_for_segment_0.cend());

    auto indexes_for_segment_1 =
        chunk->get_indexes(std::vector<ColumnID>{ColumnID{1}});
    // Make sure it only finds the single-column index
    EXPECT_NE(std::find(indexes_for_segment_1.cbegin(),
                        indexes_for_segment_1.cend(), index_str),
              indexes_for_segment_1.cend());
    EXPECT_EQ(std::find(indexes_for_segment_1.cbegin(),
                        indexes_for_segment_1.cend(), index_int_str),
              indexes_for_segment_1.cend());
}
...
TEST_F(StorageChunkTest, GetIndexesBySegmentPointers)
{
    ...
    auto indexes_for_segment_0 = chunk->get_indexes(
        std::vector<std::shared_ptr<const AbstractSegment>>{ds_int});
    // Make sure it finds both the single-column index as well as the
    // multi-column index
    EXPECT_NE(std::find(indexes_for_segment_0.cbegin(),
    ...
    auto indexes_for_segment_1 = chunk->get_indexes(
        std::vector<std::shared_ptr<const AbstractSegment>>{ds_str});
    // Make sure it only finds the single-column index
    EXPECT_NE(std::find(indexes_for_segment_1.cbegin(),
                        indexes_for_segment_1.cend(), index_str),
...
TEST_F(StorageChunkTest, RemoveIndex)
{
    ...
    auto indexes_for_segment_0 =
        chunk->get_indexes(std::vector<ColumnID>{ColumnID{0}});
    EXPECT_EQ(std::find(indexes_for_segment_0.cbegin(),
                        indexes_for_segment_0.cend(), index_int),
              indexes_for_segment_0.cend());
    EXPECT_NE(std::find(indexes_for_segment_0.cbegin(),
                        indexes_for_segment_0.cend(), index_int_str),
    ...
    auto indexes_for_segment_1 =
        chunk->get_indexes(std::vector<ColumnID>{ColumnID{1}});
    EXPECT_EQ(std::find(indexes_for_segment_0.cbegin(),
                        indexes_for_segment_0.cend(), index_str),
              indexes_for_segment_0.cend());
}
...
TEST_F(StorageChunkTest, SetSortedInformationVector)
{
    ...
    const auto sorted_by_vector =
        std::vector{SortColumnDefinition(ColumnID{0}, SortMode::Ascending),
    ...
    EXPECT_THROW(chunk->set_individually_sorted_by(sorted_by_vector),
                 std::logic_error);
}
...
TEST_F(StorageChunkTest, SetSortedInformationAscendingWithNulls)
{
    ...
    EXPECT_NO_THROW(chunk_with_nulls->set_individually_sorted_by(
        SortColumnDefinition(ColumnID{0}, SortMode::Ascending)));
    EXPECT_THROW(chunk_with_nulls->set_individually_sorted_by(
...
TEST_F(StorageChunkTest, SetSortedInformationUnsortedNULLs)
{
    ...
    auto value_segment = std::make_shared<ValueSegment<int32_t>>(
        pmr_vector<int32_t>{1, 1, 1}, pmr_vector<bool>{false, true, false});
    auto chunk_with_nulls = std::make_shared<Chunk>(Segments{value_segment});
    chunk_with_nulls->finalize();
    EXPECT_TRUE(chunk_with_nulls->individually_sorted_by().empty());

    // Sorted values, but NULLs always come first in Hyrise when vector is
    // sorted.
    EXPECT_THROW(chunk_with_nulls->set_individually_sorted_by(
                     SortColumnDefinition(ColumnID{0}, SortMode::Ascending)),
                 std::logic_error);
...
TEST_F(StorageChunkTest, SetSortedInformationNULLsLast)
{
    ...
    auto value_segment = std::make_shared<ValueSegment<int32_t>>(
        pmr_vector<int32_t>{1, 1, 1}, pmr_vector<bool>{false, false, true});
    auto chunk_with_nulls = std::make_shared<Chunk>(Segments{value_segment});
    chunk_with_nulls->finalize();
    EXPECT_TRUE(chunk_with_nulls->individually_sorted_by().empty());
