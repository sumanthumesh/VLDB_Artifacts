...
class StorageTableTest : public BaseTest
{
  protected:
    ...
    static tbb::concurrent_vector<std::shared_ptr<Chunk>,
                                  ZeroAllocator<std::shared_ptr<Chunk>>> &
    get_chunks(std::shared_ptr<Table> &table)
    {
...
TEST_F(StorageTableTest, ColumnCount) { EXPECT_EQ(table->column_count(), 2); }
...
{
    TableColumnDefinitions column_definitions_nullable{
        {"a", DataType::Int, true}, {"b", DataType::String, true}};
    table = std::make_shared<Table>(column_definitions_nullable,
...
TEST_F(StorageTableTest, ChunkSizeZeroThrows)
{
    ...
    TableColumnDefinitions column_definitions{};
    EXPECT_THROW(Table(column_definitions, TableType::Data, ChunkOffset{0}),
                 std::logic_error);
}
...
{
    /**
     * WARNING: Since it's hard to assert what constitutes a correct
     * "estimation", this just tests basic sanity of the memory usage
     * estimations
     */

    auto mvcc_table = std::make_shared<Table>(column_definitions,
                                              TableType::Data, ChunkOffset{2});

    const auto empty_memory_usage =
        mvcc_table->memory_usage(MemoryUsageCalculationMode::Sampled);

    mvcc_table->append({4, "Hello"});
    mvcc_table->append({5, "Hello"});

    EXPECT_GT(mvcc_table->memory_usage(MemoryUsageCalculationMode::Sampled),
              empty_memory_usage + 2 * (sizeof(int) + sizeof(pmr_string)) +
                  sizeof(TransactionID) + 2 * sizeof(CommitID));
}

TEST_F(StorageTableTest, StableChunks)
{
    // Tests that pointers to a chunk remain valid even if the table grows
    // (#1463)
    ...
    EXPECT_EQ((*(*first_chunk)->get_segment(ColumnID{0}))[ChunkOffset{0}],
              AllTypeVariant{100});
}
...
TEST_F(StorageTableTest, LastChunkOfReferenceTable)
{
    ...
    const auto segment_int =
        std::make_shared<ReferenceSegment>(table, ColumnID{0}, posList);
    const auto segment_string =
        std::make_shared<ReferenceSegment>(table, ColumnID{1}, posList);
...
TEST_F(StorageTableTest, CreatePartialHashIndex)
{
    ...
    auto access_range_equals_with_iterators_hello =
    ...
    created_hash_index->range_equals_with_iterators(
        access_range_equals_with_iterators_hello, hello_string);

    auto access_range_equals_with_iterators_world =
    ...
    created_hash_index->range_equals_with_iterators(
        access_range_equals_with_iterators_world, world_string);
}

TEST_F(StorageTableTest, CreatePartialHashIndexOnEmptyChunks)
{
    EXPECT_THROW(table->create_partial_hash_index(ColumnID{0}, {}),
                 std::logic_error);
}
