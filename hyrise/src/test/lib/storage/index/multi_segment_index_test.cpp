...
template <typename DerivedIndex> class MultiSegmentIndexTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        dict_segment_int = create_dict_segment_by_type<int>(
            DataType::Int, {3, 4, 0, 4, 2, 7, 8, 4, 1, 9});
        dict_segment_str = create_dict_segment_by_type<pmr_string>(
            DataType::String, {"foo", "bar", "baz", "foo", "bar", "baz", "foo",
                               "bar", "baz", "foo"});

        index_int_str = std::make_shared<DerivedIndex>(
            std::vector<std::shared_ptr<const AbstractSegment>>{
                dict_segment_int, dict_segment_str});
        index_str_int = std::make_shared<DerivedIndex>(
            std::vector<std::shared_ptr<const AbstractSegment>>{
                dict_segment_str, dict_segment_int});
    }

    template <class Iterator>
    static std::vector<std::vector<AllTypeVariant>> result_as_vector(
        const std::vector<std::shared_ptr<AbstractSegment>> segments,
...
TYPED_TEST_SUITE(MultiSegmentIndexTest,
                 MultiSegmentIndexTypes, ); // NOLINT(whitespace/parens)
...
TYPED_TEST(MultiSegmentIndexTest, FullRange)
{
    ...
    auto result_values_int_str =
        this->result_as_vector({this->dict_segment_int, this->dict_segment_str},
                               begin_int_str, end_int_str);
    auto expected_values_int_str = std::vector<std::vector<AllTypeVariant>>{
        {0, "baz"}, {1, "baz"}, {2, "bar"}, {3, "foo"}, {4, "bar"},
    ...
    auto result_values_str_int =
        this->result_as_vector({this->dict_segment_str, this->dict_segment_int},
                               begin_str_int, end_str_int);
    auto expected_values_str_int = std::vector<std::vector<AllTypeVariant>>{
        {"bar", 2}, {"bar", 4}, {"bar", 4}, {"baz", 0}, {"baz", 1},
...
TYPED_TEST(MultiSegmentIndexTest, TooManyReferenceValues)
{
    ...
    EXPECT_THROW(this->index_int_str->lower_bound({1, "baz", 3.0f}),
                 std::logic_error);
    EXPECT_THROW(this->index_int_str->upper_bound({1, "baz", 3.0f}),
                 std::logic_error);
}
...
TYPED_TEST(MultiSegmentIndexTest, IsIndexForTest)
{
    ...
    EXPECT_TRUE(this->index_int_str->is_index_for(
        {this->dict_segment_int, this->dict_segment_str}));
    EXPECT_TRUE(this->index_str_int->is_index_for({this->dict_segment_str}));
    EXPECT_TRUE(this->index_str_int->is_index_for(
        {this->dict_segment_str, this->dict_segment_int}));

    EXPECT_FALSE(this->index_int_str->is_index_for(
        {this->dict_segment_str, this->dict_segment_int}));
...
TYPED_TEST(MultiSegmentIndexTest, CreateAndRetrieveUsingChunk)
{
    auto chunk = std::make_shared<Chunk>(
        Segments({this->dict_segment_int, this->dict_segment_str}));

    chunk->create_index<TypeParam>({this->dict_segment_int});
    chunk->create_index<TypeParam>(
        {this->dict_segment_int, this->dict_segment_str});

    auto indexes_int = chunk->get_indexes({this->dict_segment_int});
    auto indexes_int_str =
        chunk->get_indexes({this->dict_segment_int, this->dict_segment_str});
    ...
    EXPECT_TRUE(indexes_int_str[0]->is_index_for(
        {this->dict_segment_int, this->dict_segment_str}));
}
