...
class AdaptiveRadixTreeIndexTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        // we want to custom-build the index, but we have to create an index
        // with a non-empty segment. Therefore we build an index and reset the
        // root.
        _dict_segment1 =
            create_dict_segment_by_type<pmr_string>(DataType::String, {"test"});
        _index1 = std::make_shared<AdaptiveRadixTreeIndex>(
        ...
        _keys1 = {ValueID{0x01010101u}, ValueID{0x01010102u},
                  ValueID{0x01010201u}, ValueID{0x01020101u},
                  ValueID{0x01020102u}, ValueID{0x02010101u},
                  ValueID{0x01010101u}};
        _values1 = {ChunkOffset{0x00000001}, ChunkOffset{0x00000002},
                    ChunkOffset{0x00000003}, ChunkOffset{0x00000004},
                    ChunkOffset{0x00000005}, ChunkOffset{0x00000006},
                    ChunkOffset{0x00000007}};
    ...
    {
        auto segment =
            create_dict_segment_by_type<int32_t>(DataType::Int, values);
        auto index = std::make_shared<AdaptiveRadixTreeIndex>(
            std::vector<std::shared_ptr<const AbstractSegment>>({segment}));

        std::set<std::optional<int32_t>> distinct_values(values.begin(),
                                                         values.end());

        std::set<std::optional<int32_t>> search_values = distinct_values;
        // Add further values to search_values until the size is twice the size
        // of distinct_values.
        for (auto counter = 0u; counter < distinct_values.size(); ++counter)
        {
            search_values.insert(
                _unindexed_value_pool[counter % _unindexed_value_pool.size()]);
        }
        ...
        for (const auto &search_value : search_values)
        {
            ...
            {
                // match
                EXPECT_NE(index->lower_bound({*search_value}),
                          index->upper_bound({*search_value}));
            }
            else
            {
                // no match
                EXPECT_EQ(index->upper_bound({*search_value}),
                          index->lower_bound({*search_value}));
            }
    ...
    // 0, 1, -1, the maximum int32_t value, the minimum int32_t value and 20
    // randomly generated int32_t values
    const std::array<int32_t, 25> _indexed_value_pool{
        0,           1,           -1,         2147483647, -2147483648,
        -354718155,  -450451703,  -125078514, 1173534911, -1426766195,
        -1395715196, -1974007197, 1217037717, -369295213, 912804945,
        -650700817,  -987310781,  2019494295, -142016072, -547126853,
        -1338076362, -474901211,  1039229181, 1059574590, -775950157};

    // 20 randomly generated int32_t values, the intersection of
    // _indexed_value_pool and _unindexed_value_pool is empty
    const std::array<int32_t, 20> _unindexed_value_pool{
        24841977,    40823842,    -838897495,  717133222,   -2003779882,
    ...
    std::vector<
        std::pair<AdaptiveRadixTreeIndex::BinaryComparable, ChunkOffset>>
        _pairs;
...
TEST_F(AdaptiveRadixTreeIndexTest, BinaryComparableFromChunkOffset)
{
    ...
    AdaptiveRadixTreeIndex::BinaryComparable binary_comparable =
        AdaptiveRadixTreeIndex::BinaryComparable(test_value);
...
TEST_F(AdaptiveRadixTreeIndexTest, BulkInsert)
{
    std::vector<ChunkOffset> expected_chunk_offsets = {
        ChunkOffset{0x00000001}, ChunkOffset{0x00000007},
        ChunkOffset{0x00000002}, ChunkOffset{0x00000003},
        ChunkOffset{0x00000004}, ChunkOffset{0x00000005},
        ChunkOffset{0x00000006}};
    ...
    auto child010101 =
        std::dynamic_pointer_cast<ARTNode4>(child0101->_children[0]);
    EXPECT_EQ(child0101->_partial_keys[0], static_cast<uint8_t>(0x01u));
    EXPECT_EQ(child0101->_partial_keys[1], static_cast<uint8_t>(0x02u));
    EXPECT_EQ(child0101->_partial_keys[2], static_cast<uint8_t>(0xffu));
    EXPECT_EQ(child0101->_partial_keys[3], static_cast<uint8_t>(0xffu));
    ...
    auto leaf01010101 =
        std::dynamic_pointer_cast<Leaf>(child010101->_children[0]);
    ...
    EXPECT_FALSE(std::find(leaf01010101->begin(), leaf01010101->end(),
                           static_cast<uint8_t>(0x00000001u)) ==
                 leaf01010101->end());
    EXPECT_FALSE(std::find(leaf01010101->begin(), leaf01010101->end(),
                           static_cast<uint8_t>(0x00000007u)) ==
                 leaf01010101->end());

    auto leaf01010102 =
        std::dynamic_pointer_cast<Leaf>(child010101->_children[1]);
    ...
    EXPECT_FALSE(std::find(leaf01010102->begin(), leaf01010102->end(),
                           static_cast<uint8_t>(0x00000002u)) ==
                 leaf01010102->end());
    ...
    EXPECT_FALSE(std::find(leaf02->begin(), leaf02->end(),
                           static_cast<uint8_t>(0x00000006u)) == leaf02->end());
}
...
TEST_F(AdaptiveRadixTreeIndexTest, VectorOfInts)
{
    ...
    // Let one iterator run from the beginning and one from the end of the list
    // in each other's direction. Add the iterators' elements alternately to the
    // skewed list.
    ...
    auto segment =
        create_dict_segment_by_type<int32_t>(DataType::Int, skewed_ints);
    auto index = std::make_shared<AdaptiveRadixTreeIndex>(
        std::vector<std::shared_ptr<const AbstractSegment>>({segment}));

    for (auto i :
         {0,    2,    4,    8,    12,   14,   60,   64,   128,   130,
          1024, 1026, 2048, 2050, 4096, 8190, 8192, 8194, 16382, 16384})
    {
        EXPECT_EQ((*segment)[*index->lower_bound({i})], AllTypeVariant{i});
        EXPECT_EQ((*segment)[*index->lower_bound({i + 1})],
                  AllTypeVariant{i + 2});
        EXPECT_EQ((*segment)[*index->upper_bound({i})], AllTypeVariant{i + 2});
        EXPECT_EQ((*segment)[*index->upper_bound({i + 1})],
                  AllTypeVariant{i + 2});

        auto expected_lower = i;
        for (auto it = index->lower_bound({i});
             it < index->lower_bound({i + 20}); ++it)
        {
    ...
    // In the following tests, we iterate over all integers from 0 to
    // 3*test_size. The largest elements are known to be of size 2*test_size.
    // When items are within the test_size range and even, we should find the
    // item in the index. If it is odd or above the test range, we should not
    // find the item.
    for (auto search_item = int32_t{0};
         search_item < static_cast<int32_t>(3 * test_size); ++search_item)
    {
        if (search_item % 2 == 0 &&
            search_item < static_cast<int32_t>(2 * test_size))
        {
        ...
        EXPECT_EQ(index->lower_bound({search_item}),
                  index->upper_bound({search_item}));
    }
}

TEST_F(AdaptiveRadixTreeIndexTest, SimpleTest)
{
    const auto values = std::vector<std::optional<int32_t>>{
        0, 0, 0, 0, 0, 17, 17, 17, 99, std::numeric_limits<int32_t>::max()};

    auto segment = create_dict_segment_by_type<int32_t>(DataType::Int, values);
    auto index = std::make_shared<AdaptiveRadixTreeIndex>(
        std::vector<std::shared_ptr<const AbstractSegment>>({segment}));

    // We check whether the index answer (i.e., position(s) for a search value)
    // is correct
    ...
    EXPECT_EQ(index->upper_bound({std::numeric_limits<int32_t>::max()}),
              index->cend());
}
 ...
 *   - sparse vector: wide range of values (between 1 and MAX_INT) with large
 *gaps
...
TEST_F(AdaptiveRadixTreeIndexTest, SparseVectorOfInts)
{
    ...
    {
        values[value_index] =
            _indexed_value_pool[value_index % _indexed_value_pool.size()];
    }
...
{
    // Value counters of an exemplary exponential distribution. The index of a
    // counter represents the value.
    auto value_counters =
        std::array<uint16_t, 8>{3243, 1118, 420, 143, 55, 14, 4, 3};
    ...
    DebugAssert(values.size() == overall_value_counter,
                "Number of generated values is incorrect.");
...
TEST_F(AdaptiveRadixTreeIndexTest, UpperLowerBoundForNonExistingElements)
{
    for (auto test_size : {4, 16, 48, 256})
    {
        ...
        {
            // Leave out the second and second to last element (e.g. 1 & 2, 1 &
            // 14, ...).
        ...
        const auto segment =
            create_dict_segment_by_type<int32_t>(DataType::Int, values);
        const auto index = std::make_shared<AdaptiveRadixTreeIndex>(
            std::vector<std::shared_ptr<const AbstractSegment>>({segment}));

        // As we look for the first element that is equal or greater, we should
        // give out the position of 2. For test_size 4, 2 is removed as well, so
        // we expect 3.
        if (test_size == 4)
        {
        ...
        // We want the position of the first element that is greater then
        // test_size-2, which is test_size-1.
