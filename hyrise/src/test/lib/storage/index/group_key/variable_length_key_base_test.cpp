...
class VariableLengthKeyBaseTest : public BaseTest
{
  ...
  protected:
    ...
    {
        return VariableLengthKeyBase(
            reinterpret_cast<VariableLengthKeyWord *>(&value), sizeof(value));
    }
...
TEST_F(VariableLengthKeyBaseTest, OrAssignmentWithKeyLongerThan64Bit)
{
    ...
    auto key = VariableLengthKeyBase(
        reinterpret_cast<VariableLengthKeyWord *>(&memory), sizeof(memory));
...
TEST_F(VariableLengthKeyBaseTest, ShiftAndSet)
{
    ...
    if constexpr (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
    {
        key = VariableLengthKeyBase(
            reinterpret_cast<VariableLengthKeyWord *>(&memory),
            sizeof(uint32_t));
    }
    else
    {
