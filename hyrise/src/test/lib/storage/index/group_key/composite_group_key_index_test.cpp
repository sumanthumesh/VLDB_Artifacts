...
std::vector<hyrise::VariableLengthKey>
to_vector(const hyrise::VariableLengthKeyStore &keys)
{
...
is_contained_in(hyrise::ChunkOffset value,
                const std::set<hyrise::ChunkOffset> &set)
{
    if (set.find(value) == set.end())
    {
        return testing::AssertionFailure()
               << testing::PrintToString(set) << " does not contain " << value;
    }
    else
    {
        return testing::AssertionSuccess()
               << testing::PrintToString(set) << " contains " << value;
    }
}

void EXPECT_POSITION_LIST_EQ(
    const std::vector<std::set<hyrise::ChunkOffset>> &expected,
    const std::vector<hyrise::ChunkOffset> &actual)
{
    ...
    {
        distinct_expected_positions.insert(expectation_for_position.begin(),
                                           expectation_for_position.end());
    }

    auto distinct_actual_positions =
        std::set<hyrise::ChunkOffset>(actual.begin(), actual.end());
...
class CompositeGroupKeyIndexTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        _segment_int = create_dict_segment_by_type<int32_t>(
            DataType::Int, {2, 1, 0, 1, 0, 3, 2, 3});
        _segment_str = create_dict_segment_by_type<pmr_string>(
            DataType::String, {"hotel", "delta", "frank", "delta", "apple",
                               "charlie", "charlie", "inbox"});

        _index_int_str = std::make_shared<CompositeGroupKeyIndex>(
            std::vector<std::shared_ptr<const AbstractSegment>>{_segment_int,
                                                                _segment_str});
        _index_str_int = std::make_shared<CompositeGroupKeyIndex>(
            std::vector<std::shared_ptr<const AbstractSegment>>{_segment_str,
                                                                _segment_int});
  ...
  protected:
    ...
    /**
     * Use pointers to inner data structures of CompositeGroupKeyIndex in order
     * to bypass the private scope. In order to minimize the friend classes of
     * CompositeGroupKeyIndex the fixture is used as proxy. Since the variables
     * are set in setup(), references are not possible.
     */
...
{
    auto expected_int_str = std::vector<VariableLengthKey>{
        create_key(0x0000), create_key(0x0003), create_key(0x0102),
        create_key(0x0201), create_key(0x0204), create_key(0x0301),
        create_key(0x0305)};
    auto expected_str_int = std::vector<VariableLengthKey>{
        create_key(0x0000), create_key(0x0102), create_key(0x0103),
        create_key(0x0201), create_key(0x0300), create_key(0x0402),
        create_key(0x0503)};
...
{
    auto expected_int_str = std::vector{
        ChunkOffset{0}, ChunkOffset{1}, ChunkOffset{2}, ChunkOffset{4},
        ChunkOffset{5}, ChunkOffset{6}, ChunkOffset{7}};
    auto expected_str_int = std::vector{
...
TEST_F(CompositeGroupKeyIndexTest, PositionList)
{
    auto expected_int_str =
        std::vector<std::set<ChunkOffset>>{{ChunkOffset{4}},
                                           {ChunkOffset{2}},
