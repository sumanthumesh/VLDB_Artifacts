...
class GroupKeyIndexTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        dict_segment = ChunkEncoder::encode_segment(
            value_segment_str, DataType::String,
            SegmentEncodingSpec{EncodingType::Dictionary});

        index = std::make_shared<GroupKeyIndex>(
...
{
    auto expected_offsets = std::vector{
        ChunkOffset{0}, ChunkOffset{1}, ChunkOffset{3}, ChunkOffset{5},
...
{
    const auto &dict_segment_int_no_nulls =
        create_dict_segment_by_type<int32_t>(DataType::Int, {13, 37});
    const auto &dict_segment_int_nulls = create_dict_segment_by_type<int32_t>(
        DataType::Int, {std::nullopt, std::nullopt});
    const auto &dict_segment_int_empty =
        create_dict_segment_by_type<int32_t>(DataType::Int, {});
    const auto &index_int_empty = std::make_shared<GroupKeyIndex>(
        std::vector<std::shared_ptr<const AbstractSegment>>(
            {dict_segment_int_empty}));
    const auto &index_int_no_nulls = std::make_shared<GroupKeyIndex>(
        std::vector<std::shared_ptr<const AbstractSegment>>(
            {dict_segment_int_no_nulls}));
    const auto &index_int_nulls = std::make_shared<GroupKeyIndex>(
        std::vector<std::shared_ptr<const AbstractSegment>>(
...
TEST_F(GroupKeyIndexTest, IndexPostings)
{
    ...
    auto distinct_values =
        std::unordered_set<ChunkOffset>(positions->begin(), positions->end());
    EXPECT_TRUE(distinct_values.size() == positions->size());

    // check if the correct positions are present for each value-id
    auto expected_positions = std::vector<std::unordered_set<ChunkOffset>>{
        {ChunkOffset{7}},
        ...
        {ChunkOffset{1}},
        {ChunkOffset{10}}};
    auto expected_null_positions = std::vector<std::unordered_set<ChunkOffset>>{
