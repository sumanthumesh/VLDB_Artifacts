...
class VariableLengthKeyStoreTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _store_sorted =
            VariableLengthKeyStore(ChunkOffset{4}, sizeof(uint32_t));
...
TEST_F(VariableLengthKeyStoreTest, CopyIterators)
{
    ...
    auto const_from_mutable =
        VariableLengthKeyStore::const_iterator(mutable_iter);
...
TEST_F(VariableLengthKeyStoreTest, CopyWithStd)
{
    ...
    auto expected = std::vector<VariableLengthKey>{_key_reference, _key_equal,
                                                   _key_less, _key_greater};
...
{
    auto lower_bound_of_less =
        std::lower_bound(_store_sorted.begin(), _store_sorted.end(), _key_less);
    auto upper_bound_of_less =
        std::upper_bound(_store_sorted.begin(), _store_sorted.end(), _key_less);
    auto lower_bound_of_equal = std::lower_bound(
        _store_sorted.begin(), _store_sorted.end(), _key_equal);
    auto upper_bound_of_equal = std::upper_bound(
        _store_sorted.begin(), _store_sorted.end(), _key_equal);
    auto upper_bound_of_greater = std::upper_bound(
        _store_sorted.begin(), _store_sorted.end(), _key_greater);
