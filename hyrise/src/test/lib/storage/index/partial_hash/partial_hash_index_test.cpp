...
class PartialHashIndexTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        const auto table_column_definitions =
            TableColumnDefinitions{{"column_1", DataType::String, true}};
        table =
            std::make_shared<Table>(table_column_definitions, TableType::Data);

        values1 = {"hotel", "delta",   "nullptr", "delta",
                   "apple", "charlie", "charlie", "inbox"};
        auto null_values_1 = {false, false, true,  false,
                              false, false, false, false};
        values2 = {"hello",   "delta", "funny", "names",
                   "nullptr", "paper", "clock", "inbox"};
        auto null_values_2 = {false, false, false, false,
                              true,  false, false, false};
        segment1 = std::make_shared<ValueSegment<pmr_string>>(
            std::move(values1), std::move(null_values_1));
        segment2 = std::make_shared<ValueSegment<pmr_string>>(
            std::move(values2), std::move(null_values_2));
        ...
        chunks_to_index.push_back(
            std::make_pair(ChunkID{0}, table->get_chunk(ChunkID{0})));
        chunks_to_index.push_back(
            std::make_pair(ChunkID{1}, table->get_chunk(ChunkID{1})));

        index =
            std::make_shared<PartialHashIndex>(chunks_to_index, ColumnID{0});
        index_map = &(
            static_cast<PartialHashIndexImpl<pmr_string> *>(index->_impl.get())
                ->_positions);
    }
    ...
    /**
     * Use pointers to inner data structures of PartialHashIndex in order to
     * bypass the private scope. Since the variable is set in setup() references
     * are not possible.
     */
    tsl::sparse_map<pmr_string, std::vector<RowID>> *index_map = nullptr;

    // Utility functions to access an index's protected functions.
    PartialHashIndex::Iterator
    cbegin(std::shared_ptr<PartialHashIndex> index) const
    {
        return index->_cbegin();
    }

    PartialHashIndex::Iterator
    cend(std::shared_ptr<PartialHashIndex> index) const
    {
        return index->_cend();
    }

    PartialHashIndex::Iterator
    null_cbegin(std::shared_ptr<PartialHashIndex> index) const
    {
        return index->_null_cbegin();
    }

    PartialHashIndex::Iterator
    null_cend(std::shared_ptr<PartialHashIndex> index) const
    {
        return index->_null_cend();
    }

    PartialHashIndex::IteratorRange
    range_equals(std::shared_ptr<PartialHashIndex> index,
                 const AllTypeVariant &value) const
    {
        return index->_range_equals(value);
    }

    PartialHashIndex::IteratorRangePair
    range_not_equals(std::shared_ptr<PartialHashIndex> index,
                     const AllTypeVariant &value) const
    {
...
TEST_F(PartialHashIndexTest, AccessValuesWithIterators)
{
    auto test_access_values_with_iterators = [](auto begin, auto end)
    {
        ...
        EXPECT_EQ(std::distance(begin, end), 14);
        // Test for existing value in iterator.
        EXPECT_NE(std::find(begin, end, RowID{ChunkID{0}, ChunkOffset{4}}),
                  end);
        // Test for not-existing value in iterator.
        EXPECT_EQ(std::find(begin, end, RowID{ChunkID{2}, ChunkOffset{4}}),
                  end);
    };
...
TEST_F(PartialHashIndexTest, RangeEqualsWithIteratorExistingValue)
{
    ...
    index->range_equals_with_iterators(access_range_equals_with_iterators,
                                       "delta");
}

TEST_F(PartialHashIndexTest, RangeEqualsWithIteratorNotExistingValue)
{
    auto access_range_equals_non_existing_value_with_iterators =
        [&](auto begin, auto end)
    {
        EXPECT_EQ(begin, cend(index));
        EXPECT_EQ(end, cend(index));
    };
    index->range_equals_with_iterators(
        access_range_equals_non_existing_value_with_iterators, "blub");
}
...
TEST_F(PartialHashIndexTest, RangeNotEqualsWithIteratorExistingValue)
{
    ...
    std::multiset<RowID> expected_when_value_exists = {
        RowID{ChunkID{0}, ChunkOffset{0}}, RowID{ChunkID{0}, ChunkOffset{1}},
        RowID{ChunkID{0}, ChunkOffset{3}}, RowID{ChunkID{0}, ChunkOffset{4}},
        RowID{ChunkID{0}, ChunkOffset{5}}, RowID{ChunkID{0}, ChunkOffset{6}},
        RowID{ChunkID{0}, ChunkOffset{7}}, RowID{ChunkID{1}, ChunkOffset{0}},
        RowID{ChunkID{1}, ChunkOffset{1}}, RowID{ChunkID{1}, ChunkOffset{2}},
        RowID{ChunkID{1}, ChunkOffset{5}}, RowID{ChunkID{1}, ChunkOffset{6}},
    ...
    index->range_not_equals_with_iterators(
        access_range_not_equals_with_iterators, "names");
...
TEST_F(PartialHashIndexTest, RangeNotEqualsWithIteratorNotExistingValue)
{
    ...
    std::set<RowID> expected_when_value_not_exists = {
        ...
        RowID{ChunkID{0}, ChunkOffset{7}}, RowID{ChunkID{1}, ChunkOffset{0}},
        RowID{ChunkID{1}, ChunkOffset{1}}, RowID{ChunkID{1}, ChunkOffset{2}},
        RowID{ChunkID{1}, ChunkOffset{3}}, RowID{ChunkID{1}, ChunkOffset{5}},
        RowID{ChunkID{1}, ChunkOffset{6}}, RowID{ChunkID{1}, ChunkOffset{7}}};
    std::set<RowID> actual_when_value_not_exists = {};

    auto access_range_not_equals_non_existing_value_with_iterators =
        [&](auto begin, auto end)
    {
        for (auto it = begin; it != end; ++it)
        {
    ...
    index->range_not_equals_with_iterators(
        access_range_not_equals_non_existing_value_with_iterators, "blub");
...
TEST_F(PartialHashIndexTest, NullValues)
{
    ...
    std::multiset<RowID> expected = {RowID{ChunkID{0}, ChunkOffset{2}},
                                     RowID{ChunkID{1}, ChunkOffset{4}}};
...
TEST_F(PartialHashIndexTest, IndexedNullValues)
{
    ...
    auto new_values1 =
        pmr_vector<pmr_string>{"hotel", "delta",   "gamma",   "delta",
                               "apple", "charlie", "charlie", "inbox"};
    auto new_null_values_1 = {false, false, false, false,
                              false, false, false, false};
    auto new_values2 =
        pmr_vector<pmr_string>{"hello",   "delta", "funny", "names",
                               "nullptr", "paper", "clock", "inbox"};
    auto new_null_values_2 = {false, false, false, false,
                              true,  false, false, false};

    auto new_segment1 = std::make_shared<ValueSegment<pmr_string>>(
        std::move(new_values1), std::move(new_null_values_1));
    auto new_segment2 = std::make_shared<ValueSegment<pmr_string>>(
        std::move(new_values2), std::move(new_null_values_2));
    ...
    auto chunks_to_index0 =
        std::vector<std::pair<ChunkID, std::shared_ptr<Chunk>>>{};
    auto chunks_to_index1 =
        std::vector<std::pair<ChunkID, std::shared_ptr<Chunk>>>{};

    chunks_to_index0.push_back(
        std::make_pair(ChunkID{0}, new_table->get_chunk(ChunkID{0})));
    chunks_to_index1.push_back(
        std::make_pair(ChunkID{1}, new_table->get_chunk(ChunkID{1})));

    auto new_index =
        std::make_shared<PartialHashIndex>(chunks_to_index0, ColumnID{0});
...
{
    pmr_vector<pmr_string> values = {"new1",    "new2", "new3", "new4",
                                     "nullptr", "new6", "new7", "new8"};
    pmr_vector<bool> null_values = {false, false, false, false,
                                    true,  false, false, false};
    auto segment = std::make_shared<ValueSegment<pmr_string>>(
    ...
    EXPECT_EQ(*range_equals(index, "new1").first,
              (RowID{ChunkID{2}, ChunkOffset{0}}));
...
TEST_F(PartialHashIndexTest, Remove)
{
    ...
    EXPECT_EQ(index->get_indexed_chunk_ids(),
              (tsl::sparse_set<ChunkID>{ChunkID{1}}));
...
{
    auto chunks_to_add =
        std::vector<std::vector<std::pair<ChunkID, std::shared_ptr<Chunk>>>>{};

    pmr_vector<pmr_string> values2 = {"new1",    "new2", "new3", "new4",
                                      "nullptr", "new6", "new7", "new8"};
    pmr_vector<bool> null_values2 = {false, false, false, false,
                                     true,  false, false, false};
    auto segment2 = std::make_shared<ValueSegment<pmr_string>>(
        std::move(values2), std::move(null_values2));
    ...
    pmr_vector<pmr_string> values3 = {"1",       "2", "3", "4",
                                      "nullptr", "6", "7", "8"};
    pmr_vector<bool> null_values3 = {false, false, false, false,
                                     true,  false, false, false};
    auto segment3 = std::make_shared<ValueSegment<pmr_string>>(
        std::move(values3), std::move(null_values3));
    ...
    pmr_vector<pmr_string> values4 = {"old1",    "old2", "old3", "old4",
                                      "oldlptr", "old6", "old7", "old8"};
    pmr_vector<bool> null_values4 = {false, false, false, false,
                                     false, false, false, false};
    auto segment4 = std::make_shared<ValueSegment<pmr_string>>(
        std::move(values4), std::move(null_values4));
    ...
    pmr_vector<pmr_string> values5 = {"nullptr", "new2", "new3", "new4",
                                      "nullptr", "new6", "new7", "nullptr"};
    pmr_vector<bool> null_values5 = {true, false, false, false,
                                     true, false, false, true};
    auto segment5 = std::make_shared<ValueSegment<pmr_string>>(
        std::move(values5), std::move(null_values5));
    ...
    auto insert_entries_to_index =
        [&](const std::vector<std::pair<ChunkID, std::shared_ptr<Chunk>>>
                &chunk_to_add) { index->insert(chunk_to_add); };
    ...
    for (auto thread_number = uint8_t{0}; thread_number < N_THREADS;
         ++thread_number)
    {
        if (thread_number % 2 == 1)
        {
            threads[thread_number] =
                std::thread(insert_entries_to_index,
                            chunks_to_add[(thread_number - 1) / 2]);
        }
        else
        {
...
TEST_F(PartialHashIndexTest, EqualsValue)
{
    ...
    std::multiset<RowID> expected = {RowID{ChunkID{0}, ChunkOffset{1}},
...
TEST_F(PartialHashIndexTest, NotEqualsValue)
{
    ...
    std::multiset<RowID> expected = {
        RowID{ChunkID{0}, ChunkOffset{0}}, RowID{ChunkID{0}, ChunkOffset{4}},
        RowID{ChunkID{0}, ChunkOffset{5}}, RowID{ChunkID{0}, ChunkOffset{6}},
        RowID{ChunkID{0}, ChunkOffset{7}}, RowID{ChunkID{1}, ChunkOffset{0}},
        RowID{ChunkID{1}, ChunkOffset{2}}, RowID{ChunkID{1}, ChunkOffset{3}},
        RowID{ChunkID{1}, ChunkOffset{5}}, RowID{ChunkID{1}, ChunkOffset{6}},
        RowID{ChunkID{1}, ChunkOffset{7}}};
    std::multiset<RowID> actual = {};
...
{
    auto local_values = pmr_vector<pmr_string>{"h", "d", "f", "d", "a", "c",
                                               "c", "i", "b", "z", "x"};
    auto segment =
        std::make_shared<ValueSegment<pmr_string>>(std::move(local_values));
    ...
    expected_memory_usage +=
        sizeof(tsl::sparse_map<pmr_string, std::vector<RowID>>);
    // + number of different non-NULL values * key tuple size
    expected_memory_usage +=
        9 * sizeof(std::pair<pmr_string, std::vector<RowID>>);
    ...
    expected_memory_usage +=
        0 * sizeof(std::pair<pmr_string, std::vector<RowID>>);
...
TEST_F(PartialHashIndexTest, MemoryUsageNulls)
{
    ...
    // + key tuple size if null values are indexed
    expected_memory_usage +=
        1 * sizeof(std::pair<pmr_string, std::vector<RowID>>);
    // + number of NULL values * RowID
...
{
    const auto &dict_segment_string_mixed =
        create_dict_segment_by_type<pmr_string>(
            DataType::String,
            {std::nullopt, "h", "d", "f", "d", "a", std::nullopt, std::nullopt,
             "c", std::nullopt, "c", "i", "b", "z", "x", std::nullopt});
...
{
    const auto &dict_segment_string_empty =
        create_dict_segment_by_type<pmr_string>(DataType::String, {});
