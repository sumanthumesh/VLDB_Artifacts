...
class BTreeIndexTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        values = {"hotel", "delta",   "frank",   "delta",
                  "apple", "charlie", "charlie", "inbox"};
        segment = std::make_shared<ValueSegment<pmr_string>>(std::move(values));
        sorted = {"apple", "charlie", "charlie", "delta",
                  "delta", "frank",   "hotel",   "inbox"};
        index = std::make_shared<BTreeIndex>(
    ...
    /**
     * Use pointers to inner data structures of BTreeIndex in order to bypass
     * the private scope. Since the variable is set in setup() references are
     * not possible.
     */
...
// The following tests contain switches for different implementations of the
// stdlib. Short String Optimization (SSO) stores strings of a certain size in
// the pmr_string object itself. Only strings exceeding this size (15 for
// libstdc++ and 22 for libc++) are stored on the heap.
...
TEST_F(BTreeIndexTest, MemoryConsumptionVeryShortStringNulls)
{
    ...
    const auto &index = std::make_shared<BTreeIndex>(
...
TEST_F(BTreeIndexTest, MemoryConsumptionShortString)
{
    ASSERT_GE(pmr_string("").capacity(), 7u)
        << "Short String Optimization (SSO) is expected to hold at least 7 "
           "characters";
...
TEST_F(BTreeIndexTest, MemoryConsumptionLongString)
{
    ASSERT_LE(pmr_string("").capacity(), 22u)
        << "Short String Optimization (SSO) is expected to hold at maximum 22 "
           "characters";

    auto local_values = pmr_vector<pmr_string>{
        "hotelhotelhotelhotelhotel", "deltadeltadeltadelta",
        "frankfrankfrankfrank",      "deltadeltadeltadelta",
        "appleappleappleapple",      "charliecharliecharlie",
...
#else
    // libc++ Only one string exceeds the reserved space (22 characters) for
    // small strings:
    //   264 (reported by cpp_btree implementation)
    // +  24 std::vector<ChunkOffset> object overhead
    // +  32 number of elements (8) * sizeof(ChunkOffset) (4)
