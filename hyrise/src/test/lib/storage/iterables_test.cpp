...
template <typename DataType> struct SumUpWithIterator
{
...
template <typename DataType> struct SumUp
{
    template <typename T> void operator()(const T &position) const
    {
        if (position.is_null())
        {
            return;
  ...
  protected:
    void SetUp() override
    {
        ...
        position_filter = std::make_shared<RowIDPosList>(
            RowIDPosList{{ChunkID{0}, ChunkOffset{0}},
...
class EncodedSegmentIterablesTest
    : public IterablesTest,
      public ::testing::WithParamInterface<
          std::tuple<SegmentEncodingSpec, bool, bool>>
{
  public:
    ...
    std::vector<ChunkOffset> expected_offsets(const bool with_position_filter)
    {
        ...
        {
            return std::vector<ChunkOffset>{ChunkOffset{0}, ChunkOffset{1},
                                            ChunkOffset{2}};
        }
        else
        {
            return std::vector<ChunkOffset>{ChunkOffset{0}, ChunkOffset{1},
                                            ChunkOffset{2}, ChunkOffset{3}};
        }
    }
};

auto formatter_iterables = [](const ::testing::TestParamInfo<
                               std::tuple<SegmentEncodingSpec, bool, bool>>
                                  info)
{
    auto stream = std::stringstream{};
    stream << std::get<0>(info.param)
           << (std::get<1>(info.param) ? "WithNulls" : "")
 ...
 * Summing up all values in an Int segment using iterators with all applicable
 * segment encodings, nullable/not nullable columns and a position filter.
 */

INSTANTIATE_TEST_SUITE_P(
    SegmentEncoding, EncodedSegmentIterablesTest,
...
TEST_P(EncodedSegmentIterablesTest, IteratorWithIterators)
{
    ...
    auto chunk_encoding_spec =
        ChunkEncodingSpec{test_table->column_count(),
                          SegmentEncodingSpec{EncodingType::Unencoded}};
    for (auto column_id = ColumnID{0}; column_id < test_table->column_count();
         ++column_id)
    {
        if (encoding_supports_data_type(
                encoding_spec.encoding_type,
                test_table->column_data_type(column_id)))
        {
            chunk_encoding_spec[column_id] = encoding_spec;
        }
        else
        {
            // skip test if the column that is used for testing doesn't support
            // encoding
    ...
    resolve_data_and_segment_type(
        *abstract_segment,
        [&](const auto data_type_t, const auto &segment)
        {
            ...
            {
                // Test summing up all values in the segment or all values
                // referenced by the PosList
                {
                    auto sum = ColumnDataType{0};
                    auto accessed_offsets = std::vector<ChunkOffset>{};
                    const auto functor = SumUpWithIterator<ColumnDataType>{
                        sum, accessed_offsets};
                    ...
                    EXPECT_EQ(sum,
                              expected_sum(nullable, with_position_filter));
                    EXPECT_EQ(accessed_offsets,
                              expected_offsets(with_position_filter));
                }
            }

            // Next, test that begin and end iterators are compatible (i.e.,
            // that iterating from both ends allows us to meet in the middle)
            {
                const auto iterable = create_iterable_from_segment<
                    ColumnDataType, false /* no type erasure */>(segment);
...
class EncodedStringSegmentIterablesTest
    : public IterablesTest,
      public ::testing::WithParamInterface<
          std::tuple<SegmentEncodingSpec, bool>>
{
  public:
...
auto formatter_iterables_string =
    [](const ::testing::TestParamInfo<std::tuple<SegmentEncodingSpec, bool>>
           info)
{
    auto stream = std::stringstream{};
    stream << std::get<0>(info.param) << "String"
           << (std::get<1>(info.param) ? "WithFilter" : "");
 ...
 * Concatenationg all values in a String segment using iterators with all
 * applicable segment encodings and a position filter.
 */

INSTANTIATE_TEST_SUITE_P(
    SegmentEncoding, EncodedStringSegmentIterablesTest,
    ::testing::Combine(::testing::ValuesIn(all_segment_encoding_specs),
                       ::testing::Bool()), // position filter
        ...
        [&](const auto data_type_t, const auto &segment)
        {
            ...
            if constexpr (std::is_same_v<ColumnDataType, pmr_string>)
            {
                ...
                EXPECT_EQ(concatenate,
                          expected_concatenation(with_position_filter));
            }
        });
}

class EncodedSegmentChunkOffsetTest
    : public IterablesTest,
      public ::testing::WithParamInterface<SegmentEncodingSpec>
{
};

auto formatter_chunk_offset =
    [](const ::testing::TestParamInfo<SegmentEncodingSpec> info)
{
    auto stream = std::stringstream{};
    stream << info.param;
...
TEST_F(IterablesTest, ReferenceSegmentIteratorWithIterators)
{
    auto pos_list = RowIDPosList{
        RowID{ChunkID{0}, ChunkOffset{0}}, RowID{ChunkID{0}, ChunkOffset{3}},
        RowID{ChunkID{0}, ChunkOffset{1}}, RowID{ChunkID{0}, ChunkOffset{2}},
        NULL_ROW_ID};

    const auto reference_segment = std::make_unique<ReferenceSegment>(
        table, ColumnID{0},
        std::make_shared<RowIDPosList>(std::move(pos_list)));

    const auto iterable =
        ReferenceSegmentIterable<int32_t, EraseReferencedSegmentType::No>{
            *reference_segment};
    ...
    EXPECT_EQ(accessed_offsets,
              (std::vector<ChunkOffset>{ChunkOffset{0}, ChunkOffset{1},
                                        ChunkOffset{2}, ChunkOffset{3},
                                        ChunkOffset{4}}));
}
...
TEST_F(IterablesTest,
       ReferenceSegmentIteratorWithIteratorsSingleChunkTypeErased)
{
    auto pos_list = RowIDPosList{
        RowID{ChunkID{0}, ChunkOffset{0}}, RowID{ChunkID{0}, ChunkOffset{3}},
        RowID{ChunkID{0}, ChunkOffset{1}}, RowID{ChunkID{0}, ChunkOffset{2}}};
    ...
    const auto iterable =
        ReferenceSegmentIterable<int32_t, EraseReferencedSegmentType::Yes>{
            *reference_segment};
...
TEST_F(IterablesTest, ValueSegmentIteratorForEach)
{
    ...
    const auto int_segment =
        std::dynamic_pointer_cast<const ValueSegment<int32_t>>(segment);
