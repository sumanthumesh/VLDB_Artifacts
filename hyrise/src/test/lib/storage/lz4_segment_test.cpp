...
class StorageLZ4SegmentTest : public BaseTest
{
  protected:
    ...
    std::shared_ptr<ValueSegment<pmr_string>> vs_str =
        std::make_shared<ValueSegment<pmr_string>>(true);
    std::shared_ptr<ValueSegment<int>> vs_int =
        std::make_shared<ValueSegment<int>>(true);
};

template <typename T>
std::shared_ptr<LZ4Segment<T>>
compress(std::shared_ptr<ValueSegment<T>> segment, DataType data_type)
{
    auto encoded_segment = ChunkEncoder::encode_segment(
        segment, data_type, SegmentEncodingSpec{EncodingType::LZ4});
...
{
    auto empty_int_segment =
        compress(std::make_shared<ValueSegment<int32_t>>(true), DataType::Int);
    EXPECT_FALSE(empty_int_segment->string_offset_decompressor());
    EXPECT_FALSE(empty_int_segment->null_values());

    auto empty_str_segment = compress(
        std::make_shared<ValueSegment<pmr_string>>(true), DataType::String);
...
TEST_F(StorageLZ4SegmentTest, CompressMultiBlockStringSegment)
{
    ...
    // Starts in the first block, completely fills second block and reaches the
    // third block.
    ...
    const auto third_block_size =
        (string1.size() + string2.size() + string3.size()) % block_size;
    ...
    /**
     * Access the first, second and third block. The cache should be used for
     * the third block. As a result the buffer used for decompression will
     * contain the second block since it needed to be decompressed.
     */
    ...
    // Access the same blocks again. Now, the cache should be used for the
    // second block and then contain the third block.
    result = lz4_segment->decompress(ChunkOffset{1u}, result.second, cache);
    EXPECT_EQ(cache.size(), third_block_size);
    EXPECT_EQ(result.first, string2);
    EXPECT_EQ(result.second, 2u);
    ...
    /**
     * Access the first, second and third block again. The cache should now be
     * used for the first block and afterwards be overwritten with the third
     * block.
     */
    result = lz4_segment->decompress(ChunkOffset{1u}, result.second, cache);
    EXPECT_EQ(cache.size(), third_block_size);
...
TEST_F(StorageLZ4SegmentTest, CompressDictionaryStringSegment)
{
    ...
    {
        vs_str->append(AllTypeVariant{
            pmr_string{"this is element " + std::to_string(index)}});
    }
    ...
    EXPECT_EQ(lz4_segment->decompress(ChunkOffset{4013u}),
              "this is element 4013");
    EXPECT_EQ(lz4_segment->decompress(ChunkOffset{200u}),
              "this is element 200");
    ...
    result = lz4_segment->decompress(ChunkOffset{num_rows - 1}, result.second,
                                     cache);
    EXPECT_EQ(result.first,
              pmr_string{"this is element " + std::to_string(num_rows - 1)});
