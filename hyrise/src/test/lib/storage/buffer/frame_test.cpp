...
TEST_F(FrameTest, TestStateTransitions)
{
    ...
    EXPECT_FALSE(frame.unlock_shared());
    EXPECT_EQ(Frame::version(frame.state_and_version()), 1);
    EXPECT_EQ(Frame::state(frame.state_and_version()),
              Frame::SINGLE_LOCKED_SHARED);
...
TEST_F(FrameTest, TestStreamOperator)
{
    auto frame = Frame{};
    {
        ...
        EXPECT_EQ(
            out.str(),
            "Frame(state = EVICTED, node_id = 0, dirty = 0, version = 0)");
    }
    ...
    frame.try_lock_exclusive(frame.state_and_version());
    {
        ...
        out << frame;
        EXPECT_EQ(out.str(),
                  "Frame(state = LOCKED, node_id = 0, dirty = 0, version = 0)");
    }
    ...
    frame.mark_dirty();
    {
        ...
        out << frame;
        EXPECT_EQ(out.str(),
                  "Frame(state = LOCKED, node_id = 0, dirty = 1, version = 0)");
    }
    ...
    frame.set_node_id(NodeID{13});
    {
        ...
        EXPECT_EQ(
            out.str(),
            "Frame(state = LOCKED, node_id = 13, dirty = 1, version = 0)");
    }
    ...
    frame.unlock_exclusive();
    {
        ...
        EXPECT_EQ(
            out.str(),
            "Frame(state = UNLOCKED, node_id = 13, dirty = 1, version = 1)");
    }
    ...
    frame.try_lock_shared(frame.state_and_version());
    {
        auto out = std::stringstream{};
        out << frame;
        EXPECT_EQ(out.str(), "Frame(state = LOCKED_SHARED (2), node_id = 13, "
                             "dirty = 1, version = 1)");
    }
    frame.unlock_shared();
    frame.unlock_shared();
    frame.try_mark(frame.state_and_version());
    {
        ...
        EXPECT_EQ(
            out.str(),
            "Frame(state = MARKED, node_id = 13, dirty = 1, version = 1)");
    }
}
