...
{
    auto meta = process_csv_meta_file(
        "resources/test_data/csv/sample_meta_information.csv.json");
...
{
    EXPECT_THROW(
        process_csv_meta_file("resources/test_data/csv/missing_file.csv.json"),
        std::logic_error);
}

TEST_F(CsvMetaTest, JsonSyntaxError)
{
    EXPECT_THROW(process_csv_meta_file(
                     "resources/test_data/csv/json_syntax_error.csv.json"),
