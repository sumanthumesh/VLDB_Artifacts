...
TEST_F(CsvParserTest, SingleFloatColumn)
{
    ...
    std::shared_ptr<Table> expected_table =
        load_table("resources/test_data/tbl/float.tbl", ChunkOffset{5});
    EXPECT_TABLE_EQ_ORDERED(table, expected_table);
}

TEST_F(CsvParserTest, WindowsEncoding)
{
    EXPECT_THROW(CsvParser::parse("resources/test_data/csv/float_crlf.csv"),
                 std::exception);
}
...
TEST_F(CsvParserTest, FloatIntTable)
{
    ...
    std::shared_ptr<Table> expected_table =
        load_table("resources/test_data/tbl/float_int.tbl", ChunkOffset{2});
    EXPECT_TABLE_EQ_ORDERED(table, expected_table);
}
...
TEST_F(CsvParserTest, StringNoQuotes)
{
    ...
    std::shared_ptr<Table> expected_table =
        load_table("resources/test_data/tbl/string.tbl", ChunkOffset{5});
    EXPECT_TABLE_EQ_ORDERED(table, expected_table);
}
...
TEST_F(CsvParserTest, StringEscaping)
{
    ...
    auto expected_table = std::make_shared<Table>(
        TableColumnDefinitions{{"a", DataType::String, false}}, TableType::Data,
        ChunkOffset{5});
...
{
    auto table =
        CsvParser::parse("resources/test_data/csv/float_int_empty.csv");
    std::shared_ptr<Table> expected_table = load_table(
        "resources/test_data/tbl/float_int_empty.tbl", ChunkOffset{2});
    EXPECT_TABLE_EQ_ORDERED(table, expected_table);
}
...
{
    auto table = CsvParser::parse(
        "resources/test_data/csv/float_int_trailing_newline.csv");
    std::shared_ptr<Table> expected_table =
        load_table("resources/test_data/tbl/float_int.tbl", ChunkOffset{2});
    EXPECT_TABLE_EQ_ORDERED(table, expected_table);
}
...
TEST_F(CsvParserTest, EmptyStrings)
{
    ...
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{5});
    for (int i = 0; i < 8; ++i)
    {
...
{
    std::string csv_file =
        "resources/test_data/csv/ints_semicolon_separator.csv";
    auto csv_meta =
        process_csv_meta_file(csv_file + CsvMeta::META_FILE_EXTENSION);
...
{
    auto table = CsvParser::parse(
        "resources/test_data/csv/float_int_large_chunksize_max.csv",
        Chunk::DEFAULT_SIZE);
    ...
    TableColumnDefinitions column_definitions{{"b", DataType::Float, false},
                                              {"a", DataType::Int, false}};
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{20});
...
{
    auto table =
        CsvParser::parse("resources/test_data/csv/float_int_with_null.csv");

    TableColumnDefinitions column_definitions{{"a", DataType::Float, true},
                                              {"b", DataType::Int, false},
                                              {"c", DataType::Int, true}};
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{3});
...
{
    auto table =
        CsvParser::parse("resources/test_data/csv/string_with_null.csv");
...
TEST_F(CsvParserTest, ImportUnquotedNullStringAsNull)
{
    ...
    TableColumnDefinitions column_definitions{{"a", DataType::Int, true},
...
{
    auto table =
        CsvParser::parse("resources/test_data/csv/null_literal_as_string.csv");
...
{
    EXPECT_THROW(
        CsvParser::parse("resources/test_data/csv/string_with_bad_null.csv"),
        std::exception);
}
...
TEST_F(CsvParserTest, ImportUnquotedNullStringWithNullStringAsNullThrows)
{
    EXPECT_THROW(
        CsvParser::parse(
            "resources/test_data/csv/null_as_string_with_bad_null.csv"),
        std::exception);
}
...
{
    std::string csv_file =
        "resources/test_data/csv/with_and_without_quotes.csv";
    auto csv_meta =
        process_csv_meta_file(csv_file + CsvMeta::META_FILE_EXTENSION);
...
{
    EXPECT_THROW(CsvParser::parse("resources/test_data/csv/quoted_int.csv"),
                 std::exception);
}

TEST_F(CsvParserTest, UnconvertedCharactersThrows)
{
    EXPECT_THROW(CsvParser::parse(
                     "resources/test_data/csv/unconverted_characters_int.csv"),
                 std::logic_error);

    EXPECT_THROW(
        CsvParser::parse(
            "resources/test_data/csv/unconverted_characters_float.csv"),
        std::logic_error);

    EXPECT_THROW(
        CsvParser::parse(
            "resources/test_data/csv/unconverted_characters_double.csv"),
        std::logic_error);
}

TEST_F(CsvParserTest, EmptyTableFromMetaFile)
{
    const auto csv_meta_table = CsvParser{}.create_table_from_meta_file(
        "resources/test_data/csv/float_int.csv.json");
    const auto expected_table = std::make_shared<Table>(
        TableColumnDefinitions{{"b", DataType::Float, false},
                               {"a", DataType::Int, false}},
        TableType::Data);
...
TEST_F(CsvParserTest, WithScheduler)
{
    ...
    auto table =
        CsvParser::parse("resources/test_data/csv/float_int_large.csv");
...
{
    const auto table = CsvParser::parse(
        "resources/test_data/csv/float_int_large.csv", ChunkOffset{40});
