...
class CsvWriterTest : public BaseTest
{
  protected:
    ...
    bool compare_file(const std::string &filename,
                      const std::string &expected_content)
    {
        ...
        if (equality != 0)
        {
            ...
            std::cout << "Comparison of file to expected content failed. "
                      << std::endl;
    ...
    const std::string test_meta_filename =
        test_filename + CsvMeta::META_FILE_EXTENSION;
};
...
TEST_F(CsvWriterTest, EscapeString)
{
    ...
    EXPECT_TRUE(file_exists(test_filename));
    EXPECT_TRUE(file_exists(test_meta_filename));
    EXPECT_TRUE(compare_file(
        test_filename,
        "1,\"Sie sagte: \"\"Mir geht's gut, und dir?\"\"\",3.5\n"));
}
...
TEST_F(CsvWriterTest, FixedStringDictionarySegmentFixedWidthInteger)
{
    ...
    const auto meta_filename_string_table =
        filename_string_table + CsvMeta::META_FILE_EXTENSION;

    TableColumnDefinitions column_definitions;
    column_definitions.emplace_back("a", DataType::String, false);

    auto string_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{4});
    ...
    ChunkEncoder::encode_chunks(
        string_table, {ChunkID{0}},
    ...
    EXPECT_TRUE(compare_file(filename_string_table, "\"a\"\n"
                                                    "\"string\"\n"
                                                    "\"xxx\"\n"
                                                    "\"www\"\n"
                                                    "\"yyy\"\n"
...
TEST_F(CsvWriterTest, ReferenceSegment)
{
    ...
    auto scan = create_table_scan(table_wrapper, ColumnID{0},
    ...
    EXPECT_TRUE(file_exists(test_filename));
    EXPECT_TRUE(file_exists(test_meta_filename));
    EXPECT_TRUE(compare_file(test_filename, "1,\"abc\",1.1\n"
                                            "2,\"asdf\",2.2\n"
                                            "3,\"hello\",3.3\n"));
}
...
TEST_F(CsvWriterTest, NonsensePath)
{
    ...
    EXPECT_THROW(CsvWriter::write(*table, "this/path/does/not/exist"),
                 std::exception);
}

TEST_F(CsvWriterTest, ExportNumericNullValues)
{
    auto new_table = load_table(
        "resources/test_data/tbl/int_float_with_null.tbl", ChunkOffset{4});
    CsvWriter::write(*new_table, test_filename);

    EXPECT_TRUE(file_exists(test_filename));
    EXPECT_TRUE(file_exists(test_meta_filename));
    EXPECT_TRUE(compare_file(test_filename, "12345,458.7\n"
                                            "123,\n"
                                            ",456.7\n"
                                            "1234,457.7\n"));
}
...
{
    auto new_table = load_table("resources/test_data/tbl/string_with_null.tbl",
                                ChunkOffset{4});
    CsvWriter::write(*new_table, test_filename);

    EXPECT_TRUE(file_exists(test_filename));
    EXPECT_TRUE(file_exists(test_meta_filename));
    EXPECT_TRUE(compare_file(test_filename, "\"xxx\"\n"
                                            "\"www\"\n"
                                            "\n"
                                            "\"zzz\"\n"));
}
