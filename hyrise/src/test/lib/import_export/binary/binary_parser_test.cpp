...
class BinaryParserMultiEncodingTest
    : public BinaryParserTest,
...
TEST_P(BinaryParserMultiEncodingTest, SingleChunkSingleFloatColumn)
{
    auto expected_table = std::make_shared<Table>(
        TableColumnDefinitions{{"a", DataType::Float, false}}, TableType::Data,
        ChunkOffset{5});
...
TEST_P(BinaryParserMultiEncodingTest, MultipleChunkSingleFloatColumn)
{
    ...
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{2});
    expected_table->append({5.5f});
    expected_table->append({13.0f});
    expected_table->append({16.2f});
...
TEST_P(BinaryParserMultiEncodingTest, AllTypesSegmentSorted)
{
    ...
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{2}, UseMvcc::Yes);
    expected_table->append({"AAAAA", 1, static_cast<int64_t>(100), 1.1f, 11.1});
    expected_table->append(
        {"BBBBBBBBBB", 2, static_cast<int64_t>(200), 2.2f, 22.2});
    expected_table->append(
        {"CCCCCCCCCCCCCCC", 3, static_cast<int64_t>(300), 3.3f, 33.3});
    expected_table->append(
        {"DDDDDDDDDDDDDDDDDDDD", 4, static_cast<int64_t>(400), 4.4f, 44.4});
...
TEST_P(BinaryParserMultiEncodingTest, EmptyStringsSegment)
{
    ...
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{10});
...
TEST_F(BinaryParserTest, LZ4MultipleBlocks)
{
    ...
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{20'000});

    for (int index = 0; index < 5000; ++index)
    {
        expected_table->append(
            {"AAAAA", 1, static_cast<int64_t>(100), 1.1f, 11.1});
        expected_table->append(
            {"BBBBBBBBBB", 2, static_cast<int64_t>(200), 2.2f, 22.2});
        expected_table->append(
            {"CCCCCCCCCCCCCCC", 3, static_cast<int64_t>(300), 3.3f, 33.3});
...
TEST_F(BinaryParserTest, TwoColumnsNoValues)
{
    ...
    auto expected_table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{30'000});
...
TEST_F(BinaryParserTest, SortColumnDefinitions)
{
    ...
    const auto chunk_1_sorted_columns = std::vector<SortColumnDefinition>{
        SortColumnDefinition{ColumnID{1}, SortMode::Descending}};
    expected_table->get_chunk(ChunkID{0})
        ->set_individually_sorted_by(chunk_0_sorted_columns);
    expected_table->get_chunk(ChunkID{1})
    ...
    EXPECT_TABLE_EQ_ORDERED(table, expected_table);
    EXPECT_EQ(table->get_chunk(ChunkID{0})->individually_sorted_by(),
              chunk_0_sorted_columns);
    EXPECT_EQ(table->get_chunk(ChunkID{1})->individually_sorted_by(),
              chunk_1_sorted_columns);
