...
class BinaryWriterMultiEncodingTest
...
TEST_F(BinaryWriterTest, FixedStringDictionarySingleChunk)
{
    ...
    ChunkEncoder::encode_all_chunks(
        table, SegmentEncodingSpec{EncodingType::FixedStringDictionary});
    BinaryWriter::write(*table, filename);
...
TEST_F(BinaryWriterTest, NullValuesFrameOfReferenceSegment)
{
    ...
    table->last_chunk()->finalize();
    ChunkEncoder::encode_all_chunks(
        table, SegmentEncodingSpec{EncodingType::FrameOfReference});
    BinaryWriter::write(*table, filename);
...
// A table with reference segments is materialized while exporting. The content
// of the export file should not be differ- ent from an exported table with
// ValueSegments and the same content.
TEST_F(BinaryWriterTest, AllTypesReferenceSegment)
{
    ...
    // To create reference segments, we perform a scan that emits all tuples. We
    // cannot use to_simple_reference_table(table) because it merges all input
    // chunks into one output chunk. We assume that the TableScan operator
    // produces one output segment per input segment.
    const auto table_wrapper = std::make_shared<TableWrapper>(table);
    table_wrapper->execute();
    const auto scan = create_table_scan(table_wrapper, ColumnID{1},
                                        PredicateCondition::NotEquals, 5);
    scan->execute();
    ...
    // The file should be equal to the dump of a table with ValueSegments. Thus,
    // we also dump an unencoded table and com- pare the files. We have to
    // create a new table since reference tables are exported with the default
    // target chunk size. However, the chunks have the same size as the exported
    // reference segments.
    ...
    const auto unencoded_table = std::make_shared<Table>(
        column_definitions, TableType::Data, std::move(chunks));
...
TEST_F(BinaryWriterTest, SortColumnDefinitions)
{
    ...
    const auto chunk_0_sorted_columns = std::vector<SortColumnDefinition>{
        SortColumnDefinition{ColumnID{0}},
        SortColumnDefinition{ColumnID{1}, SortMode::Descending}};
    const auto chunk_1_sorted_columns = std::vector<SortColumnDefinition>{
        SortColumnDefinition{ColumnID{1}, SortMode::Descending}};
    table->get_chunk(ChunkID{0})
        ->set_individually_sorted_by(chunk_0_sorted_columns);
    table->get_chunk(ChunkID{1})
        ->set_individually_sorted_by(chunk_1_sorted_columns);
...
TEST_P(BinaryWriterMultiEncodingTest, RunNullValues)
{
    ...
    auto null_values =
        pmr_vector<bool>{true, false, true, true, true, false, false, true};
