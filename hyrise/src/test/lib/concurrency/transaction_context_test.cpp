...
class TransactionContextTest : public BaseTest
{
  ...
  protected:
    void SetUp() override
    {
        ...
        // Insert Operator works with the Storage Manager, so the test table
 ...
 *
 * It calls the functor between a context’s assignment of a commit ID and its
 * final commit.
 */
class CommitFuncOp : public AbstractReadWriteOperator
{
  public:
    explicit CommitFuncOp(std::function<void()> func)
        : AbstractReadWriteOperator(OperatorType::Mock), _func{func}
    {
    }
