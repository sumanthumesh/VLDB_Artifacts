...
class StressTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        // Set scheduler so that we can execute multiple SQL statements on
        // separate threads.
...
{
    // Update a table with two entries and a chunk size of 2. This will lead to
    // a high number of transaction conflicts and many chunks being created
    auto table_a =
    ...
    {
        // We give this a lot of time, not because we usually need that long for
        // 100 threads to finish, but because sanitizers and other tools like
        // valgrind sometimes bring a high overhead.
...
{
    // An update-heavy load on a table with a ridiculously low target chunk
    // size, creating many new chunks. This is different from
    // TestTransactionConflicts, in that each thread has its own logical row and
    // no transaction conflicts occur. In the other test, a failed "mark for
    // deletion" (i.e., swap of the row's tid) would lead to no row being
    // appended.
    TableColumnDefinitions column_definitions;
    column_definitions.emplace_back("a", DataType::Int, false);
    column_definitions.emplace_back("b", DataType::Int, false);
    const auto table = std::make_shared<Table>(
        column_definitions, TableType::Data, ChunkOffset{3}, UseMvcc::Yes);
    Hyrise::get().storage_manager.add_table("table_b", table);

    const auto iterations_per_thread = 20;

    // Define the work package - the job id is used so that each thread has its
    // own logical row to work on
    ...
    const auto run = [&]()
    {
        ...
        for (auto iteration = 0; iteration < iterations_per_thread; ++iteration)
        {
            auto pipeline =
                SQLPipelineBuilder{
                    iteration == 0
                        ? std::string{"INSERT INTO table_b (a, b) VALUES ("} +
                              std::to_string(my_job_id) + ", 1)"
                        : std::string{"UPDATE table_b SET b = b + 1 WHERE a "
                                      "= "} +
                              std::to_string(my_job_id)}
                    .create_pipeline();
            const auto [status, _] = pipeline.get_result_table();
    ...
    for (auto &thread_future : thread_futures)
    {
        ...
        // 100 threads to finish, but because sanitizers and other tools like
        // valgrind sometimes bring a high overhead.
        if (thread_future.wait_for(std::chrono::seconds(600)) ==
            std::future_status::timeout)
        {
            ASSERT_TRUE(false)
                << "At least one thread got stuck and did not commit.";
    ...
    {
        auto pipeline =
            SQLPipelineBuilder{std::string{"SELECT MIN(b) FROM table_b"}}
                .create_pipeline();
        const auto [_, verification_table] = pipeline.get_result_table();
        EXPECT_EQ(*verification_table->get_value<int32_t>(ColumnID{0}, 0),
...
{
    // As ValueSegments store their null flags in a vector<bool>, which is not
    // safe to be modified concurrently, conflicts may (and have) occurred when
    // that vector was written without any type of protection.

    TableColumnDefinitions column_definitions;
    column_definitions.emplace_back("a", DataType::Int, false);
    column_definitions.emplace_back("b", DataType::Int, true);
    const auto table = std::make_shared<Table>(
        column_definitions, TableType::Data, Chunk::DEFAULT_SIZE, UseMvcc::Yes);
    Hyrise::get().storage_manager.add_table("table_c", table);

    const auto iterations_per_thread = 200;

    // Define the work package - each job writes a=job_id, b=(NULL or 1,
    // depending on job_id)
    std::atomic_int job_id{0};
    const auto run = [&]()
    {
        ...
        for (auto iteration = 0; iteration < iterations_per_thread; ++iteration)
        {
            ...
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"INSERT INTO table_c (a, b) VALUES ("} +
                    std::to_string(my_job_id) + ", " +
                    (my_job_id % 2 ? "NULL" : "1") + ")"}
                    .create_pipeline();
            const auto [status, _] = pipeline.get_result_table();
    ...
    auto pipeline =
        SQLPipelineBuilder{
            "SELECT a, COUNT(a), COUNT(b) FROM table_c GROUP BY a ORDER BY a"}
            .create_pipeline();
    const auto [_, verification_table] = pipeline.get_result_table();
    ...
    {
        EXPECT_EQ(*verification_table->get_value<int32_t>(ColumnID{0}, row),
                  row);
        EXPECT_EQ(*verification_table->get_value<int64_t>(ColumnID{1}, row),
                  iterations_per_thread);
        EXPECT_EQ(*verification_table->get_value<int64_t>(ColumnID{2}, row),
                  row % 2 ? 0 : iterations_per_thread);
    }
}
...
TEST_F(StressTest, NodeSchedulerStressTest)
{
    ...
    // Create a large number of nodes in a fake topology (many workers will
    ...
    const auto job_counts = std::vector<size_t>{
        node_count << 3u, node_count << 4u, node_count << 3u};
    ...
    // In the default case, tasks are added to node 0 when its load is low. In
    // this test, tasks cannot be processed until `start_jobs` is set, leading
    // to a high queue load that cannot be processed. New tasks that are
    // scheduled should thus be assigned to different TaskQueues to distribute
    // the load.
    auto second_worker = std::next(node_queue_scheduler->workers().cbegin());
    EXPECT_TRUE(std::any_of(second_worker,
                            node_queue_scheduler->workers().cend(),
                            [](const auto &worker)
    ...
    // Three batches of jobs have been concurrently scheduled. Check that the
    // incremented `num_finished_jobs` has the expected value.
    const auto job_count_sum =
        std::accumulate(job_counts.cbegin(), job_counts.cend(), size_t{0});
