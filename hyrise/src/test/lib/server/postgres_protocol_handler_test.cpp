...
class PostgresProtocolHandlerTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _protocol_handler = std::make_shared<
            PostgresProtocolHandler<boost::asio::posix::stream_descriptor>>(
            _mocked_socket->get_socket());
    }
...
TEST_F(PostgresProtocolHandlerTest, ReadStartupMessage)
{
    ...
    _mocked_socket->write(
        std::string{'\0', '\0', '\0', '\b', '\0', '\0', '\0', '\0'});
    EXPECT_EQ(_protocol_handler->read_startup_packet_header(), 0);

    // SSL request contains length (8 B) and SSL request code 80877103
    _mocked_socket->write(
        std::string{'\0', '\0', '\0', '\b', '\x04', '\xd2', '\x16', '\x2f'});
    // Server will wait for new message with authentication details. Message
    // contains length (12 B), protocol (0) and body (4 B). No body provided
    // here, since we throw it away anyway.
    _mocked_socket->write(
        std::string{'\0', '\0', '\0', '\f', '\0', '\0', '\0', '\0'});
...
{
    // Write string including type of new packet, discard them, and see if
    // packet type get correctly detected
    const std::string content = "garbageQ";
    _mocked_socket->write(content);
    _protocol_handler->read_startup_packet_body(
        static_cast<uint32_t>(content.size() - 1));
    EXPECT_EQ(_protocol_handler->read_packet_type(),
              PostgresMessageType::SimpleQueryCommand);
}
...
TEST_F(PostgresProtocolHandlerTest, SendAuthenticationResponse)
{
    ...
    EXPECT_EQ(static_cast<PostgresMessageType>(file_content.front()),
              PostgresMessageType::AuthenticationRequest);
    EXPECT_EQ(
        NetworkConversionHelper::get_message_length(file_content.cbegin() + 1),
        file_content.size() - 1);
}
...
TEST_F(PostgresProtocolHandlerTest, SendReadyForQuery)
{
    ...
    EXPECT_EQ(static_cast<PostgresMessageType>(file_content.front()),
              PostgresMessageType::ReadyForQuery);
    EXPECT_EQ(static_cast<TransactionStatusIndicator>(file_content.back()),
              TransactionStatusIndicator::Idle);
    EXPECT_EQ(
        NetworkConversionHelper::get_message_length(file_content.cbegin() + 1),
        file_content.size() - 1);
}
...
TEST_F(PostgresProtocolHandlerTest, SendRowDescriptionHeader)
{
    ...
    _protocol_handler->send_row_description_header(total_column_name_length,
                                                   column_count);
    _protocol_handler->force_flush();
    const std::string file_content = _mocked_socket->read();

    EXPECT_EQ(static_cast<PostgresMessageType>(file_content.front()),
              PostgresMessageType::RowDescription);
    EXPECT_EQ(NetworkConversionHelper::get_small_int(
                  file_content.begin() + sizeof(uint32_t) +
                  sizeof(PostgresMessageType)),
              column_count);
}
...
TEST_F(PostgresProtocolHandlerTest, SendRowDescription)
{
    ...
    _protocol_handler->send_row_description(column_name, object_id,
                                            sizeof(uint32_t));
    _protocol_handler->force_flush();
    const std::string file_content = _mocked_socket->read();

    auto start = 0;
    EXPECT_EQ(std::string(file_content.c_str(), column_name.size()),
              column_name);
    start += column_name.size() + sizeof('\0');
    // Misuse NetworkConversionHelper::get_message_length to get converted
    // uint32_t
    EXPECT_EQ(NetworkConversionHelper::get_message_length(file_content.begin() +
                                                          start),
              0);
    start += sizeof(uint32_t);
    EXPECT_EQ(
        NetworkConversionHelper::get_small_int(file_content.begin() + start),
        0);
...
TEST_F(PostgresProtocolHandlerTest, SendValuesAsStrings)
{
    ...
    _protocol_handler->send_data_row(
        {value1, value2, std::nullopt},
        static_cast<uint32_t>(value1.size() + value2.size()));
    _protocol_handler->force_flush();
    const std::string file_content = _mocked_socket->read();

    EXPECT_EQ(static_cast<PostgresMessageType>(file_content.front()),
              PostgresMessageType::DataRow);
...
TEST_F(PostgresProtocolHandlerTest, SendCommandComplete)
{
    ...
    EXPECT_EQ(static_cast<PostgresMessageType>(file_content.front()),
              PostgresMessageType::CommandComplete);
    EXPECT_EQ(std::string(file_content,
                          sizeof(PostgresMessageType) + sizeof(uint32_t),
                          completion_message.size()),
              completion_message);
    EXPECT_EQ(
        NetworkConversionHelper::get_message_length(file_content.cbegin() + 1),
        file_content.size() - 1);
...
TEST_F(PostgresProtocolHandlerTest, ReadParsePacket)
{
    ...
    _mocked_socket->write(query);
    _mocked_socket->write(std::string{"\0", 1});
    // Specify data type of parameter. This value has currently no effect, but
    // needs to be read.
    _mocked_socket->write(std::string{'\0', '\x01'});
    _mocked_socket->write(std::string{'\0', '\0', '\0', '\x17'});

    const auto &[statement_name_read, query_read] =
        _protocol_handler->read_parse_packet();
...
TEST_F(PostgresProtocolHandlerTest, ReadBindPacket)
{
    ...
    EXPECT_EQ(statement_information.parameters,
              std::vector<AllTypeVariant>{"test"});
}
...
TEST_F(PostgresProtocolHandlerTest, SendErrorMessage)
{
    ...
    const auto error_message = ErrorMessages{
        {PostgresMessageType::HumanReadableError, error_description}};
    _protocol_handler->send_error_message(error_message);
    const std::string file_content = _mocked_socket->read();

    auto start = 0;
    EXPECT_EQ(static_cast<PostgresMessageType>(file_content[start]),
              PostgresMessageType::ErrorResponse);
    start += sizeof(uint32_t) + sizeof(PostgresMessageType);
    EXPECT_EQ(static_cast<PostgresMessageType>(file_content[start]),
              PostgresMessageType::HumanReadableError);
    start += sizeof(PostgresMessageType);
    EXPECT_EQ(std::string(file_content, start, error_description.size()),
              error_description);
    EXPECT_EQ(
        NetworkConversionHelper::get_message_length(file_content.cbegin() + 1),
        file_content.size() - 1);
...
TEST_F(PostgresProtocolHandlerTest, SendExecutionInfo)
{
    ...
    auto start = 0;
    EXPECT_EQ(static_cast<PostgresMessageType>(file_content[start]),
              PostgresMessageType::Notice);
    start += sizeof(uint32_t) + sizeof(PostgresMessageType);
    EXPECT_EQ(static_cast<PostgresMessageType>(file_content[start]),
              PostgresMessageType::HumanReadableError);
    start += sizeof(PostgresMessageType);
    EXPECT_EQ(std::string(file_content, start, execution_information.size()),
