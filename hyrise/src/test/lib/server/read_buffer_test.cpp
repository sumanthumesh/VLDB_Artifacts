...
class ReadBufferTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _read_buffer =
            std::make_shared<ReadBuffer<boost::asio::posix::stream_descriptor>>(
                _mocked_socket->get_socket());
    }

    std::shared_ptr<MockSocket> _mocked_socket;
    std::shared_ptr<ReadBuffer<boost::asio::posix::stream_descriptor>>
        _read_buffer;
};
...
TEST_F(ReadBufferTest, ReadValues)
{
    ...
    _mocked_socket->write(std::string(
        reinterpret_cast<const char *>(&converted_short), sizeof(uint16_t)));
    _mocked_socket->write(std::string(
        reinterpret_cast<const char *>(&converted), sizeof(uint32_t)));
...
TEST_F(ReadBufferTest, ReadLargeString)
{
    ...
    _mocked_socket->write(original_content);
    EXPECT_EQ(_read_buffer->get_string(original_content.size(),
