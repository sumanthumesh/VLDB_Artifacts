...
// This class tests supported operations of the server implementation. This does
// not include statements with named portals which are used for CURSOR
// operations.
class ServerTestRunner : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _server_thread =
            std::make_unique<std::thread>(server_runner, std::ref(*_server));

        // Get randomly assigned port number for client connection
        _connection_string =
            "hostaddr=127.0.0.1 port=" + std::to_string(_server->server_port());
    ...
    void TearDown() override
    {
        ...
        // Give the server time to shut down gracefully before force-closing the
        // socket it's working on
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        _server_thread->join();
    ...
    std::unique_ptr<Server> _server = std::make_unique<Server>(
        ...
        SendExecutionInfo::No); // Port 0 to select random open port
...
{
    EXPECT_NE(std::dynamic_pointer_cast<NodeQueueScheduler>(
                  Hyrise::get().scheduler()),
              nullptr);
...
TEST_F(ServerTestRunner, TestSimpleSelect)
{
    ...
    // We use nontransactions because the regular transactions use "begin" and
    // "commit" keywords that we do not support. Nontransactions auto commit.
...
{
    const auto all_types_table = load_table(
        "resources/test_data/tbl/all_data_types_sorted.tbl", ChunkOffset{2});
    Hyrise::get().storage_manager.add_table("all_types_table", all_types_table);

    pqxx::connection connection{_connection_string};

    // We use nontransactions because the regular transactions use SQL that we
    // do not support. Nontransactions auto commit.
    pqxx::nontransaction transaction{connection};
    ...
    EXPECT_EQ(result[0].size(),
              static_cast<size_t>(all_types_table->column_count()));

    for (uint64_t row_id = 0; row_id < all_types_table->row_count(); row_id++)
    {
        const auto current_row = all_types_table->get_row(row_id);
        for (ColumnID column_count{0};
             column_count < all_types_table->column_count(); column_count++)
        {
            // Representation of NULL values in pqxx::field differ from result
            // of lexical_cast
            if (result[row_id][column_count].is_null())
            {
                EXPECT_EQ("NULL", boost::lexical_cast<std::string>(
                                      current_row[column_count]));
            }
            else
            {
                EXPECT_EQ(result[row_id][column_count].c_str(),
...
TEST_F(ServerTestRunner, TestCopyImport)
{
    ...
    transaction.exec(
        "COPY another_table FROM 'resources/test_data/tbl/int_float.tbl';");

    EXPECT_TRUE(Hyrise::get().storage_manager.has_table("another_table"));
    EXPECT_TABLE_EQ_ORDERED(
        Hyrise::get().storage_manager.get_table("another_table"), _table_a);
}
...
TEST_F(ServerTestRunner, TestInvalidCopyImport)
{
    ...
    // Ill-formed
    EXPECT_THROW(transaction.exec("COPY another_table FROM;"),
                 pqxx::broken_connection);

    // File is not existing
    EXPECT_THROW(
        transaction.exec("COPY another_table FROM 'not/existing/file.tbl';"),
        pqxx::broken_connection);

    // Unsupported file extension
    EXPECT_THROW(
        transaction.exec(
            "COPY another_table FROM 'resources/test_data/tbl/float';"),
        pqxx::broken_connection);
...
TEST_F(ServerTestRunner, TestCopyExport)
{
    ...
    EXPECT_TRUE(compare_files(_export_filename + ".bin",
                              "resources/test_data/bin/int_float.bin"));
}
...
TEST_F(ServerTestRunner, TestInvalidCopyExport)
{
    ...
    EXPECT_THROW(
        transaction.exec("COPY not_existing TO './does_not_work.tbl';"),
        pqxx::broken_connection);

    // Unsupported file extension
    EXPECT_THROW(transaction.exec("COPY table_a TO './does_not_work.mp3';"),
                 pqxx::broken_connection);
...
TEST_F(ServerTestRunner, TestCopyIntegration)
{
    ...
    EXPECT_TRUE(compare_files(_export_filename + ".bin",
                              "resources/test_data/bin/int_float_deleted.bin"));
    EXPECT_TRUE(compare_files(_export_filename + ".csv",
                              "resources/test_data/bin/int_float_deleted.csv"));
...
TEST_F(ServerTestRunner, TestInvalidStatement)
{
    ...
    EXPECT_THROW(transaction.exec("SELECT * FROM non_existent;"),
                 pqxx::broken_connection);
...
TEST_F(ServerTestRunner, TestTransactionCommit)
{
...

    {
        ...
        const auto verification_result =
            verification_transaction.exec("SELECT * FROM table_a;");
...
{
    // Test that open sessions are allowed to finish before the server is
    // destroyed. This is more relevant for tests than for the actual execution.
    // In "real-life", i.e., during our experiments, we usually simply kill the
    // server. In tests however, the server finishing while sessions might not
    // be completely finished could lead to issues like #1977.
    ...
    std::thread(
        [&]
        {
            ...
            pqxx::nontransaction transaction{connection};
            transaction.exec(
                "SELECT * FROM table_a t1, table_a t2 WHERE t1.a = 123");
        })
        .detach();

    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // No assertions here. If the server would destroy resources early, we would
    // expect to see errors in asan/tsan and segfaults in regular execution.
}
...
TEST_F(ServerTestRunner, TestInvalidPreparedStatement)
{
    ...
    EXPECT_THROW(connection.prepare(prepared_name, "SELECT * FROM WHERE a > ?"),
                 pqxx::broken_connection);

    // Well-formed but table does not exist
    EXPECT_ANY_THROW(connection.prepare(
        prepared_name, "SELECT * FROM non_existent WHERE a > ?"));

    // Wrong number of parameters
    connection.prepare(prepared_name,
                       "SELECT * FROM table_a WHERE a > ? and a > ?");
...
{
    // This test is by no means perfect, as it can show flaky behaviour. But it
    // is rather hard to get reliable tests with multiple concurrent connections
    // to detect a randomly (but often) occurring bug. This test will/can only
    // fail if a bug is present but it should not fail if no bug is present. It
    // just sends 100 parallel connections and if that fails, there probably is
    // a bug.
    const std::string sql = "SELECT * FROM table_a;";
    const auto expected_num_rows = _table_a->row_count();
    ...
    // Create the async objects and spawn them asynchronously (i.e., as their
    // own threads) Note that async has a bunch of issues:
    //  -
    //  https://stackoverflow.com/questions/12508653/what-is-the-issue-with-stdasync
    //  - Mastering the C++17 STL, pages 205f
    // TODO(anyone): Change this to proper threads+futures, or at least do not
    // reuse this code.
    ...
    {
        // We want a future to the thread running, so we can kill it after a
        // future.wait(timeout) or the test would freeze
        thread_futures.emplace_back(
            std::async(std::launch::async, connection_run));
    }

    // Wait for completion or timeout (should not occur)
    for (auto &thread_future : thread_futures)
    {
        // We give this a lot of time, not because we need that long for 100
        // threads to finish, but because sanitizers and other tools like
        // valgrind sometimes bring a high overhead that exceeds 10 seconds.
        if (thread_future.wait_for(std::chrono::seconds(150)) ==
            std::future_status::timeout)
        {
            ASSERT_TRUE(false)
                << "At least one thread got stuck and did not commit.";
            // Retrieve the future so that exceptions stored in its state are
            // thrown
            thread_future.get();
        }
    }
...
TEST_F(ServerTestRunner, TestTransactionConflicts)
{
    ...
    // expecting some conflicts on the way Also similar to
    // StressTest.TestTransactionConflicts, only that we go through the server
    auto initial_sum = int64_t{};
    {
        auto pipeline =
            SQLPipelineBuilder{std::string{"SELECT SUM(a) FROM table_a"}}
                .create_pipeline();
    ...
    const auto connection_run = [&]()
    {
        for (auto iteration = 0; iteration < iterations_per_thread; ++iteration)
        {
            ...
            try
            {
                const std::string sql = "UPDATE table_a SET a = a + 1 WHERE a "
                                        "= (SELECT MIN(a) FROM table_a);";
    ...
    for (auto &thread_future : thread_futures)
    {
        ...
        // threads to finish, but because sanitizers and other tools like
        // valgrind sometimes bring a high overhead that exceeds 10 seconds.
        if (thread_future.wait_for(std::chrono::seconds(180)) ==
            std::future_status::timeout)
        {
            ASSERT_TRUE(false)
                << "At least one thread got stuck and did not commit.";
    ...
    // We also want to see at least one conflict so that we can be sure that
    // conflict handling in the server works. In case this fails, start playing
    // with num_threads to provoke more conflicts.
    EXPECT_GE(conflicted_increments, 1);

    EXPECT_EQ(successful_increments + conflicted_increments,
