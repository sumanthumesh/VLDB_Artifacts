...
class WriteBufferTest : public BaseTest
{
  protected:
    void SetUp() override
    {
        ...
        _write_buffer = std::make_shared<
            WriteBuffer<boost::asio::posix::stream_descriptor>>(
            _mocked_socket->get_socket());
    }

    std::shared_ptr<MockSocket> _mocked_socket;
    std::shared_ptr<WriteBuffer<boost::asio::posix::stream_descriptor>>
        _write_buffer;
};
...
TEST_F(WriteBufferTest, Full)
{
    ...
    _write_buffer->put_string(
        std::string(_write_buffer->maximum_capacity(), 'a'),
...
TEST_F(WriteBufferTest, WriteValues)
{
    ...
    std::copy_n(file_content.begin(), sizeof(uint16_t),
                reinterpret_cast<char *>(&small_value));
    EXPECT_EQ(ntohs(small_value), 16);

    uint32_t value = 0;
    std::copy_n(file_content.begin() + sizeof(uint16_t), sizeof(uint32_t),
                reinterpret_cast<char *>(&value));
    EXPECT_EQ(ntohl(value), 32);

    uint64_t long_value = 0;
    std::copy_n(file_content.begin() + sizeof(uint16_t) + sizeof(uint32_t),
                +sizeof(uint64_t), reinterpret_cast<char *>(&long_value));
...
TEST_F(WriteBufferTest, WriteLargeString)
{
    const std::string original_content =
        std::string(SERVER_BUFFER_SIZE + 2u, 'a');
    _write_buffer->put_string(original_content, HasNullTerminator::No);
    // Since we use transfer_at_least when flushing the buffer we do not know
    // how many bytes are going to written. It is at least the amount bytes of
    // the characters to be inserted and at maximum the buffer size.
    EXPECT_TRUE((original_content.size() - _write_buffer->maximum_capacity()) <=
                    _mocked_socket->read().size() &&
                _mocked_socket->read().size() <=
                    _write_buffer->maximum_capacity());
    _write_buffer->flush();
