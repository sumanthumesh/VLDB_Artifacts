...
TEST_F(QueryHandlerTest, ExecutePipeline)
{
    ...
    EXPECT_PRED_FORMAT2(testing::IsSubstring, "Execution info:",
                        execution_information.pipeline_metrics);
    EXPECT_EQ(execution_information.root_operator_type,
              OperatorType::Projection);
}

TEST_F(QueryHandlerTest, CreatePreparedPlan)
{
    QueryHandler::setup_prepared_plan("test_statement",
                                      "SELECT * FROM table_a WHERE a > ?");

    EXPECT_TRUE(
        Hyrise::get().storage_manager.has_prepared_plan("test_statement"));
}
...
TEST_F(QueryHandlerTest, BindParameters)
{
    ...
    const auto specification =
        PreparedStatementDetails{"test_statement", "", {12345}};

    const auto bound_plan = QueryHandler::bind_prepared_plan(specification);
    EXPECT_EQ(bound_plan->type(), OperatorType::Validate);

    const auto get_table = std::dynamic_pointer_cast<const GetTable>(
        bound_plan->left_input()->left_input());
    ASSERT_TRUE(get_table);

    // Check that the optimizer was executed. We cannot distinguish an optimized
    // PQP from an unoptimized PQP, so we check whether the chunk pruning
    // information was set in the GetTable operator. That would have been done
    // by the ChunkPruningRule, which could not have been successful before the
    // bound value (12345) was known.
...
TEST_F(QueryHandlerTest, ExecutePreparedStatement)
{
    QueryHandler::setup_prepared_plan("test_statement",
                                      "SELECT * FROM table_a WHERE a > ?");
    const auto specification =
        PreparedStatementDetails{"test_statement", "", {123}};
