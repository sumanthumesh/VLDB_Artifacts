...
{
    const std::string query = "BEGIN; CREATE TABLE users (id INT); INSERT INTO "
                              "users(id) VALUES (1); COMMIT;";
...
TEST_F(TransactionHandlingTest, TestTransactionContextInternals)
{
...

    {
        ...
        // when the user begins a transaction, a new transaction context is
        // created internally (not in "auto-commit" mode) the transaction is
        // therefore still active until the user either rolls back or commits
...

    {
        ...
        // the transaction context is in the successful state of having been
        // rolled back on purpose
        EXPECT_EQ(transaction_ctx->phase(), TransactionPhase::RolledBackByUser);
        EXPECT_TRUE(execution_information.error_messages.empty());

        // internally the transaction context returned by the pipeline is
        // nullptr in order to force creating a new one in the next pipeline
        // execution
        transaction_ctx = execution_info_transaction_context_pair.second;
        EXPECT_EQ(transaction_ctx, nullptr);
    }
...
// unit test ensuring that transactions don't see the changes of other
// uncommitted transactions read more about visibility of transaction changes on
// the MVCC wiki page
TEST_F(TransactionHandlingTest, TestTransactionSideEffects)
{
...

    {
        ...
        auto execution_info_transaction_context_pair_2 =
            QueryHandler::execute_pipeline(query_2, SendExecutionInfo::Yes,
                                           nullptr);

        auto execution_information_2 =
            execution_info_transaction_context_pair_2.first;
...
{
    {
        auto transaction_ctx =
            Hyrise::get().transaction_manager.new_transaction_context(
                AutoCommit::No);
        std::string query = "BEGIN";
        EXPECT_THROW(QueryHandler::execute_pipeline(
                         query, SendExecutionInfo::Yes, transaction_ctx),
...

    {
        ...
        EXPECT_THROW(QueryHandler::execute_pipeline(
                         query, SendExecutionInfo::Yes, nullptr),
                     InvalidInputException);
    }
