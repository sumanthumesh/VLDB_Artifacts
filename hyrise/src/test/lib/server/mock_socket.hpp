...
// Instead of writing data to the network device this class puts the information
// into a file. This allows an easier verification of input and output and works
...
class MockSocket
{
  public:
    ...
    {
        _file_descriptor =
            open(_path.c_str(), O_RDWR | O_CREAT | O_APPEND, 0755);
        _stream = std::make_shared<AsioStreamDescriptor>(_io_service,
                                                         _file_descriptor);
        _io_service.run();
    }
    ...
    {
        std::ofstream(std::filesystem::path{_path}, std::ios_base::app)
            << value;
    }
    ...
    std::string read()
    {
        ...
        return {std::istreambuf_iterator<char>(AsioStreamDescriptor),
                std::istreambuf_iterator<char>()};
    }

    bool empty()
    {
        return std::ifstream(std::filesystem::path{_path}).peek() ==
...
class NetworkConversionHelper
{
  public:
    static uint32_t get_message_length(std::string::const_iterator start)
    {
        ...
        std::copy_n(start, sizeof(uint32_t),
                    reinterpret_cast<char *>(&network_value));
    ...
    static uint16_t get_small_int(std::string::const_iterator start)
    {
        ...
        std::copy_n(start, sizeof(uint16_t),
                    reinterpret_cast<char *>(&network_value));
