...
class ResultSerializerTest : public BaseTest
{
  protected:
    ...
    {
        _test_table =
            load_table("resources/test_data/tbl/all_data_types_sorted.tbl",
                       ChunkOffset{2});
...
TEST_F(ResultSerializerTest, RowDescription)
{
    ...
    EXPECT_EQ(
        ...
        file_content.size() - 1);
    EXPECT_EQ(NetworkConversionHelper::get_small_int(file_content.cbegin() + 5),
              _test_table->column_count());
    for (auto column_id = ColumnID{0}; column_id < _test_table->column_count();
         column_id++)
    {
        EXPECT_NE(file_content.find(_test_table->column_name(column_id)),
                  std::string::npos);
    }
}
...
TEST_F(ResultSerializerTest, QueryResponse)
{
    ...
    EXPECT_EQ(std::count(file_content.begin(), file_content.end(), 'D'),
              _test_table->row_count());
}

TEST_F(ResultSerializerTest, CommandCompleteMessage)
{
    EXPECT_EQ(ResultSerializer::build_command_complete_message(
                  OperatorType::Insert, 1),
              "INSERT 0 1");
    EXPECT_EQ(ResultSerializer::build_command_complete_message(
                  OperatorType::Update, 1),
              "UPDATE -1");
    EXPECT_EQ(ResultSerializer::build_command_complete_message(
                  OperatorType::Delete, 1),
              "DELETE -1");
    EXPECT_EQ(ResultSerializer::build_command_complete_message(
                  OperatorType::Projection, 1),
              "SELECT 1");
}
