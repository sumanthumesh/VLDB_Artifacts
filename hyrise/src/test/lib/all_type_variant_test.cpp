...
template <typename T> class AllTypeVariantTest : public BaseTest
{
};

using AllTypeVariantTestDataTypes =
    ::testing::Types<int32_t, int64_t, float, double, pmr_string>;
TYPED_TEST_SUITE(AllTypeVariantTest,
                 AllTypeVariantTestDataTypes, ); // NOLINT(whitespace/parens)
...
TYPED_TEST(AllTypeVariantTest, GetExtractsExactNumericalValue)
{
    ...
    if constexpr (std::is_same_v<TypeParam, pmr_string>)
    {
        ...
        values.emplace_back(
            pmr_string{"reallyreallylongstringthatcantbestoredusingsso"});
    }
    else
    {
