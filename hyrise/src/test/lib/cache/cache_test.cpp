...
class CachePolicyTest : public BaseTest
{
  protected:
    ...
    template <typename Key, typename Value>
    const boost::heap::fibonacci_heap<
        typename GDFSCache<Key, Value>::GDFSCacheEntry> &
    queue(const GDFSCache<Key, Value> &cache) const
    {
        return cache._queue;
    }

    template <typename Key, typename Value>
    const typename GDFSCache<Key, Value>::GDFSCacheEntry
    get_full_entry(const GDFSCache<Key, Value> &cache, const Key &key) const
    {
