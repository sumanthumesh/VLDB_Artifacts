...
class MockCostEstimator : public AbstractCostEstimator
{
  public:
    ...
    Cost estimate_node_cost(
        const std::shared_ptr<AbstractLQPNode> &node) const override
    {
...
TEST_F(AbstractCostEstimatorTest, DiamondShape)
{
    ...
    const auto predicate_a =
        PredicateNode::make(less_than_equals_(a_a, 5), node_a);
    ...
    const auto union_node =
        UnionNode::make(SetOperationMode::Positions, predicate_b, predicate_c);

    const auto mock_costs = MockCosts{{node_a, 13.0f},
                                      {predicate_a, 1.0f},
                                      {predicate_b, 3.0f},
                                      {predicate_c, 5.0f},
                                      {union_node, 7.0f}};

    EXPECT_EQ(MockCostEstimator{mock_costs}.estimate_plan_cost(union_node),
              29.0f);
}
...
TEST_F(AbstractCostEstimatorTest, PlanCostCache)
{
    ...
    cost_estimator.cost_estimation_by_lqp_cache->emplace(
        node_a, 1000.0f); // Should not be retrieved from cache
    cost_estimator.cost_estimation_by_lqp_cache->emplace(predicate_b, 77.0f);

    // This should sum the cost of predicate_c with the cached cost for its
    // input plan
    ...
    EXPECT_EQ(cost_estimator.cost_estimation_by_lqp_cache->count(predicate_c),
              1u);
    EXPECT_EQ(cost_estimator.cost_estimation_by_lqp_cache->at(predicate_c),
              82.0f);
}
...
TEST_F(AbstractCostEstimatorTest, PlanCostCacheDiamondShape)
{
    ...
    const auto union_node =
        UnionNode::make(SetOperationMode::Positions, predicate_d, predicate_c);

    const auto mock_costs = MockCosts{{node_a, 13.0f},      {predicate_a, 1.0f},
                                      {predicate_b, 3.0f},  {predicate_c, 5.0f},
                                      {predicate_d, 99.0f}, {union_node, 9.0f}};
    ...
    // This should sum the cost of predicate_d, union_node with the cached cost
    // for predicate_c
    EXPECT_EQ(cost_estimator.estimate_plan_cost(union_node),
              115.0f + 5.0f + 9.0f);
