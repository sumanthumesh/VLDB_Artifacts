...
create_dict_segment_by_type(DataType data_type,
                            const std::vector<std::optional<T>> &values)
{
    ...
    const auto &dict_segment = ChunkEncoder::encode_segment(
        value_segment, data_type,
...
#define INSTANTIATE_CREATE_DICT_SEGMENT(r, data, type)                         \
    template std::shared_ptr<DictionarySegment<type>>                          \
    create_dict_segment_by_type(                                               \
...
get_column_expression(const std::shared_ptr<AbstractOperator> &op,
                      const ColumnID column_id)
{
    ...
    const auto &column_definition =
        output_table->column_definitions().at(column_id);

    return pqp_column_(column_id, column_definition.data_type,
                       column_definition.nullable, column_definition.name);
}

std::shared_ptr<TableScan> create_table_scan(
    const std::shared_ptr<AbstractOperator> &in, const ColumnID column_id,
    ...
    const std::optional<AllTypeVariant> &value2)
{
    ...
    {
        predicate = std::make_shared<IsNullExpression>(predicate_condition,
                                                       column_expression);
    }
    ...
    else
    {
        predicate = std::make_shared<BinaryPredicateExpression>(
            predicate_condition, column_expression, value_(value));
    }

    return std::make_shared<TableScan>(in, predicate);
}

void set_statistics_for_mock_node(
    const std::shared_ptr<MockNode> &mock_node, const size_t row_count,
    const std::vector<std::shared_ptr<AbstractStatisticsObject>>
        &statistics_objects)
{
    ...
    for (auto column_id = ColumnID{0}; column_id < column_definitions.size();
         ++column_id)
    {
        resolve_data_type(
            column_definitions[column_id].first,
...
std::shared_ptr<MockNode> create_mock_node_with_statistics(
    ...
    const std::vector<std::shared_ptr<AbstractStatisticsObject>>
        &statistics_objects)
{
    Assert(column_definitions.size() == statistics_objects.size(),
...
create_between_table_scan(const std::shared_ptr<AbstractOperator> &in,
                          ...
                          const PredicateCondition predicate_condition)
{
    ...
    const auto predicate = std::make_shared<BetweenExpression>(
        predicate_condition, column_expression, value_(value), value_(*value2));

    return std::make_shared<TableScan>(in, predicate);
}

ChunkEncodingSpec create_compatible_chunk_encoding_spec(
    const Table &table, const SegmentEncodingSpec &desired_segment_encoding)
{
    auto chunk_encoding_spec = ChunkEncodingSpec{
        table.column_count(), SegmentEncodingSpec{EncodingType::Unencoded}};

    for (auto column_id = ColumnID{0}; column_id < table.column_count();
         ++column_id)
    {
        if (encoding_supports_data_type(desired_segment_encoding.encoding_type,
...
void assert_chunk_encoding(const std::shared_ptr<Chunk> &chunk,
                           const ChunkEncodingSpec &spec)
{
    ...
    for (auto column_id = ColumnID{0u}; column_id < column_count; ++column_id)
    {
        ...
        ASSERT_EQ(segment_spec.encoding_type,
                  get_segment_encoding_spec(segment).encoding_type);
        ASSERT_TRUE(
            !segment_spec.vector_compression_type ||
            (*get_segment_encoding_spec(segment).vector_compression_type ==
             *segment_spec.vector_compression_type));
        if (segment_spec.encoding_type == EncodingType::Unencoded)
        {
            const auto value_segment =
                std::dynamic_pointer_cast<const BaseValueSegment>(segment);
            ASSERT_NE(value_segment, nullptr);
        }
        else
        {
            const auto encoded_segment =
                std::dynamic_pointer_cast<const AbstractEncodedSegment>(
                    segment);
            ASSERT_NE(encoded_segment, nullptr);
            ASSERT_EQ(encoded_segment->encoding_type(),
                      segment_spec.encoding_type);
            if (segment_spec.vector_compression_type &&
                encoded_segment->compressed_vector_type())
            {
                // Both optionals need to be set for comparison, because some
                // encodings only use vector compression for certain types
                // (e.g., LZ4 for pmr_string segments).
                ASSERT_EQ(*segment_spec.vector_compression_type,
                          parent_vector_compression_type(
                              *encoded_segment->compressed_vector_type()));
            }
        }
    }
...
std::vector<SegmentEncodingSpec>
get_supporting_segment_encodings_specs(const DataType data_type,
                                       const bool include_unencoded)
{
    ...
    {
        // Add all encoding types to the returned vector if they support the
        // given data type. As some test cases work on encoded segments only, it
        // is further tested if the segment is not encoded and if segments of
        // type Unencoded should be included or not (flag `include_unencoded`).
        if (encoding_supports_data_type(spec.encoding_type, data_type) &&
            (spec.encoding_type != EncodingType::Unencoded ||
             include_unencoded))
        {
...
bool compare_files(const std::string &original_file,
                   const std::string &created_file)
{
    std::ifstream original(original_file);
    Assert(original.is_open(),
           "compare_file: Could not find file " + original_file);

    std::ifstream created(created_file);
    Assert(created.is_open(),
           "compare_file: Could not find file " + created_file);
...
to_simple_reference_table(const std::shared_ptr<const Table> &table)
{
    Assert(table->type() == TableType::Data,
           "Input table already is a reference table");

    auto pos_list = std::make_shared<RowIDPosList>();
    pos_list->reserve(table->row_count());

    for (auto chunk_id = ChunkID{0u}; chunk_id < table->chunk_count();
         ++chunk_id)
    {
        const auto chunk = table->get_chunk(chunk_id);

        const auto chunk_size = chunk->size();
        for (auto chunk_offset = ChunkOffset{0u}; chunk_offset < chunk_size;
             ++chunk_offset)
        {
    ...
    for (auto column_id = ColumnID{0}; column_id < column_count; ++column_id)
    {
        ...
        auto segment_out =
            std::make_shared<ReferenceSegment>(table, column_id, pos_list);
