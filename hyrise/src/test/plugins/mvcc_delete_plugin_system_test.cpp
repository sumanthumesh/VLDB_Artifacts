...
class MvccDeletePluginSystemTest : public BaseTest
{
  public:
    /**
     * Create a table with three (INITIAL_CHUNK_COUNT) chunks of CHUNK_SIZE rows
     * each. The column number contains increasing integer values, starting from
     * zero.
     */
    void SetUp() override
    {
        ...
        auto begin_value = 0;
        for (auto chunk_id = ChunkID{0}; chunk_id < INITIAL_CHUNK_COUNT;
             ++chunk_id)
        {
            ...
            const auto value_segment =
                std::make_shared<ValueSegment<int>>(std::move(values));
            auto segments = Segments{};
            segments.emplace_back(value_segment);
            const auto mvcc_data = std::make_shared<MvccData>(
                segments.front()->size(), CommitID{0});
  ...
  protected:
    /**
     * Updates a single row to make it invalid in its chunk. Data modification
     * is not involved, so the row gets reinserted at the end of the table.
     * - Updates start at position 220 (INITIAL_UPDATE_OFFSET), so the first
     * chunk stays untouched.
     * - Updates stop just before the end of Chunk 3 (at position 598), so that
     * it is "fresh" and not cleaned up.
     */
    void update_next_row()
    {
        ...
        auto column = expression_functional::pqp_column_(
            ColumnID{0}, DataType::Int, false, "number");
        ...
        const auto expr =
            expression_functional::equals_(column, static_cast<int>(_counter));
        const auto where = std::make_shared<TableScan>(validate, expr);
        where->set_transaction_context(transaction_context);

        const auto update =
            std::make_shared<Update>(_t_name_test, where, where);
    ...
    void validate_table()
    {
        ...
        // sum(0, 1, ... , 599) = 179'700
        EXPECT_EQ(aggregate->get_output()->get_value<int64_t>(ColumnID{0}, 0),
                  179'700);
    }
...
/**
 * Tests the logical and physical delete operations of the MvccDeletePlugin in
 * practice.
 */
TEST_F(MvccDeletePluginSystemTest, CheckPlugin)
{
    ...
    // While it exists, no physical delete should be performed because the
    // context might operate on old rows.
    auto blocker_transaction_context =
        Hyrise::get().transaction_manager.new_transaction_context(
            AutoCommit::No);

    // (4) Prepare clean-up of chunk 2
    // (4.1) Create and run a thread that invalidates and reinserts rows of
    // chunk 2 and 3 It calls update_next_row() continuously. As a
    // PausableLoopThread, it gets terminated together with the test.
    auto table_update_thread = std::make_unique<PausableLoopThread>(
        std::chrono::milliseconds(10), [&](size_t) { update_next_row(); });
    ...
    // High number of attempts chosen so that even slow builds (especially
    // sanitizers) can finish
    ...
    {
        // Updates started from row 220 on. So chunk 2 contained 20 rows still
        // valid before its logical deletion. These rows must have been
        // invalidated and reinserted to the table during the logical delete
        // operation by the MvccDeletePlugin.
        validate_table();
    }

    // (7) Set a prerequisite for the physical delete of chunk 2
    // So far the active-state of the following TransactionContext's
    // snapshot-commit-id prevented a physical delete.
    {
        auto blocker_snapshot_cid =
            blocker_transaction_context->snapshot_commit_id();
        auto lowest_active_snapshot_cid =
            Hyrise::get()
                .transaction_manager.get_lowest_active_snapshot_commit_id();
        EXPECT_TRUE(lowest_active_snapshot_cid &&
                    lowest_active_snapshot_cid <= blocker_snapshot_cid);

        // Make snapshot-cid inactive
        blocker_transaction_context = nullptr;

        lowest_active_snapshot_cid =
            Hyrise::get()
                .transaction_manager.get_lowest_active_snapshot_commit_id();
        EXPECT_TRUE(!lowest_active_snapshot_cid ||
                    lowest_active_snapshot_cid > blocker_snapshot_cid);
    ...
    // (8) Wait for the MvccDeletePlugin to delete chunk 2 physically
    {
        ...
        while (attempts_remaining--)
        {
            ...
            // Not yet. Give the plugin some more time.
            std::this_thread::sleep_for(
                MvccDeletePlugin::IDLE_DELAY_PHYSICAL_DELETE);
        }
    ...
    // (9) Check after conditions
    {
        ...
        // Chunk 3 was the last to be modified, so it should not have been
        // cleaned up either (compare criterion 2).
        const auto chunk3 = _table->get_chunk(ChunkID{2});
        EXPECT_TRUE(chunk3);
        EXPECT_FALSE(chunk3->get_cleanup_commit_id());
    }

    // (10) Create a blocker for the physical delete of chunk 3, similar to step
    // (3)
    blocker_transaction_context =
        Hyrise::get().transaction_manager.new_transaction_context(
            AutoCommit::No);
    {
        const auto chunk3 = _table->get_chunk(ChunkID{2});
        EXPECT_TRUE(
            chunk3 &&
            !chunk3
                 ->get_cleanup_commit_id()); // otherwise our blocker won't work
    }
    ...
    // (11) Prepare clean-up of chunk 3
    {
        ...
        // Kill a couple of commit IDs so that criterion 2 is fulfilled and
        // chunk 3 is eligible for clean-up, too.
        for (auto transaction_idx = CommitID{0};
             transaction_idx < MvccDeletePlugin::DELETE_THRESHOLD_LAST_COMMIT;
             ++transaction_idx)
        {
            // To increase the global _last_commit_id, we need to execute a
            // transaction with read-write operators We perform some dummy
            // updates so that the table is unmodified and the validation
            // routine does not complain
            auto pipeline =
                SQLPipelineBuilder{
                    std::string{"UPDATE " + _t_name_test +
                                " SET number = number WHERE number = -1"}}
                    .create_pipeline();

            // Execute and verify update transaction
            EXPECT_EQ(pipeline.get_result_table().first,
                      SQLPipelineStatus::Success);
        }
    }
