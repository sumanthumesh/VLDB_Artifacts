...
class MvccDeletePluginTest : public BaseTest
{
  public:
    ...
    {
        const auto &table =
            load_table("resources/test_data/tbl/int3.tbl", _chunk_size);
  ...
  protected:
    void _increment_all_values_by_one()
    {
        ...
        auto updated_values_projection =
            std::make_shared<Projection>(validate_table, update_expressions);
        updated_values_projection->execute();
        auto update_table = std::make_shared<Update>(
            _table_name, validate_table, updated_values_projection);
    ...
    static bool _try_logical_delete(const std::string &table_name,
                                    ChunkID chunk_id)
    {
        auto transaction_context =
            Hyrise::get().transaction_manager.new_transaction_context(
    ...
    static bool
    _try_logical_delete(const std::string &table_name, ChunkID chunk_id,
    ...
    static void _delete_chunk_physically(const std::string &table_name,
                                         ChunkID chunk_id)
    {
        MvccDeletePlugin::_delete_chunk_physically(
            Hyrise::get().storage_manager.get_table(table_name), chunk_id);
    }

    static int
    _get_int_value_from_table(const std::shared_ptr<const Table> &table,
                              const ChunkID chunk_id, const ColumnID column_id,
                              const ChunkOffset chunk_offset)
    {
        const auto &segment =
            table->get_chunk(chunk_id)->get_segment(column_id);
        const auto &value_alltype =
            static_cast<const AllTypeVariant &>((*segment)[chunk_offset]);
...
TEST_F(MvccDeletePluginTest, LoadUnloadPlugin)
{
    ...
    EXPECT_NO_THROW(plugin_manager.load_plugin(
        build_dylib_path("libhyriseMvccDeletePlugin")));
...
/**
 * This test checks the logical delete. All values in the table are incremented
 * to generate three invalidated rows and create a second chunk. Before the
 * logical delete is performed, the first chunk contains a mix of valid and
 * invalidated lines. After the logical delete, all its rows are invalidated and
 * a cleanup_commit_id was set, which is used for the physical delete. All
 * values are now located in the second chunk. When fetching the table, the
 * fully invalidated chunk is not visible anymore for transactions.
 */
TEST_F(MvccDeletePluginTest, LogicalDelete)
{
    ...
    EXPECT_EQ(_get_int_value_from_table(table, ChunkID{0}, ColumnID{0},
                                        ChunkOffset{0}),
              1);
    EXPECT_EQ(_get_int_value_from_table(table, ChunkID{0}, ColumnID{0},
                                        ChunkOffset{1}),
    ...
    EXPECT_EQ(_get_int_value_from_table(table, ChunkID{1}, ColumnID{0},
                                        ChunkOffset{0}),
              2);
    EXPECT_EQ(_get_int_value_from_table(table, ChunkID{1}, ColumnID{0},
                                        ChunkOffset{1}),
    ...
    // --- Invalidate records - so that chunk 1 is completely invalidated except
    // for one record
    _increment_all_values_by_one();
    ...
    EXPECT_EQ(_get_int_value_from_table(table, ChunkID{1}, ColumnID{0},
                                        ChunkOffset{3}),
              3);
    EXPECT_EQ(_get_int_value_from_table(table, ChunkID{2}, ColumnID{0},
                                        ChunkOffset{0}),
              4);
    EXPECT_EQ(_get_int_value_from_table(table, ChunkID{2}, ColumnID{0},
                                        ChunkOffset{1}),
...
TEST_F(MvccDeletePluginTest, LogicalDeleteConflicts)
{
    ...
    {
        const auto conflicting_sql =
            "DELETE FROM " + _table_name + " WHERE a < 4";
    ...
    EXPECT_FALSE(
        _try_logical_delete(_table_name, ChunkID{1}, transaction_context));
    EXPECT_EQ(transaction_context->phase(),
