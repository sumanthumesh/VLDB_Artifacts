...
class UccDiscoveryPluginTest : public BaseTest
{
  public:
    void SetUp() override
    {
        _table_A = load_table("resources/test_data/tbl/uniqueness_test_A.tbl",
                              _chunk_size);
        _table_B = load_table("resources/test_data/tbl/uniqueness_test_B.tbl",
                              _chunk_size);
  ...
  protected:
    ...
    void _discover_uccs()
    {
        UccDiscoveryPlugin::_validate_ucc_candidates(
            UccDiscoveryPlugin::_identify_ucc_candidates());
    }
...
class UccDiscoveryPluginMultiEncodingTest
    : public UccDiscoveryPluginTest,
...
TEST_F(UccDiscoveryPluginTest, LoadUnloadPlugin)
{
    ...
    EXPECT_NO_THROW(plugin_manager.load_plugin(
        build_dylib_path("libhyriseUccDiscoveryPlugin")));
...
TEST_F(UccDiscoveryPluginTest, DescriptionAndProvidedFunction)
{
    ...
    EXPECT_EQ(plugin.description(),
              "Unary Unique Column Combination Discovery Plugin");
    const auto &provided_functions =
        plugin.provided_user_executable_functions();
...
TEST_F(UccDiscoveryPluginTest, UserCallableFunction)
{
    ...
    EXPECT_NO_THROW(plugin_manager.load_plugin(
        build_dylib_path("libhyriseUccDiscoveryPlugin")));
    EXPECT_NO_THROW(plugin_manager.exec_user_function(
        "hyriseUccDiscoveryPlugin", "DiscoverUCCs"));
    EXPECT_NO_THROW(plugin_manager.unload_plugin("hyriseUccDiscoveryPlugin"));
}
...
TEST_F(UccDiscoveryPluginTest, BenchmarkHooks)
{
    ...
    EXPECT_NO_THROW(plugin_manager.load_plugin(
        build_dylib_path("libhyriseUccDiscoveryPlugin")));
    // We only check if the plugin has a pre-benchmark hook. Actually executing
    // it requires benchmark items and tables.
    EXPECT_TRUE(
        plugin_manager.has_pre_benchmark_hook("hyriseUccDiscoveryPlugin"));
    EXPECT_FALSE(
        plugin_manager.has_post_benchmark_hook("hyriseUccDiscoveryPlugin"));
    auto report = nlohmann::json{};
    EXPECT_THROW(plugin_manager.exec_post_benchmark_hook(
                     "hyriseUccDiscoveryPlugin", report),
                 std::logic_error);
    EXPECT_NO_THROW(plugin_manager.unload_plugin("hyriseUccDiscoveryPlugin"));
}
...
TEST_F(UccDiscoveryPluginTest, CorrectCandidatesGeneratedForJoin)
{
    ...
    JoinNode::make(join_mode, equals_(_join_columnA, _join_columnB),
      ...
      PredicateNode::make(equals_(_predicate_column_B, "not"), _table_node_B));
        ...
        SCOPED_TRACE("for JoinMode::" +
                     std::string{magic_enum::enum_name(join_mode)});

        // For semi joins, the plugin should only look at the right join input.
        const auto expected_candidate_count =
            join_mode == JoinMode::Inner ? 4 : 2;
        EXPECT_EQ(ucc_candidates.size(), expected_candidate_count);

        const auto join_column_A_candidate =
            UccCandidate{_table_name_A, _join_columnA->original_column_id};
        const auto predicate_column_A_candidate = UccCandidate{
            _table_name_A, _predicate_column_A->original_column_id};
        const auto join_column_B_candidate =
            UccCandidate{_table_name_B, _join_columnB->original_column_id};
        const auto predicate_column_B_candidate = UccCandidate{
            _table_name_B, _predicate_column_B->original_column_id};
...
{
    const auto join_modes = {JoinMode::Left,
                             JoinMode::Right,
                             JoinMode::FullOuter,
        ...
        const auto lqp =
            join_mode == JoinMode::Cross
                ...
                : JoinNode::make(join_mode,
                                 equals_(_join_columnA, _join_columnB));
        lqp->set_left_input(PredicateNode::make(
            equals_(_predicate_column_A, "unique"), _table_node_A));
        lqp->set_right_input(PredicateNode::make(
            equals_(_predicate_column_B, "not"), _table_node_B));
...
TEST_F(UccDiscoveryPluginTest, CorrectCandidatesGeneratedForAggregate)
{
  ...
  AggregateNode::make(expression_vector(_join_columnA), expression_vector(),
    ...
    const auto predicate_column_A_candidate =
        UccCandidate{_table_name_A, _predicate_column_A->original_column_id};
...
TEST_P(UccDiscoveryPluginMultiEncodingTest, ValidateCandidates)
{
    ...
    EXPECT_TRUE(
        constraints_A.contains({{ColumnID{0}}, KeyConstraintType::UNIQUE}));
    EXPECT_TRUE(
        constraints_A.contains({{ColumnID{2}}, KeyConstraintType::UNIQUE}));
    EXPECT_FALSE(
        constraints_A.contains({{ColumnID{1}}, KeyConstraintType::UNIQUE}));

    EXPECT_TRUE(
        constraints_B.contains({{ColumnID{0}}, KeyConstraintType::UNIQUE}));
    EXPECT_FALSE(
        constraints_B.contains({{ColumnID{1}}, KeyConstraintType::UNIQUE}));
}
...
TEST_P(UccDiscoveryPluginMultiEncodingTest, PluginFullRun)
{
  ...
  JoinNode::make(JoinMode::Inner, equals_(_join_columnA, _join_columnB),
    ...
    // Run UCC discovery: we expect both join columns and the predicate on table
    // A to be identified as unique.
    _discover_uccs();
    ...
    EXPECT_TRUE(constraints_A.contains(
        {{_join_columnA->original_column_id}, KeyConstraintType::UNIQUE}));
    EXPECT_TRUE(
        constraints_A.contains({{_predicate_column_A->original_column_id},
                                KeyConstraintType::UNIQUE}));

    EXPECT_TRUE(constraints_B.contains(
        {{_join_columnB->original_column_id}, KeyConstraintType::UNIQUE}));
    EXPECT_FALSE(
        constraints_B.contains({{_predicate_column_B->original_column_id},
                                KeyConstraintType::UNIQUE}));
...
INSTANTIATE_TEST_SUITE_P(UccDiscoveryPluginMultiEncodingTestInstances,
                         UccDiscoveryPluginMultiEncodingTest,
