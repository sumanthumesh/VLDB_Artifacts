...
ChunkEncodingSpec get_chunk_encoding_spec(const Chunk &chunk)
{
    ...
    for (auto column_id = ColumnID{0}; column_id < chunk.column_count();
         ++column_id)
    {
        const auto &abstract_segment = chunk.get_segment(column_id);
        chunk_encoding_spec[column_id] =
            get_segment_encoding_spec(abstract_segment);
    }

    return chunk_encoding_spec;
}

bool is_chunk_encoding_spec_satisfied(
    const ChunkEncodingSpec &expected_chunk_encoding_spec,
    const ChunkEncodingSpec &actual_chunk_encoding_spec)
{
    if (expected_chunk_encoding_spec.size() !=
        actual_chunk_encoding_spec.size())
    {
        return false;
    }

    for (auto column_id = ColumnID{0};
         column_id < actual_chunk_encoding_spec.size(); ++column_id)
    {
        if (expected_chunk_encoding_spec[column_id].encoding_type !=
            actual_chunk_encoding_spec[column_id].encoding_type)
        {
            return false;
        }

        // If an explicit VectorCompressionType is requested, check whether it
        // is used in the Segment. Otherwise, do not care about the
        // VectorCompressionType used.
        if (expected_chunk_encoding_spec[column_id].vector_compression_type)
        {
            if (expected_chunk_encoding_spec[column_id]
...
bool BenchmarkTableEncoder::encode(const std::string &table_name,
                                   ...
                                   const EncodingConfig &encoding_config)
{
    ...
    const auto table_has_custom_encoding =
        column_mapping_it != custom_mapping.end();
         ...
         ++column_id)
    {
        ...
        if (table_has_custom_encoding)
        {
            ...
            const auto &segment_encoding =
                encoding_by_column_name.find(column_name);
        ...
        // Use default if it is compatible with the column type or leave column
        // Unencoded if it is not.
        if (encoding_supports_data_type(
                encoding_config.default_encoding_spec.encoding_type,
                column_data_type))
        {
            chunk_encoding_spec.push_back(
                encoding_config.default_encoding_spec);
        }
        else
        {
            // Use a stringstream here to bundle all writes into a single one
            // and avoid locking.
            auto output = std::ostringstream{};
            output << " - Column '" << table_name << "."
                   << table->column_name(column_id) << "' of type ";
            output << column_data_type << " cannot be encoded as ";
            output << encoding_config.default_encoding_spec.encoding_type
                   << " and is ";
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
        const auto encode = [&, chunk_id]()
        {
            ...
            if (!is_chunk_encoding_spec_satisfied(
                    chunk_encoding_spec, get_chunk_encoding_spec(*chunk)))
            {
                ChunkEncoder::encode_chunk(chunk, column_data_types,
                                           chunk_encoding_spec);
