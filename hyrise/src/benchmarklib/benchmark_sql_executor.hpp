...
// This class provides SQL functionality to BenchmarkItemRunners. See
// AbstractBenchmarkItemRunner::_on_execute_item(). If sqlite_wrapper is set
// (i.e. verification is enabled), all SQL queries executed in the benchmark
// will also be executed using SQLite. Hyrise's and SQLite's results are then
// compared.
class BenchmarkSQLExecutor
{
  public:
    // @param visualize_prefix    Prefix for the filename of the generated query
    // plans (e.g., "TPC-H_6-").
    //                            The suffix will be
    //                            "LQP/PQP-<statement_idx>.<extension>"
    ...
    // This executes the given SQL query, records its metrics and returns a
    // single table (the same as SQLPipeline::get_result_table() would). If
    // visualization and/or verification are enabled, these are transparently
    // done as well.
    ...
    // Can optionally be set by the caller. Otherwise, pipelines are
    // auto-committed
    std::shared_ptr<TransactionContext> transaction_context = nullptr;

  private:
    void _compare_tables(
