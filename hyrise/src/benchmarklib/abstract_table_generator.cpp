...
BenchmarkTableInfo::BenchmarkTableInfo(const std::shared_ptr<Table> &init_table)
    : table(init_table)
{
}

AbstractTableGenerator::AbstractTableGenerator(
...
void AbstractTableGenerator::generate_and_store()
{
    ...
    // Encoding table data and generating table statistics are time consuming
    // processes. To reduce the required execution time, we execute these data
    // preparation steps in a multi-threaded way. We store the current scheduler
    // here in case a single-threaded scheduler is used. After data preparation,
    // we switch back to the initially used scheduler.
    const auto initial_scheduler = Hyrise::get().scheduler();
    //In case we are running with snipersim we want to preserve the number of cores we previously set so that we dont exceed core limit
#ifndef SIPERSIM
    Hyrise::get().topology.use_default_topology(
        _benchmark_config->data_preparation_cores);
#endif
    ...
    std::cout << "- Loading/Generating tables done ("
              << format_duration(metrics.generation_duration) << ")"
              << std::endl;
     ...
     */
    {
        ...
        {
            // If there is no clustering for the benchmark defined, there should
            // not be a single sorted chunk. This check is necessary to avoid
            // loading sorted binary data (created with a clustering
            // configuration) in a run that is supposed to be unclustered.
            for (auto &[table_name, table_info] : table_info_by_name)
            {
                auto &table = table_info_by_name[table_name].table;
                const auto chunk_count = table->chunk_count();
                     ...
                     ++chunk_id)
                {
                    const auto &chunk = table->get_chunk(chunk_id);
                    Assert(
                        chunk->individually_sorted_by().empty(),
                        "Tables are sorted, but no clustering has been "
                        "requested. "
        ...
        else
        {
            ...
            {
                // Cannot use structured binding here as it cannot be captured
                // in the lambda:
                ...
                {
                    auto &table = table_info_by_name[table_name].table;
                    const auto sort_mode =
                        SortMode::Ascending; // currently fixed to ascending
                    const auto sort_column_id =
                        table->column_id_by_name(column_name);
                    const auto chunk_count = table->chunk_count();

                    // Check if table is already sorted
                    auto is_sorted = true;
                    resolve_data_type(
                        table->column_data_type(sort_column_id),
                        [&](auto type)
                        {
                            ...
                            {
                                const auto &segment =
                                    table->get_chunk(chunk_id)->get_segment(
                                        sort_column_id);
                    ...
                    if (is_sorted)
                    {
                        ...
                        output << "-  Table '" << table_name
                               << "' is already sorted by '" << column_name
                               << "'\n";
                        std::cout << output.str() << std::flush;
                        const SortColumnDefinition sort_column{sort_column_id,
                                                               sort_mode};
                             ...
                             ++chunk_id)
                        {
                            ...
                            Assert(
                                chunk->individually_sorted_by().empty(),
                                "Chunk SortColumnDefinitions need to be empty");
                    ...
                    // We sort the tables after their creation so that we are
                    // independent of the order in which they are filled. For
                    // this, we use the sort operator. Because it returns a
                    // `const Table`, we need to recreate the table and migrate
                    // the sorted chunks to that table.
                    ...
                    auto sort = std::make_shared<Sort>(
                        table_wrapper,
                        std::vector<SortColumnDefinition>{
                            SortColumnDefinition{sort_column_id, sort_mode}},
                        _benchmark_config->chunk_size,
                        Sort::ForceMaterialization::Yes);
                    sort->execute();
                    const auto immutable_sorted_table = sort->get_output();

                    Assert(immutable_sorted_table->chunk_count() ==
                               table->chunk_count(),
                           "Mismatching chunk_count");

                    table = std::make_shared<Table>(
                        immutable_sorted_table->column_definitions(),
                        TableType::Data, table->target_chunk_size(),
                        UseMvcc::Yes);
                    const auto column_count =
                        immutable_sorted_table->column_count();
                    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count;
                         ++chunk_id)
                    {
                        const auto &chunk =
                            immutable_sorted_table->get_chunk(chunk_id);
                        auto mvcc_data = std::make_shared<MvccData>(
                            chunk->size(), CommitID{0});
                        ...
                        table->get_chunk(chunk_id)->set_individually_sorted_by(
                            SortColumnDefinition(sort_column_id, sort_mode));
                    }

                    auto output = std::stringstream{};
                    output << "-  Sorted '" << table_name << "' by '"
                           << column_name << "' ("
            ...
            std::cout << "- Sorting tables done ("
                      << format_duration(metrics.sort_duration) << ")"
                      << std::endl;
        }
    }
     ...
     */
    {
        std::cout << "- Encoding tables (if necessary) and generating pruning "
                     "statistics"
                  << std::endl;
        ...
        for (auto &table_info_by_name_pair : table_info_by_name)
        {
            ...
            {
                auto per_table_timer = Timer{};
                table_info.re_encoded = BenchmarkTableEncoder::encode(
                    table_name, table_info.table,
                    _benchmark_config->encoding_config);
                auto output = std::stringstream{};
                output << "-  Processing '" + table_name << "' - "
                       << (table_info.re_encoded ? "encoding applied"
    ...
    if (_benchmark_config->cache_binary_tables)
    {
        for (auto &[table_name, table_info] : table_info_by_name)
        {
            ...
            if (table->chunk_count() > 1 &&
                table->get_chunk(ChunkID{0})->size() !=
                    _benchmark_config->chunk_size)
            {
                Fail("Table '" + table_name +
                     "' was loaded from binary, but has a mismatching chunk "
                     "size of " +
                     std::to_string(table->get_chunk(ChunkID{0})->size()) +
                     ". Delete cached files or use "
                     "'--dont_cache_binary_tables'.");
            }
        }
        ...
        for (auto &[table_name, table_info] : table_info_by_name)
        {
            if (table_info.loaded_from_binary && !table_info.re_encoded &&
                !table_info.binary_file_out_of_date)
            {
                continue;
            }
            ...
            std::cout << "-  Writing '" << table_name << "' into binary file "
                      << binary_file_path << " " << std::flush;
            auto per_table_timer = Timer{};
        ...
        std::cout << "- Writing tables into binary files done ("
                  << format_duration(metrics.binary_caching_duration) << ")"
                  << std::endl;
    }
     ...
     */
    {
        std::cout << "- Adding tables to StorageManager and generating table "
                     "statistics"
                  << std::endl;
        ...
        for (auto &table_info_by_name_pair : table_info_by_name)
        {
            ...
            const auto add_table = [&]()
            {
                ...
                const auto output = std::string{"-  Added '"} + table_name +
                                    "' " + "(" +
                                    per_table_timer.lap_formatted() + ")\n";
    ...
    {
        std::cout << "- No chunk indexes created as --chunk_indexes was not "
    ...
    {
        std::cout << "- No table indexes created as --table_indexes was not "
                     "specified or set to false"
                  << std::endl;
    }

    // Set scheduler back to previously used scheduler.
    // No need to reset topology if we are running with snipersim since we never changed topology
#ifndef SNIPERSIM
...
std::shared_ptr<BenchmarkConfig>
AbstractTableGenerator::create_benchmark_config_with_chunk_size(
    ...
    std::unordered_map<std::string, BenchmarkTableInfo> &table_info_by_name)
{
    ...
    std::cout << "- Creating chunk indexes done ("
              << format_duration(metrics.chunk_index_duration) << ")"
              << std::endl;
}
    ...
    std::unordered_map<std::string, BenchmarkTableInfo> &table_info_by_name)
{
    ...
    for (const auto &[table_name, indexes] : indexes_by_table)
    {
        ...
        for (const auto &index_column_names : indexes)
        {
            Assert(index_column_names.size() == 1,
                   "Multi-column indexes are currently not supported.");

            for (const auto &column_name : index_column_names)
            {
                std::cout << "-  Creating index on '" << table_name << "."
                          << column_name << "'" << std::flush;

                auto per_table_index_timer = Timer{};
                table->create_partial_hash_index(
                    table->column_id_by_name(column_name), chunk_ids);
    ...
    std::cout << "- Creating table indexes done ("
              << format_duration(metrics.table_index_duration) << ")"
              << std::endl;
}

AbstractTableGenerator::IndexesByTable
AbstractTableGenerator::_indexes_by_table() const
{
    // Indexes can be specified in a derived concrete class by overriding this
    // function.
    return {};
}

AbstractTableGenerator::SortOrderByTable
AbstractTableGenerator::_sort_order_by_table() const
{
    return {};
}
...
bool AbstractTableGenerator::_all_chunks_sorted_by(
    ...
    const SortColumnDefinition &sort_column)
{
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id)
    {
        const auto &sorted_columns =
            table->get_chunk(chunk_id)->individually_sorted_by();
        ...
        for (const auto &sorted_column : sorted_columns)
        {
            ...
            {
                Assert(sorted_column.sort_mode == sort_column.sort_mode,
...
AbstractTableGenerator::_load_binary_tables_from_path(
    const std::string &cache_directory)
{
    ...
    for (const auto &table_file : list_directory(cache_directory))
    {
        ...
        std::cout << "-  Loading table '" << table_name.string()
                  << "' from cached binary " << table_file.relative_path();
