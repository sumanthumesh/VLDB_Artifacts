...
TPCCBenchmarkItemRunner::TPCCBenchmarkItemRunner(
...
{
    static const auto items = std::vector<BenchmarkItemID>{
        BenchmarkItemID{0}, BenchmarkItemID{1}, BenchmarkItemID{2},
        BenchmarkItemID{3}, BenchmarkItemID{4}};
    return items;
}

bool TPCCBenchmarkItemRunner::_on_execute_item(
    const BenchmarkItemID item_id, BenchmarkSQLExecutor &sql_executor)
{
    ...
    {
        Assert(successful, "TPC-C transactions should always be successful if "
...
std::string
TPCCBenchmarkItemRunner::item_name(const BenchmarkItemID item_id) const
{
    // This should be the only place where this is needed. If we need an
    // ID-to-name mapping (or vice versa) again, we should use a proper bimap.
    switch (item_id)
    {
    case 0:
        return "Delivery";
...
{
    // Except for New-Order, the given weights are minimums (see 5.2.3 in the
    // standard). Since New-Order is the transaction being counted for tpmC, we
    // want it to have the highest weight possible.
