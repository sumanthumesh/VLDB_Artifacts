...
class AbstractTPCCProcedure
{
  public:
    ...
    // Executes the procedure; returns true if it was successful and false if a
  ...
  protected:
    ...
    // As random values are generate during creation of the procedure, this is
    // mostly done in a single thread, not in the database worker's. As such,
    // having a fixed seed for all thread-local random engines should not be an
    // issue.
