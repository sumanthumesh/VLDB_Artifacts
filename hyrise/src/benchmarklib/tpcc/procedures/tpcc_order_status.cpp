...
bool TPCCOrderStatus::_on_execute()
{
    ...
    if (!select_customer_by_name)
    {
        ...
        std::tie(std::ignore, customer_table) = _sql_executor.execute(
            std::string{"SELECT C_ID, C_BALANCE, C_FIRST, C_MIDDLE, C_LAST "
    ...
    else
    {
        ...
        std::tie(std::ignore, customer_table) = _sql_executor.execute(
            ...
            "' ORDER BY C_FIRST");
        Assert(customer_table->row_count() >= 1,
               "Did not find customer by name");

        // Calculate ceil(n/2)
        const auto customer_offset = static_cast<size_t>(std::min(
            std::ceil(static_cast<double>(customer_table->row_count()) / 2.0),
    ...
    const auto order_select_pair = _sql_executor.execute(
        std::string{"SELECT O_ID, O_ENTRY_D, O_CARRIER_ID FROM \"ORDER\" WHERE "
                    "O_W_ID = "} +
    ...
    const auto order_line_select_pair = _sql_executor.execute(
        std::string{"SELECT OL_I_ID, OL_SUPPLY_W_ID, OL_QUANTITY, OL_AMOUNT "
    ...
    Assert(order_line_table && order_line_table->row_count() >= 5 &&
               order_line_table->row_count() <= 15,
           "Did not find order lines");
    for (auto row = size_t{0}; row < order_line_table->row_count(); ++row)
    {
        ol_quantity_sum +=
            *order_line_table->get_value<int32_t>(ColumnID{2}, row);
    }
