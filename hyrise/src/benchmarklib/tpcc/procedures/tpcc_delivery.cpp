...
TPCCDelivery::TPCCDelivery(const int num_warehouses,
    ...
    : AbstractTPCCProcedure(sql_executor), ol_delivery_d{static_cast<int32_t>(
                                               std::time(nullptr))}
{
...
bool TPCCDelivery::_on_execute()
{
    ...
    {
        // TODO(anyone): This could be optimized by querying only once and
        // grouping by NO_D_ID
        const auto new_order_select_pair = _sql_executor.execute(
            std::string{"SELECT MIN(NO_O_ID) FROM NEW_ORDER WHERE NO_W_ID = "} +
            std::to_string(w_id) + " AND NO_D_ID = " + std::to_string(d_id));
        const auto &new_order_table = new_order_select_pair.second;
        const auto min_no_o_id =
            new_order_table->get_value<int32_t>(ColumnID{0}, 0);
        if (!min_no_o_id)
        {
            continue;
        }
        ...
        const auto new_order_update_pair = _sql_executor.execute(
            std::string{"DELETE FROM NEW_ORDER WHERE NO_W_ID = "} +
            std::to_string(w_id) + " AND NO_D_ID = " + std::to_string(d_id) +
        ...
        const auto order_select_pair = _sql_executor.execute(
            std::string{"SELECT O_C_ID FROM \"ORDER\" WHERE O_W_ID = "} +
        ...
        const auto order_update_pair = _sql_executor.execute(
            std::string{"UPDATE \"ORDER\" SET O_CARRIER_ID = "} +
            std::to_string(o_carrier_id) + " WHERE O_W_ID = " +
            std::to_string(w_id) + " AND O_D_ID = " + std::to_string(d_id) +
            " AND O_ID = " + std::to_string(no_o_id));
        ...
        const auto order_line_select_pair = _sql_executor.execute(
            ...
            std::to_string(w_id) + " AND OL_D_ID = " + std::to_string(d_id) +
            " AND OL_O_ID = " + std::to_string(no_o_id));
        const auto &order_line_table = order_line_select_pair.second;
        Assert(order_line_table && order_line_table->row_count() == 1,
               "Did not find order lines");
        const auto amount =
            *order_line_table->get_value<double>(ColumnID{0}, 0);

        // Set delivery date in ORDER_LINE
        const auto order_line_update_pair = _sql_executor.execute(
            std::string{"UPDATE ORDER_LINE SET OL_DELIVERY_D = "} +
            std::to_string(ol_delivery_d) + " WHERE OL_W_ID = " +
            std::to_string(w_id) + " AND OL_D_ID = " + std::to_string(d_id) +
            " AND OL_O_ID = " + std::to_string(no_o_id));
        Assert(order_update_pair.first == SQLPipelineStatus::Success,
               "We have already 'locked' the order ID when we deleted it from "
               "NEW_ORDER. No conflict should be possible.");
    ...
    // TPC-C would allow us to use one transaction per order. We did not yet
    // measure if this would give us an advantage.
