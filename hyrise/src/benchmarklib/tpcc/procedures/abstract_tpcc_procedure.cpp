...
AbstractTPCCProcedure::AbstractTPCCProcedure(BenchmarkSQLExecutor &sql_executor)
    : _sql_executor(sql_executor)
{
    PerformanceWarning(
        "The TPC-C support is in a very early stage. Constraints are not "
        "enforced, indexes are often not used, and even "
...
{
    DebugAssert(
        !_sql_executor.transaction_context ||
            _sql_executor.transaction_context->is_auto_commit(),
        "The SQLExecutor should not already have a transaction context set");

    // Private to the AbstractTPCCProcedure. The actual procedures should not
    ...
    DebugAssert(transaction_context->phase() == TransactionPhase::Committed ||
                ...
                "Expected TPC-C transaction to either commit or roll back the "
                "MVCC transaction");
...
AbstractTPCCProcedure::operator=(const AbstractTPCCProcedure &other)
{
    DebugAssert(&_sql_executor == &other._sql_executor,
                "Can only assign AbstractTPCCProcedure if the sql_executors "
                "are the same");
    // Doesn't assign anything as the only member _sql_executor is already the
    // same.
    return *this;
}
