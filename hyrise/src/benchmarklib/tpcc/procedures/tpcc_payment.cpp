    ...
    : AbstractTPCCProcedure(sql_executor)
{
    ...
    select_customer_by_name =
        customer_selection_method_dist(_random_engine) <= 6;
    if (select_customer_by_name)
    {
        customer = pmr_string{_tpcc_random_generator.last_name(
            _tpcc_random_generator.nurand(255, 0, 999))};
    }
    else
    {
        customer =
            static_cast<int32_t>(_tpcc_random_generator.nurand(1023, 1, 3000));
    }
...
bool TPCCPayment::_on_execute()
{
    ...
    const auto warehouse_select_pair = _sql_executor.execute(
        std::string{"SELECT W_NAME, W_STREET_1, W_STREET_2, W_CITY, W_STATE, "
                    "W_ZIP, W_YTD FROM WAREHOUSE WHERE W_ID = "} +
        std::to_string(w_id));
    ...
    std::tie(pipeline_status, std::ignore) =
        _sql_executor.execute(std::string{"UPDATE WAREHOUSE SET W_YTD = "} +
                              std::to_string(w_ytd + h_amount) +
    ...
    const auto district_select_pair = _sql_executor.execute(
        std::string{"SELECT D_NAME, D_STREET_1, D_STREET_2, D_CITY, D_STATE, "
                    "D_ZIP, D_YTD FROM DISTRICT WHERE D_W_ID = "} +
    ...
    const auto district_update_pair =
        _sql_executor.execute(std::string{"UPDATE DISTRICT SET D_YTD = "} +
                              std::to_string(d_ytd + h_amount) +
                              " WHERE D_W_ID = " + std::to_string(w_id) +
    ...
    if (!select_customer_by_name)
    {
        ...
        std::tie(std::ignore, customer_table) = _sql_executor.execute(
            std::string{"SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, "
                        "C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, "
                        "C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, "
                        "C_BALANCE, C_DATA FROM CUSTOMER WHERE C_W_ID = "} +
        ...
        Assert(customer_table && customer_table->row_count() == 1,
               "Did not find customer by ID (or found more than one)");
    ...
    else
    {
        ...
        std::tie(std::ignore, customer_table) = _sql_executor.execute(
            std::string{"SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, "
                        ...
                        "C_BALANCE, C_DATA FROM CUSTOMER WHERE C_W_ID = "} +
            std::to_string(w_id) + " AND C_D_ID = " + std::to_string(c_d_id) +
            " AND C_LAST = '" + std::string{std::get<pmr_string>(customer)} +
            "' ORDER BY C_FIRST");
        Assert(customer_table && customer_table->row_count() >= 1,
               "Did not find customer by name");

        // Calculate ceil(n/2)
        customer_offset = static_cast<size_t>(std::max(
            0.0,
            std::min(
                std::ceil(static_cast<double>(customer_table->row_count()) /
                          2.0),
                static_cast<double>(customer_table->row_count() - 1))));
        c_id =
            *customer_table->get_value<int32_t>(ColumnID{0}, customer_offset);
    }

    // There is a possible optimization here if we take `customer_table` as an
    // input to an UPDATE operator, but that would be outside of the SQL realm.
    // Also, it would it make it impossible to run _execute_sql via the network
    // layer later on.
    const auto customer_update_balance_pair = _sql_executor.execute(
        std::string{"UPDATE CUSTOMER SET C_BALANCE = C_BALANCE - "} +
        std::to_string(h_amount) + ", C_YTD_PAYMENT = C_YTD_PAYMENT + " +
    ...
    if (*customer_table->get_value<pmr_string>(ColumnID{11}, customer_offset) ==
        "BC")
    {
        std::stringstream new_c_data_stream;
        new_c_data_stream << *customer_table->get_value<int32_t>(
            ColumnID{0}, customer_offset); // C_ID
        new_c_data_stream << c_d_id;
        new_c_data_stream << c_w_id;
        new_c_data_stream << d_id;
        new_c_data_stream << w_id;
        new_c_data_stream << h_amount;
        new_c_data_stream << *customer_table->get_value<pmr_string>(
            ColumnID{15}, customer_offset); // C_DATA
        ...
        const auto customer_update_data_pair = _sql_executor.execute(
            std::string{"UPDATE CUSTOMER SET C_DATA = '"} + new_c_data +
            "' WHERE C_W_ID = " + std::to_string(w_id) + " AND C_D_ID = " +
            std::to_string(c_d_id) + " AND C_ID = " + std::to_string(c_id));
    ...
    const auto history_insert_pair = _sql_executor.execute(std::string{
        ...
        "H_DATA, H_DATE, H_AMOUNT) VALUES (" +
        std::to_string(c_id) + ", " + std::to_string(c_d_id) + ", " +
        std::to_string(c_w_id) + ", " + std::to_string(d_id) + ", " +
        std::to_string(w_id) + ", '" + std::string{w_name + "    " + d_name} +
        "', '" + std::to_string(h_date) + "', " + std::to_string(h_amount) +
        ")"});
    Assert(history_insert_pair.first == SQLPipelineStatus::Success,
