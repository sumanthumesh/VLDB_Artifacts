...
{
    // The implementation of this procedure follows the standard's example
    // implementation (see Appendix A.5, p. 116) and also conforms to py-tpcc
    // (see
    // https://github.com/apavlo/py-tpcc/blob/master/pytpcc/drivers/sqlitedriver.py).

    // Retrieve next order ID.
    const auto &[_, district_table] = _sql_executor.execute(
    ...
    // Retrieve the number of items that have a stock level under the threshold
    // value and belong to the 20 orders before the next order ID. The alias is
    // needed for verification with SQLite, which names the result column
    // slightly differently.
    _sql_executor.execute(
        "SELECT COUNT(DISTINCT(S_I_ID)) item_count FROM ORDER_LINE, STOCK "
        "WHERE OL_W_ID = " +
        std::to_string(w_id) + " AND OL_D_ID = " + std::to_string(d_id) +
        " AND OL_O_ID < " + std::to_string(next_o_id) +
        " AND OL_O_ID >= " + std::to_string(next_o_id - 20) +
        " AND S_W_ID = " + std::to_string(w_id) +
