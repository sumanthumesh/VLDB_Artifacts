...
class TPCCRandomGenerator : public RandomGenerator
{
  public:
    ...
    size_t nurand(size_t a, size_t x, size_t y)
    {
        ...
        return (((random_number(0, a) | random_number(x, y)) + c) %
                (y - x + 1)) +
               x;
    }
     ...
     *            for i's greater than 1000 we calculate a non-uniform random
     * number below 1000
    ...
    std::string generate_string(size_t lower_length, size_t upper_length,
                                char base_character, int num_characters)
    {
        ...
        {
            s.append(1,
                     static_cast<char>(base_character +
                                       random_number(0, num_characters - 1)));
        }
        return s;
    }
    ...
    void reset_c_for_c_last()
    {
        ...
        while (true)
        {
            ...
            const auto diff =
                std::abs(static_cast<long>(current_c - old_c)); // NOLINT

            if (current_c != old_c && diff >= 64 && diff <= 120 && diff != 96 &&
                diff != 112)
            {
                break;
            }
