...
JCCHBenchmarkItemRunner::JCCHBenchmarkItemRunner(
    const bool skewed, const std::string &dbgen_path,
    ...
    : TPCHBenchmarkItemRunner(config, use_prepared_statements, scale_factor,
      ...
      _skewed(skewed), _dbgen_path(dbgen_path), _data_path(data_path)
{
    _load_params();
}
...
JCCHBenchmarkItemRunner::JCCHBenchmarkItemRunner(
    ...
    : TPCHBenchmarkItemRunner(config, use_prepared_statements, scale_factor,
                              clustering_configuration, items),
      _skewed(skewed), _dbgen_path(dbgen_path), _data_path(data_path)
{
    _load_params();
}
...
std::string
JCCHBenchmarkItemRunner::item_name(const BenchmarkItemID item_id) const
{
    Assert(item_id < 22u, "item_id out of range");
    return std::string("JCC-H ") + (_skewed ? "(skewed) " : "(normal) ") +
           (item_id + 1 < 10 ? "0" : "") + std::to_string(item_id + 1);
}
...
void JCCHBenchmarkItemRunner::_load_params()
{
    ...
    const auto params_path =
        local_queries_path + "params-" + (_skewed ? "skewed" : "normal");
    ...
    if (!std::filesystem::exists(params_path))
    {
        ...
        std::cout << "- Creating query parameters by calling external qgen"
                  << std::flush;

        // Check for the existence of dbgen's query templates (1.sql etc.) at
        // the expected location
        const auto dbgen_queries_path = _dbgen_path + "/queries/";
        Assert(std::filesystem::exists(dbgen_queries_path),
               std::string{"Query templates not found at "} +
                   dbgen_queries_path);

        // NOLINTBEGIN(concurrency-mt-unsafe)
        // clang-tidy complains that system() is not thread-safe. We can ignore
        // this warning, because _load_params is only called in the constructor
        // once.

        // Create local directory and copy query templates if needed
        const auto local_queries_dir_created =
            std::filesystem::create_directory(local_queries_path);
        Assert(std::filesystem::exists(local_queries_path),
               "Creating JCC-H queries folder failed");
        if (local_queries_dir_created)
        {
            auto cmd = std::stringstream{};
            cmd << "cd " << local_queries_path << " && ln -s " << _dbgen_path
                << "/queries/*.sql .";
            auto ret = system(cmd.str().c_str());
            Assert(!ret, "Creating symlinks to query templates failed");
        }

        // Call qgen a couple of times with different PRNG seeds and store the
        // resulting query parameters in queries/params. dbgen doesn't like `-r
        // 0`, so we start at 1.
        for (auto seed = 1;
             seed <= (_config->max_runs > 0 ? _config->max_runs : 100'000);
             ++seed)
        {
            auto cmd = std::stringstream{};
            cmd << "cd " << local_queries_path << " && " << _dbgen_path
                << "/qgen " << (_skewed ? "-k" : "") << " -s " << _scale_factor
                << " -b " << _dbgen_path << "/dists.dss -r " << seed << " -l "
                << params_path << " >/dev/null";
            auto ret = system(cmd.str().c_str());
    ...
    Assert(file.is_open(),
           std::string{"Could not open JCC-H parameters at "} + params_path);
...
bool JCCHBenchmarkItemRunner::_on_execute_item(
    const BenchmarkItemID item_id, BenchmarkSQLExecutor &sql_executor)
{
    ...
    std::uniform_int_distribution<> params_dist{
        0, static_cast<int>(this_item_params.size() - 1)};
    const auto raw_params_iter =
        this_item_params.begin() + params_dist(random_engine);

    auto parameters = std::vector<std::string>{};
    auto sql = std::string{};

    // This mirrors TPCHBenchmarkItemRunner::_on_execute_item. Instead of
    // generating random parameters according to the TPC-H specifications, it
    // uses the ones generated by JCC-H's qgen
    switch (item_id)
    {
    ...
    case 1 - 1:
    {
        // In some cases, we still need to do the date calculations that SQLite
        // (used for verification) does not support yet. When parsing a date, we
        // expect the generator to provide sound date strings and omit checks
        // when dereferencing the optionals.
        const auto date = date_interval(boost::gregorian::date{1998, 12, 01},
                                        -std::stoi(raw_params_iter->at(0)),
    ...
    case 4 - 1:
    {
        const auto begin_date =
            string_to_timestamp(raw_params_iter->at(0))->date();
        const auto end_date =
    ...
    {
        const auto begin_date =
            string_to_timestamp(raw_params_iter->at(1))->date();
        const auto end_date =
    ...
    case 9 - 1:
    {
        ...
        {
            std::cerr << "\nWarning: JCC-H Query 9 needs optimization. "
                         "Consider skipping it using -q\n\n";
    ...
    {
        const auto begin_date =
            string_to_timestamp(raw_params_iter->at(2))->date();
        const auto end_date =
            date_interval(begin_date, 1, DatetimeComponent::Year);
    ...
    case 18 - 1:
    {
        ...
        {
            std::cerr << "\nWarning: JCC-H Query 18 as used by Hyrise slightly "
                         "diverges from the specification.\n";
            std::cerr << "         See jcch_benchmark_item_runner.cpp for "
                         "details.\n\n";
        ...
        //   // NOLINT
        // We do not use this parameter as it would bring a structural change to
        // the SQL query template, which is also used for TPC-H.
        parameters.emplace_back(raw_params_iter->at(0));
        break;
    }
