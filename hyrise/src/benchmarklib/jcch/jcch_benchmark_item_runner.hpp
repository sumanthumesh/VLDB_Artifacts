...
// The generation of JCC-H items is based on top of that of the TPC-H. Instead
// of creating random values in the C++ code, we use those generated by JCC-H's
// qgen. See _on_execute_item for details.
...
class JCCHBenchmarkItemRunner : public TPCHBenchmarkItemRunner
{
  public:
    ...
    JCCHBenchmarkItemRunner(const bool skewed, const std::string &dbgen_path,
    ...
    // Constructor for a JCCHBenchmarkItemRunner containing a subset of TPC-H
    // queries
    JCCHBenchmarkItemRunner(const bool skewed, const std::string &dbgen_path,
                            const std::string &data_path,
                            const std::shared_ptr<BenchmarkConfig> &config,
