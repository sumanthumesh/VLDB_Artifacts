...
EncodingConfig::EncodingConfig(
    const SegmentEncodingSpec &init_default_encoding_spec)
...
SegmentEncodingSpec
EncodingConfig::encoding_spec_from_strings(const std::string &encoding_str,
                                           const std::string &compression_str)
{
    const auto encoding = EncodingConfig::encoding_string_to_type(encoding_str);
    const auto compression =
        EncodingConfig::compression_string_to_type(compression_str);

    return compression ? SegmentEncodingSpec{encoding, *compression}
                       : SegmentEncodingSpec{encoding};
}

EncodingType
EncodingConfig::encoding_string_to_type(const std::string &encoding_str)
{
...
std::optional<VectorCompressionType>
EncodingConfig::compression_string_to_type(const std::string &compression_str)
{
    ...
    const auto compression =
        vector_compression_type_to_string.right.find(compression_str);
...
nlohmann::json EncodingConfig::to_json() const
{
    const auto encoding_spec_to_string_map = [](const SegmentEncodingSpec &spec)
    {
        ...
        mapping["encoding"] =
            std::string{magic_enum::enum_name(spec.encoding_type)};
        if (spec.vector_compression_type)
        {
            mapping["compression"] = vector_compression_type_to_string.left.at(
                *spec.vector_compression_type);
        }
        return mapping;
    };
...
// This is intentionally limited to 80 chars per line, as cxxopts does this too
// and it looks bad otherwise.
