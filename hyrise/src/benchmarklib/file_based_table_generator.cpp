...
FileBasedTableGenerator::FileBasedTableGenerator(
...
FileBasedTableGenerator::generate()
{
    Assert(std::filesystem::is_directory(_path),
           std::string{"Table path "} + _path + " must be a directory");

    auto table_info_by_name =
        std::unordered_map<std::string, BenchmarkTableInfo>{};
    const auto table_extensions =
        std::unordered_set<std::string>{".csv", ".tbl", ".bin"};

    /**
     * 1. Explore the directory and identify tables to be loaded
     * Recursively walk through the specified directory and collect all tables
     * found on the way. A tables name is determined by its filename. Multiple
     * file extensions per table are allowed, for example there could be a CSV
     * and a binary version of a table.
     */
    for (const auto &directory_entry : list_directory(_path))
    {
        ...
        {
            table_info_by_name_iter =
                table_info_by_name.emplace(table_name, BenchmarkTableInfo{})
                    .first;
        }
        ...
        if (extension == ".bin")
        {
            Assert(!table_info.binary_file_path,
                   "Multiple binary files found for table '"s +
                       table_name.string() + "'");
        ...
        else
        {
            Assert(!table_info.text_file_path,
                   "Multiple text files found for table '"s +
                       table_name.string() + "'");
    ...
    /**
     * 2. Check for "out of date" binary files, i.e., whether both a binary and
     * textual file exists AND the binary file is older than the textual file.
     */
    for (auto &[table_name, table_info] : table_info_by_name)
    {
        if (table_info.binary_file_path && table_info.text_file_path)
        {
            const auto last_binary_write =
                std::filesystem::last_write_time(*table_info.binary_file_path);
            const auto last_text_write =
                std::filesystem::last_write_time(*table_info.text_file_path);
    ...
    /**
     * 3. Actually load the tables. Load from binary file if a up-to-date binary
     * file exists for a Table.
     */
    for (auto &[table_name, table_info] : table_info_by_name)
    {
        Timer timer;
        ...
        if (table_info.binary_file_path && !table_info.binary_file_out_of_date)
        {
            ...
            table_info.table =
                BinaryParser::parse(*table_info.binary_file_path);
        ...
        else
        {
            ...
            if (extension == ".tbl")
            {
                table_info.table = load_table(*table_info.text_file_path,
                                              _benchmark_config->chunk_size);
            }
            else if (extension == ".csv")
            {
                table_info.table = CsvParser::parse(
                    *table_info.text_file_path, _benchmark_config->chunk_size);
            }
            else
            {
                Fail("Unknown textual file format. This should have been "
                     "caught earlier.");
            }
        }
...
void FileBasedTableGenerator::set_add_constraints_callback(
    const std::function<void(std::unordered_map<std::string, BenchmarkTableInfo>
                                 &)> &add_constraints_callback)
{
