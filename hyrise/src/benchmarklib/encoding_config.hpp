...
using DataTypeEncodingMapping =
    std::unordered_map<DataType, SegmentEncodingSpec>;
...
class EncodingConfig
{
  public:
    EncodingConfig();
    EncodingConfig(const SegmentEncodingSpec &default_encoding_spec,
    ...
    static SegmentEncodingSpec
    encoding_spec_from_strings(const std::string &encoding_str,
                               const std::string &compression_str);
    static EncodingType
    encoding_string_to_type(const std::string &encoding_str);
    static std::optional<VectorCompressionType>
    compression_string_to_type(const std::string &compression_str);
