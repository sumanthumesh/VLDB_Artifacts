...
class BenchmarkConfig
{
  public:
    BenchmarkConfig(const BenchmarkMode init_benchmark_mode,
                    ...
                    const uint32_t init_data_preparation_cores,
                    const uint32_t init_clients,
                    const bool init_enable_visualization,
    ...
    bool verify{false};
    bool cache_binary_tables{false}; // Defaults to false for internal use, but
                                     // the CLI sets it to true by default.
