    ...
    : AbstractBenchmarkItemRunner(config)
{
    ...
    Assert(std::filesystem::exists(path),
           "No such file or directory '" + query_path + "'");

    if (std::filesystem::is_regular_file(path))
    {
        Assert(is_sql_file(query_path),
               "Specified file '" + query_path + "' is not an .sql file");
        _parse_query_file(query_path, query_subset);
    }
    else
    {
        // Recursively walk through the specified directory and add all files on
        // the way
        for (const auto &entry : list_directory(path))
        {
            if (is_sql_file(entry))
            {
                if (filename_blacklist.find(entry.filename()) !=
                    filename_blacklist.end())
                {
                    continue;
                }
    ...
    std::sort(_queries.begin(), _queries.end(),
              [](const Query &lhs, const Query &rhs)
              { return lhs.name < rhs.name; });
}

bool FileBasedBenchmarkItemRunner::_on_execute_item(
...
std::string
FileBasedBenchmarkItemRunner::item_name(const BenchmarkItemID item_id) const
{
    ...
    const std::optional<std::unordered_set<std::string>> &query_subset)
{
    ...
    // The names of queries from, e.g., "queries/TPCH-7.sql" will be prefixed
    // with "TPCH-7."
    ...
    /**
     * A file can contain multiple SQL statements, and each statement may cover
     * one or more lines. We use the SQLParser to split up the content of the
     * file into the individual SQL statements.
     */

    hsql::SQLParserResult parse_result;
    hsql::SQLParser::parse(content, &parse_result);
    Assert(parse_result.isValid(),
           create_sql_parser_error_message(content, parse_result));

    std::vector<Query> queries_in_file{parse_result.size()};

    size_t sql_string_offset{0u};
    for (auto statement_idx = size_t{0}; statement_idx < parse_result.size();
         ++statement_idx)
    {
        const auto item_name =
            item_name_prefix + '.' + std::to_string(statement_idx);
        const auto statement_string_length =
            parse_result.getStatement(statement_idx)->stringLength;
        const auto statement_string = boost::trim_copy(
            content.substr(sql_string_offset, statement_string_length));
    ...
    {
        queries_in_file.front().name.erase(queries_in_file.front().name.end() -
                                               2,
                                           queries_in_file.front().name.end());
    }
