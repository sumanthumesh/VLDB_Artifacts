...
void dbgen_cleanup()
{
    for (auto *distribution :
         {&nations,     &regions,        &o_priority_set, &l_instruct_set,
...
TPCHTableGenerator::generate()
{
    Assert(_scale_factor < 1.0f || std::round(_scale_factor) == _scale_factor,
           "Due to tpch_dbgen limitations, only scale factors less than one "
           "can have a fractional part.");

    const auto cache_directory = std::string{"tpch_cached_tables/sf-"} +
                                 std::to_string(_scale_factor); // NOLINT
    ...
    auto customer_builder =
        TableBuilder{_benchmark_config->chunk_size, customer_column_types,
                     customer_column_names, customer_count};
    auto order_builder =
        TableBuilder{_benchmark_config->chunk_size, order_column_types,
                     order_column_names, order_count};
    auto lineitem_builder =
        TableBuilder{_benchmark_config->chunk_size, lineitem_column_types,
                     lineitem_column_names, ChunkOffset{order_count * 4}};
    auto part_builder =
        TableBuilder{_benchmark_config->chunk_size, part_column_types,
                     part_column_names, part_count};
    auto partsupp_builder =
        TableBuilder{_benchmark_config->chunk_size, partsupp_column_types,
                     partsupp_column_names, ChunkOffset{part_count * 4}};
    auto supplier_builder =
        TableBuilder{_benchmark_config->chunk_size, supplier_column_types,
                     supplier_column_names, supplier_count};
    auto nation_builder =
        TableBuilder{_benchmark_config->chunk_size, nation_column_types,
                     nation_column_names, nation_count};
    auto region_builder =
        TableBuilder{_benchmark_config->chunk_size, region_column_types,
                     region_column_names, region_count};
    ...
    {
        auto customer = call_dbgen_mk<customer_t>(row_idx + 1, mk_cust,
                                                  TPCHTable::Customer);
        customer_builder.append_row(customer.custkey, customer.name,
                                    customer.address, customer.nation_code,
                                    customer.phone,
    ...
    for (auto order_idx = size_t{0}; order_idx < order_count; ++order_idx)
    {
        ...
        order_builder.append_row(
            order.okey, order.custkey, pmr_string(1, order.orderstatus),
            convert_money(order.totalprice), order.odate, order.opriority,
            order.clerk, order.spriority, order.comment);

        for (auto line_idx = int64_t{0}; line_idx < order.lines; ++line_idx)
        {
            const auto &lineitem = order.l[line_idx];

            lineitem_builder.append_row(
                lineitem.okey, lineitem.partkey, lineitem.suppkey,
                lineitem.lcnt, lineitem.quantity,
                convert_money(lineitem.eprice),
                convert_money(lineitem.discount), convert_money(lineitem.tax),
                pmr_string(1, lineitem.rflag[0]),
                pmr_string(1, lineitem.lstatus[0]), lineitem.sdate,
                lineitem.cdate, lineitem.rdate, lineitem.shipinstruct,
    ...
    for (auto part_idx = size_t{0}; part_idx < part_count; ++part_idx)
    {
        ...
        part_builder.append_row(part.partkey, part.name, part.mfgr, part.brand,
                                part.type, part.size, part.container,
                                convert_money(part.retailprice), part.comment);

        // Some scale factors (e.g., 0.05) are not supported by tpch-dbgen as
        // they produce non-unique partkey/suppkey combinations. The reason is
        // probably somewhere in the magic in PART_SUPP_BRIDGE. As the partkey
        // is ascending, those are easy to identify:
        ...
        {
            {
                // Make sure we do not generate non-unique combinations (see
                // above)
                if (partsupp.partkey != last_partkey)
                {
                    Assert(
                        partsupp.partkey > last_partkey,
                        "Expected partkey to be generated in ascending order");
                    last_partkey = partsupp.partkey;
                    suppkeys.clear();
                }
                Assert(std::find(suppkeys.begin(), suppkeys.end(),
            ...
            partsupp_builder.append_row(
                partsupp.partkey, partsupp.suppkey, partsupp.qty,
    ...
    for (auto supplier_idx = size_t{0}; supplier_idx < supplier_count;
         ++supplier_idx)
    {
        const auto supplier = call_dbgen_mk<supplier_t>(
            supplier_idx + 1, mk_supp, TPCHTable::Supplier);

        supplier_builder.append_row(
            supplier.suppkey, supplier.name, supplier.address,
            supplier.nation_code, supplier.phone,
    ...
    for (auto nation_idx = size_t{0}; nation_idx < nation_count; ++nation_idx)
    {
        ...
        nation_builder.append_row(nation.code, nation.text, nation.join,
                                  nation.comment);
    }
...
TPCHTableGenerator::_indexes_by_table() const
{
...
TPCHTableGenerator::_sort_order_by_table() const
{
    if (_clustering_configuration == ClusteringConfiguration::Pruning)
    {
        // This clustering improves the pruning of chunks for the two largest
        // tables in TPC-H, lineitem and orders. Both tables are frequently
        // filtered by the sorted columns, which improves the pruning rate
        // significantly. Allowed as per TPC-H Specification, paragraph 1.5.2.
        return {{"lineitem", "l_shipdate"}, {"orders", "o_orderdate"}};
    }

    // Even though the generated TPC-H data is implicitly sorted by the primary
    // keys, we do neither set the corresponding flags in the table nor in the
    // chunks. This is done on purpose, as the non-clustered mode is designed to
    // pass as little extra information into Hyrise as possible. In the future,
    // these sort orders might be automatically identified with flags being set
    // automatically.
    return {};
}
    ...
    const
{
    ...
    const auto orders_pk_constraint =
        TableKeyConstraint{{orders_table->column_id_by_name("o_orderkey")},
                           KeyConstraintType::PRIMARY_KEY};
    orders_table->add_soft_key_constraint(orders_pk_constraint);

    const auto &lineitem_table = table_info_by_name.at("lineitem").table;
    const auto lineitem_pk_constraint =
        TableKeyConstraint{{lineitem_table->column_id_by_name("l_orderkey"),
                            lineitem_table->column_id_by_name("l_linenumber")},
                           KeyConstraintType::PRIMARY_KEY};
    ...
    const auto part_table_pk_constraint =
        TableKeyConstraint{{part_table->column_id_by_name("p_partkey")},
                           KeyConstraintType::PRIMARY_KEY};
    ...
    const auto partsupp_pk_constraint =
        TableKeyConstraint{{partsupp_table->column_id_by_name("ps_partkey"),
                            partsupp_table->column_id_by_name("ps_suppkey")},
                           KeyConstraintType::PRIMARY_KEY};
    ...
    const auto supplier_pk_constraint =
        TableKeyConstraint{{supplier_table->column_id_by_name("s_suppkey")},
                           KeyConstraintType::PRIMARY_KEY};
    ...
    const auto nation_pk_constraint =
        TableKeyConstraint{{nation_table->column_id_by_name("n_nationkey")},
                           KeyConstraintType::PRIMARY_KEY};
    ...
    const auto region_pk_constraint =
        TableKeyConstraint{{region_table->column_id_by_name("r_regionkey")},
                           KeyConstraintType::PRIMARY_KEY};
