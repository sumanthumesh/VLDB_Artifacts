...
/**
 * Wrapper around the official tpch-dbgen tool, making it directly generate
 * hyrise::Table instances without having to generate and then load .tbl files.
 *
 * NOT thread safe because the underlying tpch-dbgen is not (since it has global
 * data and malloc races).
 */
class TPCHTableGenerator : virtual public AbstractTableGenerator
{
  public:
    // Convenience constructor for creating a TPCHTableGenerator without a
    // benchmarking context
    explicit TPCHTableGenerator(
