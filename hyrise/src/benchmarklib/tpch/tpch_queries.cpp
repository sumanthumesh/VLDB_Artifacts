 ...
 * ps_partkey AND s_suppkey = ps_suppkey AND p_size = [SIZE] AND p_type like
 * '%[TYPE]' AND s_nationkey = n_nationkey AND n_regionkey = r_regionkey AND
 * r_name = '[REGION]' AND ps_supplycost = ( SELECT min(ps_supplycost) FROM
 * partsupp, supplier, nation, region WHERE p_partkey = ps_partkey AND s_suppkey
 ...
 * LIMIT 100;
 *
 * The limit is not part of the printed query but hidden in the specification
 * text.
 */
 ...
 * o_orderkey AND o_orderdate < date '[DATE]' AND l_shipdate > date '[DATE]'
 ...
 * FROM LINEITEM
 * WHERE L_SHIPDATE >= '[DATE]' AND L_SHIPDATE < dateadd(yy, 1, cast('[DATE]' as
 * datetime)) AND L_DISCOUNT BETWEEN [DISCOUNT] - 0.01 AND [DISCOUNT] + 0.01 AND
 * L_QUANTITY < [QUANTITY]
 *
 * Changes:
 *  1. SQLite does not support dates and intervals
 ...
 *    a. Add a small offset ".06 + 0.01001" to include records with a l_discount
 * of "0.07"
 */
 ...
 *    a. Use SUBSTR instead (because our date columns are strings AND SQLite
 * doesn't support EXTRACT)
 */
 ...
 *
 * Changes:
 *  1. Subselect column aliases are moved into subselect because SQLite does not
 * support aliases at the original position
 */
 ...
 * >= date '[DATE]' AND l_shipdate < date '[DATE]' + interval '1' month;
 *
 * Changes:
 *  1. SQLite does not support dates and intervals
 *    a. use strings as data type for now
 ...
 * [SIZE7], [SIZE8]) AND ps_suppkey not in ( SELECT s_suppkey FROM supplier
 ...
 * l_orderkey FROM lineitem GROUP BY l_orderkey having sum(l_quantity) >
 * [QUANTITY]
 *    )
 *    AND c_custkey = o_custkey
 *    AND o_orderkey = l_orderkey
 ...
 * ('[I1]','[I2]’,'[I3]','[I4]','[I5]','[I6]','[I7]') AND c_acctbal > ( SELECT
 *                 AVG(c_acctbal)
 *             FROM
 *                 customer
 *             WHERE
...
const std::map<size_t, const char *> tpch_queries = {
    ...
    {4, tpch_query_4},   {5, tpch_query_5},   {6, tpch_query_6},
    {7, tpch_query_7},   {8, tpch_query_8},   {9, tpch_query_9},
    {10, tpch_query_10}, {11, tpch_query_11}, {12, tpch_query_12},
    {13, tpch_query_13}, {14, tpch_query_14}, {15, tpch_query_15},
    {16, tpch_query_16}, {17, tpch_query_17}, {18, tpch_query_18},
    {19, tpch_query_19}, {20, tpch_query_20}, {21, tpch_query_21},
    {22, tpch_query_22}};
