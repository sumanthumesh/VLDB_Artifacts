...
BenchmarkRunner::BenchmarkRunner(
    ...
    : _config(config), _benchmark_item_runner(std::move(benchmark_item_runner)),
      _table_generator(std::move(table_generator)), _context(context)
{
    // Enable caching only if no metrics are requested. When metrics are
    // desired, we want to explicitly measure translation and optimization
    // runtimes for each SQL statement. These stages are skipped for cached
    // plans, making it hard to interpret the measurements.
    if (!_config.pipeline_metrics)
    {
        Hyrise::get().default_pqp_cache =
            std::make_shared<SQLPhysicalPlanCache>();
        Hyrise::get().default_lqp_cache =
            std::make_shared<SQLLogicalPlanCache>();
        std::cout << "- SQL plan caching switched on." << std::endl;
    }
    else
    {
        std::cout << "- SQL plan caching switched off since SQL pipeline "
                     "metrics tracking is requested."
                  << std::endl;
    }

    // Initialise the scheduler if the benchmark was requested to run
    // multi-threaded.
    if (config.enable_scheduler)
    {
        ...
        // Add NUMA topology information to the context, for processing in the
        // benchmark_multithreaded.py script.
        ...
        _context.push_back(
            {"utilized_cores_per_numa_node", numa_cores_per_node});
    ...
    // SQLite data is only loaded if the dedicated result set is not complete,
    // i.e, items exist for which no dedicated result could be loaded.
    if (_config.verify &&
        _benchmark_item_runner->has_item_without_dedicated_result())
    {
        ...
        for (const auto &[table_name, table] :
             Hyrise::get().storage_manager.tables())
        {
            std::cout << "-  Loading '" << table_name << "' into SQLite "
                      << std::flush;
        ...
        std::cout << "- All tables loaded into SQLite ("
                  << timer.lap_formatted() << ")" << std::endl;
...
void BenchmarkRunner::run()
{
    ...
    auto system_utilization_tracker = std::thread{
        [&]
        {
            ...
            SQLPipelineBuilder{
                "CREATE TABLE benchmark_system_utilization_log AS SELECT "
                "CAST(0 as LONG) AS \"timestamp\", * FROM "
            ...
            while (track_system_utilization)
            {
                ...
                sql_builder << "INSERT INTO benchmark_system_utilization_log "
                               "SELECT CAST("
                            << timestamp
                            << "as LONG), * FROM meta_system_utilization";

                SQLPipelineBuilder{sql_builder.str()}
                    .create_pipeline()
                    .get_result_table();

                std::this_thread::sleep_for(
                    SYSTEM_UTILIZATION_TRACKING_INTERVAL);
            }
        }};
    ...
    if (_config.system_metrics)
    {
        ...
        SQLPipelineBuilder{
            "CREATE TABLE benchmark_segments_log AS SELECT 0 AS snapshot_id, "
            "'init' AS moment, * FROM meta_segments"}
            .create_pipeline()
            .get_result_table();
    }
    ...
    if (!items.empty())
    {
        _results = std::vector<BenchmarkItemResult>{
            *std::max_element(items.begin(), items.end()) + 1u};
    }
    ...
    for (const auto &plugin : _loaded_plugins)
    {
        ...
        std::cout << "- Run pre-benchmark hook of '" << plugin << "'"
                  << std::endl;
        Hyrise::get().plugin_manager.exec_pre_benchmark_hook(
            plugin, *_benchmark_item_runner);
    }
    ...
    const auto write_report =
        _config.output_file_path &&
        (!_config.verify && !_config.enable_visualization);
    ...
    {
        std::cout << "- Not writing JSON result as either verification or "
                     "visualization are activated."
                  << std::endl;
    ...
    if (_config.benchmark_mode == BenchmarkMode::Shuffled && !_config.verify &&
        !_config.enable_visualization)
    {
        for (const auto &item_id : items)
        {
            std::cout << "- Results for "
                      << _benchmark_item_runner->item_name(item_id)
                      << std::endl;
            std::cout << "  -> Executed "
                      << _results[item_id].successful_runs.size() << " times"
                      << std::endl;
            if (!_results[item_id].unsuccessful_runs.empty())
            {
                std::cout << "  -> "
                          << _results[item_id].unsuccessful_runs.size()
                          << " additional runs failed" << std::endl;
            }
        }
    }
    ...
    if (_config.verify)
    {
        ...
        for (const auto &item_id : items)
        {
            ...
            Assert(verification_status,
                   "Verification result should have been set");
    ...
    if (Hyrise::get().scheduler())
    {
        ...
        Hyrise::get().set_scheduler(
            std::make_shared<ImmediateExecutionScheduler>());
    }
...
void BenchmarkRunner::_benchmark_shuffled()
{
    ...
    if (const auto &weights = _benchmark_item_runner->weights();
        !weights.empty())
    {
        ...
        for (const auto &selected_item_id : item_ids)
        {
            ...
            item_ids_weighted.resize(item_ids_weighted.size() + item_weight,
                                     selected_item_id);
        }
    ...
    Assert(_currently_running_clients == 0,
           "Did not expect any clients to run at this time");

    _state = BenchmarkState{_config.max_duration};

    while (_state.keep_running() &&
           (_config.max_runs < 0 ||
            _total_finished_runs.load(std::memory_order_relaxed) <
                static_cast<size_t>(_config.max_runs)))
    {
        // We want to only schedule as many items simultaneously as we have
        // simulated clients.
        if (_currently_running_clients.load(std::memory_order_relaxed) <
            _config.clients)
        {
            if (item_ids_shuffled.empty())
            {
                item_ids_shuffled = item_ids;
                std::shuffle(item_ids_shuffled.begin(), item_ids_shuffled.end(),
                             random_generator);
            }
    ...
    {
        // As the execution of benchmark items is intermingled, we use the total
        // duration for all items.
        result.duration = _state.benchmark_duration;
    }

    // Wait for the rest of the tasks that didn't make it in time - they will
    // not count towards the results.
    Hyrise::get().scheduler()->wait_for_all_tasks();
    Assert(_currently_running_clients == 0,
           "All runs must be finished at this point");
...
void BenchmarkRunner::_benchmark_ordered()
{
    for (const auto &item_id : _benchmark_item_runner->items())
    {
        ...
        while (
            _state.keep_running() &&
            (_config.max_runs < 0 ||
             (result.successful_runs.size() + result.unsuccessful_runs.size()) <
                 static_cast<size_t>(_config.max_runs)))
        {
            // We want to only schedule as many items simultaneously as we have
            // simulated clients.
        ...
        // Wait for the rest of the tasks that didn't make it in time - they
        // will not count toward the results.
        ...
        result.duration = _state.benchmark_duration;
        // chrono::seconds uses an integer precision duration type, but we need
        // a floating-point value.
        const auto duration_seconds =
            std::chrono::duration<double>{_state.benchmark_duration}.count();
        const auto items_per_second =
            static_cast<double>(result.successful_runs.size()) /
            duration_seconds;

        // Compute mean by using accumulators.
        auto accumulator = boost::accumulators::accumulator_set<
            double,
            boost::accumulators::stats<boost::accumulators::tag::mean>>{};
        for (const auto &entry : result.successful_runs)
        {
            // For readability and to be consistent with compare_benchmarks.py,
            // query latencies should be in milliseconds. chrono::milliseconds
            // uses an integer precision duration type, but we need a
            // floating-point value.
            accumulator(
                std::chrono::duration<double, std::milli>{entry.duration}
                    .count());
        }
        const auto mean_in_milliseconds =
            boost::accumulators::mean(accumulator);

        if (!_config.verify && !_config.enable_visualization)
        {
            std::cout << "  -> Executed " << result.successful_runs.size()
                      << " times in " << duration_seconds
                      << " seconds (Latency: " << mean_in_milliseconds
                      << " ms/iter, Throughput: " << items_per_second
                      << " iter/s)" << std::endl;
            if (!result.unsuccessful_runs.empty())
            {
                std::cout << "  -> " << result.unsuccessful_runs.size()
                          << " additional runs failed" << std::endl;
            }
        }

        // Taking the snapshot at this point means that both warmup runs and
        // runs that finish after the deadline are taken into account, too. In
        // light of the significant amount of data added by the snapshots to the
        // JSON file and the unclear advantage of excluding those runs, we only
        // take one snapshot here.
...
void BenchmarkRunner::_schedule_item_run(const BenchmarkItemID item_id)
{
    ...
    auto task = std::make_shared<JobTask>(
        [&, item_id]()
        {
            ...
            auto [success, metrics, any_run_verification_failed] =
                _benchmark_item_runner->execute_item(item_id);
            const auto run_end = std::chrono::steady_clock::now();

            --_currently_running_clients;
            ++_total_finished_runs;

            // If result.verification_passed was previously unset, set it;
            // otherwise only invalidate it if the run failed.
            result.verification_passed =
                result.verification_passed.load().value_or(true) &&
                !any_run_verification_failed;
            ...
            if (!_state.is_done())
            {
                ...
                const auto item_result = BenchmarkItemRunResult{
                    run_start - _benchmark_start, run_end - run_start,
                    std::move(metrics)};
                if (success)
                {
...
nlohmann::json BenchmarkRunner::_create_report() const
{
    const std::chrono::nanoseconds total_duration =
        std::chrono::steady_clock::now() - _benchmark_start;
    ...
    for (const auto &item_id : _benchmark_item_runner->items())
    {
        ...
        const auto runs_to_json = [](auto runs)
        {
            ...
            {
                // Convert the SQLPipelineMetrics for each run of the
                // BenchmarkItem into JSON.
                ...
                for (const auto &pipeline_metrics : run_result.metrics)
                {
                    auto pipeline_metrics_json = nlohmann::json{
                        {"parse_duration",
                         pipeline_metrics.parse_time_nanos.count()},
                        {"statements", nlohmann::json::array()}};

                    for (const auto &sql_statement_metrics :
                         pipeline_metrics.statement_metrics)
                    {
                        nlohmann::json rule_metrics_json;
                        for (const auto &rule_duration :
                             sql_statement_metrics->optimizer_rule_durations)
                        {
                            rule_metrics_json[rule_duration.rule_name] +=
                                rule_duration.duration.count();
                        }

                        auto sql_statement_metrics_json = nlohmann::json{
                            {"sql_translation_duration",
                             sql_statement_metrics->sql_translation_duration
                                 .count()},
                            {"optimization_duration",
                             sql_statement_metrics->optimization_duration
                                 .count()},
                            {"optimizer_rule_durations", rule_metrics_json},
                            {"lqp_translation_duration",
                             sql_statement_metrics->lqp_translation_duration
                                 .count()},
                            {"plan_execution_duration",
                             sql_statement_metrics->plan_execution_duration
                                 .count()},
                            {"query_plan_cache_hit",
                             sql_statement_metrics->query_plan_cache_hit}};

                        pipeline_metrics_json["statements"].push_back(
                            sql_statement_metrics_json);
                    }
        ...
        // For ordered benchmarks, report the time that this individual item
        // ran. For shuffled benchmarks, return the duration of the entire
        // benchmark. This means that items_per_second of ordered and shuffled
        // runs are not comparable.
        const auto reported_item_duration =
            _config.benchmark_mode == BenchmarkMode::Shuffled ? total_duration
                                                              : result.duration;
        // chrono::seconds uses an integer precision duration type, but we need
        // a floating-point value.
        const auto duration_seconds =
            std::chrono::duration<double>(reported_item_duration).count();
        const auto items_per_second =
            duration_seconds > 0
                ? (static_cast<double>(result.successful_runs.size()) /
                   duration_seconds)
        ...
        // The field items_per_second is relied upon by a number of
        // visualization scripts. Carefully consider if you really want to touch
        // this and potentially break the comparability across commits. Note
        // that items_per_second only includes successful iterations.
    ...
    auto summary = nlohmann::json{{"table_size_in_bytes", table_size},
                                  {"total_duration", total_duration.count()}};

    // To get timestamps relative to the benchmark start, we substract the
    // benchmark start timepoint. We have to use system_clock here, as the
    // LogManager uses it to provide human-readable timestamps. Because the
    // system_clock can be readjusted anytime, the timestamps could be slightly
    // out of line.
    const auto benchmark_start_ns =
        std::chrono::nanoseconds{_benchmark_wall_clock_start.time_since_epoch()}
            .count();
    auto log_json = _sql_to_json(
        std::string{"SELECT \"timestamp\" - "} +
        std::to_string(benchmark_start_ns) +
        " AS \"timestamp\", log_level, reporter, message FROM meta_log");

    auto report =
        nlohmann::json{{"context", _context},
                       {"benchmarks", std::move(benchmarks)},
                       {"log", std::move(log_json)},
                       {"summary", std::move(summary)},
                       {"table_generation", _table_generator->metrics}};

    // Add information that was temporarily stored in the `benchmark_...` tables
    // during the benchmark execution.
    if (Hyrise::get().storage_manager.has_table(
            "benchmark_system_utilization_log"))
    {
        report["system_utilization"] =
            _sql_to_json("SELECT * FROM benchmark_system_utilization_log");
    }

    if (Hyrise::get().storage_manager.has_table("benchmark_segments_log"))
    {
        report["segments"] =
            _sql_to_json("SELECT * FROM benchmark_segments_log");
    }
    return report;
}

void BenchmarkRunner::_write_report_to_file(const std::string &file_name,
...
cxxopts::Options
BenchmarkRunner::get_basic_cli_options(const std::string &benchmark_name)
{
    cxxopts::Options cli_options{benchmark_name};

    // Create a comma separated strings with the encoding and compression
    // options.
    const auto get_first =
        boost::adaptors::transformed([](const auto it) { return it.first; });
    const auto compression_strings_option = boost::algorithm::join(
        vector_compression_type_to_string.right | get_first, ", ");

    // Make TPC-C run in shuffled mode. While it can also run in ordered mode,
    // it would run out of orders to fulfill at some point. The way this is
    // solved here is not really nice, but as the TPC-C benchmark binary has
    // just a main method and not a class, retrieving this default value
    // properly would require some major refactoring of how benchmarks interact
    // with the BenchmarkRunner. At this moment, that does not seem to be worth
    // the effort.
    const auto *const default_mode =
        (benchmark_name == "TPC-C Benchmark" ? "Shuffled" : "Ordered");
    // Similar to the mode of the benchmark, we set a different setting for
    // table indexes when TPC-C is run. For analytical benchmarks, we do not
    // create indexes unless the user requests them as they are usually not used
    // by the query optimizer. In contrast, we create table indexes for TPC-C as
    // they improve the performance of selections on primary key columns.
    const auto *const default_table_indexes =
        (benchmark_name == "TPC-C Benchmark" ? "true" : "false");
...
nlohmann::json BenchmarkRunner::create_context(const BenchmarkConfig &config)
{
    ...
    auto local_time =
        *std::localtime(&current_time); // NOLINT(concurrency-mt-unsafe) - not
                                        // called in parallel
  ...
  std::stringstream compiler;
    ...
    return nlohmann::json{
        ...
        {"time_unit", "ns"},
        {"GIT-HASH",
         GIT_HEAD_SHA1 + std::string(GIT_IS_DIRTY ? "-dirty" : "")}};
}
...
nlohmann::json BenchmarkRunner::_sql_to_json(const std::string &sql)
{
    ...
    Assert(pipeline_status == SQLPipelineStatus::Success,
           "_sql_to_json failed");
...
void BenchmarkRunner::_snapshot_segment_access_counters(
    const std::string &moment)
{
    ...
    auto sql_builder = std::stringstream{};
    sql_builder
        << "INSERT INTO benchmark_segments_log SELECT " << _snapshot_id << ", '"
        << moment_or_timestamp + "'"
