...
const auto ssb_table_names = std::vector<std::string>{
    "part", "customer", "supplier", "date", "lineorder"};

SSBTableGenerator::SSBTableGenerator(const std::string &dbgen_path,
                                     const std::string &csv_meta_path,
                                     const std::string &data_path,
                                     float scale_factor, ChunkOffset chunk_size)
    : SSBTableGenerator(dbgen_path, csv_meta_path, data_path, scale_factor,
                        create_benchmark_config_with_chunk_size(chunk_size))
{
}

SSBTableGenerator::SSBTableGenerator(
    const std::string &dbgen_path, const std::string &csv_meta_path,
    ...
    : AbstractTableGenerator{benchmark_config},
      ...
      _dbgen_path{dbgen_path}, _csv_meta_path{csv_meta_path}, _scale_factor{
                                                                  scale_factor}
{
}

std::unordered_map<std::string, BenchmarkTableInfo>
SSBTableGenerator::generate()
{
    generate_csv_tables_with_external_dbgen(_dbgen_path, ssb_table_names,
                                            _csv_meta_path, _path,
                                            _scale_factor, "-T a");

    // Having generated the .csv files, call the FileBasedTableGenerator just as
    // if those files were user-provided.
    const auto &generated_tables = FileBasedTableGenerator::generate();

    // FileBasedTableGenerator automatically stores a binary file. Remove the
    // CSV data to save some space.
    ...
    const
{
    ...
    const auto lineorder_pk_constraint = TableKeyConstraint{
        {lineorder_table->column_id_by_name("lo_orderkey"),
         lineorder_table->column_id_by_name("lo_linenumber")},
        KeyConstraintType::PRIMARY_KEY};
    ...
    const auto date_pk_constraint =
        TableKeyConstraint{{date_table->column_id_by_name("d_datekey")},
                           KeyConstraintType::PRIMARY_KEY};
