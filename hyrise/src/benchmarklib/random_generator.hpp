...
class RandomGenerator
{
  public:
    ...
    /**
     * Generates a set of unique size_ts in the half open interval [0,
     * id_length). This function is used, e.g., to generate foreign key
     * relationships.
    ...
    std::set<size_t> select_unique_ids(size_t num_unique, size_t id_length)
    {
        ...
        Assert(num_unique <= id_length,
               "There are not enough IDs to be selected!");
        while (true)
        {
