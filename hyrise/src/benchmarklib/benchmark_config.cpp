...
BenchmarkConfig::BenchmarkConfig(
    ...
    const bool init_pipeline_metrics,
    const std::vector<std::string> &init_plugins)
    : benchmark_mode(init_benchmark_mode), chunk_size(init_chunk_size),
      encoding_config(init_encoding_config), chunk_indexes(init_chunk_indexes),
      table_indexes(init_table_indexes), max_runs(init_max_runs),
      max_duration(init_max_duration), warmup_duration(init_warmup_duration),
      output_file_path(init_output_file_path),
      enable_scheduler(init_enable_scheduler), cores(init_cores),
      data_preparation_cores(init_data_preparation_cores),
      clients(init_clients), enable_visualization(init_enable_visualization),
      verify(init_verify), cache_binary_tables(init_cache_binary_tables),
      system_metrics(init_system_metrics),
      pipeline_metrics(init_pipeline_metrics), plugins(init_plugins)
{
}
