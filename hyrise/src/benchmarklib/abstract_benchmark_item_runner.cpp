...
AbstractBenchmarkItemRunner::AbstractBenchmarkItemRunner(
    ...
    const std::filesystem::path &expected_results_directory_path)
{
    Assert(std::filesystem::is_directory(expected_results_directory_path),
           "Expected results path (" +
               expected_results_directory_path.string() +
        ...
        if (is_tbl_file(entry))
        {
            ...
            const auto iter =
                std::find_if(items().cbegin(), items().cend(),
                             [this, &item_name](const auto &item)
...
{
    // `_dedicated_expected_results` is either empty if
    // `load_dedicated_expected_results` was not called or a sparse vector with
    // the same size as `items()`.
...
AbstractBenchmarkItemRunner::execute_item(const BenchmarkItemID item_id)
{
    ...
    return {success, std::move(sql_executor.metrics),
            sql_executor.any_verification_failed};
}

void AbstractBenchmarkItemRunner::set_sqlite_wrapper(
