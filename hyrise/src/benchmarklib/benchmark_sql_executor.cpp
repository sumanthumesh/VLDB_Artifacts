...
BenchmarkSQLExecutor::BenchmarkSQLExecutor(
    const std::shared_ptr<SQLiteWrapper> &sqlite_wrapper,
    const std::optional<std::string> &visualize_prefix)
    : _sqlite_connection(
          sqlite_wrapper
              ? std::optional<SQLiteWrapper::Connection>{sqlite_wrapper
                                                             ->new_connection()}
    ...
    const std::shared_ptr<const Table> &expected_result_table)
{
    ...
    {
        _compare_tables(result_table, expected_result_table,
...
BenchmarkSQLExecutor::~BenchmarkSQLExecutor()
{
    if (transaction_context)
    {
        Assert(transaction_context->phase() == TransactionPhase::Committed ||
                   transaction_context->phase() ==
                       TransactionPhase::RolledBackByUser ||
                   transaction_context->phase() ==
                       TransactionPhase::RolledBackAfterConflict,
               "Explicitly created transaction context should have been "
               "explicitly committed or rolled back");
    }

    // If the benchmark item does not explicitly manage the transaction life
    // time by calling commit or rollback, we automatically commit the sqlite
    // transaction so that the next item can start a new one:
    if (_sqlite_transaction_open)
    {
        DebugAssert(_sqlite_connection,
                    "Expected an open connection to SQLite.");
...
void BenchmarkSQLExecutor::commit()
{
    ...
    Assert(transaction_context->phase() == TransactionPhase::Active,
...
{
    Assert(
        transaction_context && !transaction_context->is_auto_commit(),
...
{
    Assert(pipeline.statement_count() == 1,
           "Expecting single statement for SQLite verification");

    DebugAssert(_sqlite_connection, "Expected an open connection to SQLite.");
    const auto sqlite_result =
        _sqlite_connection->execute_query(pipeline.get_sql());
    const auto [pipeline_status, result_table] = pipeline.get_result_table();
    Assert(pipeline_status == SQLPipelineStatus::Success,
           "Non-successful pipeline should have been caught earlier");

    // Modifications (INSERT, UPDATE, DELETE) do not return a table. We do not
    // know what changed - we do not even know which table has been modified.
    // Extracting that info from the plan and verifying the entire table would
    // take way too long. As such, we rely on any errors here to be found when
    // the potentially corrupted data is SELECTed the next time.
    if (!result_table)
    {
        return;
    }

    _compare_tables(result_table, sqlite_result,
                    "Using SQLite's result table as expected result table");
}

void BenchmarkSQLExecutor::_compare_tables(
    ...
    const std::optional<const std::string> &description)
{
    ...
    {
        Assert(expected_result_table,
               "Verifying SQL statement on sqlite failed: No SQLite result");
        if (expected_result_table->row_count() == 0)
        {
            ...
            std::cout << "- Verification failed: Hyrise's actual result is not "
                         "empty, but the expected result is ("
                      << timer.lap_formatted() << ")"
                      << "\n";
        }
        else if (const auto table_difference_message = check_table_equal(
                     actual_result_table, expected_result_table,
                     OrderSensitivity::No, TypeCmpMode::Lenient,
    ...
    else
    {
        if (expected_result_table && expected_result_table->row_count() > 0)
        {
            ...
            std::cout << "- Verification failed: Expected result table is not "
                         "empty, but Hyrise's actual result is ("
                      << timer.lap_formatted() << ")"
                      << "\n";
        }
    }
...
void BenchmarkSQLExecutor::_visualize(SQLPipeline &pipeline)
{
    ...
    Assert(_visualize_prefix,
           "Expected a filename prefix when visualization is desired.");
    auto prefix = *_visualize_prefix;

    if (_num_visualized_plans == 1)
    {
        // We have already visualized a prior SQL pipeline in this benchmark
        // item - rename the existing file
    ...
    LQPVisualizer{graphviz_config, {}, {}, {}}.visualize(lqps,
                                                         prefix + "-LQP.svg");
    PQPVisualizer{graphviz_config, {}, {}, {}}.visualize(pqps,
                                                         prefix + "-PQP.svg");
