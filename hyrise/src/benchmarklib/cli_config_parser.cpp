...
BenchmarkConfig
CLIConfigParser::parse_cli_options(const cxxopts::ParseResult &parse_result)
{
    ...
    const auto number_of_cores_str =
        (cores == 0) ? "all available" : std::to_string(cores);
    const auto core_info =
        enable_scheduler ? " using " + number_of_cores_str + " cores" : "";
    std::cout << "- Running in " +
                     std::string(enable_scheduler ? "multi" : "single") +
                     "-threaded mode"
              << core_info << std::endl;
    const auto data_preparation_cores =
        parse_result["data_preparation_cores"].as<uint32_t>();
    const auto number_of_data_preparation_cores_str =
        (data_preparation_cores == 0) ? "all available"
                                      : std::to_string(data_preparation_cores);
    std::cout << "- Data preparation will use "
              << number_of_data_preparation_cores_str
    ...
    std::cout << (clients == 1 ? "client is " : "clients are ")
              << "scheduling items";
    ...
    if (cores != default_config.cores || clients != default_config.clients)
    {
        if (!enable_scheduler)
        {
            PerformanceWarning("'--cores' or '--clients' specified but "
                               "ignored, because '--scheduler' is false");
        }
    }
    ...
    {
        std::cout << "\n\n- WARNING: You are running in multi-threaded (MT) "
                     "mode but have set --clients=1.\n";
        std::cout << "           You will achieve better MT performance by "
    ...
    auto benchmark_mode =
        BenchmarkMode::Ordered; // Just to init it deterministically
    ...
    {
        throw std::runtime_error("Invalid benchmark mode: '" +
                                 benchmark_mode_str + "'");
    }
    ...
    {
        Assert(clients == 1, "Cannot visualize plans with multiple clients as "
                             "files may be overwritten");
        std::cout << "- Visualizing the plans into SVG files. This will make "
                     "the performance numbers invalid."
                  << std::endl;
    }
    ...
    const auto compression_type_str =
        parse_result["compression"].as<std::string>();
    if (boost::algorithm::ends_with(encoding_type_str, ".json"))
    {
        ...
        encoding_config = std::make_unique<EncodingConfig>(
            parse_encoding_config(encoding_type_str));
        std::cout << "- Encoding is custom from " << encoding_type_str << ""
                  << std::endl;

        Assert(compression_type_str.empty(),
               "Specified both compression type and an encoding file. Invalid "
               "combination.");
    }
    else
    {
        encoding_config = std::make_unique<EncodingConfig>(
            EncodingConfig::encoding_spec_from_strings(encoding_type_str,
    ...
    if (chunk_indexes)
    {
        std::cout << "- Creating chunk indexes (separate index per chunk; "
                     "columns defined by benchmark)"
                  << std::endl;
    }
    ...
    if (table_indexes)
    {
        std::cout << "- Creating table indexes (index per table column; "
                     "columns defined by benchmark)"
                  << std::endl;
    }
    ...
    if (verify)
    {
        std::cout << "- Automatically verifying results with SQLite. This will "
                     "make the performance numbers invalid."
                  << std::endl;
    }

    const auto cache_binary_tables =
        !parse_result["dont_cache_binary_tables"].as<bool>();
    ...
    if (system_metrics)
    {
        Assert(!output_file_string.empty(),
               "--system_metrics only makes sense when an output file is set.");
    ...
    {
        Assert(
            !output_file_string.empty(),
            "--pipeline_metrics only makes sense when an output file is set.");
    ...
    if (!comma_separated_plugins.empty())
    {
        ...
        boost::split(plugins, comma_separated_plugins, boost::is_any_of(","),
...
EncodingConfig
CLIConfigParser::parse_encoding_config(const std::string &encoding_file_str)
{
    Assert(std::filesystem::is_regular_file(encoding_file_str),
           "No such file: " + encoding_file_str);
    ...
    const auto encoding_spec_from_json = [](const nlohmann::json &json_spec)
    {
        ...
        return EncodingConfig::encoding_spec_from_strings(encoding_str,
                                                          compression_str);
    };

    Assert(encoding_config_json.count("default"),
           "Config must contain default encoding.");
    const auto default_spec =
        encoding_spec_from_json(encoding_config_json["default"]);

    DataTypeEncodingMapping type_encoding_mapping;
    const auto has_type_encoding =
        encoding_config_json.find("type") != encoding_config_json.end();
    if (has_type_encoding)
    {
        const auto type_encoding = encoding_config_json["type"];
        Assert(type_encoding.is_object(),
               "The type encoding needs to be specified as a json object.");
        ...
        for (const auto &type : type_encoding.items())
        {
            ...
            Assert(encoding_info.is_object(), "The type encoding info needs to "
                                              "be specified as a json object.");

            const auto data_type = data_type_it->second;
            type_encoding_mapping[data_type] =
                encoding_spec_from_json(encoding_info);
        }
    }

    TableSegmentEncodingMapping custom_encoding_mapping;
    const auto has_custom_encoding =
        encoding_config_json.find("custom") != encoding_config_json.end();

    if (has_custom_encoding)
    {
        const auto custom_encoding = encoding_config_json["custom"];
        Assert(custom_encoding.is_object(),
               "The custom table encoding needs to be specified as a json "
               "object.");
        ...
        for (const auto &table : custom_encoding.items())
        {
            ...
            Assert(columns.is_object(),
                   "The custom encoding for column types needs to be specified "
                   "as a json object.");
            custom_encoding_mapping.emplace(
                table_name,
                std::unordered_map<std::string, SegmentEncodingSpec>());
            ...
            for (const auto &column : columns.items())
            {
                ...
                Assert(encoding_info.is_object(),
                       "The custom encoding for column types needs to be "
                       "specified as a json object.");
                custom_encoding_mapping[table_name][column_name] =
                    encoding_spec_from_json(encoding_info);
            }
        }
    }

    return EncodingConfig{default_spec, std::move(type_encoding_mapping),
                          std::move(custom_encoding_mapping)};
}

bool CLIConfigParser::print_help_if_requested(
