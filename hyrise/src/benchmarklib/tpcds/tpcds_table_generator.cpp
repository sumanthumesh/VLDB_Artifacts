...
void init_tpcds_tools(uint32_t scale_factor, int rng_seed)
{
    ...
    auto distributions_value =
        std::string{"resources/benchmark/tpcds/tpcds.idx"};
...
std::optional<pmr_string> resolve_date_id(int column_id, ds_key_t date_id)
{
    ...
    const auto snprintf_rc =
        std::snprintf(result.data(), result.size() + 1, "%4d-%02d-%02d",
...
{
    return nullCheck(column_id) || key == -1
               ? std::nullopt
               : std::optional{static_cast<tpcds_key_t>(key)};
}

std::optional<pmr_string> resolve_string(int column_id, pmr_string string)
{
    return nullCheck(column_id) || string.empty()
               ? std::nullopt
               : std::optional{std::move(string)};
}
...
std::optional<float> resolve_decimal(int column_id, decimal_t decimal)
{
    ...
    return nullCheck(column_id)
               ? std::nullopt
               : std::optional{static_cast<float>(result / 10)};
}

std::optional<float> resolve_gmt_offset(int column_id, int32_t gmt_offset)
{
    return nullCheck(column_id) ? std::nullopt
                                : std::optional{static_cast<float>(gmt_offset)};
}

std::optional<pmr_string> resolve_street_name(int column_id,
                                              const ds_addr_t &address)
{
    return nullCheck(column_id) ? std::nullopt
           : address.street_name2 == nullptr
               ? std::optional{pmr_string{address.street_name1}}
               : std::optional{pmr_string{address.street_name1} + " " +
                               address.street_name2};
}

// mapping types used by tpcds-dbgen as follows (according to create table
// statements in tpcds.sql): ds_key_t -> tpcds_key_t int -> int32_t char*,
// char[], bool, date (ds_key_t as date_id), time (ds_key_t as time_id) ->
// pmr_string decimal, float -> float ds_addr_t -> corresponding types for types
// in struct ds_addr_t, see address.h
...
TPCDSTableGenerator::TPCDSTableGenerator(uint32_t scale_factor,
                                         ChunkOffset chunk_size, int rng_seed)
...
TPCDSTableGenerator::TPCDSTableGenerator(
...
std::unordered_map<std::string, BenchmarkTableInfo>
TPCDSTableGenerator::generate()
{
    const auto cache_directory =
        "tpcds_cached_tables/sf-" + std::to_string(_scale_factor); // NOLINT
    ...
    for (const auto &table_name :
         {"call_center", "catalog_page", "customer_address", "customer",
    ...
    for (const auto &[sales_table_name, returns_table_name] :
         std::vector<std::pair<std::string, std::string>>{
             {"catalog_sales", "catalog_returns"},
             {"store_sales", "store_returns"},
             {"web_sales", "web_returns"}})
    {
        auto catalog_sales_and_returns =
            _generate_sales_and_returns_tables(sales_table_name);
        table_info_by_name[sales_table_name].table =
            catalog_sales_and_returns.first;
        table_info_by_name[returns_table_name].table =
            catalog_sales_and_returns.second;
    }
    ...
    if (_benchmark_config->cache_binary_tables)
    {
        ...
        for (auto &[table_name, table_info] : table_info_by_name)
        {
            table_info.binary_file_path =
                cache_directory + "/" + table_name + ".bin"; // NOLINT
        }
    }
...
TPCDSTableGenerator::_generate_table(const std::string &table_name) const
{
...
TPCDSTableGenerator::_generate_sales_and_returns_tables(
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_call_center(ds_key_t max_rows) const
{
    auto [call_center_first, call_center_count] =
        prepare_for_table(CALL_CENTER);
    call_center_count = std::min(call_center_count, max_rows);

    auto call_center_builder = TableBuilder{
        _benchmark_config->chunk_size, call_center_column_types,
         ...
         call_center_index < call_center_count; ++call_center_index)
    {
        ...
        call_center_builder.append_row(
            ...
            resolve_date_id(CC_REC_START_DATE_ID,
                            call_center.cc_rec_start_date_id),
            ...
            resolve_string(CC_ADDRESS, pmr_string{std::to_string(
                                           call_center.cc_address.street_num)}),
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_catalog_page(ds_key_t max_rows) const
{
    auto [catalog_page_first, catalog_page_count] =
        prepare_for_table(CATALOG_PAGE);
    catalog_page_count = std::min(catalog_page_count, max_rows);

    auto catalog_page_builder =
        TableBuilder{_benchmark_config->chunk_size, catalog_page_column_types,
                     catalog_page_column_names,
                     static_cast<ChunkOffset>(catalog_page_count)};

    auto catalog_page = CATALOG_PAGE_TBL{};
    // NOLINTBEGIN(cppcoreguidelines-pro-type-vararg,hicpp-vararg)
    const auto snprintf_rc =
        std::snprintf(catalog_page.cp_department,
                      sizeof(catalog_page.cp_department), "%s", "DEPARTMENT");
    // NOLINTEND(cppcoreguidelines-pro-type-vararg,hicpp-vararg)
    Assert(snprintf_rc > 0, "Unexpected string to parse.");
    for (auto catalog_page_index = ds_key_t{0};
         catalog_page_index < catalog_page_count; ++catalog_page_index)
    {
        // need a pointer to the previous result of mk_w_catalog_page, because
        // cp_department is only set once
        mk_w_catalog_page(&catalog_page,
                          catalog_page_first + catalog_page_index);
        ...
        catalog_page_builder.append_row(
            ...
            resolve_integer(CP_CATALOG_PAGE_NUMBER,
...
std::pair<std::shared_ptr<Table>, std::shared_ptr<Table>>
TPCDSTableGenerator::generate_catalog_sales_and_returns(ds_key_t max_rows) const
{
    auto [catalog_sales_first, catalog_sales_count] =
        prepare_for_table(CATALOG_SALES);
    // catalog_sales_count is NOT the actual number of catalog sales created,
    // for each of these "master" catalog_sales multiple "detail" catalog sales
    // are created and possibly returned

    auto catalog_sales_builder =
        TableBuilder{_benchmark_config->chunk_size, catalog_sales_column_types,
                     catalog_sales_column_names,
                     static_cast<ChunkOffset>(catalog_sales_count)};

    auto catalog_returns_builder = TableBuilder{_benchmark_config->chunk_size,
                                                catalog_returns_column_types,
                                                catalog_returns_column_names};

    for (auto catalog_sale_index = ds_key_t{0};
         catalog_sale_index < catalog_sales_count; ++catalog_sale_index)
    {
        auto catalog_sales = W_CATALOG_SALES_TBL{};
        auto catalog_returns = W_CATALOG_RETURNS_TBL{};

        // modified call to mk_w_catalog_sales(&catalog_sales,
        // catalog_sales_first + catalog_sale_index,
        //                                     &catalog_returns, &was_returned);
        {
            mk_w_catalog_sales_master(
                &catalog_sales, catalog_sales_first + catalog_sale_index, 0);
            auto n_lineitems = int{0};
            genrand_integer(&n_lineitems, DIST_UNIFORM, 4, 14, 0,
                            CS_ORDER_NUMBER);
            for (auto lineitem_index = int{0}; lineitem_index < n_lineitems;
                 ++lineitem_index)
            {
                auto was_returned = int{0};
                mk_w_catalog_sales_detail(&catalog_sales, 0, &catalog_returns,
                                          &was_returned);

                if (catalog_sales_builder.row_count() <
                    static_cast<size_t>(max_rows))
                {
                    catalog_sales_builder.append_row(
                        resolve_key(CS_SOLD_DATE_SK,
                                    catalog_sales.cs_sold_date_sk),
                        resolve_key(CS_SOLD_TIME_SK,
                                    catalog_sales.cs_sold_time_sk),
                        resolve_key(CS_SHIP_DATE_SK,
                                    catalog_sales.cs_ship_date_sk),
                        resolve_key(CS_BILL_CUSTOMER_SK,
                                    catalog_sales.cs_bill_customer_sk),
                        resolve_key(CS_BILL_CDEMO_SK,
                                    catalog_sales.cs_bill_cdemo_sk),
                        resolve_key(CS_BILL_HDEMO_SK,
                                    catalog_sales.cs_bill_hdemo_sk),
                        resolve_key(CS_BILL_ADDR_SK,
                                    catalog_sales.cs_bill_addr_sk),
                        resolve_key(CS_SHIP_CUSTOMER_SK,
                                    catalog_sales.cs_ship_customer_sk),
                        resolve_key(CS_SHIP_CDEMO_SK,
                                    catalog_sales.cs_ship_cdemo_sk),
                        resolve_key(CS_SHIP_HDEMO_SK,
                                    catalog_sales.cs_ship_hdemo_sk),
                        resolve_key(CS_SHIP_ADDR_SK,
                                    catalog_sales.cs_ship_addr_sk),
                        resolve_key(CS_CALL_CENTER_SK,
                                    catalog_sales.cs_call_center_sk),
                        resolve_key(CS_CATALOG_PAGE_SK,
                                    catalog_sales.cs_catalog_page_sk),
                        resolve_key(CS_SHIP_MODE_SK,
                                    catalog_sales.cs_ship_mode_sk),
                        resolve_key(CS_WAREHOUSE_SK,
                                    catalog_sales.cs_warehouse_sk),
                        catalog_sales.cs_sold_item_sk,
                        resolve_key(CS_PROMO_SK, catalog_sales.cs_promo_sk),
                        catalog_sales.cs_order_number,
                        resolve_integer(CS_PRICING_QUANTITY,
                                        catalog_sales.cs_pricing.quantity),
                        resolve_decimal(
                            CS_PRICING_WHOLESALE_COST,
                            catalog_sales.cs_pricing.wholesale_cost),
                        resolve_decimal(CS_PRICING_LIST_PRICE,
                                        catalog_sales.cs_pricing.list_price),
                        resolve_decimal(CS_PRICING_SALES_PRICE,
                                        catalog_sales.cs_pricing.sales_price),
                        resolve_decimal(
                            CS_PRICING_EXT_DISCOUNT_AMOUNT,
                            catalog_sales.cs_pricing.ext_discount_amt),
                        resolve_decimal(
                            CS_PRICING_EXT_SALES_PRICE,
                            catalog_sales.cs_pricing.ext_sales_price),
                        resolve_decimal(
                            CS_PRICING_EXT_WHOLESALE_COST,
                            catalog_sales.cs_pricing.ext_wholesale_cost),
                        resolve_decimal(
                            CS_PRICING_EXT_LIST_PRICE,
                            catalog_sales.cs_pricing.ext_list_price),
                        resolve_decimal(CS_PRICING_EXT_TAX,
                                        catalog_sales.cs_pricing.ext_tax),
                        resolve_decimal(CS_PRICING_COUPON_AMT,
                                        catalog_sales.cs_pricing.coupon_amt),
                        resolve_decimal(CS_PRICING_EXT_SHIP_COST,
                                        catalog_sales.cs_pricing.ext_ship_cost),
                        resolve_decimal(CS_PRICING_NET_PAID,
                                        catalog_sales.cs_pricing.net_paid),
                        resolve_decimal(
                            CS_PRICING_NET_PAID_INC_TAX,
                            catalog_sales.cs_pricing.net_paid_inc_tax),
                        resolve_decimal(
                            CS_PRICING_NET_PAID_INC_SHIP,
                            catalog_sales.cs_pricing.net_paid_inc_ship),
                        resolve_decimal(
                            CS_PRICING_NET_PAID_INC_SHIP_TAX,
                            catalog_sales.cs_pricing.net_paid_inc_ship_tax),
                        resolve_decimal(CS_PRICING_NET_PROFIT,
                                        catalog_sales.cs_pricing.net_profit));
                }

                if (was_returned != 0)
                {
                    catalog_returns_builder.append_row(
                        resolve_key(CR_RETURNED_DATE_SK,
                                    catalog_returns.cr_returned_date_sk),
                        resolve_key(CR_RETURNED_TIME_SK,
                                    catalog_returns.cr_returned_time_sk),
                        catalog_returns.cr_item_sk,
                        resolve_key(CR_REFUNDED_CUSTOMER_SK,
                                    catalog_returns.cr_refunded_customer_sk),
                        resolve_key(CR_REFUNDED_CDEMO_SK,
                                    catalog_returns.cr_refunded_cdemo_sk),
                        resolve_key(CR_REFUNDED_HDEMO_SK,
                                    catalog_returns.cr_refunded_hdemo_sk),
                        resolve_key(CR_REFUNDED_ADDR_SK,
                                    catalog_returns.cr_refunded_addr_sk),
                        resolve_key(CR_RETURNING_CUSTOMER_SK,
                                    catalog_returns.cr_returning_customer_sk),
                        resolve_key(CR_RETURNING_CDEMO_SK,
                                    catalog_returns.cr_returning_cdemo_sk),
                        resolve_key(CR_RETURNING_HDEMO_SK,
                                    catalog_returns.cr_returning_hdemo_sk),
                        resolve_key(CR_RETURNING_ADDR_SK,
                                    catalog_returns.cr_returning_addr_sk),
                        resolve_key(CR_CALL_CENTER_SK,
                                    catalog_returns.cr_call_center_sk),
                        resolve_key(CR_CATALOG_PAGE_SK,
                                    catalog_returns.cr_catalog_page_sk),
                        resolve_key(CR_SHIP_MODE_SK,
                                    catalog_returns.cr_ship_mode_sk),
                        resolve_key(CR_WAREHOUSE_SK,
                                    catalog_returns.cr_warehouse_sk),
                        resolve_key(CR_REASON_SK, catalog_returns.cr_reason_sk),
                        catalog_returns.cr_order_number,
                        resolve_integer(CR_PRICING_QUANTITY,
                                        catalog_returns.cr_pricing.quantity),
                        resolve_decimal(CR_PRICING_NET_PAID,
                                        catalog_returns.cr_pricing.net_paid),
                        resolve_decimal(CR_PRICING_EXT_TAX,
                                        catalog_returns.cr_pricing.ext_tax),
                        resolve_decimal(
                            CR_PRICING_NET_PAID_INC_TAX,
                            catalog_returns.cr_pricing.net_paid_inc_tax),
                        resolve_decimal(CR_PRICING_FEE,
                                        catalog_returns.cr_pricing.fee),
                        resolve_decimal(
                            CR_PRICING_EXT_SHIP_COST,
                            catalog_returns.cr_pricing.ext_ship_cost),
                        resolve_decimal(
                            CR_PRICING_REFUNDED_CASH,
                            catalog_returns.cr_pricing.refunded_cash),
                        resolve_decimal(
                            CR_PRICING_REVERSED_CHARGE,
                            catalog_returns.cr_pricing.reversed_charge),
                        resolve_decimal(
                            CR_PRICING_STORE_CREDIT,
                            catalog_returns.cr_pricing.store_credit),
                        resolve_decimal(CR_PRICING_NET_LOSS,
                                        catalog_returns.cr_pricing.net_loss));

                    if (catalog_returns_builder.row_count() ==
                        static_cast<size_t>(max_rows))
                    {
                        break;
                    }
    ...
    return {catalog_sales_builder.finish_table(),
            catalog_returns_builder.finish_table()};
}

std::shared_ptr<Table>
TPCDSTableGenerator::generate_customer_address(ds_key_t max_rows) const
{
    auto [customer_address_first, customer_address_count] =
        prepare_for_table(CUSTOMER_ADDRESS);
    customer_address_count = std::min(customer_address_count, max_rows);

    auto customer_address_builder = TableBuilder{
        _benchmark_config->chunk_size, customer_address_column_types,
        customer_address_column_names,
        static_cast<ChunkOffset>(customer_address_count)};

    for (auto customer_address_index = ds_key_t{0};
         customer_address_index < customer_address_count;
         ++customer_address_index)
    {
        const auto customer_address =
            call_dbgen_mk<W_CUSTOMER_ADDRESS_TBL, &mk_w_customer_address,
                          CUSTOMER_ADDRESS>(customer_address_first +
                                            customer_address_index);

        customer_address_builder.append_row(
            customer_address.ca_addr_sk, customer_address.ca_addr_id,
            resolve_string(CA_ADDRESS_STREET_NUM,
                           pmr_string{std::to_string(
                               customer_address.ca_address.street_num)}),
            resolve_street_name(CA_ADDRESS_STREET_NAME1,
                                customer_address.ca_address),
            resolve_string(CA_ADDRESS_STREET_TYPE,
                           customer_address.ca_address.street_type),
            resolve_string(CA_ADDRESS_SUITE_NUM,
                           customer_address.ca_address.suite_num),
            resolve_string(CA_ADDRESS_CITY, customer_address.ca_address.city),
            resolve_string(CA_ADDRESS_COUNTY,
                           customer_address.ca_address.county),
            resolve_string(CA_ADDRESS_STATE, customer_address.ca_address.state),
            resolve_string(CA_ADDRESS_ZIP,
                           zip_to_string(customer_address.ca_address.zip)),
            resolve_string(CA_ADDRESS_COUNTRY,
                           customer_address.ca_address.country),
            resolve_gmt_offset(CA_ADDRESS_GMT_OFFSET,
                               customer_address.ca_address.gmt_offset),
            resolve_string(CA_LOCATION_TYPE,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_customer(ds_key_t max_rows) const
{
    auto [customer_first, customer_count] = prepare_for_table(CUSTOMER);
    customer_count = std::min(customer_count, max_rows);

    auto customer_builder = TableBuilder{
        _benchmark_config->chunk_size, customer_column_types,
        customer_column_names, static_cast<ChunkOffset>(customer_count)};

    for (auto customer_index = ds_key_t{0}; customer_index < customer_count;
         ++customer_index)
    {
        const auto customer =
            call_dbgen_mk<W_CUSTOMER_TBL, &mk_w_customer, CUSTOMER>(
                customer_first + customer_index);
        ...
        customer_builder.append_row(
            ...
            resolve_integer(C_FIRST_SHIPTO_DATE_ID,
                            customer.c_first_shipto_date_id),
            resolve_integer(C_FIRST_SALES_DATE_ID,
                            customer.c_first_sales_date_id),
            resolve_string(C_SALUTATION, customer.c_salutation),
            resolve_string(C_FIRST_NAME, customer.c_first_name),
            resolve_string(C_LAST_NAME, customer.c_last_name),
            resolve_string(C_PREFERRED_CUST_FLAG,
                           boolean_to_string(customer.c_preferred_cust_flag)),
            resolve_integer(C_BIRTH_DAY, customer.c_birth_day),
            resolve_integer(C_BIRTH_MONTH, customer.c_birth_month),
            resolve_integer(C_BIRTH_YEAR, customer.c_birth_year),
            resolve_string(C_BIRTH_COUNTRY, customer.c_birth_country),
            resolve_string(C_LOGIN, customer.c_login),
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_customer_demographics(ds_key_t max_rows) const
{
    auto [customer_demographics_first, customer_demographics_count] =
        prepare_for_table(CUSTOMER_DEMOGRAPHICS);
    customer_demographics_count =
        std::min(customer_demographics_count, max_rows);

    auto customer_demographics_builder = TableBuilder{
        _benchmark_config->chunk_size, customer_demographics_column_types,
        customer_demographics_column_names,
        static_cast<ChunkOffset>(customer_demographics_count)};

    for (auto customer_demographic = ds_key_t{0};
         customer_demographic < customer_demographics_count;
         ++customer_demographic)
    {
        const auto customer_demographics =
            call_dbgen_mk<W_CUSTOMER_DEMOGRAPHICS_TBL,
                          &mk_w_customer_demographics, CUSTOMER_DEMOGRAPHICS>(
        ...
        customer_demographics_builder.append_row(
            ...
            resolve_string(CD_MARITAL_STATUS,
                           customer_demographics.cd_marital_status),
            resolve_string(CD_EDUCATION_STATUS,
                           customer_demographics.cd_education_status),
            resolve_integer(CD_PURCHASE_ESTIMATE,
                            customer_demographics.cd_purchase_estimate),
            resolve_string(CD_CREDIT_RATING,
                           customer_demographics.cd_credit_rating),
            resolve_integer(CD_DEP_COUNT, customer_demographics.cd_dep_count),
            resolve_integer(CD_DEP_EMPLOYED_COUNT,
                            customer_demographics.cd_dep_employed_count),
            resolve_integer(CD_DEP_COLLEGE_COUNT,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_date_dim(ds_key_t max_rows) const
{
    auto [date_first, date_count] = prepare_for_table(DATE);
    date_count = std::min(date_count, max_rows);

    auto date_builder =
        TableBuilder{_benchmark_config->chunk_size, date_column_types,
                     date_column_names, static_cast<ChunkOffset>(date_count)};

    for (auto date_index = ds_key_t{0}; date_index < date_count; ++date_index)
    {
        const auto date = call_dbgen_mk<W_DATE_TBL, &mk_w_date, DATE>(
            date_first + date_index);

        auto quarter_name = pmr_string{std::to_string(date.d_year) + "Q" +
                                       std::to_string(date.d_qoy)};
        ...
        date_builder.append_row(
            ...
            resolve_string(D_FOLLOWING_HOLIDAY,
                           boolean_to_string(date.d_following_holiday)),
            ...
            resolve_string(D_CURRENT_DAY,
                           boolean_to_string(date.d_current_day)),
            resolve_string(D_CURRENT_WEEK,
                           boolean_to_string(date.d_current_week)),
            resolve_string(D_CURRENT_MONTH,
                           boolean_to_string(date.d_current_month)),
            resolve_string(D_CURRENT_QUARTER,
                           boolean_to_string(date.d_current_quarter)),
            resolve_string(D_CURRENT_YEAR,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_household_demographics(ds_key_t max_rows) const
{
    auto [household_demographics_first, household_demographics_count] =
        prepare_for_table(HOUSEHOLD_DEMOGRAPHICS);
    household_demographics_count =
        std::min(household_demographics_count, max_rows);

    auto household_demographics_builder = TableBuilder{
        _benchmark_config->chunk_size, household_demographics_column_types,
        household_demographics_column_names,
        static_cast<ChunkOffset>(household_demographics_count)};

    for (auto household_demographic = ds_key_t{0};
         household_demographic < household_demographics_count;
         ++household_demographic)
    {
        const auto household_demographics =
            call_dbgen_mk<W_HOUSEHOLD_DEMOGRAPHICS_TBL,
                          &mk_w_household_demographics, HOUSEHOLD_DEMOGRAPHICS>(
                household_demographics_first + household_demographic);

        household_demographics_builder.append_row(
            household_demographics.hd_demo_sk,

            resolve_key(HD_INCOME_BAND_ID,
                        household_demographics.hd_income_band_id),
            resolve_string(HD_BUY_POTENTIAL,
                           household_demographics.hd_buy_potential),
            resolve_integer(HD_DEP_COUNT, household_demographics.hd_dep_count),
            resolve_integer(HD_VEHICLE_COUNT,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_income_band(ds_key_t max_rows) const
{
    auto [income_band_first, income_band_count] =
        prepare_for_table(INCOME_BAND);
    income_band_count = std::min(income_band_count, max_rows);

    auto income_band_builder = TableBuilder{
        _benchmark_config->chunk_size, income_band_column_types,
        income_band_column_names, static_cast<ChunkOffset>(income_band_count)};

    for (auto income_band_index = ds_key_t{0};
         income_band_index < income_band_count; ++income_band_index)
    {
        const auto income_band =
            call_dbgen_mk<W_INCOME_BAND_TBL, &mk_w_income_band, INCOME_BAND>(
                income_band_first + income_band_index);
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_inventory(ds_key_t max_rows) const
{
    auto [inventory_first, inventory_count] = prepare_for_table(INVENTORY);
    inventory_count = std::min(inventory_count, max_rows);

    auto inventory_builder = TableBuilder{
        _benchmark_config->chunk_size, inventory_column_types,
        inventory_column_names, static_cast<ChunkOffset>(inventory_count)};

    for (auto inventory_index = ds_key_t{0}; inventory_index < inventory_count;
         ++inventory_index)
    {
        const auto inventory =
            call_dbgen_mk<W_INVENTORY_TBL, &mk_w_inventory, INVENTORY>(
                inventory_first + inventory_index);

        inventory_builder.append_row(
            inventory.inv_date_sk, inventory.inv_item_sk,
            inventory.inv_warehouse_sk,
            resolve_integer(INV_QUANTITY_ON_HAND,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_item(ds_key_t max_rows) const
{
    auto [item_first, item_count] = prepare_for_table(ITEM);
    item_count = std::min(item_count, max_rows);

    auto item_builder =
        TableBuilder{_benchmark_config->chunk_size, item_column_types,
                     item_column_names, static_cast<ChunkOffset>(item_count)};

    for (auto item_index = ds_key_t{0}; item_index < item_count; ++item_index)
    {
        const auto item = call_dbgen_mk<W_ITEM_TBL, &mk_w_item, ITEM>(
            item_first + item_index);

        item_builder.append_row(
            item.i_item_sk, item.i_item_id,
            resolve_date_id(I_REC_START_DATE_ID, item.i_rec_start_date_id),
            resolve_date_id(I_REC_END_DATE_ID, item.i_rec_end_date_id),
            resolve_string(I_ITEM_DESC, item.i_item_desc),
            resolve_decimal(I_CURRENT_PRICE, item.i_current_price),
            resolve_decimal(I_WHOLESALE_COST, item.i_wholesale_cost),
            resolve_key(I_BRAND_ID, item.i_brand_id),
            resolve_string(I_BRAND, item.i_brand),
            resolve_key(I_CLASS_ID, item.i_class_id),
            resolve_string(I_CLASS, item.i_class),
            resolve_key(I_CATEGORY_ID, item.i_category_id),
            resolve_string(I_CATEGORY, item.i_category),
            resolve_key(I_MANUFACT_ID, item.i_manufact_id),
            resolve_string(I_MANUFACT, item.i_manufact),
            resolve_string(I_SIZE, item.i_size),
            resolve_string(I_FORMULATION, item.i_formulation),
            resolve_string(I_COLOR, item.i_color),
            resolve_string(I_UNITS, item.i_units),
            resolve_string(I_CONTAINER, item.i_container),
            resolve_key(I_MANAGER_ID, item.i_manager_id),
            resolve_string(I_PRODUCT_NAME, item.i_product_name));
    }
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_promotion(ds_key_t max_rows) const
{
    auto [promotion_first, promotion_count] = prepare_for_table(PROMOTION);
    promotion_count = std::min(promotion_count, max_rows);

    auto promotion_builder = TableBuilder{
        _benchmark_config->chunk_size, promotion_column_types,
        promotion_column_names, static_cast<ChunkOffset>(promotion_count)};

    for (auto promotion_index = ds_key_t{0}; promotion_index < promotion_count;
         ++promotion_index)
    {
        const auto promotion =
            call_dbgen_mk<W_PROMOTION_TBL, &mk_w_promotion, PROMOTION>(
                promotion_first + promotion_index);

        promotion_builder.append_row(
            promotion.p_promo_sk, promotion.p_promo_id,
            ...
            resolve_string(P_CHANNEL_DMAIL,
                           boolean_to_string(promotion.p_channel_dmail)),
            resolve_string(P_CHANNEL_EMAIL,
                           boolean_to_string(promotion.p_channel_email)),
            resolve_string(P_CHANNEL_CATALOG,
                           boolean_to_string(promotion.p_channel_catalog)),
            resolve_string(P_CHANNEL_TV,
                           boolean_to_string(promotion.p_channel_tv)),
            resolve_string(P_CHANNEL_RADIO,
                           boolean_to_string(promotion.p_channel_radio)),
            resolve_string(P_CHANNEL_PRESS,
                           boolean_to_string(promotion.p_channel_press)),
            resolve_string(P_CHANNEL_EVENT,
                           boolean_to_string(promotion.p_channel_event)),
            resolve_string(P_CHANNEL_DEMO,
                           boolean_to_string(promotion.p_channel_demo)),
            resolve_string(P_CHANNEL_DETAILS, promotion.p_channel_details),
            resolve_string(P_PURPOSE, promotion.p_purpose),
            resolve_string(P_DISCOUNT_ACTIVE,
                           boolean_to_string(promotion.p_discount_active)));
    }
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_reason(ds_key_t max_rows) const
{
    auto [reason_first, reason_count] = prepare_for_table(REASON);
    reason_count = std::min(reason_count, max_rows);

    auto reason_builder = TableBuilder{_benchmark_config->chunk_size,
                                       reason_column_types, reason_column_names,
                                       static_cast<ChunkOffset>(reason_count)};

    for (auto reason_index = ds_key_t{0}; reason_index < reason_count;
         ++reason_index)
    {
        const auto reason = call_dbgen_mk<W_REASON_TBL, &mk_w_reason, REASON>(
            reason_first + reason_index);

        reason_builder.append_row(
            reason.r_reason_sk, reason.r_reason_id,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_ship_mode(ds_key_t max_rows) const
{
    auto [ship_mode_first, ship_mode_count] = prepare_for_table(SHIP_MODE);
    ship_mode_count = std::min(ship_mode_count, max_rows);

    auto ship_mode_builder = TableBuilder{
        _benchmark_config->chunk_size, ship_mode_column_types,
        ship_mode_column_names, static_cast<ChunkOffset>(ship_mode_count)};

    for (auto ship_mode_index = ds_key_t{0}; ship_mode_index < ship_mode_count;
         ++ship_mode_index)
    {
        const auto ship_mode =
            call_dbgen_mk<W_SHIP_MODE_TBL, &mk_w_ship_mode, SHIP_MODE>(
                ship_mode_first + ship_mode_index);

        ship_mode_builder.append_row(
            ship_mode.sm_ship_mode_sk, ship_mode.sm_ship_mode_id,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_store(ds_key_t max_rows) const
{
    auto [store_first, store_count] = prepare_for_table(STORE);
    store_count = std::min(store_count, max_rows);

    auto store_builder =
        TableBuilder{_benchmark_config->chunk_size, store_column_types,
                     store_column_names, static_cast<ChunkOffset>(store_count)};

    for (auto store_index = ds_key_t{0}; store_index < store_count;
         ++store_index)
    {
        const auto store = call_dbgen_mk<W_STORE_TBL, &mk_w_store, STORE>(
            store_first + store_index);
        ...
        store_builder.append_row(
            ...
            resolve_string(
                ...
                pmr_string{std::to_string(store.address.street_num)}),
            resolve_street_name(W_STORE_ADDRESS_STREET_NAME1, store.address),
            resolve_string(W_STORE_ADDRESS_STREET_TYPE,
                           store.address.street_type),
            ...
            resolve_string(W_STORE_ADDRESS_ZIP,
                           zip_to_string(store.address.zip)),
            resolve_string(W_STORE_ADDRESS_COUNTRY, store.address.country),
            resolve_gmt_offset(W_STORE_ADDRESS_GMT_OFFSET,
                               store.address.gmt_offset),
...
std::pair<std::shared_ptr<Table>, std::shared_ptr<Table>>
TPCDSTableGenerator::generate_store_sales_and_returns(ds_key_t max_rows) const
{
    auto [store_sales_first, store_sales_count] =
        prepare_for_table(STORE_SALES);

    auto store_sales_builder = TableBuilder{
        _benchmark_config->chunk_size, store_sales_column_types,
        store_sales_column_names, static_cast<ChunkOffset>(store_sales_count)};
    auto store_returns_builder =
        TableBuilder{_benchmark_config->chunk_size, store_returns_column_types,
                     store_returns_column_names};

    for (auto store_sale = ds_key_t{0}; store_sale < store_sales_count;
         ++store_sale)
    {
        auto store_sales = W_STORE_SALES_TBL{};
        auto store_returns = W_STORE_RETURNS_TBL{};

        // modified call to mk_w_store_sales(&store_sales, store_sales_first +
        // store_sale, &store_returns, &was_returned)
        {
            mk_w_store_sales_master(&store_sales,
                                    store_sales_first + store_sale, 0);

            auto n_lineitems = int{0};
            genrand_integer(&n_lineitems, DIST_UNIFORM, 8, 16, 0,
                            SS_TICKET_NUMBER);
            for (auto lineitem_index = int{0}; lineitem_index < n_lineitems;
                 ++lineitem_index)
            {
                auto was_returned = int{0};
                mk_w_store_sales_detail(&store_sales, 0, &store_returns,
                                        &was_returned);

                if (store_sales_builder.row_count() <
                    static_cast<size_t>(max_rows))
                {
                    store_sales_builder.append_row(
                        resolve_key(SS_SOLD_DATE_SK,
                                    store_sales.ss_sold_date_sk),
                        resolve_key(SS_SOLD_TIME_SK,
                                    store_sales.ss_sold_time_sk),
                        store_sales.ss_sold_item_sk,
                        resolve_key(SS_SOLD_CUSTOMER_SK,
                                    store_sales.ss_sold_customer_sk),
                        resolve_key(SS_SOLD_CDEMO_SK,
                                    store_sales.ss_sold_cdemo_sk),
                        resolve_key(SS_SOLD_HDEMO_SK,
                                    store_sales.ss_sold_hdemo_sk),
                        resolve_key(SS_SOLD_ADDR_SK,
                                    store_sales.ss_sold_addr_sk),
                        resolve_key(SS_SOLD_STORE_SK,
                                    store_sales.ss_sold_store_sk),
                        resolve_key(SS_SOLD_PROMO_SK,
                                    store_sales.ss_sold_promo_sk),
                        store_sales.ss_ticket_number,
                        resolve_integer(SS_PRICING_QUANTITY,
                                        store_sales.ss_pricing.quantity),
                        resolve_decimal(SS_PRICING_WHOLESALE_COST,
                                        store_sales.ss_pricing.wholesale_cost),
                        resolve_decimal(SS_PRICING_LIST_PRICE,
                                        store_sales.ss_pricing.list_price),
                        resolve_decimal(SS_PRICING_SALES_PRICE,
                                        store_sales.ss_pricing.sales_price),
                        resolve_decimal(SS_PRICING_COUPON_AMT,
                                        store_sales.ss_pricing.coupon_amt),
                        resolve_decimal(SS_PRICING_EXT_SALES_PRICE,
                                        store_sales.ss_pricing.ext_sales_price),
                        resolve_decimal(
                            SS_PRICING_EXT_WHOLESALE_COST,
                            store_sales.ss_pricing.ext_wholesale_cost),
                        resolve_decimal(SS_PRICING_EXT_LIST_PRICE,
                                        store_sales.ss_pricing.ext_list_price),
                        resolve_decimal(SS_PRICING_EXT_TAX,
                                        store_sales.ss_pricing.ext_tax),
                        resolve_decimal(SS_PRICING_COUPON_AMT,
                                        store_sales.ss_pricing.coupon_amt),
                        resolve_decimal(SS_PRICING_NET_PAID,
                                        store_sales.ss_pricing.net_paid),
                        resolve_decimal(
                            SS_PRICING_NET_PAID_INC_TAX,
                            store_sales.ss_pricing.net_paid_inc_tax),
                        resolve_decimal(SS_PRICING_NET_PROFIT,
                                        store_sales.ss_pricing.net_profit));
                ...
                if (was_returned != 0)
                {
                    store_returns_builder.append_row(
                        resolve_key(SR_RETURNED_DATE_SK,
                                    store_returns.sr_returned_date_sk),
                        resolve_key(SR_RETURNED_TIME_SK,
                                    store_returns.sr_returned_time_sk),
                        store_returns.sr_item_sk,
                        resolve_key(SR_CUSTOMER_SK,
                        ...
                        resolve_integer(SR_PRICING_QUANTITY,
                                        store_returns.sr_pricing.quantity),
                        resolve_decimal(SR_PRICING_NET_PAID,
                                        store_returns.sr_pricing.net_paid),
                        resolve_decimal(SR_PRICING_EXT_TAX,
                                        store_returns.sr_pricing.ext_tax),
                        resolve_decimal(
                            SR_PRICING_NET_PAID_INC_TAX,
                            store_returns.sr_pricing.net_paid_inc_tax),
                        resolve_decimal(SR_PRICING_FEE,
                                        store_returns.sr_pricing.fee),
                        resolve_decimal(SR_PRICING_EXT_SHIP_COST,
                                        store_returns.sr_pricing.ext_ship_cost),
                        resolve_decimal(SR_PRICING_REFUNDED_CASH,
                                        store_returns.sr_pricing.refunded_cash),
                        resolve_decimal(
                            SR_PRICING_REVERSED_CHARGE,
                            store_returns.sr_pricing.reversed_charge),
                        resolve_decimal(SR_PRICING_STORE_CREDIT,
                                        store_returns.sr_pricing.store_credit),
                        resolve_decimal(SR_PRICING_NET_LOSS,
                                        store_returns.sr_pricing.net_loss));
                    if (store_returns_builder.row_count() ==
                        static_cast<size_t>(max_rows))
                    {
                        break;
                    }
    ...
    return {store_sales_builder.finish_table(),
            store_returns_builder.finish_table()};
}

std::shared_ptr<Table>
TPCDSTableGenerator::generate_time_dim(ds_key_t max_rows) const
{
    auto [time_first, time_count] = prepare_for_table(TIME);
    time_count = std::min(time_count, max_rows);

    auto time_builder =
        TableBuilder{_benchmark_config->chunk_size, time_column_types,
                     time_column_names, static_cast<ChunkOffset>(time_count)};

    for (auto time_index = ds_key_t{0}; time_index < time_count; ++time_index)
    {
        const auto time = call_dbgen_mk<W_TIME_TBL, &mk_w_time, TIME>(
            time_first + time_index);

        time_builder.append_row(time.t_time_sk, time.t_time_id,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_warehouse(ds_key_t max_rows) const
{
    auto [warehouse_first, warehouse_count] = prepare_for_table(WAREHOUSE);
    warehouse_count = std::min(warehouse_count, max_rows);

    auto warehouse_builder = TableBuilder{
        _benchmark_config->chunk_size, warehouse_column_types,
        warehouse_column_names, static_cast<ChunkOffset>(warehouse_count)};

    for (auto warehouse_index = ds_key_t{0}; warehouse_index < warehouse_count;
         ++warehouse_index)
    {
        const auto warehouse =
            call_dbgen_mk<W_WAREHOUSE_TBL, &mk_w_warehouse, WAREHOUSE>(
                warehouse_first + warehouse_index);
        ...
        warehouse_builder.append_row(
            ...
            resolve_string(W_ADDRESS_ZIP,
                           zip_to_string(warehouse.w_address.zip)),
            resolve_string(W_ADDRESS_COUNTRY, warehouse.w_address.country),
            resolve_gmt_offset(W_ADDRESS_GMT_OFFSET,
...
std::shared_ptr<Table>
TPCDSTableGenerator::generate_web_page(ds_key_t max_rows) const
{
    auto [web_page_first, web_page_count] = prepare_for_table(WEB_PAGE);
    web_page_count = std::min(web_page_count, max_rows);

    auto web_page_builder = TableBuilder{
        _benchmark_config->chunk_size, web_page_column_types,
        web_page_column_names, static_cast<ChunkOffset>(web_page_count)};

    for (auto web_page_index = ds_key_t{0}; web_page_index < web_page_count;
         ++web_page_index)
    {
        const auto web_page =
            call_dbgen_mk<W_WEB_PAGE_TBL, &mk_w_web_page, WEB_PAGE>(
                web_page_first + web_page_index);

        web_page_builder.append_row(
            web_page.wp_page_sk, web_page.wp_page_id,
            resolve_date_id(WP_REC_START_DATE_ID,
                            web_page.wp_rec_start_date_id),
            ...
            resolve_string(WP_AUTOGEN_FLAG,
                           boolean_to_string(web_page.wp_autogen_flag)),
...
std::pair<std::shared_ptr<Table>, std::shared_ptr<Table>>
TPCDSTableGenerator::generate_web_sales_and_returns(ds_key_t max_rows) const
{
    auto [web_sales_first, web_sales_count] = prepare_for_table(WEB_SALES);

    auto web_sales_builder = TableBuilder{
        _benchmark_config->chunk_size, web_sales_column_types,
        web_sales_column_names, static_cast<ChunkOffset>(web_sales_count)};
    auto web_returns_builder =
        TableBuilder{_benchmark_config->chunk_size, web_returns_column_types,
                     web_returns_column_names};

    for (auto web_sales_index = ds_key_t{0}; web_sales_index < web_sales_count;
         ++web_sales_index)
    {
        auto web_sales = W_WEB_SALES_TBL{};
        auto web_returns = W_WEB_RETURNS_TBL{};

        // modified call to mk_w_web_sales(&web_sales, web_sales_first +
        // web_sales_index, &web_returns, &was_returned);
        {
            mk_w_web_sales_master(&web_sales, web_sales_first + web_sales_index,
                                  0);

            auto n_lineitems = int{0};
            genrand_integer(&n_lineitems, DIST_UNIFORM, 8, 16, 9,
                            WS_ORDER_NUMBER);
            for (auto lineitem_index = int{0}; lineitem_index < n_lineitems;
                 ++lineitem_index)
            {
                auto was_returned = 0;
                mk_w_web_sales_detail(&web_sales, 0, &web_returns,
                                      &was_returned, 0);

                if (web_sales_builder.row_count() <
                    static_cast<size_t>(max_rows))
                {
                    web_sales_builder.append_row(
                        ...
                        resolve_key(WS_BILL_CUSTOMER_SK,
                                    web_sales.ws_bill_customer_sk),
                        resolve_key(WS_BILL_CDEMO_SK,
                                    web_sales.ws_bill_cdemo_sk),
                        resolve_key(WS_BILL_HDEMO_SK,
                                    web_sales.ws_bill_hdemo_sk),
                        resolve_key(WS_BILL_ADDR_SK, web_sales.ws_bill_addr_sk),
                        resolve_key(WS_SHIP_CUSTOMER_SK,
                                    web_sales.ws_ship_customer_sk),
                        resolve_key(WS_SHIP_CDEMO_SK,
                                    web_sales.ws_ship_cdemo_sk),
                        resolve_key(WS_SHIP_HDEMO_SK,
                        ...
                        resolve_integer(WS_PRICING_QUANTITY,
                                        web_sales.ws_pricing.quantity),
                        resolve_decimal(WS_PRICING_WHOLESALE_COST,
                                        web_sales.ws_pricing.wholesale_cost),
                        resolve_decimal(WS_PRICING_LIST_PRICE,
                                        web_sales.ws_pricing.list_price),
                        resolve_decimal(WS_PRICING_SALES_PRICE,
                                        web_sales.ws_pricing.sales_price),
                        resolve_decimal(WS_PRICING_EXT_DISCOUNT_AMT,
                                        web_sales.ws_pricing.ext_discount_amt),
                        resolve_decimal(WS_PRICING_EXT_SALES_PRICE,
                                        web_sales.ws_pricing.ext_sales_price),
                        resolve_decimal(
                            WS_PRICING_EXT_WHOLESALE_COST,
                            web_sales.ws_pricing.ext_wholesale_cost),
                        resolve_decimal(WS_PRICING_EXT_LIST_PRICE,
                                        web_sales.ws_pricing.ext_list_price),
                        resolve_decimal(WS_PRICING_EXT_TAX,
                                        web_sales.ws_pricing.ext_tax),
                        resolve_decimal(WS_PRICING_COUPON_AMT,
                                        web_sales.ws_pricing.coupon_amt),
                        resolve_decimal(WS_PRICING_EXT_SHIP_COST,
                                        web_sales.ws_pricing.ext_ship_cost),
                        resolve_decimal(WS_PRICING_NET_PAID,
                                        web_sales.ws_pricing.net_paid),
                        resolve_decimal(WS_PRICING_NET_PAID_INC_TAX,
                                        web_sales.ws_pricing.net_paid_inc_tax),
                        resolve_decimal(WS_PRICING_NET_PAID_INC_SHIP,
                                        web_sales.ws_pricing.net_paid_inc_ship),
                        resolve_decimal(
                            WS_PRICING_NET_PAID_INC_SHIP_TAX,
                            web_sales.ws_pricing.net_paid_inc_ship_tax),
                        resolve_decimal(WS_PRICING_NET_PROFIT,
                                        web_sales.ws_pricing.net_profit));
                }

                if (was_returned != 0)
                {
                    web_returns_builder.append_row(
                        resolve_key(WR_RETURNED_DATE_SK,
                                    web_returns.wr_returned_date_sk),
                        resolve_key(WR_RETURNED_TIME_SK,
                                    web_returns.wr_returned_time_sk),
                        web_returns.wr_item_sk,
                        resolve_key(WR_REFUNDED_CUSTOMER_SK,
                                    web_returns.wr_refunded_customer_sk),
                        resolve_key(WR_REFUNDED_CDEMO_SK,
                                    web_returns.wr_refunded_cdemo_sk),
                        resolve_key(WR_REFUNDED_HDEMO_SK,
                                    web_returns.wr_refunded_hdemo_sk),
                        resolve_key(WR_REFUNDED_ADDR_SK,
                                    web_returns.wr_refunded_addr_sk),
                        resolve_key(WR_RETURNING_CUSTOMER_SK,
                                    web_returns.wr_returning_customer_sk),
                        resolve_key(WR_RETURNING_CDEMO_SK,
                                    web_returns.wr_returning_cdemo_sk),
                        resolve_key(WR_RETURNING_HDEMO_SK,
                                    web_returns.wr_returning_hdemo_sk),
                        resolve_key(WR_RETURNING_ADDR_SK,
                        ...
                        resolve_integer(WR_PRICING_QUANTITY,
                                        web_returns.wr_pricing.quantity),
                        resolve_decimal(WR_PRICING_NET_PAID,
                                        web_returns.wr_pricing.net_paid),
                        resolve_decimal(WR_PRICING_EXT_TAX,
                                        web_returns.wr_pricing.ext_tax),
                        resolve_decimal(
                            WR_PRICING_NET_PAID_INC_TAX,
                            web_returns.wr_pricing.net_paid_inc_tax),
                        resolve_decimal(WR_PRICING_FEE,
                                        web_returns.wr_pricing.fee),
                        resolve_decimal(WR_PRICING_EXT_SHIP_COST,
                                        web_returns.wr_pricing.ext_ship_cost),
                        resolve_decimal(WR_PRICING_REFUNDED_CASH,
                                        web_returns.wr_pricing.refunded_cash),
                        resolve_decimal(WR_PRICING_REVERSED_CHARGE,
                                        web_returns.wr_pricing.reversed_charge),
                        resolve_decimal(WR_PRICING_STORE_CREDIT,
                                        web_returns.wr_pricing.store_credit),
                        resolve_decimal(WR_PRICING_NET_LOSS,
                                        web_returns.wr_pricing.net_loss));
                    if (web_returns_builder.row_count() ==
                        static_cast<size_t>(max_rows))
                    {
                        break;
                    }
    ...
    return {web_sales_builder.finish_table(),
            web_returns_builder.finish_table()};
}

std::shared_ptr<Table>
TPCDSTableGenerator::generate_web_site(ds_key_t max_rows) const
{
    auto [web_site_first, web_site_count] = prepare_for_table(WEB_SITE);
    web_site_count = std::min(web_site_count, max_rows);

    auto web_site_builder = TableBuilder{
        _benchmark_config->chunk_size, web_site_column_types,
        web_site_column_names, static_cast<ChunkOffset>(web_site_count)};

    auto web_site = W_WEB_SITE_TBL{};
    static_assert(sizeof(web_site.web_class) == 51);
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg,hicpp-vararg)
    const auto snprintf_rc = std::snprintf(
        web_site.web_class, sizeof(web_site.web_class), "%s", "Unknown");
    Assert(snprintf_rc > 0, "Unexpected string to parse.");
    for (auto web_site_index = ds_key_t{0}; web_site_index < web_site_count;
         ++web_site_index)
    {
        // mk_w_web_site needs a pointer to the previous result because it
        // expects values set previously to still be there
        ...
        web_site_builder.append_row(
            ...
            resolve_date_id(WEB_REC_START_DATE_ID,
                            web_site.web_rec_start_date_id),
            resolve_date_id(WEB_REC_END_DATE_ID, web_site.web_rec_end_date_id),
            resolve_string(WEB_NAME, web_site.web_name),
            resolve_key(WEB_OPEN_DATE, web_site.web_open_date),
            resolve_key(WEB_CLOSE_DATE, web_site.web_close_date),
            resolve_string(WEB_CLASS, web_site.web_class),
            resolve_string(WEB_MANAGER, web_site.web_manager),
            ...
            resolve_string(
                WEB_ADDRESS_STREET_NUM,
                pmr_string{std::to_string(web_site.web_address.street_num)}),
            resolve_street_name(WEB_ADDRESS_STREET_NAME1, web_site.web_address),
            resolve_string(WEB_ADDRESS_STREET_TYPE,
                           web_site.web_address.street_type),
            resolve_string(WEB_ADDRESS_SUITE_NUM,
                           web_site.web_address.suite_num),
            ...
            resolve_string(WEB_ADDRESS_ZIP,
                           zip_to_string(web_site.web_address.zip)),
            resolve_string(WEB_ADDRESS_COUNTRY, web_site.web_address.country),
            resolve_gmt_offset(WEB_ADDRESS_GMT_OFFSET,
                               web_site.web_address.gmt_offset),
...
AbstractTableGenerator::IndexesByTable
TPCDSTableGenerator::_indexes_by_table() const
{
    return {
...
{
    /**
     * Adds all PRIMARY KEY key constraints as described in the official TPC-DS
     * specification. (Section 2: Logical Database Design)
     */

    // Fact Tables (7)
    const auto &store_sales_table = table_info_by_name.at("store_sales").table;
    store_sales_table->add_soft_key_constraint(
        {{store_sales_table->column_id_by_name("ss_item_sk"),
          store_sales_table->column_id_by_name("ss_ticket_number")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &store_returns_table =
        table_info_by_name.at("store_returns").table;
    store_returns_table->add_soft_key_constraint(
        {{store_returns_table->column_id_by_name("sr_item_sk"),
          store_returns_table->column_id_by_name("sr_ticket_number")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &catalog_sales_table =
        table_info_by_name.at("catalog_sales").table;
    catalog_sales_table->add_soft_key_constraint(
        {{catalog_sales_table->column_id_by_name("cs_item_sk"),
          catalog_sales_table->column_id_by_name("cs_order_number")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &catalog_returns_table =
        table_info_by_name.at("catalog_returns").table;
    catalog_returns_table->add_soft_key_constraint(
    ...
    web_sales_table->add_soft_key_constraint(
        {{web_sales_table->column_id_by_name("ws_item_sk"),
          web_sales_table->column_id_by_name("ws_order_number")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &web_returns_table = table_info_by_name.at("web_returns").table;
    web_returns_table->add_soft_key_constraint(
        {{web_returns_table->column_id_by_name("wr_item_sk"),
          web_returns_table->column_id_by_name("wr_order_number")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &inventory_table = table_info_by_name.at("inventory").table;
    inventory_table->add_soft_key_constraint(
        {{inventory_table->column_id_by_name("inv_date_sk"),
          inventory_table->column_id_by_name("inv_item_sk"),
    ...
    const auto &catalog_page_table =
        table_info_by_name.at("catalog_page").table;
    ...
    const auto &customer_address_table =
        table_info_by_name.at("customer_address").table;
    customer_address_table->add_soft_key_constraint(
        {{customer_address_table->column_id_by_name("ca_address_sk")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &customer_demographics_table =
        table_info_by_name.at("customer_demographics").table;
    ...
    const auto &household_demographics_table =
        table_info_by_name.at("household_demographics").table;
    ...
    item_table->add_soft_key_constraint(
