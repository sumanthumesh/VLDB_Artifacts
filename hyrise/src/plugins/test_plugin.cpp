...
void TestPlugin::start()
{
    const auto column_definitions =
        TableColumnDefinitions{{"col_1", DataType::Int, false}};
    const auto table = std::make_shared<Table>(
        column_definitions, TableType::Data, std::nullopt, UseMvcc::Yes);
...
TestPlugin::provided_user_executable_functions()
{
...
{
    const auto column_definitions =
        TableColumnDefinitions{{"col_A", DataType::Int, false}};
    const auto table = std::make_shared<Table>(
        column_definitions, TableType::Data, std::nullopt, UseMvcc::Yes);

    storage_manager.add_table(
        "TableOfTestPlugin_" + std::to_string(_added_tables_count), table);
    ++_added_tables_count;
}
...
std::optional<PreBenchmarkHook> TestPlugin::pre_benchmark_hook()
{
    ...
    {
        const auto column_definitions =
            TableColumnDefinitions{{"item_id", DataType::Int, false}};
        const auto table = std::make_shared<Table>(
            column_definitions, TableType::Data, std::nullopt, UseMvcc::Yes);
