...
void MvccDeletePlugin::start()
{
    _loop_thread_logical_delete = std::make_unique<PausableLoopThread>(
        IDLE_DELAY_LOGICAL_DELETE,
        [&](size_t /*unused*/) { _logical_delete_loop(); });

    _loop_thread_physical_delete = std::make_unique<PausableLoopThread>(
        IDLE_DELAY_PHYSICAL_DELETE,
        [&](size_t /*unused*/) { _physical_delete_loop(); });
}
...
/**
 * This function analyzes each chunk of every table and triggers a
 * chunk-cleanup-procedure if a certain threshold of invalidated rows is
 * exceeded.
 */
void MvccDeletePlugin::_logical_delete_loop()
{
    ...
    for (const auto &[table_name, table] : tables)
    {
        ...
        // Check all chunks, except for the last one, which is currently used
        // for insertions
        const auto max_chunk_id =
            static_cast<ChunkID>(table->chunk_count() - 1);
        for (auto chunk_id = ChunkID{0}; chunk_id < max_chunk_id; ++chunk_id)
        {
            ...
            if (chunk && !chunk->get_cleanup_commit_id())
            {
                const auto chunk_memory =
                    chunk->memory_usage(MemoryUsageCalculationMode::Sampled);

                // Calculate metric 1 – Chunk invalidation level
                const auto invalidated_rows_ratio =
                    static_cast<double>(chunk->invalid_row_count()) /
                    chunk->size();
                const auto criterion1 =
                    (DELETE_THRESHOLD_PERCENTAGE_INVALIDATED_ROWS <=
                     invalidated_rows_ratio);
                     ...
                     chunk_offset < chunk_size; ++chunk_offset)
                {
                    const auto commit_id =
                        chunk->mvcc_data()->get_end_cid(chunk_offset);
                    if (commit_id != MvccData::MAX_COMMIT_ID &&
                        commit_id > highest_end_commit_id)
                    {
                ...
                const auto criterion2 =
                    highest_end_commit_id + DELETE_THRESHOLD_LAST_COMMIT <=
                ...
                auto transaction_context =
                    Hyrise::get().transaction_manager.new_transaction_context(
                        AutoCommit::No);
                const auto success = _try_logical_delete(table_name, chunk_id,
                                                         transaction_context);

                if (success)
                {
                    DebugAssert(
                        table->get_chunk(chunk_id)->get_cleanup_commit_id(),
                        "Chunk needs to be deleted logically before deleting "
                        "it physically.");

                    const auto lock = std::lock_guard<std::mutex>{
                        _physical_delete_queue_mutex};
        ...
        if (saved_memory > 0)
        {
            ...
            const auto saved_mb =
                static_cast<float>(saved_memory) / (1000.0f * 1000.0f);
            message << "Consolidated " << num_chunks << " chunk(s) of "
                    << table_name << ", saved approx. " << std::setprecision(2)
                    << saved_mb << " MB";
            Hyrise::get().log_manager.add_message(
                "MvccDeletePlugin", message.str(), LogLevel::Info);
        }
    }
}
...
void MvccDeletePlugin::_physical_delete_loop()
{
    ...
    if (!_physical_delete_queue.empty())
    {
        ...
        DebugAssert(
            chunk, "Chunk does not exist. Physical Delete can not be applied.");

        if (chunk->get_cleanup_commit_id())
        {
            // Check whether there are still active transactions that might use
            // the chunk.
            auto transactions_conflict = false;
            const auto lowest_snapshot_commit_id =
                Hyrise::get()
                    .transaction_manager.get_lowest_active_snapshot_commit_id();

            if (lowest_snapshot_commit_id)
            {
                transactions_conflict = *chunk->get_cleanup_commit_id() >
                                        *lowest_snapshot_commit_id;
            }
...
bool MvccDeletePlugin::_try_logical_delete(
    ...
    const std::shared_ptr<TransactionContext> &transaction_context)
{
    ...
    Assert(chunk_id < (table->chunk_count() - 1),
           "MVCC Logical Delete should not be applied on the last/current "
           "mutable chunk.");

    // Create temporary referencing table that contains the given chunk only
    //   Include all ChunksIDs of current table except chunk_id for pruning in
    //   GetTable
    auto excluded_chunk_ids = std::vector<ChunkID>(table->chunk_count() - 1);
    std::iota(excluded_chunk_ids.begin(), excluded_chunk_ids.begin() + chunk_id,
              0);
    std::iota(excluded_chunk_ids.begin() + chunk_id, excluded_chunk_ids.end(),
              chunk_id + 1);

    const auto get_table = std::make_shared<GetTable>(
        table_name, excluded_chunk_ids, std::vector<ColumnID>());
    ...
    const auto update =
        std::make_shared<Update>(table_name, validate, validate);
    ...
    {
        // Transaction conflict. Usually, the OperatorTask would call rollback,
        // but as we executed Update directly, that is our job.
...
void MvccDeletePlugin::_delete_chunk_physically(
    const std::shared_ptr<Table> &table, const ChunkID chunk_id)
{
    const auto &chunk = table->get_chunk(chunk_id);

    Assert(chunk->get_cleanup_commit_id(),
           "The cleanup commit id of the chunk is not set. This should have "
           "been done by the logical delete.");
