...
void generate_benchmark_data(std::string argument_string)
{
    ...
    // Remove dashes and convert to lower case to unify different writings of
    // benchmarks ("TPC-H", "tpch", or "tpc-h")
    ...
    boost::split(benchmark_data_config, argument_string, boost::is_any_of(":"),
                 boost::token_compress_on);
    Assert(benchmark_data_config.size() == 2,
           "Malformed input for benchmark data generation. Expecting a "
           "benchmark name and a sizing factor.");

    const auto benchmark_name = benchmark_data_config[0];
    const auto sizing_factor =
        boost::lexical_cast<float, std::string>(benchmark_data_config[1]);

    Assert(benchmark_name == "tpch" || benchmark_name == "tpcds" ||
               benchmark_name == "tpcc",
           "Benchmark data generation is only supported for TPC-C, TPC-DS, and "
           "TPC-H.");

    auto config = std::make_shared<hyrise::BenchmarkConfig>(
        hyrise::BenchmarkConfig::get_default_config());
    config->cache_binary_tables = true;
    if (benchmark_name == "tpcc")
    {
        hyrise::TPCCTableGenerator{static_cast<uint32_t>(sizing_factor), config}
            .generate_and_store();
    }
    else if (benchmark_name == "tpcds")
    {
        hyrise::TPCDSTableGenerator{static_cast<uint32_t>(sizing_factor),
                                    config}
            .generate_and_store();
    }
    else if (benchmark_name == "tpch")
    {
        hyrise::TPCHTableGenerator{sizing_factor, ClusteringConfiguration::None,
                                   config}
            .generate_and_store();
    }
    else
...
cxxopts::Options get_server_cli_options()
{
    cxxopts::Options cli_options(
        "./hyriseServer",
        "Starts Hyrise server in order to accept network requests.");
...
int main(int argc, char *argv[])
{
    ...
    /**
     * The optional parameter `benchmark_data` allows users to generate
     * benchmark data when starting the hyrise server. This is not an ideal
     * solution, but due to several users' requests and our goal to facilitate
     * easy evaluation of Hyrise, we decided to integrate the data generation
     * into the server nonetheless.
     *
     * We do not plan on exposing other parameters, such as the encoding or the
     * chunk size via this facility. You can change the modify the config object
     * as needed.
     */
    if (parsed_options.count("benchmark_data"))
    {
        generate_benchmark_data(
            parsed_options["benchmark_data"].as<std::string>());
    }
    ...
    const auto address = boost::asio::ip::make_address(
        parsed_options["address"].as<std::string>(), error);

    Assert(!error, "Not a valid IPv4 address: " +
                       parsed_options["address"].as<std::string>() +
                       ", terminating...");

    auto server = hyrise::Server{
        address, port, static_cast<hyrise::SendExecutionInfo>(execution_info)};
    server.run();
