...
enum access_counter_display_mode
{
  all,
  table,
  segment,
...
class Console : public Singleton<Console>
{
  public:
    ...
    /*
     * Outputs a table, either directly to the _out stream if it is small
     * enough, or using pagination.
     *
     * @param output The output table.
     * @param flags  Flags for the Print operator.
     */
    void out(const std::shared_ptr<const Table> &table,
  ...
  protected:
    ...
    int _wait(const std::string &time);
    int _cores(const std::string &time);
    int _pintool(const std::string &time);
    int _vtune(const std::string &time);
    int _iotop(const std::string &time);
    int _iostat(const std::string &time);
    int _coreutil(const std::string &time);
    int _python(const std::string &args);
    int _mem_blks();
    int _coalesce();
    int _pid();
    int _details();
    int _seg();
    int _acctr(const std::string &time);
    int _create_chunk_index(const std::string &args);
    int _list_index();
    int _reset();
    void _rollback();

    // Creates the pipelines and returns whether is was successful (true) or not
    // (false)
