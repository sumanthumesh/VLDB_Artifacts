...
/**
 * The Join Order Benchmark was introduced by Leis et al. "How good are query
 * optimizers, really?". It runs on an IMDB database from ~2013 that gets
 * downloaded if necessary as part of running this benchmark. Its 113 queries
 * are obtained from the "third_party/join-order-benchmark" submodule
 */
...
/**
 * Each of the 21 JOB tables has one surrogate key. This function registers key
    ...
    std::unordered_map<std::string, BenchmarkTableInfo> &table_info_by_name)
{
    ...
    aka_name_table->add_soft_key_constraint(
    ...
    const auto &comp_cast_type_table =
        table_info_by_name.at("comp_cast_type").table;
    comp_cast_type_table->add_soft_key_constraint(
        {{comp_cast_type_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &company_name_table =
        table_info_by_name.at("company_name").table;
    company_name_table->add_soft_key_constraint(
        {{company_name_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &company_type_table =
        table_info_by_name.at("company_type").table;
    company_type_table->add_soft_key_constraint(
        {{company_type_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &complete_cast_table =
        table_info_by_name.at("complete_cast").table;
    complete_cast_table->add_soft_key_constraint(
        {{complete_cast_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});
    ...
    keyword_table->add_soft_key_constraint(
    ...
    const auto &movie_companies_table =
        table_info_by_name.at("movie_companies").table;
    movie_companies_table->add_soft_key_constraint(
        {{movie_companies_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &movie_info_table = table_info_by_name.at("movie_info").table;
    movie_info_table->add_soft_key_constraint(
        {{movie_info_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &movie_info_idx_table =
        table_info_by_name.at("movie_info_idx").table;
    movie_info_idx_table->add_soft_key_constraint(
        {{movie_info_idx_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &movie_keyword_table =
        table_info_by_name.at("movie_keyword").table;
    movie_keyword_table->add_soft_key_constraint(
        {{movie_keyword_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &movie_link_table = table_info_by_name.at("movie_link").table;
    movie_link_table->add_soft_key_constraint(
        {{movie_link_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});

    const auto &name_table = table_info_by_name.at("name").table;
    name_table->add_soft_key_constraint({{name_table->column_id_by_name("id")},
                                         KeyConstraintType::PRIMARY_KEY});

    const auto &person_info_table = table_info_by_name.at("person_info").table;
    person_info_table->add_soft_key_constraint(
        {{person_info_table->column_id_by_name("id")},
         KeyConstraintType::PRIMARY_KEY});
    ...
    title_table->add_soft_key_constraint(
...
{
    auto cli_options =
        BenchmarkRunner::get_basic_cli_options("Hyrise Join Order Benchmark");
  ...
  ("q,queries", "Subset of queries to run as a comma separated list", cxxopts::value<std::string>()->default_value("all")); // NOLINT
    ...
    {
        std::cerr << "Need to specify --query_path=path/to/queries and "
                     "--table_path=path/to/table_files"
                  << std::endl;
    ...
    /**
     * Use a Python script to download and unzip the IMDB. We do this in Python
     * and not in C++ because downloading and unzipping is straight forward in
     * Python (and we suspect in C++ it might be... cumbersome).
     */
    const auto setup_imdb_command =
        "python3 scripts/setup_imdb.py "s + table_path;
    const auto setup_imdb_return_code = system(setup_imdb_command.c_str());
    Assert(setup_imdb_return_code == 0, "setup_imdb.py failed. Did you run the "
                                        "benchmark from the project root dir?");

    // The join-order-benchmark ships with these two .sql scripts, but we do not
    // want to run them as part of the benchmark as they do not contains actual
    // queries
    const auto non_query_file_names =
        std::unordered_set<std::string>{"fkindexes.sql", "schema.sql"};
    ...
    else
    {
        ...
        boost::algorithm::split(query_subset_untrimmed, queries_str,
                                boost::is_any_of(","));
    ...
    auto table_generator =
        std::make_unique<FileBasedTableGenerator>(benchmark_config, table_path);
    ...
    auto benchmark_runner = std::make_shared<BenchmarkRunner>(
        *benchmark_config, std::move(benchmark_item_runner),
        std::move(table_generator), context);
    Hyrise::get().benchmark_runner = benchmark_runner;

    if (benchmark_config->verify)
    {
        add_indices_to_sqlite(query_path + "/schema.sql",
                              query_path + "/fkindexes.sql",
