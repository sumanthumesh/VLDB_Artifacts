 ...
 * approaches in a minimal setup. Of course you can also use it to just
 * benchmark one single thing.
 *
 ...
 *   to memory. It acts as a memory barrier. In combination with
 * DoNotOptimize(e), this function effectively declares that it could touch any
 * part of memory, in particular globally aliased memory.
...
class BenchmarkPlaygroundFixture : public MicroBenchmarkBasicFixture
{
  public:
    void SetUp(::benchmark::State &state) override
    {
        ...
        // ... The "TableScan" will scan for one value (2), so it will select
        // 25%.
 ...
 */
BENCHMARK_F(BenchmarkPlaygroundFixture, BM_Playground_Reference)
(benchmark::State &state)
{
    ...
    for (auto _ : state)
    {
        ...
        benchmark::DoNotOptimize(
            result.data()); // Do not optimize out the vector
        ...
        for (auto index = size_t{0}; index < size; ++index)
        {
            if (_vec[index] == 2)
            {
                ...
                benchmark::ClobberMemory(); // Force that record to be written
                                            // to memory
            }
        }
    }
 ...
 */
BENCHMARK_F(BenchmarkPlaygroundFixture, BM_Playground_PreAllocate)
(benchmark::State &state)
{
    // Add some benchmark-specific setup here
