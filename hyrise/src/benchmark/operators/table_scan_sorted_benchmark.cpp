...
create_table(const DataType data_type, const int table_size,
             ...
             const EncodingType encoding_type, const std::string &mode)
{
    ...
    for (auto chunk_index = 0u; chunk_index < table_size / CHUNK_SIZE;
         ++chunk_index)
    {
        const auto first = chunk_values.cbegin() + CHUNK_SIZE * chunk_index;
        const auto last =
            chunk_values.cbegin() + CHUNK_SIZE * (chunk_index + 1);
        ...
        auto null_elements = static_cast<int>(
            std::round(static_cast<double>(value_vector.size()) * 0.1));
        std::fill(null_values.begin(), null_values.begin() + null_elements,
                  true);
        if (mode == "Shuffled")
        {
        ...
        const auto value_segment = std::make_shared<ValueSegment<Type>>(
            std::move(value_vector), std::move(null_values));
    ...
    if (encoding_type != EncodingType::Unencoded)
    {
        ...
        ChunkEncoder::encode_all_chunks(table,
                                        SegmentEncodingSpec(encoding_type));
    }
...
void BM_TableScanSorted(
    ...
    const EncodingType encoding_type, const std::string &mode,
    const bool is_between_scan, const bool is_reference_scan,
    const std::function<std::shared_ptr<TableWrapper>(const EncodingType,
                                                      const std::string)>
        table_creator)
{
    micro_benchmark_clear_cache();

    // The benchmarks all run with different selectivities (ratio of values in
    // the output to values in the input). At this point the search value is
    // selected in a way that our results correspond to the chosen selectivity.

    const auto table_wrapper = table_creator(encoding_type, mode);
    table_wrapper->never_clear_output();

    const auto table_column_definitions =
        table_wrapper->get_output()->column_definitions();
    ...
    const auto column_expression =
        pqp_column_(column_index, column_definition.data_type,
    ...
    resolve_data_type(
        ...
        [&](const auto data_type_t)
        {
            ...
            if (is_between_scan)
            {
                ...
                const auto left_raw_value =
                    static_cast<int>(table_size * (0.5 - selectivity / 2.0));
                const auto right_raw_value =
                    static_cast<int>(table_size * (0.5 + selectivity / 2.0));
                if constexpr (std::is_same_v<ColumnDataType, pmr_string>)
                {
                    left_search_value =
                        pad_string(std::to_string(left_raw_value), STRING_SIZE);
                    right_search_value = pad_string(
                        std::to_string(right_raw_value), STRING_SIZE);
                }
                else
                {
                    left_search_value =
                        static_cast<ColumnDataType>(left_raw_value);
                    right_search_value =
                        static_cast<ColumnDataType>(right_raw_value);
                }

                predicate = std::make_shared<BetweenExpression>(
                    PredicateCondition::BetweenUpperExclusive,
                    column_expression, value_(left_search_value),
                    value_(right_search_value));
            }
            else
            {
                ...
                {
                    search_value =
                        pad_string(std::to_string(raw_value), STRING_SIZE);
                }
                else
                {
                    search_value = static_cast<ColumnDataType>(raw_value);
                }
                predicate = std::make_shared<BinaryPredicateExpression>(
                    PredicateCondition::LessThan, column_expression,
                    value_(search_value));
            }

            if constexpr (std::is_same_v<ColumnDataType, pmr_string>)
            {
                auto reference_scan_value =
                    pad_string(std::to_string(0), STRING_SIZE);
                reference_scan_predicate =
                    std::make_shared<BinaryPredicateExpression>(
                        PredicateCondition::GreaterThan, column_expression,
    ...
    {
        input = std::make_shared<TableScan>(table_wrapper,
                                            reference_scan_predicate);
...
{
    using EncodingAndSupportedDataTypes =
        std::pair<EncodingType, std::vector<std::string>>;
    const std::map<std::string, EncodingAndSupportedDataTypes> encoding_types{
        {"None", EncodingAndSupportedDataTypes(EncodingType::Unencoded,
                                               {"Int", "String"})},
        {"Dictionary", EncodingAndSupportedDataTypes(EncodingType::Dictionary,
                                                     {"Int", "String"})},
        {"FixedStringDictionary",
         EncodingAndSupportedDataTypes(EncodingType::FixedStringDictionary,
                                       {"String"})},
        {"FrameOfReference", EncodingAndSupportedDataTypes(
                                 EncodingType::FrameOfReference, {"Int"})},
        {"RunLength", EncodingAndSupportedDataTypes(EncodingType::RunLength,
                                                    {"Int", "String"})},
        {"LZ4",
         EncodingAndSupportedDataTypes(EncodingType::LZ4, {"Int", "String"})}};

    const std::vector<double> selectivities{0.001, 0.01, 0.1, 0.3, 0.5,
                                            0.7,   0.8,  0.9, 0.99};

    const std::map<std::string, std::function<std::shared_ptr<TableWrapper>(
                                    const EncodingType, const std::string)>>
        table_types{
            {"Int",
             [&](const EncodingType encoding_type, const std::string mode)
             {
                 return create_table<int32_t>(DataType::Int, ROWS,
                                              generate_values<int32_t>,
                                              encoding_type, mode);
             }},
            {"String",
             [&](const EncodingType encoding_type, const std::string mode)
             {
                 return create_table<pmr_string>(DataType::String, ROWS,
                                                 generate_values<pmr_string>,
                                                 encoding_type, mode);
             }}};

    const std::vector<std::string> modes{"Sorted", "SortedDescending",
                                         "SortedNotMarked", "Shuffled"};
            ...
            for (const auto &data_type : supported_data_types)
            {
                for (const auto &mode : modes)
                {
                    for (const auto is_reference_scan : {false, true})
                    {
                        ...
                        {
                            const auto &table_generator =
                                table_types.at(data_type);

                            const std::string between_label =
                                is_between_scan ? "Between" : "";
                            const std::string ref_scan_label =
                                is_reference_scan ? "ReferenceTableScan"
                                                  : "DataTableScan";
                            benchmark::RegisterBenchmark(
                                ("BM_Table" + between_label + "ScanSorted/" +
                                 ref_scan_label + "/" + encoding_name + "/" +
                                 std::to_string(selectivity) + "/" + data_type +
                                 "/" + mode)
                                    .c_str(),
                                BM_TableScanSorted, ROWS, selectivity,
                                encoding_type, mode, is_between_scan,
...
// We need to call the registerTableScanSortedBenchmarks() to register the
// benchmarks. We could call it inside the micro_benchmark_main.cpp::main()
// method, but then these benchmarks would also be included when building the
// hyriseBenchmarkPlayground. Instead, we create a global object whose sole
// purpose is to register the benchmarks in its constructor.
class StartUp
{
  public:
