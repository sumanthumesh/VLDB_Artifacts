...
generate_custom_table(const size_t row_count,
                      ...
                      const float null_ratio = 0.0f)
{
    ...
    const auto column_data_types =
        std::vector<DataType>{NUM_COLUMNS, data_type};

    auto column_specifications = std::vector<ColumnSpecification>(
        NUM_COLUMNS,
        ColumnSpecification(
            ColumnDataDistribution::make_uniform_config(0.0, LARGEST_VALUE),
            data_type, SegmentEncodingSpec{EncodingType::Unencoded},
            std::nullopt, null_ratio));
...
static void BM_Sort(benchmark::State &state, const size_t row_count = 40'000,
                    const DataType data_type = DataType::Int,
                    const float null_ratio = 0.0f,
                    const bool multi_column_sort = true,
                    const bool use_reference_segment = false)
{
    micro_benchmark_clear_cache();

    const auto input_table =
        generate_custom_table(row_count, data_type, null_ratio);
    std::shared_ptr<AbstractOperator> input_operator =
        std::make_shared<TableWrapper>(input_table);
    input_operator->never_clear_output();
    input_operator->execute();
    if (use_reference_segment)
    {
        input_operator = std::make_shared<Limit>(
            input_operator, expression_functional::to_expression(
...
BENCHMARK(BM_SortWithReferenceSegments)
    ->RangeMultiplier(100)
    ->Range(100, 1'000'000);
BENCHMARK(BM_SortWithReferenceSegmentsTwoColumns)
    ->RangeMultiplier(100)
    ->Range(100, 1'000'000);
