...
class SQLBenchmark : public MicroBenchmarkBasicFixture
{
  public:
    void SetUp(benchmark::State &st) override
    {
        ...
        // This is required for the translator to get the column names of a
        // table.
        auto &storage_manager = Hyrise::get().storage_manager;
        storage_manager.add_table(
            "customer",
            load_table("resources/test_data/tbl/tpch/minimal/customer.tbl"));
        storage_manager.add_table(
            "lineitem",
            load_table("resources/test_data/tbl/tpch/minimal/lineitem.tbl"));
        storage_manager.add_table(
            "orders",
            load_table("resources/test_data/tbl/tpch/minimal/orders.tbl"));
    }
    ...
    void BM_CompileQuery(benchmark::State &state)
    {
        for (auto _ : state)
        {
            ...
            auto result_node = SQLTranslator{UseMvcc::No}
                                   .translate_parser_result(result)
                                   .lqp_nodes.at(0);
    ...
    // Run a benchmark that plans the query operator with the given query with
    ...
    void BM_QueryPlanCache(benchmark::State &state)
    {
        ...
        for (auto _ : state)
        {
            auto pipeline_statement = SQLPipelineBuilder{query}
                                          .with_pqp_cache(pqp_cache)
                                          .create_pipeline();
