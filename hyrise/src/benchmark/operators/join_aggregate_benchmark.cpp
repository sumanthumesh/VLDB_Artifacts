...
pmr_vector<int32_t> generate_ages(const size_t table_size)
{
    ...
    // The result ages are always the same in each table because of the constant
    // seed
...
std::shared_ptr<Table> create_table(const size_t table_size,
                                    const pmr_vector<int32_t> &values)
{
    const auto chunk_size =
        static_cast<ChunkOffset>(table_size / NUMBER_OF_CHUNKS_JOIN_AGGREGATE);
    ...
    auto table = std::make_shared<Table>(table_column_definitions,
                                         TableType::Data, chunk_size);

    for (auto chunk_index = size_t{0};
         chunk_index < NUMBER_OF_CHUNKS_JOIN_AGGREGATE; ++chunk_index)
    {
        const auto ids_value_segment =
            std::make_shared<ValueSegment<int32_t>>(pmr_vector<int32_t>(
                ids_vector.begin() + (chunk_index * chunk_size),
                ids_vector.begin() + ((chunk_index + 1) * chunk_size)));
        const auto value_segment =
            std::make_shared<ValueSegment<int32_t>>(pmr_vector<int32_t>(
                values.cbegin() + (chunk_index * chunk_size),
                values.cbegin() + ((chunk_index + 1) * chunk_size)));
...
std::shared_ptr<TableWrapper> create_zip_table(const size_t table_size)
{
    ...
    for (auto chunk_index = ChunkID{0}; chunk_index < chunk_count;
         ++chunk_index)
    {
...
void BM_Join_Aggregate(benchmark::State &state)
{
    ...
    auto operator_join_predicate = OperatorJoinPredicate(
    ...
    auto aggregates = std::vector<std::shared_ptr<WindowFunctionExpression>>{
        std::static_pointer_cast<WindowFunctionExpression>(
            avg_(pqp_column_(ColumnID{0}, DataType::Int, false, "b")))};

    const auto groupby = std::vector<ColumnID>{ColumnID{0}, ColumnID{2}};

    auto warmup_join =
        std::make_shared<JoinType>(table_wrapper_left, table_wrapper_right,
                                   JoinMode::Inner, operator_join_predicate);
    warmup_join->execute();

    auto warmup_aggregate =
        std::make_shared<AggregateType>(warmup_join, aggregates, groupby);
    warmup_aggregate->execute();

    for (auto _ : state)
    {
        auto join = std::make_shared<JoinType>(
            table_wrapper_left, table_wrapper_right, JoinMode::Inner,
            operator_join_predicate);
        join->execute();
        auto aggregate =
            std::make_shared<AggregateType>(join, aggregates, groupby);
        aggregate->execute();
    }
}
