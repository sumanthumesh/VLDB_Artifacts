...
// These numbers were arbitrarily chosen to form a representative group of
// JoinBenchmarks that run in a tolerable amount of time
...
std::shared_ptr<TableWrapper> generate_table(const size_t number_of_rows)
{
    ...
    const auto chunk_size =
        static_cast<ChunkOffset>(number_of_rows / NUMBER_OF_CHUNKS);
    Assert(chunk_size > 0,
           "The chunk size is 0 or less, can not generate such a table");

    auto table = table_generator->generate_table(
        1ul, number_of_rows, chunk_size,
    ...
    for (ChunkID chunk_id{0}; chunk_id < chunk_count; ++chunk_id)
    {
        ...
        for (auto column_id = ColumnID{0}; column_id < chunk->column_count();
             ++column_id)
        {
            chunk->create_index<AdaptiveRadixTreeIndex>(
...
template <class C>
void bm_join_impl(benchmark::State &state,
                  std::shared_ptr<TableWrapper> table_wrapper_left,
                  std::shared_ptr<TableWrapper> table_wrapper_right)
{
    clear_cache();

    auto warm_up = std::make_shared<C>(
...
template <class C> void BM_Join_SmallAndSmall(benchmark::State &state)
...
template <class C> void BM_Join_SmallAndBig(benchmark::State &state)
...
template <class C> void BM_Join_MediumAndMedium(benchmark::State &state)
